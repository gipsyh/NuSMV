...
void Compass_check_prob_list(TypeChecker_ptr tc, NodeList_ptr list)
{
	...
	{
		ProbAssign_ptr ta =
			PROB_ASSIGN(NodeList_get_elem_at(list, iter));
		...
		if (!TypeChecker_is_expression_wellformed(tc, asgns, Nil)) {
			const NuSMVEnv_ptr env =
				EnvObject_get_environment(ENV_OBJECT(tc));
			const ErrorMgr_ptr errmgr = ERROR_MGR(
				NuSMVEnv_get_value(env, ENV_ERROR_MANAGER));
			ErrorMgr_rpterr(
				errmgr,
				"An error was found in probabilistic list\n");
		}
	}
}
...
/* #define DEBUG_PROB */
	...
	NODE_LIST_FOREACH(list, iter)
	{
       ...
       are no incomplete assignments */
		...
		asgns_prob_add =
			add_apply(dd, &compass_add_mul, asgns_add, val_add);
		tmp_add = add_apply(dd, &compass_add_sum, res_add,
				    asgns_prob_add);
