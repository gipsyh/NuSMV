...
)
{
	...
	{ /* to control dynamic reordering */
		...
		{ /* calculates the transition relation */
			bdd_ptr bdd_sinvar = BddFsm_get_state_constraints(fsm);
			bdd_ptr bdd_nsinvar =
				BddEnc_state_var_to_next_state_var(enc,
								   bdd_sinvar);
			bdd_ptr bdd_iinvar = BddFsm_get_input_constraints(fsm);
			...
			bdd_and_accumulate(dd, &bdd_trans,
					   next_and_curr_state_mask);
...
/* #define DEBUG_TRANS */
       ...
       before entering this block */
		...
		if (probs_list != NODE_LIST(NULL)) {
			prob_add = Compass_process_prob_list(enc, probs_list,
							     bdd_trans);
		} else
       ...
       * var do not work */
		...
		if (NODE_LIST(NULL) != ap_list) {
			...
			{
				node_ptr ap_el = (node_ptr)NodeList_get_elem_at(
					ap_list, iter);
				...
				expr_add_mask =
					BddEnc_apply_state_frozen_vars_mask_add(
						enc, expr_add);
				NodeList_append(ap_list_add,
						cons(nodemgr, lab,
						     (node_ptr)expr_add_mask));
		...
		/* dump the whole fsm */
		compass_write_sigref_adds(sigref_file, enc, init_add, trans_add,
					  prob_add, tau_add, ap_list_add,
					  do_indent);
...
int Compass_write_language_sigref(BddEnc_ptr enc, FILE *file)
{
	...
	static const char *IVAR_FM =
		"<var index=\"%d\" name=\"%s\" type=\"%s\"/>\n";
	...
	{ /* loop over variables */
		...
		NodeList_ptr layers =
			BaseEnc_get_committed_layers(BASE_ENC(enc));
		ListIter_ptr lay_it;
		NODE_LIST_FOREACH(layers, lay_it)
		{
			SymbLayer_ptr lay = SYMB_LAYER(
				NodeList_get_elem_at(layers, lay_it));
			SymbLayerIter lay_it;

			SYMB_LAYER_FOREACH_FILTER(
				lay, lay_it, STT_VAR,
				SymbLayer_iter_filter_bool_vars, NULL)
			{
				node_ptr cvar =
					SymbLayer_iter_get_symbol(lay, &lay_it);

				boolean is_ivar =
					SymbTable_is_symbol_input_var(st, cvar);
				int cidx = BddEnc_get_var_index_from_name(enc,
									  cvar);
				char *cvarn = sprint_node(wffprint, cvar);

				if (is_ivar) { /* input variable */
					fprintf(file, IVAR_FM, cidx, cvarn,
						IVAR_TYPE);
				} else { /* state variables */
					node_ptr nvar =
						ExprMgr_next(exprs, cvar, st);
					int nidx =
						BddEnc_get_var_index_from_name(
							enc, nvar);
					char *nvarn =
						sprint_node(wffprint, nvar);
					fprintf(file, SVAR_FM, cidx, cvarn,
						SVAR_TYPE, nidx);
					fprintf(file, SVAR_FM, nidx, nvarn,
						NVAR_TYPE, cidx);
					FREE(nvarn);
				}
				FREE(cvarn);
				    ...
				    boolean do_indent)
{
	...
	res = compass_print_add_sigref_format(dd, add, file, 1, hash,
					      do_indent);
	free_assoc(hash);
	return res;
}
					  ...
					  hash_ptr hash, boolean do_indent)
{
	...
	if (add_isleaf(add)) {
		...
		if (add_is_false(dd, add))
			...
			val = sprint_node(wffprint,
					  (node_ptr)add_get_leaf(dd, add));
		...
		if (find_assoc(hash, (node_ptr)add) == NULL) {
			...
			if (res < 0)
				return -1;
			res = compass_print_add_sigref_format(
				dd, add, file, indent + 1, hash, do_indent);
			if (res < 0)
				return -1;
			_INDENT(file, indent, do_indent);
			res = fprintf(file, "</%s>\n", child_tag);
			...
			insert_assoc(hash, (node_ptr)add,
					   ...
					   hash_ptr hash, boolean do_indent)
{
	...
	if (add_isleaf(add)) {
		...
		res = compass_print_add_child_sigref(dd, add, file, indent,
						     TAG_NODE, hash, do_indent);
		if (res < 0)
			return -1;
	} else {
		...
		res = compass_print_add_child_sigref(
			dd, _then, file, indent + 1, TAG_THEN, hash, do_indent);
		if (res < 0)
			return -1;
		res = compass_print_add_child_sigref(
			dd, _else, file, indent + 1, TAG_ELSE, hash, do_indent);
		if (res < 0)
			return -1;
		_INDENT(file, indent, do_indent);
		res = fprintf(file, "</%s>\n", TAG_NODE);
...
)
{
	...
	if (prob != (add_ptr)NULL) {
		...
		StreamMgr_print_output(streams, "==================\n");
#endif
		compass_sigref_dump_dd(file, dd, prob, SIGREF_PROB_MAP_LBL,
				       NULL, do_indent);
	}

	if (tau != (add_ptr)NULL) {
		fprintf(file, "\n");
		compass_sigref_dump_dd(file, dd, tau, SIGREF_TAU_LBL, NULL,
				       do_indent);
	}
	...
	if (NODE_LIST(NULL) != ap_list) {
		...
		{
			node_ptr ap_el =
				(node_ptr)NodeList_get_elem_at(ap_list, iter);
			const char *ap_lab = UStringMgr_get_string_text(
				(string_ptr)car(car(ap_el)));
			add_ptr ap_bdd = (add_ptr)cdr(ap_el);

			nusmv_assert(node_get_type(car(ap_el)) == ATOM);

			fprintf(file, "\n");
			compass_sigref_dump_dd(file, dd, ap_bdd, SIGREF_AP_LBL,
					       ap_lab, do_indent);
		}
	}
...
static void print_trans(BddEnc_ptr enc, bdd_ptr bdd_to_print, FILE *f)
{
	...
	do {
		...
		node_ptr varValueList = BddEnc_assign_symbols(
			enc, bdd, vars_list, true, &bdd_of_assigns);
