...
static void set_union_to_set_aux(NodeMgr_ptr nodemgr, node_ptr a, Set_t *set);
...
Set_t Set_MakeEmpty(void) { return (Set_t)NULL; }
...
Set_t Set_Copy(const Set_t set) { return set_copy(set); }

void Set_ReleaseSet(Set_t set) { set_destroy(set); }
...
Set_t Set_AddMember(Set_t set, Set_Element_t el) {
  if (set == NULL)
    set = Set_MakeSingleton(el);
  else {
    if (Set_IsMember(set, el))
      return set;
    /*
        node_ptr key = find_node(nodemgr, COLON, (node_ptr) __func__,
       find_node(nodemgr, COLON, (node_ptr) set, (node_ptr) el)); Set_t res =
       lookup_op_hash(key); if (res != NULL) return res;
    */
...
boolean Set_Contains(const Set_t set1, const Set_t set2) {
  ...
  if (Set_IsEmpty(set2) || (set1 == set2))
    return true;
  if (Set_GiveCardinality(set1) < Set_GiveCardinality(set2))
    return false;
  SET_FOREACH(set2, iter) {
    if (!Set_IsMember(set1, Set_GetMember(set2, iter)))
      return false;
  }
...
boolean Set_Intersects(const Set_t set1, const Set_t set2) {
  ...
  SET_FOREACH(set1, iter) {
    if (Set_IsMember(set2, Set_GetMember(set1, iter)))
      return true;
  }
...
Set_t Set_Union(Set_t set1, const Set_t set2) {
  if (Set_IsEmpty(set1))
    return Set_Copy(set2);
  if (Set_IsEmpty(set2))
    return set1;
  if (Set_Contains(set1, set2))
    return set1;
...
Set_Iterator_t Set_GetFirstIter(Set_t set1) {
  if (Set_IsEmpty(set1))
    return (Set_Iterator_t)ListIter_get_end();
...
void Set_PrintSet(MasterPrinter_ptr wffprint, FILE *file, const Set_t s,
  ...
  while (keep) {
    if (printer != NULL)
      printer(file, Set_GetMember(s, iter), printer_arg);
    else
      print_node(wffprint, file, (node_ptr)Set_GetMember(s, iter));
...
boolean Set_IsFrozen(Set_t const set) { return set_is_frozen(set); }
...
*/
static Set_t set_check_frozen(Set_t self) {
  if (self == NULL || self->fam == (int *)NULL)
    return self;
...
static void set_union_to_set_aux(NodeMgr_ptr nodemgr, node_ptr a, Set_t *set) {
  ...
  if (TWODOTS == node_get_type(a)) {
    ...
    for (first = node_get_int(car(a)), last = node_get_int(cdr(a));
         first <= last; first++) {
      *set = Set_AddMember(
          *set, find_node(nodemgr, NUMBER, NODE_FROM_INT(first), Nil));
    }
  }
