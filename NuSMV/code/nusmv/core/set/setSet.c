...
node_ptr Set_Set2Union(const Set_t set, NodeMgr_ptr nodemgr)
{
	...
	SET_FOREACH(set, s_iter)
	{
		...
		res = (Nil == res) ? el :
				     NodeMgr_find_node(nodemgr, UNION, el, res);
	}
...
Set_t Set_AddMembersFromList(Set_t set, const NodeList_ptr list)
{
	...
	{
		set = Set_AddMember(
			set, (Set_Element_t)NodeList_get_elem_at(list, iter));
	}
	return set;
}
...
Set_t Set_Difference(Set_t set1, const Set_t set2)
{
	...
	set1 = set_check_frozen(set1);
	NodeList_remove_elems(set1->list, set2->list, NULL /* no disposal */,
			      NULL);
		  ...
		  void *printer_arg)
{
	...
	while (keep) {
		if (printer != NULL)
			...
			print_node(wffprint, file,
				   (node_ptr)Set_GetMember(s, iter));
...
static void set_union_to_set_aux(NodeMgr_ptr nodemgr, node_ptr a, Set_t *set)
{
	...
	if (TWODOTS == node_get_type(a)) {
		...
		for (first = node_get_int(car(a)), last = node_get_int(cdr(a));
		     first <= last; first++) {
			*set = Set_AddMember(
				*set, find_node(nodemgr, NUMBER,
						NODE_FROM_INT(first), Nil));
		}
	}
