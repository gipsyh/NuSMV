...
Pair_ptr Wff_Rewrite_rewrite_formula(
	...
	FlatHierarchy_ptr outfh, const node_ptr spec, const short int spec_type)
{
	return Wff_Rewrite_rewrite_formula_generic(env, method, eproptype,
						   layer, outfh, spec,
						   spec_type, true, false);
}
	...
	if (_wff_rewrite_is_rewriting_needed(st, spec, Nil, eproptype)) {
		...
		case WFF_REWRITER_REWRITE_INPUT_NEXT:
			retval = _wff_rewrite_generalized_property(
				env, method, eproptype, layer, outfh, spec,
		...
		retval = Pair_create(new_node(nodemgr, node_get_type(spec),
					      car(spec), cdr(spec)),
					...
					node_ptr context)
{
	...
	cone = Formula_GetDependenciesByType(st, wff, Nil, VFT_NEXT | VFT_INPUT,
					     true);
...
static Pair_ptr
_wff_rewrite_extract_next_input_predicates(WffRewriter *rewriter, node_ptr spec)
{
	NodeMgr_ptr const nodemgr =
						...
						node_ptr *expr)
{
	...
	OptsHandler_ptr const opts = OPTS_HANDLER(
		NuSMVEnv_get_value(rewriter->env, ENV_OPTS_HANDLER));
	Logger_ptr const logger =
		LOGGER(NuSMVEnv_get_value(rewriter->env, ENV_LOGGER));
	MasterPrinter_ptr const wffprint = MASTER_PRINTER(
		NuSMVEnv_get_value(rewriter->env, ENV_WFF_PRINTER));
	SymbTable_ptr symb_table = NULL;
...
#ifdef DEBUG_WFF_REWRITE
	{
		Logger_ptr const logger =
			LOGGER(NuSMVEnv_get_value(rewriter->env, ENV_LOGGER));
		MasterPrinter_ptr const sexpprint = MASTER_PRINTER(
			NuSMVEnv_get_value(rewriter->env, ENV_SEXP_PRINTER));
		OptsHandler_ptr const opts = OPTS_HANDLER(
			NuSMVEnv_get_value(rewriter->env, ENV_OPTS_HANDLER));
	...
	if (Nil == *expr) {
		...
		case CONTEXT:
			...
			*expr = new_node(nodemgr, node_get_type(*expr),
					 car(*expr), expr2);
			retval = kind2;
			break;
		...
		case FLOOR:
			...
			*expr = new_node(nodemgr, node_get_type(*expr), expr1,
					 Nil);
			retval = kind1;
			break;
		...
		case XNOR: {
			...
			if ((WFF_REWRITE_FORMULA_KIND_INPUT_NEXT & kind1) &&
				...
				expr1 = _wff_rewrite_create_substitution(
					rewriter, expr1);
				kind1 = WFF_REWRITE_FORMULA_KIND_STATE;
			} else if ((WFF_REWRITE_FORMULA_KIND_INPUT_NEXT &
				    kind2) &&
				   WFF_REWRITE_FORMULA_KIND_TEMP == kind1) {
				expr2 = _wff_rewrite_create_substitution(
					rewriter, expr2);
		...
		default: {
			StreamMgr_ptr const streams = STREAM_MGR(
				NuSMVEnv_get_value(rewriter->env,
						   ENV_STREAM_MANAGER));
...
static void _wff_rewrite_fill_layer_and_hierarchy(WffRewriter *rewriter)
{
	...
	NODE_LIST_FOREACH(rewriter->new_var_exprs, iter)
	{
		...
		Pair_ptr tmp = PAIR(
			NodeList_get_elem_at(rewriter->new_var_exprs, iter));
		...
		bool_type =
			SymbType_create(rewriter->env, SYMB_TYPE_BOOLEAN, Nil);
		...
		case WFF_REWRITE_METHOD_DEADLOCK_FREE:
			if (rewriter->initialize_monitor_to_true) {
				...
				FlatHierarchy_add_init(rewriter->outfh,
						       ExprMgr_not(exprs, var));
			}
			FlatHierarchy_add_trans(
				rewriter->outfh,
				ExprMgr_iff(exprs,
					    ExprMgr_next(exprs, var,
							 symb_table),
					    expr));
			break;
	...
	const boolean initialize_monitor_to_true)
{
	...
	nusmv_assert(
		SymbTable_has_layer(symb_table, SymbLayer_get_name(layer)));
...

	{
		...
		NODE_LIST_FOREACH(rewriter.new_var_exprs, iter)
		{
			Pair_ptr elem = PAIR(NodeList_get_elem_at(
				rewriter.new_var_exprs, iter));
			Pair_destroy(elem);
		}
	}
				 ...
				 WffRewriterExpectedProperty eproptype)
{
	...
	case WFF_REWRITER_REWRITE_INPUT_NEXT: {
		...
		cone = Formula_GetDependenciesByType(
			st, wff, Nil, VFT_NEXT | VFT_INPUT, true);
		       ...
		       const boolean ltl2invar_negate_property)
{
	if (INVARSPEC == spec_type) {
		return _wff_rewrite_generalized_property(
		...
		if (WFF_REWRITER_NONE == kind ||
		    WFF_REWRITER_PROPOSITIONAL == kind) {
			/* No rewriting is needed */
			const NodeMgr_ptr nodemgr =
				NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));

			return Pair_create(new_node(nodemgr,
						    node_get_type(spec),
						    car(spec), cdr(spec)),
					   VOIDPTR_FROM_INT(spec_type));
		}
		...
		{
			node_ptr flatten_spec =
				Compile_FlattenSexpExpandDefine(st, spec, Nil);
			Pair_ptr prop_pair = _wff_invariant_get_members(
				st, flatten_spec, Nil);
			...
			if (WFF_REWRITER_PURE_INVAR == kind) {
				const ExprMgr_ptr exprs = EXPR_MGR(
					NuSMVEnv_get_value(env,
							   ENV_EXPR_MANAGER));
				node_ptr invariant =
					NODE_PTR(Pair_get_second(prop_pair));

				if (ltl2invar_negate_property) {
					invariant =
						ExprMgr_not(exprs, invariant);
				}
				nusmv_assert(Nil == NODE_PTR(Pair_get_first(
							    prop_pair)));
				retval = _wff_rewrite_generalized_property(
					env, method, eproptype, layer, outfh,
					invariant, INVARSPEC,
					initialize_monitor_to_true);
				...
				const ExprMgr_ptr exprs = EXPR_MGR(
					NuSMVEnv_get_value(env,
							   ENV_EXPR_MANAGER));
				node_ptr premises =
					NODE_PTR(Pair_get_first(prop_pair));
				node_ptr invariant =
					NODE_PTR(Pair_get_second(prop_pair));
				FlatHierarchy_ptr premises_fh =
					FlatHierarchy_create(st);
				...
				if (Wff_Rewrite_is_rewriting_needed(
					    st, premises, Nil)) {
          ...
          /* declare a new variable */
					const ExprMgr_ptr exprs =
						EXPR_MGR(NuSMVEnv_get_value(
							env, ENV_EXPR_MANAGER));
					node_ptr new_var =
						SymbTable_get_fresh_symbol_name(
							st, WFF_MONITOR_PREFIX);
					SymbType_ptr bool_type = SymbType_create(
						env, SYMB_TYPE_BOOLEAN, Nil);

					FlatHierarchy_add_var(premises_fh,
							      new_var);
					SymbLayer_declare_state_var(
						layer, new_var, bool_type);
					FlatHierarchy_add_init(premises_fh,
							       new_var);
					FlatHierarchy_add_trans(
						premises_fh,
						ExprMgr_and(
           ...
           * an INIT */
				...
				if (Wff_Rewrite_is_rewriting_needed(
					...
					const NodeMgr_ptr nodemgr =
						NODE_MGR(NuSMVEnv_get_value(
							env, ENV_NODE_MGR));
					retval = Pair_create(
						invariant,
						VOIDPTR_FROM_INT(INVARSPEC));
				}
				...
				if (!Set_IsEmpty(premises_cone)) {
					...
					FlatHierarchy_add_trans(premises_fh,
								premises);
				} else {
             ...
             within the transition relation. */
					FlatHierarchy_add_invar(premises_fh,
								premises);
				}
				if (Wff_Rewrite_is_rewriting_needed(
					    st, invariant, Nil)) {
    ...
    */
		if (WFF_REWRITER_NONE == outer) {
			...
			if (NOT == node_get_type(car(spec))) {
				left = _wff_invariant_kind(st, car(car(spec)),
							   context, outer);
				if (WFF_REWRITER_PROPOSITIONAL == left ||
				    WFF_REWRITER_PURE_INVAR == left) {
					right = _wff_invariant_kind(
						st, cdr(spec), context, outer);

					if (WFF_REWRITER_PROPOSITIONAL ==
						    left &&
					    WFF_REWRITER_PROPOSITIONAL == right)
						return WFF_REWRITER_PROPOSITIONAL;
				...
				right = _wff_invariant_kind(st, car(cdr(spec)),
							    context, outer);
				if (WFF_REWRITER_PROPOSITIONAL == right ||
				    WFF_REWRITER_PURE_INVAR == right) {
					left = _wff_invariant_kind(
						st, car(spec), context, outer);
					   ...
					   node_ptr spec, node_ptr context)
{
	...
	case CONTEXT:
		error_unreachable_code_msg(
			"Expression expected to be already flattened");
		return NULL;
	...
	case AND: {
		...
		const NodeMgr_ptr nodemgr =
			NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
		Pair_ptr left =
			_wff_invariant_get_members(st, car(spec), context);
		Pair_ptr right =
			_wff_invariant_get_members(st, cdr(spec), context);
		Pair_ptr result = Pair_create(
			Nil,
			new_node(nodemgr, AND, NODE_PTR(Pair_get_second(left)),
	...
	case OR: { /* this was added to fix issue 5178 */
       ...
       (!p | G q), (!G p | G q), (G q | !p), (G q | !G p) */
		if (NOT == node_get_type(car(spec))) {
			left = _wff_invariant_get_members(st, car(car(spec)),
							  context);
			right = _wff_invariant_get_members(st, cdr(spec),
							   context);
		} else {
			nusmv_assert(NOT == node_get_type(cdr(spec)));
			/* here cdr for left and car for right are used on purpose */
			left = _wff_invariant_get_members(st, car(cdr(spec)),
							  context);
			right = _wff_invariant_get_members(st, car(spec),
							   context);
		}
