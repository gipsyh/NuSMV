...
{
	return expr == self->expr_true ||
	       TRUEEXP == node_get_type(NODE_PTR(expr));
}

boolean ExprMgr_is_false(const ExprMgr_ptr self, const Expr_ptr expr)
{
	return expr == self->expr_false ||
	       FALSEEXP == node_get_type(NODE_PTR(expr));
}

Expr_ptr ExprMgr_boolean_range(const ExprMgr_ptr self)
{
	return EXPR(FN(self, CONS, self->expr_false,
		       FN(self, CONS, self->expr_true, Nil)));
}
...
boolean ExprMgr_is_number(const ExprMgr_ptr self, const Expr_ptr expr,
			  const int value)
{
	return (NUMBER == node_get_type(expr)) &&
	       (NODE_TO_INT(car(expr)) == value);
}
				...
				const int type)
{
	if (type == NUMBER_UNSIGNED_WORD) {
		return EXPR(FN(self, type,
			       NODE_PTR(WordNumberMgr_max_unsigned_value(
				       self->words, size)),
			       Nil));
	} else if (type == NUMBER_SIGNED_WORD) {
		return EXPR(FN(self, type,
			       NODE_PTR(WordNumberMgr_max_signed_value(
				       self->words, size)),
			       Nil));
	}
	nusmv_assert(false);
			       ...
			       SymbTable_ptr symb_table)
{
	...
	return ExprMgr_and_nil(self, EXPR(car(list)),
			       ExprMgr_and_from_list(self, cdr(list),
						     symb_table));
}
...
Expr_ptr ExprMgr_not(const ExprMgr_ptr self, const Expr_ptr expr)
{
...

	{
		...
		if (ta == NUMBER_UNSIGNED_WORD || ta == NUMBER_SIGNED_WORD) {
			return FN(self, ta,
				  NODE_PTR(WordNumberMgr_not(
					  self->words, WORD_NUMBER(car(expr)))),
				  Nil);
		}
	}
		     ...
		     const SymbTable_ptr symb_table)
{
	...
	if (ExprMgr_is_false(self, t)) {
		if (FAILURE == node_get_type(e)) {
			...
			SymbType_ptr et =
				TypeChecker_get_expression_type(tc, e, Nil);
			if (!SymbType_is_set(et)) {
				return ExprMgr_and(self,
						   ExprMgr_not(self, cond), e);
			}
		}
	}
  ...
  */
	{
		if ((CASE == node_get_type(e)) ||
			...
			if (_t == t) {
				return ExprMgr_ite(self,
						   ExprMgr_or(self, cond, _c),
						   t, _e, symb_table);
			}
		}
	}
		      ...
		      const SymbTable_ptr symb_table)
{
	...
	/* scalar constants */
	ta = node_get_type(a);
	if (ta == NUMBER || ta == NUMBER_UNSIGNED_WORD ||
		       ...
		       const Expr_ptr b, const SymbTable_ptr st)
{
...

	{
		...
		/* words */
		else if (NUMBER_UNSIGNED_WORD == ta ||
			 NUMBER_UNSIGNED_WORD == tb ||
			 NUMBER_SIGNED_WORD == ta || NUMBER_SIGNED_WORD == tb) {
			WordNumber_ptr va = (NUMBER_UNSIGNED_WORD == ta ||
					     NUMBER_SIGNED_WORD == ta) ?
						    WORD_NUMBER(car(a)) :
						    WORD_NUMBER(NULL);
			WordNumber_ptr vb = (NUMBER_UNSIGNED_WORD == tb ||
					     NUMBER_SIGNED_WORD == tb) ?
						    WORD_NUMBER(car(b)) :
						    WORD_NUMBER(NULL);

			if (va != WORD_NUMBER(NULL) &&
			    vb != (WORD_NUMBER(NULL)))
...

	{
		...
		if (NUMBER == ta && NUMBER == tb) {
			...
			if (va != NULL && vb != NULL) {
				nusmv_assert(
					ta ==
					tb); /* signess has to be the same by type rules */

				return (NUMBER_UNSIGNED_WORD == ta ?
						WordNumber_unsigned_less(va,
									 vb) :
						WordNumber_signed_less(va,
								       vb)) ?
					       self->expr_true :
					       self->expr_false;
			}
		      ...
		      const Expr_ptr b)
{
	...
	if (ta == NUMBER && tb == NUMBER) {
		return FN(self, NUMBER,
			  NODE_FROM_INT((node_get_int(a) + node_get_int(b))),
			  Nil);
	}
	...
	if ((ta == NUMBER_UNSIGNED_WORD && tb == NUMBER_UNSIGNED_WORD) ||
		...
		if (car(a) > car(b)) {
			return FN(self, ta,
				  (node_ptr)WordNumberMgr_plus(
					  self->words, WORD_NUMBER(car(b)),
					  WORD_NUMBER(car(a))),
				  Nil);
		}
#endif
		...
		return FN(self, ta,
			  (node_ptr)WordNumberMgr_plus(self->words,
						       WORD_NUMBER(car(a)),
						       WORD_NUMBER(car(b))),
			  Nil);
	}
		       ...
		       const Expr_ptr b)
{
	...
	if ((ta == NUMBER_UNSIGNED_WORD && tb == NUMBER_UNSIGNED_WORD) ||
	    (ta == NUMBER_SIGNED_WORD && tb == NUMBER_SIGNED_WORD)) {
		return FN(self, ta,
			  (node_ptr)WordNumberMgr_minus(self->words,
							WORD_NUMBER(car(a)),
							WORD_NUMBER(car(b))),
			  Nil);
	}
		       ...
		       const Expr_ptr b)
{
	...
	if ((ta == NUMBER_UNSIGNED_WORD && tb == NUMBER_UNSIGNED_WORD) ||
		...
		return FN(self, ta,
			  (node_ptr)WordNumberMgr_times(self->words,
							WORD_NUMBER(car(a)),
							WORD_NUMBER(car(b))),
			  Nil);
	}
			...
			const Expr_ptr b)
{
	...
	if (ta == NUMBER && tb == NUMBER) {
		...
		return FN(self, NUMBER, NODE_FROM_INT((node_get_int(a) / vb)),
			  Nil);
	}
	if (ta == NUMBER_UNSIGNED_WORD && tb == NUMBER_UNSIGNED_WORD) {
...
Expr_ptr ExprMgr_unary_minus(const ExprMgr_ptr self, const Expr_ptr a)
{
	...
	case NUMBER_SIGNED_WORD:
		return FN(self, node_get_type(a),
			  (node_ptr)WordNumberMgr_unary_minus(
				  self->words, WORD_NUMBER(car(a))),
			  Nil);
	}
...
Expr_ptr ExprMgr_array_const(const ExprMgr_ptr self, const Expr_ptr a,
			     const Expr_ptr v)
{
	return EXPR(FN(self, CONST_ARRAY, FN(self, TYPEOF, NODE_PTR(a), Nil),
		       NODE_PTR(v)));
}
				 ...
				 const Expr_ptr b)
{
	...
	if (ta == NUMBER_UNSIGNED_WORD || ta == NUMBER_SIGNED_WORD) {
		...
		if (bits > 0) {
			...
			return FN(self, ta,
				  NODE_PTR(WordNumberMgr_left_shift(
					  self->words, WORD_NUMBER(car(a)),
					  bits)),
				  Nil);
		}
				  ...
				  const Expr_ptr b)
{
	...
	if (ta == NUMBER_UNSIGNED_WORD || ta == NUMBER_SIGNED_WORD) {
		...
		if (bits > 0) {
			...
			if (ta == NUMBER_UNSIGNED_WORD) {
				rs = WordNumberMgr_unsigned_right_shift(
					self->words, WORD_NUMBER(car(a)), bits);
			} else {
				rs = WordNumberMgr_signed_right_shift(
					self->words, WORD_NUMBER(car(a)), bits);
			}
	...
	if (ta == NUMBER_UNSIGNED_WORD || ta == NUMBER_SIGNED_WORD) {
		...
		if (bits > 0) {
			if (bits > WordNumber_get_width(WORD_NUMBER(car(a)))) {
				ErrorMgr_error_wrong_word_operand(
					self->errors,
					"Right operand of rotate is out of range",
					b);
			}
			return FN(self, ta,
				  NODE_PTR(WordNumberMgr_left_rotate(
					  self->words, WORD_NUMBER(car(a)),
					  bits)),
				  Nil);
		}
				   ...
				   const Expr_ptr b)
{
	...
	if (ta == NUMBER_UNSIGNED_WORD || ta == NUMBER_SIGNED_WORD) {
		...
		if (bits > 0) {
			...
			return FN(self, ta,
				  NODE_PTR(WordNumberMgr_right_rotate(
					  self->words, WORD_NUMBER(car(a)),
					  bits)),
				  Nil);
		}
				 ...
				 const Expr_ptr r)
{
	...
	if (/* Simplification can be done iff the range is constant. If the
		...
		return EXPR(node_word_selection(
			w, r, EnvObject_get_environment(ENV_OBJECT(self))));
	}
	...
	if (SYMB_TABLE(NULL) != st) {
		...
		if (SymbType_is_unsigned_word(wt)) {
			...
			if (EXTEND == node_get_type(w)) {
				Expr_ptr _w = car(w);
				SymbType_ptr _wt =
					TypeChecker_get_expression_type(tc, _w,
									Nil);

				int orig_width = SymbType_get_word_width(_wt);
				nusmv_assert(0 < orig_width &&
					     argt_width >= orig_width);

				{
					Expr_ptr res = Nil;
					int pivot =
						orig_width; /* starting bit position for '0' padding */
             ...
             EXTEND operation */
						...
						res = ExprMgr_simplify_word_bit_select(
							self, st, _w, r);
					}
				      ...
				      Expr_ptr i)
{
	...
	if (Nil != _i && NUMBER == node_get_type(_i) &&
		...
		if (m == n) {
			...
			if (NUMBER_UNSIGNED_WORD ==
			    node_get_type(w)) { /* unsigned */
				...
				return ExprMgr_word_bit_select(
					...
					FN(self, COLON,
					   ...
					   FN(self, NUMBER, NODE_FROM_INT(0),
					      Nil)));
			} else { /* signed */
				node_ptr msb_sel, rightmost_sel, nexpr;
				nusmv_assert(NUMBER_SIGNED_WORD ==
					     node_get_type(w));

				msb_sel = FN(self, COLON,
					     FN(self, NUMBER,
				...
				nexpr = ExprMgr_word_concatenate(
					self,
					ExprMgr_word_bit_select(self, w,
								msb_sel),
					ExprMgr_word_bit_select(self, w,
								rightmost_sel));

				return ExprMgr_unsigned_word_to_signed(self,
								       nexpr);
			}
		}
	}
			...
			const SymbTable_ptr symb_table)
{
	...
	/* scalar constants */
	te = node_get_type(e);
	if (te == NUMBER || te == NUMBER_UNSIGNED_WORD ||
	    te == NUMBER_SIGNED_WORD) {
		return e;
	}
		       ...
		       const Expr_ptr b)
{
	...
	{ /* checks if cardinality is 1 */
		...
		if (Set_GiveCardinality(set) == 1) {
			res = (Expr_ptr)Set_GetMember(set,
						      Set_GetFirstIter(set));
		}
		Set_ReleaseSet(set);
	}
				       ...
				       Expr_ptr arg)
{
	...
	return EXPR(FN(self, CAST_TO_UNSIGNED_WORD, NODE_PTR(arg),
		       NODE_PTR(width)));
}
...
#define _ENABLE_GLOBAL_SIMPLIFICATION_MEMOIZATION_ 1
	...
	FAIL(self->errors)
	{
		ErrorMgr_rpterr(self->errors,
				"An error occurred during Expr_simplify");
	}
			       ...
			       Expr_ptr l, Expr_ptr r)
{
	...
	if (SYMB_TABLE(NULL) != symb_table) {
		value = CompileFlatten_resolve_number(symb_table, NODE_PTR(l),
						      Nil);
		size = CompileFlatten_resolve_number(symb_table, NODE_PTR(r),
						     Nil);
	...
	if ((NUMBER == size_type || NUMBER_UNSIGNED_WORD == size_type ||
		...
		if (size_int <= 0 || size_int > WordNumberMgr_max_width()) {
			ErrorMgr_rpterr(
				self->errors,
				"size specifier is out of range [0, %i]",
       ...
       overflow. */
		if ((value_int > 0 &&
		     ((UWCONST == type &&
		       value_int >> (size_int - 1) >> 1 != 0) ||
		      (SWCONST == type &&
		       value_int >> (size_int - 2) >> 1 != 0))) ||
		...
		if (value_int >= 0) {
			value_word = WordNumberMgr_integer_to_word_number(
...
Expr_ptr ExprMgr_plus_one(const ExprMgr_ptr self, const Expr_ptr a)
{
	...
	if (ta == NUMBER) {
		return FN(self, NUMBER, NODE_FROM_INT((node_get_int(a) + 1)),
			  Nil);
	} else {
		...
		return FN(self, ta,
			  (node_ptr)WordNumberMgr_plus(
				  self->words, WORD_NUMBER(car(a)), one),
			  Nil);
	}
}
				...
				const Expr_ptr number)
{
	...
	case NUMBER_UNSIGNED_WORD: {
		...
		return WordNumber_unsigned_greater_or_equal(
			WORD_NUMBER(car(input)), value);
	}
	case NUMBER_SIGNED_WORD: {
		WordNumber_ptr value = WordNumberMgr_integer_to_word_number(
			words, node_get_int(number),
			WordNumber_get_width(WORD_NUMBER(car(input))));
		return WordNumber_signed_greater_or_equal(
			WORD_NUMBER(car(input)), value);
	}
			     ...
			     const Expr_ptr b)
{
	...
	if (ta == NUMBER) {
		...
		WordNumber_ptr converted =
			WordNumberMgr_signed_integer_to_word_number(
				words, node_get_int(b),
				WordNumber_get_width(WORD_NUMBER(car(a))));

		return ExprMgr_plus(self, a,
				    FN(self, ta, NODE_PTR(converted), Nil));
	}
}
...
boolean ExprMgr_is_syntax_correct(Expr_ptr exp, ExprKind expectedKind)
{
	...
	case CAST_TO_UNSIGNED_WORD:
		if (EXPR_CTL == expectedKind) {
			return ExprMgr_is_syntax_correct(car(exp),
							 EXPR_SIMPLE) &&
				      ...
				      boolean in_next, hash_ptr cache)
{
	...
	case ATTIME: {
		...
		if (time2 == EXPR_UNTIMED_CURRENT || time2 == curr_time) {
			res = expr_timed_to_untimed(self, st, car(expr),
						    curr_time, in_next, cache);
			      ...
			      node_ptr expr, hash_ptr cache)
{
	...
	case ATTIME: {
		...
		if (time2 == EXPR_UNTIMED_DONTCARE) {
			...
			ErrorMgr_internal_error(self->errors,
						"%s:%d:%s: Unexpected NEXT",
						__FILE__, __LINE__, __func__);
		} else { /* time2 is absolute and this is wrong */
			nusmv_assert(0 <= time2);
			ErrorMgr_internal_error(
				self->errors, "%s:%d:%s: Invalid nested ATTIME",
				__FILE__, __LINE__, __func__);
		}
	...
	default: {
		...
		/* both are DON'T CARE? */
		if ((EXPR_UNTIMED_DONTCARE == time1) &&
		    (EXPR_UNTIMED_DONTCARE == time2)) {
			res = EXPR_UNTIMED_DONTCARE;
		}
				  ...
				  Expr_ptr expr, hash_ptr hash)
{
	...
	case CASE: {
		Expr_ptr cond =
			expr_simplify_aux(self, st, car(car(expr)), hash);
		Expr_ptr _then, _else;

		if (ExprMgr_is_true(self, cond)) {
			_then = expr_simplify_aux(self, st, cdr(car(expr)),
						  hash);
			_else = cdr(expr);
	...
	case ABU:
		...
		res = ExprMgr_resolve(self, st, node_get_type(expr),
				      expr_simplify_aux(self, st, car(expr),
							hash),
				      cdr(expr));
		break;
					...
					int *max)
{
	...
	case ATTIME: {
		...
		expr_get_curr_time_interval(self, st, car(expr), cache, &min1,
					    &max1);
		nusmv_assert(min1 ==
			     max1); /* must be single time inside attime */
	...
	case NEXT: {
		...
		expr_get_curr_time_interval(self, st, car(expr), cache, &min1,
					    &max1);
		nusmv_assert(min1 ==
			     max1); /* must be single time inside next */
		...
		if (min1 == EXPR_UNTIMED_DONTCARE) {
			...
			*max = EXPR_UNTIMED_NEXT;
			...
			ErrorMgr_internal_error(
				self->errors, "%s:%d:%s: Invalid nexted ATTIME",
				__FILE__, __LINE__, __func__);
		}
	...
	default: {
		...
		expr_get_curr_time_interval(self, st, car(expr), cache, &min1,
					    &max1);
		expr_get_curr_time_interval(self, st, cdr(expr), cache, &min2,
					    &max2);

		/* both are DON'T CARE? */
		if ((EXPR_UNTIMED_DONTCARE == min1) &&
		    (EXPR_UNTIMED_DONTCARE == min2)) {
			*min = EXPR_UNTIMED_DONTCARE;
			*max = EXPR_UNTIMED_DONTCARE;
		}
	...
	nusmv_assert((*min == EXPR_UNTIMED_DONTCARE &&
		      *max == EXPR_UNTIMED_DONTCARE) ||
		     (*min != EXPR_UNTIMED_DONTCARE &&
		      *max != EXPR_UNTIMED_DONTCARE));
}
					  ...
					  boolean in_next)
{
	...
	{
		Expr_ptr left =
			move_next_to_leaves_recur(self, st, car(expr), in_next);
		Expr_ptr right =
			move_next_to_leaves_recur(self, st, cdr(expr), in_next);

		return ExprMgr_resolve(self, st, node_get_type(expr), left,
				       right);
	}
}
