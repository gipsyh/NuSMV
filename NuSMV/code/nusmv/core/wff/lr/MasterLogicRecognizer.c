...
MasterLogicRecognizer_create_with_default_recognizers(NuSMVEnv_ptr env)
{
	...
	has_been_registered = MasterNodeWalker_register_walker(
		MASTER_NODE_WALKER(self), walker);
					  ...
					  node_ptr expression, node_ptr context)
{
...
#ifndef NDEBUG
	{
		...
		nusmv_assert(TypeChecker_is_expression_wellformed(
			type_checker, expression, context));
	}
#endif
	...
	if (EXP_NONE == retval) {
		retval = master_logic_recognizer_recognize(self, expression,
							   context);
	}
					    ...
					    node_ptr context)
{
	...
	while (!ListIter_is_end(iter)) {
		LogicRecognizerBase_ptr recognizer = LOGIC_RECOGNIZER_BASE(
			NodeList_get_elem_at(MASTER_NODE_WALKER(self)->walkers,
					     iter));

		if (NodeWalker_can_handle(NODE_WALKER(recognizer),
					  expression)) {
			return LogicRecognizerBase_recognize(
				recognizer, expression, context);
		}
