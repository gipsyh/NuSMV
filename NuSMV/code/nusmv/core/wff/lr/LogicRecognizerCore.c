...
LogicRecognizerCore_ptr LogicRecognizerCore_create(NuSMVEnv_ptr env)
{
	...
	logic_recognizer_core_init(
		self, env, "Core WFF Logic Recognizer", NUSMV_EXPR_SYMBOL_FIRST,
				...
				int low, size_t num)
{
	...
	logic_recognizer_base_init(LOGIC_RECOGNIZER_BASE(self), env, name, low,
				   num);
	...
	OVERRIDE(LogicRecognizerBase, recognize) =
		logic_recognizer_core_recognize;
}
	...
	if (EXP_NONE == retval) {
		...
		case CONS:
			...
			if (NULL != cdr(exp)) {
				...
				retval = master_logic_recognizer_merge(
					master, left, right);
			} else
				retval = LR_THROW(self, car(exp), context);
		...
		case GE:
			left = LR_THROW(self, car(exp), context);
			right = LR_THROW(self, cdr(exp), context);
			retval = master_logic_recognizer_merge(master, left,
							       right);
			break;
