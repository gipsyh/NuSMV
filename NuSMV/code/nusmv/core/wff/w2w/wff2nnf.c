...
/*---------------------------------------------------------------------------*/
static void w2w_wff2nnf_hash_insert_entry(hash_ptr hash, NodeMgr_ptr nodemgr,
					  node_ptr wff, boolean polarity,
					  node_ptr nnf);

static node_ptr w2w_wff2nnf_hash_lookup_entry(hash_ptr hash,
					      NodeMgr_ptr nodemgr, node_ptr wff,
					      boolean polarity);
...
static node_ptr w2w_wff_mk_nnf(hash_ptr, NodeMgr_ptr nodemgr, node_ptr wff,
			       boolean pol);
					  ...
					  node_ptr nnf)
{
	...
	insert_assoc(wff2nnf, find_node(nodemgr, CONS, wff, (node_ptr)polarity),
		     nnf);
}
...
{
	nusmv_assert(wff2nnf != (hash_ptr)NULL);
	return find_assoc(wff2nnf,
			  find_node(nodemgr, CONS, wff, (node_ptr)polarity));
}
...
static node_ptr w2w_wff_expand_case(const NuSMVEnv_ptr env, node_ptr wff)
{
	...
	nusmv_assert(CASE == node_get_type(wff) ||
		     IFTHENELSE == node_get_type(wff));
...
static node_ptr w2w_wff_expand_case_aux(const NuSMVEnv_ptr env, node_ptr wff)
{
    ...
    */
		if (cur_cond == Wff_make_truth(nodemgr))
			return cur_rslt;
		if (cur_cond == Wff_make_falsity(nodemgr))
			return case_rest;

		res = Wff_make_or(
			nodemgr, Wff_make_and(nodemgr, cur_cond, cur_rslt),
		...
		if (FAILURE == node_get_type(wff)) {
			if (ErrorMgr_failure_get_kind(errmgr, wff) ==
			    FAILURE_CASE_NOT_EXHAUSTIVE) {
				ErrorMgr_warning_case_not_exhaustive(errmgr,
								     wff);
				/* forces a default */
				return Wff_make_truth(nodemgr);
			} else if (ErrorMgr_failure_get_kind(errmgr, wff) ==
				   FAILURE_DIV_BY_ZERO) {
				ErrorMgr_warning_possible_div_by_zero(errmgr,
								      wff);
				/* forces a default */
				return Wff_make_truth(nodemgr);
			} else if (ErrorMgr_failure_get_kind(errmgr, wff) ==
				   FAILURE_ARRAY_OUT_OF_BOUNDS) {
				ErrorMgr_warning_possible_array_out_of_bounds(
					errmgr, wff);
				/* forces a default */
				return Wff_make_truth(nodemgr);
			} else {
				ErrorMgr_report_failure_node(
					errmgr,
					wff); /* some error in the input expr */
			       ...
			       node_ptr wff, boolean pol)
{
	...
	case TRUEEXP:
		if (pol)
			res = Wff_make_truth(nodemgr);
		else
			res = Wff_make_falsity(nodemgr); /* !1 <-> 0 */
		break;

	case FALSEEXP:
		if (pol)
			res = Wff_make_falsity(nodemgr);
		else
			res = Wff_make_truth(nodemgr); /* !0 <-> 1 */
		break;
	...
	case AND:
		if (pol) {
			res = Wff_make_and(nodemgr,
					   w2w_wff_mk_nnf(wff2nnf, nodemgr,
							  car(wff), true),
					   w2w_wff_mk_nnf(wff2nnf, nodemgr,
							  cdr(wff), true));
		} else {
	...
	case OP_NEXT:
		/* !X(a) <-> X(!a) */
		res = Wff_make_opnext(nodemgr, w2w_wff_mk_nnf(wff2nnf, nodemgr,
							      car(wff), pol));
		break;
	...
	case OP_PREC:
		/* !Y(a) <-> Z(!a) */
		if (pol)
			res = Wff_make_opprec(nodemgr,
					      w2w_wff_mk_nnf(wff2nnf, nodemgr,
							     car(wff), pol));
		else
			res = Wff_make_opnotprecnot(
				nodemgr, w2w_wff_mk_nnf(wff2nnf, nodemgr,
							car(wff), pol));
		break;
	...
	case OP_GLOBAL:
		if (pol)
			res = Wff_make_globally(nodemgr,
						w2w_wff_mk_nnf(wff2nnf, nodemgr,
							       car(wff), pol));
		else {
			/* !G(a) <-> F(!a) */
			res = Wff_make_eventually(
				nodemgr, w2w_wff_mk_nnf(wff2nnf, nodemgr,
							car(wff), pol));
		}
		break;
	...
	case OP_HISTORICAL:
		if (pol)
			res = Wff_make_historically(
				nodemgr, w2w_wff_mk_nnf(wff2nnf, nodemgr,
							car(wff), pol));
		else {
			/* !H(a) <-> O(!a) */
			res = Wff_make_once(nodemgr,
					    w2w_wff_mk_nnf(wff2nnf, nodemgr,
							   car(wff), pol));
		}
		break;
	...
	case SINCE:
		if (pol)
			...
			/* !(a S b) <-> (!a T !b) */
			res = Wff_make_triggered(
				nodemgr,
				w2w_wff_mk_nnf(wff2nnf, nodemgr, car(wff), pol),
				w2w_wff_mk_nnf(wff2nnf, nodemgr, cdr(wff),
					       pol));
		}
		break;
	...
	case NUMBER_EXP: {
       ...
       hidden within DOT and ARRAY */
		ErrorMgr_internal_error(errmgr,
					"w2w_wff_mk_nnf: unexpected leaf %d\n",
					node_get_type(wff));
		res = (node_ptr)NULL;
	} break;
	...
	case MOD: {
		...
		ErrorMgr_internal_error(
			errmgr, "w2w_wff_mk_nnf: unexpected mod operator\n");
	}
	...
	default: {
		...
		ErrorMgr_internal_error(errmgr,
					"w2w_wff_mk_nnf: unexpected TOKEN %d\n",
					node_get_type(wff));
	}
	}
