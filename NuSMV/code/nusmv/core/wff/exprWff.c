...
static node_ptr expr_wff_make_binary(NodeMgr_ptr nodemgr, int type,
				     node_ptr arg1, node_ptr arg2);

static node_ptr expr_wff_make_unary(NodeMgr_ptr nodemgr, int type,
				    node_ptr arg);
...
node_ptr Wff_make_opnext_times(NodeMgr_ptr nodemgr, node_ptr arg, int x)
{
	...
	if (x == 0)
		...
		return Wff_make_opnext(
			nodemgr, Wff_make_opnext_times(nodemgr, arg, x - 1));
}
...
int Wff_get_depth(const NuSMVEnv_ptr env, node_ptr ltl_wff)
{
	...
	case IMPLIES: {
		...
		ErrorMgr_internal_error(
			errmgr, "implies should have been nnf-ef away!\n");
		break;
	}
			     ...
			     node_ptr context, boolean is_next_allowed)
{
	...
	master_recogn = MASTER_LOGIC_RECOGNIZER(
		NuSMVEnv_get_value(env, ENV_MASTER_LOGIC_RECOGNIZER));

	type_checker = SymbTable_get_type_checker(symb_table);

	if (TypeChecker_is_expression_type_checked(type_checker, wff,
						   context)) {
		symb_type = TypeChecker_get_expression_type(type_checker, wff,
							    context);
		if (!SymbType_is_boolean(symb_type))
			retval = false;
	}

	if (retval) {
		logic = MasterLogicRecognizer_recognize(master_recogn, wff,
							context);
		switch (logic) {
		case EXP_CTL:
		case EXP_LTL:
	...
	if (retval) {
		if (!TypeChecker_is_expression_type_checked(type_checker, wff,
							    context)) {
			symb_type = TypeChecker_get_expression_type(
				type_checker, wff, context);
			if (!SymbType_is_boolean(symb_type))
				retval = false;
		}
	}
...
/*!
  ...
  \se node hash may change
*/
static node_ptr expr_wff_make_unary(NodeMgr_ptr nodemgr, int type, node_ptr arg)
{
