	...
	while (new != old) {
		...
		old = bdd_dup(new);
		{
			...
			while (!FairnessListIterator_is_end(iter)) {
				...
				if (opt_verbose_level_gt(opts, 0)) {
					...
					Logger_log(
						logger,
						"evaluating justice constraint");
					Logger_log(logger, "\n");
				}
				{
					bdd_ptr result;

					bdd_and_accumulate(dd_manager, &spec,
							   new);
				...
				}
				{
					...
					nextnew =
						BddEnc_state_var_to_next_state_var(
							enc, new);
					bdd_and_accumulate(dd_manager, &R,
							   nextnew);
		...
		}
		{
			...
			while (!FairnessListIterator_is_end(iter)) {
				/* Loop over specifications */
				bdd_ptr p_spec =
					CompassionList_get_p(compassion, iter);
				bdd_ptr q_spec =
					CompassionList_get_q(compassion, iter);
				...
				if (opt_verbose_level_gt(opts, 0)) {
					...
					Logger_log(
						logger,
						"evaluating compassion constraint");
					Logger_log(logger, "\n");
				}
				{
...
node_ptr witness(BddFsm_ptr fsm, BddEnc_ptr enc, bdd_ptr feasib)
{
	...
	state = BddEnc_pick_one_state(enc, final);
	{
		...
		bdd_and_accumulate(dd_manager, &R, final);
		{
			...
			next_final =
				BddEnc_state_var_to_next_state_var(enc, final);
		...
		while (!FairnessListIterator_is_end(iter)) {
			...
			}
			{
				...
				while (!found) {
					...
					found = bdd_isnot_false(dd_manager,
								tmp);
				...
				if (!found) {
					bdd_ptr tmp = bdd_and(dd_manager, final,
							      spec);
	...
	{
		node_ptr temp;
		temp = reverse(path(enc, (bdd_ptr)last(period),
				    (bdd_ptr)last(prefix), R));
		temp = cdr(temp);
				      ...
				      node_ptr path)
{
	...
	if (p != Nil) {
		...
		result = cons(nodemgr, (node_ptr)bdd_dup((bdd_ptr)car(p)),
			      result);
	}
