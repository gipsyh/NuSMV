		      ...
		      const Ltl2SmvPrefixes *prefixes)
{
	...
	in_ltl_expr =
		transform_ltl_expression(env, in_ltl_expr, false, prefixes);
	in_ltl_expr =
		generate_smv_module(env, in_ltl_expr, single_justice, prefixes);
...
static node_ptr normalise_formula(const NuSMVEnv_ptr env, node_ptr t)
{
	...
	if (Nil == t) {
		...
		case OP_PREC: /* Y */
			left = normalise_formula(env, car(t));
			retval =
				find_node(nodemgr, node_get_type(t), left, Nil);
			break;
		...
		case SINCE: /* S */
			left = normalise_formula(env, car(t));
			right = normalise_formula(env, cdr(t));
			retval = find_node(nodemgr, node_get_type(t), left,
					   right);
			break;
		...
		case IMPLIES: /* ->  */
			left = normalise_formula(env, car(t));
			right = normalise_formula(env, cdr(t));
			retval = ExprMgr_or(exprs, ExprMgr_not(exprs, left),
					    right);
			break;
		...
		case XNOR: /* xnor */
			...
			retval = ExprMgr_or(exprs, ExprMgr_not(exprs, or1),
					    ExprMgr_not(exprs, or2));
			break;
		...
		case OP_NOTPRECNOT: /* Z  */
			left = normalise_formula(env, car(t));
			tmp = find_node(nodemgr, OP_PREC,
					ExprMgr_not(exprs, left), Nil);
			retval = ExprMgr_not(exprs, tmp);
			break;

		case OP_FUTURE: /* F */
			left = normalise_formula(env, car(t));
			retval = find_node(nodemgr, UNTIL, ExprMgr_true(exprs),
					   left);
			break;
		...
		case OP_ONCE: /* O */
			left = normalise_formula(env, car(t));
			retval = find_node(nodemgr, SINCE, ExprMgr_true(exprs),
					   left);
			break;
					 ...
					 const Ltl2SmvPrefixes *prefixes)
{
	...
	case OR:
		...
		nameLeft = transform_ltl_expression(env, car(t), polarity,
						    prefixes);
		nameRight = transform_ltl_expression(env, cdr(t), polarity,
						     prefixes);
		/* generate a new DEFINE */
		result = find_node(nodemgr, EQDEF, name,
				   ExprMgr_or(exprs, nameLeft, nameRight));
	...
	case OP_NEXT: /* X */
		...
		result = ExprMgr_equal(
			exprs, ExprMgr_next(exprs, nameLeft, SYMB_TABLE(NULL)),
	...
	case OP_PREC: /* Y */
       ...
       is applied to a different term */
		result = ExprMgr_equal(exprs, nameLeft,
				       ExprMgr_next(exprs, name,
						    SYMB_TABLE(NULL)),
				       SYMB_TABLE(NULL));
		add_to_list(trans_declarations, result);

		/* generate a new INIT */
		result = ExprMgr_equal(exprs, name, ExprMgr_false(exprs),
				       SYMB_TABLE(NULL));
	...
	case UNTIL:
		...
		nameXY = expr_to_name(env, find_node(nodemgr, OP_NEXT, t, Nil),
				      false, prefixes);
	...
	case SINCE:
		...
		nameXY = expr_to_name(env, find_node(nodemgr, OP_PREC, t, Nil),
				      false, prefixes);
		add_to_list(var_declarations, nameXY);
		...
		result = ExprMgr_equal(exprs, name,
				       ExprMgr_next(exprs, nameXY,
						    SYMB_TABLE(NULL)),
				       SYMB_TABLE(NULL));
		add_to_list(trans_declarations, result);
		/* generate a new INIT */
		result = ExprMgr_equal(exprs, nameXY, ExprMgr_false(exprs),
				       SYMB_TABLE(NULL));
		add_to_list(init_declarations, result);
		return name;
			     ...
			     boolean always, const Ltl2SmvPrefixes *prefixes)
{
	...
	case NFUNCTION:
		if (always) {
			result = generate_expr_name(env, pre_prefix,
						    prefix_name);
		} else {
			result = node;
		}
				    ...
				    const Ltl2SmvPrefixes *prefixes)
{
	...
	all_declr = cons(nodemgr,
			 new_node(nodemgr, INIT, whole_expression_name, Nil),
			 Nil);
	...
	if (single_justice) {
		...
		string_ptr name = generate_string(
			strings, "%s%u%s_one_fairness", pre_prefix,
			specificationNumber, prefix_name);
		node_ptr one_fairness_name =
			find_node(nodemgr, ATOM, (node_ptr)name, Nil);

		NODE_LIST_FOREACH(justice_declarations, iter)
		{
			node_ptr val = NodeList_get_elem_at(
				justice_declarations, iter);
			node_ptr fairness_name =
				expr_to_name(env, val, true, prefixes);
			all_declr = cons(
				nodemgr,
			...
			all_declr = cons(
				...
				new_node(
					...
					Nil),
				all_declr);
			one_fairness_expr = ExprMgr_and(
				exprs, one_fairness_expr, fairness_name);
		}
		all_declr = cons(nodemgr,
		...
		NODE_LIST_FOREACH(justice_declarations, iter)
		{
			node_ptr val = NodeList_get_elem_at(
				justice_declarations, iter);
			tmp = cons(nodemgr,
				   new_node(nodemgr, JUSTICE, val, Nil), tmp);
		}
     ...
     * Nil), all_declr); */
		...
		{
			node_ptr val =
				NodeList_get_elem_at(define_declarations, iter);
			tmp = cons(nodemgr, val, tmp);
		}
		all_declr = cons(nodemgr, new_node(nodemgr, DEFINE, tmp, Nil),
				 all_declr);
	}
	...
	/* add the VARs to the list of all declarations */
	{
		...
		{
			node_ptr val =
				NodeList_get_elem_at(var_declarations, iter);
			new_vars = cons(nodemgr,
					new_node(nodemgr, COLON, val,
						 boolean_type),
					new_vars);
		}
		all_declr = cons(nodemgr, new_node(nodemgr, VAR, new_vars, Nil),
				 all_declr);
	}

	/* create the module name */
	{
		string_ptr name = generate_string(strings, "%s%u",
						  ltl_module_base_name,
						  specificationNumber);
				   ...
				   const char *prefix_name)
{
	...
	str = generate_string(strings, "%s%u%s%d", pre_prefix,
			      specificationNumber, prefix_name,
	...
	while (Nil != iter) {
		...
		case VAR: { /* variable declarations */
			...
			if (Nil != var) {
				fprintf(ostream, "VAR\n");
				while (Nil !=
				       var) { /* iterate over variable declarations */

					nusmv_assert(CONS ==
						     node_get_type(var));
					nusmv_assert(COLON ==
						     node_get_type(car(var)));
					nusmv_assert(
						ATOM ==
						node_get_type(car(car(var))));
					nusmv_assert(
						BOOLEAN ==
						node_get_type(cdr(car(var))));
		...
		case DEFINE: { /* define declarations */
			...
			if (Nil != def) {
				...
				while (Nil !=
				       def) { /* iterate over define declarations */

					nusmv_assert(CONS ==
						     node_get_type(def));
					nusmv_assert(EQDEF ==
						     node_get_type(car(def)));
