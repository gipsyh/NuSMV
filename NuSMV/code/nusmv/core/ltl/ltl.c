...
static BddFsm_ptr ltlPropAddTableau(Ltl_StructCheckLtlSpec_ptr,
				    FlatHierarchy_ptr);
static void ltl_structcheckltlspec_remove_layer(Ltl_StructCheckLtlSpec_ptr);
...
static void ltl_structcheckltlspec_init(Ltl_StructCheckLtlSpec_ptr,
					NuSMVEnv_ptr env, Prop_ptr prop);
static void ltl_structcheckltlspec_prepare(Ltl_StructCheckLtlSpec_ptr);
static int ltl_structcheckltlspec_build_tableau_and_prop_fsm(
...
void Ltl_CheckLtlSpec(NuSMVEnv_ptr env, Prop_ptr prop)
{
	...
	if (bdd_isnot_false(cls->dd, cls->s0) && opt_counter_examples(opts)) {
       ...
       Ltl_StructCheckLtlSpec_build procedure. It must exist. */
		...
		Ltl_StructCheckLtlSpec_explain(
			cls, SexpFsm_get_symbols_list(sexp_fsm));
	}
	...
	if (elfwd_saved_options != (BddELFwdSavedOptions_ptr)NULL) {
		Bdd_elfwd_restore_options(env, BDD_ELFWD_OPT_ALL,
					  elfwd_saved_options);
	}
}
...
void Ltl_StructCheckLtlSpec_build(Ltl_StructCheckLtlSpec_ptr self)
{
	...
	if (opt_verbose_level_gt(opts, 0)) {
		...
		print_ltlspec(Logger_get_ostream(logger), self->prop,
	...
	FAIL(errmgr)
	{
		...
		StreamMgr_print_error(
			streams,
			"An error occured during tableau construction.\n");
...
void Ltl_StructCheckLtlSpec_check(Ltl_StructCheckLtlSpec_ptr self)
{
	...
	full_fairness = !FairnessList_is_empty(
		FAIRNESS_LIST(BddFsm_get_compassion(self->fsm)));
	...
	if (self->do_rewriting) {
		...
		self->prop =
			Prop_Rewriter_get_original_property(self->rewriter);
	}
}
...
void Ltl_StructCheckLtlSpec_print_result(Ltl_StructCheckLtlSpec_ptr self)
{
	const StreamMgr_ptr streams = STREAM_MGR(
		NuSMVEnv_get_value(EnvObject_get_environment(ENV_OBJECT(self)),
				   ENV_STREAM_MANAGER));
	const OptsHandler_ptr opts = OPTS_HANDLER(NuSMVEnv_get_value(
		EnvObject_get_environment(ENV_OBJECT(self)), ENV_OPTS_HANDLER));

	/* Prints out the result, if not true explain. */
	StreamMgr_print_output(streams, "-- ");
	print_spec(StreamMgr_get_output_ostream(streams), self->prop,
		   get_prop_print_method(opts));
					     ...
					     NodeList_ptr symbols)
{
	const OptsHandler_ptr opts = OPTS_HANDLER(NuSMVEnv_get_value(
		EnvObject_get_environment(ENV_OBJECT(self)), ENV_OPTS_HANDLER));
	const NodeMgr_ptr nodemgr = NODE_MGR(NuSMVEnv_get_value(
		EnvObject_get_environment(ENV_OBJECT(self)), ENV_NODE_MGR));
	...
	if (full_fairness) {
		...
		exp = reverse(explain(self->fsm, self->bdd_enc,
				      cons(nodemgr, (node_ptr)self->s0, Nil),
	...
	trace = Mc_create_trace_from_bdd_state_input_list(self->bdd_enc,
							  symbols, trace_title,
...

	{
		...
		for (l = 0; l < array_n(loops); l++) {
			...
			Trace_step_force_loopback(
				trace, Trace_ith_iter(trace, loopback));
		}
	}
				    ...
				    NodeList_ptr symbols)
{
	...
	tm = TRACE_MGR(NuSMVEnv_get_value(
		EnvObject_get_environment(ENV_OBJECT(self)), ENV_TRACE_MGR));
...
static BddFsm_ptr ltlPropAddTableau(Ltl_StructCheckLtlSpec_ptr self,
				    FlatHierarchy_ptr hierarchy)
{
	FsmBuilder_ptr builder = FSM_BUILDER(NuSMVEnv_get_value(
		EnvObject_get_environment(ENV_OBJECT(self)), ENV_FSM_BUILDER));
   ...
   */
	...
	{ /* Forces all the variable in the model to be declared also in
       ...
       cost much more then approximating */
		FlatHierarchy_ptr fh_model = SexpFsm_get_hierarchy(
			Prop_get_scalar_sexp_fsm(self->prop));
		Set_t vars = FlatHierarchy_get_vars(fh_model);
		Set_Iterator_t iter;
		SET_FOREACH(vars, iter)
		{
			FlatHierarchy_add_var(hierarchy,
					      Set_GetMember(vars, iter));
		}
	}

	/* Creation of the corresponding FSMs: */
	tableau_sexp_fsm =
		SexpFsm_create(hierarchy, FlatHierarchy_get_vars(hierarchy));

	trans_type = GenericTrans_get_type(
		GENERIC_TRANS(BddFsm_get_trans(prop_bdd_fsm)));

	tableau_bdd_fsm = FsmBuilder_create_bdd_fsm(
		builder, self->bdd_enc, tableau_sexp_fsm, trans_type);
			   ...
			   SymbLayer_ptr layer, FlatHierarchy_ptr outfh)
{
	...
	if (NuSMVEnv_has_value(env, ENV_LTL_SPEC_COUNTER)) {
		...
		ltl_spec_counter = NODE_TO_INT(NuSMVEnv_get_value(
					   env, ENV_LTL_SPEC_COUNTER)) -
				   2;
	}

	if (ExprMgr_is_true(exprs, spec))
		return; /* nothing to be done */

	module_name = ALLOC(char, module_name_len);
	if (module_name == (char *)NULL) {
		ErrorMgr_internal_error(errmgr,
					"Unable to allocate module name.");
	}

	c = snprintf(module_name, module_name_len, "%s%u", LTL_MODULE_BASE_NAME,
		     ltl_spec_counter);
	...
	modfh = Compile_FlattenHierarchy(
		env, st, layer, sym_intern(env, module_name), context,
		Nil, /* no actual */
	...
	Ltl_StructCheckLtlSpec_ptr self)
{
	...
	Ltl_spec_to_hierarchy(EnvObject_get_environment(ENV_OBJECT(self)),
			      ltl_formula, context, self->oreg2smv,
			      self->tableau_layer, hierarchy);
	...
	if (Nil != FlatHierarchy_get_ltlspec(hierarchy)) {
       ...
       */
		...
		Ltl_spec_to_hierarchy(
			EnvObject_get_environment(ENV_OBJECT(self)),
			conj_ltlspecs, Nil /* nil context */, self->ltl2smv,
			self->tableau_layer, hierarchy);
	}
...
{
	LTL_STRUCTCHECKLTLSPEC_CHECK_INSTANCE(self);
	nusmv_assert(!FairnessList_is_empty(
		FAIRNESS_LIST(BddFsm_get_compassion(self->fsm))));
...
/*!
  ...
  initialized before with Ltl_StructCheckLtlSpec_build.
*/
static void ltl_structcheckltlspec_check_el_bwd(Ltl_StructCheckLtlSpec_ptr self)
{
	const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(self));
	const StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	...
	if (opt_verbose_level_gt(opts, 2)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Prop_ptr phi = Prop_create_partial(
			EnvObject_get_environment(ENV_OBJECT(self)),
			self->spec_formula, Prop_Ctl);
		Logger_log(logger, "Checking CTL ");
		print_spec(Logger_get_ostream(logger), phi,
			   get_prop_print_method(opts));
	...
	CATCH(errmgr)
	{
		self->s0 = eval_ctl_spec(self->fsm, self->bdd_enc,
					 self->spec_formula, Nil);
	}
	FAIL(errmgr)
	{
...
/*!
  ...
  initialized before with Ltl_StructCheckLtlSpec_build.
*/
static void ltl_structcheckltlspec_check_el_fwd(Ltl_StructCheckLtlSpec_ptr self)
{
	const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(self));
	const StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	...
	nusmv_assert(Bdd_elfwd_check_options(
		...
		false));
	{
		const ErrorMgr_ptr errmgr = ERROR_MGR(NuSMVEnv_get_value(
			EnvObject_get_environment(ENV_OBJECT(self)),
			ENV_ERROR_MANAGER));
...
static void ltl_structcheckltlspec_remove_layer(Ltl_StructCheckLtlSpec_ptr self)
{
	...
	if (SymbTable_layer_class_exists(self->symb_table,
...
static bdd_ptr ltl_clean_bdd(Ltl_StructCheckLtlSpec_ptr self, bdd_ptr bdd)
{
	...
	tableau_cube = BddEnc_get_layer_vars_cube(
		self->bdd_enc, self->tableau_layer, VFT_CNIF);
					...
					NuSMVEnv_ptr env, Prop_ptr prop)
{
	...
	self->rewriter = Prop_Rewriter_create(env, prop,
...
static void ltl_structcheckltlspec_deinit(Ltl_StructCheckLtlSpec_ptr self)
{
	...
	Prop_Rewriter_destroy(self->rewriter);
	self->rewriter = NULL;
}
...
static void ltl_structcheckltlspec_prepare(Ltl_StructCheckLtlSpec_ptr self)
{
	...
	/* Prepare property's FSMs, if needed. Prop_compute_ground_bdd_fsm
     takes care of this */
	bdd_fsm = Prop_compute_ground_bdd_fsm(
		EnvObject_get_environment(ENV_OBJECT(self)), self->prop);
	...
	self->tableau_layer = SymbTable_create_layer(
		self->symb_table, NULL /* temp name */, SYMB_LAYER_POS_BOTTOM);
   ...
   * the tableau symbols from resulting ctx trace (if any). */
	...
	if ((!opt_cone_of_influence(opts)) && opt_use_reachable_states(opts) &&
