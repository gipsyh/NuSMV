...
void Ltl_CheckLtlSpec(NuSMVEnv_ptr env, Prop_ptr prop) {
  ...
  if ((Nil == FlatHierarchy_get_compassion(hierarchy)) &&
    ...
    elfwd_saved_options =
        Bdd_elfwd_check_set_and_save_options(env, BDD_ELFWD_OPT_ALL);
  }
...
void Ltl_StructCheckLtlSpec_set_ltl2smv(
    Ltl_StructCheckLtlSpec_ptr self, Ltl_StructCheckLtlSpec_ltl2smv ltl2smv) {
  LTL_STRUCTCHECKLTLSPEC_CHECK_INSTANCE(self);
...
void Ltl_StructCheckLtlSpec_build(Ltl_StructCheckLtlSpec_ptr self) {
  ...
  if (res == 1) {
    ...
    StreamMgr_print_error(
        streams, "Ltl_CheckLtlSpec: Problems in Tableau generation.\n");
    ErrorMgr_nusmv_exit(errmgr, 1);
  }
}
...
void Ltl_StructCheckLtlSpec_print_result(Ltl_StructCheckLtlSpec_ptr self) {
  const StreamMgr_ptr streams = STREAM_MGR(NuSMVEnv_get_value(
      EnvObject_get_environment(ENV_OBJECT(self)), ENV_STREAM_MANAGER));
...
Ltl_StructCheckLtlSpec_build_counter_example(Ltl_StructCheckLtlSpec_ptr self,
  ...
  trace = Mc_create_trace_from_bdd_state_input_list(
      self->bdd_enc, symbols, trace_title, TRACE_TYPE_CNTEXAMPLE, exp);
...
void Ltl_StructCheckLtlSpec_explain(Ltl_StructCheckLtlSpec_ptr self,
  ...
  (void)TraceMgr_execute_plugin(tm, TRACE_OPT(NULL), TRACE_MGR_DEFAULT_PLUGIN,
...
void Ltl_spec_to_hierarchy(NuSMVEnv_ptr env, Expr_ptr spec, node_ptr context,
  ...
  if (NuSMVEnv_has_value(env, ENV_LTL_SPEC_COUNTER)) {
    ...
    ltl_spec_counter =
        NODE_TO_INT(NuSMVEnv_get_value(env, ENV_LTL_SPEC_COUNTER)) - 2;
  }
...
ltl_structcheckltlspec_check_el_bwd(Ltl_StructCheckLtlSpec_ptr self) {
  ...
  self->spec_formula = find_node(
      nodemgr, NOT,
      find_node(nodemgr, EG, find_node(nodemgr, TRUEEXP, Nil, Nil), Nil), Nil);

  if (opt_verbose_level_gt(opts, 2)) {
    Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
    Prop_ptr phi =
        Prop_create_partial(EnvObject_get_environment(ENV_OBJECT(self)),
  ...
  CATCH(errmgr) {
    self->s0 = eval_ctl_spec(self->fsm, self->bdd_enc, self->spec_formula, Nil);
  }
  FAIL(errmgr) {
    ltl_structcheckltlspec_remove_layer(self);
    StreamMgr_print_error(
...
*/
static void
ltl_structcheckltlspec_remove_layer(Ltl_StructCheckLtlSpec_ptr self) {
  BoolEnc_ptr bool_enc;
...
static bdd_ptr ltl_clean_bdd(Ltl_StructCheckLtlSpec_ptr self, bdd_ptr bdd) {
  ...
  tableau_cube =
      BddEnc_get_layer_vars_cube(self->bdd_enc, self->tableau_layer, VFT_CNIF);
