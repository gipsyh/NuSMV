...
static void trace_xml_loader_tag_begin(TraceXmlLoader_ptr self,
...
static inline node_ptr trace_xml_loader_flatten_symbol(const NuSMVEnv_ptr env,
						       node_ptr symbol);
					 ...
					 boolean halt_on_wrong_section)
{
	...
	trace_xml_loader_init(self, xml_filename, halt_on_undefined_symbols,
			      halt_on_wrong_section);
	return self;
}
	...
	if (NIL(FILE) != stream) {
		...
		CATCH(errmgr)
		{
			...
			do {
				...
				len = fread(self->stream_buf, sizeof(char),
					    LIBXML2_BUFSIZE, stream);
				...
				CATCH(errmgr)
				{
					error_status = xmlParseChunk(
						self->parser, self->stream_buf,
						len, len == 0);
					ok = !error_status &&
					     (!self->parse_error);
				}
				...
				if (!ok) {
					xmlErrorPtr err = xmlCtxtGetLastError(
						self->parser);
				     ...
				     const NodeList_ptr symbols)
{
	...
	self->nusmv_input_file = util_strsav(get_input_file(opts));
...
static void trace_xml_loader_reset(TraceXmlLoader_ptr self)
{
	const OptsHandler_ptr opts = OPTS_HANDLER(
		NuSMVEnv_get_value(self->environment, ENV_OPTS_HANDLER));

	if (opt_verbose_level_ge(opts, 4)) {
		Logger_ptr logger = LOGGER(
			NuSMVEnv_get_value(self->environment, ENV_LOGGER));
...
static void trace_xml_loader_store_loopbacks(TraceXmlLoader_ptr self)
{
	...
	Trace_freeze(
		self->trace); /* loopbacks can be added only to frozen traces */
	NODE_LIST_FOREACH(self->loopback_states, liter)
	{
		TraceIter loop_iter = trace_ith_iter(
			self->trace, NODE_TO_INT(NodeList_get_elem_at(
					     self->loopback_states, liter)));
				       ...
				       const char *name, const char **atts)
{
	...
	case TRACE_XML_VALUE_TAG:
		...
		for (i = 0; atts[i]; i += 2) {
			if (!strncmp("variable", atts[i], 8)) {
				strncpy(self->curr_symb, atts[i + 1],
					MAX_ID_LEN);
			}
		}
		break;
	...
	case TRACE_XML_INVALID_TAG: {
		const NuSMVEnv_ptr env =
			EnvObject_get_environment(ENV_OBJECT(self->trace));
		const StreamMgr_ptr streams =
			STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));

		StreamMgr_print_error(
			streams, "Invalid TAG : <%s> Encountered in XML File\n",
			name);
	...
	default: {
		...
		ErrorMgr_internal_error(
			errmgr,
			"%s:%d:%s: trace_xml_loader_tag_begin: unknown tag '%s'\n",
	...
	if (time < self->last_time) {
		...
		StreamMgr_print_error(
			streams, "Invalid time : <%d> detected in XML File\n",
			time);
		self->parse_error = true;
	}
	...
	while (time > self->last_time) {
		++self->last_time;
		self->step =
			(TRACE_END_ITER != trace_iter_get_next(self->step)) ?
...
static void trace_xml_loader_tag_end(TraceXmlLoader_ptr self, const char *name)
{
		...
		if (!self->parse_error) { /* if already in error state do nothing. */
         ...
         (see issue #1802) */
			if (NIL(char) == strchr(self->curr_val, ',')) {
				int c = snprintf(self->curr_eq, MAX_EQ_LEN,
						 "%s = %s", self->curr_symb,
						 self->curr_val);
			...
			if (0 == Parser_ReadSimpExprFromString(
					 self->environment, self->curr_eq,
					 &parsed)) {
				const NodeMgr_ptr nodemgr = NODE_MGR(
					NuSMVEnv_get_value(self->environment,
							   ENV_NODE_MGR));

				if (0 != trace_xml_load_put_expr(
						 self, cdar(parsed))) {
					self->parse_error = true;
				}
	...
	default: {
		...
		/* unknown tag */
		ErrorMgr_internal_error(
			errmgr, "trace_xml_loader_tag_end: unknown tag '%s'\n",
			name);
	}
	} /* switch */
}
...
static inline int trace_xml_load_put_expr(TraceXmlLoader_ptr self, node_ptr eq)
{
	...
	value = MasterNormalizer_normalize_node(
		normalizer, Compile_FlattenSexp(st, cdr(eq), Nil));
	...
	if (trace_symbol_fwd_lookup(self->trace, symb, &section, &dummy)) {
		...
		case TRACE_SECTION_STATE_DEFINE:
			if (self->curr_parsing != TRACE_XML_STATE_TAG) {
				...
				trace_xml_report_invalid_assignment(
					self, symb, invalid_wrong_section);
				if (self->halt_on_wrong_section)
					return 1;
			}
			break;
		...
		{ /* time adjusting */
			...
			if (0 < time_ofs) { /* move backward */
				...
				if (TRACE_END_ITER == step) {
					step = trace_append_step(self->trace);
				}
			} else {
				step = self->step;
			} /* no time adjusting necessary */

			nusmv_assert(TRACE_END_ITER != step);
			if (!trace_step_put_value(self->trace, step, symb,
						  value)) {
		...
		trace_xml_report_invalid_assignment(self, symb,
						    invalid_undefined_symbol);
		if (self->halt_on_undefined_symbols)
			return 1;
	}
						       ...
						       node_ptr symbol)
{
	...
	if (op == DOT && car(symbol) == Nil) {
		return MasterNormalizer_normalize_node(
			normalizer, find_node(nodemgr, DOT, Nil, cdr(symbol)));
	}

	if (op == ATOM) {
		return MasterNormalizer_normalize_node(
			normalizer, find_node(nodemgr, DOT, Nil, symbol));
	}
	...
	if (op == ARRAY) {
       ...
       having NUMBER <negative_number>. Fix the tree here */
		if (UMINUS == node_get_type(cdr(symbol))) {
			symbol = find_node(
				nodemgr, op, car(symbol),
				ExprMgr_unary_minus(exprs, car(cdr(symbol))));
		}
	}

	return find_node(
		nodemgr, op, trace_xml_loader_flatten_symbol(env, car(symbol)),
						...
						node_ptr symbol, int reason)
{
	...
	const char *cat_repr =
		(trace_symbol_in_language(self->trace, symbol)) ?
			trace_symb_category_to_string(trace_symbol_get_category(
				self->trace, symbol)) :
	...
	if (Nil == find_assoc(self->all_wrong_symbols, symbol)) {
		if (invalid_wrong_section == reason) {
			StreamMgr_print_error(streams, "%s: %s '",
					      (self->halt_on_wrong_section) ?
						      "Error" :
						      "Warning",
					      cat_repr);
		} else if (invalid_undefined_symbol == reason) {
			StreamMgr_print_error(
				streams, "%s: undefined symbol '",
				(self->halt_on_undefined_symbols) ? "Error" :
								    "Warning");
		} else
			error_unreachable_code(); /* unreachable */
		...
		StreamMgr_print_error(streams,
				      "' is in section %s (time %d).\n",
				      TraceXmlTag_to_string(self->curr_parsing),
				      self->last_time);

		insert_assoc(self->all_wrong_symbols, symbol,
			     NODE_FROM_INT(true));
		StreamMgr_print_error(
			streams, "(Each symbol is reported only once)\n\n");
	} /* Nil == find_assoc */
}
...
static xmlParserCtxtPtr trace_xml_parser_create(TraceXmlLoader_ptr self)
{
	...
	parser = xmlCreatePushParserCtxt(sax, self, (char *)NULL, 0,
					 (char *)NULL);
