  ...
  if (NIL(FILE) != stream) {
    ...
    CATCH(errmgr) {
      ...
      do {
        ...
        CATCH(errmgr) {
          error_status =
              xmlParseChunk(self->parser, self->stream_buf, len, len == 0);
        ...
        if (!ok) {
          ...
          if ((xmlErrorPtr)NULL != err) {
            ...
            if (msg != (const char *)NULL) {
              StreamMgr_print_error(streams, "At line %d: %s", line, msg);
            }
          }
...
static void trace_xml_loader_reset(TraceXmlLoader_ptr self) {
  ...
  if (opt_verbose_level_ge(opts, 4)) {
    Logger_ptr logger =
        LOGGER(NuSMVEnv_get_value(self->environment, ENV_LOGGER));
...
static void trace_xml_loader_tag_begin(TraceXmlLoader_ptr self,
  ...
  case TRACE_XML_INVALID_TAG: {
    ...
    StreamMgr_print_error(streams,
                          "Invalid TAG : <%s> Encountered in XML File\n", name);
  ...
  default: {
    ...
    ErrorMgr_internal_error(
        errmgr, "%s:%d:%s: trace_xml_loader_tag_begin: unknown tag '%s'\n",
  ...
  case TRACE_XML_VALUE_TAG:
    if (!self->parse_error) { /* if already in error state do nothing. */
      ...
      if (NIL(char) == strchr(self->curr_val, ',')) {
        int c = snprintf(self->curr_eq, MAX_EQ_LEN, "%s = %s", self->curr_symb,
                         self->curr_val);
        SNPRINTF_CHECK(c, MAX_EQ_LEN);
      } else {
        int c = snprintf(self->curr_eq, MAX_EQ_LEN, "%s in { %s }",
                         self->curr_symb, self->curr_val);
        SNPRINTF_CHECK(c, MAX_EQ_LEN);
      }

      if (0 == Parser_ReadSimpExprFromString(self->environment, self->curr_eq,
                                             &parsed)) {
...
/*!
  ...
  \todo Missing description
*/
static inline int trace_xml_load_put_expr(TraceXmlLoader_ptr self,
                                          node_ptr eq) {
  NuSMVEnv_ptr env;
  SymbTable_ptr st;
  ...
  const char *cat_repr =
      (trace_symbol_in_language(self->trace, symbol))
          ? trace_symb_category_to_string(
                trace_symbol_get_category(self->trace, symbol))
  ...
  if (Nil == find_assoc(self->all_wrong_symbols, symbol)) {
    ...
    if (invalid_wrong_section == reason) {
      StreamMgr_print_error(streams, "%s: %s '",
                            (self->halt_on_wrong_section) ? "Error" : "Warning",
                            cat_repr);
...
static xmlParserCtxtPtr trace_xml_parser_create(TraceXmlLoader_ptr self) {
  ...
  sax->characters = (CharHandlerFunction_ptr)trace_xml_loader_char_handler;
