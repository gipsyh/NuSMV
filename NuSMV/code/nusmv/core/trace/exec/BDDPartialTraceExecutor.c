	...
	const NodeList_ptr language, int *n_steps)
{
	...
	trace_states = TraceUtils_fetch_as_bdd(
	...
	/* 1- Check Start State */
	{
		...
		if (!bdd_is_false(dd, source_states)) {
			...
			path = cons(nodemgr, NODE_PTR(bdd_dup(source_states)),
				    Nil);
			...
			do {
				...
				BddStatesInputsNexts
					next_combo; /* state-input-next constraints */

				BddStatesInputsNexts constraints;

				step = TraceIter_get_next(step);
				if (TRACE_END_ITER != step) {
					next_input = TraceUtils_fetch_as_bdd(
						trace, step,
					...
					constraints = bdd_and(dd, next_input,
							      next_combo);
					bdd_and_accumulate(dd, &constraints,
							   next_state);
		...
		if (success) {
			if (0 < count) {
				res = bdd_partial_trace_executor_generate(
				...
				res = bdd_partial_trace_executor_generate(
					self, source_states, Nil, 0, language,
					trace_description);
			}
	...
	const char *trace_name)
{
	...
	if (Nil != reachable) {
		...
		while (0 != length) {
			...
			/* pick source state */
			bwd_image =
				BddFsm_get_backward_image(self->fsm, target);
			intersect =
				bdd_and(dd, bwd_image, (bdd_ptr)car(reachable));
