				     ...
				     boolean use_restart)
{
	...
	OVERRIDE(PartialTraceExecutor, execute) =
		sat_partial_trace_executor_execute;
}
	...
	const NodeList_ptr language, int *n_steps)
{
	...
	if (SAT_SOLVER_UNSATISFIABLE_PROBLEM == satResult) {
		...
		be_trans = sat_partial_trace_executor_get_transition_relation(
			self->fsm);
		...
		do {
			...
			if (TRACE_END_ITER != step) {
				if (0 <
				    BaseTraceExecutor_get_verbosity(executor)) {
				...
				be_input = TraceUtils_fetch_as_be(
					trace, step, TRACE_ITER_I_SYMBOLS,
					self->enc, self->bdd_enc);
				be_comb = TraceUtils_fetch_as_be(
					trace, step, TRACE_ITER_COMBINATORIAL,
					self->enc, self->bdd_enc);
				be_next = TraceUtils_fetch_as_be(
	...
	if (success) {
		...
		res = Bmc_Utils_generate_cntexample(
			self->enc, SAT_SOLVER(solver), be_problem, count,
	...
	if (TRACE(NULL) != res) {
		if (Trace_get_length(trace) == count &&
		    partial_trace_executor_check_loopbacks(partial_executor,
							   trace, res)) {
		...
		do {
           ...
           relation (previous state has already been pushed) */
				...
				if (SAT_SOLVER_UNSATISFIABLE_PROBLEM ==
					...
					if (partial_trace_executor_is_complete_state(
						    partial_executor, trace,
						    step)) {
						if (0 <
						    BaseTraceExecutor_get_verbosity(
							    executor)) {
							fprintf(BaseTraceExecutor_get_output_stream(
									executor),
						...
						if (TRACE(NULL) ==
						    res) { /* no previous fragment exists */
							res = Bmc_Utils_generate_cntexample(
								self->enc,
								SAT_SOLVER(
									solver),
