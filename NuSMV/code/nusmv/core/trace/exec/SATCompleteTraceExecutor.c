...
/* Type declarations                                                         */
/*---------------------------------------------------------------------------*/
/* See 'SATCompleteTraceExecutor_private.h' for class 'SATCompleteTraceExecutor'
 * definition. */
				      ...
				      const BddEnc_ptr bdd_enc)
{
	...
	complete_trace_executor_init(
		COMPLETE_TRACE_EXECUTOR(self),
	...
	OVERRIDE(CompleteTraceExecutor, execute) =
		sat_complete_trace_executor_execute;
}
...
*/
static boolean sat_complete_trace_executor_execute(
	const CompleteTraceExecutor_ptr complete_executor,
	const Trace_ptr trace, int *n_steps)
{
	...
	if (!Trace_is_complete(trace, Trace_get_sf_vars(trace), false) ||
		...
		ErrorMgr_internal_error(
			errmgr,
			"%s:%d:%s: This executor does not support partial traces.",
	...
	{ /* 1- Check Start State */
		...
		be_current = TraceUtils_fetch_as_be(trace, step,
						    TRACE_ITER_SF_SYMBOLS,
						    self->enc, self->bdd_enc);

		be_problem =
			Be_And(be_mgr, be_current,
			       sat_complete_trace_executor_get_initial_state(
				       self->fsm));
	...
	if (SAT_SOLVER_UNSATISFIABLE_PROBLEM == satResult) {
		...
		bmc_add_be_into_solver_positively(
			...
			sat_complete_trace_executor_get_transition_relation(
				self->fsm),
		...
		do {
           ...
           complete trace re-execution (untimed) */
				be_ptr be_input = TraceUtils_fetch_as_be(
					trace, step, TRACE_ITER_I_SYMBOLS,
					self->enc, self->bdd_enc);
				be_ptr be_comb = TraceUtils_fetch_as_be(
					trace, step, TRACE_ITER_COMBINATORIAL,
					self->enc, self->bdd_enc);
				be_ptr be_next = TraceUtils_fetch_as_be(
					trace, step, TRACE_ITER_SF_SYMBOLS,
					self->enc, self->bdd_enc);
				...
				if (BaseTraceExecutor_get_verbosity(executor)) {
					fprintf(BaseTraceExecutor_get_output_stream(
							executor),
						"-- executing step %d ... ",
						1 + count);
					fflush(BaseTraceExecutor_get_output_stream(
						executor));
				}

				satGroup = SatIncSolver_create_group(solver);
				bmc_add_be_into_solver_positively(
					SAT_SOLVER(solver), satGroup,
					be_problem, self->enc,
					get_rbc2cnf_algorithm(opts));
				...
				if (SAT_SOLVER_UNSATISFIABLE_PROBLEM ==
				    satResult) {
					if (0 < BaseTraceExecutor_get_verbosity(
							executor)) {
						fprintf(BaseTraceExecutor_get_output_stream(
								executor),
							"failed!\n");
					...
					if (BaseTraceExecutor_get_verbosity(
						    executor)) {
						fprintf(BaseTraceExecutor_get_output_stream(
								executor),
							"ok\n");
				...
				if (0 == count) {
					fprintf(BaseTraceExecutor_get_error_stream(
							executor),
	...
	{ /* as a last check, verify looback consistency using internal
       ...
       have been performed *and* loopback data is consistent. */
		...
		if (Trace_get_length(trace) == count) {
			if (complete_trace_executor_check_loopbacks(
				    complete_executor, trace)) {
				fprintf(BaseTraceExecutor_get_output_stream(
						executor),
...
sat_complete_trace_executor_get_initial_state(BeFsm_ptr be_fsm)
{
	...
	be_ptr init = Be_And(BeEnc_get_be_manager(be_enc),
			     BeFsm_get_init(be_fsm), BeFsm_get_invar(be_fsm));
	return init;
}
						     ...
						     Be_CnfAlgorithm cnf_alg)
{
	...
	cnf = Be_ConvertToCnf(
		be_mgr, Bmc_Utils_apply_inlining4inc(be_mgr, prob), 1, cnf_alg);
