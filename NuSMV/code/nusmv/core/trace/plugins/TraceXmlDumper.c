...
int trace_xml_dumper_action(TracePlugin_ptr self)
{
	...
	while (stop_iter != step) {
		...
		TRACE_STEP_FOREACH(trace, step, combo_iter_type, iter, symb,
				   val)
		{
		...
		TRACE_STEP_FOREACH(trace, step, input_iter_type, iter, symb,
				   val)
		{
			/* skip non-visible symbols */
			if (!trace_plugin_is_visible_symbol(self, symb))
		...
		TRACE_STEP_FOREACH(trace, step, state_iter_type, iter, symb,
				   val)
		{
			/* skip non-visible symbols */
			if (!trace_plugin_is_visible_symbol(self, symb))
	...
	if (!ListIter_is_end(loops_iter)) {
		do {
			OStream_printf(out, "%d ",
				       NODE_TO_INT(NodeList_get_elem_at(
					       loops, loops_iter)));
...
void trace_xml_dumper_init(TraceXmlDumper_ptr self, boolean is_embedded)
{
	...
	OVERRIDE(TracePlugin, print_assignment) =
		trace_xml_dumper_print_assignment;
