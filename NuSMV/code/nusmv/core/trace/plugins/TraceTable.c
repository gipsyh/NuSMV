...
static int trace_table_print_column_style(const TraceTable_ptr self)
{
	...
	while (stop_iter != step) {
		...
		if (Trace_first_iter(trace) != step) {
			...
			TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_SI_DEFINES,
					      sym_iter, sym)
			{
				node_ptr val;

				/* skip non-visible symbols */
				if (!trace_plugin_is_visible_symbol(plugin,
								    sym))
					continue;
			...
			TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_I_SYMBOLS,
					      sym_iter, sym)
			{
				node_ptr val;
				/* skip non-visible symbols */
		...
		TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_SF_SYMBOLS, sym_iter,
				      sym)
		{
			node_ptr val;
