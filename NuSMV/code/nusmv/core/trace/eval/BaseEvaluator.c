...
static Expr_ptr base_evaluator_eval_recur(BaseEvaluator_ptr self,
  ...
  const NodeMgr_ptr nodemgr = NODE_MGR(NuSMVEnv_get_value(nenv, ENV_NODE_MGR));
  ...
  case WAREAD: {
...

    {
      ...
      if (SymbType_is_wordarray(at)) {
        ...
        Expr_ptr s_expr =
            ExprMgr_word_max_value(exprs, size, NUMBER_UNSIGNED_WORD);
        Expr_ptr i_expr =
            (NUMBER != node_get_type(index_expr)) ? index_expr : Nil;
        if (ExprMgr_is_true(exprs, ExprMgr_gt(exprs, i_expr, s_expr))) {
          ...
          const char *fail_msg = "Out of bound read for a word array: (size, "
                                 "index) = '(%s, %s)'\n";
          char *s1 = sprint_node(wffprint, s_expr);
          char *s2 = sprint_node(wffprint, i_expr);
          char *buf =
              ALLOC(char, 20 + strlen(fail_msg) + strlen(s1) + strlen(s2));
