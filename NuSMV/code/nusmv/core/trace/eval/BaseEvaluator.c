...
static node_ptr base_evaluator_make_failure(const NuSMVEnv_ptr env,
					  ...
					  const Expr_ptr expr, boolean in_next)
{
	...
	if ((Nil == expr) || FAILURE == node_get_type(expr) ||
	    SymbTable_is_symbol_constant(st, expr) || node_is_leaf(expr)) {
		return expr;
	}
	...
	case RROTATE: {
		...
		/* handle failure conditions */
		if (Nil != left_expr) {
			left_expr = base_evaluator_eval_recur(self, left_expr,
							      in_next);
			if (FAILURE == node_get_type(left_expr))
				return left_expr;
		}
		...
		if (Nil != right_expr) {
			right_expr = base_evaluator_eval_recur(self, right_expr,
							       in_next);
			if (FAILURE == node_get_type(right_expr))
				return right_expr;
		}

		tmp = find_node(nodemgr, node_get_type(expr), left_expr,
				right_expr);
		res = find_assoc(self->cache, tmp);
		...
		if (SETIN == node_get_type(expr)) {
         ...
         for left and right and verifying that left is contained into right */
			...
			res = Set_Contains(right, left) ? ExprMgr_true(exprs) :
							  ExprMgr_false(exprs);
			Set_ReleaseSet(left);
	...
	case BIT_SELECTION: {
		...
		if (Nil != w) {
			...
			if (FAILURE == node_get_type(w))
				return w;
		}
		tmp = find_node(nodemgr, node_get_type(expr), w,
				find_node(nodemgr, COLON, m, M));
	...
	case CAST_TO_UNSIGNED_WORD: {
		...
		if (Nil != wexpr) {
			...
			if (FAILURE == node_get_type(wexpr))
				return wexpr;
		}
		if (Nil != wsize) {
			wsize = base_evaluator_eval_recur(self, wsize, in_next);
			if (FAILURE == node_get_type(wsize))
				return wsize;
		}
	...
	case CASE: {
		...
		Expr_ptr cond_value =
			base_evaluator_eval_recur(self, cond_expr, in_next);
		if (FAILURE == node_get_type(cond_value)) {
			return cond_value;
		}

		/* condition is a predicate */
		nusmv_assert(ExprMgr_is_true(exprs, cond_value) ||
			     ExprMgr_is_false(exprs, cond_value));
		if (ExprMgr_is_true(exprs, cond_value)) {
			res = base_evaluator_eval_recur(self, cdar(expr),
							in_next);
		} else {
			res = base_evaluator_eval_recur(self, cdr(expr),
							in_next);
		}
		break;
	}
	...
	case CONST_ARRAY: {
		Expr_ptr value =
			base_evaluator_eval_recur(self, cdr(expr), in_next);
	...
	case WAREAD: {
       ...
       or unsigned word */
		index_expr =
			base_evaluator_eval_recur(self, index_expr, in_next);

		nusmv_assert(Nil != index_expr);
		if (FAILURE == node_get_type(index_expr))
			return index_expr;

		nusmv_assert(
			(INTEGER == node_get_type(index_expr)) ||
			(NUMBER_UNSIGNED_WORD == node_get_type(index_expr)));

		array_expr =
			base_evaluator_eval_recur(self, array_expr, in_next);

		if (FAILURE == node_get_type(array_expr))
			return array_expr;
...

		{
			...
			SymbType_ptr at = TypeChecker_get_expression_type(
				tc, array_expr, Nil);

			nusmv_assert(SymbType_is_wordarray(at) ||
				     SymbType_is_intarray(at));

			if (SymbType_is_wordarray(at)) {
				int size = SymbType_get_wordarray_awidth(at);
				Expr_ptr s_expr = ExprMgr_word_max_value(
					exprs, size, NUMBER_UNSIGNED_WORD);
				Expr_ptr i_expr =
					(NUMBER != node_get_type(index_expr)) ?
						index_expr :
						Nil;
				if (ExprMgr_is_true(exprs,
						    ExprMgr_gt(exprs, i_expr,
							       s_expr))) {
					...
					const MasterPrinter_ptr wffprint =
						MASTER_PRINTER(NuSMVEnv_get_value(
							nenv, ENV_WFF_PRINTER));
					const char *fail_msg =
						"Out of bound read for a word array: (size, "
						"index) = '(%s, %s)'\n";
					char *s1 =
						sprint_node(wffprint, s_expr);
					char *s2 =
						sprint_node(wffprint, i_expr);
					char *buf = ALLOC(
						char, 20 + strlen(fail_msg) +
							      strlen(s1) +
							      strlen(s2));

					sprintf(buf, fail_msg, s1, s2);
					res = ErrorMgr_failure_make(
						errmgr, buf,
...

		{
             ...
             */
               ...
               READ(array_expr, index_expr) */
					...
					const char *fail_msg =
						"Uknown expression: '%s'";
					res = base_evaluator_make_failure(
						nenv, fail_msg, array_expr);
					found = true;
				}
			}
		}
	...
	case WAWRITE: {
		...
		/* handle failure conditions */
		if (Nil != array_expr) {
			array_expr = base_evaluator_eval_recur(self, array_expr,
							       in_next);
			if (FAILURE == node_get_type(array_expr))
				return array_expr;
		}
		if (Nil != index_expr) {
			index_expr = base_evaluator_eval_recur(self, index_expr,
							       in_next);
			if (FAILURE == node_get_type(index_expr))
				return index_expr;
		}
		if (Nil != value_expr) {
			value_expr = base_evaluator_eval_recur(self, value_expr,
							       in_next);
			if (FAILURE == node_get_type(value_expr))
				return value_expr;
		}
		tmp = find_node(nodemgr, WAWRITE, array_expr,
				new_node(nodemgr, WAWRITE, index_expr,
					 value_expr));
		res = find_assoc(self->cache, tmp);
	...
	case NFUNCTION: {
		...
		while (Nil != params) {
			tmp = base_evaluator_eval_recur(self, car(params),
							in_next);
		...
		{ /* Regeneration of conslist parameters */
			...
			NODE_LIST_FOREACH(args_list, iter)
			{
				...
				params_resolved = find_node(nodemgr, CONS, arg,
							    params_resolved);
			}
		}
		...
		res = find_node(nodemgr, node_get_type(expr), fun_name,
				params_resolved);
		break;
	}
