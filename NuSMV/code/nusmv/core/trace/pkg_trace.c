			      ...
			      boolean halt_if_wrong_section)
{
	...
	TraceXmlLoader_ptr loader = TraceXmlLoader_create(
		filename, halt_if_undef, halt_if_wrong_section);
			    ...
			    int trace_no)
{
	...
	if (NULL == engine) {
		...
		executor = BASE_TRACE_EXECUTOR(
			TraceMgr_get_complete_trace_executor(trace_mgr,
							     engine));
	}
	...
	res = TraceMgr_execute_traces(trace_mgr,
				      COMPLETE_TRACE_EXECUTOR(executor),
				    ...
				    int verbosity, int trace_no)
{
	...
	res = TraceMgr_execute_partial_traces(trace_mgr,
					      PARTIAL_TRACE_EXECUTOR(executor),
					      first_trace, last_trace);
