...
int TracePkg_test_package(NuSMVEnv_ptr env, FILE *out, FILE *err)
{
	...
	trace_pkg_test_ptr tests[] = {
		trace_test_creation, trace_test_language, trace_test_population,
		trace_test_copy,     trace_test_concat,
	};
	...
	for (i = 0; i < sizeof(tests) / sizeof(tests[0]); ++i) {
		...
		/* run test */
		res = trace_test_setup(env, out, err) || (*f)(env, out, err) ||
...
static int trace_test_setup(NuSMVEnv_ptr env, FILE *out, FILE *err)
{
   ...
   */
	...
	if (0 == res) {
		...
		BoolEnc_ptr benc =
			BoolEncClient_get_bool_enc(BOOL_ENC_CLIENT(bdd_enc));
		SymbTableIter iter;
		...
		while (!SymbTable_iter_is_end(st, &iter)) {
			...
			if (BoolEnc_is_var_bit(benc, symb))
				continue;
			print_node(wffprint, out, symb);
			fprintf(out, " ");
...
static int trace_test_creation(NuSMVEnv_ptr env, FILE *out, FILE *err)
{
	...
	for (i = 0; i < 10; ++i) {
		...
		if (0 != Trace_get_length(trace))
			return 1;

		/* retrieve TRACE TYPE */
		if (TRACE_TYPE_SIMULATION != Trace_get_type(trace))
			return 1;

		/* ... and description */
		if (0 != strcmp(Trace_get_desc(trace), desc))
			return 1;
...
static int trace_test_population(NuSMVEnv_ptr env, FILE *out, FILE *err)
{
	...
	{ /* populate the trace with default values */
		...
		TRACE_FOREACH(trace, step)
		{
			...
			++i;
			TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_ALL_VARS,
					      sym_iter, var)
			{
				node_ptr val =
					Trace_step_get_value(trace, step, var);
				if (Nil == val) { /* not assigned */
					...
					if (!SymbType_is_boolean(
						...
						if (BoolEnc_is_var_bit(bool_enc,
								       var)) {
							var = BoolEnc_get_scalar_var_from_bit(
								bool_enc, var);
							bv = BitValues_create(
								bool_enc, var);
						} else
