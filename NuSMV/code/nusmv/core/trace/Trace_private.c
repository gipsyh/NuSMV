...
Trace_ptr trace_copy(Trace_ptr self, TraceIter until_here,
                     boolean is_volatile) {
  Trace_ptr res = ALLOC(Trace, 1);
  ...
  { /* phase 0: initialize instance fields */
    trace_set_language(res, trace_get_symb_table(self), trace_get_symbols(self),
                       is_volatile, self->allow_bits);
  ...
  /* phase 4: copy var and define frame data until given iterator */
  {
    ...
    while (TRACE_END_ITER != src_iter) {
      ...
      for (section = TRACE_SECTION_STATE_VAR; section < TRACE_SECTION_END;
           ++section) {
        ...
        if ((node_ptr *)(NULL) != src_base) {
          trace_copy_section_storage(
              trace_iter_get_section_base(res, dst_iter, section, true),
              src_base, trace_get_n_section_symbols(self, section));
        }
      }
...
TraceStepIter trace_step_iter(const Trace_ptr trace, const TraceIter step,
  ...
  do {
    if ((0 != (res.type & (1 << res.section))) &&
        (node_ptr *)(NULL) != trace_iter_get_section_base(res.trace, res.step,
                                                          res.section, false)) {
...
boolean trace_step_iter_fetch(TraceStepIter *iter, node_ptr *symb_name,
  ...
  while (true) {
    ...
    base = trace_iter_get_section_base(iter->trace, iter->step, iter->section,
                                       false);
  ...
  (*symb_value) = value;
  (*symb_name) =
      trace_symbol_bwd_lookup(iter->trace, iter->section, iter->cursor - 1);
...
/*!
  ...
  \todo Missing description
*/
static inline void trace_inc_length(const Trace_ptr self) { ++(self->length); }

unsigned trace_get_length(const Trace_ptr self) { return self->length; }
    ...
    NODE_LIST_FOREACH(symbols, iter) {
      ...
      { /* only valid symbols are allowed into traces */
        ...
        if (SYMBOL_INVALID == cat) {
          ErrorMgr_internal_error(
              errmgr, "%s:%d:%s: Invalid symbol detected (%s)", __FILE__,
              __LINE__, __func__, sprint_node(wffprint, sym));
        }
      }
      ...
      { /* skip variables in layers registered in ARTIFACTS_LAYERS_CLASS
        ...
        if (SymbTable_layer_class_exists(st, ARTIFACTS_LAYERS_CLASS) &&
            SymbTable_is_layer_in_class(st, layer_name,
...
int trace_get_id(const Trace_ptr self) { return self->id; }

TraceType trace_get_type(const Trace_ptr self) { return self->type; }
...
boolean trace_is_volatile(const Trace_ptr self) { return self->is_volatile; }
...
static TraceVarFrame_ptr trace_var_frame_create(Trace_ptr self) {
  ...
  trace_var_frame_set_state_base(
      res, trace_setup_section_storage(self, trace_get_n_state_vars(self)));

  trace_var_frame_set_input_base(
      res, trace_setup_section_storage(self, trace_get_n_input_vars(self)));
...
TraceIter trace_append_step(Trace_ptr self) {
  ...
  if (TRACE_VAR_FRAME(NULL) != last_frame) {
    ...
    trace_var_frame_set_bwd_define_frame(
        var_frame, trace_var_frame_get_fwd_define_frame(last_frame));
  }
...
SymbCategory trace_symbol_get_category(Trace_ptr self, node_ptr symb) {
  ...
  if (!hit) {
    ErrorMgr_internal_error(errmgr, "%s:%d:%s: Symbol not found (%s)", __FILE__,
                            __LINE__, __func__, sprint_node(wffprint, symb));
  }

  return trace_section_to_category(section);
}

boolean trace_symbol_is_assigned(Trace_ptr self, TraceIter step,
                                 node_ptr symb) {
  TraceSection section;
  unsigned offset;
  node_ptr value;
...
boolean trace_is_frozen(const Trace_ptr self) { return self->frozen; }

boolean trace_is_thawed(const Trace_ptr self) { return !self->frozen; }
...
static inline void trace_print_alloc_stats(Trace_ptr self) {
  ...
  if (opt_verbose_level_ge(opts, 4)) {
    ...
    Logger_log(logger,
               "frozen variables [ %d ] : ", trace_get_n_frozen_vars(self));
    ...
    Logger_log(logger,
               "state variables: [ %d ] : ", trace_get_n_state_vars(self));
    ...
    Logger_log(logger,
               "input variables: [ %d ] : ", trace_get_n_input_vars(self));
    ...
    Logger_log(logger,
               "state defines:  [ %d ] : ", trace_get_n_state_defines(self));
    ...
    Logger_log(logger,
               "input defines: [ %d ] : ", trace_get_n_input_defines(self));
    ...
    Logger_log(logger,
               "next defines: [ %d ] : ", trace_get_n_next_defines(self));
...
static inline node_ptr *trace_iter_get_section_base(Trace_ptr trace,
  ...
  case TRACE_SECTION_STATE_DEFINE: {
    ...
    if ((node_ptr *)(NULL) == res && create) {
      res =
          trace_setup_section_storage(trace, trace_get_n_state_defines(trace));
  ...
  case TRACE_SECTION_INPUT_DEFINE: {
    ...
    if ((node_ptr *)(NULL) == res && create) {
      res =
          trace_setup_section_storage(trace, trace_get_n_input_defines(trace));
  ...
  case TRACE_SECTION_STATE_INPUT_DEFINE: {
    ...
    if ((node_ptr *)(NULL) == res && create) {
      res = trace_setup_section_storage(trace,
                                        trace_get_n_state_input_defines(trace));
  ...
  case TRACE_SECTION_STATE_NEXT_DEFINE: {
    ...
    if ((node_ptr *)(NULL) == res && create) {
      res = trace_setup_section_storage(trace,
                                        trace_get_n_state_next_defines(trace));
  ...
  case TRACE_SECTION_INPUT_NEXT_DEFINE: {
    ...
    if ((node_ptr *)(NULL) == res && create) {
      res = trace_setup_section_storage(trace,
                                        trace_get_n_input_next_defines(trace));
  ...
  case TRACE_SECTION_STATE_INPUT_NEXT_DEFINE: {
    ...
    if ((node_ptr *)(NULL) == res && create) {
      res = trace_setup_section_storage(
          trace, trace_get_n_state_input_next_defines(trace));
  ...
  default:
    ErrorMgr_internal_error(errmgr, "%s:%d:%s: invalid section (%d)", __FILE__,
                            __LINE__, __func__, section);
  } /* of switch */
    ...
    NODE_LIST_FOREACH(vars, list_iter) {
      ...
      if (SYMBOL_FUNCTION != SymbTable_get_symbol_category(self->st, var)) {
        if (trace_symbol_fwd_lookup(self, var, &section, &dummy)) {
          ...
          if ((initial == step) && (TRACE_SECTION_INPUT_VAR == section))
            continue;
          if (!trace_symbol_is_assigned(self, step, var)) {
            ...
            if (NIL(FILE) != report_stream) {
              fprintf(report_stream,
                      ...
                      step_count);
              print_node(wffprint, report_stream, var);
