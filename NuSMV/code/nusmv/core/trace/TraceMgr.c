	...
	const CompleteTraceExecutor_ptr executor)
{
	...
	entry = find_assoc(self->complete_trace_executors,
	...
	if (Nil != entry) {
		if (opt_verbose_level_ge(opts, 4)) {
			...
			Logger_log(logger,
				   "Replacing complete trace executor '%s'\n",
		...
		setcar(entry, NODE_PTR(UStringMgr_find_string(strings,
							      _executor_desc)));
		...
		if (opt_verbose_level_ge(opts, 4)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger,
				   "Registering complete trace executor '%s'\n",
				   UStringMgr_get_string_text(executor_id));
		}
...
TraceMgr_get_default_complete_trace_executor(const TraceMgr_ptr self)
{
	...
	if (0 == array_n(tmp)) {
		return COMPLETE_TRACE_EXECUTOR(NULL);
	}
...
TraceMgr_get_default_partial_trace_executor(const TraceMgr_ptr self)
{
	...
	if (0 == array_n(tmp)) {
		return PARTIAL_TRACE_EXECUTOR(NULL);
	}

	executor_id = array_fetch(string_ptr, tmp, 0);
	entry = find_assoc(self->partial_trace_executors,
			   NODE_PTR(executor_id));
	array_free(tmp);
	...
	if (Nil != entry) {
		if (opt_verbose_level_ge(opts, 4)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger,
				   "Replacing partial trace executor '%s'\n",
				   UStringMgr_get_string_text(executor_id));
		}
		...
		if (opt_verbose_level_ge(opts, 4)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger,
				   "Registering partial trace executor '%s'\n",
				   UStringMgr_get_string_text(executor_id));
		}
...
void TraceMgr_register_layer(TraceMgr_ptr self, const char *layer_name)
{
	...
	if (TraceMgr_is_layer_registered(self, layer_name))
		return;
			    ...
			    int plugin_index, int trace_index)
{
	...
	trace = TraceMgr_get_trace_at_index(
		self, (0 <= trace_index) ? trace_index :
					   (array_n(self->trace_list) - 1));

	plugin = TraceMgr_get_plugin_at_index(
		self, (0 <= plugin_index) ? plugin_index :
	...
	res = TracePlugin_action(plugin, trace,
				 TRACE_OPT(NULL) != opt ? opt :
							  self->default_opt);
...
TraceIter TraceMgr_get_iterator_from_label(TraceMgr_ptr self, TraceLabel label)
{
	...
	trace = TraceMgr_get_trace_at_index(self, TraceLabel_get_trace(label));
	...
	for (i = 0; i < state_no; ++i) {
		...
		if (TRACE_END_ITER == res) {
			...
			ErrorMgr_internal_error(
				errmgr, "%s:%d:%s: invalid trace number (%d)",
...
int TraceMgr_get_abs_index_from_label(TraceMgr_ptr self, TraceLabel label)
{
	...
	if (state_no < -1)
		state_no = Trace_get_length(trace) + (state_no + 2);
...
boolean TraceMgr_is_label_valid(TraceMgr_ptr self, TraceLabel label)
{
	...
	if (trace_no < 0 || trace_no >= TraceMgr_get_size(self)) {
		return false;
	}

	trace_len =
		Trace_get_length(TraceMgr_get_trace_at_index(self, trace_no));
	...
	if (state_no < 1 || (1 + trace_len) < state_no) {
		return false;
	}
...
boolean TraceMgr_is_visible_symbol(TraceMgr_ptr self, node_ptr symbol)
{
...

	{
		...
		while ((Nil != iter) && (NIL(char) == name)) {
			...
			default:
				error_unreachable_code(); /* not handled */
			} /* switch */
		} /* while */
	}
			 ...
			 int from_state, int to_state)
{
	...
	boolean print_header =
		(plugin_index !=
		 PTR_TO_INT(NuSMVEnv_get_value(env, ENV_TRACE_XML_DUMPER)) -
			 1) &&
		(plugin_index != PTR_TO_INT(NuSMVEnv_get_value(
					 env, ENV_TRACE_EMBEDDED_XML_DUMPER)) -
					 1) &&
		(TraceMgr_is_plugin_internal(
	...
	if (is_all == false) { /* state interval selection check */
		...
		Trace_ptr t = TraceMgr_get_trace_at_index(
			TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)),
			trace - 1);
		...
		{ /* check from_state */
			...
			if (from_state < 0) {
				from_state =
					2 + Trace_get_length(t) + from_state;
			}
		...
		if (to_state < from_state) {
			StreamMgr_print_error(
				streams,
				"Invalid state range specified: %d:%d\n",
				from_state, to_state);
			  ...
			  const int dp)
{
	...
	if (is_show_all || (dp < 0)) {
		...
		if (TraceMgr_get_plugin_size(TRACE_MGR(
			    NuSMVEnv_get_value(env, ENV_TRACE_MGR))) <= 0) {
			StreamMgr_print_error(
				streams,
				"There are no registered plugins to be shown\n");

			return 1;
		}

		for (i = 0;
		     i < TraceMgr_get_plugin_size(TRACE_MGR(
				 NuSMVEnv_get_value(env, ENV_TRACE_MGR)));
		     i++) {
			TracePlugin_ptr p_i = TraceMgr_get_plugin_at_index(
				TRACE_MGR(
					NuSMVEnv_get_value(env, ENV_TRACE_MGR)),
				i);

			if (i ==
			    TraceMgr_get_default_plugin(TRACE_MGR(
				    NuSMVEnv_get_value(env, ENV_TRACE_MGR)))) {
				StreamMgr_print_output(
					streams, "[D]  %d\t %s\n", i,
					TracePlugin_get_desc(p_i));
			} else {
				StreamMgr_print_output(
					streams, "     %d\t %s\n", i,
					TracePlugin_get_desc(p_i));
		...
		if (dp < TraceMgr_get_plugin_size(TRACE_MGR(
				 NuSMVEnv_get_value(env, ENV_TRACE_MGR)))) {
			p_i = TraceMgr_get_plugin_at_index(
				TRACE_MGR(
					NuSMVEnv_get_value(env, ENV_TRACE_MGR)),
				dp);
			...
			StreamMgr_print_error(
				streams,
				"Error: Plugin %d is not yet registered\n", dp);
			return 1;
		}
	}
			    ...
			    const int first_trace, const int last_trace)
{
	...
	for (trace_iter = first_trace; trace_iter <= last_trace; ++trace_iter) {
		...
		if (Trace_execute_trace(env, trace, executor))
			return 1;
	}
				    ...
				    const int first_trace, const int last_trace)
{
	...
	for (trace_iter = first_trace; trace_iter <= last_trace; ++trace_iter) {
		...
		trace = TraceMgr_get_trace_at_index(self, trace_index);
		{
			...
			if (Trace_execute_partial_trace(
				    env, trace, executor,
...
trace_manager_destroy_executor_entry(char *key, char *data, char *arg)
{
	...
	const NuSMVEnv_ptr env =
		EnvObject_get_environment(ENV_OBJECT(executor));
	const NodeMgr_ptr nodemgr =
		NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));

	BaseTraceExecutor_destroy(executor);
...
static int cmp_string_ptr(string_ptr *a, string_ptr *b)
{
	return strcmp(UStringMgr_get_string_text(*a),
		      UStringMgr_get_string_text(*b));
}
...
static void trace_manager_deinit(TraceMgr_ptr self)
{
	...
	for (i = 0; i < num; ++i) {
		Object_destroy(OBJECT(TraceMgr_get_plugin_at_index(self, i)),
			       NULL);
	}
	...
	for (i = 0; i < num; ++i) {
		const char *name =
			array_fetch(const char *, self->layer_names, i);
		if (name != (const char *)NULL) {
			FREE(name);
		}
	}
