...
void TraceMgr_register_complete_trace_executor(
  ...
  if (Nil != entry) {
    ...
    insert_assoc(self->complete_trace_executors, NODE_PTR(executor_id),
                 cons(nodemgr,
                      NODE_PTR(UStringMgr_find_string(strings, _executor_desc)),
                      NODE_PTR(executor)));
  }
}
...
void TraceMgr_register_partial_trace_executor(
  ...
  if (Nil != entry) {
    ...
    insert_assoc(self->partial_trace_executors, NODE_PTR(executor_id),
                 cons(nodemgr,
                      NODE_PTR(UStringMgr_find_string(strings, _executor_desc)),
                      NODE_PTR(executor)));
  }
...
int TraceMgr_execute_plugin(const TraceMgr_ptr self, const TraceOpt_ptr opt,
  ...
  trace = TraceMgr_get_trace_at_index(
      self, (0 <= trace_index) ? trace_index : (array_n(self->trace_list) - 1));

  plugin = TraceMgr_get_plugin_at_index(
      self,
      (0 <= plugin_index) ? plugin_index : TraceMgr_get_default_plugin(self));
  ...
  res = TracePlugin_action(plugin, trace,
                           TRACE_OPT(NULL) != opt ? opt : self->default_opt);
...
const char *TraceMgr_get_complete_trace_executor_desc(const TraceMgr_ptr self,
  ...
  nusmv_assert(NIL(char) != name);
  {
    ...
    node_ptr entry =
        find_assoc(self->complete_trace_executors,
...
const char *TraceMgr_get_partial_trace_executor_desc(const TraceMgr_ptr self,
  ...
  nusmv_assert(NIL(char) != name);
  {
    ...
    node_ptr entry =
        find_assoc(self->partial_trace_executors,
                   NODE_PTR(UStringMgr_find_string(strings, (char *)(name))));
    nusmv_assert(Nil != entry);
    return UStringMgr_get_string_text((string_ptr)(car(entry)));
  }
...
int TraceMgr_show_traces(TraceMgr_ptr const self, const int plugin_index,
  ...
  boolean print_header =
      (plugin_index !=
       PTR_TO_INT(NuSMVEnv_get_value(env, ENV_TRACE_XML_DUMPER)) - 1) &&
      (plugin_index !=
       PTR_TO_INT(NuSMVEnv_get_value(env, ENV_TRACE_EMBEDDED_XML_DUMPER)) -
           1) &&
      (TraceMgr_is_plugin_internal(
          TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), plugin_index));
  ...
  if (is_all == false) { /* state interval selection check */
    ...
    Trace_ptr t = TraceMgr_get_trace_at_index(
        TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), trace - 1);
      ...
      if (0 < from_state) { /* forward index */
        if (from_state > 1 + Trace_get_length(t)) {
          StreamMgr_print_error(
              streams, "Invalid starting state specified: %d\n", from_state);
          res = 1;
          return res;
        }
      }
    ...
    { /* check to_state */
      ...
      if (0 < to_state) { /* forward index */
        if (to_state > 1 + Trace_get_length(t)) {
          StreamMgr_print_error(streams, "Invalid end state specified: %d\n",
                                to_state);
          res = 1;
          return res;
        }
...
int TraceMgr_show_plugins(TraceMgr_ptr const self, const boolean is_show_all,
  ...
  if (is_show_all || (dp < 0)) {
    ...
    for (i = 0; i < TraceMgr_get_plugin_size(
                        TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)));
         i++) {
      TracePlugin_ptr p_i = TraceMgr_get_plugin_at_index(
          TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), i);

      if (i == TraceMgr_get_default_plugin(
                   TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)))) {
    ...
    if (dp < TraceMgr_get_plugin_size(
                 TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)))) {
      p_i = TraceMgr_get_plugin_at_index(
          TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), dp);

      if (dp == TraceMgr_get_default_plugin(
                    TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)))) {
        StreamMgr_print_output(streams, "[D]  %d\t %s\n", dp,
                               TracePlugin_get_desc(p_i));
      } else {
