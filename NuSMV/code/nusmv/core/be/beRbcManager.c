...
*/
#define GET_RBC_MGR(be_manager) (Rbc_Manager_t *)be_manager->spec_manager
				    ...
				    const size_t capacity)
{
	...
	Be_Manager_ptr self =
		Be_Manager_Create(env, spec, &beRbc_Rbc2Be, &beRbc_Be2Rbc);
	return self;
}
...
{
	return BE(manager,
		  Rbc_MakeNot(GET_RBC_MGR(manager), RBC(manager, left)));
}

be_ptr Be_And(Be_Manager_ptr manager, be_ptr left, be_ptr right)
{
	return BE(manager, Rbc_MakeAnd(GET_RBC_MGR(manager), RBC(manager, left),
				       RBC(manager, right), RBC_TRUE));
}

be_ptr Be_Or(Be_Manager_ptr manager, be_ptr left, be_ptr right)
{
	return BE(manager, Rbc_MakeOr(GET_RBC_MGR(manager), RBC(manager, left),
				      RBC(manager, right), RBC_TRUE));
}

be_ptr Be_Xor(Be_Manager_ptr manager, be_ptr left, be_ptr right)
{
	return BE(manager, Rbc_MakeXor(GET_RBC_MGR(manager), RBC(manager, left),
				       RBC(manager, right), RBC_TRUE));
}
...
{
	return BE(manager, Rbc_MakeIff(GET_RBC_MGR(manager), RBC(manager, left),
				       RBC(manager, right), RBC_TRUE));
}
...
{
	return BE(manager,
		  Rbc_MakeIte(GET_RBC_MGR(manager), RBC(manager, c),
			      RBC(manager, t), RBC(manager, e), RBC_TRUE));
}
...
{
	/* lazy evaluation: */
	if (Be_IsConstant(manager, f))
		return f;

	return BE(manager,
		  Rbc_LogicalShift(GET_RBC_MGR(manager), RBC(manager, f), shift,
				   log2phy, phy2log));
}
...
{
	return BE(manager,
		  Rbc_LogicalSubst(GET_RBC_MGR(manager), RBC(manager, f), subst,
				   log2phy, phy2log));
}
			   ...
			   Be_CnfAlgorithm alg)
{
	...
	cnf = Be_Cnf_Create(f);
	max_var_idx = Rbc_Convert2Cnf(GET_RBC_MGR(manager), RBC(manager, f),
				      polarity, alg, Be_Cnf_GetClausesList(cnf),
	...
	if (opt_verbose_level_gt(opts, 1)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   " Conversion returned maximum variable index = %d\n",
			   Be_Cnf_GetMaxVarIndex(cnf));
		Logger_log(logger,
			   " Length of list of clauses = %" PRIuPTR "\n",
			   Be_Cnf_GetClausesNumber(cnf));
		Logger_log(logger,
			   " Length of list of variables = %" PRIuPTR "\n",
...
Slist_ptr Be_CnfModelToBeModel(Be_Manager_ptr manager, Slist_ptr cnfModel)
{
	...
	SLIST_FOREACH(cnfModel, iter)
	{
		...
		beLiteral = (nusmv_ptrint)Be_CnfLiteral2BeLiteral(manager,
								  cnfLiteral);
...
int Be_CnfLiteral2BeLiteral(const Be_Manager_ptr self, int cnfLiteral)
{
	...
	if (-1 != rbcIndex)
		return (cnfLiteral > 0) ? (rbcIndex + 1) : (-rbcIndex - 1);
	else
		return 0;
}
...
int Be_BeLiteral2CnfLiteral(const Be_Manager_ptr self, int beLiteral)
{
	...
	return (beLiteral > 0) ? Be_BeIndex2CnfLiteral(self, be_idx) :
