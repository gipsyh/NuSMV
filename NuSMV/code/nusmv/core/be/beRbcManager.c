...
be_ptr Be_Ite(Be_Manager_ptr manager, be_ptr c, be_ptr t, be_ptr e) {
  return BE(manager, Rbc_MakeIte(GET_RBC_MGR(manager), RBC(manager, c),
...
be_ptr Be_LogicalShiftVar(Be_Manager_ptr manager, be_ptr f, int shift,
  ...
  return BE(manager, Rbc_LogicalShift(GET_RBC_MGR(manager), RBC(manager, f),
                                      shift, log2phy, phy2log));
}
...
Be_Cnf_ptr Be_ConvertToCnf(Be_Manager_ptr manager, be_ptr f, int polarity,
  ...
  cnf = Be_Cnf_Create(f);
  max_var_idx =
      Rbc_Convert2Cnf(GET_RBC_MGR(manager), RBC(manager, f), polarity, alg,
                      Be_Cnf_GetClausesList(cnf), Be_Cnf_GetVarsList(cnf),
...
static void *beRbc_Be2Rbc(Be_Manager_ptr mgr, be_ptr be) { return (Rbc_t *)be; }
