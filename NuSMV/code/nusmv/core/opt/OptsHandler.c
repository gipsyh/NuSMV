...
#define OPTS_BOOLEAN_REC                      \
	{                                     \
		{ OPTS_FALSE_VALUE, false },  \
		{                             \
			OPTS_TRUE_VALUE, true \
		}                             \
	}
				    ...
				    Option_Type type, void *arg)
{
	...
	return opts_handler_register_generic_option(
		self, name, def, ovl, check, get, is_public, type, false, arg);
}
					    ...
					    boolean is_public)
{
	...
	return opts_handler_register_generic_option(
		self, name, def, ovl, opts_check_string, opts_get_string,
		is_public, GENERIC_OPTION, false, NULL);
}
					 ...
					 int npv, boolean is_public)
{
	...
	if ((opt_ptr)NULL == opt) {
		...
		result = opts_handler_register_generic_option(
			...
			(Opts_ReturnFnType)opt_enum_get, is_public, ENUM_OPTION,
			false, NULL);
	}
	return (result);
}
					 ...
					 boolean value, boolean is_public)
{
	...
	result = opts_handler_register_generic_option(
		self, name, def, ovl, (Opts_CheckFnType)opt_enum_check,
		(Opts_ReturnFnType)opt_enum_get, is_public, BOOL_OPTION, false,
		NULL);
					...
					int value, boolean is_public)
{
	...
	result = opts_handler_register_generic_option(
		self, name, def, ovl, opts_isinteger, opts_getinteger,
...
boolean OptsHandler_is_option_registered(OptsHandler_ptr self, const char *name)
{
	opt_ptr opt;
				     ...
				     const char *value)
{
	...
	if ((opt_ptr)NULL != opt) {
		if (ovl_isempty(opt->possible_values) == 1) {
			/* user defined option */
			if ((*opt->check)(self, (char *)value, opt->arg) ==
			    true) {
				/* Run triggers, if any.. */
				if (!opts_handler_run_triggers(self, opt, name,
							       value,
							       ACTION_SET)) {
					return false;
				}
					  ...
					  const char *name, boolean value)
{
	...
	if ((opt_ptr)NULL != opt) {
		...
		/* Run triggers, if any.. */
		if (!opts_handler_run_triggers(self, opt, name, v,
					       ACTION_SET)) {
			return false;
		}
...
boolean OptsHandler_reset_option_value(OptsHandler_ptr self, const char *name)
{
	...
	if ((opt_ptr)NULL != opt) {
		/* Run triggers, if any.. */
		if (!opts_handler_run_triggers(self, opt, name, opt->value,
					       ACTION_RESET)) {
			return false;
		}
...
void *OptsHandler_get_option_value(OptsHandler_ptr self, const char *name)
{
	...
	if ((opt_ptr)NULL != opt) {
		/* Run triggers, if any.. */
		opts_handler_run_triggers(self, opt, name, opt->value,
					  ACTION_GET);
	...
	if ((opt_ptr)NULL != opt) {
		...
		if ((char *)NULL != opt->default_value) {
			if (ovl_isempty(opt->possible_values) == 1) {
				/* user defined option */
				result = (*opt->getvalue)(
					self, opt->default_value, opt->arg);
			} else {
				/* internally handled option */
				Opts_GetOplFnType f;

				f = (Opts_GetOplFnType)opt->getvalue;
				result = (*f)(self, opt->default_value,
					      opt->possible_values);
			}
		}
	} else {
					  ...
					  const char *name)
{
	...
	if ((opt_ptr)NULL != opt) {
		...
		if ((char *)NULL != opt->value) {
			...
			f = (Opts_GetOplFnType)opt->getvalue;
			result = (boolean)(*f)(self, opt->value,
					       opt->possible_values);
		}
	}
	return (result);
						  ...
						  const char *name)
{
	...
	if ((opt_ptr)NULL != opt) {
		...
		if ((char *)NULL != opt->default_value) {
			...
			f = (Opts_GetOplFnType)opt->getvalue;
			result = (boolean)(*f)(self, opt->default_value,
					       opt->possible_values);
		}
	}
...
void Opts_Gen_init(OptsHandler_ptr self)
{
	...
	if ((st_generator *)NULL != gen) {
		...
		error_unreachable_code_msg(
			"Opts_GenInit: Unable to allocate generator\n");
	}
}
		...
		if (OptsHandler_is_option_public(self, name)) {
			if (gen_unset) {
				...
				if (strcmp(name, "output_word_format") == 0) {
					...
					fprintf(of, " {%s \"*\", PASS[]}\n|",
						name);
				} else if (strcmp(name, "bmc_inc_invar_alg") ==
					   0) {
					...
					fprintf(of, " {%s dual, PASS[]}\n|",
						name);
					fprintf(of,
						" {%s falsification, PASS[]}\n|",
						name);
				} else if (strcmp(name, "bmc_invar_alg") == 0) {
					fprintf(of, " {%s classic, PASS[]}\n|",
						name);
					fprintf(of,
						" {%s een-sorensson, PASS[]}\n|",
						name);
				}
				/* STANDARD VALUES VARIABLES */
				else if (OptsHandler_is_bool_option(self,
								    name)) {
					fprintf(of, " {%s, PASS[]}\n|", name);
					fprintf(of, " {%s 1, PASS[]}\n|", name);
					fprintf(of, " {%s 0, PASS[]}\n|", name);
					fprintf(of, " {%s 2, FAIL[]}\n|", name);
				} else if (OptsHandler_is_enum_option(self,
								      name)) {
					char **values;
					int num_values, i;
					...
					fprintf(of,
						" {%s __i_am_not_valid__, FAIL[]}\n|",
						name);
				} else if (OptsHandler_is_int_option(self,
								     name)) {
					fprintf(of, " {%s 1, PASS[]}\n|", name);
					fprintf(of, " {%s 2, PASS[]}\n|", name);
					fprintf(of, " {%s 3, PASS[]}\n|", name);
					fprintf(of, " {%s NaN, FAIL[]}\n|",
						name);
				} else if (OptsHandler_is_generic_option(
						   self, name)) {
					fprintf(of, " {%s \"\", PASS[]}\n|",
						name);
					fprintf(of,
						" {%s \"custom_string\", PASS[]}\n|",
						name);
				}
			}
		}
...
void OptsHandler_copy(OptsHandler_ptr src_opts, OptsHandler_ptr dst_opts)
{
	...
	OPTS_FOREACH_OPTION(src_opts, &name, &value)
	{
		opt_ptr opt = (opt_ptr)find_assoc(
			src_opts->table, NODE_PTR(UStringMgr_find_string(
						 src_opts->strings, name)));
		nusmv_assert((opt_ptr)NULL != opt);
		...
		if (!OptsHandler_is_option_registered(dst_opts, name)) {
			...
			opts_handler_register_generic_option(
				dst_opts, name, opt->default_value, dst_ovl,
				opt->check, opt->getvalue, opt->public,
				opt->type, opt->user_defined, NULL);
		}
		{
			boolean copy;
			opt_ptr dst_opt = (opt_ptr)find_assoc(
				...
				NODE_PTR(UStringMgr_find_string(
					dst_opts->strings, name)));
			nusmv_assert((opt_ptr)NULL != dst_opt);
			/* dst_opt->value = opts_strsav(value); */

			copy = false;
			if (dst_opt->value != value) {
				if ((char *)NULL != dst_opt->value &&
				    (char *)NULL != value) {
					copy = 0 !=
					       strcmp(dst_opt->value, value);
				}
			}

			if (copy) {
				int res;
				res = OptsHandler_set_option_value(dst_opts,
								   name, value);
				nusmv_assert(res);
			}
		}
...
static void option_free(opt_ptr *p)
{
	...
	if (SLIST(NULL) != o->triggers) {
		...
		{
			option_trigger *ot =
				(option_trigger *)Siter_element(iter);
			FREE(ot);
		}
	...
	Option_Type type, boolean user_defined, void *arg)
{
	...
	if ((opt_ptr)NULL == opt) {
		opt = option_create(name, def, def, ovl, check, get, is_public,
				    type, user_defined, arg);

		if ((opt_ptr)NULL != opt) {
			unsigned int l;

			insert_assoc(self->table,
				     (node_ptr)NODE_PTR(UStringMgr_find_string(
					 ...
					 Trigger_Action action)
{
	...
	SLIST_FOREACH(opt->triggers, iter)
	{
		option_trigger *trigger_struct =
			(option_trigger *)Siter_element(iter);
		Opts_TriggerFnType f =
			(Opts_TriggerFnType)trigger_struct->trigger;
