...
void Opt_Pkg_init(NuSMVEnv_ptr const env)
{
	...
	res = OptsHandler_register_generic_option(opts, SCRIPT_FILE, NULL,
						  false);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, QUIET_MODE, false, false);
	nusmv_assert(res);

	res = OptsHandler_register_generic_option(
		opts, INPUT_ORDER_FILE, DEFAULT_INPUT_ORDER_FILE, true);
	nusmv_assert(res);

	res = OptsHandler_register_generic_option(
		opts, OUTPUT_ORDER_FILE, DEFAULT_OUTPUT_ORDER_FILE, true);
	nusmv_assert(res);

	res = OptsHandler_register_generic_option(
		opts, TRANS_ORDER_FILE, DEFAULT_TRANS_ORDER_FILE, true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_generic_option(
		opts, OUTPUT_FLATTEN_MODEL_FILE, (char *)NULL, true);
	nusmv_assert(res);

	res = OptsHandler_register_generic_option(
		opts, OUTPUT_BOOLEAN_MODEL_FILE, (char *)NULL, true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, TYPE_CHECKING_WARNING_ON,
					       DEFAULT_TYPE_CHECKING_WARNING_ON,
					       true);
	nusmv_assert(res);
	...
	{
		res = OptsHandler_register_int_option(
			opts, DEFAULT_TRACE_PLUGIN, 0, true);
		nusmv_assert(res);
	...
	res = OptsHandler_add_option_trigger(opts, RUN_CPP, opt_run_cpp_trigger,
					     env);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, OPT_CHECK_FSM, false,
					       true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, IGNORE_COMPUTE, false,
					       false);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, IGNORE_LTLSPEC, false,
					       false);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, IGNORE_PSLSPEC, false,
					       false);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, IGNORE_INVAR, false,
					       false);
	nusmv_assert(res);
	/* Those above were batch related */

	res = OptsHandler_register_bool_option(opts, FORWARD_SEARCH, true,
					       true);
	nusmv_assert(res);

	/* if this option is used, also their use is enabled */
	res = OptsHandler_add_option_trigger(
		opts, FORWARD_SEARCH, opt_set_reachable_states_trigger, env);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, USE_REACHABLE_STATES, true,
					       false);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, PRINT_REACHABLE, false,
					       false);
	nusmv_assert(res);

	/* if this option is used, also their use is enabled */
	res = OptsHandler_add_option_trigger(
		opts, PRINT_REACHABLE, opt_set_reachable_states_trigger, env);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, ENABLE_REORDER, false,
					       false);
	nusmv_assert(res);
...

	{
		Opts_EnumRec reorders[20] = {
			{ DynOrderTypeConvertToString(REORDER_RANDOM),
			  REORDER_RANDOM },
			{ DynOrderTypeConvertToString(REORDER_RANDOM_PIVOT),
			  REORDER_RANDOM_PIVOT },
			{ DynOrderTypeConvertToString(REORDER_SIFT),
			  REORDER_SIFT },
			{ DynOrderTypeConvertToString(REORDER_GROUP_SIFT),
			  REORDER_GROUP_SIFT },
			{ DynOrderTypeConvertToString(REORDER_SIFT_CONV),
			  REORDER_SIFT_CONV },
			{ DynOrderTypeConvertToString(REORDER_SYMM_SIFT),
			  REORDER_SYMM_SIFT },
			{ DynOrderTypeConvertToString(REORDER_SYMM_SIFT_CONV),
			  REORDER_SYMM_SIFT_CONV },
			{ DynOrderTypeConvertToString(REORDER_WINDOW2),
			  REORDER_WINDOW2 },
			{ DynOrderTypeConvertToString(REORDER_WINDOW3),
			  REORDER_WINDOW3 },
			{ DynOrderTypeConvertToString(REORDER_WINDOW4),
			  REORDER_WINDOW3 },
			{ DynOrderTypeConvertToString(REORDER_WINDOW2_CONV),
			  REORDER_WINDOW2_CONV },
			{ DynOrderTypeConvertToString(REORDER_WINDOW3_CONV),
			  REORDER_WINDOW3_CONV },
			{ DynOrderTypeConvertToString(REORDER_WINDOW4_CONV),
			  REORDER_WINDOW4_CONV },
			{ DynOrderTypeConvertToString(REORDER_GROUP_SIFT_CONV),
			  REORDER_GROUP_SIFT_CONV },
			{ DynOrderTypeConvertToString(REORDER_ANNEALING),
			  REORDER_ANNEALING },
			{ DynOrderTypeConvertToString(REORDER_GENETIC),
			  REORDER_GENETIC },
			{ DynOrderTypeConvertToString(REORDER_EXACT),
			  REORDER_EXACT },
			{ DynOrderTypeConvertToString(REORDER_LINEAR),
			  REORDER_LINEAR },
			{ DynOrderTypeConvertToString(REORDER_LINEAR_CONV),
			  REORDER_LINEAR_CONV },
			{ DynOrderTypeConvertToString(REORDER_SAME),
			  REORDER_SAME },
		};

		const char *def = DynOrderTypeConvertToString(DEFAULT_REORDER);
		res = OptsHandler_register_enum_option(opts, REORDER_METHOD,
						       def, reorders, 20, true);
		nusmv_assert(res);

		res = OptsHandler_add_option_trigger(
			opts, REORDER_METHOD, opt_reorder_method_trigger, env);
		nusmv_assert(res);
	}

	res = OptsHandler_register_bool_option(opts, DYNAMIC_REORDER, false,
					       true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, ENABLE_SEXP2BDD_CACHING,
					       true, true);
	nusmv_assert(res);
...

	{
		...
		res = OptsHandler_register_enum_option(
			opts, PARTITION_METHOD, TRANS_TYPE_THRESHOLD_STRING, pm,
			3, true);
		nusmv_assert(res);
	}

	res = OptsHandler_register_int_option(
		opts, CONJ_PART_THRESHOLD, DEFAULT_CONJ_PART_THRESHOLD, true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, IWLS95_PREORDER, false,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, AFFINITY_CLUSTERING, true,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, APPEND_CLUSTERS, true,
					       false);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, IGNORE_INIT_FILE, false,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, AG_ONLY_SEARCH, false,
					       true);

	res = OptsHandler_register_bool_option(opts, CONE_OF_INFLUENCE, false,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, LIST_PROPERTIES, false,
					       false);
	nusmv_assert(res);

	{
		Opts_EnumRec pm[4] = { { "name", PROP_PRINT_FMT_NAME },
	...
	res = OptsHandler_register_bool_option(opts, USE_FAIR_STATES, true,
					       false);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, COUNTER_EXAMPLES, true,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_generic_option(
		opts, TRACES_HIDING_PREFIX, DEFAULT_TRACES_HIDING_PREFIX, true);
	nusmv_assert(res);

	/* This is an internal option not visible at user level */
	res = OptsHandler_register_bool_option(opts, BDD_ENCODE_WORD_BITS,
					       DEFAULT_BDD_ENCODE_WORD_BITS,
					       false);

#if NUSMV_HAVE_REGEX_H
	res = OptsHandler_register_generic_option(opts, TRACES_REGEXP,
						  (char *)NULL, true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, WRITE_ORDER_DUMPS_BITS,
					       true, true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, USE_ANSI_C_DIV_OP, true,
					       false);
	nusmv_assert(res);

	{
		...
		const char *def =
			Enc_vars_ord_to_string(VARS_ORD_INPUTS_BEFORE_BI);
		res = OptsHandler_register_enum_option(opts, VARS_ORD_TYPE, def,
						       vot, 6, true);
		nusmv_assert(res);
	}

	{
		...
		const char *def = Enc_bdd_static_order_heuristics_to_string(
			BDD_STATIC_ORDER_HEURISTICS_BASIC);
		res = OptsHandler_register_enum_option(
			opts, BDD_STATIC_ORDER_HEURISTICS, def, bdd, 2, true);
		nusmv_assert(res);
	}

	res = OptsHandler_register_bool_option(opts, SYMB_INLINING, false,
					       true);
	nusmv_assert(res);

#if NUSMV_HAVE_SAT_SOLVER
	{
		Opts_EnumRec rbc[2] = { { RBC_SHERIDAN_CONVERSION_NAME,
					  RBC_SHERIDAN_CONVERSION },
					{ RBC_TSEITIN_CONVERSION_NAME,
					  RBC_TSEITIN_CONVERSION } };

		res = OptsHandler_register_enum_option(
			opts, RBC_CNF_ALGORITHM, RBC_SHERIDAN_CONVERSION_NAME,
			rbc, 2, true);
		nusmv_assert(res);
	}

	res = OptsHandler_register_bool_option(opts, RBC_INLINING, true, true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, RBC_INLINING_LAZY, false,
					       false);
	nusmv_assert(res);

	res = OptsHandler_add_option_trigger(
		opts, RBC_INLINING_LAZY, opt_rbc_inlining_lazy_trigger, env);
	nusmv_assert(res);

#endif

	res = OptsHandler_register_option(
		opts, A_SAT_SOLVER, DEFAULT_SAT_SOLVER,
		(Opts_CheckFnType)opt_check_sat_solver,
		(Opts_ReturnFnType)opt_get_sat_solver, true, GENERIC_OPTION,
		env);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, SHOW_DEFINES_IN_TRACES,
					       DEFAULT_SHOW_DEFINES_IN_TRACES,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, SHOW_DEFINES_WITH_NEXT,
					       DEFAULT_SHOW_DEFINES_WITH_NEXT,
					       true);
...
#endif
			{ (char *)opt_check_invar_strategy_to_string(FORWARD),
			  FORWARD },
			{ (char *)opt_check_invar_strategy_to_string(BACKWARD),
			  BACKWARD },
			{ (char *)opt_check_invar_strategy_to_string(
				  FORWARD_BACKWARD),
			  FORWARD_BACKWARD }
#if NUSMV_HAVE_SAT_SOLVER
			,
			{ (char *)opt_check_invar_strategy_to_string(BDD_BMC),
			  BDD_BMC }
#endif
		};
		const char *def = opt_check_invar_strategy_to_string(
			DEFAULT_INVAR_CHECK_STRATEGY);

#if NUSMV_HAVE_SAT_SOLVER
		res = OptsHandler_register_enum_option(
			opts, INVAR_CHECK_STRATEGY, def, strategies, 4, true);
#else
		res = OptsHandler_register_enum_option(
			opts, INVAR_CHECK_STRATEGY, def, strategies, 3, true);
#endif
		nusmv_assert(res);
	}

	{
		Opts_EnumRec heuristics[2] = {
			{ (char *)opt_check_invar_fb_heuristic_to_string(
				  ZIGZAG_HEURISTIC),
			  ZIGZAG_HEURISTIC },
			{ (char *)opt_check_invar_fb_heuristic_to_string(
		...
		const char *def = opt_check_invar_fb_heuristic_to_string(
	...
	{
		Opts_EnumRec heuristics[2] = {
			{ (char *)opt_check_invar_bddbmc_heuristic_to_string(
				  STEPS_HEURISTIC),
			  STEPS_HEURISTIC },
			{ (char *)opt_check_invar_bddbmc_heuristic_to_string(
				  SIZE_HEURISTIC),
		...
		const char *def = opt_check_invar_bddbmc_heuristic_to_string(
			DEFAULT_BDD2BMC_HEURISTIC);

		res = OptsHandler_register_enum_option(
			opts, CHECK_INVAR_BDDBMC_HEURISTIC, def, heuristics, 2,
			true);
		nusmv_assert(res);
	}

	res = OptsHandler_register_int_option(
		opts, CHECK_INVAR_BDDBMC_HEURISTIC_THRESHOLD,
		DEFAULT_BDD2BMC_HEURISTIC_THRESHOLD, true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, DAGGIFIER_ENABLED,
					       DEFAULT_DAGGIFIER_ENABLED, true);
	nusmv_assert(res);

	res = OptsHandler_register_int_option(opts, DAGGIFIER_COUNTER_THRESHOLD,
					      DEFAULT_DAGGIFIER_COUNTER_THS,
					      true);
	nusmv_assert(res);

	res = OptsHandler_register_int_option(opts, DAGGIFIER_DEPTH_THRESHOLD,
					      DEFAULT_DAGGIFIER_DEPTH_THS,
					      true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(opts, DAGGIFIER_STATISTICS,
					       false, true);
	nusmv_assert(res);

	{
		...
		res = OptsHandler_register_enum_option(
			opts, OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM, def, oreg,
			2, true);
		nusmv_assert(res);
	}

	res = OptsHandler_register_bool_option(
		opts, USE_COI_SIZE_SORTING, DEFAULT_USE_COI_SIZE_SORTING, true);
	nusmv_assert(res);
	...
	res = OptsHandler_register_bool_option(opts, LTL2SMV_SINGLE_JUSTICE,
					       DEFAULT_LTL2SMV_SINGLE_JUSTICE,
					       true);
	nusmv_assert(res);

	res = OptsHandler_register_bool_option(
		opts, BOOLEAN_CONVERSION_USES_PREDICATE_NORMALIZATION,
	...
	if (tmp != OPTS_VALUE_ERROR) {
		if ((new_format == 2) || (new_format == 8) ||
			...
			StreamMgr_print_error(
				streams, "Valid values are 2, 8, 10 and 16.\n");
		}
	}
	return false;
}
