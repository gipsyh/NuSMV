...
void Opt_Pkg_init(NuSMVEnv_ptr const env) {
  ...
  res = OptsHandler_add_option_trigger(opts, PP_M4_PATH, opt_pp_m4_path_trigger,
                                       env);
  ...
  res = OptsHandler_register_generic_option(opts, SCRIPT_FILE, NULL, false);
  nusmv_assert(res);
...

  {
    ...
    res = OptsHandler_register_option(
        opts, OUTPUT_WORD_FORMAT, def, (Opts_CheckFnType)opt_check_word_format,
  ...
  res = OptsHandler_register_bool_option(
      opts, TYPE_CHECKING_WARNING_ON, DEFAULT_TYPE_CHECKING_WARNING_ON, true);
  nusmv_assert(res);
  ...
  res = OptsHandler_add_option_trigger(opts, RUN_CPP, opt_run_cpp_trigger, env);
  nusmv_assert(res);
...

  {
    ...
    res = OptsHandler_register_option(
        opts, SHOWN_STATES, def, (Opts_CheckFnType)opt_check_shown_states,
        (Opts_ReturnFnType)opt_get_integer, true, INTEGER_OPTION, env);
    nusmv_assert(res);
  }
  ...
  res =
      OptsHandler_register_bool_option(opts, USE_REACHABLE_STATES, true, false);
  nusmv_assert(res);
...

  {
    ...
    res = OptsHandler_register_enum_option(opts, REORDER_METHOD, def, reorders,
                                           20, true);
    nusmv_assert(res);
  ...
  res = OptsHandler_register_bool_option(opts, ENABLE_SEXP2BDD_CACHING, true,
                                         true);
  nusmv_assert(res);
  ...
  {
    Opts_EnumRec pm[3] = {{TRANS_TYPE_MONOLITHIC_STRING, TRANS_TYPE_MONOLITHIC},
    ...
    res = OptsHandler_register_enum_option(
        opts, PARTITION_METHOD, TRANS_TYPE_THRESHOLD_STRING, pm, 3, true);
    nusmv_assert(res);
  }
  ...
  res = OptsHandler_register_bool_option(opts, CONE_OF_INFLUENCE, false, true);
  nusmv_assert(res);
...

  {
    ...
    res = OptsHandler_register_enum_option(opts, PROP_PRINT_METHOD, "formula",
                                           pm, 4, true);
    nusmv_assert(res);
  }
  ...
  res = OptsHandler_register_generic_option(opts, TRACES_HIDING_PREFIX,
                                            DEFAULT_TRACES_HIDING_PREFIX, true);
  nusmv_assert(res);
  ...
  res = OptsHandler_register_generic_option(opts, TRACES_REGEXP, (char *)NULL,
                                            true);
  nusmv_assert(res);
  ...
  res = OptsHandler_register_bool_option(opts, ON_FAILURE_SCRIPT_QUITS, false,
                                         true);
  nusmv_assert(res);

  res = OptsHandler_register_bool_option(opts, WRITE_ORDER_DUMPS_BITS, true,
                                         true);
  nusmv_assert(res);
...

  {
    ...
    res = OptsHandler_register_enum_option(opts, VARS_ORD_TYPE, def, vot, 6,
                                           true);
    nusmv_assert(res);
  }

  {
    Opts_EnumRec bdd[2] = {{(char *)Enc_bdd_static_order_heuristics_to_string(
  ...
  res = OptsHandler_register_bool_option(opts, SYMB_INLINING, false, true);
  nusmv_assert(res);
...
#if NUSMV_HAVE_SAT_SOLVER
  {
    ...
    res = OptsHandler_register_enum_option(
        opts, RBC_CNF_ALGORITHM, RBC_SHERIDAN_CONVERSION_NAME, rbc, 2, true);
    nusmv_assert(res);
  }
  ...
  res = OptsHandler_register_bool_option(opts, RBC_INLINING_LAZY, false, false);
  nusmv_assert(res);
  ...
  res = OptsHandler_register_option(opts, A_SAT_SOLVER, DEFAULT_SAT_SOLVER,
                                    (Opts_CheckFnType)opt_check_sat_solver,
                                    (Opts_ReturnFnType)opt_get_sat_solver, true,
                                    GENERIC_OPTION, env);
  nusmv_assert(res);
...
#if NUSMV_HAVE_SAT_SOLVER
    res = OptsHandler_register_enum_option(opts, INVAR_CHECK_STRATEGY, def,
                                           strategies, 4, true);
#else
    res = OptsHandler_register_enum_option(opts, INVAR_CHECK_STRATEGY, def,
                                           strategies, 3, true);
...

  {
    ...
    res = OptsHandler_register_enum_option(opts, CHECK_INVAR_FB_HEURISTIC, def,
                                           heuristics, 2, true);
    nusmv_assert(res);
  }
  ...
  res =
      OptsHandler_register_bool_option(opts, DAGGIFIER_STATISTICS, false, true);
  nusmv_assert(res);

  {
    ...
    res = OptsHandler_register_enum_option(
        opts, OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM, def, oreg, 2, true);
    nusmv_assert(res);
  }

  res = OptsHandler_register_bool_option(opts, USE_COI_SIZE_SORTING,
                                         DEFAULT_USE_COI_SIZE_SORTING, true);
  nusmv_assert(res);

  res = OptsHandler_register_bool_option(opts, DISABLE_SYNTACTIC_CHECKS, false,
                                         true);
  nusmv_assert(res);

  res = OptsHandler_register_bool_option(opts, KEEP_SINGLE_VALUE_VARS, false,
                                         true);
  nusmv_assert(res);

  res =
      OptsHandler_register_int_option(opts, DEFAULT_SIMULATION_STEPS, 10, true);
  nusmv_assert(res);
...
static boolean opt_check_word_format(OptsHandler_ptr opts, const char *val,
  ...
  if (tmp != OPTS_VALUE_ERROR) {
    if ((new_format == 2) || (new_format == 8) || (new_format == 10) ||
        (new_format == 16)) {
      return true;
    } else {
