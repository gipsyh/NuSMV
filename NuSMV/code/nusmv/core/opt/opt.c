...
static void check_user_preprocessor(const NuSMVEnv_ptr env,
				    Trigger_Action action, const char *value);
...
boolean is_default_order_file(OptsHandler_ptr opt)
{
	...
	if (DEFAULT_OUTPUT_ORDER_FILE == NIL(char))
		return false;
...
void set_output_flatten_model_file(OptsHandler_ptr opt, char *str)
{
	boolean res = OptsHandler_set_option_value(
		opt, OUTPUT_FLATTEN_MODEL_FILE, str);
	nusmv_assert(res);
}
void reset_output_flatten_model_file(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_reset_option_value(opt, OUTPUT_FLATTEN_MODEL_FILE);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_option_value(
		opt, OUTPUT_BOOLEAN_MODEL_FILE, str);
	nusmv_assert(res);
}
void reset_output_boolean_model_file(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_reset_option_value(opt, OUTPUT_BOOLEAN_MODEL_FILE);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_int_option_value(opt, OUTPUT_WORD_FORMAT, i);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_int_option_value(opt, VERBOSE_LEVEL, level);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_int_option_value(opt, SHOWN_STATES, level);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_SPEC, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_COMPUTE, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_COMPUTE, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_LTLSPEC, true);
	nusmv_assert(res);
}
void unset_ignore_ltlspec(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_LTLSPEC, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_PSLSPEC, true);
	nusmv_assert(res);
}
void unset_ignore_pslspec(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_PSLSPEC, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_INVAR, true);
	nusmv_assert(res);
}
void unset_ignore_invar(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_INVAR, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, OPT_CHECK_FSM, true);
	nusmv_assert(res);
}
void unset_check_fsm(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, OPT_CHECK_FSM, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, FORWARD_SEARCH, true);
	nusmv_assert(res);
}
void unset_forward_search(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, FORWARD_SEARCH, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, PRINT_REACHABLE, true);
	nusmv_assert(res);
}
void unset_print_reachable(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, PRINT_REACHABLE, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, ENABLE_REORDER, true);
	nusmv_assert(res);
}
void unset_reorder(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, ENABLE_REORDER, false);
	nusmv_assert(res);
}
...
void set_reorder_method(OptsHandler_ptr opt, unsigned int method)
{
	...
	boolean res =
		OptsHandler_set_enum_option_value(opt, REORDER_METHOD, value);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, DYNAMIC_REORDER, true);
	nusmv_assert(res);
}
void unset_dynamic_reorder(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, DYNAMIC_REORDER, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, ENABLE_SEXP2BDD_CACHING, true);
	nusmv_assert(res);
}
void unset_enable_sexp2bdd_caching(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, ENABLE_SEXP2BDD_CACHING, false);
	nusmv_assert(res);
}
boolean opt_enable_sexp2bdd_caching(OptsHandler_ptr opt)
{
	return OptsHandler_get_bool_option_value(opt, ENABLE_SEXP2BDD_CACHING);
}
...
void set_partition_method(OptsHandler_ptr opt, const TransType method)
{
	...
	case TRANS_TYPE_MONOLITHIC:
		str = TRANS_TYPE_MONOLITHIC_STRING;
		break;
	case TRANS_TYPE_IWLS95:
		str = TRANS_TYPE_IWLS95_STRING;
		break;
	case TRANS_TYPE_THRESHOLD:
		str = TRANS_TYPE_THRESHOLD_STRING;
		break;
	default:
...
{
	return (TransType)OptsHandler_get_enum_option_value(opt,
							    PARTITION_METHOD);
}
void set_monolithic(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_set_enum_option_value(
		opt, PARTITION_METHOD, TRANS_TYPE_MONOLITHIC_STRING);
	nusmv_assert(res);
}
void set_conj_partitioning(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_set_enum_option_value(
		opt, PARTITION_METHOD, TRANS_TYPE_THRESHOLD_STRING);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_enum_option_value(
		opt, PARTITION_METHOD, TRANS_TYPE_IWLS95_STRING);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_INIT_FILE, true);
	nusmv_assert(res);
}
void unset_ignore_init_file(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IGNORE_INIT_FILE, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, AG_ONLY_SEARCH, true);
	nusmv_assert(res);
}
void unset_ag_only(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, AG_ONLY_SEARCH, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, CONE_OF_INFLUENCE, true);
	nusmv_assert(res);
}
void unset_cone_of_influence(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_set_bool_option_value(opt, CONE_OF_INFLUENCE,
							false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, LIST_PROPERTIES, true);
	nusmv_assert(res);
}
void unset_list_properties(OptsHandler_ptr opt)
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, LIST_PROPERTIES, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_option_value(opt, A_SAT_SOLVER, satSolver);
	nusmv_assert(res);
}
...
{
	return OptsHandler_set_int_option_value(opt, DEFAULT_TRACE_PLUGIN,
						plugin);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IWLS95_PREORDER, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, IWLS95_PREORDER, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, APPEND_CLUSTERS, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, COUNTER_EXAMPLES, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, COUNTER_EXAMPLES, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_option_value(opt, TRACES_HIDING_PREFIX, prefix);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, BDD_ENCODE_WORD_BITS, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, USE_FAIR_STATES, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, USE_FAIR_STATES, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(opt, USE_ANSI_C_DIV_OP,
							false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_enum_option_value(
		opt, VARS_ORD_TYPE, (char *)Enc_vars_ord_to_string(type));
	nusmv_assert(res);
}

VarsOrdType get_vars_order_type(OptsHandler_ptr opt)
{
	return (VarsOrdType)OptsHandler_get_enum_option_value(opt,
							      VARS_ORD_TYPE);
}
...
void set_bdd_static_order_heuristics(OptsHandler_ptr opt, BddSohEnum value)
{
	...
	boolean res = OptsHandler_set_enum_option_value(
		opt, BDD_STATIC_ORDER_HEURISTICS, val);
	nusmv_assert(res);
}

BddSohEnum get_bdd_static_order_heuristics(OptsHandler_ptr opt)
{
	return (BddSohEnum)OptsHandler_get_enum_option_value(
		opt, BDD_STATIC_ORDER_HEURISTICS);
}
...
{
	return ((Be_CnfAlgorithm)OptsHandler_get_enum_option_value(
		opt, RBC_CNF_ALGORITHM));
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, SYMB_INLINING, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, SYMB_INLINING, false);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, RBC_INLINING, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, RBC_INLINING, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(opt, RBC_INLINING_LAZY,
							false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, SHOW_DEFINES_IN_TRACES, true);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, SHOW_DEFINES_IN_TRACES, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, SHOW_DEFINES_WITH_NEXT, true);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, SHOW_DEFINES_WITH_NEXT, false);
	nusmv_assert(res);
}
...
{
	nusmv_assert(opt != NULL);
	return (Check_Strategy)OptsHandler_get_enum_option_value(
		opt, INVAR_CHECK_STRATEGY);
}
...
const char *opt_check_invar_strategy_as_string(OptsHandler_ptr opt)
{
	...
	return opt_check_invar_strategy_to_string(
		opt_check_invar_strategy(opt));
}
...
void set_check_invar_fb_heuristic(OptsHandler_ptr opt, FB_Heuristic h)
{
	...
	boolean res = OptsHandler_set_enum_option_value(
		opt, CHECK_INVAR_FB_HEURISTIC, str);
	nusmv_assert(res);
}
...
{
	nusmv_assert(opt != NULL);
	return (FB_Heuristic)OptsHandler_get_enum_option_value(
		opt, CHECK_INVAR_FB_HEURISTIC);
}
...
const char *opt_check_invar_fb_heuristic_as_string(OptsHandler_ptr opt)
{
	...
	return opt_check_invar_fb_heuristic_to_string(
		opt_check_invar_fb_heuristic(opt));
}
...
void set_check_invar_bddbmc_heuristic(OptsHandler_ptr opt, Bdd2bmc_Heuristic h)
{
	...
	boolean res = OptsHandler_set_enum_option_value(
		opt, CHECK_INVAR_BDDBMC_HEURISTIC, str);
	nusmv_assert(res);
}

Bdd2bmc_Heuristic opt_check_invar_bddbmc_heuristic(OptsHandler_ptr opt)
{
	return (Bdd2bmc_Heuristic)OptsHandler_get_enum_option_value(
		opt, CHECK_INVAR_BDDBMC_HEURISTIC);
}

const char *opt_check_invar_bddbmc_heuristic_as_string(OptsHandler_ptr opt)
{
	return opt_check_invar_bddbmc_heuristic_to_string(
		opt_check_invar_bddbmc_heuristic(opt));
}
...
{
	boolean res = OptsHandler_set_int_option_value(
		opt, CHECK_INVAR_BDDBMC_HEURISTIC_THRESHOLD, h);
	nusmv_assert(res);
}

int opt_check_invar_bddbmc_heuristic_threshold(OptsHandler_ptr opt)
{
	return OptsHandler_get_int_option_value(
		opt, CHECK_INVAR_BDDBMC_HEURISTIC_THRESHOLD);
}
...
{
	boolean res =
		OptsHandler_set_bool_option_value(opt, DAGGIFIER_ENABLED, true);
	nusmv_assert(res);
}

void opt_disable_daggifier(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_set_bool_option_value(opt, DAGGIFIER_ENABLED,
							false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_int_option_value(
		opt, DAGGIFIER_COUNTER_THRESHOLD, x);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_int_option_value(
		opt, DAGGIFIER_DEPTH_THRESHOLD, x);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, DAGGIFIER_STATISTICS, false);
	nusmv_assert(res);
}
...
get_oreg_justice_emptiness_bdd_algorithm(OptsHandler_ptr opt)
{
	...
	res = OptsHandler_get_enum_option_value(
		opt, OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM);
	...
	OptsHandler_ptr opt, BddOregJusticeEmptinessBddAlgorithmType alg)
{
	...
	boolean res = OptsHandler_set_enum_option_value(
		opt, OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM, str);
	nusmv_assert(res);
}

void reset_oreg_justice_emptiness_bdd_algorithm(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_reset_option_value(
		opt, OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, USE_COI_SIZE_SORTING, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, DISABLE_SYNTACTIC_CHECKS, true);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, DISABLE_SYNTACTIC_CHECKS, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, KEEP_SINGLE_VALUE_VARS, true);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, KEEP_SINGLE_VALUE_VARS, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(opt, QUIET_MODE, true);
	nusmv_assert(res);
}

void unset_quiet_mode(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_set_bool_option_value(opt, QUIET_MODE, false);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_int_option_value(
		opt, DEFAULT_SIMULATION_STEPS, val);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, LTL2SMV_SINGLE_JUSTICE, true);
	nusmv_assert(res);
}
...
{
	boolean res =
		OptsHandler_reset_option_value(opt, LTL2SMV_SINGLE_JUSTICE);
	nusmv_assert(res);
}
...
{
	boolean res = OptsHandler_set_bool_option_value(
		opt, BOOLEAN_CONVERSION_USES_PREDICATE_NORMALIZATION, true);
	nusmv_assert(res);
}
void unset_boolconv_uses_prednorm(OptsHandler_ptr opt)
{
	boolean res = OptsHandler_reset_option_value(
		opt, BOOLEAN_CONVERSION_USES_PREDICATE_NORMALIZATION);
	nusmv_assert(res);
}
boolean opt_boolconv_uses_prednorm(OptsHandler_ptr opt)
{
	return OptsHandler_get_bool_option_value(
		opt, BOOLEAN_CONVERSION_USES_PREDICATE_NORMALIZATION);
}
...
{
	if (ACTION_SET == action || ACTION_RESET == action) {
		boolean enable = (strcmp(value, OPTS_TRUE_VALUE) == 0);
		boolean res = OptsHandler_set_bool_option_value(
			opts, USE_REACHABLE_STATES, enable);
		if (strcmp(opt, FORWARD_SEARCH) != 0) {
			res &= OptsHandler_set_bool_option_value(
				opts, FORWARD_SEARCH, enable);
		}
		nusmv_assert(res);
	}
				   ...
				   void *arg)
{
	...
	if (ACTION_SET == action) {
		DDMgr_ptr dd;
		unsigned int method =
			StringConvertToDynOrderType((char *)value);
...
boolean opt_run_cpp_trigger(OptsHandler_ptr opts, const char *opt,
			    const char *value, Trigger_Action action, void *arg)
{
	...
	case ACTION_RESET:
		StreamMgr_print_error(
			streams,
			"Error: the \"%s\" option is no longer supported",
			RUN_CPP);
			    ...
			    const char *value, Trigger_Action action, void *arg)
{
	...
	case ACTION_RESET: {
		...
		if (strcmp(stripped, value) != 0) {
			StreamMgr_print_error(
				streams, "Some of the specified preprocessors "
					 "does not exist\n");
			return false;
		}
	}
				      ...
				      void *arg)
{
	...
	case ACTION_SET:
		StreamMgr_print_error(
			streams,
			"Warning: setting of variable rbc_inlining_lazy"
				  ...
				  void *arg)
{
	...
	case ACTION_SET: {
		...
		if (0 != cr) {
			...
			StreamMgr_print_error(
				streams, "Error: could not parse '%s' (%s)\n",
				value, buf);
...
void *opt_get_integer(OptsHandler_ptr opts, const char *value, void *arg)
{
	void *result;
	char *e[1];
				     ...
				     const NuSMVEnv_ptr env)
{
	...
	if (OptsHandler_is_option_registered(opt, "open_path")) {
		open_path =
			OptsHandler_get_string_option_value(opt, "open_path");
	}
    ...
    */
		pp = pp + strlen(pp) +
		     1; /* 1 is used to pass past the current '\0' */
		pp = strtok(pp, " \t\n\r");
	} /* while loop */
