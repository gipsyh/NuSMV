	...
	/* actual parsing */
	{
		...
		if (type != Prop_Psl) {
			const char *parsing_type =
				PropType_to_parsing_string(type);
...
SymbTable_ptr Prop_get_symb_table(const Prop_ptr self)
{
	...
	if (NULL != Prop_get_bdd_fsm(self)) {
		result = BaseEnc_get_symb_table(BASE_ENC(
			BddFsm_get_bdd_encoding(Prop_get_bdd_fsm(self))));
...
boolean Prop_needs_rewriting(SymbTable_ptr st, const Prop_ptr self)
{
	...
	if (Prop_Psl == self->type) {
		...
		result = Wff_Rewrite_is_rewriting_needed(
			st, Prop_get_expr(self), Nil);
	}
...
void Prop_apply_coi_for_scalar(const NuSMVEnv_ptr env, Prop_ptr self)
{
	...
	/* scalar sexp fsm */
	{
		...
		scalar_fsm = FsmBuilder_create_scalar_sexp_fsm(builder,
							       hierarchy, cone);

		Prop_set_cone(self, cone);
		prop_set_scalar_sexp_fsm(self, scalar_fsm,
					 false); /* does not dup */
	}
}
	...
	if (!applied) {
		if (opt_verbose_level_gt(opts, 0)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger,
				   "Using previously built model for COI...\n");
		}
	}
}
	...
	if (BOOL_SEXP_FSM(NULL) == bool_fsm) {
		...
		{ /* commits the layer: */
         ...
         counter-examples: */
			...
			bool_enc = BoolEncClient_get_bool_enc(
				BOOL_ENC_CLIENT(be_enc));
			bdd_enc = BDD_ENC(
				NuSMVEnv_get_value(env, ENV_BDD_ENCODER));

			/* creates the boolean FSM */
			bool_fsm = FsmBuilder_create_boolean_sexp_fsm(
				helper, hierarchy, cone, bdd_enc, det_layer);
		...
		Prop_set_cone(self, cone);
		prop_set_bool_sexp_fsm(self, bool_fsm,
				       false); /* does not dup */
       ...
       format. */
		...
		if (be_fsm == BE_FSM(NULL)) {
			if (opt_verbose_level_gt(opts, 0)) {
				...
				Logger_log(
					logger,
					"BeFsm was found unexpectedly to be constructed\n");
			}
...
char *Prop_get_number_as_string(const Prop_ptr self)
{
	...
	nusmv_assert(Prop_get_type(self) ==
		     Prop_Compute); /* compute type only */
			...
			Prop_PrintFmt fmt)
{
...

	{
		...
		case PROP_PRINT_FMT_NAME:
			if (Nil != self->name) {
				OStream_nprintf(file, wffprint, "'%N'",
						self->name);
				break;
			}
...
void prop_print_db_tabular(const Prop_ptr self, OStream_ptr file)
{
...

	{
		...
		if (self->type == Prop_Compute) {
			...
			OStream_printf(file, "%-15s",
				       Prop_get_status_as_string(self));
		...
		case Prop_Ctl:
			if (opt_ag_only(opts)) {
				if (opt_forward_search(opts)) {
					...
					ErrorMgr_warning_ag_only_without_reachables(
						errmgr);
		...
		case Prop_Psl:
			if (Prop_is_psl_ltl(self)) {
               ...
               */
					...
					ErrorMgr_error_psl_not_supported_feature(
						errmgr);
				}
			}
			break;
...
Prop_ptr prop_convert_to_invar(Prop_ptr self)
{
...
#ifdef DEBUG_CONVERT_PROPERTY_TO_INVAR
	{
		Logger_ptr const logger =
			LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_vnlog_error(logger, wffprint, opts, "%s",
				   "Output prop:\n");
		if (NULL != retval) {
			Prop_print(retval, Logger_get_ostream(logger),
...
static Expr_ptr prop_get_expr_core_for_coi(const Prop_ptr self)
{
	...
	if (Prop_get_type(self) == Prop_Psl) {
		res = PslNode_remove_forall_replicators(env,
							Prop_get_expr(self));
	} else
		res = Prop_get_expr(self); /* usual expression */
