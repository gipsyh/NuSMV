				  ...
				  const char *indices)
{
	...
	for (token = strtok(_copy_indices, delimiters); token != (char *)NULL;
		...
		if (low > high) {
			FREE(_copy_indices);
			Set_ReleaseSet(res);
			ErrorMgr_rpterr(errmgr, "Range error: %d-%d", low,
					high);
		}
		...
		for (idx = low; idx <= high; ++idx) {
			...
			if (prop == (Prop_ptr)NULL) {
				...
				ErrorMgr_rpterr(
					errmgr,
					"Property index %d is not valid (must be in the range [0,%d])",
...
void PropDb_print_list_header(const PropDb_ptr self, OStream_ptr file)
{
	PROP_DB_CHECK_INSTANCE(self);
	{
		...
		case PROPDB_PRINT_FMT_TABULAR:
			OStream_printf(
				file,
				"**** PROPERTY LIST [ Type, Status, Counter-example "
				"Number, Name ] ****\n");
			OStream_printf(
				file,
				"--------------------------  PROPERTY LIST  "
		...
		case PROPDB_PRINT_FMT_XML:
			...
			OStream_printf(
				file,
				"<properties xmlns=\"http://es.fbk.eu\"\n");
			OStream_printf(
				file,
		...
		default:
			ErrorMgr_internal_error(
				errmgr, "Unsupported prop print format");
		}
	}
}
...
void PropDb_print_all(const PropDb_ptr self, OStream_ptr file)
{
...

	{
		...
		PropDb_print_all_status_type(self, file, Prop_NoStatus,
					     Prop_NoType);
			      ...
			      const node_ptr expr_name)
{
	...
	res = prop_db_prop_parse_from_arg_and_add(self, symb_table, argc, argv,
						  type);
...
int PropDb_prop_parse_name(const PropDb_ptr self, const char *str)
{
	...
	if (parse_result != 0 || parsed_command == Nil) {
		StreamMgr_print_error(
			streams, "Parsing error: expected a property name.\n");
		return -1;
	}
					   ...
					   const FlatHierarchy_ptr hierarchy)
{
	...
	for (i = 0; i < PropDb_get_size(self); ++i) {
		...
		node_ptr new_entry =
			cons(nodemgr, NODE_PTR(prop), NODE_PTR(cone));
				  ...
				  const FlatHierarchy_ptr hierarchy)
{
	...
	NODE_LIST_FOREACH(order, iter)
	{
		...
		if (!found) {
			...
			node_ptr new_entry =
				cons(nodemgr, NODE_PTR(cone), NODE_PTR(props));
			 ...
			 FILE *outstream)
{
	...
	if (print_props_num) {
		...
		if ((type == Prop_NoType) && (status == Prop_NoStatus) &&
			...
			PropDb_print_all_status_type(self, output, status,
						     type);
		} else if ((type != Prop_NoType) && (status == Prop_NoStatus)) {
			...
			retval = PropDb_print_prop_at_index(self, output,
							    prop_no);
		}
			  ...
			  const char *formula, const int prop_no)
{
	...
	if (pt != Prop_NoType) {
		if (formula == NIL(char)) {
			...
			int result;
			SymbTable_ptr st = SYMB_TABLE(
				NuSMVEnv_get_value(env, ENV_SYMB_TABLE));
			result = PropDb_prop_parse_and_add(self, st, formula,
							   pt, Nil);
			if (result == -1)
				return (1);
			...
			CATCH(errmgr)
			{
				if (opt_use_coi_size_sorting(opts)) {
					FlatHierarchy_ptr hierarchy =
						FLAT_HIERARCHY(NuSMVEnv_get_value(
							env,
							ENV_FLAT_HIERARCHY));

					PropDb_ordered_verify_all(self,
								  hierarchy);
				} else
				...
				node_ptr spec, Prop_Type type)
{
	...
	if (type == Prop_Psl) {
		...
		if (!PslNode_is_handled_psl(env, psl_prop)) {
			...
			if (PslNode_is_obe(psl_prop))
				...
				/* it is not supported */
				ErrorMgr_warning_psl_not_supported_feature(
					errmgr, spec, index);
	...
	if (allow_checking) {
		...
		if (!TypeChecker_check_property(
			    SymbTable_get_type_checker(symb_table), prop)) {
			const OStream_ptr oerr =
				StreamMgr_get_error_ostream(streams);
		...
		if (is_ctl || (type == Prop_Compute)) {
			...
			expr_vars = Formula_GetDependencies(
				symb_table, Prop_get_expr_core(prop), Nil);
       ...
       operators */
		{
			...
			if (Prop_Invar == type || Prop_Ltl == type) {
				Compile_check_next(symb_table, body, context,
						   true);

				Compile_check_input_next(symb_table, body,
							 context);
			} else {
				Compile_check_next(symb_table, body, context,
						   false);
					       ...
					       const Prop_Type type)
{
	...
	case Prop_CompId: {
		StreamMgr_print_error(
			streams, "Required to parse a property of Prop_CompId. "
...

	{
		...
		if (type != Prop_Psl) {
			const char *parsing_type =
				prop_db_get_prop_type_as_parsing_string(self,
									type);
			...
			int parse_result = Parser_read_psl_from_string(
				env, argc, argv, &parsed_command);
			if (parse_result != 0 || parsed_command == Nil) {
				StreamMgr_print_error(
					streams,
					"Parsing error: expected an \"%s\" expression.\n",
			...
			if (node_get_type(parsed_command) == CONTEXT) {
				node_ptr new_ctx =
					CompileFlatten_concat_contexts(
						env, Nil, car(parsed_command));
				property = PslNode_new_context(
					nodemgr, new_ctx, cdr(parsed_command));
			} else {
				property = PslNode_new_context(nodemgr, NULL,
							       parsed_command);
			}
		}

		return PropDb_prop_create_and_add(self, symb_table, property,
						  type);
	}
}
...
*/
static const char *prop_db_get_prop_type_as_parsing_string(PropDb_ptr self,
							   const Prop_Type type)
{
	UNUSED_PARAM(self);
