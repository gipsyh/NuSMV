				       ...
				       FsmType fsm_type, BddEnc_ptr bddenc)
{
	...
	prop_rewriter_init(self, env, prop, method, expprop, fsm_type, bddenc,
			   NULL);
	return self;
}
...
Prop_ptr prop_rewriter_rewrite(Prop_Rewriter_ptr self)
{
	...
	/* Maybe there is nothing to do */
       ...
       developer forgets to call Prop_Rewriter_destroy */
		if (NULL == self->layer) {
			layer = SymbTable_create_layer(self->symb_table, NULL,
...

		{
			...
			spec_type = PropType_to_node_type(
				Prop_get_type(self->original));

			rewritten_formula = Wff_Rewrite_rewrite_formula_generic(
				env, self->method, self->expprop, layer, outfh,
				formula, spec_type,
				self->monitor_variable_initialized_to_true,
				self->ltl2invar_negate_property);
		}

		nusmv_assert((INVARSPEC == PTR_TO_INT(Pair_get_second(
						   rewritten_formula)) ||
			      LTLSPEC == PTR_TO_INT(Pair_get_second(
						 rewritten_formula))));
		new_prop_type =
			(INVARSPEC ==
			 PTR_TO_INT(Pair_get_second(rewritten_formula))) ?
				Prop_Invar :
				Prop_Ltl;
		retval = Prop_create_partial(
			env, NODE_PTR(Pair_get_first(rewritten_formula)),
			new_prop_type);
		...
		/* Commit the needed encoders */
		{
			...
			boolenc = BoolEncClient_get_bool_enc(
				BOOL_ENC_CLIENT(bddenc));
		...
		/* Build the needed fsm */
		{
			...
			if (NULL == boolfsm &&
				...
				if (NULL == sexpfsm) {
					sexpfsm = SexpFsm_create(
						outfh,
						FlatHierarchy_get_vars(outfh));
				}

				SexpFsm_apply_synchronous_product(sexpfsm,
								  orig_sexpfsm);

				boolfsm = BoolSexpFsm_create_from_scalar_fsm(
					sexpfsm, bddenc, layer);
			}
			...
			if (NULL == bddfsm && (FSM_TYPE_BDD & self->fsm_type)) {
				...
				fsm_builder = FSM_BUILDER(NuSMVEnv_get_value(
					env, ENV_FSM_BUILDER));

				SexpFsm_apply_synchronous_product(sexpfsm,
								  orig_sexpfsm);

				bddfsm = FsmBuilder_create_bdd_fsm(
					fsm_builder, bddenc, sexpfsm,
			...
			if (NULL == sexpfsm &&
				...
				sexpfsm = SexpFsm_create(
					outfh, FlatHierarchy_get_vars(outfh));
			...
			if (NULL == befsm && (FSM_TYPE_BE & self->fsm_type)) {
				...
				beenc = BE_ENC(
					BeFsm_get_be_encoding(orig_befsm));
				fsm_builder = FSM_BUILDER(NuSMVEnv_get_value(
					env, ENV_FSM_BUILDER));

				if (NULL != boolfsm) {
					befsm = BeFsm_create_from_sexp_fsm(
						beenc, boolfsm);
				} else {
					boolfsm =
						FsmBuilder_create_boolean_sexp_fsm(
							fsm_builder, outfh,
             ...
             one (it should not create problems) */
					if (NULL != orig_sexpfsm) {
						...
						boolfsm =
							BoolSexpFsm_create_from_scalar_fsm(
								sexpfsm, bddenc,
								layer);
					}
				}
         ...
         copy *****************************************************************/
			{
				...
				if (NULL != sexpfsm) {
					Prop_set_scalar_sexp_fsm(retval,
								 sexpfsm);
		...
		Logger_vnlog_debug(logger, wffprint, opts,
				   "Output property\n%N\n",
		...
		if (NULL != Prop_get_scalar_sexp_fsm(retval) &&
			...
			if (NULL != self->layer) {
				...
				if (SymbTable_has_layer(self->symb_table,
							layer_name)) {
			...
			Compile_WriteFlattenFsm(
				env, Logger_get_stream(logger),
				self->symb_table, layers, "MODULE main",
				SexpFsm_get_hierarchy(
					Prop_get_scalar_sexp_fsm(retval)),
				false);
			...
			BddEnc_ptr bddenc, SymbLayer_ptr layer)
{
	...
	if ((FSM_TYPE_SEXP | FSM_TYPE_BOOL_SEXP | FSM_TYPE_BDD | FSM_TYPE_BE) &
		...
		if (NULL == fsm) {
			if (opt_cone_of_influence(opts)) {
				...
				fsm = SEXP_FSM(
					NuSMVEnv_get_value(env, ENV_SEXP_FSM));
	...
	if (FSM_TYPE_BDD & fsm_type) {
		...
		if (NULL == fsm) {
			if (opt_cone_of_influence(opts)) {
				...
				fsm = BDD_FSM(
					NuSMVEnv_get_value(env, ENV_BDD_FSM));
	...
	if (NULL != Prop_get_bdd_fsm(original)) {
		self->symb_table = BaseEnc_get_symb_table(BASE_ENC(
			BddFsm_get_bdd_encoding(Prop_get_bdd_fsm(original))));
		nusmv_assert(bddenc == BddFsm_get_bdd_encoding(
					       Prop_get_bdd_fsm(original)));
		...
		self->symb_table = SexpFsm_get_symb_table(
			Prop_get_scalar_sexp_fsm(original));
	} else {
		error_unreachable_code_msg(
			"At least one FSM built is assumed\n");
	}
	...
	if (Prop_needs_rewriting(self->symb_table, self->original) ||
		...
		if (FSM_TYPE_BE & self->fsm_type) {
			...
			if (BaseEnc_layer_occurs(BASE_ENC(beenc), layer_name)) {
				BaseEnc_remove_layer(BASE_ENC(beenc),
						     layer_name);
			}
		}
...
static void pr_free_node(NuSMVEnv_ptr env, NodeMgr_ptr nodemgr, node_ptr expr)
{
	if (NULL != expr) {
		...
		default: {
			...
			ErrorMgr_ptr const errmgr = ERROR_MGR(
