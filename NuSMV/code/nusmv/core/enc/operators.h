...
/*!
  \brief returns NUMBER with value 1 (symbol ExprMgr_true(exprs)) if
  the nodes are of different values, and value 0 (symbol ExprMgr_false(exprs))
  otherwise
...
/*!
  ...
  If "n1" is a list of values then <code>ExprMgr_true(exprs)</code> is returned
  only if all elements of "n1" is a subset of "n2".
