...
BddEnc_ptr BddEnc_create(SymbTable_ptr symb_table, BoolEnc_ptr bool_enc,
			 VarsHandler_ptr dd_vars_hndr, OrdGroups_ptr ord_groups)
{
...
BddVarSet_ptr BddEnc_get_state_vars_cube(const BddEnc_ptr self)
{
	...
	if (self->state_vars_bdd != (bdd_ptr)NULL) {
		...
		self->state_vars_bdd =
			add_to_bdd(self->dd, self->state_vars_add);
		res = bdd_dup(self->state_vars_bdd);
	} else
		res = (bdd_ptr)NULL;
...
BddVarSet_ptr BddEnc_get_next_state_vars_cube(const BddEnc_ptr self)
{
	...
	if (self->next_state_vars_bdd != (bdd_ptr)NULL) {
		...
		self->next_state_vars_bdd =
			add_to_bdd(self->dd, self->next_state_vars_add);
		res = bdd_dup(self->next_state_vars_bdd);
	} else
		res = (bdd_ptr)NULL;
...
BddVarSet_ptr BddEnc_get_frozen_vars_cube(const BddEnc_ptr self)
{
	...
	if (self->frozen_vars_bdd != (bdd_ptr)NULL) {
		...
		self->frozen_vars_bdd =
			add_to_bdd(self->dd, self->frozen_vars_add);
		res = bdd_dup(self->frozen_vars_bdd);
	} else
		res = (bdd_ptr)NULL;
...
BddVarSet_ptr BddEnc_get_input_vars_cube(const BddEnc_ptr self)
{
	...
	if (self->input_vars_bdd != (bdd_ptr)NULL) {
		...
		self->input_vars_bdd =
			add_to_bdd(self->dd, self->input_vars_add);
		res = bdd_dup(self->input_vars_bdd);
	} else
		res = (bdd_ptr)NULL;
...
BddVarSet_ptr BddEnc_get_layer_vars_cube(const BddEnc_ptr self,
					 SymbLayer_ptr layer, SymbFilterType vt)
{
	SymbTable_ptr st;
	...
	for (iter = NodeList_get_first_iter(booled_vars);
		...
		if (((vt & VFT_INPUT) &&
		     SymbTable_is_symbol_input_var(st, vname)) ||
		    ((vt & VFT_CURRENT) &&
		     SymbTable_is_symbol_state_var(st, vname)) ||
		    ((vt & VFT_FROZEN) &&
		     SymbTable_is_symbol_frozen_var(st, vname))) {
			bdd_and_accumulate(self->dd, &result, curr);
		}
		if ((vt & VFT_NEXT) &&
		    SymbTable_is_symbol_state_var(st, vname)) {
			bdd_ptr next =
				BddEnc_state_var_to_next_state_var(self, curr);
...
BddVarSet_ptr BddEnc_get_unfiltered_vars_cube(const BddEnc_ptr self, Set_t vars)
{
	...
	normal_result = BddEnc_get_vars_cube(
		self, normal_vars, VFT_CURRENT | VFT_INPUT | VFT_FROZEN);
			      ...
			      add_ptr cube)
{
	...
	for (width = AddArray_get_size(add_sym) - 1; width >= 0; --width) {
		...
		add_ptr bit_cube =
			add_support(self->dd, AddArray_get_n(add_sym, width));
			   ...
			   const node_ptr context)
{
	...
	if (opt_verbose_level_gt(opts, 4)) {
		...
		Logger_nlog(logger, wffprint,
			    "BddEnc: evaluating expression %N\n", expr);
	}
...

	{
		...
		FAIL(errmgr)
		{
			ErrorMgr_internal_error(
				errmgr,
				"BddEnc_expr_to_bdd: cannot convert non-propositional"
				" expression\n");
		}
	}
					       ...
					       add_ptr add)
{
	...
	return add_permute(self->dd, add,
			   array_fetch_p(int, self->current2next, 0));
}
					       ...
					       add_ptr add)
{
	...
	return add_permute(self->dd, add,
			   array_fetch_p(int, self->next2current, 0));
}
...
bdd_ptr BddEnc_state_var_to_next_state_var(const BddEnc_ptr self, bdd_ptr bdd)
{
	...
	return bdd_permute(self->dd, bdd,
			   array_fetch_p(int, self->current2next, 0));
}
			    ...
			    boolean changes_only)
{
...

	{
		...
		self->print_stack =
			cons(nodemgr, (node_ptr)info, self->print_stack);
	}
}
...
void BddEnc_print_bdd_end(BddEnc_ptr self)
{
	...
	BDD_ENC_CHECK_INSTANCE(self);
	nusmv_assert(self->print_stack !=
		     Nil); /*print_bdd_begin previously called*/
	...
	clear_assoc_and_free_entries_arg(info->hash, hash_node_free,
					 (char *)nodemgr);
		     ...
		     OStream_ptr file, void *arg)
{
	...
	valueList = BddEnc_assign_symbols(self, bdd, info->symbols, false,
					  (bdd_ptr *)NULL);
				...
				VPFBEFNNV p_fun, OStream_ptr file, void *arg)
{
	...
	/* Retrieve the vars list from committed layers */
	{
		...
		{
			node_ptr symbol =
				SymbTable_iter_get_symbol(st, &sfiter);

			arrayForEachItem(const char *, layer_names, i,
					 layer_name)
			{
				SymbLayer_ptr layer;

				layer = SymbTable_get_layer(
					BASE_ENC(self)->symb_table, layer_name);

				/* Only symbols in committed layers will be kept */
				if (SymbLayer_is_symbol_in_layer(layer,
								 symbol)) {
					if (SymbTable_is_symbol_define(
						    st, symbol)) {
						...
						bool_enc =
							BoolEncClient_get_bool_enc(
								BOOL_ENC_CLIENT(
									self));

						/* Append everything but bits */
						if (!BoolEnc_is_var_bit(
							    bool_enc, symbol)) {
							NodeList_append(
								committed_vars,
								symbol);
				      ...
				      OStream_ptr file)
{
	...
	{ /* We append next variables */
		...
		{
			node_ptr v =
				(node_ptr)NodeList_get_elem_at(sv_list, iter);

			if (SymbTable_is_symbol_state_var(st, v)) {
				NodeList_append(vars_list,
						find_node(nodemgr, NEXT, v,
							  Nil));
			}
		}
	}
	...
	do {
		...
		node_ptr varValueList = BddEnc_assign_symbols(
			self, bdd, vars_list, true, &bdd_of_assigns);
			       ...
			       bdd_ptr *resultAssignment)
{
	...
	NODE_LIST_FOREACH(symbols, iter)
	{
		...
		node_ptr sym_cleaned = NEXT == node_get_type(sym) ? car(sym) :
								    sym;
		...
		/* obtain the type */
		if (SymbTable_is_symbol_var(BASE_ENC(self)->symb_table,
					    sym_cleaned)) {
			type = SymbTable_get_var_type(
				BASE_ENC(self)->symb_table, sym_cleaned);
		} else if (SymbTable_is_symbol_define(
				   BASE_ENC(self)->symb_table, sym_cleaned)) {
			type = TypeChecker_get_expression_type(
				self->type_checker, sym_cleaned, Nil);
			nusmv_assert(!SymbType_is_error(
				type)); /* cannot be an type error */
		} else {
       ...
       string or integer) */
		...
		if (onlyRequiredSymbs) {
			...
			boolean printVar =
				BddEnc_is_var_in_cube(self, sym, support);
    ...
    */
		if (SymbType_is_word(type)) {
			const NuSMVEnv_ptr env =
				EnvObject_get_environment(ENV_OBJECT(self));
			const ExprMgr_ptr exprs = EXPR_MGR(
				NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
			...
			WordNumber_ptr one =
				WordNumberMgr_integer_to_word_number(words, 1,
								     width);
			WordNumber_ptr result =
				WordNumberMgr_integer_to_word_number(words, 0,
								     width);

			/* number of bits in ADD should be equal to number of bits in the type */
			nusmv_assert(width == SymbType_get_word_width(type) &&
				     width > 0);

			/* compute the Word value from bits */
			for (--width; width >= 0; --width) {
				add_ptr tmp_add = add_if_then(
					self->dd, add,
				...
				if (WordNumber_get_width(result) != 1) {
					result = WordNumberMgr_left_shift(
						words, result, 1);
				}
				/* add the bit to the result */
				if (ExprMgr_is_true(exprs, bit))
					result = WordNumberMgr_plus(
						words, result, one);
			} /* for */

			sym_value = find_node(nodemgr,
    ...
    */
			...
			add_ptr add_values = add_if_then(
				self->dd, add, AddArray_get_add(sym_add));
    ...
    */
		TypeChecker_is_expression_wellformed(self->type_checker,
						     sym_value, Nil);
			       ...
			       node_ptr list_of_sym, OStream_ptr file)
{
	...
	for (los = list_of_sym; los != Nil; los = cdr(los)) {
		node_ptr sym =
			bdd_enc_get_next_state_var_name(nodemgr, car(los));
		boolean printVar = BddEnc_is_var_in_cube(self, sym, add_cube);
		if (printVar) {
			OStream_nprintf(file, wffprint, "%N ", sym);
		}
...
NodeList_ptr BddEnc_get_vars_in_cube(const BddEnc_ptr self, bdd_ptr cube,
				     node_ptr list_of_sym, boolean include_next)
{
	node_ptr los;
	add_ptr add_cube;
	NuSMVEnv_ptr env;
	...
	for (los = list_of_sym; los != Nil; los = cdr(los)) {
		...
		if (include_next) {
			node_ptr next_sym =
				bdd_enc_get_next_state_var_name(nodemgr, sym);
				       ...
				       bdd_ptr formula)
{
	...
	arrayForEachItem(bdd_ptr, primes, i, bddprime)
	{
		...
		NODE_LIST_FOREACH(syms, iter)
		{
			...
			node_ptr sym_cleaned =
				(NEXT == node_get_type(sym)) ? car(sym) : sym;
			...
			if (true == present) {
				...
				if (SymbType_is_word(type)) {
					ErrorMgr_internal_error(
						errmgr,
						"BddEnc_ComputePrimeImplicants: words not yet handled\n");
				} else {
					/* sym_add must have only one element in the array */
					add_ptr add_values = add_if_then(
						self->dd, addprime,
						AddArray_get_add(sym_add));
					sym_value =
						add_value(self->dd, add_values);
					add_free(self->dd, add_values);
				}
				listpair = cons(nodemgr,
						find_node(nodemgr, COLON, sym,
							  sym_value),
						listpair);
				    ...
				    bdd_ptr inputs_states)
{
	...
	return bdd_pick_one_minterm(
		...
		array_fetch_p(bdd_ptr, self->minterm_state_frozen_input_vars,
			      0),
		self->minterm_state_frozen_input_vars_dim);
}
					    ...
					    const int array_len)
{
	...
	if (self->minterm_state_frozen_input_vars_dim > 0) {
		res = bdd_pick_all_terms(
			...
			self->minterm_state_frozen_input_vars_dim, result_array,
			array_len);
	}
	return res == 1;
}
...
boolean BddEnc_pick_all_terms_states(const BddEnc_ptr self, bdd_ptr bdd,
				     bdd_ptr *result_array, const int array_len)
{
	BDD_ENC_CHECK_INSTANCE(self);
	...
	if (self->minterm_state_frozen_vars_dim > 0) {
		...
		res = bdd_pick_all_terms(
			self->dd, bdd,
			array_fetch_p(bdd_ptr, self->minterm_state_frozen_vars,
				      0),
			self->minterm_state_frozen_vars_dim, result_array,
			array_len);
		return res == 1;
	} else {
		int i;
...
int BddEnc_get_var_index_from_name(const BddEnc_ptr self, node_ptr name)
{
	...
	if (NEXT != node_get_type(name)) {
		nusmv_assert(SymbTable_is_symbol_var(BASE_ENC(self)->symb_table,
						     name));
	} else {
		nusmv_assert(SymbTable_is_symbol_var(BASE_ENC(self)->symb_table,
						     car(name)));
...
int BddEnc_eval_num(BddEnc_ptr self, node_ptr e, node_ptr context)
{
...

	{
		...
		FAIL(errmgr)
		{
			...
			ErrorMgr_rpterr(
				errmgr,
				"BddEnc_eval_num: an error occurred during evaluation.\n");
		}
		...
		if (node_get_type(n) != NUMBER) {
			ErrorMgr_rpterr(
				errmgr,
				"BddEnc_eval_num: numeric constant required");
		}
	}
...
add_ptr BddEnc_eval_constant(BddEnc_ptr self, Expr_ptr expr, node_ptr context)
{
	...
	if (AddArray_get_size(evaluation) != 1 || add_isleaf(result) == 0) {
		...
		ErrorMgr_internal_error(errmgr,
					"BddEnc_eval_constant: Evaluating a non"
					" constant expression");
	}
	add_ref(result);
...
AddArray_ptr BddEnc_get_symbol_add(BddEnc_ptr self, node_ptr name)
{
  ...
  */
    ...
    */
		nusmv_assert(SymbTable_is_symbol_define(
	...
	/* Is it a define? */
       ...
       expressions are quite printable. */
		...
		node_ptr body = SymbTable_get_define_body(
			BASE_ENC(self)->symb_table, name);
		node_ptr new_context = SymbTable_get_define_context(
			BASE_ENC(self)->symb_table, name);
		...
		node_ptr def = SymbTable_get_define_flatten_body(
			BASE_ENC(self)->symb_table, name);

		BddEncCache_set_evaluation(self->cache, name,
					   BDD_ENC_EVALUATING);
		ErrorMgr_io_atom_push(errmgr, name); /* for error reporting */
	...
	/* gets the encoding associated with possible variable */
       ...
       */
			if (SymbTable_is_symbol_bool_var(
				    BASE_ENC(self)->symb_table, name)) {
#if __BDDENC_LAZY_COMMIT_LAYER__
				add_ptr tmp = lazy_commit_layer_and_get_add(
					self, name, __EVAL_VAR__);
#else
				add_ptr tmp = BddEncCache_lookup_boolean_var(
					self->cache, name);
#endif
			...
			/* usual (not boolean) variable => evaluate its boolean encoding */
				...
				node_ptr def = BoolEnc_get_var_encoding(
					BOOL_ENC_CLIENT(self)->bool_enc, name);
				BddEncCache_set_evaluation(self->cache, name,
							   BDD_ENC_EVALUATING);
				ErrorMgr_io_atom_push(
					errmgr, name); /* for error reporting */
				res = bdd_enc_eval(self, def, Nil);
				ErrorMgr_io_atom_pop(errmgr);
			}
		}
	...
	else if (SymbTable_is_symbol_parameter(BASE_ENC(self)->symb_table,
					       name)) {
...
add_ptr BddEnc_get_state_frozen_vars_mask_add(BddEnc_ptr self)
{
	...
	if (self->state_frozen_vars_mask_add == (add_ptr)NULL) {
		...
		iter_layer = NodeList_get_first_iter(
			BASE_ENC(self)->committed_layers);
		while (!ListIter_is_end(iter_layer)) {
			add_ptr mask;
			SymbLayer_ptr layer;
			SymbLayerIter iter;

			layer = SYMB_LAYER(NodeList_get_elem_at(
				BASE_ENC(self)->committed_layers, iter_layer));

			SymbLayer_gen_iter(layer, &iter,
...
add_ptr BddEnc_get_state_frozen_input_vars_mask_add(BddEnc_ptr self)
{
	...
	if (self->state_frozen_input_vars_mask_add == (add_ptr)NULL) {
		...
		nusmv_assert(self->state_frozen_input_vars_mask_add !=
			     (add_ptr)NULL);
...
bdd_ptr BddEnc_get_state_frozen_vars_mask_bdd(BddEnc_ptr self)
{
	...
	if (self->state_frozen_vars_mask_bdd == (bdd_ptr)NULL) {
		...
		self->state_frozen_vars_mask_bdd =
			add_to_bdd(self->dd, mask_add);
...
bdd_ptr BddEnc_get_state_frozen_input_vars_mask_bdd(BddEnc_ptr self)
{
	...
	if (self->state_frozen_input_vars_mask_bdd == (bdd_ptr)NULL) {
		...
		self->state_frozen_input_vars_mask_bdd =
			add_to_bdd(self->dd, mask_add);
		add_free(self->dd, mask_add);
	}
				     ...
				     const VarOrderingType ord_type)
{
	...
	for (cvl = 1; cvl < max_level; ++cvl) {
		...
		if (name != Nil && (node_get_type(name) != NEXT)) {
			if (BoolEnc_is_var_bit(BOOL_ENC_CLIENT(self)->bool_enc,
					       name) &&
			    (ord_type != DUMP_BITS)) {
				/* retrieve the corresponding scalar variable: */
				name = BoolEnc_get_scalar_var_from_bit(
					BOOL_ENC_CLIENT(self)->bool_enc, name);
			}
			      ...
			      const VarOrderingType dump_type)
{
	...
	if (NuSMVEnv_has_value(env, ENV_PROC_SELECTOR_VNAME)) {
		node_ptr proc_selector_vname = NODE_PTR(
			NuSMVEnv_get_value(env, ENV_PROC_SELECTOR_VNAME));
		...
		if (dump_type != DUMP_BITS) {
			...
			proc_sel_vars = BoolEnc_get_var_bits(
				BOOL_ENC_CLIENT(self)->bool_enc,
				proc_selector_vname);
		}
		...
		while (!ListIter_is_end(iter)) {
			node_ptr name =
				NodeList_get_elem_at(proc_sel_vars, iter);
	...
	if (!util_is_string_null(output_order_file_name)) {
		...
		if (oof == NULL) {
			ErrorMgr_rpterr(errmgr,
					"output_order: unable to open file %s",
	...
	if (oof != outstream) {
		if (fclose(oof) != 0) {
			ErrorMgr_rpterr(errmgr, "cannot close %s",
					output_order_file_name);
		}
	}
	...
	if (opt_verbose_level_gt(opts, 0)) {
		...
		if (output_order_file_name != NULL) {
			Logger_log(logger,
				   "%s: variable order output to file %s\n",
   ...
   */
	...
	{ /* performs the reordering according to new_ord_groups */
		...
		for (group_id = 0; group_id < groups_size; ++group_id) {
			...
			vars = OrdGroups_get_vars_in_group(new_ord_groups,
							   group_id);
					...
					current_layer =
						SymbTable_variable_get_layer(
							BASE_ENC(self)
								->symb_table,
							name);
					nusmv_assert(SYMB_LAYER(NULL) !=
						     current_layer);
				}

				/* To guarantee the variables of a group belong to the same layer */
				nusmv_assert(SymbLayer_is_variable_in_layer(
					current_layer, name));

				if (-1 == low_index) {
					low_index = bdd_enc_name_to_index(self,
									  name);
					index = low_index;
				} else {
					index = bdd_enc_name_to_index(self,
								      name);
				}

				if (SymbTable_is_symbol_state_var(
					    BASE_ENC(self)->symb_table, name)) {
					if (-1 == chunk)
						chunk = 2;
					else {
		...
		{ /* now it is time to construct the new variable blocks */
			for (group_id = 0; group_id < groups_size; ++group_id) {
				int min_lev = dd_get_level_at_index(
					self->dd, group2info[group_id].min_idx);
				int real_lev = bdd_enc_create_block(
					self, group2info[group_id].layer,
...
void BddEnc_force_order_from_file(BddEnc_ptr self, FILE *orderfile)
{
	...
	nusmv_assert((FILE *)NULL != orderfile);
	{
		...
		ParserOrd_ptr parser =
			ParserOrd_create(ENV_OBJECT(self)->environment);
		...
		grps = enc_utils_create_vars_ord_groups(
			  ...
			  const boolean verbose, const boolean formula)
{
	...
	if (SymbType_is_boolean(
		...
		BddEnc_print_formula_info(bdd_enc, car(constr), verbose,
					  formula, outstream);
		StreamMgr_print_output(
			streams, "##########################################"
				 "############################\n");
		     ...
		     FILE *outfile)
{
	...
	const SymbType_ptr tp =
		TypeChecker_get_expression_type(tc, node_expr, Nil);
	int res = 1;

	/* checks type compatibility */
	if (SymbType_is_error(tp)) {
		StreamMgr_print_error(streams,
				      "Type of expression is not correct\n");
		return 1;
	}
	...
	{ /* the actual job */
		AddArray_ptr addarray =
			BddEnc_expr_to_addarray(self, node_expr, Nil);
		...
		if (1 == adr_size) {
			...
			const int oname_len = (strlen(str_constr) +
					       strlen(oname_fmt) + digits + 1);

			for (i = 0; i < adr_size; ++i) {
				char *oname = ALLOC(char, oname_len);
				int c;
				nusmv_assert((char *)NULL != oname);
				c = snprintf(oname, oname_len, oname_fmt,
					     str_constr, digits, i);
		...
		case DUMP_FORMAT_DOT:
			res = BddEnc_dump_addarray_dot(self, addarray, onames,
						       outfile);
			break;

		case DUMP_FORMAT_DAVINCI:
			res = BddEnc_dump_addarray_davinci(self, addarray,
							   onames, outfile);
			break;
		  ...
		  OrdGroups_ptr ord_groups)
{
	...
	for (i = 0; i < BDD_ENC_INIT_VAR_NUM; ++i) {
		...
		array_insert(bdd_ptr, self->minterm_input_vars, i,
			     (bdd_ptr)NULL);
		array_insert(bdd_ptr, self->minterm_state_vars, i,
			     (bdd_ptr)NULL);
		array_insert(bdd_ptr, self->minterm_next_state_vars, i,
			     (bdd_ptr)NULL);
		array_insert(bdd_ptr, self->minterm_frozen_vars, i,
			     (bdd_ptr)NULL);
		array_insert(bdd_ptr, self->minterm_state_frozen_vars, i,
			     (bdd_ptr)NULL);
...
void bdd_enc_deinit(BddEnc_ptr self)
{
	...
	for (i = 0; i < self->minterm_state_frozen_input_vars_dim; ++i) {
		bdd_free(self->dd,
			 array_fetch(bdd_ptr,
				     self->minterm_state_frozen_input_vars, i));
		array_insert(bdd_ptr, self->minterm_state_frozen_input_vars, i,
			     NULL);
	}
	...
	/* hashes */
	{
		...
		ASSOC_FOREACH(self->layer2groups, aiter, &layer, &groups)
		{
			if ((array_t *)NULL != groups) {
				...
				/* releases all groups */
				arrayForEachItem(GroupInfo_ptr, groups, giter,
						 binfo)
				{
					/* this releases and frees binfo which was returned by
             VarsHandler_reserve_group */
					VarsHandler_release_group(
						self->dd_vars_hndr, binfo);
				}
...
void bdd_enc_commit_layer(BaseEnc_ptr enc_base, const char *layer_name)
{
	...
	bool_layer_name =
		BoolEnc_scalar_layer_to_bool_layer(bool_enc, layer_name);
	...
	while (layers[idx] != SYMB_LAYER(NULL)) {
		...
		{
			node_ptr constant =
				SymbLayer_iter_get_symbol(layers[idx], &iter);
			...
			if (opt_verbose_level_gt(opts, 4)) {
				...
				Logger_nlog(logger, wffprint,
					    "BddEnc: encoding constant %N\n",
					    constant);
			}

			add_constant = add_leaf(self->dd, constant);
			BddEncCache_new_constant(self->cache, constant,
						 add_constant);
     ...
     groups, and creates corresponding bdd indices and variables */
	{
		...
		for (group_id = 0; group_id < groups_num; ++group_id) {
			...
			NodeList_ptr group =
				OrdGroups_get_vars_in_group(groups, group_id);
         ...
         of required variables */
			...
			{
				node_ptr name =
					NodeList_get_elem_at(group, gr_iter);
				nusmv_assert(SymbTable_is_symbol_var(symbTable,
								     name));

				if (SymbTable_is_symbol_state_var(symbTable,
								  name)) {
					if (-1 == group_chunk)
						group_chunk = 2;
           ...
           allocating the bdd blocks */
				...
				bdd_enc_lock_reordering(
					self); /* locks reordering */

				block_layer = SymbTable_get_layer(symbTable,
								  layer_name);
				level = bdd_enc_reserve_consecutive_vars(
					self, block_layer, group_size,
					group_chunk);

				gr_iter = NodeList_get_first_iter(group);
				while (!ListIter_is_end(gr_iter)) {
					node_ptr name = NodeList_get_elem_at(
						group, gr_iter);
					...
					if (SymbTable_is_symbol_state_var(
						...
						bdd_enc_add_state_var(
							self, block_layer, name,
							cindex, nindex);
					}

					/* input variable */
					else if (SymbTable_is_symbol_input_var(
							 symbTable, name)) {
						int cindex =
							dd_get_index_at_level(
								self->dd,
					...
					/* frozen variable */
						...
						nusmv_assert(
							SymbTable_is_symbol_frozen_var(
								symbTable,
								name));
						cindex = dd_get_index_at_level(
							self->dd, level++);
						bdd_enc_add_frozen_var(
							self, name, cindex);
					}
				...
				bdd_enc_unlock_reordering(
					self); /* release reordering lock */
			}
		} /* for groups */
             ...
             created, and dissolved, thus here we re-created */
					...
					int idx =
						BddEnc_get_var_index_from_name(
							self, name);
					int lev = dd_get_level_at_index(
						self->dd, idx);
					int real_lev = bdd_enc_create_block(
						self, current_layer, lev, chunk,
						chunk, false);
...
void bdd_enc_remove_layer(BaseEnc_ptr enc_base, const char *layer_name)
{
	...
	layers[1] = SymbTable_get_layer(BASE_ENC(self)->symb_table,
					bool_layer_name);
	...
	{ /* At first dissolves the associated groups */
		...
		while (SYMB_LAYER(NULL) != layers[i]) {
         ...
         boolean layer is not empty, and it has associated groups. */
			groups = (array_t *)find_assoc(self->layer2groups,
						       (node_ptr)layers[i]);
			if (groups != (array_t *)NULL) {
				...
				arrayForEachItem(GroupInfo_ptr, groups, iter,
						 binfo)
				{
					(void)VarsHandler_release_group(
						self->dd_vars_hndr, binfo);
				}
				...
				array_free(groups);
				insert_assoc(self->layer2groups,
					     (node_ptr)layers[i],
					     (node_ptr)NULL);
			}
			i += 1;
	...
	while (layers[i] != SYMB_LAYER(NULL)) {
		...
		{
			node_ptr constant =
				SymbLayer_iter_get_symbol(layers[i], &iter);

			if (opt_verbose_level_gt(opts, 4)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_nlog(logger, wffprint,
					    "BddEnc: removing constant %N\n",
					    constant);
			}
					  ...
					  SymbLayer_iter_filter_bool_vars, NULL)
		{
			...
			if (opt_verbose_level_gt(opts, 4)) {
				...
				Logger_nlog(
					...
					name,
					BddEnc_get_var_index_from_name(self,
								       name));
			}
	...
	self->minterm_next_state_vars_dim = bdd_enc_compact_minterms_array(
		self, self->minterm_next_state_vars,
	...
	self->minterm_state_frozen_vars_dim = bdd_enc_compact_minterms_array(
		self, self->minterm_state_frozen_vars,
		self->minterm_state_frozen_vars_dim);
	self->minterm_state_frozen_input_vars_dim =
		bdd_enc_compact_minterms_array(
...
void bdd_enc_shuffle_variables_order(BddEnc_ptr self, NodeList_ptr vars)
{
	...
	NODE_LIST_FOREACH(vars, iter)
	{
		...
		if (SymbTable_is_symbol_state_var(BASE_ENC(self)->symb_table,
						  name) &&
		    (node_get_type(name) != NEXT)) {
			node_ptr next_name =
				bdd_enc_get_next_state_var_name(nodemgr, name);
			if (!NodeList_belongs_to(vars, next_name)) {
				/* it is assumed here that next is always closed to current */
				int nidx = array_fetch(int, self->current2next,
						       idx);
				if (level !=
				    dd_get_level_at_index(self->dd, nidx))
					changed = true;

				array_insert(int, self->level2index, level++,
					     nidx);
				NodeList_append(used_indices,
						NODE_FROM_INT(nidx));
			}
		}
	...
	if (level < tot_vars_num) {
       ...
       list vars. Those must be put at the end of the ordering. */
		...
		for (index = 1; index < tot_vars_num; ++index) {
			if (!NodeList_belongs_to(used_indices,
						 NODE_FROM_INT(index))) {
				if (level !=
				    dd_get_level_at_index(self->dd, index))
					changed = true;
				array_insert(int, self->level2index, level++,
					     index);
			}
	...
	if (changed) {
		res = dd_set_order(self->dd,
				   array_fetch_p(int, self->level2index, 0));
	}
	...
	if (opt_verbose_level_gt(opts, 5)) {
		...
		if (changed) {
			...
			for (idx = 0; idx < tot_vars_num; ++idx) {
				Logger_log(logger, "%*d ", str_len,
					   array_fetch(int, self->level2index,
						       idx));
			}
			Logger_log(logger, "\n");
			...
			{
				char *req_lev = ALLOC(
					char, (str_len + 1) * tot_vars_num + 1);

				for (idx = 0;
				     idx < (str_len + 1) * tot_vars_num;
				     ++idx) {
					req_lev[idx] = ' ';
				}
				...
				for (idx = 0; idx < tot_vars_num; ++idx) {
					int ofs = array_fetch(int,
							      self->level2index,
							      idx) *
						  (str_len + 1);
					char trail =
						*(req_lev + ofs + str_len + 1);
					sprintf(req_lev + ofs, "%*d ", str_len,
						idx);
	...
	NODE_LIST_FOREACH(self->index_gaps, iter)
	{
       ...
       candidates for being reused */
		for (idx = 0; idx < gap_size; ++idx) {
			int lev =
				dd_get_level_at_index(self->dd, idx + gap_idx);
			nusmv_assert(lev != -1);
			...
			if ((prev_lev != INVALID_LEV && lev != prev_lev + 1) ||
				...
				if (lev_count >= idx_chunk_size &&
				    VarsHandler_can_group(
					    self->dd_vars_hndr, curr_lev_init,
     ...
     of gaps. */
	{
		...
		if (size_min != -1) {
			...
			for (i = 0; i < block_size; i += idx_chunk_size) {
				int idx = dd_get_index_at_level(self->dd,
								lev_min + i);
		...
		real_lev = bdd_enc_create_block(
			self, layer, lev_min, block_size, idx_chunk_size, true);
				...
				size_t idx_chunk_size, boolean shared)
{
	...
	GroupInfo_ptr gid = VarsHandler_reserve_group(self->dd_vars_hndr,
						      lev_min, block_size,
						      idx_chunk_size, shared,
						      &real_lev);
	array_t *blocks =
		(array_t *)find_assoc(self->layer2groups, (node_ptr)layer);
	if (blocks == (array_t *)NULL) {
		/* creates one */
		blocks = array_alloc(GroupInfo_ptr, 1);
		nusmv_assert((array_t *)NULL != blocks);
		insert_assoc(self->layer2groups, (node_ptr)layer,
			     (node_ptr)blocks);
	}
	...
	if (opt_verbose_level_gt(opts, 4)) {
		...
		Logger_log(
			logger,
			"BddEnc: created a (%s) block of %" PRIuPTR
			" variables: "
			"Asked from level %d, obtained from level %d to level %" PRIuPTR
			"\n",
			shared ? "shared" : "not shared", block_size, lev_min,
			real_lev, real_lev + block_size - 1);
	}
...
*/
static void bdd_enc_add_state_var(BddEnc_ptr self, SymbLayer_ptr layer,
				  node_ptr name, int curr_index, int next_index)
{
	NuSMVEnv_ptr env = ENV_OBJECT(self)->environment;
	const NodeMgr_ptr nodemgr =
		NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
	...
	if (opt_verbose_level_gt(opts, 4)) {
		...
		Logger_nlog(
			...
			"level %d\n",
			name, curr_index,
			dd_get_level_at_index(self->dd, curr_index));
	}
	...
	if (opt_verbose_level_gt(opts, 4)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "  BDD next state variable index %d, level %d\n",
			   next_index,
			   dd_get_level_at_index(self->dd, next_index));
	}
					      ...
					      add_ptr next)
{
	...
	if (self->next_state_vars_add != (add_ptr)NULL) {
		add_and_accumulate(self->dd, &(self->next_state_vars_add),
				   next);
	} else
...
static void bdd_enc_remove_input_var(BddEnc_ptr self, node_ptr name)
{
	...
	bdd_enc_remove_var_from_minterm(
		self, index, self->minterm_state_frozen_input_vars,
		self->minterm_state_frozen_input_vars_dim);
...
static void bdd_enc_remove_state_var(BddEnc_ptr self, node_ptr name)
{
	...
	bdd_enc_remove_var_from_minterm(
		self, curr_index, self->minterm_state_frozen_input_vars,
		self->minterm_state_frozen_input_vars_dim);
...
static void bdd_enc_remove_var(BddEnc_ptr self, node_ptr name)
{
	...
	else if (SymbTable_is_symbol_input_var(BASE_ENC(self)->symb_table,
					       name)) {
		bdd_enc_remove_input_var(self, name);
	} else {
		nusmv_assert(SymbTable_is_symbol_frozen_var(
			BASE_ENC(self)->symb_table, name));
				     ...
				     boolean *layers_interleaved)
{
	...
	node_ptr sorting_cache =
		Nil; /* used only if bit-interleaving is set up */
	...
	*layers_interleaved = false;
	...
	NODE_LIST_FOREACH(layers, layer_iter)
	{
		...
		int i;
		SymbLayer_ptr layer =
			SYMB_LAYER(NodeList_get_elem_at(layers, layer_iter));
       ...
       boolean layer for it. */
		...
		while (types_array[i] != STT_NONE) {
			...
			{
				node_ptr var =
					SymbLayer_iter_get_symbol(layer, &iter);

				if (SymbTable_is_symbol_bool_var(symb_table,
								 var)) {
					if (isBitInterleaved) {
						Enc_append_bit_to_sorted_list(
							symb_table, result, var,
							&sorting_cache);
					} else
						NodeList_append(
							result,
							var); /* pushes it as it is */
				} else {
					/* Skip variables that are not booleanizable. */
					if (Compile_is_expr_booleanizable(
						    symb_table, var,
						    !opt_bdd_encoding_word_bits(
							    opts),
						    cache)) {
               ...
               Grouping is done by caller function */
						...
						NODE_LIST_FOREACH(bits,
								  bit_iter)
						{
							node_ptr var =
								NodeList_get_elem_at(
									bits,
									bit_iter);
							if (isBitInterleaved) {
								Enc_append_bit_to_sorted_list(
...
*/
static NodeList_ptr bdd_enc_get_var_booleanizations(BddEnc_ptr self, Set_t vars)
{
	NodeList_ptr result;
		...
		if (SymbTable_is_symbol_bool_var(symb_table, var)) {
			...
			if (Compile_is_expr_booleanizable(symb_table, var,
				...
				NODE_LIST_FOREACH(bits, bit_iter)
				{
					...
					bvar = NodeList_get_elem_at(bits,
								    bit_iter);
					NodeList_append(
						result,
						bvar); /* pushes it as it is */
				}
				NodeList_destroy(bits);
			}
		}
	...
	boolean *interleaved)
{
	...
	for (iter = NodeList_get_first_iter(BASE_ENC(self)->committed_layers);
       ...
       that boolean layer exists, than com_layer is not a boolean layer */
		...
		if (SymbTable_get_layer(BASE_ENC(self)->symb_table,
					bool_name) != SYMB_LAYER(NULL)) {
     ...
     group_layers */
	{
         ...
         */
				for (gr_id = curr_group; gr_id <= group;
				     ++gr_id) {
					/*
           ...
           */
					...
					gr_vars = OrdGroups_get_vars_in_group(
						old_group, gr_id);

					for (gr_iter = NodeList_get_first_iter(
						     gr_vars);
					     !ListIter_is_end(gr_iter);
					     gr_iter = ListIter_get_next(
						     gr_iter)) {
						var_name = NodeList_get_elem_at(
							gr_vars, gr_iter);

						if (NodeList_belongs_to(
							    all_po_vars,
							    var_name) &&
						    !NodeList_belongs_to(
							    res_ord_vars,
               ...
               */
							if (new_group_id ==
							    -1) {
								new_group_id = OrdGroups_create_group(
									res_group);
							}
				...
				curr_group =
					group +
					1; /* passes to the following group */
			} /* if (-1 == group) */
						      ...
						      boolean *interleaved)
{
	...
	NodeList_remove_elem_at(layer_list,
				NodeList_get_first_iter(layer_list));
     ...
     group_layers */
	{
		...
		while (!ListIter_is_end(iter)) {
			...
			if (group == -1) {
				...
				if (!NodeList_belongs_to(res_ord_vars, name)) {
					NodeList_append(res_ord_vars, name);
					if (NodeList_belongs_to(new_po_vars,
								name)) {
						if (BoolEnc_is_var_bit(bool_enc,
								       name)) {
							node_ptr scalar_name =
								BoolEnc_get_scalar_var_from_bit(
									bool_enc,
									name);
							if (curr_scalar_var ==
								    Nil ||
							    scalar_name !=
								    curr_scalar_var) {
								curr_scalar_group = OrdGroups_create_group(
									res_group);
								curr_scalar_var =
									scalar_name;
							}
           ...
           up to the group that has been just found */
             ...
             only if it contains at least one variable */
					...
					gr_vars = OrdGroups_get_vars_in_group(
						self->ord_groups, gr_id);
					gr_iter = NodeList_get_first_iter(
						gr_vars);
					while (!ListIter_is_end(gr_iter)) {
						node_ptr var_name =
							NodeList_get_elem_at(
...
static void bdd_enc_lock_reordering(BddEnc_ptr self)
{
	if (self->reord_locked_num == 0) {
		self->reord_status = dd_reordering_status(
			self->dd, &(self->curr_reord_type));
...
static void bdd_enc_insert_gap(BddEnc_ptr self, int index, size_t gap_size)
{
	...
	while (!ListIter_is_end(iter)) {
		...
		if (index < iter_idx) {
			if (prev != LIST_ITER(NULL)) { /* not to the head */
				...
				el = NodeList_get_elem_at(self->index_gaps,
							  prev);
				...
				nusmv_assert(
					index >=
					prev_idx + prev_size); /* no overlap */
				if (index == prev_idx + prev_size) {
					/* attaches to the previous gap */
					setcdr(el, NODE_FROM_INT(prev_size +
								 gap_size));
				} else {
	...
	if (!inserted) { /* to the tail */
		if (prev != LIST_ITER(NULL)) {
			node_ptr el = NodeList_get_elem_at(self->index_gaps,
							   prev); /* last */
...
static void bdd_enc_remove_gap(BddEnc_ptr self, int index, size_t size)
{
	...
	NODE_LIST_FOREACH(self->index_gaps, iter)
	{
		...
		if ((index >= gap_idx) && (index < gap_idx + gap_size)) {
			...
			nusmv_assert(size <= gap_idx + gap_size -
						     index); /* enough space */

			if (index == gap_idx &&
			    size == (unsigned int)gap_size) {
				/* removes the whole gap */
				node_ptr el;
				el = NodeList_remove_elem_at(self->index_gaps,
							     iter);
			...
			if (index == gap_idx) { /* at the begin */
				...
				setcar(el, NODE_FROM_INT(gap_idx + size));
				   ...
				   gap_idx + gap_size) { /* at the end */
				setcdr(el, NODE_FROM_INT(gap_size - size));
			} else { /* in the middle, splits the gap */
				setcdr(el,
				       NODE_FROM_INT(
					       index -
					       gap_idx)); /* left gap adjust size */

				/* adds a new gap */
				NodeList_insert_after(
					self->index_gaps, iter,
					cons(nodemgr,
					     NODE_FROM_INT(index + size),
					     NODE_FROM_INT(gap_size -
							   (index + size -
							    gap_idx))));
			}
	...
	ErrorMgr_internal_error(
		errmgr, "bdd_enc_remove_gap: gap index=%d size=%d not found",
...
static int bdd_enc_name_to_index(const BddEnc_ptr self, node_ptr name)
{
	...
	if (node_get_type(name) == NEXT) {
       ...
       and that each correspond to the other */
		int curr_idx =
			NODE_TO_INT(find_assoc(self->name2index, car(name)));
		nusmv_assert(curr_idx != 0);
		nusmv_assert((array_fetch(int, self->current2next, curr_idx) ==
			      index) &&
			     (array_fetch(int, self->next2current, index) ==
			      curr_idx));
	}
					  ...
					  add_ptr cube)
{
	...
	if (add_isleaf(cube)) {
		...
		if (add_isleaf(var_encoding)) {
			...
			t = bdd_enc_get_var_mask_recur(
				self, var_encoding, add_then(self->dd, cube));
			nusmv_assert(t != (add_ptr)NULL);

			res = add_build(self->dd, add_index(self->dd, cube),
					zero, t);
         ...
         to fill */
			...
			t = bdd_enc_get_var_mask_recur(
				self, add_then(self->dd, var_encoding),
				add_then(self->dd, cube));
			nusmv_assert(t != (add_ptr)NULL);

			e = bdd_enc_get_var_mask_recur(
				self, add_else(self->dd, var_encoding),
				add_then(self->dd, cube));
			nusmv_assert(e != (add_ptr)NULL);

			res = add_build(self->dd, add_index(self->dd, cube), t,
					e);
			nusmv_assert(res != (add_ptr)NULL);
				 ...
				 node_ptr context)
{
	...
	if ((res == (AddArray_ptr)NULL) || (res == BDD_ENC_EVALUATING)) {
		...
		if (res == BDD_ENC_EVALUATING) {
			...
			ErrorMgr_rpterr(
				errmgr,
				"WARNING : THE SAME EXPR IS EVALUATED TWICE!\n");
		}
		temp = nusmv_yylineno;
...

{
...
#if __BDDENC_LAZY_COMMIT_LAYER__
		constant = lazy_commit_layer_and_get_add(self, resName,
							 __EVAL_CONSTANT__);
#else
	...
	if (ResolveSymbol_is_parameter(rs)) {
		param = SymbTable_get_flatten_actual_parameter(
			BASE_ENC(self)->symb_table, resName);
	}
				       ...
				       node_ptr ctx)
{
	...
	case BIT: {
		...
		rs = SymbTable_resolve_symbol(BASE_ENC(self)->symb_table, expr,
					      ctx);
		name = ResolveSymbol_get_resolved_name(rs);
#if __BDDENC_LAZY_COMMIT_LAYER__
		return AddArray_from_add(lazy_commit_layer_and_get_add(
			self, name, __EVAL_VAR__));
#else
	...
	case ARRAY: {
		...
		if (ResolveSymbol_is_undefined(rs)) {
         ...
         function is to be invoked */
			node_ptr tmp = Compile_FlattenSexp(
				BASE_ENC(self)->symb_table, expr, ctx);
			nusmv_assert(
				tmp !=
				expr); /* loop in recursion is impossible */
			return bdd_enc_eval_recur(self, tmp, Nil);
		} else {
			/* array is actually identifier => process it with other identifiers */
			return bdd_enc_eval_recur_case_dot_array(self, expr,
								 ctx);
		}
	}
	...
	case CONS:
		if (Nil ==
		    cdr(expr)) { /* list of one element. return the element */
    ...
    */
	...
	case EQUAL:
		...
		if (bdd_enc_is_bit_vector(self, car(expr), ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_equal, expr, ctx);
		}
		/* This is a usual (scalar) expression */
		else
			return bdd_enc_binary_node_op(self, node_equal, expr,
						      ctx);
	...
	case NOTEQUAL:
		...
		if (bdd_enc_is_bit_vector(self, car(expr), ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_not_equal, expr, ctx);
		} else
			return bdd_enc_binary_node_op(self, node_not_equal,
						      expr, ctx);
	...
	case PLUS:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_plus, expr, ctx);
		} else
			return bdd_enc_binary_node_op(self, node_plus, expr,
						      ctx);

	case UMINUS:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_unary_vector_op(
				self, AddArray_word_unary_minus, expr, ctx);
		}
	...
	case MINUS:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_minus, expr, ctx);
		} else
			return bdd_enc_binary_node_op(self, node_minus, expr,
						      ctx);

	case TIMES:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_times, expr, ctx);
		} else
			return bdd_enc_binary_node_op(self, node_times, expr,
						      ctx);
	...
	case DIVIDE:
		if (bdd_enc_is_bit_vector(self, expr, ctx, &tmp_bool)) {
			...
			return bdd_enc_binary_node_op(self, node_divide, expr,
						      ctx);
	...
	case MOD:
		if (bdd_enc_is_bit_vector(self, expr, ctx, &tmp_bool)) {
			...
			return bdd_enc_binary_node_op(self, node_mod, expr,
						      ctx);

	case LSHIFT:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_left_shift, expr, ctx);
		} else
			ErrorMgr_internal_error(
				errmgr, "l-shift has always to be a word exp");
	...
	case RSHIFT:
		if (bdd_enc_is_bit_vector(self, expr, ctx, &tmp_bool)) {
			...
			ErrorMgr_internal_error(
				errmgr, "r-shift has always to be a word exp");

	case LROTATE:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_left_rotate, expr, ctx);
		} else
			ErrorMgr_internal_error(
				errmgr, "l-rotate has always to be a word exp");

	case RROTATE:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_right_rotate, expr, ctx);
		} else
			ErrorMgr_internal_error(
				errmgr, "r-rotate has always to be a word exp");

	case CONCATENATION:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_concatenation, expr, ctx);
		} else
			ErrorMgr_internal_error(
				errmgr,
	...
	case BIT_SELECTION:
		if (bdd_enc_is_bit_vector(self, expr, ctx, NULL)) {
			return bdd_enc_binary_vector_op(
				self, AddArray_word_bit_selection, expr, ctx);
		} else
			ErrorMgr_internal_error(
				errmgr,
...
#else
			add_ptr add = BddEncCache_lookup_boolean_var(
				self->cache, expr);
#endif
			if (add != (add_ptr)NULL)
	...
	case EQDEF: {
		...
		{
			ResolveSymbol_ptr rs;
			rs = SymbTable_resolve_symbol(
				BASE_ENC(self)->symb_table, t2, ctx);
			name = ResolveSymbol_get_resolved_name(rs);
		}

		if (!SymbTable_is_symbol_declared(BASE_ENC(self)->symb_table,
						  name)) {
		...
		if (opt_verbose_level_gt(opts, 4)) {
			...
			Logger_nlog(logger, wffprint,
				    "evaluating operands of assignment of %N\n",
				    t1);
		}
       ...
       assignment (lhs := rhs) */
		{
			...
			nusmv_assert(AddArray_get_size(lhs) ==
				     AddArray_get_size(rhs));
         ...
         for the lhs */
			...
			case SYMB_TYPE_ENUM:
				Utils_set_data_for_range_check(
					env, t2, /* var and range */
					SymbType_get_enum_type_values(type));
				add_walkleaves(self->dd, bdd_enc_range_check,
					       AddArray_get_add(rhs));
				break;
			...
			if (opt_verbose_level_gt(opts, 4)) {
				...
				Logger_nlog(
					logger, wffprint,
					"construction of assignment of %N:\n",
					t1);
			}
      ...
      */
			{
				int i;
				res = add_true(self->dd);
				for (i = AddArray_get_size(lhs) - 1; i >= 0;
				     --i) {
					/* [MRMR]: With words we cannot have non-determinism. */
					add_ptr bit = add_apply(
						self->dd, node_setin,
						AddArray_get_n(lhs, i),
						AddArray_get_n(rhs, i));
					add_ptr tmp =
						add_and(self->dd, res, bit);
					add_free(self->dd, res);
	...
	case NUMBER:
		return AddArray_from_add(
			add_leaf(self->dd, find_atom(nodemgr, expr)));
	...
	case SWCONST: {
       ...
       rid of SWCONST and UWCONST */
		node_ptr wn = Compile_FlattenSexp(BASE_ENC(self)->symb_table,
						  expr, ctx);
	...
	case WRESIZE: {
		boolean isWord =
			bdd_enc_is_bit_vector(self, car(expr), ctx, &tmp_bool);
    ...
    */
	...
	case TWODOTS: /* this expression is used in bit selection */
	{
		...
		if (t == Nil) {
			ErrorMgr_rpterr(errmgr, "empty range: %d..%d", dim1,
					dim2);
		}
				     ...
				     node_ptr context, boolean *isSigned)
{
	...
	if (type == SYMB_TYPE(NULL) || SymbType_is_error(type)) {
		...
		ErrorMgr_internal_error(
			errmgr, "bdd_enc_is_bit_vector: invalid or empty type");
	}
				       ...
				       boolean is_scalar)
{
	...
	if (add_isleaf(add_expr)) {
		...
		/* checks boolean case */
		if (ExprMgr_is_true(exprs, leaf) ||
		    ExprMgr_is_false(exprs, leaf)) {
		...
		if (CONS == node_get_type(leaf)) {
			nusmv_assert(llength(leaf) >
				     1); /* singleton are not allowed here */
           ...
           of values to a set and returns it. */
				node_ptr leaf_iter;
				result = Nil;
				for (leaf_iter = leaf; Nil != leaf_iter;
				     leaf_iter = cdr(leaf_iter)) {
					node_ptr elem;

					nusmv_assert(CONS ==
						     node_get_type(leaf_iter));
           ...
           boolean encoding of the non-deterministic set */
             ...
             "v" and "v ? F : T" to "!v" */
					det_enc = ExprMgr_simplify(
						exprs, symb_table, det_enc);
				}
           ...
           determinization layer */
				...
				{
					node_ptr bit = (node_ptr)Set_GetMember(
						det_vars, det_vars_iter);
					SymbLayer_declare_state_var(
						det_layer, bit,
						SymbType_create(
							env, SYMB_TYPE_BOOLEAN,
							Nil));
				}
		...
		t = bdd_enc_add2expr_recur(
			self, det_layer, add_then(dd, add_expr), lc, is_scalar);
		if (t == (node_ptr)NULL)
			return (node_ptr)NULL;

		e = bdd_enc_add2expr_recur(
			self, det_layer, add_else(dd, add_expr), lc, is_scalar);
		if (e == (node_ptr)NULL)
			return (node_ptr)NULL;
		...
		result = NODE_PTR(
			ExprMgr_simplify(exprs, symb_table,
					 ExprMgr_ite(exprs, EXPR(var), EXPR(t),
						     EXPR(e), symb_table)));
		if (result == (node_ptr)NULL)
			return (node_ptr)NULL;

		if (st_add_direct(lc, (char *)add_expr, (char *)result) ==
		    ST_OUT_OF_MEM) {
			ErrorMgr_internal_error(
				errmgr,
	...
	for (iter = NodeList_get_first_iter(BASE_ENC(self)->committed_layers);
		...
		if ((array_t *)NULL != groups) {
			...
			arrayForEachItem(GroupInfo_ptr, groups, giter, binfo)
			{
				/* this releases and frees binfo which was returned by
           VarsHandler_reserve_group */
				VarsHandler_dissolve_group(self->dd_vars_hndr,
							   binfo);
			}
       ...
       * have groups within this layer */
		...
		insert_assoc(self->layer2groups, (node_ptr)com_layer,
			     (node_ptr)NULL);
	}
	bdd_enc_unlock_reordering(self);
}
					     ...
					     boolean use_dot)
{
	...
	if (use_dot) {
		int tmp = dd_dump_dot(dd, adr_size, dd_array, inames, labels,
				      outfile);
		if (tmp == 0)
			res = (tmp == 0) ? 1 : 0;
	} else {
		int tmp = dd_dump_davinci(dd, adr_size, dd_array, inames,
					  labels, outfile);
		if (tmp == 0)
			res = (tmp == 0) ? 1 : 0;
	}
