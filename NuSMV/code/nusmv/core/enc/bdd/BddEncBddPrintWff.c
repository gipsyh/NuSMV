			       ...
			       OStream_ptr out)
{
	...
	if (print_models) {
		...
		BddEnc_print_bdd_wff(self, phi, scalar_vars, true, false, 0,
				     out);
...
)
{
   ...
   **/
	if (do_sharing) {
		...
		dag_expr = Compile_convert_to_dag(
			env, BaseEnc_get_symb_table(BASE_ENC(self)), expr,
			dag_info_hash, defines_hash);
	...
	if (do_indent) {
		const MasterPrinter_ptr iwffprint = MASTER_PRINTER(
			NuSMVEnv_get_value(env, ENV_IWFF_PRINTER));
   ...
   **/
	if (do_sharing) {
		OStream_printf(out, "\n");
		Compile_write_dag_defines(env, OStream_get_stream(out),
					  defines_hash);
	}
...
)
{
	...
	res = bdd_enc_bdd_to_wff_rec(self, preprocessed_vars, bdd,
				     memoization_hash);
	...
	clear_assoc_and_free_entries_arg(
		memoization_hash, bdd_enc_hash_free_bdd_counted, (char *)dd);
...
)
{
	...
	NODE_LIST_FOREACH(vars, i)
	{
		...
		if (is_next) {
			...
			ErrorMgr_internal_error(
				errmgr,
				"No NEXT variables allowed here (got %s)",
				var_name);
			FREE(var_name);
		}
		...
		if (is_enc) {
			...
			ErrorMgr_internal_error(
				errmgr,
				"No bit encoding variables allowed here (got %s)",
				var_name);
			FREE(var_name);
		}
     ...
     */
		...
		if (is_word) {
			...
			{
				node_ptr b =
					(node_ptr)NodeList_get_elem_at(bits, j);
       ...
       variable. */
			...
			if (SymbTable_is_symbol_state_var(st, v)) {
				NodeList_append(res, find_node(nodemgr, NEXT, v,
							       Nil));
			}
		}
	}
...

{
   ...
   */
     ...
     **/
		...
		bdd_ptr essentials =
			bdd_enc_get_scalar_essentials(self, local_bdd, vars);
		if (!bdd_is_false(dd, essentials) &&
		    !bdd_is_true(dd, essentials)) {
			node_ptr avars = BddEnc_assign_symbols(
				self, essentials, vars, true, NULL);
			...
			for (p = avars; Nil != p; p = cdr(p)) {
				...
				boolean is_next =
					(node_get_type(var_expr) == NEXT);
				node_ptr var_value = cdr(v);
				node_ptr var_name = (is_next) ? car(var_expr) :
								var_expr;

				/* variables are _not_ necessarily bits here... */
				boolean is_bit = SymbTable_is_symbol_bool_var(
					st, var_name);

				if (is_bit) {
					boolean is_enc = BoolEnc_is_var_bit(
						benc, var_name);
				...
				{
					bdd_ptr cube =
						bdd_support(dd, essentials);
					bdd_ptr tmp = bdd_forsome(dd, local_bdd,
								  cube);
...

		{
			...
			node_ptr root_node =
				BddEnc_get_var_name_from_index(self, root_ndx);
       ...
       */
			...
			node_ptr var_name = (is_next) ? car(root_node) :
							root_node;
       ...
       */
			...
			if (is_enc_bit) {
				node_ptr x = BoolEnc_get_scalar_var_from_bit(
					benc, var_name);
				SymbType_ptr x_type =
					SymbTable_get_var_type(st, x);
			...
			if ((is_pure_bit) || (is_word_bit)) {
				...
				cofact_expr = ExprMgr_ite(
					exprs, var_expr,
					bdd_enc_bdd_to_wff_rec(self, vars,
							       t_bdd, cache),
					bdd_enc_bdd_to_wff_rec(self, vars,
							       e_bdd, cache),
					st);
			...
			/* scalar case */
				...
				var_name = BoolEnc_get_scalar_var_from_bit(
					benc, var_name);
				...
				root_add =
					BddEnc_expr_to_add(self, var_expr, Nil);
				...
				root_type =
					SymbTable_get_var_type(st, var_name);
				range = SymbType_get_enum_type_values(
					root_type);
