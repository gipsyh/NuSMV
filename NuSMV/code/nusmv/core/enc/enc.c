...
void Enc_init_bdd_encoding(NuSMVEnv_ptr env, const char *input_order_file_name)
{
	...
	dd_vars_handler =
		VARS_HANDLER(NuSMVEnv_get_value(env, ENV_DD_VARS_HANDLER));
	opts = OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));
	st = SYMB_TABLE(NuSMVEnv_get_value(env, ENV_SYMB_TABLE));
	...
	if (!util_is_string_null(input_order_file_name)) {
		...
		ord_groups = enc_utils_parse_ordering_file(
			env, input_order_file_name, bool_enc);
		...
		ord_groups = enc_utils_parse_ordering_file(env, opts_order_file,
							   bool_enc);
		...
		if (bse != BDD_STATIC_ORDER_HEURISTICS_NONE) {
			...
			if (!NuSMVEnv_has_value(env, ENV_FLAT_HIERARCHY)) {
				...
				FlatHierarchy_ptr fh = FLAT_HIERARCHY(
					NuSMVEnv_get_value(env,
							   ENV_FLAT_HIERARCHY));
				enc_construct_bdd_order_statically(env, fh,
								   ord_groups);
			}
		} else {
...
void Enc_quit_encodings(NuSMVEnv_ptr env)
{
	if (NuSMVEnv_has_value(env, ENV_BDD_ENCODER)) {
		BddEnc_ptr bdd_enc =
			BDD_ENC(NuSMVEnv_remove_value(env, ENV_BDD_ENCODER));
		BddEnc_destroy(bdd_enc);
	}

	if (NuSMVEnv_has_value(env, ENV_BE_ENCODER)) {
		BeEnc_ptr be_enc =
			BE_ENC(NuSMVEnv_remove_value(env, ENV_BE_ENCODER));
				   ...
				   node_ptr *sorting_cache)
{
  ...
  */
     ...
     */
		...
		nusmv_assert(cdr(cache) == Nil ||
			     node_get_type(cdr(cache)) < bit_number);

		setcdr(cache, new_node(nodemgr, bit_number, (node_ptr)iter,
				       cdr(cache)));
		return;
	}
}
...
VarsOrdType Enc_string_to_vars_ord(const char *str, StreamMgr_ptr streams)
{
	...
	if (strcmp(str, VARS_ORD_STR_LEXICOGRAPHIC) == 0) {
		if (STREAM_MGR(NULL) != streams) {
			StreamMgr_print_error(
				streams,
				"Warning: value \"" VARS_ORD_STR_LEXICOGRAPHIC
...
{
	return VARS_ORD_STR_INPUTS_BEFORE ", " VARS_ORD_STR_INPUTS_AFTER
					       ...
					       OrdGroups_ptr ord_groups)
{
	...
	/* get rid of bits in the list */
	{
		...
		while (!ListIter_is_end(iter)) {
			node_ptr var =
				NodeList_get_elem_at(original_var_order, iter);
	...
	PredicateExtractor_compute_preds_from_hierarchy(extractor,
							flat_hierarchy);
	...
	while (NodeList_get_length(original_var_order) != 0) {
		...
		Set_t cluster =
			PredicateExtractor_get_var_cluster(extractor, var);

		if (NULL == cluster) { /* not grouped var */
			NodeList_append(subListVars, var);

			NodeList_remove_elem_at(
				original_var_order,
				NodeList_get_first_iter(original_var_order));
    ...
    */
		...
		NODE_LIST_FOREACH(subListBits, subIter)
		{
         ...
         in bdd_enc_sort_variables_and_groups_according */
			...
			nusmv_assert(-1 ==
				     OrdGroups_get_var_group(ord_groups, var));
