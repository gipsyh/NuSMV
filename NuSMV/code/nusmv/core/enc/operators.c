...
/*---------------------------------------------------------------------------*/
static node_ptr node_word_full_adder(node_ptr ai, node_ptr bi,
				     node_ptr carry_in, node_ptr *carry_out,
...
static node_ptr node_word_signed_divide_reminder_simple(node_ptr a, node_ptr b,
							node_ptr *reminder,
							const NuSMVEnv_ptr env);
static node_ptr node_word_signed_divide_reminder_hardware(
	node_ptr a, node_ptr b, node_ptr *reminder, const NuSMVEnv_ptr env);

static node_ptr node_word_signed_op(node_ptr a, node_ptr b,
				    node_ptr (*op)(node_ptr, node_ptr,
						   const NuSMVEnv_ptr),
				    const NuSMVEnv_ptr env);
...
node_ptr node_and(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n1) == FAILURE)
		return n1; /* error in previous expr */
	if (node_get_type(n2) == FAILURE)
		return n2; /* error in previous expr */

	if (_is_bool(n1) && _is_bool(n2)) {
		return ExprMgr_and(exprs, n1, n2);
	} else
		ErrorMgr_error_not_proper_numbers(errmgr, "&", n1, n2);
		  ...
		  const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n) == FAILURE)
		return n; /* error in previous expr */

	if (_is_bool(n)) {
		return ExprMgr_not(exprs, n);
	} else
		ErrorMgr_error_not_proper_number(errmgr, "!", n);
	return (node_ptr)NULL; /* return something to suppress warnings */
}
...
node_ptr node_xor(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (_is_bool(n1) && _is_bool(n2)) {
		...
		ErrorMgr_error_not_proper_numbers(errmgr, "xor", n1, n2);
	return (node_ptr)NULL; /* return something to suppress warnings */
}
...
node_ptr node_implies(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (_is_bool(n1) && _is_bool(n2)) {
		...
		ErrorMgr_error_not_proper_numbers(errmgr, "implies", n1, n2);
	return (node_ptr)NULL; /* return something to suppress warnings */
}
			  ...
			  const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n) == NUMBER) {
		return find_node(nodemgr, NUMBER,
				 NODE_FROM_INT(-NODE_TO_INT(car(n))), Nil);
	} else
		ErrorMgr_error_not_proper_number(errmgr, "- (unary)", n);
	return (node_ptr)NULL; /* return something to suppress warnings */
}
...
node_ptr node_plus(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
		return find_node(nodemgr, NUMBER,
				 NODE_FROM_INT(NODE_TO_INT(car(n1)) +
					       NODE_TO_INT(car(n2))),
				 Nil);
	} else
		ErrorMgr_error_not_proper_numbers(errmgr, "+", n1, n2);
...
node_ptr node_divide(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n2) == NUMBER && 0 == NODE_TO_INT(car(n2))) {
		return ErrorMgr_failure_make(errmgr, "Division by zero",
					     FAILURE_DIV_BY_ZERO,
					     nusmv_yylineno);
	}
	...
	if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
		...
		/* the semantics of the division is from old version of NuSMV */
			...
			int result =
				a / b -
				(r < 0); /*IF r < 0 THEN a/b - 1 ELSE a/b */
			return find_node(nodemgr, NUMBER, NODE_FROM_INT(result),
					 Nil);
		}
	} else
		ErrorMgr_error_not_proper_numbers(errmgr, "/", n1, n2);
...
node_ptr node_mod(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
		...
		/* the semantics of the division is from old version of NuSMV */
			...
			if (r < 0)
				r += b;
			return find_node(nodemgr, NUMBER, NODE_FROM_INT(r),
					 Nil);
		}
	} else
		ErrorMgr_error_not_proper_numbers(errmgr, "mod", n1, n2);
	return (node_ptr)NULL; /* return something to suppress warnings */
}
...
node_ptr node_bit_range(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
		...
		ErrorMgr_error_not_proper_numbers(errmgr, "bit-selection-range",
						  n1, n2);
	return (node_ptr)NULL; /* return something to suppress warnings */
}
...
node_ptr node_union(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(n1) == FAILURE)
		return n1; /* error in operand */
	if (node_get_type(n2) == FAILURE)
		return n2; /* error in operand */

	/* convert singleton elements to lists */
	if (node_get_type(n1) != CONS)
		n1 = find_node(nodemgr, CONS, n1, Nil);
	if (node_get_type(n2) != CONS)
		n2 = find_node(nodemgr, CONS, n2, Nil);
  ...
  */
	...
	while (n1 != Nil && n2 != Nil) {
		if (car(n1) == car(n2)) {
			...
			n2 = cdr(n2);
			   ...
			   car(n2)) { /* < is used because the list will be reversed */
			tmp = cons(nodemgr, car(n1), tmp);
			n1 = cdr(n1);
		} else { /*car(n2) > car(n1) */
	...
	if (Nil == n1)
		n1 = n2; /* remaining elements (they were in n1 or n2) */
...
node_ptr node_setin(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env)
{
	...
	/* convert singleton elements to lists */
	if (CONS != node_get_type(n1))
		n1 = find_node(nodemgr, CONS, n1, Nil);
	if (CONS != node_get_type(n2))
		n2 = find_node(nodemgr, CONS, n2, Nil);
...
node_ptr node_word_create(node_ptr bitval, size_t w, const NuSMVEnv_ptr env)
{
	...
	node_ptr enc = Nil;
	for (; w > 0; --w) {
		enc = find_node(nodemgr, CONS, bitval, enc);
	}
				    ...
				    const NuSMVEnv_ptr env)
{
	...
	return find_node(nodemgr, UNSIGNED_WORD, l,
			 find_node(nodemgr, NUMBER, NODE_FROM_INT(w), Nil));
}
				       ...
				       const NuSMVEnv_ptr env)
{
	...
	WordNumber_ptr wn =
		WordNumberMgr_integer_to_word_number(words, value, width);
...
node_ptr node_word_create_from_array(array_t *arr, const NuSMVEnv_ptr env)
{
	...
	arrayForEachItem(node_ptr, arr, i, bit)
	{
...
static node_ptr _node_word_apply_unary_aux(node_ptr e, void *arg)
{
	...
	switch (apply_op) {
	case NOT:
		return ExprMgr_not(exprs, e);
	default:
		return find_node(nodemgr, apply_op, e, Nil);
	}
}
...
static node_ptr _node_word_apply_binary_aux(node_ptr e1, node_ptr e2, int op,
					    const NuSMVEnv_ptr env)
{
	const ExprMgr_ptr exprs =
		EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
	...
	switch (op) {
	case AND:
		return ExprMgr_and(exprs, e1, e2);
	case OR:
		return ExprMgr_or(exprs, e1, e2);
	case XOR:
		return ExprMgr_xor(exprs, e1, e2);
	case XNOR:
		return ExprMgr_xnor(exprs, e1, e2);
	case IMPLIES:
		return ExprMgr_implies(exprs, e1, e2);
	case IFF:
		return ExprMgr_iff(exprs, e1, e2);
	default:
		return find_node(nodemgr, op, e1, e2);
	}
}
				...
				const NuSMVEnv_ptr env)
{
	...
	res = map2_param(_node_word_apply_binary_aux, car(wenc1), car(wenc2),
			 op, env);
...
node_ptr node_word_make_conjuction(node_ptr w, const NuSMVEnv_ptr env)
{
	...
	for (w = car(w); w != Nil; w = cdr(w))
		res = ExprMgr_and(exprs, car(w), res);
	return res;
}
...
node_ptr node_word_make_disjunction(node_ptr w, const NuSMVEnv_ptr env)
{
	...
	for (w = car(w); w != Nil; w = cdr(w))
		res = ExprMgr_or(exprs, car(w), res);
	return res;
}
...
node_ptr node_word_concat(node_ptr a, node_ptr b, const NuSMVEnv_ptr env)
{
	...
	return node_word_create_from_list(append_ns(nodemgr, car(a), car(b)), w,
					  env);
}
			     ...
			     const NuSMVEnv_ptr env)
{
	...
	if (node_get_type(word) == NUMBER_UNSIGNED_WORD ||
		...
		return find_node(nodemgr, NUMBER_UNSIGNED_WORD, (node_ptr)w,
				 Nil);
	}
			 ...
			 node_ptr *carry_out, const NuSMVEnv_ptr env)
{
	...
	for (a = reverse_ns(nodemgr, car(a)), b = reverse_ns(nodemgr, car(b));
		...
		node_ptr bit_carry;
		node_ptr bit = node_word_full_adder(car(a), car(b), carry_in,
						    &bit_carry, env);
		res = find_node(nodemgr, CONS, bit, res);
		carry_in = bit_carry;
	}
...
node_ptr node_word_minus(node_ptr a, node_ptr b, const NuSMVEnv_ptr env)
{
	...
	/* a-b ==  a+(not b)+1 */
	node_ptr carry_out;
	return node_word_adder(a, node_word_not(b, env), ExprMgr_true(exprs),
			       &carry_out, env);
}
...
node_ptr node_word_uminus(node_ptr a, const NuSMVEnv_ptr env)
{
	...
	zenc = Nil;
	for (w = node_get_int(cdr(a)); w > 0; --w)
		zenc = find_node(nodemgr, CONS, zero, zenc);
	wzero = find_node(nodemgr, UNSIGNED_WORD, zenc, cdr(a));

	return node_word_adder(node_word_not(a, env), wzero,
			       ExprMgr_true(exprs), &carry_out, env);
}
...
node_ptr node_word_times(node_ptr a, node_ptr b, const NuSMVEnv_ptr env)
{
	...
	for (i = 1; i < w; ++i) {
		...
		for (k = 0; k < w - i; ++k) {
			...
			sum = node_word_full_adder(
				array_fetch(node_ptr, vab, i + k),
				ExprMgr_and(exprs, bit_a, bit_b), cin, &ctmp,
				env);
...
node_ptr node_word_unsigned_less(node_ptr a, node_ptr b, const NuSMVEnv_ptr env)
{
	...
	node_ptr carry_out;
	(void)node_word_adder(node_word_not(a, env), b, ExprMgr_false(exprs),
			      &carry_out, env);
	return carry_out;
}
				       ...
				       const NuSMVEnv_ptr env)
{
	...
	node_ptr carry_out;
	(void)node_word_adder(node_word_not(a, env), b, ExprMgr_true(exprs),
			      &carry_out, env);
	return carry_out;
}
						   ...
						   const NuSMVEnv_ptr env)
{
	...
	vrem = node_word_to_array(
		node_word_create(ExprMgr_false(exprs), width, env));
	va = node_word_to_array(a);
	vb = node_word_to_array(b);
    ...
    */
		{
			...
			node_ptr is_dividable =
				node_word_unsigned_greater_equal(rem, b, env);
			...
			for (k = width - 1, iter = car(substruction); k >= 0;
			     --k, iter = cdr(iter)) {
				node_ptr bit = ExprMgr_ite(
					exprs, is_dividable, car(iter),
     ...
     that the divisor is not equal to zero */
	{
       ...
       resulting encoding */
		...
		node_ptr dbz = ErrorMgr_failure_make(
		...
		for (i = 0; i < width; ++i) {
			...
			ite = ExprMgr_ite(exprs, b_nz,
					  array_fetch(node_ptr, vquot, i), dbz,
					  SYMB_TABLE(NULL));
			array_insert(node_ptr, vquot, i, ite);

			/* reminder */
			ite = ExprMgr_ite(exprs, b_nz,
					  array_fetch(node_ptr, vrem, i), dbz,
					  SYMB_TABLE(NULL));
							...
							const NuSMVEnv_ptr env)
{
	...
	list = Nil;
	for (iter = car(a), iter_neg = car(positive_a); iter != Nil;
	     iter = cdr(iter), iter_neg = cdr(iter_neg)) {
		node_ptr bit = ExprMgr_ite(exprs, sign_a, car(iter_neg),
					   car(iter), SYMB_TABLE(NULL));
	...
	list = Nil;
	for (iter = car(b), iter_neg = car(positive_b); iter != Nil;
	     iter = cdr(iter), iter_neg = cdr(iter_neg)) {
		node_ptr bit = ExprMgr_ite(exprs, sign_b, car(iter_neg),
					   car(iter), SYMB_TABLE(NULL));
		list = cons(nodemgr, bit, list);
	}
	list = reverse(list);
	...
	/* negate the remainder if the dividend was negative */
	{
		...
		for (i = 0; i < width; ++i) {
			...
			bit = ExprMgr_ite(exprs, sign_a, bit,
...

	{
		...
		for (i = 0; i < width; ++i) {
			...
			bit = ExprMgr_ite(exprs, diff_sign, bit,
	...
	node_ptr a, node_ptr b, node_ptr *reminder, const NuSMVEnv_ptr env)
{
	...
	ErrorMgr_rpterr(
		errmgr,
		"node_word_signed_divide_reminder_hardware is not impelemnted yet.");
	return Nil;
}
