...
node_ptr node_lt(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env) {
  ...
  if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
    return (NODE_TO_INT(car(n1)) < NODE_TO_INT(car(n2))) ? ExprMgr_true(exprs)
                                                         : ExprMgr_false(exprs);
  } else
    ErrorMgr_error_not_proper_numbers(errmgr, "<", n1, n2);
  return (node_ptr)NULL; /* return something to suppress warnings */
...
node_ptr node_plus(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env) {
  ...
  if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
    return find_node(nodemgr, NUMBER,
                     NODE_FROM_INT(NODE_TO_INT(car(n1)) + NODE_TO_INT(car(n2))),
                     Nil);
  } else
    ErrorMgr_error_not_proper_numbers(errmgr, "+", n1, n2);
  return (node_ptr)NULL; /* return something to suppress warnings */
...
node_ptr node_divide(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env) {
  ...
  if (node_get_type(n2) == NUMBER && 0 == NODE_TO_INT(car(n2))) {
    return ErrorMgr_failure_make(errmgr, "Division by zero",
                                 FAILURE_DIV_BY_ZERO, nusmv_yylineno);
  }
...
node_ptr node_bit_range(node_ptr n1, node_ptr n2, const NuSMVEnv_ptr env) {
  ...
  if (node_get_type(n1) == NUMBER && node_get_type(n2) == NUMBER) {
    ...
    ErrorMgr_error_not_proper_numbers(errmgr, "bit-selection-range", n1, n2);
  return (node_ptr)NULL; /* return something to suppress warnings */
}
...
node_ptr node_word_create_from_wordnumber(WordNumber_ptr wn,
  ...
  for (i = 0; i < w; ++i) {
    bits = find_node(nodemgr, CONS,
                     WordNumber_get_bit(wn, i) ? ExprMgr_true(exprs)
                                               : ExprMgr_false(exprs),
                     bits);
  }
...
size_t node_word_get_width(node_ptr w) { return node_get_int(cdr(w)); }
...
node_ptr node_word_apply_binary(node_ptr wenc1, node_ptr wenc2, int op,
  ...
  res =
      map2_param(_node_word_apply_binary_aux, car(wenc1), car(wenc2), op, env);
...
node_ptr node_word_adder(node_ptr a, node_ptr b, node_ptr carry_in,
  ...
  for (a = reverse_ns(nodemgr, car(a)), b = reverse_ns(nodemgr, car(b));
    ...
    node_ptr bit_carry;
    node_ptr bit =
        node_word_full_adder(car(a), car(b), carry_in, &bit_carry, env);
    res = find_node(nodemgr, CONS, bit, res);
    carry_in = bit_carry;
  }
...
node_ptr node_word_uminus(node_ptr a, const NuSMVEnv_ptr env) {
  ...
  return node_word_adder(node_word_not(a, env), wzero, ExprMgr_true(exprs),
                         &carry_out, env);
}
...
static node_ptr node_word_unsigned_divide_reminder(node_ptr a, node_ptr b,
    ...
    */
    {
      ...
      for (k = width - 1, iter = car(substruction); k >= 0;
           --k, iter = cdr(iter)) {
        node_ptr bit =
            ExprMgr_ite(exprs, is_dividable, car(iter),
                        array_fetch(node_ptr, vrem, k), SYMB_TABLE(NULL));
        array_insert(node_ptr, vrem, k, bit);
      }
    }
  } /* for i */
     ...
     that the divisor is not equal to zero */
  {
    ...
    node_ptr dbz =
        ErrorMgr_failure_make(errmgr, "Division by zero", FAILURE_DIV_BY_ZERO,
    ...
    for (i = 0; i < width; ++i) {
      ...
      /* quotient */
      ite = ExprMgr_ite(exprs, b_nz, array_fetch(node_ptr, vquot, i), dbz,
                        SYMB_TABLE(NULL));
      array_insert(node_ptr, vquot, i, ite);

      /* reminder */
      ite = ExprMgr_ite(exprs, b_nz, array_fetch(node_ptr, vrem, i), dbz,
                        SYMB_TABLE(NULL));
...
*/
static node_ptr node_word_signed_divide_reminder_simple(
    node_ptr a, node_ptr b, node_ptr *reminder, const NuSMVEnv_ptr env) {
  const ExprMgr_ptr exprs = EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
  const NodeMgr_ptr nodemgr = NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
  ...
  for (iter = car(a), iter_neg = car(positive_a); iter != Nil;
    ...
    node_ptr bit =
        ExprMgr_ite(exprs, sign_a, car(iter_neg), car(iter), SYMB_TABLE(NULL));
  ...
  for (iter = car(b), iter_neg = car(positive_b); iter != Nil;
       iter = cdr(iter), iter_neg = cdr(iter_neg)) {
    node_ptr bit =
        ExprMgr_ite(exprs, sign_b, car(iter_neg), car(iter), SYMB_TABLE(NULL));
    list = cons(nodemgr, bit, list);
  }
  list = reverse(list);
  ...
  /* negate the remainder if the dividend was negative */
  {
    ...
    for (i = 0; i < width; ++i) {
      ...
      bit = ExprMgr_ite(exprs, sign_a, bit, array_fetch(node_ptr, arr_rem, i),
                        SYMB_TABLE(NULL));
...

  {
    ...
    for (i = 0; i < width; ++i) {
      ...
      bit = ExprMgr_ite(exprs, diff_sign, bit,
                        array_fetch(node_ptr, arr_quot, i), SYMB_TABLE(NULL));
