...
static void add_array_signed_division_remainder_hardware(
	DDMgr_ptr dd, AddArray_ptr arg1, AddArray_ptr arg2,
	AddArray_ptr *quotient, AddArray_ptr *remainder);
...
AddArray_ptr AddArray_from_word_number(DDMgr_ptr dd, WordNumber_ptr wn)
{
	...
	for (i = 0; i < width; ++i) {
		node_ptr value = WordNumber_get_bit(wn, i) ?
					 ExprMgr_true(exprs) :
					 ExprMgr_false(exprs);
				 ...
				 AddArray_ptr arg2)
{
	...
	for (index = 0; index < N; ++index) {
		AddArray_set_n(res, index,
			       add_and(dd, AddArray_get_n(arg1, 0),
   ...
   Index begins with 1 because first bit has been already processed */
	for (index = 1; index < N; ++index) {
		...
		carry_in = add_false(
			dd); /* it will be de-referenced in the loop */
		...
		for (k = 0; k < N - index; ++k) {
			...
			add_array_full_adder(dd, AddArray_get_n(res, k + index),
					     AddArray_get_n(adder, k), carry_in,
					     &sum, &tmp_carry);
					   ...
					   AddArray_ptr arg2)
{
	...
	add_array_unsigned_division_remainder(dd, arg1, arg2, &quotient,
					      &remainder);
			...
			if (is_word) { /* number is a word */
				AddArray_ptr an = AddArray_from_word_number(
					...
					WordNumberMgr_integer_to_word_number(
						words, n, numWidth));
				AddArray_ptr numeqn =
					AddArray_word_equal(dd, number, an);
				numeqn_add = add_dup(AddArray_get_add(numeqn));
				AddArray_destroy(dd, numeqn);
				AddArray_destroy(dd, an);
			} else { /* number is not a word.  */
				add_ptr n_add = add_leaf(
					dd, find_node(nodemgr, NUMBER,
						      NODE_FROM_INT(n), Nil));
				numeqn_add = add_apply(dd, node_equal,
						       AddArray_get_add(number),
						       n_add);
				add_free(dd, n_add);
			}

			tmp = add_ifthenelse(dd, numeqn_add,
					     AddArray_get_n(arg, i - n), bit);
			...
			if (is_word) {
				AddArray_ptr ak = AddArray_from_word_number(
					...
					WordNumberMgr_integer_to_word_number(
						words, k, numWidth));
				AddArray_ptr aneqk =
					AddArray_word_equal(dd, number, ak);
				neqk = add_dup(AddArray_get_add(aneqk));
				AddArray_destroy(dd, aneqk);
				AddArray_destroy(dd, ak);
			} else {
				add_ptr k_add = add_leaf(
					dd, find_node(nodemgr, NUMBER,
						      NODE_FROM_INT(k), Nil));
				neqk = add_apply(dd, node_equal,
						 AddArray_get_add(number),
						 k_add);
				add_free(dd, k_add);
			}
					...
					AddArray_ptr number)
{
	...
	for (i = 0; i < width; ++i) {
		...
		for (k = maxPossibleValue; k >= 0; --k) {
			...
			tmp = add_ifthenelse(
				dd, neqk,
				AddArray_get_n(arg, ((i + k) % width)), bit);
...
AddArray_ptr AddArray_word_signed_less_equal(DDMgr_ptr dd, AddArray_ptr arg1,
					     AddArray_ptr arg2)
{
	return add_array_word_signed_comparison(
		dd, AddArray_word_unsigned_less_equal, arg1, arg2);
}
						...
						AddArray_ptr arg2)
{
	/* A >=s B === B <=s A */
	return add_array_word_signed_comparison(
		dd, AddArray_word_unsigned_less_equal, arg2, arg1);
}
					 ...
					 AddArray_ptr arg_repeat)
{
	...
	AddArray_ptr res =
		add_array_word_extend(dd, arg, arg_repeat, paddingBit);
	return res;
}
			       ...
			       AddArray_ptr _then, AddArray_ptr _else)
{
	...
	for (i = 0; i < width; ++i) {
		add_ptr add = add_ifthenelse(dd, if_add,
					     AddArray_get_n(_then, i),
					     is_else_failure ?
						     AddArray_get_add(_else) :
					 ...
					 AddArray_ptr range)
{
	...
	{
		add_ptr range_add =
			AddArray_get_add(range); /*one ADD may be in range*/
		node_ptr range_node;

		nusmv_assert(add_isleaf(
			range_add)); /* range must be a value, not ADD tree */
		...
		nusmv_assert(0 <= lowBit && lowBit <= highBit &&
			     highBit < width);
	}
					 ...
					 AddArray_ptr new_size)
{
	...
	{
		add_ptr size_add =
			AddArray_get_add(new_size); /*one ADD may be in size*/
		node_ptr size_node;

		nusmv_assert(add_isleaf(
			size_add)); /* range must be a value, not ADD tree */
...

	{
		...
		if (width < new_width) { /* signed extension */
			...
			for (; i < width; ++i) {
				AddArray_set_n(
					res, i,
					add_dup(AddArray_get_n(word, i)));
			}

			/* extends sign bit */
			for (; i < new_width; ++i) {
				AddArray_set_n(res, i,
					       add_dup(AddArray_get_n(
						       word, width - 1)));
			}
		} else {
			    ...
			    add_ptr *carry_out)
{
	...
	nusmv_assert(res != (AddArray_ptr *)NULL &&
		     carry_out != (add_ptr *)NULL);
	...
	*res = AddArray_create(width);
	...
	for (i = 0; i < width; ++i) {
		...
		add_array_full_adder(dd, AddArray_get_n(arg1, i),
				     AddArray_get_n(arg2, i), carry_in, &sum,
				     &tmp_carry);
						  ...
						  AddArray_ptr *remainder)
{
	...
	for (index = N - 1; index >= 0; --index) {
		...
		/* shift remainder left by 1 bit and add the bit arg1[index] on the left */
		{
			...
			for (k = N - 1; k > 0; --k) {
				AddArray_set_n(rem, k,
					       AddArray_get_n(rem, k - 1));
			}
    ...
    */
		{
			AddArray_ptr tmp = AddArray_word_unsigned_greater_equal(
				dd, rem, arg2);
  ...
  */
	{
		...
		for (isNotZero = add_false(dd), index = 0; index < N; ++index) {
			add_or_accumulate(dd, &isNotZero,
					  AddArray_get_n(arg2, index));
		}
		...
		for (index = 0; index < N; ++index) {
			add_ptr guarded;
			/* quotient */
			guarded = add_ifthenelse(dd, isNotZero,
						 AddArray_get_n(quot, index),
						 error);
			add_free(dd, AddArray_get_n(quot, index));
			AddArray_set_n(quot, index, guarded);
			/* remainder */
			guarded = add_ifthenelse(dd, isNotZero,
						 AddArray_get_n(rem, index),
						 error);
						       ...
						       AddArray_ptr *remainder)
{
	...
	for (index = 0; index < N; ++index) {
		...
		new = add_ifthenelse(dd, sign1, bitPositive,
				     AddArray_get_n(arg1, index));
		AddArray_set_n(arg1_positive, index, new);
		add_free(dd, bitPositive);

		bitPositive = AddArray_get_n(arg2_positive, index);
		new = add_ifthenelse(dd, sign2, bitPositive,
				     AddArray_get_n(arg2, index));
		AddArray_set_n(arg2_positive, index, new);
		add_free(dd, bitPositive);
	}

	add_array_unsigned_division_remainder(dd, arg1_positive, arg2_positive,
					      &quot, &rem);
	...
	/* negate the remainder if the dividend was negative */
	{
		...
		for (index = 0; index < N; ++index) {
			...
			new = add_ifthenelse(
				dd, sign1, AddArray_get_n(rem_neg, index), old);
	...
	/* negate the quotient if the dividend and divisor have different signs */
	{
		...
		for (index = 0; index < N; ++index) {
			...
			new = add_ifthenelse(dd, diff_signs,
					     AddArray_get_n(quot_neg, index),
					     old);
	...
	AddArray_ptr *quotient, AddArray_ptr *remainder)
{
	...
	rem = AddArray_create(N);
	for (index = 0; index < N; ++index) {
		AddArray_set_n(rem, index,
			       add_dup(AddArray_get_n(arg1, N - 1)));
	}
    ...
    */

		{
			...
			add_ptr areSignsEqual =
				add_iff(dd, AddArray_get_n(rem, N - 1),
    ...
    */
		{
			...
			AddArray_set_n(
				quot, index,
				isDividable); /* do not free isDividable */

			for (k = 0; k < N; ++k) {
				add_ptr old_bit = AddArray_get_n(rem, k);
				add_ptr new_bit = add_ifthenelse(
					dd, isDividable,
  ...
  */
	{
		...
		for (k = 0; k < N; ++k) {
			...
			add_ptr new_bit =
				add_ifthenelse(dd, areSignsEqual, old_bit,
	...
	const char *errMessage)
{
	...
	if (is_word) { /* 'number' is a word. Words (unsigned) are always >= 0 */
		if ((width >= (2ULL << (numWidth - 1)) -
         ...
         a check that number<=width is not required. */
			...
			AddArray_ptr maxValid = AddArray_from_word_number(
				dd, WordNumberMgr_integer_to_word_number(
					    words, width, numWidth));
			AddArray_ptr nlemax = AddArray_word_unsigned_less_equal(
				dd, number, maxValid);
			add_ptr cond = AddArray_get_add(
				nlemax); /* only one add can be there */
		...
		zero = add_leaf(dd, find_node(nodemgr, NUMBER, NODE_FROM_INT(0),
					      Nil));
		aw = add_leaf(dd, find_node(nodemgr, NUMBER,
					...
					AddArray_ptr number, boolean isSigned)
{
	...
	{
		add_ptr defaultBit =
			isSigned ? add_dup(AddArray_get_n(arg, width - 1)) :
				   add_false(dd);
...
*/
static inline boolean add_array_is_word(DDMgr_ptr dd, const AddArray_ptr number)
{
