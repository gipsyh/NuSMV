					  ...
					  node_ptr values, Set_t *bits)
{
	...
	var_name = SymbTable_get_determinization_var_name(
		BASE_ENC(self)->symb_table);
	return bool_enc_compute_set_encoding(self, values, var_name, 0, bits,
					     true);
}
					 ...
					 const BitValues_ptr bit_values)
{
	...
	if (SymbType_is_enum(var_type)) {
		while (true) {
			...
			case IFTHENELSE: {
				int bit_index = BoolEnc_get_index_from_bit(
					self, car(car(enc)));
	...
	if (SymbType_is_word(var_type)) {
		...
		const WordNumberMgr_ptr wmgr = WORD_NUMBER_MGR(
       ...
       check the type for var for sign instead */
		if (SymbType_is_unsigned_word(var_type)) {
			return find_node(nodemgr, NUMBER_UNSIGNED_WORD,
					 (node_ptr)value, Nil);
		} else {
			return find_node(nodemgr, NUMBER_SIGNED_WORD,
					 (node_ptr)value, Nil);
		}
	} /* a word variable */
...
void bool_enc_init(BoolEnc_ptr self, SymbTable_ptr symb_table)
{
	...
	if (NuSMVEnv_has_value(env, ENV_BOOL_ENC_COUNT)) {
		counter = NODE_TO_INT(
			NuSMVEnv_get_value(env, ENV_BOOL_ENC_COUNT));
	}
	...
	if (counter == 0) {
		...
		NuSMVEnv_set_value(env, ENV_BOOL_ENC_OWNED_LAYERS,
				   bool_enc_owned_layers);
	}
...
void bool_enc_deinit(BoolEnc_ptr self)
{
		...
		while (!ListIter_is_end(iter)) {
			...
			if (count == 1) { /* the layer has to be destroyed */
				...
				SymbTable_remove_layer(
					BASE_ENC(self)->symb_table, lyr);

				insert_assoc(bool_enc_owned_layers,
					     (node_ptr)lyr,
	...
	bool_enc_instances =
		NODE_TO_INT(NuSMVEnv_get_value(env, ENV_BOOL_ENC_COUNT));

	if (bool_enc_instances ==
	    1) { /* This is the last instance in the env */
		hash_ptr bool_enc_owned_layers =
			(hash_ptr)NuSMVEnv_remove_value(
				env, ENV_BOOL_ENC_OWNED_LAYERS);
...
VIRTUAL void bool_enc_commit_layer(BaseEnc_ptr base_enc, const char *layer_name)
{
	const NuSMVEnv_ptr env =
		EnvObject_get_environment(ENV_OBJECT(base_enc));
	...
	dest_layer = SymbTable_get_layer(BASE_ENC(self)->symb_table,
					 dest_layer_name);
	...
	if (dest_layer == SYMB_LAYER(NULL)) {
       ...
       the layer it derives from has */
		dest_layer = SymbTable_create_layer(
			BASE_ENC(self)->symb_table, dest_layer_name,
			SymbLayer_get_insert_policy(src_layer));

		/* encoders become the new layer owners */
		nusmv_assert(find_assoc(bool_enc_owned_layers,
					(node_ptr)dest_layer) ==
			     (node_ptr)NULL);
		...
		int count = PTR_TO_INT(find_assoc(bool_enc_owned_layers,
						  (node_ptr)dest_layer));
		nusmv_assert(count >= 0);
		if (count > 0) {
			insert_assoc(bool_enc_owned_layers,
...
VIRTUAL void bool_enc_remove_layer(BaseEnc_ptr base_enc, const char *layer_name)
{
	const NuSMVEnv_ptr env =
		EnvObject_get_environment(ENV_OBJECT(base_enc));
	...
	bool_layer = SymbTable_get_layer(BASE_ENC(self)->symb_table,
					 bool_layer_name);
	...
	SYMB_LAYER_FOREACH(layer, iter, STT_VAR)
	{
		...
		case SYMB_TYPE_SIGNED_WORD: /* Word type */
           ...
           of this variable */
				...
				while (!ListIter_is_end(iter)) {
					...
					if (bool_enc_get_var_encoding(
						    self, bit) != Nil) {
						bool_enc_set_var_encoding(
							self, bit, Nil);
					}
					iter = ListIter_get_next(iter);
				}
		...
		case SYMB_TYPE_WORDARRAY: /* WordArray type */
			StreamMgr_print_error(
				streams, "Unable to booleanize WordArrays.\n");
			nusmv_assert((false));
			break;
     ...
     is the last user of it */
	{
		...
		if (count != 0) {
			...
			if (count == 1) {
				SymbTable_remove_layer(
					BASE_ENC(self)->symb_table, bool_layer);
			}
		}
	}
}
				...
				SymbLayer_ptr dest_layer)
{
     ...
     */
		for (suffix = width - 1; suffix >= 0; --suffix) {
			node_ptr bitVar =
				BoolEnc_make_var_bit(self, var, suffix);
    ...
    */
		for (scalar_enc = Nil, iter = bits; iter != Nil;
		     iter = cdr(iter)) {
			scalar_enc =
				find_node(nodemgr, CONS, car(iter), scalar_enc);
		}
     ...
     and the type is of no importance */
		scalar_enc = find_node(nodemgr, UNSIGNED_WORD, scalar_enc,
					   ...
					   SymbLayer_ptr dest_layer)
{
	...
	{ /* calculates the encoding, and declares needed bits */
		...
		result = bool_enc_compute_set_encoding(self, values, name,
						       suffix, &bits, true);
		...
		SET_FOREACH(bits, iter)
		{
			...
			if (!SymbTable_is_symbol_var(BASE_ENC(self)->symb_table,
				...
				if (SymbTable_is_symbol_input_var(
					...
					SymbLayer_declare_input_var(dest_layer,
								    bit, type);
				} else if (SymbTable_is_symbol_state_var(
						   BASE_ENC(self)->symb_table,
						   name)) {
					SymbLayer_declare_state_var(dest_layer,
								    bit, type);
				} else {
					SymbLayer_declare_frozen_var(dest_layer,
								     bit, type);
				}
			}

			bool_enc_set_var_encoding(
				self, bit, bool_enc_get_boolean_type(self));
		}
					      ...
					      boolean top)
{
	...
	*out_bits = Set_AddMember(*out_bits, (Set_Element_t)var);
	...
	return find_node(nodemgr, IFTHENELSE,
			 find_node(nodemgr, COLON, var, left), right);
}
	...
	{ /* first loop along the given list, to avoid one unneeded traversal */
		...
		while (iter != Nil) {
			...
			if (cdr(iter) != Nil) {
				...
				if (Nil == var) {
					if ((ExprMgr_is_false(exprs, el1) &&
					     ExprMgr_is_true(exprs, el2)) ||
					    (ExprMgr_is_false(exprs, el2) &&
					     ExprMgr_is_true(exprs, el1))) {
						var = bit_prefix; /* reuse bit for boolean local encoding */
					} else {
						var = BoolEnc_make_var_bit(
							self, bit_prefix,
							bit_suffix);
					}
					bit_list = cons(nodemgr, var, bit_list);
				}
				el = find_node(nodemgr, IFTHENELSE,
					       find_node(nodemgr, COLON, var,
							 el1),
					       el2);
	...
	while (array_n(array1) > 1) {
		...
		while (idx < array_n(array1)) {
			if (idx + 1 < array_n(array1)) {
				...
				node_ptr el2 =
					array_fetch(node_ptr, array1, idx + 1);
	...
	{ /* dumps all collected bits in reversed order, to keep the
       right order (higher indices before lower) */
		for (; Nil != bit_list; bit_list = cdr(bit_list)) {
			*out_bits = Set_AddMember(*out_bits,
						  (Set_Element_t)car(bit_list));
		}
	}
...
/*!
  ...
  Private service
*/
static node_ptr bool_enc_get_var_encoding(const BoolEnc_ptr self, node_ptr name)
{
...
#define _IS_ITE(enc)                                           \
	((enc != Nil) && (IFTHENELSE == node_get_type(enc)) && \
       ...
       we must be guaranteed to be on a leaf of the DAG */
		...
		if (_IS_LEAF(self, enc) || _COND(enc) != var) {
         ...
         we assign a value to missing variables in cube */
			node_ptr t = bool_enc_get_var_mask_recur(
				self, enc, cube, ListIter_get_next(cube_iter));

			res = ExprMgr_ite(exprs, var, ExprMgr_false(exprs), t,
					  symb_table);
		} else {
			/* It is a variable:
         ...
         the enc is the same as cube */
			...
			t = bool_enc_get_var_mask_recur(
				self, _THEN(enc), cube,
				ListIter_get_next(cube_iter));

			e = bool_enc_get_var_mask_recur(
				self, _ELSE(enc), cube,
				ListIter_get_next(cube_iter));
...
static node_ptr bool_enc_get_boolean_type(const BoolEnc_ptr self)
{
	...
	if (!NuSMVEnv_has_value(env, ENV_BOOLEAN_TYPE)) {
		...
		boolean_type =
			NODE_PTR(NuSMVEnv_get_value(env, ENV_BOOLEAN_TYPE));
	}
