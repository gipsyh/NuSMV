...
be_enc_is_log_index_untimed_next_state(const BeEnc_ptr self, int log_idx)
{
	return be_enc_is_log_index_untimed(self, log_idx) &&
	       (!be_enc_is_log_index_untimed_curr_state_frozen_input(self,
								     log_idx));
}
			    ...
			    const int time)
{
	...
	return Be_Index2Var(self->be_mgr,
			    be_enc_index_log_to_phy(self, log_idx));
}
...
int BeEnc_index_to_time(const BeEnc_ptr self, const int index)
{
	...
	return be_enc_index_log_to_time(self,
...
{
	BE_ENC_CHECK_INSTANCE(self);
	return BeEnc_index_to_timed(self, Be_Var2Index(self->be_mgr, var),
				    time);
}
...
be_ptr BeEnc_var_to_untimed(const BeEnc_ptr self, const be_ptr var)
{
	...
	log_idx =
		be_enc_index_phy_to_log(self, Be_Var2Index(self->be_mgr, var));
...
{
	BE_ENC_CHECK_INSTANCE(self);
	return BeEnc_index_to_timed(self, BeEnc_name_to_index(self, name),
				    time);
}
...
be_ptr BeEnc_var_curr_to_next(const BeEnc_ptr self, const be_ptr curr)
{
	...
	log_idx = be_enc_index_log_curr_to_next(
		self, be_enc_index_phy_to_log(self, Be_Var2Index(self->be_mgr,
								 curr)));
	return Be_Index2Var(self->be_mgr,
			    be_enc_index_log_to_phy(self, log_idx));
}
...
{
	BE_ENC_CHECK_INSTANCE(self);
	return be_enc_is_log_index_untimed(
		self, be_enc_index_phy_to_log(self, index));
}
...
int BeEnc_get_first_untimed_var_index(const BeEnc_ptr self, BeVarType type)
{
	...
	} else if (((type & BE_VAR_TYPE_INPUT) != 0) &&
		   (self->input_vars_num > 0)) {
		res = be_enc_index_log_to_phy(
			self, self->state_vars_num + self->frozen_vars_num);
	} else if (((type & BE_VAR_TYPE_NEXT) != 0) &&
				    ...
				    int offset, BeVarType type)
{
	...
	case BE_VAR_TYPE_CURR:
		newType = type & (BE_VAR_TYPE_FROZEN | BE_VAR_TYPE_INPUT |
				  BE_VAR_TYPE_NEXT);
		break;
	default:
					   ...
					   const int from, const int to)
{
	...
	if (from > to) {
		...
		res = Be_And(self->be_mgr,
			     BeEnc_untimed_to_timed_and_interval(self, exp,
								 from, to - 1),
					  ...
					  const int from, const int to)
{
	...
	if (from > to) {
		...
		res = Be_Or(self->be_mgr,
			    ...
			    BeEnc_untimed_to_timed_or_interval(self, exp,
							       from + 1, to));
	}
...
void be_enc_init(BeEnc_ptr self, SymbTable_ptr symb_table, BoolEnc_ptr bool_enc)
{
	...
	self->shift_hash = st_init_table(&be_enc_shift_hash_key_cmp,
					 &be_enc_shift_hash_key_hash);
...
static inline void be_enc_remove_var_aux(BeEnc_ptr self, int phy_idx)
{
	...
	if (be_enc_is_log_index_untimed_frozen(self, log_idx)) {
		...
		for (time = 0; time <= self->max_allocated_time; ++time) {
			int timed_log_idx = be_enc_index_log_untimed_to_timed(
				self, log_idx, time);
			nusmv_assert(self->log2phy[timed_log_idx] ==
				     phy_idx); /* double check */
...
static inline void be_enc_remove_var(BeEnc_ptr self, node_ptr name)
{
	...
	nusmv_assert(type !=
		     BE_VAR_TYPE_NEXT); /* next state vars are removed only
	...
	if (BE_VAR_TYPE_CURR == type) {
		...
		be_enc_remove_var_aux(self,
				      next_phy_idx); /* untimed next var */
	}
	...
	if (BE_VAR_TYPE_CURR == type || BE_VAR_TYPE_INPUT == type) {
		for (time = 0; time <= self->max_allocated_time; ++time) {
			int timed_log_idx = be_enc_index_log_untimed_to_timed(
				self, log_idx, time);
			int timed_phy_idx =
				be_enc_index_log_to_phy(self, timed_log_idx);
...
static void be_enc_compact_log_level(BeEnc_ptr self)
{
	...
	for (src_idx = 0; src_idx < total_vars; ++src_idx) {
		...
		if (phy_idx != BE_ENC_INVALID_IDX) {
			if (dest_idx != src_idx) {
				if (MAX(dest_idx, src_idx) <
				    self->index2name_size) {
					self->index2name[dest_idx] =
						self->index2name[src_idx];
					self->index2name[src_idx] =
						(node_ptr)NULL;
				}
				self->log2phy[dest_idx] =
					self->log2phy[src_idx];
           ...
           to fix bug #1621 */
				if (be_enc_is_log_index_untimed_frozen(
					    self,
					    be_enc_index_log_timed_to_untimed(
						    self, src_idx))) {
					if (!Set_IsMember(
						    frozen_phy_idx,
						    PTR_FROM_INT(Set_Element_t,
								 phy_idx))) {
			...
			++total_rm_vars;
			switch (be_enc_type_of_log_index_untimed(self,
								 src_idx)) {
			case BE_VAR_TYPE_INPUT:
				++rm_ivars;
				break;
				   ...
				   int frozen_vars_delta, int ofs)
{
	...
	} else if (SymbTable_is_symbol_frozen_var(BASE_ENC(self)->symb_table,
						  name)) {
		type = BE_VAR_TYPE_FROZEN;
	} else {
		ErrorMgr_internal_error(
			errmgr, "be_enc_instantiate_var: undefined identifier");
	}
	...
	case BE_VAR_TYPE_INPUT: /* input var */
		idx_log = self->state_vars_num + state_vars_delta +
			  self->frozen_vars_num + frozen_vars_delta +
			  self->input_vars_num + ofs;
		be_enc_create_be_var(self, idx_log, name);
		break;

	default:
		ErrorMgr_internal_error(
			errmgr, "be_enc_instantiate_var : impossible code(1)");
	} /* switch */
	...
	{ /* calculate the new size of timed frame a*/
       ...
       * blocks */
		case BE_VAR_TYPE_CURR:
		case BE_VAR_TYPE_INPUT:
			for (time = 0; time <= self->max_allocated_time;
			     ++time) {
				be_enc_create_be_var(self, idx_timed,
						     (node_ptr)NULL);
					  ...
					  int frozen_vars_num)
{
	...
	self->index2name =
		REALLOC(node_ptr, self->index2name, self->index2name_size);
	...
	for (time = self->max_allocated_time; time >= 0; --time) {
		...
		be_enc_move_log_block(self, idx_old, idx_new,
				      self->input_vars_num);
...
static void be_enc_extend_timed_blocks(BeEnc_ptr self, int maxtime)
{
	...
	if (times > 0) {
       ...
       expands the memory */
		be_enc_allocate_space_for_new_vars(self,
						   timed_block_size * times);
		...
		end_new_idx =
			idx + timed_block_size *
				      times; /* an index AFTER last new index */

		for (; idx < end_new_idx; ++idx) {
			int idx_untimed =
				be_enc_index_log_timed_to_untimed(self, idx);
...
*/
static void be_enc_create_be_var(BeEnc_ptr self, int logical_idx, node_ptr name)
{
	NuSMVEnv_ptr env = ENV_OBJECT(self)->environment;
	const OptsHandler_ptr opts =
		OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));
...
static int be_enc_index_log_timed_to_untimed(const BeEnc_ptr self, int log_idx)
{
	...
	} else if (be_enc_is_log_index_untimed_curr_state_frozen_input(
			   self, log_idx)) {
		res = log_idx;
	} else {
...
static void be_enc_realloc_subst_array(BeEnc_ptr self)
{
	...
	if (self->subst_array_size < untimed_block_size) {
		self->subst_array =
			REALLOC(int, self->subst_array, untimed_block_size);
				       ...
				       int time)
{
	...
	if (!st_lookup(self->shift_hash, (char *)&key, (char **)&result)) {
		...
		int delta;
		be_enc_shift_memoize_key *key_copy =
			ALLOC(be_enc_shift_memoize_key, 1);
		...
		if (!Be_IsConstant(self->be_mgr, exp)) {
			delta = be_enc_index_log_untimed_to_timed(self, 0,
								  time);
					...
					int n_time)
{
	...
	if (!st_lookup(self->shift_hash, (char *)&key, (char **)&result)) {
		...
		if (Be_IsConstant(self->be_mgr, exp))
			...
			if (c_time >= 0)
				c_delta = be_enc_index_log_untimed_to_timed(
					self, 0, c_time);
			else
				switch (c_time) {
				...
				case BE_ENC_NEXT_UNTIMED_TIME:
					c_delta = be_enc_index_log_curr_to_next(
						self, 0);
					break;
				default:
			...
			if (f_time >= 0)
				f_delta = be_enc_index_log_untimed_to_timed(
					self, 0, f_time);
			else
				switch (f_time) {
				case BE_ENC_INVALID_TIME:
			...
			if (i_time >= 0)
				i_delta = be_enc_index_log_untimed_to_timed(
					self, 0, i_time);
			else
				switch (i_time) {
				case BE_ENC_INVALID_TIME:
        ...
        */
				n_delta =
					be_enc_index_log_untimed_to_timed(
						self, 0, n_time) -
			...
			idx = 0;
			for (stopper = self->state_vars_num; idx < stopper;
			     ++idx) { /*current*/
				if (BE_ENC_INVALID_TIME == c_time) {
					self->subst_array[idx] =
						BE_INVALID_SUBST_VALUE;
				} else
					self->subst_array[idx] = idx + c_delta;
			}
			for (stopper += self->frozen_vars_num; idx < stopper;
			     ++idx) { /*frozen*/
			...
			for (stopper += self->input_vars_num; idx < stopper;
			     ++idx) { /*input*/
			...
			result = Be_LogicalVarSubst(self->be_mgr, exp,
						    self->subst_array,
						    self->log2phy,
						    self->phy2log);
		} /* if */
...
static void be_enc_clean_shift_hash(BeEnc_ptr self)
{
	...
	st_foreach(self->shift_hash,
		   &be_enc_shift_hash_callback_del_entry_and_key,
		   NULL /*unused*/);
}
...
static int be_enc_shift_hash_key_cmp(const char *_key1, const char *_key2)
{
	const be_enc_shift_memoize_key *key1 =
		(be_enc_shift_memoize_key *)_key1;
	const be_enc_shift_memoize_key *key2 =
		(be_enc_shift_memoize_key *)_key2;

	return (key1->be != key2->be) || (key1->c_time != key2->c_time) ||
	       (key1->f_time != key2->f_time) ||
