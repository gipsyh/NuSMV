	...
	if (res == 0) {
		...
		if (PslNode_is_obe(psl_parsed_tree)) {
			StreamMgr_print_output(
				streams, "Parsed an OBE (CTL) expression\n");
#if DOEXPAN
			StreamMgr_print_output(
				streams, "The expanded OBE expression is\n");
			print_node(wffprint, outstream,
				   PslNode_pslobe2ctl(psl_parsed_tree,
						      PSL2PSL));
			StreamMgr_print_output(streams, "\n\n");
#endif
			...
			StreamMgr_print_output(
				streams,
				"The SMV compatible CTL expression is\n");
			print_node(wffprint, outstream,
				   PslNode_pslobe2ctl(psl_parsed_tree,
						      PSL2SMV));
			StreamMgr_print_output(streams, "\n\n");
			...
			if (!PslNode_is_ltl(expr)) {
				...
				StreamMgr_print_output(
					streams,
					"The translated SERE expression is\n");
				StreamMgr_nprint_output(streams, wffprint, "%N",
							expr);
				StreamMgr_print_output(streams, "\n\n");
			}
#endif
			StreamMgr_print_output(streams,
					       "Parsed a LTL expression\n");
#if DOEXPAN
			StreamMgr_print_output(
				streams, "The expanded LTL expression is\n");
			print_node(wffprint, outstream,
			...
			StreamMgr_print_output(
				streams,
				"The SMV compatible LTL expression is\n");
			print_node(wffprint, outstream,
			...
			StreamMgr_print_output(
				streams, "Parsed a NON LTL/CTL expression\n");
		}
	} else {
