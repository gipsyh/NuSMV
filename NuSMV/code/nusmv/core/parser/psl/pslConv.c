...
#define PSL_OP_CONV3(err, tok, psl, smv)                                       \
	if (op == ((type == TOK2PSL) ? tok :                                   \
		...
		default:                                                       \
			ErrorMgr_internal_error(                               \
				err,                                           \
				"PSL_OP_CONV: invalid conversion type.\n");    \
		}                                                              \
	}
			       ...
			       PslOpConvType type)
{
	...
	case DOT:
		result = psl_new_node(
			nodemgr, psl_conv_op(env, type, op),
				       ...
				       NodeList_ptr replicator_id_stack)
{
	...
	case PSL_ITE: {
		...
		PslNode_ptr case_else = psl_node_make_case(
			nodemgr, psl_node_make_true(nodemgr), _else,
	...
	case PSL_REPLPROP: {
		...
		NodeList_prepend(replicator_id_stack,
				 PslNode_convert_to_node_ptr(id));

		result = psl_node_expand_replicator(env, rep, wff, rop);
		result = psl_node_pslobe2ctl(env, result, type,
					     replicator_id_stack);
					      ...
					      PslNode_ptr expr)
{
	NodeList_ptr repl_stack = NodeList_create();
	PslNode_ptr res =
		psl_node_remove_forall_replicators(env, expr, repl_stack);
				   ...
				   NodeList_ptr replicator_id_stack)
{
	...
	case PSL_REPLPROP: {
		...
		result = psl_node_expand_replicator(env, rep, wff, rop);
		result = psl_node_remove_forall_replicators(
			env, result, replicator_id_stack);
	...
	default:
		result = psl_new_node(nodemgr, psl_node_get_op(expr),
				      psl_node_remove_forall_replicators(
					      env, psl_node_get_left(expr),
					      replicator_id_stack),
				      psl_node_remove_forall_replicators(
					      env, psl_node_get_right(expr),
					      replicator_id_stack));
		break;
	}
	return result;
				       ...
				       NodeList_ptr replicator_id_stack)
{
	...
	case PSL_NEXT:
	case PSL_NEXTBANG: {
		PslNode_ptr l = psl_node_pslltl2ltl(env,
		...
		if (r != PSL_NULL) {
			...
			if (!psl_node_is_number(lim_expr)) {
				...
				StreamMgr_nprint_error(streams, wffprint, "%N",
						       expr);
				StreamMgr_print_error(streams, "'\n");
				ErrorMgr_error_psl_not_supported_feature_next_number(
					errmgr);
			}
			...
			nusmv_assert(lim >= 0);
			for (; lim > 0; --lim) {
				l = psl_new_node(nodemgr,
						 psl_conv_op(env, type, op), l,
						 PSL_NULL);
			}
		} else {
			/* when is not specified */
			l = psl_new_node(nodemgr, psl_conv_op(env, type, op), l,
					 PSL_NULL);
		}
	...
	case PSL_NEXT_ABANG: {
...

		{
			...
			inf = psl_node_number_get_value(
				psl_node_get_left(lim_expr));
			...
			sup = psl_node_number_get_value(
				psl_node_get_right(lim_expr));
      ...
      */
			...
			for (i = inf + 1; i <= sup; ++i) {
				...
				if ((op == PSL_NEXT_E) ||
					...
					result = psl_new_node(
						nodemgr,
						psl_conv_op(env, type, OR),
						result, rr);
				} else {
					/* result' = result & rr */
					result = psl_new_node(
						nodemgr,
						psl_conv_op(env, type, AND),
						result, rr);
				}
			}
#endif
	...
	case PSL_NEXT_EVENT_ABANG: {
...

		{
			...
			rr = psl_node_expand_next_event(
				env, op,
				psl_node_pslltl2ltl(env, l, type,
						    replicator_id_stack),
				b, type);
			...
			for (i = 2; i <= inf; ++i) {
				rr = psl_new_node(nodemgr,
						  psl_conv_op(env, type,
							      PSL_XBANG),
						  rr, PSL_NULL);
				rr = psl_node_expand_next_event(env, op, rr, b,
								type);
			}
	...
	case PSL_NEVER: {
		...
		not_l = psl_new_node(nodemgr, psl_conv_op(env, type, NOT), l,
				     PSL_NULL);
		result = psl_new_node(nodemgr,
				      psl_conv_op(env, type, PSL_ALWAYS), not_l,
				      PSL_NULL);
		break;
	}
	...
	case PSL_UNTIL: {
		...
		result = psl_new_node(nodemgr, psl_conv_op(env, type, UNTIL),
				      f1, f2);
		result = psl_new_node(
			nodemgr, psl_conv_op(env, type, OR), result,
			psl_new_node(nodemgr,
				     psl_conv_op(env, type, PSL_ALWAYS), f1,
				     PSL_NULL));
		break;
	}
	...
	case PSL_BEFOREBANG: {
		...
		f2 = psl_new_node(nodemgr, psl_conv_op(env, type, NOT), f2,
				  PSL_NULL);
	...
	case PSL_BEFORE: {
		...
		result = psl_new_node(
			nodemgr, psl_conv_op(env, type, OR), result,
			psl_new_node(nodemgr,
				     psl_conv_op(env, type, PSL_ALWAYS), f2,
				     PSL_NULL));
		break;
	}
	...
	case PSL_REPLPROP: {
		...
		result = psl_node_expand_replicator(env, rep, wff, rop);
		result = psl_node_pslltl2ltl(env, result, type,
					     replicator_id_stack);
					      ...
					      PslOpConvType type)
{
	...
	if ((op == PSL_NEXT_EVENT) || (op == PSL_NEXT_EVENT_E) ||
		...
		result = psl_new_node(
			...
			psl_new_node(nodemgr, psl_conv_op(env, type, UNTIL),
				     notb, result),
			psl_new_node(nodemgr,
				     psl_conv_op(env, type, PSL_ALWAYS), notb,
				     PSL_NULL));
	} else {
		result = psl_new_node(nodemgr, psl_conv_op(env, type, UNTIL),
				      notb, result);
...

	{
		...
		for (r = erange; r != PSL_NULL; r = psl_node_cons_get_next(r)) {
			...
			if (result == PSL_NULL)
				...
				result =
					psl_new_node(nodemgr, op, result, rwff);
					     ...
					     PslNode_ptr e)
{
	...
	if (psl_node_get_op(e) == PSL_SERECOMPOUND && psl_node_get_left(e) &&
		...
		l = psl_node_sere_remove_disj(
			env, psl_node_get_left(e)); /* gets r1 */
		r = psl_node_sere_remove_disj(
			env, psl_node_get_right(e)); /* gets r2 */
		return psl_new_node(nodemgr, OR /* here is logical or */, l, r);
	}

	return psl_new_node(
		nodemgr, psl_node_get_op(e),
						...
						boolean *inserted)
{
	...
	if (psl_node_get_op(e) == PSL_UNTILBANG) {
		*inserted = true;
		return psl_new_node(
			...
			psl_new_node(nodemgr, AND, psl_node_get_right(e),
				     to_be_inserted));
	}
	...
	if (psl_node_get_op(e) == PSL_EVENTUALLYBANG &&
		...
		/* inside F */
		*inserted = true;
		return psl_new_node(nodemgr, PSL_EVENTUALLYBANG, to_be_inserted,
				    PSL_NULL);
	}

	{
		PslNode_ptr l = psl_node_insert_inside_holes(
			env, psl_node_get_left(e), to_be_inserted, inserted);
		PslNode_ptr r = psl_node_insert_inside_holes(
			env, psl_node_get_right(e), to_be_inserted, inserted);
						   ...
						   PslNode_ptr phi)
{
	...
	/* recursion on atomic  */
	if (psl_node_get_op(e) == PSL_SERE) {
		return psl_node_sere_concat_fusion2ltl(
			env, psl_node_get_left(e), phi);
	}
	...
	/* atomic */
	{
		...
		return (phi != PSL_NULL) ?
			       psl_new_node(nodemgr, AND, rec, phi) :
			       rec;
	}
}
					      ...
					      PslNode_ptr e, boolean *modified)
{
	...
	*modified = false;
	...
	real_op = (op == PSL_SERECOMPOUND) ?
			  psl_node_get_op(psl_node_get_left(e)) :
			  op;
	...
	if (real_op == PSL_SERECONCAT || real_op == PSL_SEREFUSION ||
		...
		if (psl_node_sere_is_disj(l)) {
			...
			*modified = true;
			...
			disj = psl_node_get_left(
				l); /* to remove PSL_SERECOMPOUND */
			...
			if (real_op == PSL_SERECONCAT ||
			    real_op == PSL_SEREFUSION) {
				r1_op_r3 =
					psl_new_node(nodemgr, real_op, r1, r3);
				r2_op_r3 =
					psl_new_node(nodemgr, real_op, r2, r3);
			} else {
				r1_op_r3 = psl_node_make_sere_compound(
					nodemgr, r1, real_op, r3);
				r2_op_r3 = psl_node_make_sere_compound(
					nodemgr, r2, real_op, r3);
			}

			return psl_node_make_sere_compound(nodemgr, r1_op_r3,
							   OR, r2_op_r3);
			...
			*modified = true;
			...
			if (real_op == PSL_SERECONCAT ||
			    real_op == PSL_SEREFUSION) {
				r3_op_r1 =
					psl_new_node(nodemgr, real_op, r3, r1);
				r3_op_r2 =
					psl_new_node(nodemgr, real_op, r3, r2);
			} else {
				r3_op_r1 = psl_node_make_sere_compound(
					nodemgr, r3, real_op, r1);
				r3_op_r2 = psl_node_make_sere_compound(
					nodemgr, r3, real_op, r2);
			}

			return psl_node_make_sere_compound(nodemgr, r3_op_r1,
							   OR, r3_op_r2);
		}
	}
	...
	{
		boolean lm, rm;
		PslNode_ptr l = psl_node_sere_distrib_disj(
			env, psl_node_get_left(e), &lm);
		PslNode_ptr r = psl_node_sere_distrib_disj(
			env, psl_node_get_right(e), &rm);
		*modified = (lm || rm);
		return psl_new_node(nodemgr, psl_node_get_op(e), l, r);
	}
}
						   ...
						   PslNode_ptr e)
{
	...
	if (psl_node_get_op(e) == PSL_SERE) {
		return psl_node_sere_remove_star_count(env,
						       psl_node_get_left(e));
	}
	...
	if (psl_node_sere_is_star_count(e)) {
		...
		if (count > 0) {
           ...
           treated as if applied to the sere {True} */
				mul = psl_new_node(nodemgr, PSL_SERE,
						   psl_node_make_true(nodemgr),
						   PSL_NULL);
			}
			{
				PslNode_ptr acc = mul;
				for (count--; count > 0; count--)
					acc = psl_new_node(nodemgr,
							   PSL_SERECONCAT, mul,
							   acc);
	...
	{
		PslNode_ptr l = psl_node_sere_remove_star_count(
			env, psl_node_get_left(e));
		PslNode_ptr r = psl_node_sere_remove_star_count(
			env, psl_node_get_right(e));
		return psl_new_node(nodemgr, psl_node_get_op(e), l, r);
	}
}
...
static PslNode_ptr psl_node_sere_get_rightmost(PslNode_ptr e)
{
	if (psl_node_get_op(e) == PSL_SERE) {
		if (!psl_node_is_sere(psl_node_get_left(e)))
			...
			return psl_node_sere_get_rightmost(
				psl_node_get_left(e));
	}
					     ...
					     PslNode_ptr e, boolean toplevel)
{
	...
	if (toplevel && psl_node_sere_is_standalone_plus(e)) {
		return psl_new_node(nodemgr, PSL_SERE,
				    psl_node_make_true(nodemgr), PSL_NULL);
	}
	...
	if (psl_node_sere_is_plus(e)) {
       ...
       here we are NOT toplevel */
		...
		return psl_new_node(nodemgr, PSL_SERE,
				    psl_new_node(nodemgr, PSL_UNTILBANG, expr,
						 expr),
				    PSL_NULL);
	}
   ...
   * psl_node_get_op(e)==PSL_SEREFUSION) { */
	...
	{
		PslNode_ptr l = psl_node_sere_remove_plus(
			env, psl_node_get_left(e), toplevel_l);
		PslNode_ptr r = psl_node_sere_remove_plus(
			env, psl_node_get_right(e), toplevel_r);
						      ...
						      PslNode_ptr e)
{
	...
	if (psl_node_is_suffix_implication(e)) {
		...
		if (!psl_node_is_sere(con)) {
			sere_con =
				psl_new_node(nodemgr, PSL_SERE, con, PSL_NULL);
		} else
			sere_con = con;
		...
		if (op == PSL_PIPEMINUSGT) {
			sere_con = psl_new_node(nodemgr, PSL_SEREFUSION, pre,
						sere_con);
		} else {
			nusmv_assert(op == PSL_PIPEEQGT);
			sere_con = psl_new_node(nodemgr, PSL_SERECONCAT, pre,
						sere_con);
		}
		...
		PslNode_ptr l, r;
		l = psl_node_remove_suffix_implication(env,
						       psl_node_get_left(e));
		r = psl_node_remove_suffix_implication(env,
	...
	if (psl_node_get_op(e) == PSL_SERECONCAT) {
		if (psl_node_sere_is_standalone_star(psl_node_get_right(e))) {
         ...
         here we are NOT toplevel */
			...
			l = psl_node_sere_remove_star(env, psl_node_get_left(e),
						      false, modified);
		...
		if (psl_node_sere_is_standalone_star(psl_node_get_left(e))) {
			...
			r = psl_node_sere_remove_star(
				env, psl_node_get_right(e), false, modified);
   ...
   */
	...
	/* fallback rule */
	{
		...
		if (psl_node_is_sere_compound_binary(e)) {
			...
			l = psl_node_sere_remove_star(env, l, toplevel_l,
						      &rec_modified);
			*modified |= rec_modified;

			r = psl_node_sere_remove_star(env, r, toplevel_r,
						      &rec_modified);
			*modified |= rec_modified;
		...
		l = psl_node_sere_remove_star(env, psl_node_get_left(e),
					      toplevel_l, &rec_modified);
		*modified |= rec_modified;

		r = psl_node_sere_remove_star(env, psl_node_get_right(e),
					      toplevel_r, &rec_modified);
		*modified |= rec_modified;
           ...
           not null */
				...
				PslNode_ptr tails = psl_new_node(
					...
					psl_new_node(nodemgr, AND,
						     psl_node_prune(nodemgr, l,
								    head_l),
						     psl_node_prune(nodemgr, r,
								    head_r)),
					PSL_NULL);

				tails_rec = psl_node_sere_remove_ampersand(
					env, tails, modified);
				/* regardless the effects of the recursive call on tail, the
         *expression has been modified */
				*modified = true;
   ...
   * concatenations */
	{
		...
		if (psl_node_sere_is_ampersand(e)) {
			/* special handling of & */
			PslNode_ptr exp =
				psl_node_get_left(e); /* gets r1 & r2 */
			PslNode_ptr lexp = psl_node_get_left(exp); /* gets r1 */
			PslNode_ptr rexp =
				psl_node_get_right(exp); /* gets r2 */
			PslNode_ptr lrec =
				psl_node_sere_remove_ampersand(env, lexp, &lm);
			PslNode_ptr rrec =
				psl_node_sere_remove_ampersand(env, rexp, &rm);
			...
			PslNode_ptr l = psl_node_sere_remove_ampersand(
				env, psl_node_get_left(e), &lm);
			PslNode_ptr r = psl_node_sere_remove_ampersand(
				env, psl_node_get_right(e), &rm);
	...
	*modified = false;
	...
	if (psl_node_sere_is_2ampersand(e)) {
       ...
       (as a result of a recursion) */
		if ((psl_node_get_op(l) == FALSEEXP) ||
		    (psl_node_get_op(r) == FALSEEXP)) {
			error_unreachable_code();
		}
   ...
   * concatenations */
	{
		...
		if (psl_node_sere_is_2ampersand(e)) {
			...
			PslNode_ptr rexp =
				psl_node_get_right(exp); /* gets r2 */
			PslNode_ptr lrec =
				psl_node_sere_remove_2ampersand(env, lexp, &lm);
			PslNode_ptr rrec =
				psl_node_sere_remove_2ampersand(env, rexp, &rm);
			...
			PslNode_ptr l = psl_node_sere_remove_2ampersand(
				env, psl_node_get_left(e), &lm);
			PslNode_ptr r = psl_node_sere_remove_2ampersand(
				env, psl_node_get_right(e), &rm);
					       ...
					       PslNode_ptr e, boolean *modified)
{
	...
	*modified = false;
			...
			*modified = true;
			return psl_new_node(
				...
				psl_new_node(
					...
					AND, /* here is logical "and" */
					psl_node_get_left(
						psl_node_sere_get_leftmost(l)),
					psl_node_get_left(
						psl_node_sere_get_leftmost(r))),
				PSL_NULL);
		}
       ...
       element of the result of the recursion */
		if (lb) {
         ...
         concat_or_fusion */
			...
			PslNode_ptr rec =
				psl_node_sere_remove_fusion(env, r, &m);
			PslNode_ptr b =
				psl_node_sere_get_leftmost(rec); /* r head */
			PslNode_ptr head = psl_new_node(
				nodemgr, PSL_SERE,
				psl_new_node(nodemgr,
					     AND, /* here is logical "and" */
					     psl_node_get_left(a),
		...
		if (rb) {
         ...
         concat_or_fusion */
			...
			PslNode_ptr rec =
				psl_node_sere_remove_fusion(env, l, &m);
			PslNode_ptr b =
				psl_node_sere_get_rightmost(rec); /* r tail */
			PslNode_ptr tail = psl_new_node(
				nodemgr, PSL_SERE,
				psl_new_node(nodemgr,
					     AND, /* here is logical "and" */
					     psl_node_get_left(b),
		...
		/* both arguments are concat_or_fusion */
		{
			...
			PslNode_ptr l_rest =
				psl_node_prune(nodemgr, l_rec, l_tail);
			PslNode_ptr r_rest =
				psl_node_prune(nodemgr, r_rec, r_head);
			...
			*modified = m;
			return psl_new_node(nodemgr, PSL_SERECONCAT, l_rest,
					    psl_new_node(nodemgr,
							 PSL_SERECONCAT,
							 merge_point, r_rest));
		}
     ...
     continue on its arguments */
	{
		...
		PslNode_ptr l = psl_node_sere_remove_fusion(
			env, psl_node_get_left(e), &lm);
		PslNode_ptr r = psl_node_sere_remove_fusion(
			env, psl_node_get_right(e), &rm);
		PslNode_ptr result;
