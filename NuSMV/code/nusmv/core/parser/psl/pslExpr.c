				    ...
				    const PslExpr *bool_expr, PslExpr *res)
{
	...
	res->psl_node = psl_new_node(
		nodemgr, TOK2SYM(env, op_id), fl_property->psl_node,
				 ...
				 PslExpr id, PslExpr range, PslExpr value_set)
{
	...
	res.psl_node =
		psl_new_node(nodemgr, TOK2SYM(env, op_id), id.psl_node,
...
PslExpr psl_expr_make_atom(const NuSMVEnv_ptr env, const char *str)
{
	...
	res.psl_node = psl_new_node(
		...
		(PslNode_ptr)UStringMgr_find_string(strings, (char *)str),
		NULL);
	return res;
}
...
PslExpr psl_expr_make_id_array(const NuSMVEnv_ptr env, PslExpr id, PslExpr idx)
{
	...
	res.klass = SC_IDENTIFIER;
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKARRAY), id.psl_node,
				    idx.psl_node);
	return res;
}
...
PslExpr psl_expr_make_context(const NuSMVEnv_ptr env, PslExpr ctx, PslExpr node)
{
	...
	res.psl_node = PslNode_new_context(
		nodemgr,
		psl_node_context_to_main_context(nodemgr, ctx.psl_node),
		node.psl_node);
...
PslExpr psl_expr_make_inf(const NuSMVEnv_ptr env)
{
	...
	res.psl_node =
		psl_new_node(nodemgr, TOK2SYM(env, TKINF), PSL_NULL, PSL_NULL);
	return res;
}
...
PslExpr psl_expr_make_boolean_type(const NuSMVEnv_ptr env)
{
	...
	PslExpr res;
	res.klass = SC_NONE;
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKBOOLEAN), PSL_NULL,
				    PSL_NULL);
	return res;
}
...
PslExpr psl_expr_make_boolean_value(const NuSMVEnv_ptr env, int val)
{
	...
	res.klass = SC_BOOL_EXPR;
	res.psl_node =
		psl_new_node(nodemgr, TOK2SYM(env, op), PSL_NULL, PSL_NULL);
	return res;
}
...
PslExpr psl_expr_make_range(const NuSMVEnv_ptr env, PslExpr low, PslExpr high)
{
	...
	res.klass = SC_RANGE;
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKDOTDOT),
				    low.psl_node, high.psl_node);
	return res;
}
			   ...
			   PslExpr _list)
{
	...
	res.psl_node = psl_node_make_case(nodemgr, cond.psl_node,
					  _then.psl_node, _list.psl_node);
	return res;
}
			       ...
			       PslExpr seq)
{
	...
	res.klass = SC_FL_PROPERTY;
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, op), expr.psl_node,
				    seq.psl_node);
	return res;
}
...
PslExpr psl_expr_make_sere(const NuSMVEnv_ptr env, PslExpr expr)
{
	...
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKSERE),
				    expr.psl_node, PSL_NULL);
	return res;
}
					      ...
					      PslExpr seq2)
{
	...
	nusmv_assert(op == TKAMPERSANDAMPERSAND || op == TKAMPERSAND ||
		     op == TKPIPE);
	...
	res.klass = SC_SEQUENCE;
	res.psl_node = psl_node_make_sere_compound(
		nodemgr, seq1.psl_node, TOK2SYM(env, op), seq2.psl_node);
	return res;
}
				    ...
				    PslExpr sere, PslExpr count)
{
	...
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKSEREREPEATED),
				    psl_new_node(nodemgr, TOK2SYM(env, op),
						 sere.psl_node, PSL_NULL),
				    count.psl_node);
	return res;
}
				 ...
				 PslExpr right)
{
	...
	res.klass = SC_OBE_PROPERTY;
	res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, op), left.psl_node,
				    right.psl_node);
	return res;
}
				    ...
				    PslExpr left, PslExpr right)
{
	...
	res.klass = SC_WORD_EXPR;
	res.psl_node = psl_new_node(
		nodemgr, TOK2SYM(env, TKWSELECT), word_expr.psl_node,
