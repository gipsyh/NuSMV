...
static void psl_expr_require_klass(const NuSMVEnv_ptr env, const PslExpr psl,
                                   SyntaxClass expected);

static void psl_expr_is_valid_flproperty(const NuSMVEnv_ptr env, PslOp op_id,
                                         const PslExpr *left,
                                         const PslExpr *right);
...
void psl_expr_make_unary_op(const NuSMVEnv_ptr env, PslExpr *res,
  ...
  if (op_id == TKMINUS) {
    res->psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKUMINUS),
                                 right->psl_node, PSL_NULL);
    return;
  }
  if (op_id == TKPLUS) {
  ...
  res->psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, op_id), right->psl_node, PSL_NULL);
}
...
void psl_expr_make_binary_op(const NuSMVEnv_ptr env, PslExpr *res,
  ...
  /* takes the action here */
  res->klass = res_klass;
  res->psl_node = psl_new_node(nodemgr, TOK2SYM(env, op_id), left->psl_node,
                               right->psl_node);
}
...
void psl_expr_make_extended_next_op(const NuSMVEnv_ptr env, PslOp op_id,
  ...
  res->psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, op_id), fl_property->psl_node,
                   psl_new_node(nodemgr, TOK2SYM(env, TKCOLON), when->psl_node,
                                bool_expr->psl_node));
}
...
PslExpr psl_expr_make_replicator(const NuSMVEnv_ptr env, PslOp op_id,
  ...
  res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, op_id), id.psl_node,
                              psl_new_node(nodemgr, TOK2SYM(env, TKIN),
                                           range.psl_node, value_set.psl_node));
  return res;
}
...
PslExpr psl_expr_make_replicated_property(const NuSMVEnv_ptr env,
  ...
  res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKREPLPROP),
                              replicator.psl_node, expr.psl_node);
  return res;
}
...
PslExpr psl_expr_make_atom(const NuSMVEnv_ptr env, const char *str) {
  ...
  res.psl_node = psl_new_node(
      nodemgr, TOK2SYM(env, TKATOM),
...
/* ---------- Creates: -------------------------------------------------
   ...
   or TKATOM       or TKATOM
   ---------------------------------------------------------------------- */
PslExpr psl_expr_make_id_array(const NuSMVEnv_ptr env, PslExpr id,
                               PslExpr idx) {
  NodeMgr_ptr nodemgr = NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
  PslExpr res;
  ...
  res.klass = SC_IDENTIFIER;
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, TKARRAY), id.psl_node, idx.psl_node);
  return res;
}
...
PslExpr psl_expr_make_context(const NuSMVEnv_ptr env, PslExpr ctx,
  ...
  res.psl_node = PslNode_new_context(
      nodemgr, psl_node_context_to_main_context(nodemgr, ctx.psl_node),
      node.psl_node);
...
PslExpr psl_expr_make_boolean_type(const NuSMVEnv_ptr env) {
  ...
  res.klass = SC_NONE;
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, TKBOOLEAN), PSL_NULL, PSL_NULL);
  return res;
}
...
PslExpr psl_expr_make_real_number(const NuSMVEnv_ptr env, char *fval) {
  ...
  res.klass = SC_NUM_EXPR;
  res.psl_node = psl_new_node(
      nodemgr, TOK2SYM(env, TKREALNUMBER),
...
PslExpr psl_expr_make_word_number(const NuSMVEnv_ptr env, char *fval) {
  ...
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, sign), (PslNode_ptr)number, PSL_NULL);
  return res;
}
...
PslExpr psl_expr_make_range(const NuSMVEnv_ptr env, PslExpr low, PslExpr high) {
  ...
  res.klass = SC_RANGE;
  res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKDOTDOT), low.psl_node,
                              high.psl_node);
  return res;
}
...
PslExpr psl_expr_make_suffix_implication_weak(const NuSMVEnv_ptr env,
  ...
  res.psl_node = psl_new_node(
      nodemgr, TOK2SYM(env, op),
      psl_new_node(nodemgr, TOK2SYM(env, TKCOLON), seq.psl_node, expr.psl_node),
      PSL_NULL);
  return res;
}
...
PslExpr psl_expr_make_suffix_implication_strong(const NuSMVEnv_ptr env,
  ...
  res.psl_node = psl_new_node(
      nodemgr, TOK2SYM(env, op),
      psl_new_node(nodemgr, TOK2SYM(env, TKCOLON), seq.psl_node, expr.psl_node),
      psl_new_node(nodemgr, TOK2SYM(env, TKBANG), PSL_NULL, PSL_NULL));
  return res;
}
...
PslExpr psl_expr_make_within(const NuSMVEnv_ptr env, PslOp op, PslExpr begin,
  ...
  nusmv_assert(op == TKWITHINBANG || op == TKWITHIN || op == TKWITHINBANG_ ||
               op == TKWITHIN_);
  ...
  res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, op),
                              psl_new_node(nodemgr, TOK2SYM(env, TKCOMMA),
                                           begin.psl_node, end.psl_node),
                              seq.psl_node);
  return res;
}
...
PslExpr psl_expr_make_whilenot(const NuSMVEnv_ptr env, PslOp op, PslExpr expr,
  ...
  res.klass = SC_FL_PROPERTY;
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, op), expr.psl_node, seq.psl_node);
  return res;
}
...
PslExpr psl_expr_make_abort(const NuSMVEnv_ptr env, PslExpr fl_prop,
  ...
  res.klass = SC_FL_PROPERTY;
  res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKABORT), fl_prop.psl_node,
                              cond.psl_node);
  return res;
}
...
PslExpr psl_expr_make_sere(const NuSMVEnv_ptr env, PslExpr expr) {
  ...
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, TKSERE), expr.psl_node, PSL_NULL);
  return res;
}
...
PslExpr psl_expr_make_sere_concat(const NuSMVEnv_ptr env, PslExpr seq1,
  ...
  res.klass = SC_SEQUENCE;
  res.psl_node = psl_new_node(nodemgr, TOK2SYM(env, TKSERECONCAT),
...
PslExpr psl_expr_make_sere_compound_binary_op(const NuSMVEnv_ptr env,
  ...
  res.klass = SC_SEQUENCE;
  res.psl_node = psl_node_make_sere_compound(nodemgr, seq1.psl_node,
                                             TOK2SYM(env, op), seq2.psl_node);
  return res;
}
...
PslExpr psl_expr_make_repeated_sere(const NuSMVEnv_ptr env, PslOp op,
  ...
  nusmv_assert(op == TKLBSPLAT || op == TKLBPLUSRB || op == TKLBEQ ||
               op == TKLBMINUSGT);

  if (sere.klass != SC_NONE)
    psl_expr_require_klass(env, sere, SC_SEQUENCE);
  if (count.klass != SC_NONE)
    psl_expr_require_klass(env, count, SC_NUM_RANGE);

  res.klass = SC_SEQUENCE;
  res.psl_node = psl_new_node(
      nodemgr, TOK2SYM(env, TKSEREREPEATED),
      psl_new_node(nodemgr, TOK2SYM(env, op), sere.psl_node, PSL_NULL),
      count.psl_node);
  return res;
}
...
/* ---------- Creates: -------------------------------------------------
   ...
   ---------------------------------------------------------------------- */
PslExpr psl_expr_make_obe_unary(const NuSMVEnv_ptr env, PslOp op,
                                PslExpr expr) {
  NodeMgr_ptr nodemgr = NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
  PslExpr res;
  ...
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, op), expr.psl_node, PSL_NULL);
  return res;
}
...
PslExpr psl_expr_make_obe_binary(const NuSMVEnv_ptr env, PslExpr left, PslOp op,
  ...
  res.klass = SC_OBE_PROPERTY;
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, op), left.psl_node, right.psl_node);
  return res;
}
...
PslExpr psl_expr_make_bit_selection(const NuSMVEnv_ptr env, PslExpr word_expr,
  ...
  res.klass = SC_WORD_EXPR;
  res.psl_node =
      psl_new_node(nodemgr, TOK2SYM(env, TKWSELECT), word_expr.psl_node,
                   psl_new_node(nodemgr, TOK2SYM(env, TKCOLON), left.psl_node,
                                right.psl_node));
  return res;
}
...
static void psl_expr_print_klass(FILE *file, SyntaxClass type) {
  ...
  case SC_NUM_BOOL_WORD_EXPR:
    name = "Numeric or Boolean or Word";
    break;
  case SC_NUM_BOOL_EXPR:
    name = "Numeric or Boolean";
    break;
  case SC_NUM_WORD_EXPR:
    name = "Numeric or Word";
    break;
  case SC_BOOL_WORD_EXPR:
    name = "Boolean or Word";
    break;
  case SC_WORD_EXPR:
    name = "Word";
    break;
...
*/
static int psl_expr_is_boolean(const PslExpr psl) {
  if ((psl.klass == SC_BOOL_EXPR) || (psl.klass == SC_IDENTIFIER) ||
      (psl.klass == SC_NUM_BOOL_WORD_EXPR) || (psl.klass == SC_NUM_BOOL_EXPR) ||
      (psl.klass == SC_BOOL_WORD_EXPR) || (psl.klass == SC_PROPERTY) ||
      (psl.klass == SC_OBE_PROPERTY) || (psl.klass == SC_FL_PROPERTY))
    return 1;
  ...
  if (psl_node_is_number(psl.psl_node)) {
    ...
    if ((psl_node_number_get_value(psl.psl_node) == 0) ||
        (psl_node_number_get_value(psl.psl_node) == 1))
      return 1;
  }
...
static int psl_expr_check_klass(const PslExpr psl, SyntaxClass expected) {
  ...
  case SC_NUM_WORD_EXPR:
    res = (res || (psl.klass == SC_NUM_EXPR) || (psl.klass == SC_WORD_EXPR) ||
           (psl.klass == SC_IDENTIFIER) || (psl.klass == SC_BOOL_WORD_EXPR));
    break;
  ...
  case SC_BOOL_WORD_EXPR:
    res = (res || (psl.klass == SC_WORD_EXPR) || psl_expr_is_boolean(psl) ||
           (psl.klass == SC_NUM_WORD_EXPR));
    break;
  ...
  case SC_FL_PROPERTY:
    res = (res || (psl.klass == SC_IDENTIFIER) || (psl.klass == SC_BOOL_EXPR) ||
           (psl.klass == SC_BOOL_WORD_EXPR) || (psl.klass == SC_SEQUENCE));
    break;
  ...
  case SC_NUM_RANGE:
    res = (res || (psl.klass == SC_IDENTIFIER) || (psl.klass == SC_NUM_EXPR) ||
           (psl.klass == SC_RANGE));
    break;
