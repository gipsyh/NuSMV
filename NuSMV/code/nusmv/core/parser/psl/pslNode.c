...
boolean PslNode_propositional_contains_next(const PslNode_ptr expr) {
  ...
  case CASE:
    result =
        PslNode_propositional_contains_next(psl_node_get_case_cond(expr)) ||
...
boolean psl_node_is_sere(PslNode_ptr expr) {
  ...
  return (op == PSL_SERE || op == PSL_SERECONCAT || op == PSL_SEREFUSION ||
          op == PSL_SEREREPEATED || op == PSL_SERECOMPOUND);
}
    ...
    if (psl_node_is_number(count)) {
      ...
      return (psl_node_number_get_value(count) > 0) &&
             psl_node_is_handled_sere(
                 env, psl_node_sere_repeated_get_expr(expr), toplevel);
    }
...
PslNode_ptr psl_node_make_sere_2ampersand(NodeMgr_ptr nodemgr, PslNode_ptr seq1,
                                          PslNode_ptr seq2) {
  return psl_node_make_sere_compound(nodemgr, seq1, PSL_AMPERSANDAMPERSAND,
                                     seq2);
}
...
PslNode_ptr psl_node_sere_concat_cut_leftmost(NodeMgr_ptr nodemgr,
  ...
  if (psl_node_sere_is_concat(left))
    return psl_node_make_sere_concat(
        nodemgr, psl_node_sere_concat_cut_leftmost(nodemgr, left), right);
  else
    return right;
}
...
PslNode_ptr psl_node_prune(NodeMgr_ptr nodemgr, PslNode_ptr tree,
  ...
  return psl_new_node(
      nodemgr, op, psl_node_prune(nodemgr, psl_node_get_left(tree), branch),
...
PslNode_ptr psl_node_make_case(NodeMgr_ptr nodemgr, PslNode_ptr _cond,
  ...
  return psl_new_node(nodemgr, CASE, psl_new_node(nodemgr, COLON, _cond, _then),
                      _next);
}
...
boolean psl_node_is_extended_next(PslNode_ptr e) {
  ...
  return (op == PSL_X || op == PSL_XBANG || op == PSL_NEXT ||
          op == PSL_NEXTBANG || op == PSL_NEXT_E || op == PSL_NEXT_EBANG ||
          op == PSL_NEXT_A || op == PSL_NEXT_ABANG || op == PSL_NEXT_EVENT ||
...
boolean psl_node_is_leaf(PslNode_ptr expr) {
  ...
  return (NUMBER == op || NUMBER_FRAC == op || NUMBER_REAL == op ||
          NUMBER_EXP == op || ATOM == op || TRUEEXP == op || FALSEEXP == op ||
...
PslNode_ptr psl_node_get_replicator_normalized_value_set(const NuSMVEnv_ptr env,
  ...
  if (psl_node_is_boolean_type(vset) == true) {
    result = psl_node_make_cons_new(
        ...
        psl_node_make_cons_new(nodemgr, psl_node_make_true(nodemgr), result));
  } else {
      ...
      case PSL_RANGE: {
        ...
        if (inf > sup) {
          ...
          ErrorMgr_error_invalid_numeric_value(
              errmgr, sup, "Range with high bound greater than low bound.");
        }

        for (i = inf; i <= sup; i++) {
          result = psl_node_make_cons_new(
              nodemgr, psl_node_make_number(nodemgr, i), result);
        }
        break;
      }
      case BOOLEAN:
  ...
  if (op == PSL_SERECOMPOUND) {
    ...
    if (op == AND) {
      ...
      return psl_node_is_star_free(psl_node_get_left(psl_node_get_left(e))) &&
             ...
             psl_node_is_handled_sere(
                 env, psl_node_get_left(psl_node_get_left(e)), toplevel) &&
             psl_node_is_handled_sere(
                 env, psl_node_get_right(psl_node_get_left(e)), toplevel);
    }
...
static boolean psl_node_is_handled_fl_op(PslOp op) {
  return (op != PSL_WHILENOTBANG && op != PSL_WHILENOT &&
          ...
          op != PSL_WITHINBANG && op != PSL_WITHIN && op != PSL_WITHINBANG_ &&
          op != PSL_WITHIN_ &&
...
*/
static boolean psl_node_is_fl_op(PslOp op) {
  return (op == OP_NEXT || op == PSL_X || op == PSL_XBANG || op == PSL_NEXT ||
          op == PSL_NEXTBANG || op == PSL_NEXT_E || op == PSL_NEXT_EBANG ||
          op == PSL_NEXT_A || op == PSL_NEXT_ABANG || op == PSL_NEXT_EVENT ||
          op == PSL_NEXT_EVENTBANG || op == PSL_NEXT_EVENT_E ||
          op == PSL_NEXT_EVENT_EBANG || op == PSL_NEXT_EVENT_A ||
          op == PSL_NEXT_EVENT_ABANG || op == OP_FUTURE || op == PSL_NEVER ||
          op == OP_GLOBAL || op == PSL_ALWAYS || op == PSL_EVENTUALLYBANG ||
          op == PSL_W || op == PSL_UNTIL || op == UNTIL ||
          op == PSL_UNTILBANG || op == PSL_UNTIL_ || op == PSL_UNTILBANG_ ||
          op == PSL_BEFORE || op == PSL_BEFOREBANG || op == PSL_BEFORE_ ||
