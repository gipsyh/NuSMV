...
static boolean psl_node_is_handled_sere(const NuSMVEnv_ptr env, PslNode_ptr e,
					boolean toplevel);
...
boolean PslNode_is_handled_psl(const NuSMVEnv_ptr env, PslNode_ptr e)
{
	...
	/* no sere */
	if (PslNode_is_propositional(e))
		return true;

	/* no obe expressions */
	if (psl_node_is_obe_op(op))
		return false;
	...
	if (psl_node_is_suffix_implication(e)) {
		...
		PslNode_ptr con =
			psl_node_suffix_implication_get_consequence(e);
		...
		if (!PslNode_is_handled_psl(env, pre) ||
		    !PslNode_is_handled_psl(env, con)) {
			return false;
		}
	...
	if (op == PSL_SERE || op == PSL_SERECONCAT || op == PSL_SEREFUSION ||
		...
		if (psl_node_is_handled_sere(env, e, true))
			return true;
...
boolean PslNode_propositional_contains_next(const PslNode_ptr expr)
{
	...
	case CONTEXT:
		result = PslNode_propositional_contains_next(
	...
	case CASE:
		result = PslNode_propositional_contains_next(
				 psl_node_get_case_cond(expr)) ||
			 PslNode_propositional_contains_next(
				 psl_node_get_case_then(expr)) ||
			 PslNode_propositional_contains_next(
				 psl_node_get_case_next(expr));
		break;

	case PSL_ITE:
		result = PslNode_propositional_contains_next(
				 psl_node_get_ite_cond(expr)) ||
			 PslNode_propositional_contains_next(
				 psl_node_get_ite_then(expr)) ||
			 PslNode_propositional_contains_next(
				 psl_node_get_ite_else(expr));
		break;

	case PSL_REPLPROP:
		result = PslNode_propositional_contains_next(
			psl_node_repl_prop_get_property(expr));
		break;
...
boolean PslNode_is_obe(const PslNode_ptr expr)
{
	...
	/* no FL expressions */
	if (psl_node_is_fl_op(psl_node_get_op(expr)))
		return false;
...
boolean PslNode_is_ltl(const PslNode_ptr expr)
{
	...
	/* no obe expressions */
	if (psl_node_is_obe_op(psl_node_get_op(expr)))
		return false;
...
PslNode_ptr psl_new_node(NodeMgr_ptr nodemgr, PslOp op, PslNode_ptr left,
			 PslNode_ptr right)
{
	return (PslNode_ptr)NodeMgr_find_node(nodemgr, (int)op, (node_ptr)left,
					      (node_ptr)right);
}
...
PslNode_ptr psl_node_sere_star_get_count(const PslNode_ptr e)
{
	...
	if (psl_node_get_right(e) == PSL_NULL)
		return PSL_EMPTYSTAR;
	else {
				 ...
				 boolean toplevel)
{
		...
		if (psl_node_is_number(count)) {
			...
			return (psl_node_number_get_value(count) > 0) &&
			       psl_node_is_handled_sere(
				       env,
				       psl_node_sere_repeated_get_expr(expr),
				       toplevel);
		}
	...
	if (psl_node_sere_is_stareq(expr) ||
	    psl_node_sere_is_starminusgt(expr)) {
		return false;
	}

	/* toplevel stars are not handled, only plus are handled at toplevel */
	if (toplevel && psl_node_sere_is_star(expr))
		return false;
...
boolean psl_node_sere_is_propositional(PslNode_ptr e)
{
	...
	if (psl_node_get_op(e) != PSL_SERE)
		return false;
	if (PslNode_is_propositional(psl_node_get_left(e)))
		return true;
...
boolean psl_node_sere_is_star_count_zero(PslNode_ptr e)
{
	return (e != PSL_NULL) && (psl_node_sere_is_star_count(e)) &&
	       ...
	       (0 ==
		psl_node_number_get_value(psl_node_sere_star_get_count(e)));
}
...
PslNode_ptr psl_node_sere_propositional_get_expr(PslNode_ptr e)
{
	...
	/* getting rid of { } */
	while (psl_node_get_op(expr) == PSL_SERE)
		expr = psl_node_get_left(expr);
...
PslNode_ptr psl_node_make_sere_star(NodeMgr_ptr nodemgr, PslNode_ptr seq)
{
	return psl_new_node(nodemgr, PSL_SEREREPEATED,
			    psl_new_node(nodemgr, PSL_LBSPLAT, seq, PSL_NULL),
			    PSL_NULL);
}
...
PslNode_ptr psl_node_make_sere_2ampersand(NodeMgr_ptr nodemgr, PslNode_ptr seq1,
					  PslNode_ptr seq2)
{
	return psl_node_make_sere_compound(nodemgr, seq1,
					   PSL_AMPERSANDAMPERSAND, seq2);
}
					...
					PslOp op, PslNode_ptr seq2)
{
	return psl_new_node(nodemgr, PSL_SERECOMPOUND,
			    psl_new_node(nodemgr, op, seq1, seq2), PSL_NULL);
}
...
boolean psl_node_sere_is_concat_holes_free(PslNode_ptr e)
{
	...
	if (psl_node_get_op(e) == PSL_SERE) {
		return psl_node_sere_is_concat_fusion_holes_free(
			psl_node_get_left(e));
	}

	if (psl_node_get_op(e) == PSL_SERECONCAT) {
		return psl_node_sere_is_concat_holes_free(
			       psl_node_get_left(e)) &&
...
boolean psl_node_sere_is_concat(PslNode_ptr e)
{
	...
	if (psl_node_get_op(e) == PSL_SERE && psl_node_get_left(e) != NULL &&
...
PslNode_ptr psl_node_sere_concat_get_leftmost(PslNode_ptr e)
{
	if (psl_node_sere_is_concat(e))
		return psl_node_sere_concat_get_leftmost(
			psl_node_sere_concat_get_left(e));
	else
		return e;
}
...
{
	if (psl_node_sere_is_concat(e))
		return psl_node_sere_concat_get_rightmost(
			psl_node_sere_concat_get_right(e));
	else
		return e;
}
					      ...
					      PslNode_ptr e)
{
	...
	if (psl_node_sere_is_concat(left))
		return psl_node_make_sere_concat(
			nodemgr,
			   ...
			   PslNode_ptr branch)
{
	...
	/* base case */
	if (psl_node_is_equal(tree, branch))
		return PSL_NULL;

	/* atomic tree which is not equal to the branch to be pruned */
	if (psl_node_sere_is_propositional(tree))
		return tree;
...
boolean psl_node_is_propstar(PslNode_ptr e)
{
	...
	while (psl_node_get_op(e) == PSL_SERE)
		e = psl_node_get_left(e);
	...
	return (expr != PSL_NULL) && PslNode_is_propositional(expr);
}
...
PslNode_ptr psl_node_make_cons_new(NodeMgr_ptr nodemgr, PslNode_ptr elem,
				   PslNode_ptr next)
{
	return (PslNode_ptr)NodeMgr_new_node(nodemgr, CONS, (node_ptr)elem,
					     (node_ptr)next);
}
...
boolean psl_node_is_case(PslNode_ptr _case)
{
	if (_case == PSL_NULL)
		return false;
	if (psl_node_get_op(_case) != CASE)
		return false;
	if (psl_node_get_op(psl_node_get_left(_case)) == COLON)
		return true;
	else
		return false;
}
...
boolean psl_node_is_range(PslNode_ptr expr)
{
	...
	if (psl_node_get_op(expr) == PSL_RANGE)
		return true;
	return false;
}
				  ...
				  FailureKind kind)
{
	...
	return psl_new_node(nodemgr, FAILURE,
			    psl_new_node(nodemgr, COLON,
					 (PslNode_ptr)UStringMgr_find_string(
						 strings, (char *)msg),
					 (PslNode_ptr)kind),
...
{
	return psl_new_node(nodemgr, op, expr,
			    psl_new_node(nodemgr, COLON, when, condition));
}
...
boolean psl_node_is_extended_next(PslNode_ptr e)
{
	...
	return (op == PSL_X || op == PSL_XBANG || op == PSL_NEXT ||
		op == PSL_NEXTBANG || op == PSL_NEXT_E ||
		op == PSL_NEXT_EBANG || op == PSL_NEXT_A ||
		op == PSL_NEXT_ABANG || op == PSL_NEXT_EVENT ||
		op == PSL_NEXT_EVENTBANG || op == PSL_NEXT_EVENT_E ||
		op == PSL_NEXT_EVENT_EBANG || op == PSL_NEXT_EVENT_A ||
...
boolean psl_node_is_id(PslNode_ptr expr)
{
	...
	return (psl_node_get_op(expr) == DOT || psl_node_get_op(expr) == ATOM ||
...
boolean psl_node_is_id_equal(PslNode_ptr _id1, PslNode_ptr _id2)
{
	...
	if (psl_node_get_op(_id1) != psl_node_get_op(_id2))
		return false;
	...
	case ARRAY:
		res = psl_node_is_id_equal(psl_node_get_left(_id1),
		      ...
		      psl_node_is_id_equal(psl_node_get_right(_id1),
					   psl_node_get_right(_id2));
		break;
...
boolean psl_node_is_leaf(PslNode_ptr expr)
{
	...
	return (NUMBER == op || NUMBER_FRAC == op || NUMBER_REAL == op ||
		NUMBER_EXP == op || ATOM == op || TRUEEXP == op ||
		FALSEEXP == op || NUMBER_SIGNED_WORD == op ||
		NUMBER_UNSIGNED_WORD == op || FAILURE == op);
}
...
boolean psl_node_is_replicator(PslNode_ptr _repl)
{
	...
	if (psl_node_get_op(_repl) != PSL_FORALL &&
	    psl_node_get_op(_repl) != PSL_FORANY)
		return false;
	if (psl_node_get_op(psl_node_get_right(_repl)) != SETIN)
		return false;
	return true;
}
							 ...
							 PslNode_ptr rep)
{
	...
	if (psl_node_is_boolean_type(vset) == true) {
		result = psl_node_make_cons_new(
			nodemgr, psl_node_make_false(nodemgr),
			psl_node_make_cons_new(
				nodemgr, psl_node_make_true(nodemgr), result));
	} else {
		...
		for (; vset != PSL_NULL; vset = psl_node_cons_get_next(vset)) {
			...
			case PSL_RANGE: {
				...
				if (!psl_node_is_number(
					    psl_node_range_get_low(v))) {
			...
			case BOOLEAN:
				...
				break;
			case NUMBER:
				result = psl_node_make_cons_new(nodemgr, v,
								result);
				break;
		...
		result = psl_node_cons_reverse(
			result); /* We must reverse the list */
	}
					...
					boolean toplevel)
{
	...
	if (op == PSL_SERE) {
		return psl_node_is_handled_sere(env, psl_node_get_left(e),
						toplevel);
	}

	if (op == PSL_SERECONCAT) {
		if (toplevel) {
			return (psl_node_is_emptystar_free(
					psl_node_get_left(e)) ||
				psl_node_is_emptystar_free(
					psl_node_get_right(e))) &&
			       psl_node_is_handled_sere(
				       env, psl_node_get_left(e), false) &&
			       psl_node_is_handled_sere(
				       env, psl_node_get_right(e), false);
		}

		return psl_node_is_handled_sere(env, psl_node_get_left(e),
						false) &&
		       psl_node_is_handled_sere(env, psl_node_get_right(e),
						false);
	}
	...
	if (op == PSL_SERECOMPOUND) {
        ...
        {c;d;e} */
			return psl_node_is_star_free(psl_node_get_left(
				       psl_node_get_left(e))) &&
			       psl_node_is_star_free(psl_node_get_right(
				       psl_node_get_left(e))) &&
			       psl_node_is_handled_sere(
				       env,
				       psl_node_get_left(psl_node_get_left(e)),
				       toplevel) &&
			       psl_node_is_handled_sere(
				       env,
				       psl_node_get_right(psl_node_get_left(e)),
				       toplevel);
		}
		...
		if (op == OR) {
			...
			return psl_node_is_handled_sere(
				       env,
				       psl_node_get_left(psl_node_get_left(e)),
				       toplevel) &&
			       psl_node_is_handled_sere(
		...
		return psl_node_is_emptystar_free(
		       ...
		       psl_node_is_handled_sere(
			       ...
			       toplevel) &&
		       psl_node_is_handled_sere(
			       env, psl_node_get_right(psl_node_get_left(e)),
			       toplevel);
	}

	if (op == PSL_SEREREPEATED)
		return psl_node_is_handled_star(env, e, toplevel);
...
static boolean psl_node_is_fl_op(PslOp op)
{
	return (op == OP_NEXT || op == PSL_X || op == PSL_XBANG ||
		op == PSL_NEXT || op == PSL_NEXTBANG || op == PSL_NEXT_E ||
		op == PSL_NEXT_EBANG || op == PSL_NEXT_A ||
		op == PSL_NEXT_ABANG || op == PSL_NEXT_EVENT ||
		op == PSL_NEXT_EVENTBANG || op == PSL_NEXT_EVENT_E ||
		op == PSL_NEXT_EVENT_EBANG || op == PSL_NEXT_EVENT_A ||
		op == PSL_NEXT_EVENT_ABANG || op == OP_FUTURE ||
		op == PSL_NEVER || op == OP_GLOBAL || op == PSL_ALWAYS ||
		op == PSL_EVENTUALLYBANG || op == PSL_W || op == PSL_UNTIL ||
		op == UNTIL || op == PSL_UNTILBANG || op == PSL_UNTIL_ ||
		op == PSL_UNTILBANG_ || op == PSL_BEFORE ||
		op == PSL_BEFOREBANG || op == PSL_BEFORE_ ||
		op == PSL_BEFOREBANG_ || op == PSL_WHILENOTBANG ||
		op == PSL_WHILENOT || op == PSL_WHILENOTBANG_ ||
		op == PSL_WHILENOT_ || op == PSL_ABORT);
}
...
static boolean psl_node_is_obe_op(PslOp op)
{
	return (op == AX || op == EX || op == AG || op == EG || op == AF ||
		op == EF || op == EU || op == AU);
}
					 ...
					 boolean accept_next)
{
	...
	case CASE:
		result = PslNode_is_propositional(
				 psl_node_get_case_cond(expr)) &&
	...
	case PSL_ITE:
		result =
			PslNode_is_propositional(psl_node_get_ite_cond(expr)) &&
