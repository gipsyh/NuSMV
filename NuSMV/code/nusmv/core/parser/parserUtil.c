...
void Parser_OpenInput(const NuSMVEnv_ptr env, const char *filename)
{
	...
	if (filename != (char *)NULL) {
		...
		if (nusmv_yyin == (FILE *)NULL)
			ErrorMgr_rpterr(errmgr, "cannot open input file %s",
					filename);
		nusmv_yylineno = 1;
	} else {
		nusmv_yyin = stdin;
				  ...
				  node_ptr *res)
{
	...
	*res = Nil;
	return Parser_ReadCmdFromString(env, sizeof(argv) / sizeof(argv[0]),
					argv, head, tail, res);
}
				...
				const char **argv, node_ptr *res)
{
	...
	*res = Nil;
	...
	len += 1 + 2 +
	       1; /* semicolon, last two YY_END_OF_BUFFER_CHAR, terminator */
	cmd = ALLOC(char, len);
	cmd1 = ALLOC(char, len);
	...
	sprintf(cmd1, "%s;%c%c", cmd, YY_END_OF_BUFFER_CHAR,
		YY_END_OF_BUFFER_CHAR);
...
node_ptr Parser_get_syntax_errors_list(const NuSMVEnv_ptr env)
{
	...
	if (NuSMVEnv_has_value(env, ENV_PARSED_ERRORS)) {
		if (!NuSMVEnv_get_flag(env, ENV_ERROR_REVERSED)) {
			parsed_errors = NODE_PTR(
				NuSMVEnv_remove_value(env, ENV_PARSED_ERRORS));
			parsed_errors = reverse(parsed_errors);
			NuSMVEnv_set_value(env, ENV_PARSED_ERRORS,
					   parsed_errors);
			NuSMVEnv_set_flag(env, ENV_ERROR_REVERSED, true);
		} else {
			parsed_errors = NODE_PTR(
				NuSMVEnv_get_value(env, ENV_PARSED_ERRORS));
		}
	}
...
void parser_add_syntax_error(const NuSMVEnv_ptr env, const char *fname,
			     int lineno, const char *token, const char *err_msg)
{
	...
	node_ptr err = new_node(
		...
		new_node(nodemgr, COLON,
			 (node_ptr)UStringMgr_find_string(strings,
							  (char *)fname),
	...
	if (NuSMVEnv_has_value(env, ENV_PARSED_ERRORS)) {
		parsed_errors =
			NODE_PTR(NuSMVEnv_remove_value(env, ENV_PARSED_ERRORS));
	}
...
static void parser_open_input_pp(const NuSMVEnv_ptr env, const char *filename)
{
	...
	while (pp_next != NULL) {
		...
		if (tmpfname1 != (char *)NULL) {
			pp_cmd = ALLOC(char,
				       strlen(pp_exec) + strlen(tmpfname1) + 2);
		} else {
			pp_cmd =
				ALLOC(char, strlen(pp_exec) +
						    strlen(STDIN_OPT_NAME) + 1);
		}
	...
	if (tmpfname1 != (char *)NULL) {
		...
		pp_cmd = ALLOC(char,
			       strlen(pp_exec) + strlen(STDIN_OPT_NAME) + 1);
		strcpy(pp_cmd, pp_exec);
		strcat(pp_cmd, STDIN_OPT_NAME);
	}
	...
	if (!(nusmv_yyin = popen(pp_cmd, "r"))) {
		FREE(pp_list_copy);
		ErrorMgr_rpterr(errmgr, "error executing command \"%s\"",
				pp_cmd);
	}
