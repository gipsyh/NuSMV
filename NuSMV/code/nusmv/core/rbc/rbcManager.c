...
Rbc_Manager_t *Rbc_ManagerAlloc(const NuSMVEnv_ptr env, int varCapacity)
{
	...
	rbcManager->one = Dag_VertexInsert(rbcManager->dagManager, RBCTOP,
					   NIL(char), (Dag_Vertex_t **)NULL, 0);
...
{
	return (RbcDfsFunctions_t *)Dag_ManagerGetDfsCleanFun(
		rbcManager->dagManager);
}
