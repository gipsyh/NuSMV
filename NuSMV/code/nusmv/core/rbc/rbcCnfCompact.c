			   ...
			   int *literalAssignedToWholeFormula)
{
	...
	if (polarity >= 0) {
		renamed = Rbc_get_node_cnf(rbcManager, RBCDUMMY,
					   &(cnfData.maxVar));
		nusmv_assert(0 != renamed);
		rename_clauses(cnfData.clgManager, &(cnfData.posClauses),
			       renamed, &(cnfData.clauses));
	}
	if (polarity <= 0) {
		if (polarity < 0) {
			renamed = Rbc_get_node_cnf(rbcManager, RBCDUMMY,
		...
		nusmv_assert(0 != renamed);
		rename_clauses(cnfData.clgManager, &(cnfData.negClauses),
			       -renamed, &(cnfData.clauses));
	}
...
static int CnfCompactPolSet(Rbc_t *f, char *cnfData, nusmv_ptrint sign)
{
	...
	if (nd == NULL) {
       ...
       sign and whether this is an IFF node */
		if (f->symbol == RBCIFF) {
			f->iRef =
				0; /* Both a positive and a negative reference */
		} else if (sign) {
			f->iRef = -(cd->pol);
		} else {
			f->iRef = cd->pol;
	...
	if (f->iRef < -1) {
		if (f->symbol == RBCITE) {
         ...
         of the ITE node itself */
			f->iRef +=
				10; /* Reset polarity for THEN and ELSE branches */
			cd->pol =
				0; /* IF branch has to be considered in both polarities */
	...
	if (f->symbol != RBCVAR) {
		if (nd->posClauses == NULL && nd->negClauses == NULL) {
         ...
         the IF branch will have been considered for both polarities */
			...
			if (nd->posRef > 0) {
				disjunction(cd->clgManager, &(nd->negClauses),
					    &(cd->posClauses), &(cd->maxVar),
					    &(cd->clauses), cd->rbcManager);
				nd->posClauses = Clg_Disj(cd->clgManager,
							  nd->negClauses,
							  cd->posClauses);
			} else
				nd->posClauses = NULL;
			if (nd->negRef > 0) {
				disjunction(cd->clgManager, &(nd->negClauses),
					    &(cd->negClauses), &(cd->maxVar),
					    &(cd->clauses), cd->rbcManager);
				nd->negClauses = Clg_Disj(cd->clgManager,
							  nd->negClauses,
							  cd->negClauses);
			} else
				nd->negClauses = NULL;
...
static void CnfCompactLast(Rbc_t *f, char *cnfData, nusmv_ptrint sign)
{
	...
	case RBCAND: /* Conjunction or disjunction */
		...
		if (nd->negRef > 0) {
			disjunction(cd->clgManager, &leftNeg, &rightNeg,
				    &(cd->maxVar), &(cd->clauses),
				    cd->rbcManager);
	...
	case RBCIFF:
		if ((nd->posRef) > 0) {
			...
			disjunction2(cd->clgManager, &leftNeg, &rightPos,
				     &leftPos, &rightNeg, &(cd->maxVar),
				     &(cd->clauses), cd->rbcManager);
			resultPos = Clg_Conj(
				cd->clgManager,
		...
		if ((nd->negRef) > 0) {
			...
			disjunction2(cd->clgManager, &leftPos, &rightPos,
				     &leftNeg, &rightNeg, &(cd->maxVar),
				     &(cd->clauses), cd->rbcManager);
			resultNeg = Clg_Conj(
				cd->clgManager,
	...
	case RBCITE:
       ...
       branch in cd->ifClauses */
		...
		if (nd->posRef > 0) {
			...
			disjunction(cd->clgManager, &(nd->ifClauses),
				    &(cd->posClauses), &(cd->maxVar),
				    &(cd->clauses), cd->rbcManager);

			resultPos =
				Clg_Conj(cd->clgManager, nd->posClauses,
					 Clg_Disj(cd->clgManager, nd->ifClauses,
						  cd->posClauses));
		} else
			resultPos = NULL;
	...
	if ((nd->posRef) > 1) {
		...
		rename_clauses(cd->clgManager, &resultPos, cnfVar,
			       &(cd->clauses));
	}
	if ((nd->negRef) > 1) {
		nusmv_assert(0 != cnfVar);
		rename_clauses(cd->clgManager, &resultNeg, -cnfVar,
			       &(cd->clauses));
	}
