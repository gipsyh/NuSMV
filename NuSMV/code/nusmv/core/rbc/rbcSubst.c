...
Rbc_t *Rbc_SubstRbc(Rbc_Manager_t *rbcManager, Rbc_t *f, Rbc_t **substRbc)
{
	...
	if (Rbc_IsConstant(rbcManager, f))
		return f;
	...
	if (f->symbol == RBCVAR) {
		...
		if (RBC_INVALID_SUBST_VALUE == idx) {
			ErrorMgr_internal_error(
				errmgr,
				"%s: Tried to substitute an invalid index",
				__func__);
		}
		...
		if (f->symbol == RBCAND)
			sd->result = Rbc_MakeAnd(sd->rbcManager, sons[0],
						 sons[1], RBC_TRUE);
		else if (f->symbol == RBCIFF)
			sd->result = Rbc_MakeIff(sd->rbcManager, sons[0],
						 sons[1], RBC_TRUE);
		else if (f->symbol == RBCITE)
			sd->result = Rbc_MakeIte(sd->rbcManager, sons[0],
						 sons[1], sons[2], RBC_TRUE);
		else {
			ErrorMgr_internal_error(
				errmgr, "SubstLast: unknown RBC symbol");
		}
	...
	if (f->symbol == RBCVAR) {
       ...
       to log, shifts, and then passes again down from log to phy: */
		...
		if (f->symbol == RBCAND)
			...
			ErrorMgr_internal_error(
				errmgr, "LogicalSubstLast: unknown RBC symbol");
		}
		/* Clean the temporary sons list. */
		FREE(sons);
	}
...
static void ShiftLast(Rbc_t *f, char *shiftData, nusmv_ptrint sign)
{
	...
	if (f->symbol == RBCVAR) {
		...
		sd->result = Rbc_GetIthVar(
			sd->rbcManager, (int)(sd->shift) + PTR_TO_INT(f->data));
	} else {
		/* Substitutions may trigger simplifications. */
		if (f->symbol == RBCAND)
			sd->result = Rbc_MakeAnd(sd->rbcManager, sons[0],
			...
			ErrorMgr_internal_error(
				errmgr, "ShiftLast: unknown RBC symbol");
		/* Clean the temporary sons list. */
		FREE(sons);
	}
...
static void LogicalShiftLast(Rbc_t *f, char *shiftData, nusmv_ptrint sign)
{
   ...
   log, shifts, and then passes again down from log to phy: */
		sd->result = Rbc_GetIthVar(
			sd->rbcManager,
			sd->log2phy[sd->phy2log[PTR_TO_INT(f->data)] +
				    sd->shift]);
	} else {
		/* Substitutions may trigger simplifications. */
		if (f->symbol == RBCAND)
			...
			ErrorMgr_internal_error(
				errmgr, "LogicalShiftLast: unknown RBC symbol");
		}
		/* Clean the temporary sons list. */
		FREE(sons);
	}
...
static void SubstRbcLast(Rbc_t *f, char *SubstRbcData, nusmv_ptrint sign)
{
	...
	if (f->symbol == RBCVAR) {
		...
		if (f->symbol == RBCAND)
			...
			ErrorMgr_internal_error(
				errmgr, "SubstRbcLast: unknown RBC symbol");
		}
		/* Clean the temporary sons list. */
		FREE(sons);
	}
...
static void LogicalSubstRbcLast(Rbc_t *f, char *SubstRbcData, nusmv_ptrint sign)
{
	...
	if (f->symbol == RBCVAR) {
		...
		if (f->symbol == RBCAND)
			...
			ErrorMgr_internal_error(
				errmgr,
				"LogicalSubstRbcLast: unknown RBC symbol");
		}
		/* Clean the temporary sons list. */
		FREE(sons);
	}
