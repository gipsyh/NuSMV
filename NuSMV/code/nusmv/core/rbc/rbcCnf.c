  ...
  *literalAssignedToWholeFormula = INT_MAX;
  ...
  default: {
    ...
    StreamMgr_print_error(
        streams, "%s: No RBC2CNF conversion algorithm has been enabled.\n",
        __func__);
...
int Rbc_CnfVar2RbcIndex(Rbc_Manager_t *rbcManager, int cnfVar) {
  ...
  if (Dag_VertexGetRef(rbcNode) == RBCDUMMY)
    return -1;
...
int Rbc_get_node_cnf(Rbc_Manager_t *rbcm, Rbc_t *f, int *maxvar) {
  ...
  var = NODE_TO_INT(
      find_assoc(rbcm->rbcNode2cnfVar_model, (node_ptr)Dag_VertexGetRef(f)));

  /* try to fetch cnf var as a fallback */
  if (0 == var) {
    var = NODE_TO_INT(
        find_assoc(rbcm->rbcNode2cnfVar_cnf, (node_ptr)Dag_VertexGetRef(f)));
  }
  ...
  if (0 == var) {
    ...
    if (Dag_VertexGetRef(f) != RBCDUMMY) {
      insert_assoc(is_model ? rbcm->rbcNode2cnfVar_model
                            : rbcm->rbcNode2cnfVar_cnf,
                   (node_ptr)Dag_VertexGetRef(f), NODE_FROM_INT(var));
    }

    /* make the association: CNF var -> RBC node */
    insert_assoc(is_model ? rbcm->cnfVar2rbcNode_model
                          : rbcm->cnfVar2rbcNode_cnf,
                 NODE_FROM_INT(var), (node_ptr)Dag_VertexGetRef(f));
  }
...
const char *Rbc_CnfGetValidRbc2CnfAlgorithms() {
  return RBC_TSEITIN_CONVERSION_NAME ", " RBC_SHERIDAN_CONVERSION_NAME;
}
