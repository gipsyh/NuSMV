		    ...
		    int *literalAssignedToWholeFormula)
{
	...
	*literalAssignedToWholeFormula = INT_MAX;
	...
	if ((rbcManager->maxUnchangedRbcVariable ==
	     rbcManager->maxCnfVariable) &&
	...
	case RBC_SHERIDAN_CONVERSION:
		result = Rbc_Convert2CnfCompact(rbcManager, f, polarity,
						clauses, vars,
						literalAssignedToWholeFormula);
		break;
	...
	default: {
		const StreamMgr_ptr streams = STREAM_MGR(NuSMVEnv_get_value(
			Rbc_ManagerGetEnvironment(rbcManager),
			ENV_STREAM_MANAGER));
...
int Rbc_RbcIndex2CnfVar(Rbc_Manager_t *rbcManager, int rbcIndex)
{
	...
	if (0 == var) { /* cnf cache as a fallback */
		var = NODE_TO_INT(
			find_assoc(rbcManager->rbcNode2cnfVar_cnf,
				   (node_ptr)Dag_VertexGetRef(rbcNode)));
	}
