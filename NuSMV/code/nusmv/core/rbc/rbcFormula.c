...
#define REDUCE_ONE_POSITIVE_SIDE(a, b, c, d, rbcManager, changed, s)           \
	...
	{                                                                      \
		if (a == left) {                                               \
			RBC_GET_CHILDREN_WITH_ASSERT(a, NIL(Rbc_t), c, d, c,   \
						     d)                        \
		} /* if (a == right) */                                        \
		else {                                                         \
			RBC_GET_CHILDREN_WITH_ASSERT(NIL(Rbc_t), a, c, d, c,   \
						     d)                        \
		}                                                              \
                                                                               \
...
#define REDUCE_ONE_NEGATIVE_SIDE(a, b, c, d, changed, s)                             \
	...
	{                                                                            \
		...
		if CHECK_ASYMMETRIC_SUBSTITUTION_O3 (c, d, b, a, changed)            \
				...
				d, c, b, a,                                          \
				changed) else if CHECK_ASYMMETRIC_SUBSUMPTION_O2 (c, \
										  d, \
										  b, \
										  s) \
	}
...
#define REDUCE_ONE_NEG_AND_ONE_POS_SIDES(a, b, c, d, e, f, changed, s)               \
	...
	{                                                                            \
		...
		if CHECK_SYMMETRIC_SUBSUMPTION_O2 (c, d, e, f, b, s)                 \
				...
				c, d, e, f, a, changed,                              \
				s) else if CHECK_SYMMETRIC_SUBSTITUTION_O3 (d,       \
									    c,       \
									    e,       \
									    f,       \
									    a,       \
...
Rbc_t *Rbc_GetIthVar(Rbc_Manager_t *rbcManager, int varIndex)
{
	...
	if (rbcManager->varTable[varIndex] == NIL(Rbc_t)) {
		rbcManager->varTable[varIndex] =
			Dag_VertexInsert(rbcManager->dagManager, RBCVAR,
		   ...
		   Rbc_Bool_c sign)
{
	...
	while (changed) {
		...
		/* case: AND(AND(a,b),c) */
		if REDUCE_ONE_POSITIVE_SIDE (left, right, l1, l2, rbcManager,
					     changed, sign)
			/* case: AND(~AND(a,b),c) */
			else if REDUCE_ONE_NEGATIVE_SIDE (left, right, l1, l2,
							  changed, sign) else
			{
				l1 = l2 =
					(Rbc_t *)NULL; /* to silent warnings */
			}

		/* case: AND(c,AND(a,b)) */
		if REDUCE_ONE_POSITIVE_SIDE (right, left, r1, r2, rbcManager,
					     changed, sign)
			/* case: AND(c,~AND(a,b)) */
			else if REDUCE_ONE_NEGATIVE_SIDE (right, left, r1, r2,
							  changed, sign) else
			{
				r1 = r2 =
					(Rbc_t *)NULL; /* to silent warnings */
		...
		if (!RbcIsSet(left) && !RbcIsSet(right) &&
			...
			RBC_GET_CHILDREN_WITH_ASSERT(left, right, l1, l2, r1,
						     r2)
			...
			} else if CHECK_IDEMPOTENCE_O4 (l1, l2, r1, r2, right,
							changed)
				else if CHECK_IDEMPOTENCE_O4 (r1, r2, l2, l1, left, changed) else if CHECK_IDEMPOTENCE_O4 (
					l1, l2, r2, r1, right,
					changed) else if CHECK_IDEMPOTENCE_O4 (r1,
									       r2,
									       l1,
									       l2,
									       left,
		...
		/* case: AND(~AND(a,b),~AND(c,d)) */
			...
			if CHECK_RESOLUTION_O3 (l1, l2, r1, r2, sign)
				else if CHECK_RESOLUTION_O3 (l2, l1, r1, r2,
							     sign)
		}
		/* case: AND(~AND(a,b),AND(c,d)) */
		else if REDUCE_ONE_NEG_AND_ONE_POS_SIDES (left, right, l1, l2,
							  r1, r2, changed, sign)
			/* case: AND(AND(a,b),~AND(c,d)) */
			else if REDUCE_ONE_NEG_AND_ONE_POS_SIDES (right, left,
								  r1, r2, l1,
								  l2, changed,
								  sign)
#endif
	...
	rTemp = Dag_VertexLookup(rbcManager->dagManager, RBCAND, NIL(char),
				 sons, 2);
		   ...
		   Rbc_Bool_c sign)
{
	...
	/* Lookup the formula in the dag. */
	rTemp = Dag_VertexLookup(rbcManager->dagManager, RBCIFF, NIL(char),
				 sons, 2);
...
/* Eliminate the IFF connective changing it in an AND formula. */
	...
	return Rbc_MakeAnd(rbcManager,
			   (Rbc_MakeAnd(rbcManager, left,
					RbcId(right, RBC_FALSE), RBC_FALSE)),
			   (Rbc_MakeAnd(rbcManager, RbcId(left, RBC_FALSE),
					right, RBC_FALSE)),
			   (Rbc_Bool_c)(sign));
#endif
} /* End of Rbc_MakeIff. */
		   ...
		   Rbc_Bool_c sign)
{
	...
	while (changed) {
		...
		if (i == rbcManager->one) {
			...
			/* ITE(i,F,e) == AND(~i,e) */
			return Rbc_MakeAnd(rbcManager,
					   Rbc_MakeNot(rbcManager, i), e, sign);
		} else if (e == rbcManager->one) {
			/* ITE(i,t,T) == OR(~i,t) */
			return Rbc_MakeOr(rbcManager,
					  Rbc_MakeNot(rbcManager, i), t, sign);
	...
	/* Lookup the formula in the dag. */
	rTemp = Dag_VertexLookup(rbcManager->dagManager, RBCITE, NIL(char),
				 sons, 3);
