...
node_ptr ex_explain(BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, bdd_ptr f)
{
	...
	if (bdd_is_false(dd_manager, acc)) {
		...
		path = Extend_trace_with_state_input_pair(fsm, enc, path,
							  starting_state, acc,
							  "ex_explain: (1).");
	}
	...
	if ((res != Nil) && (g != g_si)) {
		...
		if (state != si) {
         ...
         not know. */
			...
			next_states = BddFsm_get_constrained_forward_image(
				fsm, state, input);
			...
			res = cons(nodemgr, (node_ptr)bdd_dup(next_state),
				   cons(nodemgr, (node_ptr)bdd_dup(input),
					res));
		    ...
		    bdd_ptr g)
{
	...
	if (opt_verbose_level_gt(opts, 1)) {
		...
		Logger_nlog(logger, wffprint,
			    "searching (counter)example for %N\n",
       ...
       time to restrict new path to minterms and return. */
		...
		if (bdd_isnot_false(dd_manager, tmp)) {
			...
			witness_path = Extend_trace_with_states_inputs_pair(
				fsm, enc, witness_path,
				(bdd_ptr)car(witness_path), state,
				"eu_explain: (1).");
			bdd_free(dd_manager, state);
			mc_eu_explain_restrict_state_input_to_minterms(
				fsm, enc, witness_path, path);
    ...
    */
		witness_path = Extend_trace_with_states_inputs_pair(
			fsm, enc, witness_path, (bdd_ptr)car(witness_path),
			_new, "eu_explain: (1).");
	} /* while true */
		    ...
		    bdd_ptr arg_g)
{
  ...
  */
    ...
    */
		...
		witness_path =
			ex_explain(fsm, enc, witness_path, starting_state);
    ...
    */
		...
		if (witness_path == Nil) {
			...
			ErrorMgr_internal_error(
				errmgr, "eg_explain: witness_path == Nil");
		}
		bdd_free(dd_manager, starting_state);
	} /* loop */
	...
	{ /* Block 1 */
		...
		if (new_witness_path != Nil) {
			...
			if (i <= (sup - inf)) {
				...
				bdd_ptr x = BddEnc_pick_one_state(
					enc, (bdd_ptr)car(witness_path));
				...
				while (true) {
					...
					mc_explain_debug_check_not_empty_state(
						fsm, enc, x, "ebu_explain");
					...
					if (m ==
					    path) { /* if we reached the first state, it's over */
             ...
             there is a path from the current x to it. */
					tmp_1 = BddFsm_get_constrained_backward_image(
						fsm, x, is);
		     ...
		     int inf, int sup)
{
  ...
  */
	...
	{ /* Block 2 */
		...
		if (opt_use_fair_states(opts)) {
			bdd_ptr fair_states_bdd = BddFsm_get_fair_states(fsm);
			bdd_and_accumulate(dd_manager, &fair_g,
					   fair_states_bdd);
			bdd_free(dd_manager, fair_states_bdd);
		}
    ...
    */
		...
		{ /* Block 3 */
			node_ptr m = witness_path;
			bdd_ptr tmp_1 =
				bdd_and(dd_manager, fair_g, (bdd_ptr)car(m));
           ...
           car(m) may include states not satisfying fair_g */
...

				{
					...
					bdd_ptr inputs =
						BddFsm_states_to_states_get_inputs(
							fsm, nx, x);
					bdd_ptr input = BddEnc_pick_one_input(
						enc, inputs);
			      ...
			      node_ptr formula_expr, node_ptr context)
{
	...
	case CONTEXT:
		return explain_recur(fsm, enc, path, cdr(formula_expr),
				     car(formula_expr));
	...
	case IMPLIES:
	case IFF:
		new_path = explain_recur(fsm, enc, path, car(formula_expr),
					 context);
		if (new_path != Nil)
			return new_path;
		else
	...
	case EX:
		...
		if (new_path != Nil) {
			node_ptr q = explain_recur(fsm, enc, new_path,
						   car(formula_expr), context);
			if (q != Nil)
				return q;
		}
		return new_path;
	...
	case EBU:
		...
		{
			int inf = BddEnc_eval_num(enc, car(cdr(formula_expr)),
						  context);
			int sup = BddEnc_eval_num(enc, cdr(cdr(formula_expr)),
						  context);

			ErrorMgr_set_the_node(errmgr, formula_expr);
			new_path =
				ebu_explain(fsm, enc, path, a1, a2, inf, sup);
		}
		bdd_free(dd_manager, a2);
		bdd_free(dd_manager, a1);
    ...
    */
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_ambiguous(rs)) {
			char *err =
				ResolveSymbol_get_error_message(rs, wffprint);
			ErrorMgr_rpterr(errmgr, "%s", err);
			FREE(err);
		}

		if (ResolveSymbol_is_parameter(rs)) {
			node_ptr par = SymbTable_get_flatten_actual_parameter(
				st, resName);
				   ...
				   bdd_ptr next_states, const char *comment)
{
	...
	next_state = BddEnc_pick_one_state(enc, next_states);
	inputs = BddFsm_states_to_states_get_inputs(fsm, starting_state,
						    next_state);
	input = BddEnc_pick_one_input(enc, inputs);
				   ...
				   const char *varname)
{
	...
	for (i = 0; i < l; i++) {
		...
		if (must_abort) {
			StreamMgr_print_error(
				streams, "**** DOOMED TO ABORT, STEP %d\n", i);
		}
	...
	BddFsm_ptr fsm, BddEnc_ptr enc, node_ptr path, node_ptr initial_node)
{
	...
	for (iter = path; iter != initial_node; iter = cdr(cdr(iter))) {
       ...
       by construction it is such. */
		...
		nusmv_assert(iter != Nil && cdr(iter) != Nil &&
			     cdr(cdr(iter)) != Nil);
      ...
      */
			...
			bdd_ptr image = BddFsm_get_constrained_backward_image(
				fsm, current_state, inputs);
						   ...
						   const char *message)
{
	...
	if (bdd_is_false(dd_manager, tmp)) {
		...
		bdd_free(dd_manager, tmp);
		ErrorMgr_internal_error(errmgr, "%s: state not reachable",
					message);
	} else {
		bdd_free(dd_manager, tmp);
	}
