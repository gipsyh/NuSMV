...
void Mc_CheckInvarSilently(NuSMVEnv_ptr env, Prop_ptr prop, Trace_ptr *trace) {
  ...
  Mc_CheckInvar_With_Strategy(env, prop, opt_check_invar_strategy(opts), trace,
                              true);
...
void Mc_CheckInvar_With_Strategy(NuSMVEnv_ptr env, Prop_ptr prop,
  ...
  Mc_CheckInvar_With_Strategy_And_Symbols(env, prop, strategy, output_trace,
                                          silent, symbols);
...
void Mc_CheckInvar_With_Strategy_And_Symbols(NuSMVEnv_ptr env, Prop_ptr prop,
  ...
  if (BddFsm_has_cached_reachable_states(fsm) &&
    ...
    if (Prop_needs_rewriting(symb_table, prop)) {
      ...
      bdd_ptr tmp =
          BddTrans_get_backward_image_state(BddFsm_get_trans(fsm), not_prop);
  ...
  {
    check_invariant(env, Prop_get_bdd_fsm(prop), prop, strategy, symbols,
                    &trace);
...
static Trace_ptr compute_path_fb(BddFsm_ptr fsm, bdd_ptr target_states,
  ...
  /* Use the ''compute_and_complete_path'' without asking any
     completions */
  res = compute_and_complete_path(fsm, bad_state, bad_state, f_list, b_list,
                                  symbols, &empty);
...
static Trace_ptr compute_and_complete_path(BddFsm_ptr fsm,
  ...
  ftrace = Mc_create_trace_from_bdd_state_input_list(
      enc, symbols, "AG alpha Counterexample", TRACE_TYPE_CNTEXAMPLE, fpath);

  btrace = Mc_create_trace_from_bdd_state_input_list(
      enc, symbols, "AG alpha Counterexample", TRACE_TYPE_CNTEXAMPLE, bpath);
...
static int check_invariant_forward_backward_with_break(
  ...
  rewriter =
      Prop_Rewriter_create(env, inv_prop, WFF_REWRITE_METHOD_DEADLOCK_FREE,
                           WFF_REWRITER_REWRITE_INPUT_NEXT, fsm_type, enc);
  ...
  if (diameter > 0) {
    ...
    for (i = 0; i < diameter; i++) {
      forward_reachable_list =
          cons(nodemgr, (node_ptr)bdd_dup(layers[i]), forward_reachable_list);
    }
    ...
    forward_reachable_list =
        cons(nodemgr, (node_ptr)bdd_dup(last_reachable_states), Nil);
  ...
  if (bdd_isnot_false(dd, buggy)) {
    ...
    if (bdd_isnot_false(dd, buggy_first)) {
      ...
      if (opt_counter_examples(opts)) {
        if (opt_verbose_level_gt(opts, 1)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(logger, "Starting the binary search to fin the shortest "
                             "counterexample.\n");
        }
    ...
    if (completed) {
      ...
      if (opt_verbose_level_gt(opts, 1)) {
        Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
        Logger_log(logger, "Cached analysis is complete, and there is no "
      ...
      */
      ...
      while (bdd_isnot_false(dd, bad_frontier) &&
        ...
        if (stopping_h(dd, bad_frontier, reachable_frontier,
                       last_reachable_states, last_bad_states, turn)) {
          /* The answer is NO: we have to continue the BDD search */

          /* Ask heuristic function for the direction to follow */
          if (FORWARD_STEP == heuristic(dd, bad_frontier, reachable_frontier,
                                        last_reachable_states, last_bad_states,
                                        turn)) {
            ...
            reachable_frontier =
                bdd_and(dd, last_reachable_states, not_f_prev_reachable);
            ...
            /* Append reachable states to the layers list */
            forward_reachable_list =
                cons(nodemgr, (node_ptr)bdd_dup(last_reachable_states),
                     forward_reachable_list);
            ...
            if (opt_verbose_level_gt(opts, 1)) {
              ...
              Logger_log(logger, "New frontier size: %d.\n",
                         bdd_size(dd, reachable_frontier));
            }
          } else {
            ...
            /* Append reachable states to the layers list */
            backward_reachable_list =
                cons(nodemgr, (node_ptr)bdd_dup(last_bad_states),
                     backward_reachable_list);
            ...
            if (opt_verbose_level_gt(opts, 1)) {
              ...
              Logger_log(logger, "New bug frontier size: %d.\n",
          ...
          if (forwardStep > 1) {
            ...
            reachables = BddEnc_bdd_to_expr(
                bdd_enc, (bdd_ptr)car(cdr(forward_reachable_list)));
          } else {
          ...
          if (backwardStep > 1) {
            ...
            bads = BddEnc_bdd_to_expr(
                bdd_enc, bdd_dup((bdd_ptr)car(cdr(backward_reachable_list))));
          } else {
            /* No space explored, so nothing to exclude */
          ...
          new_fsm = BeFsm_create(be_enc, be_init, be_invars,
          ...
          binvarspec =
              Wff2Nnf(env, BddEnc_bdd_to_expr(bdd_enc, bdd_not_bad_frontier));
          ...
          if (strcasecmp(algorithm_name, BMC_INVAR_ALG_CLASSIC) == 0) {
            res = Bmc_induction_algorithm(env, new_fsm, binvarspec, &trace,
                                          symbols);
          } else if (strcasecmp(algorithm_name, BMC_INVAR_ALG_EEN_SORENSSON) ==
                     0) {
            res = Bmc_een_sorensson_algorithm_without_dump(
                env, new_fsm, bool_fsm, binvarspec, max_k, false, &trace);
          } else {
            StreamMgr_print_error(
                streams, "'%s' is an invalid algorithm for BDD-BMC approach.\n",
                algorithm_name);
          ...
          case BMC_FALSE: {
            if (opt_counter_examples(opts)) {
              ...
              *output_trace = complete_bmc_trace_with_bdd(
                  &trace, symbols, bdd_enc, fsm, forward_reachable_list,
                  backward_reachable_list);
            }
          ...
          if (forwardStep > diameter) {
            ...
            BddFsm_update_cached_reachable_states(fsm, forward_reachable_list,
                                                  forwardStep + 1, false);
          } else {
      ...
      */
      if (bdd_isnot_false(dd, target_states)) {
        ...
        if (opt_counter_examples(opts)) {
          *output_trace =
              compute_path_fb(fsm, target_states, forward_reachable_list,
                              backward_reachable_list, symbols);
        }
...
static int check_invariant(NuSMVEnv_ptr env, BddFsm_ptr fsm, Prop_ptr inv_prop,
  ...
  case FORWARD:
    return check_invariant_forward_backward_with_break(
        env, fsm, inv_prop, forward_heuristic, never_stopping_heuristic,
        symbols, trace);

  case BACKWARD:
    return check_invariant_forward_backward_with_break(
        env, fsm, inv_prop, backward_heuristic, never_stopping_heuristic,
        symbols, trace);
  ...
  case BDD_BMC:
    return check_invariant_forward_backward_with_break(
        env, fsm, inv_prop, forward_backward_heuristic, stopping_heuristic,
        symbols, trace);
  default:
...
static Trace_ptr complete_bmc_trace_with_bdd(Trace_ptr *trace,
  ...
  res = compute_and_complete_path(bdd_fsm, begin_bdd, end_bdd, f_list, b_list,
                                  symbols, trace);
