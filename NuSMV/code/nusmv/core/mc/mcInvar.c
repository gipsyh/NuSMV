...
static int check_invariant(NuSMVEnv_ptr env, BddFsm_ptr fsm, Prop_ptr inv_prop,
			   Check_Strategy strategy, NodeList_ptr symbols,
			   Trace_ptr *trace);
...
static boolean stopping_heuristic(DDMgr_ptr dd, bdd_ptr reachable_frontier,
				  bdd_ptr bad_frontier,
				  bdd_ptr reachable_states, bdd_ptr bad_states,
				  int turn);
...
static Trace_ptr complete_bmc_trace_with_bdd(Trace_ptr *trace,
					     ...
					     BddEnc_ptr bdd_enc,
					     BddFsm_ptr bdd_fsm,
					     node_ptr f_list, node_ptr b_list);

static Trace_ptr compute_path_fb(BddFsm_ptr fsm, bdd_ptr target_states,
				 node_ptr f_list, node_ptr b_list,
				 NodeList_ptr symbols);
...
static int check_invariant_forward_backward_with_break(
	NuSMVEnv_ptr env, BddFsm_ptr fsm, Prop_ptr inv_prop,
	heuristic_type heuristic, stopping_heuristic_type stopping_h,
	NodeList_ptr symbols, Trace_ptr *output_trace);
...
void Mc_CheckInvar_With_Strategy(NuSMVEnv_ptr env, Prop_ptr prop,
				 ...
				 Trace_ptr *output_trace, boolean silent)
{
	const OptsHandler_ptr opts =
		OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));
	...
	Mc_CheckInvar_With_Strategy_And_Symbols(env, prop, strategy,
						output_trace, silent, symbols);
					     ...
					     NodeList_ptr symbols)
{
	...
	if (opt_verbose_level_gt(opts, 0)) {
		...
		print_invar(Logger_get_ostream(logger), prop,
	...
	if (BddFsm_has_cached_reachable_states(fsm) &&
		...
		bdd_ptr prop_bdd =
			BddEnc_expr_to_bdd(enc, Prop_get_expr_core(prop), Nil);
		boolean prop_holds;

		if (Prop_needs_rewriting(symb_table, prop)) {
			bdd_ptr not_prop = bdd_not(dd, prop_bdd);
			bdd_ptr tmp = BddTrans_get_backward_image_state(
				BddFsm_get_trans(fsm), not_prop);
		...
		if (opt_counter_examples(opts) && TRACE(NULL) != trace) {
			if ((Trace_ptr *)NULL != output_trace) {
				*output_trace = trace;
				...
				int tr;
				TraceMgr_ptr tm = TRACE_MGR(
					NuSMVEnv_get_value(env, ENV_TRACE_MGR));
				 ...
				 NodeList_ptr symbols)
{
	...
	/* Use the ''compute_and_complete_path'' without asking any
     completions */
	res = compute_and_complete_path(fsm, bad_state, bad_state, f_list,
					b_list, symbols, &empty);
					   ...
					   Trace_ptr *middle_trace)
{
   ...
   *********************/
	...
	while (Nil != b_list) {
		...
		if (car(b_list) == Nil)
			ErrorMgr_internal_error(errmgr, "b_list == Nil");
       ...
       with the reachable step in the ''b_list'' list */
		...
		inputs = BddFsm_states_to_states_get_inputs(fsm, state,
							    state_next);
   ...
   *********************/
	...
	while (Nil != f_list) {
		...
		/* The element must be a valid reachable step */
		if (car(f_list) == Nil)
			ErrorMgr_internal_error(errmgr, "f_list == Nil");
       ...
       intersection with the reachable step in the ''f_list'' list */
		...
		inputs = BddFsm_states_to_states_get_inputs(fsm, state_prev,
							    state);
		input = BddEnc_pick_one_input(enc, inputs);
   ...
   *********************/
	...
	ftrace = Mc_create_trace_from_bdd_state_input_list(
		enc, symbols, "AG alpha Counterexample", TRACE_TYPE_CNTEXAMPLE,
		fpath);

	btrace = Mc_create_trace_from_bdd_state_input_list(
		enc, symbols, "AG alpha Counterexample", TRACE_TYPE_CNTEXAMPLE,
		bpath);
	...
	NodeList_ptr symbols, Trace_ptr *output_trace)
{
	...
	if (diameter > 0) {
		...
		for (i = 0; i < diameter; i++) {
			forward_reachable_list =
				cons(nodemgr, (node_ptr)bdd_dup(layers[i]),
				     forward_reachable_list);
		}
      ...
      */
			...
			reachable_frontier =
				bdd_and(dd, last_reachable_states, neg);
			bdd_free(dd, neg);
		} else {
		...
		forward_reachable_list = cons(
			nodemgr, (node_ptr)bdd_dup(last_reachable_states), Nil);
	...
	backward_reachable_list = cons(nodemgr,
				       (node_ptr)bdd_dup(last_bad_states),
	...
	if (bdd_isnot_false(dd, buggy)) {
		...
		if (opt_verbose_level_gt(opts, 1)) {
			...
			Logger_log(
				logger,
				"Bug found in initial state or in cached states\n");
		}
		...
		if (bdd_isnot_false(dd, buggy_first)) {
			...
			path = cons(nodemgr, (node_ptr)bdd_dup(init_state),
				    Nil);

			/* Get the counter example if needed */
			if (opt_counter_examples(opts)) {
				*output_trace = compute_path_fb(
					fsm, buggy_first, path, Nil, symbols);
			}
         ...
         then cache the result */
           ...
           NOTE: this function frees path list */
				BddFsm_update_cached_reachable_states(fsm, path,
								      1, false);
			} else {
				/* Free the path list */
         ...
         area */
			...
			if (opt_verbose_level_gt(opts, 1)) {
				...
				Logger_log(
					logger,
					"The bug is not in the initial state.\n");
			}
			...
			if (opt_counter_examples(opts)) {
				if (opt_verbose_level_gt(opts, 1)) {
					...
					Logger_log(
						logger,
						"Starting the binary search to fin the shortest "
						"counterexample.\n");
				}
         ...
         */
				...
				do {
					...
					target_states =
						bdd_and(dd, layers[pos],
							last_bad_states);

					state_buggy = bdd_isnot_false(
						dd, target_states);

					tmp = bdd_and(dd, layers[pos - 1],
						      last_bad_states);
					previous_state_safe =
						bdd_is_false(dd, tmp);
					bdd_free(dd, tmp);
		...
		if (completed) {
         ...
         is no bugs */
			...
			if (opt_verbose_level_gt(opts, 1)) {
				...
				Logger_log(
					logger,
					"Cached analysis is complete, and there is no "
					"intersection with buggy states.\n");
			}
      ...
      */
			...
			while (bdd_isnot_false(dd, bad_frontier) &&
               ...
               already intersected with invars) */
						...
						not_f_prev_reachable = bdd_not(
							dd, f_prev_reachable);

						/* Update the current frontier */
						bdd_free(dd,
							 reachable_frontier);
						reachable_frontier = bdd_and(
							dd,
							last_reachable_states,
							not_f_prev_reachable);
               ...
               already intersected with invars) */
						image = BddFsm_get_backward_image(
							fsm, bad_frontier);
						...
						not_f_prev_bad =
							bdd_not(dd, f_prev_bad);

						/* Update the current frontier */
						bdd_free(dd, bad_frontier);
						bad_frontier = bdd_and(
							dd, last_bad_states,
							not_f_prev_bad);
             ...
             the current frontiers. */
               ...
               BMC search */
						bads = BddEnc_bdd_to_expr(
							bdd_enc,
							bdd_dup((bdd_ptr)car(cdr(
								backward_reachable_list))));
					} else {
						/* No space explored, so nothing to exclude */
					...
					be_reachables = Bmc_Conv_Bexp2Be(
						be_enc, reachables);
					be_bads =
						Bmc_Conv_Bexp2Be(be_enc, bads);
					...
					be_invars = Be_And(
						manager, be_invars,
						BeFsm_get_invar(model_fsm));

					/* Calculate new inits */
					be_init = Bmc_Conv_Bexp2Be(
						be_enc,
						BddEnc_bdd_to_expr(
							bdd_enc,
							reachable_frontier));
             ...
             by the ''bmc_invar_alg'' system variable */
					algorithm_name = util_strsav((
						char *)get_bmc_invar_alg(opts));

					if (strcasecmp(algorithm_name,
						       BMC_INVAR_ALG_CLASSIC) ==
					    0) {
						res = Bmc_induction_algorithm(
							env, new_fsm,
							binvarspec, &trace,
							symbols);
					} else if (strcasecmp(
						   ...
						   0) {
						res = Bmc_een_sorensson_algorithm_without_dump(
							env, new_fsm, bool_fsm,
							binvarspec, max_k,
							false, &trace);
					} else {
					...
					case BMC_UNKNOWN: {
						int bmc_tr;
						if (opt_counter_examples(
							    opts) &&
						    TRACE(NULL) !=
							    trace) { /* register trace (if applicable) */
							bmc_tr = TraceMgr_register_trace(
								TRACE_MGR(NuSMVEnv_get_value(
									env,
									ENV_TRACE_MGR)),
								trace);
							Prop_set_trace(
                 ...
                 be NULL, depending on other fragments. */
							*output_trace = complete_bmc_trace_with_bdd(
								&trace, symbols,
								bdd_enc, fsm,
								forward_reachable_list,
								backward_reachable_list);
					...
					default:
						ErrorMgr_rpterr(
							errmgr,
							"Unexpected result in BMC check!");
					}
...
static void print_result(NuSMVEnv_ptr env, Prop_ptr p)
{
	...
	print_invar(StreamMgr_get_output_ostream(streams), p,
		    get_prop_print_method(opts));
	...
	if (Prop_False == Prop_get_status(p)) {
		...
		if (opt_counter_examples(opts)) {
			if (Prop_get_trace(p) > 0) {
				...
				StreamMgr_print_output(
					streams,
					"-- Cannot build counter-example. Layers "
		...
		StreamMgr_print_output(
			streams, " is true or false : the induction fails\n");
			   ...
			   Trace_ptr *trace)
{
	switch (strategy) {
	case FORWARD:
		return check_invariant_forward_backward_with_break(
			env, fsm, inv_prop, forward_heuristic,
			never_stopping_heuristic, symbols, trace);

	case BACKWARD:
		return check_invariant_forward_backward_with_break(
			env, fsm, inv_prop, backward_heuristic,
			never_stopping_heuristic, symbols, trace);

	case FORWARD_BACKWARD:
		return check_invariant_forward_backward_with_break(
			env, fsm, inv_prop, forward_backward_heuristic,
			never_stopping_heuristic, symbols, trace);
	case BDD_BMC:
		return check_invariant_forward_backward_with_break(
			env, fsm, inv_prop, forward_backward_heuristic,
			stopping_heuristic, symbols, trace);
	default:
						 ...
						 bdd_ptr bad_states, int turn)
{
	...
	case SMALLEST_BDD_HEURISTIC:
		if (bdd_size(dd, reachable_states) <=
		    bdd_size(dd, bad_states)) {
			return BACKWARD_STEP;
		} else {
			return FORWARD_STEP;
		}
				  ...
				  int turn)
{
	...
	case SIZE_HEURISTIC:
		return (bdd_size(dd, reachable_states) +
				bdd_size(dd, bad_states) <
					     ...
					     node_ptr f_list, node_ptr b_list)
{
	...
	res = compute_and_complete_path(bdd_fsm, begin_bdd, end_bdd, f_list,
					b_list, symbols, trace);
