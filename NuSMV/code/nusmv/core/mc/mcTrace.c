						    ...
						    node_ptr path)
{
	...
	trace = Trace_create(BaseEnc_get_symb_table(BASE_ENC(bdd_enc)), desc,
			     type, symbols, false);
						  ...
						  node_ptr path)
{
	...
	while (path != Nil) {
		...
		mc_trace_step_put_values(trace, step, bdd_enc,
					 BDD_INPUTS(car(path)), i_vars);

#if MC_MODEL_DEBUG
		mc_model_trace_step_print(trace, step, TRACE_ITER_I_VARS, "I",
					  ++i);
#endif
		...
		mc_trace_step_put_values(trace, step, bdd_enc,
					 BDD_STATES(car(path)), sf_vars);
		path = cdr(path);

#if MC_MODEL_DEBUG
		mc_model_trace_step_print(trace, step, TRACE_ITER_S_VARS, "S",
					  i);
#endif
	}
				      ...
				      BddEnc_ptr bdd_enc, bdd_ptr bdd)
{
	...
	mc_trace_step_put_values(trace, step, bdd_enc, bdd,
				 Trace_get_sf_vars(trace));
#if MC_MODEL_DEBUG
				     ...
				     NodeList_ptr symbols)
{
	...
	NODE_LIST_FOREACH(symbols, iter)
	{
		...
		if (SymbTable_is_symbol_var(st, sym_cleaned)) {
			...
			type = TypeChecker_get_expression_type(tc, sym_cleaned,
							       Nil);
       ...
       reconsidered */
		if (!Compile_is_expr_booleanizable(st, sym_cleaned, false,
						   NULL)) {
		...
		if (!opt_keep_single_value_vars(opts)) {
			boolean printVar =
				BddEnc_is_var_in_cube(bdd_enc, sym, support);
			if (!printVar)
				continue; /* skip this var */
		}
    ...
    */
			...
			add_ptr add_values =
				add_if_then(dd, add, AddArray_get_add(sym_add));
