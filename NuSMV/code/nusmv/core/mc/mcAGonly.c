...
node_ptr make_AG_counterexample(BddFsm_ptr fsm, BddStates target_states)
{
	...
	for (i = distance - 1; i >= 0; --i) {
		...
		inputs = BddFsm_states_to_states_get_inputs(fsm, pre_image,
							    state);
			     ...
			     NodeList_ptr symbols, Trace_ptr *out_trace)
{
	...
	case AND:
		res = check_AG_only(fsm, enc, prop, car(spec), context, symbols,
				    out_trace);
		if (res) { /* lazy mc */
	...
	default:
		if (opt_verbose_level_gt(opts, 0)) {
			...
			print_spec(Logger_get_ostream(logger), prop,
				   get_prop_print_method(opts));
			Logger_log(logger,
				   "skipped: it is not an AG-only formula\n");
		}
	} /* switch */
