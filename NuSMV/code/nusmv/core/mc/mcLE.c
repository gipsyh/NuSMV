			       ...
			       boolean allinit, boolean verbose)
{
	...
	if (alg == BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_FWD) {
		elfwd_saved_options = Bdd_elfwd_check_set_and_save_options(
			env, BDD_ELFWD_OPT_FORWARD_SEARCH |
					       ...
					       boolean allinit, boolean verbose)
{
	...
	if (allinit) {
		...
		{ /* prints the number of fair-init states */
			...
			bdd_ptr mask =
				BddEnc_get_state_frozen_vars_mask_bdd(bddenc);

			bdd_and_accumulate(dd, &fair_init, mask);
			reached_cardinality =
				BddEnc_count_states_of_bdd(bddenc, fair_init);
			search_space_cardinality =
				BddEnc_count_states_of_bdd(bddenc, mask);
			bdd_free(dd, mask);
			StreamMgr_print_output(
				streams,
				"fair states: %g (2^%g) out of %g (2^%g)\n",
				reached_cardinality,
				log(reached_cardinality) / log(2.0),
				search_space_cardinality,
