...
void Mc_CheckCTLSpec(NuSMVEnv_ptr env, Prop_ptr prop)
{
	...
	if (opt_verbose_level_gt(opts, 0)) {
		...
		print_spec(Logger_get_ostream(logger), prop,
	...
	if (bdd_is_false(dd, s0)) {
		...
		if (opt_counter_examples(opts)) {
			...
			exp = reverse(explain(fsm, enc,
					      cons(nodemgr,
						   (node_ptr)bdd_dup(s0), Nil),
					      spec, Nil));
       ...
       is in the form "LTL Counterexample" */
			trace_title = ALLOC(
				char, strlen(Prop_get_type_as_string(prop)) +
...
BddStates eu(BddFsm_ptr fsm, BddStates f, BddStates g)
{
	...
	while (bdd_isnot_false(dd, new)) {
		...
		if (opt_verbose_level_gt(opts, 1)) {
			...
			Logger_log(logger,
				   "size of Y%d = %g states, %d BDD nodes\n",
				   n++, states, size);
		}
		bdd_free(dd, oldY);
		oldY = bdd_dup(Y);
	...
	while (bdd_isnot_false(dd, newY)) {
		...
		if (opt_verbose_level_gt(opts, 5)) {
			...
			Logger_log(
				...
				i++,
				BddEnc_count_states_inputs_of_bdd(enc, resY),
				bdd_size(dd, resY));
		}
...
bdd_ptr eg_si(BddFsm_ptr fsm, bdd_ptr g_si)
{
	...
	if (opt_use_reachable_states(opts)) {
		...
		bdd_and_accumulate(dd, &applicable_states_inputs,
				   reachable_states_bdd);
		bdd_free(dd, reachable_states_bdd);
	}

	fair_states_inputs =
		Mc_get_fair_si_subset(fsm, applicable_states_inputs);
	...
	do {
		if (opt_verbose_level_gt(opts, 1)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger,
				   "size of Rp%d = %g states, %d BDD nodes\n",
