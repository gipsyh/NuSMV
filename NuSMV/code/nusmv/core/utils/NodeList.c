...
static inline void node_list_update_count(const NodeList_ptr self,
                                          const node_ptr elem,
...
ListIter_ptr NodeList_search(const NodeList_ptr self, NodeListPred pred,
  ...
  if (pred == NULL) {
    if (!NodeList_belongs_to(self, (node_ptr)arg))
      return NULL_LINK;

    NODE_LIST_FOREACH(self, iter) {
      if (NodeList_get_elem_at(self, iter) == (node_ptr)arg)
        return iter;
    }
  } else {
    NODE_LIST_FOREACH(self, iter) {
      if (pred(NodeList_get_elem_at(self, iter), arg))
        return iter;
    }
  }
...
boolean ListIter_is_end(const ListIter_ptr self) { return (NULL_LINK == self); }

ListIter_ptr ListIter_get_end(void) { return NULL_LINK; }
...
void NodeList_sort(NodeList_ptr self,
  ...
  if (self->size > 1) {
    ...
    for (iter = NodeList_get_first_iter(self), j = 0; !NodeList_is_empty(self);
