...
int WordNumberMgr_max_width() { return INT_MAX; }
...
#define err_buf_size 200
  ...
  *tmpStr = '\0';
  ...
  succes =
      BigNumber_assign_number_from_string(buffer, error_char, base, &number);
  /* error in the value */
  if (!succes) {
...
WordNumber_ptr WordNumberMgr_concatenate(WordNumberMgr_ptr self,
  ...
  return word_number_create(self, nvalue, v1->value.width + v2->value.width,
                            (char *)NULL);
}
