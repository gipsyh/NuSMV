...
static WordNumber_ptr word_number_create(WordNumberMgr_ptr self,
					 ...
					 int width, const char *parsedString);
...
static int BigWordNumberMgr_max_ull_width(void)
{
	...
	if (0 ==
	    wordNumberMaxWidth) { /* the constant has not been initialized*/
		...
		wordNumberMaxWidth =
			i; /* max number of bits in "unsigned long long" */
						   ...
						   char *str, int base)
{
	...
	succes =
		BigNumber_assign_number_from_string(str, tmpStr, base, &number);
...
#define err_buf_size 200
	...
	nusmv_assert('0' ==
		     *currentStr); /*first character of Word constant is "0"*/
	++currentStr;
	...
	if ('_' != *currentStr) { /* the width is explicitly specified */
		nusmv_assert(
			isdigit(*currentStr)); /* only a digit can go here */
		...
		if ('_' !=
		    *tmpStr) { /* 'underscore must go after the width specifier */
			...
			if (NULL != errorString) {
				...
				SNPRINTF_CHECK(
					i,
					err_buf_size); /* above print was successful */
				*errorString = err_buf;
			}
			return WORD_NUMBER(NULL);
		}
	...
	if (bufferSize <
	    ((int)strlen(currentStr))) { /* reallocate the buffer */
	...
	for (tmpStr = buffer; '\0' != *currentStr; ++currentStr) {
		if ('_' != *currentStr) {
			*tmpStr =
				*currentStr; /* = toupper(*str); not needed at the moment */
			++tmpStr;
		}
	} /* for */
	*tmpStr = '\0';
	...
	if (-1 == width) { /* there was no width specifier => calculate it */
		...
		case 10: /* error */
			if ((char **)NULL != errorString) {
				*errorString =
					"decimal Word constant without width specifier";
			}
			return WORD_NUMBER(NULL);
		case 16:
			width *= 4;
	...
	succes = BigNumber_assign_number_from_string(buffer, error_char, base,
						     &number);
	/* error in the value */
	if (!succes) {
  ...
  */
	...
	if (isSigned && 10 == base && BigNumber_less_than(&pow2, &number)) {
	...
	if ((char **)NULL !=
	    errorString) { /* potential error in the constructor */
					    ...
					    WordNumberValue value, int width)
{
	...
	new_value = ((~0ULL) >> (BigWordNumberMgr_max_ull_width() - width)) &
		    value;
				     ...
				     WordNumber_ptr v2)
{
	...
	newv = WordNumber_evaluate_implies(v1->value, v2->value);
					 ...
					 WordNumber_ptr v1, WordNumber_ptr v2)
{
	...
	return word_number_create(
		self, nvalue, v1->value.width + v2->value.width, (char *)NULL);
}
					...
					int lowBit)
{
	...
	WORD_NUMBER_CHECK_INSTANCE(v);
	nusmv_assert(v->value.width > highBit && highBit >= lowBit &&
		     lowBit >= 0);
					   ...
					   WordNumber_ptr v, int numberOfTimes)
{
	WordNumberValue_intern nvalue;
	WORD_NUMBER_CHECK_INSTANCE(v);
	nusmv_assert(v->value.width + numberOfTimes <=
		     WordNumberMgr_max_width());
