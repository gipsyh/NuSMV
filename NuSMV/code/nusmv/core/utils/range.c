...
void Utils_range_check(const NuSMVEnv_ptr env, node_ptr n)
{
	...
	node_ptr the_var =
		NODE_PTR(NuSMVEnv_get_value(env, ENV_UTILS_CHECK_RANGE_VAR));
	node_ptr the_range =
		NODE_PTR(NuSMVEnv_get_value(env, ENV_UTILS_CHECK_RANGE_RANGE));
	boolean range_err = true;

	if (NuSMVEnv_has_flag(env, ENV_UTILS_CHECK_RANGE_IS_FATAL)) {
		range_err =
			NuSMVEnv_get_flag(env, ENV_UTILS_CHECK_RANGE_IS_FATAL);
	}
	...
	if (node_get_type(n) == CONS) {
		while (n != (node_ptr)NULL) {
			...
			if (node_get_type(car(n)) != FAILURE &&
			    !in_list(car(n), the_range)) {
				if (range_err) {
					ErrorMgr_range_error(errmgr, car(n),
							     the_var);
				} else {
					ErrorMgr_range_warning(errmgr, car(n),
							       the_var);
				}
			}
			n = cdr(n);
		...
		if (node_get_type(n) != FAILURE && !in_list(n, the_range)) {
			if (range_err) {
				ErrorMgr_range_error(errmgr, n, the_var);
			} else {
				ErrorMgr_range_warning(errmgr, n, the_var);
			}
		}
	}
}
...
void Utils_failure_node_check(const NuSMVEnv_ptr env, node_ptr n)
{
	...
	if (node_get_type(n) == FAILURE)
		ErrorMgr_report_failure_node(errmgr, n);
}
