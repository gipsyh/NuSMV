...
/*!
  ...
  * reverse a list or create a reversed copy.
  ...
  To have more functionalities you can use Olist and have slightly less
...
unsigned int Slist_get_size(Slist_ptr self) { return self->size; }
...
boolean Slist_is_empty(Slist_ptr self) { return NULL == self->first; }
...
boolean Siter_is_end(Siter iter) { return NULL == iter.node; }

boolean Siter_is_last(Siter iter) {
  if (Siter_is_end(iter))
    return false;
...
void Slist_append(Slist_ptr self, const Slist_ptr other) {
  ...
  for (iter = Slist_first(other); !Siter_is_end(iter);
...
void Slist_print(Slist_ptr const self, MasterPrinter_ptr wffprint,
  ...
  SLIST_FOREACH(self, iter) {
    ...
    if (NULL == printer) {
      ...
      printer(file, element, printer_arg);
