...
void *DLlist_delete_first(DLlist_ptr self)
{
	...
	DL_LIST_CHECK_INSTANCE(self);
	nusmv_assert(self->first !=
		     &(self->past_last)); /* list must not be empty */
	...
	node->element = PTR_FROM_INT(
		void *, 1); /* for sure incorrect value for debugging */
	node->prev = PTR_FROM_INT(
		void *, 1); /* for sure incorrect value for debugging */
	node->next = PTR_FROM_INT(
		void *, 1); /* for sure incorrect value for debugging */
	FREE(node);
	self->size -= 1;
...
DLiter DLiter_next(DLiter self)
{
	...
	nusmv_assert(self.node->next !=
		     NULL); /* iterator is past the last element */
...
DLiter DLiter_prev(DLiter self)
{
	...
	nusmv_assert(self.node->prev !=
		     NULL); /* iterator points to the first */
...
DLiter DLlist_insert_after(DLlist_ptr self, DLiter iter, void *element)
{
	...
	nusmv_assert(iter.node->next !=
		     NULL); /* points past the last element */
...
DLiter DLlist_delete(DLlist_ptr self, DLiter iter, void **element)
{
	...
	if (NULL ==
	    node->prev) { /* the first element was removed => care required */
		self->first = node->next;
	} else {
...
void dl_list_testing_function(StreamMgr_ptr streams)
{
	...
	StreamMgr_print_output(streams,
			       "TESTING DLlist class : in process ....\n");
...
static void dl_list_deinit(DLlist_ptr self)
{
	...
	while (node != &(self->past_last)) {
		DLnode_ptr tmp = node;
		node = tmp->next;
		tmp->prev = PTR_FROM_INT(
			void *, 1); /* for sure incorrect value for debugging */
		tmp->next = PTR_FROM_INT(
			void *, 1); /* for sure incorrect value for debugging */
		FREE(tmp);
	...
	self->first = PTR_FROM_INT(
		void *, 1); /* for sure incorrect value for debugging */
	self->past_last.prev = PTR_FROM_INT(
		void *, 1); /* for sure incorrect value for debugging */
