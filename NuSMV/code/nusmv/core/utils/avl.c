...
static void do_rebalance(register avl_node ***stack_nodep, register int stack_n)
{
	register avl_node **node_p, *node;
	register int hl, hr;
	int height;
			 ...
			 int (*compar)(char *, char *), int *error)
{
	...
	if (comp_height != node->height) {
		(void)StreamMgr_print_output(
			streams, "Bad height for 0x%p: computed=%d stored=%d\n",
	...
	if (bal > 1 || bal < -1) {
		(void)StreamMgr_print_output(
			streams, "Out of balance at node 0x%p, balance = %d\n",
			(void *)node, bal);
		++*error;
	}
	...
	if (node->left != NIL(avl_node) &&
		...
		(void)StreamMgr_print_output(
			streams, "Bad ordering between 0x%p and 0x%p",
