...
lsStatus lsDestroyListList(lsList list) { return lsDestroy(list, lsDestroyNN); }
...
lsList lsCopyListList(lsList list) { return lsCopy(list, (LS_PFLSG)lsCopyNN); }
...
lsGeneric (*copyFunc)(lsGeneric); /* Routine to copy user data */
{
  ...
  if (copyFunc == LS_COPY)
    return lsAppendCopy(list1, list2);
  if (!copyFunc)
    return lsAppendNN(list1, list2);
  newList = lsCreate();
  gen = lsStart(list1);
  while (lsNext(gen, &data, LS_NH) == LS_OK) {
    (void)lsNewEnd(newList, (*copyFunc)(data), LS_NH);
...
void lsJoin(list1, list2, copyFunc) lsList list1,
    list2;                        /* Lists to be joined        */
lsGeneric (*copyFunc)(lsGeneric); /* Routine to copy user data */
{
  lsGen gen;
  lsGeneric data;
...
lsStatus (*gen_func)(lsGen, lsGeneric *,
                     lsHandle *);         /* Generator function to use      */
