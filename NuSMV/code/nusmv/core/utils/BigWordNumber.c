...
char *WordNumber_to_based_string(WordNumber_ptr self, int base,
  ...
  return WordNumber_Internal_value_to_based_string(self->value, base, isSigned);
}
...
char *WordNumber_Internal_value_to_based_string(WordNumberValue_intern value,
  ...
  case 16:
    ...
    printed = snprintf(buffer, buffer_size, "0%s%c%d_%s", isSigned ? "s" : "u",
  ...
  case 10:
    ...
    printed =
        snprintf(buffer, buffer_size, "%s0%s%c%d_%s", (sign ? "-" : ""),
                 (isSigned ? "s" : "u"), base_char, value.width, number_rep);
