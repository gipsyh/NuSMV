				 ...
				 boolean isSigned)
{
	...
	return WordNumber_Internal_value_to_based_string(self->value, base,
							 isSigned);
}
						...
						int base, boolean isSigned)
{
   ...
   */
     ...
     */
		...
		printed = snprintf(buffer, buffer_size, "0%s%c%d_%s",
				   isSigned ? "s" : "u", base_char, value.width,
				   number_rep);
		break;
	case 10:
		sign = sign &&
		       BigNumber_test_bit(&(value.dat), value.width - 1);
		...
		if (sign && value.width > 1) {
			...
			plus_factor = WordNumber_evaluate_select(
				value, (value.width - 2), 0);
			copy = BigNumber_minus(&minus_factor,
					       &(plus_factor.dat));
		...
		printed = snprintf(buffer, buffer_size, "%s0%s%c%d_%s",
				   (sign ? "-" : ""), (isSigned ? "s" : "u"),
				   base_char, value.width, number_rep);
