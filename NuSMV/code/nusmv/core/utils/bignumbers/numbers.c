...
void QNumber_operator_self_plus_big(QNumber *self, const QNumber *r)
{
	...
	if (!QNumber_big(r)) {
		if (QNumber_is_int_normal(r) && QNumber_is_int_big(r)) {
			...
			mpz_divexact(self->data.gmp->den, self->data.gmp->den,
				     *bg);
			...
			mpz_divexact(self->data.gmp->num, self->data.gmp->num,
				     *bg);
		...
		if (QNumber_is_int_big(self) && QNumber_is_int_big(r)) {
			mpz_add(self->data.gmp->num, self->data.gmp->num,
				r->data.gmp->num);
			return;
		} else {
			...
			mpz_mul(self->data.gmp->den, self->data.gmp->den,
				*r_den);
...
void QNumber_operator_self_minus_big(QNumber *self, const QNumber *r)
{
	...
	if (!QNumber_big(r)) {
		...
		if (QNumber_is_int_big(self) && QNumber_is_int_big(r)) {
			mpz_sub(self->data.gmp->num, self->data.gmp->num,
				r->data.gmp->num);
			return;
		} else {
			const int mpz_pool_size = 4;
...
void QNumber_operator_self_minus(QNumber *self, const QNumber *r)
{
	...
	if (!QNumber_big(self) && !QNumber_big(r)) {
		/* should be pretty fast if both are integers ... */
		if (QNumber_is_int_normal(self) && QNumber_is_int_normal(r)) {
			if (QNumber_sub_overflow(&self->data.num,
						 self->data.num, r->data.num)) {
	...
	if (!QNumber_big(r)) {
		if (QNumber_is_int_normal(r) && QNumber_is_int_big(self)) {
         ...
         get a division by zero later on... */
				...
				gcd1 = mpz_gcd_ui(*g1, self->data.gmp->num,
						  labs(r_den));
				gcd2 = mpz_gcd_ui(*g2, self->data.gmp->den,
						  labs(r_num));
				mpz_divexact_ui(self->data.gmp->num,
						self->data.gmp->num, gcd1);
				mpz_mul_si(self->data.gmp->num,
					   self->data.gmp->num,
					   r_num / ((long)(gcd2)));
				mpz_divexact_ui(self->data.gmp->den,
						self->data.gmp->den, gcd2);
		...
		if (QNumber_is_int_big(self) && QNumber_is_int_big(r)) {
			mpz_mul(self->data.gmp->num, self->data.gmp->num,
				r->data.gmp->num);
			return;
		} else {
			const int mpz_pool_size = 4;
					      ...
					      const QNumber *r)
{
  ...
  that the denominator is always positive. */
    ...
    might overflow... */
		ret = ((long long)(n->data.num / gcd1) *
			       (long long)(r->den / gcd2) <
		       (long long)(n->den / gcd2) *
			       (long long)(r->data.num / gcd1));
	} else {
...
boolean QNumber_operator_less_than_or_equals(const QNumber *a, const QNumber *b)
{
	return (QNumber_operator_equals(a, b) ||
		QNumber_operator_less_than(a, b));
}
...
{
	return (QNumber_operator_equals(a, b) ||
		QNumber_operator_more_than(a, b));
}
...
QNumber QNumber_abs(const QNumber *n)
{
	if (!QNumber_big(n)) {
		...
		QNumber ret =
			QNumber_assign(n->data.gmp->num, n->data.gmp->den, 1);
...
QNumber QNumber_gcd(const QNumber *a, const QNumber *b)
{
	...
	if (!QNumber_big(a)) {
		if (!QNumber_big(b)) {
			return QNumber_from_long(
				QNumber_gcd_long(a->data.num, b->data.num));
		} else {
			if (!a->data.num) {
				QNumber ret = QNumber_from_other(b);
				return QNumber_sgn(b) < 0 ?
					       QNumber_operator_unaray_minus(
						       &ret) :
					       ret;
			} else {
				long n = mpz_gcd_ui(NULL, b->data.gmp->num,
						    labs(a->data.num));
		...
		if (!QNumber_big(b)) {
			if (!b->data.num) {
				...
				long n = mpz_gcd_ui(NULL, a->data.gmp->num,
						    labs(b->data.num));
				return QNumber_from_long(n);
			}
		} else {
...
boolean QNumber_divides(const QNumber *self, const QNumber *other)
{
	...
	if (!QNumber_big(self)) {
		if (!QNumber_big(other)) {
			...
			rvalue = mpz_mod_ui(*r, other->data.gmp->num,
					    self->data.num) == 0;
	...
	if (!QNumber_big(self)) {
		if (!QNumber_big(other)) {
			...
			*r = QNumber_from_long(res.rem);
			...
			mpz_fdiv_qr(q->data.gmp->num, r->data.gmp->num, *a,
				    other->data.gmp->num);
			clean_mpz_pool(&mpz_pool[0], mpz_pool_size);
		}
	} else {
		...
		if (!QNumber_big(other)) {
			...
			mpz_fdiv_qr(q->data.gmp->num, r->data.gmp->num,
				    self->data.gmp->num, *b);

			clean_mpz_pool(&mpz_pool[0], mpz_pool_size);
		} else {
			mpz_fdiv_qr(q->data.gmp->num, r->data.gmp->num,
				    self->data.gmp->num, other->data.gmp->num);
		}
	}
}
