...
#define ERROR_MGR_BEGIN_TAG(/* string_ptr */ tag)                              \
  if (error_mgr_is_tag_enabled(self,                                           \
                               UStringMgr_find_string(self->strings, tag))) {
...
static void error_mgr_init(ErrorMgr_ptr self, const NuSMVEnv_ptr env);

static void error_mgr_deinit(ErrorMgr_ptr self);

static void error_mgr_finalize(Object_ptr object, void *dummy);

static boolean error_mgr_is_tag_enabled(ErrorMgr_ptr self, string_ptr tags);
...
void ErrorMgr_start_parsing_err(ErrorMgr_ptr self) {
  ...
  if (get_input_file(self->options)) {
    StreamMgr_print_error(self->streams,
                          "file %s: ", get_input_file(self->options));
  } else {
...
void ErrorMgr_nusmv_exit(ErrorMgr_ptr self, int n) {
  ERROR_MGR_CHECK_INSTANCE(self);
...
void ErrorMgr_report_failure_node(ErrorMgr_ptr self, node_ptr n) {
  ...
  nusmv_yylineno = ErrorMgr_failure_get_lineno(
      self, n); /* set the line number of the error */
...
void ErrorMgr_error_lhs_of_index_is_not_array(ErrorMgr_ptr self) {
  ...
  StreamMgr_print_error(
      self->streams, "left hand side of index sub-scripting is not an array");
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_range_error(ErrorMgr_ptr self, node_ptr n, node_ptr var) {
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_nprint_error(self->streams, wffprint,
                         "cannot assign value %N to variable %N", n, var);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_empty_range(ErrorMgr_ptr self, node_ptr name, int dim1,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams, "empty range type %d..%d for ", dim1,
                        dim2);
...
void ErrorMgr_error_not_constant_width_of_word_array_type(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(
      self->streams, "at least one of the width of the Word Array variable ");
  print_node(wffprint, err, name);
  StreamMgr_print_error(self->streams, " is not a constant");
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_not_constant_width_of_array_type(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "at least one of the width of the Array variable ");
  print_node(wffprint, err, name);
  StreamMgr_print_error(self->streams, " is not a constant");
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_unknown_var_in_order_file(ErrorMgr_ptr self, node_ptr n) {
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_nprint_error(self->streams, wffprint,
                         "unknown variable in order file :%N", n);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_warning_variable_not_declared(ErrorMgr_ptr self, node_ptr vname) {
  ...
  StreamMgr_flush_streams(
      self->streams); /* to flush all existing messages before outputting */
  StreamMgr_print_error(self->streams,
                        "\n********   WARNING   ********\nThe variable: ");
  ...
  StreamMgr_print_error(self->streams,
                        "but it appear in the input ordering file.\n");
...
void ErrorMgr_warning_missing_variable(ErrorMgr_ptr self, node_ptr vname) {
  ...
  StreamMgr_print_error(self->streams,
                        "It has been positioned at the end of the ordering.\n");
  StreamMgr_print_error(self->streams, "******** END WARNING ********\n");
}
...
void ErrorMgr_warning_missing_variables(ErrorMgr_ptr self,
  ...
  if (NodeList_get_length(vars_list) > 1) {
    ...
    StreamMgr_print_error(self->streams,
                          "\nhas not been specified in the ordering file.\n");
    StreamMgr_print_error(
        self->streams, "It has been positioned at the end of the ordering.\n");
  }
...
void ErrorMgr_warning_non_ag_only_spec(ErrorMgr_ptr self, Prop_ptr prop) {
  ...
  StreamMgr_flush_streams(
      self->streams); /* to flush all existing messages before outputting */
  StreamMgr_print_error(self->streams, "\n********   WARNING   ********\nThe ");
  print_spec(StreamMgr_get_error_ostream(self->streams), prop,
...
void ErrorMgr_warning_fsm_init_and_fairness_empty(ErrorMgr_ptr self) {
  ...
  StreamMgr_print_error(
      self->streams,
      " of the finite state machine is empty.\n"
      "This might make results of model checking not trustable.\n");
  StreamMgr_print_error(self->streams, "******** END WARNING ********\n");
}
...
void ErrorMgr_error_var_appear_twice_in_order_file(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_nprint_error(self->streams, wffprint,
                         "variable appears twice in order file:%N", n);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_warning_var_appear_twice_in_order_file(ErrorMgr_ptr self,
  ...
  StreamMgr_flush_streams(
      self->streams); /* to flush all existing messages before outputting */
  StreamMgr_print_error(self->streams, "\n********   WARNING   ********\n");
  (void)StreamMgr_nprint_error(self->streams, wffprint,
                               "variable appears twice in order file:%N", n);
}
...
void ErrorMgr_warning_id_appears_twice_in_idlist_file(ErrorMgr_ptr self,
  ...
  StreamMgr_print_error(self->streams, "\n********   WARNING   ********\n");
  (void)StreamMgr_nprint_error(self->streams, wffprint,
                               "ID appears twice in id list file :%N\n", n);
}
...
void ErrorMgr_error_not_proper_number(ErrorMgr_ptr self, const char *op,
  ...
  StreamMgr_print_error(self->streams,
                        "not suitable constant for operator '%s' : ", op);
...
void ErrorMgr_error_not_proper_numbers(ErrorMgr_ptr self, const char *op,
  ...
  StreamMgr_print_error(self->streams,
                        "not suitable constants for operator '%s' : ", op);
...
void ErrorMgr_error_redefining_input_var(ErrorMgr_ptr self, node_ptr s) {
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "redefining or assigning input variable \"");
  print_node(wffprint, err, s);
  StreamMgr_print_error(self->streams, "\"");
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_assign_input_var(ErrorMgr_ptr self, node_ptr s) {
  ...
  if (node_get_type(s) == SMALLINIT) {
    StreamMgr_print_error(
        self->streams,
        "illegal assignment of initial value to input variable \"");
  } else if (node_get_type(s) == NEXT) {
    StreamMgr_print_error(
        self->streams, "illegal assignment of next value to input variable \"");
  } else {
    StreamMgr_print_error(self->streams,
                          "illegal assignment of value to input variable \"");
    tmp = s;
  }
...
void ErrorMgr_error_assign_frozen_var(ErrorMgr_ptr self, node_ptr s) {
  ...
  ErrorMgr_start_parsing_err(self);
  nusmv_assert(node_get_type(s) !=
               SMALLINIT); /* init assign is legal=>cannot be here*/
  StreamMgr_print_error(self->streams,
                        "illegal assignment to frozen variable \"");
  print_node(wffprint, err, NEXT == node_get_type(s) ? car(s) : s);
  StreamMgr_print_error(self->streams,
                        "\". Assignment of an initial value only is allowed.");
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_assign_expected_var(ErrorMgr_ptr self, node_ptr s) {
  ...
  StreamMgr_print_error(
      self->streams,
      "A variable is expected in left-hand-side of assignment:\n");
  print_node(wffprint, err, s);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_out_of_memory(ErrorMgr_ptr self, size_t size) {
  ...
  (void)StreamMgr_print_error(
      self->streams, "\n##################################################\n");
  (void)StreamMgr_print_error(
      self->streams, "### Out of memory allocating %" PRIuPTR " bytes\n", size);
  (void)StreamMgr_print_error(
      self->streams, "##################################################\n");
  ErrorMgr_nusmv_exit(self, 1);
}
...
void ErrorMgr_error_invalid_count_operator(ErrorMgr_ptr self, node_ptr expr) {
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "Expected boolean expressions in count operator: ");
  print_node(wffprint, err, expr);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_game_definition_contains_input_vars(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "The game difinition contains an input variable: ");
...
void ErrorMgr_error_assign_exp_contains_input_vars(ErrorMgr_ptr self,
  ...
  StreamMgr_print_error(self->streams,
                        " contains input variables, which is not allowed.");
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_next_exp_contains_input_vars(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "\"next\" expression contains input variables:\n");
  print_node(wffprint, err, exp);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_invar_exp_contains_input_vars(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "INVAR expression contains input variables:\n");
  print_node(wffprint, err, exp);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_init_exp_contains_input_vars(ErrorMgr_ptr self,
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "INIT expression contains input variables:\n");
  print_node(wffprint, err, exp);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_second_player_var(ErrorMgr_ptr self, node_ptr exp) {
  ...
  StreamMgr_print_error(
      self->streams,
      "Prohibited use of a variable of "
...
void ErrorMgr_error_second_player_next_var(ErrorMgr_ptr self, node_ptr exp) {
  ...
  StreamMgr_print_error(
      self->streams,
      "Prohibited use of a (next-state) variable of "
      "the second player in the expressions of the first player : ");
  print_node(wffprint, err, exp);
  ErrorMgr_finish_parsing_err(self);
}
...
void ErrorMgr_error_set_preprocessor(ErrorMgr_ptr self, const char *name,
  ...
  StreamMgr_print_error(self->streams, "Preprocessor file %s does not exist.\n",
                        name);
  if (false == is_warning)
    ErrorMgr_nusmv_exit(self, 1);
}
...
void ErrorMgr_error_psl_not_supported_feature(ErrorMgr_ptr self) {
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams,
                        "The specified PSL property requires features "
...
void ErrorMgr_error_psl_not_supported_feature_next_number(ErrorMgr_ptr self) {
  ...
  StreamMgr_print_error(self->streams,
                        "expr in next*[expr] must be a constant positive "
                        "integer number.\n");
...
void ErrorMgr_error_not_supported_feature(ErrorMgr_ptr self, const char *msg) {
  ...
  StreamMgr_print_error(
      self->streams, "The requested feature has not been implemented yet:\n ");
...
void ErrorMgr_warning_psl_not_supported_feature(ErrorMgr_ptr self,
  ...
  StreamMgr_print_error(
      ...
      index);
  StreamMgr_print_error(self->streams, "but the user will be not allowed to "
                                       "perform any model checking on it.\n");
  StreamMgr_print_error(
      self->streams,
...
void ErrorMgr_error_invalid_number(ErrorMgr_ptr self, const char *szNumber) {
  ...
  if (szNumber != (const char *)NULL) {
    ...
    StreamMgr_print_error(
        self->streams, "Error: you have given an empty string when integer was "
                       "expected.\n");
  }
}
...
void ErrorMgr_error_bmc_invalid_k_l(ErrorMgr_ptr self, const int k,
  ...
  StreamMgr_print_error(
      self->streams, "Error: length=%d and loopback=%s are not compatible.\n",
      k, buf);
}
...
void ErrorMgr_error_invalid_numeric_value(ErrorMgr_ptr self, int value,
  ...
  StreamMgr_print_error(self->streams, "Error: invalid numeric value: %d\n",
                        value);
  if (reason != (char *)NULL) {
...
void ErrorMgr_error_file_not_found(ErrorMgr_ptr self, const char *filename) {
  ...
  StreamMgr_print_error(self->streams, "Error: file '%s' not found.\n",
                        filename);
  ErrorMgr_nusmv_exit(self, 1);
}
...
void ErrorMgr_error_file_clobbering(ErrorMgr_ptr self, const char *filename) {
  ...
  StreamMgr_print_error(self->streams, "Error: will not overwrite file '%s'.\n",
                        filename);
  ErrorMgr_nusmv_exit(self, 1);
}
...
void ErrorMgr_warning_processes_deprecated(ErrorMgr_ptr self) {
  ...
  StreamMgr_print_error(self->streams, "WARNING *** In the future processes "
...

  {
    ...
    return find_node(
        ...
        find_node(nodemgr, COLON,
                  (node_ptr)UStringMgr_find_string(self->strings, (char *)msg),
                  (node_ptr)kind),
...
const char *ErrorMgr_failure_get_msg(ErrorMgr_ptr self, node_ptr failure) {
  ...
  if (failure != (node_ptr)NULL &&
      ...
      node_get_type(failure) == FAILURE && car(failure) != (node_ptr)NULL &&
      node_get_type(car(failure)) == COLON)
    return (const char *)UStringMgr_get_string_text(
        (string_ptr)car(car(failure)));
  return "Unknown";
}
...
FailureKind ErrorMgr_failure_get_kind(ErrorMgr_ptr self, node_ptr failure) {
  ...
  if (failure != (node_ptr)NULL && node_get_type(failure) == FAILURE &&
      car(failure) != Nil && node_get_type(car(failure)) == COLON)
...
int ErrorMgr_failure_get_lineno(ErrorMgr_ptr self, node_ptr failure) {
  ...
  nusmv_assert(failure != (node_ptr)NULL && node_get_type(failure) == FAILURE &&
               cdr(failure) != Nil);
...
/*!
  ...
  TagInfo initializer
*/
static void TagInfo_init(TagInfo *self) { self->enabled = true; }
...
/*!
  \brief
*/
static boolean error_mgr_is_tag_enabled(ErrorMgr_ptr self, string_ptr tag) {
  TagInfo *taginfo;
