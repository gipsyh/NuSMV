...
static void vars_handler_add_group(VarsHandler_ptr self, VarsGroup *parent,
				   VarsGroup *group);

static int vars_handler_remove_group(VarsHandler_ptr self, Olist_ptr list,
				     const GroupInfo_ptr ginfo);

static int vars_handler_update_levels(VarsHandler_ptr self, VarsGroup *root);

static Oiter vars_handler_promote_group(VarsHandler_ptr self, Olist_ptr list,
					const GroupInfo_ptr ginfo);

static int vars_handler_get_first_free_level(VarsHandler_ptr self, int from_lev,
					     int size);
...
boolean VarsHandler_can_group(const VarsHandler_ptr self, int level, int size,
			      int chunk)
{
	enum GroupSearchRes res;
					...
					int *lev_low)
{
	...
	if (can_share && from_lev >= 0) {
		group = vars_handler_search_group(self, from_lev, size, chunk,
						  &res);
	}
	...
	if (GS_PERF_MATCH_FOUND != res) { /* group has to be created here */
		...
		if ((VarsGroup *)NULL == group) {
			...
			int reord_status =
				dd_reordering_status(self->dd, &reord_type);
			int new_lev, new_idx;

			dd_autodyn_disable(self->dd);
			new_lev = vars_handler_get_first_free_level(
				self, from_lev, size);
			new_idx = dd_get_index_at_level(self->dd, new_lev);

			child = VarsGroup_create(new_lev, new_lev + size - 1,
						 new_idx, chunk);
			if (size >= BDD_ENC_MIN_BLOCK_SIZE) {
				...
				if (opt_verbose_level_gt(opts, 9)) {
					...
					Logger_log(
						logger,
						"VarsHandler: creating physical var block "
						"at index %d, size %d\n",
						new_idx, size);
				}
				CATCH(errmgr)
				{
					child->block = dd_new_var_block(
						self->dd, new_idx, size);
				}
				FAIL(errmgr)
				{
					if (1 == reord_status) {
						dd_autodyn_enable(self->dd,
								  reord_type);
					}
					ErrorMgr_internal_error(
						errmgr,
						"vars_handler: Failure during variable group construction\n");
				}
			}
			if (1 == reord_status) {
         ...
         sub-group of the found root */
			child = VarsGroup_create(
				from_lev, from_lev + size - 1,
...
boolean VarsHandler_release_group(VarsHandler_ptr self, GroupInfo_ptr gid)
{
	...
	if (opt_verbose_level_gt(opts, 9)) {
		...
		Logger_log(logger,
			   "VarsHandler: freeing group: id=%" PRIuPTR
			   " low=%d, len=%d\n",
			   gid->id, gid->lev_low,
			   gid->lev_high - gid->lev_low + 1);
	}
...
void VarsHandler_dissolve_group(VarsHandler_ptr self, GroupInfo_ptr gid)
{
	...
	if (0 <= gid->lev_low && 0 <= gid->lev_high) {
		VarsGroup *group;
		Oiter iter =
			vars_handler_promote_group(self, self->forest, gid);
...
void VarsHandler_update_levels(VarsHandler_ptr self)
{
	...
	OLIST_FOREACH(self->forest, iter)
	{
		...
		order_may_change |=
			(prev_delta != INT_MAX && prev_delta != delta);
		prev_delta = delta;
	}

	if (order_may_change)
		Olist_sort(self->forest, vars_group_sort, NULL);
}
				     ...
				     const GroupInfo_ptr ginfo)
{
	...
	while (!Oiter_is_end(iter)) {
		...
		if (ginfo->lev_high < group->lev_low)
		...
		if (ginfo->lev_low <= group->lev_low &&
			...
			if (res && Olist_is_empty(group->gis)) {
				...
				removed += vars_handler_remove_group(
					self, group->children, ginfo);
			}
		}
	...
	while (!Oiter_is_end(iter)) {
		...
		if (ginfo->lev_low <= group->lev_low &&
			...
			/* may be in children */
				...
				Oiter iter_child = vars_handler_promote_group(
					self, group->children, ginfo);
               ...
               new blocks out of its children */
							      ...
							      iter2)
						{
							...
							if (opt_verbose_level_gt(
								...
								Logger_log(
									logger,
									"VarsHandler: promoting physical var block "
									"at index %d, size %d\n",
									child->idx_low,
									child->lev_high -
										child->lev_low +
							...
							FAIL(errmgr)
							{
								ErrorMgr_internal_error(
									errmgr,
									"vars_handler: Failure during variable "
									"group construction\n");
							}
						}
					}
					    ...
					    enum GroupSearchRes *res)
{
	...
	OLIST_FOREACH(self->forest, iter)
	{
		...
		VarsGroup *group = vars_group_search_group_aux(
			root, lev_low, size, chunk, res);
		if ((VarsGroup *)NULL != group)
			return group;
		if (GS_NOT_FOUND != *res)
			break;
				   ...
				   VarsGroup *group)
{
	...
	OLIST_FOREACH(list, iter)
	{
		...
		if (group->lev_low < el->lev_low) { /* insert point */
			nusmv_assert(group->lev_high <
				     el->lev_low); /* no overlap */
...
static int vars_handler_update_levels(VarsHandler_ptr self, VarsGroup *root)
{
	...
	if ((Olist_ptr)NULL != root->children) { /* update children */
		...
		OLIST_FOREACH(root->children, iter)
		{
			VarsGroup *child = Oiter_element(iter);
			int child_delta =
				vars_handler_update_levels(self, child);

			children_order_may_change |=
				(prev_child_delta != INT_MAX &&
					      ...
					      enum GroupSearchRes *res)
{
	...
	/* perfect match */
	if (lev_low == in_group->lev_low && lev_high == in_group->lev_high &&
	...
	nusmv_assert(lev_low >= in_group->lev_low &&
	...
	if ((Olist_ptr)NULL != in_group->children) {
		...
		OLIST_FOREACH(in_group->children, iter)
		{
			...
			if (lev_low >= child->lev_low &&
			    lev_high <= child->lev_high) {
				/* the group has to be in child */
				return vars_group_search_group_aux(
					child, lev_low, size, chunk, res);
			}
...
static void vars_group_print(VarsGroup *group, FILE *_file, int indent)
{
	...
	fprintf(_file, "%s %d-%d idx:%d chunk:%d (%d users:",
		(NULL != group->block) ? "P" : "L", group->lev_low,
		group->lev_high, group->idx_low, group->chunk,
					     ...
					     int size)
{
	...
	if (from_lev >= 0) {
		...
		if (_from_lev + size - 1 < dd_get_size(self->dd))
			return _from_lev;
	}
	...
	{ /* no previously created level is available: create new
       variables from the frontier */
		...
		if (new_idx >= MAX_VAR_INDEX)
			ErrorMgr_error_too_many_vars(errmgr);

		dd_autodyn_disable(self->dd);
		for (i = 0; i < size; ++i) {
			add_ptr add =
				add_new_var_with_index(self->dd, i + new_idx);
			nusmv_assert(
				dd_get_level_at_index(self->dd, i + new_idx) ==
				i + new_idx);
