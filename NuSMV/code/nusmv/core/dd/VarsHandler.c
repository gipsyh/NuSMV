...
GroupInfo_ptr VarsHandler_reserve_group(VarsHandler_ptr self, int from_lev,
  ...
  if (GS_PERF_MATCH_FOUND != res) { /* group has to be created here */
    ...
    if ((VarsGroup *)NULL == group) {
      ...
      child =
          VarsGroup_create(from_lev, from_lev + size - 1,
                           dd_get_index_at_level(self->dd, from_lev), chunk);
    }
...
boolean VarsHandler_release_group(VarsHandler_ptr self, GroupInfo_ptr gid) {
  ...
  if (opt_verbose_level_gt(opts, 9)) {
    Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
    Logger_log(logger,
               "VarsHandler: freeing group: id=%" PRIuPTR " low=%d, len=%d\n",
    ...
    if (ginfo->lev_low <= group->lev_low &&
      ...
      if (res && Olist_is_empty(group->gis)) {
        ...
        if (opt_verbose_level_gt(opts, 9)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(
              logger, "VarsHandler: removing %s group at level %d, size %d\n",
  ...
  while (!Oiter_is_end(iter)) {
    ...
    if (ginfo->lev_low <= group->lev_low &&
      ...
      /* may be in children */
        ...
        Oiter iter_child =
            vars_handler_promote_group(self, group->children, ginfo);
...
static VarsGroup *vars_handler_search_group(const VarsHandler_ptr self,
  ...
  OLIST_FOREACH(self->forest, iter) {
    ...
    VarsGroup *group =
        vars_group_search_group_aux(root, lev_low, size, chunk, res);
    if ((VarsGroup *)NULL != group)
      return group;
    if (GS_NOT_FOUND != *res)
      break;
...
static int vars_handler_update_levels(VarsHandler_ptr self, VarsGroup *root) {
  const int delta =
      (dd_get_level_at_index(self->dd, root->idx_low) - root->lev_low);
  Oiter iter;
  ...
  if ((Olist_ptr)NULL != root->children) { /* update children */
    ...
    OLIST_FOREACH(root->children, iter) {
      ...
      children_order_may_change |=
          (prev_child_delta != INT_MAX && prev_child_delta != child_delta);
...
static void vars_group_print(VarsGroup *group, FILE *_file, int indent) {
  ...
  fprintf(_file, "%s %d-%d idx:%d chunk:%d (%d users:",
          (NULL != group->block) ? "P" : "L", group->lev_low, group->lev_high,
          group->idx_low, group->chunk, Olist_get_size(group->gis));
