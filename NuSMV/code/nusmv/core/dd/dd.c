...
int get_dd_nodes_allocated(DDMgr_ptr dd) { return (Cudd_ReadKeys(dd->dd)); }
...
int dd_get_index_at_level(DDMgr_ptr dd, int level) {
  ...
  if (result < 0) {
    ...
    ErrorMgr_rpterr(errmgr, "dd_get_index_at_level: level %d out of bound.",
                    level);
...
int dd_get_reorderings(DDMgr_ptr dd) { return Cudd_ReadReorderings(dd->dd); }
  ...
  ** the prefix removed. */
  ...
  OPTS_FOREACH_OPTION(opt, (char **)&paramName, (char **)&paramValue) {
    if (strncmp(paramName, "BDD.", 4) == 0) {
      st_insert(newValueTable, (char *)&paramName[4], (char *)paramValue);
    }
  }
  ...
  st_foreach_item(newValueTable, stgen, (char **)&paramName,
    ...
    if (strcmp(paramName, "Hard limit for cache size") == 0) {
    ...
    } else if (strcmp(paramName, "Maximum growth while sifting a variable") ==
               0) {
      double value;
      ...
      if (strcmp(paramValue, "yes") == 0) {
        ...
        InvalidType(file, "Realignment of BDDs to ZDDs enabled", "(yes,no)");
                      ...
                      "Dead nodes counted in triggering reordering") == 0) {
      if (strcmp(paramValue, "yes") == 0) {
        Cudd_TurnOnCountDead(mgr->dd);
      } else if (strcmp(paramValue, "no") == 0) {
        Cudd_TurnOffCountDead(mgr->dd);
      } else {
        InvalidType(file, "Dead nodes counted in triggering reordering",
                    "(yes,no)");
      }
...
add_ptr add_then(DDMgr_ptr dd, add_ptr f) { return ((add_ptr)Cudd_T(f)); }

int add_index(DDMgr_ptr dd, add_ptr f) {
  return (Cudd_NodeReadIndex((DdNode *)f));
}

add_ptr add_else(DDMgr_ptr dd, add_ptr f) { return ((add_ptr)Cudd_E(f)); }
...
void add_ref(add_ptr fn) { Cudd_Ref(fn); }

void add_deref(add_ptr fn) { Cudd_Deref(fn); }
...
int add_isleaf(add_ptr dd_node) { return (Cudd_IsConstant(dd_node)); }

int bdd_isleaf(bdd_ptr dd_node) { return (Cudd_IsConstant(dd_node)); }
...
add_ptr add_apply(DDMgr_ptr dd, NPFNNE op, add_ptr f, add_ptr g) {
  ...
  result = Cudd_addApply(
      dd->dd, (CUDD_VALUE_TYPE(*)(DdManager *, DD_AOP, DdNode *, DdNode *))op,
...
add_ptr add_exist_abstract(DDMgr_ptr dd, add_ptr a, bdd_ptr b) {
  ...
  result = Cudd_addAbstract(
      dd->dd, (CUDD_VALUE_TYPE(*)(CUDD_VALUE_TYPE))node_plus, (DdNode *)a,
      (DdNode *)cube, ENV_OBJECT(dd)->environment);
...
int add_size(DDMgr_ptr dd, add_ptr fn) { return (Cudd_DagSize((DdNode *)fn)); }
...
void bdd_ref(bdd_ptr dd_node) { Cudd_Ref(dd_node); }

void bdd_deref(bdd_ptr dd_node) { Cudd_Deref(dd_node); }
...
int bdd_intersected(DDMgr_ptr dd, bdd_ptr f, bdd_ptr g) {
  ...
  result = Cudd_bddLeq(dd->dd, (DdNode *)f, (DdNode *)Cudd_Not((DdNode *)g));
...
int bdd_size(DDMgr_ptr dd, bdd_ptr fn) { return (Cudd_DagSize((DdNode *)fn)); }
...
int bdd_pick_all_terms(
  ...
  if (bdd_is_true(dd, pick_from_set)) {
    ...
    bdd_free(dd, not_var0);
  } else if (Cudd_PickAllTerms(dd->dd, pick_from_set, vars, vars_dim, result) ==
             1) {
    StreamMgr_print_error(streams, "Error from Cudd_PickAllTerms.\n");
    return 1;
  }
  ...
  case DD_DYN_VAR_ORDER_ACTION_DISABLE: {
    ...
    if (currentMethod == REORDER_NONE) {
      StreamMgr_print_output(
          streams, "Dynamic variable ordering is already disabled.\n");
    } else {
...
int Dd_set_bdd_parameters(NuSMVEnv_ptr env, DDMgr_ptr dd_manager,
  ...
  if (0 == retval) {
    ...
    StreamMgr_print_error(streams, "Something went wrong creating the cudd"
