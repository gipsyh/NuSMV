...
void Dd_quit(NuSMVEnv_ptr env)
{
	...
	VarsHandler_ptr dd_vars =
		NuSMVEnv_remove_value(env, ENV_DD_VARS_HANDLER);
...
dd_block *dd_new_var_block(DDMgr_ptr dd, int start_index, int offset)
{
  ...
  */
	...
	if (opt_verbose_level_gt(opts, 5)) {
		...
		Logger_log(logger,
			   "dd_new_var_block: low=%d, idx=%d, size=%d\n",
...
int dd_free_var_block(DDMgr_ptr dd, dd_block *group)
{
	...
	if (opt_verbose_level_gt(opts, 5)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "dd_free_var_block: low=%d, idx=%d, size=%d\n",
			   group->low, group->index, group->size);
	}

	if (Mtr_DissolveGroup((MtrNode *)group) == (MtrNode *)NULL)
		res = 1;
	else
		res = 0;
...
int dd_get_index_at_level(DDMgr_ptr dd, int level)
{
	...
	if (result < 0) {
		const NuSMVEnv_ptr env =
			EnvObject_get_environment(ENV_OBJECT(dd));
		const ErrorMgr_ptr errmgr =
			ERROR_MGR(NuSMVEnv_get_value(env, ENV_ERROR_MANAGER));

		ErrorMgr_rpterr(errmgr,
				"dd_get_index_at_level: level %d out of bound.",
				level);
...
int dd_reorder(DDMgr_ptr dd, int method, int minsize)
{
	...
	if (result == 0) {
		...
		ErrorMgr_rpterr(errmgr,
				"dd_reorder: reordering of ADD/BDD fails.");
		ErrorMgr_nusmv_exit(errmgr, 1);
	}
	return (result);
} /* end of dd_reorder */
  ...
  ** the prefix removed. */
			...
			(char **)&paramValue)
	{
		...
		if (strcmp(paramName, "Hard limit for cache size") == 0) {
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
         ...
         performed. Note that function strtoul can be used. */
			if (*invalidChar) {
				...
				Cudd_SetMaxCacheHard(mgr->dd, uvalue);
				  ...
				  "Cache hit threshold for resizing") == 0) {
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file,
		...
		} else if (strcmp(paramName, "Garbage collection enabled") ==
			   0) {
			if (strcmp(paramValue, "yes") == 0) {
				...
				InvalidType(file, "Garbage collection enabled",
					    "(yes,no)");
			}
		} else if (strcmp(paramName,
				  "Limit for fast unique table growth") == 0) {
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(
			...
			if (*invalidChar) {
				...
				Cudd_SetSiftMaxSwap(mgr->dd, uvalue);
				  ...
				  "Maximum growth while sifting a variable") ==
			   0) {
			double value;
			...
			if (*invalidChar) {
				...
				Cudd_SetMaxGrowth(mgr->dd, value);
				  ...
				  "Dynamic reordering of BDDs enabled") == 0) {
			if (strcmp(paramValue, "yes") == 0) {
				...
				InvalidType(
					file,
					"Dynamic reordering of BDDs enabled",
					"(yes,no)");
			}
		} else if (strcmp(paramName, "Default BDD reordering method") ==
			   0) {
			...
			if (*invalidChar || reorderMethod < 0) {
				InvalidType(file,
					    "Default BDD reordering method",
					    "integer");
			} else {
				if (Cudd_ReorderingStatus(mgr->dd,
							  &reorderInt)) {
					Cudd_AutodynEnable(mgr->dd,
							   reorderMethod);
				}
			}
		} else if (strcmp(paramName,
				  "Dynamic reordering of ZDDs enabled") == 0) {
			if (strcmp(paramValue, "yes") == 0) {
				Cudd_AutodynEnableZdd(mgr->dd,
						      zddReorderMethod);
			} else if (strcmp(paramValue, "no") == 0) {
				...
				InvalidType(
					file,
					"Dynamic reordering of ZDDs enabled",
					"(yes,no)");
			}
		} else if (strcmp(paramName, "Default ZDD reordering method") ==
			   0) {
			dd_reorderingtype reorderInt;

			zddReorderMethod = (unsigned int)strtol(
				paramValue, &invalidChar, 10);
			if (*invalidChar) {
				InvalidType(file,
					    "Default ZDD reordering method",
					    "integer");
			} else {
				if (Cudd_ReorderingStatusZdd(mgr->dd,
							     &reorderInt)) {
					Cudd_AutodynEnableZdd(mgr->dd,
							      zddReorderMethod);
				}
				  ...
				  "Realignment of ZDDs to BDDs enabled") == 0) {
			if (strcmp(paramValue, "yes") == 0) {
				...
				InvalidType(
					file,
					"Realignment of ZDDs to BDDs enabled",
					"(yes,no)");
			}
		} else if (strcmp(paramName,
				  "Realignment of BDDs to ZDDs enabled") == 0) {
			if (strcmp(paramValue, "yes") == 0) {
				...
				InvalidType(
					file,
					"Realignment of BDDs to ZDDs enabled",
					"(yes,no)");
			}
		} else if (strcmp(paramName,
				  "Dead nodes counted in triggering reordering") ==
			   0) {
			if (strcmp(paramValue, "yes") == 0) {
			...
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file, "Group checking criterion",
					    "integer");
			} else {
			...
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file, "Recombination threshold",
					    "integer");
			} else {
				Cudd_SetRecomb(mgr->dd, uvalue);
			}
		} else if (strcmp(paramName, "Symmetry violation threshold") ==
			   0) {
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file,
					    "Symmetry violation threshold",
					    "integer");
			} else {
			...
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file, "Arc violation threshold",
					    "integer");
			} else {
			...
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file, "GA population size",
					    "integer");
			} else {
				Cudd_SetPopulationSize(mgr->dd, uvalue);
			}
		} else if (strcmp(paramName, "Number of crossovers for GA") ==
			   0) {
			uvalue = (unsigned int)strtol(paramValue, &invalidChar,
						      10);
			if (*invalidChar) {
				InvalidType(file, "Number of crossovers for GA",
					    "integer");
			} else {
				Cudd_SetNumberXovers(mgr->dd, uvalue);
			}
		} else {
			fprintf(file, "Warning: Parameter %s not recognized.",
				paramName);
...
int dd_printminterm(DDMgr_ptr manager, dd_ptr node)
{
	return (Cudd_PrintMinterm(manager->dd, node));
}

int dd_dump_dot(DDMgr_ptr dd /* manager */,
...
{
	return (Cudd_DumpDot(dd->dd, n, (DdNode **)f, (char **)inames,
			     (char **)onames, fp));
}

int dd_dump_davinci(DDMgr_ptr dd /* manager */,
		    int n /* number of output nodes to be dumped */,
		    dd_ptr *f /* array of output nodes to be dumped */,
		    const char **inames /* array of input names (or NULL) */,
		    const char **onames /* array of output names (or NULL) */,
		    FILE *fp /* pointer to the dump file */)
{
	return (Cudd_DumpDaVinci(dd->dd, n, (DdNode **)f, (char **)inames,
				 (char **)onames, fp));
}
...
add_ptr add_build(DDMgr_ptr dd, int level, add_ptr t, add_ptr e)
{
	...
	while (result == NULL) {
		result = (DdNode *)cuddUniqueInter(dd->dd, level, (DdNode *)t,
						   (DdNode *)e);
...
add_ptr add_new_var_with_index(DDMgr_ptr dd, int index)
{
	...
	if ((DdHalfWord)index >= (MAX_VAR_INDEX - 1)) {
		ErrorMgr_rpterr(
			errmgr,
			"Unable to allocate a new BDD variable, max. number exceeded (%d)",
			index);
		ErrorMgr_nusmv_exit(errmgr, 1);
	}
...
add_ptr add_implies(DDMgr_ptr dd, add_ptr a, add_ptr b)
{
	...
	common_error2(errmgr, dd->dd, result, tmp,
		      "add_implies: result = NULL");
	Cudd_Ref(result);
...
add_ptr add_apply(DDMgr_ptr dd, NPFNNE op, add_ptr f, add_ptr g)
{
	...
	result = Cudd_addApply(
		dd->dd,
		(CUDD_VALUE_TYPE(*)(DdManager *, DD_AOP, DdNode *, DdNode *))op,
		(DdNode *)f, (DdNode *)g, ENV_OBJECT(dd)->environment);
...
add_ptr add_exist_abstract(DDMgr_ptr dd, add_ptr a, bdd_ptr b)
{
	...
	result = Cudd_addAbstract(
		dd->dd, (CUDD_VALUE_TYPE(*)(CUDD_VALUE_TYPE))node_plus,
		(DdNode *)a, (DdNode *)cube, ENV_OBJECT(dd)->environment);
...
add_ptr add_ifthenelse(DDMgr_ptr dd, add_ptr If, add_ptr Then, add_ptr Else)
{
	...
	result = Cudd_addIte(dd->dd, (DdNode *)If, (DdNode *)Then,
			     (DdNode *)Else);
...
add_ptr add_support(DDMgr_ptr dd, add_ptr fn)
{
	...
	common_error2(errmgr, dd->dd, result, tmp_1,
		      "add_support: result = NULL");
...
{
	Cudd_addWalkLeaves(dd->dd, (void (*)(void *, node_ptr))op, (DdNode *)f,
			   (void *)dd);
	return;
}
...
bdd_ptr bdd_and_abstract(DDMgr_ptr dd, bdd_ptr T, bdd_ptr S, bdd_ptr V)
{
	...
	result = Cudd_bddAndAbstract(dd->dd, (DdNode *)T, (DdNode *)S,
				     (DdNode *)V);
...
bdd_ptr bdd_pick_one_minterm(DDMgr_ptr dd, bdd_ptr fn, bdd_ptr *vars, int n)
{
	...
	if (bdd_is_false(dd, fn)) {
		...
		result = Cudd_bddPickOneMintermNR(dd->dd, (DdNode *)fn,
						  (DdNode **)vars, n);
		common_error(errmgr, result,
			     "bdd_pick_one_minterm: result = NULL");
		Cudd_Ref(result);
		return ((bdd_ptr)result);
	}
}

bdd_ptr bdd_pick_one_minterm_rand(DDMgr_ptr dd, bdd_ptr fn, bdd_ptr *vars,
				  int n)
{
	const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(dd));
	const ErrorMgr_ptr errmgr =
	...
	if (bdd_is_false(dd, fn)) {
		...
		result = Cudd_bddPickOneMinterm(dd->dd, (DdNode *)fn,
						(DdNode **)vars, n);
		common_error(errmgr, result,
			     "bdd_pick_one_minterm_rand: result = NULL");
		Cudd_Ref(result);
		return ((bdd_ptr)result);
	}
}
	...
	int result_dim /* The size of the above array */)
{
	...
	if (bdd_is_true(dd, pick_from_set)) {
		...
		if (Cudd_PickAllTerms(dd->dd, vars[0], vars, vars_dim,
				      result) == 1) {
			StreamMgr_print_error(
				streams, "Error from Cudd_PickAllTerms.\n");
		...
		bdd_free(dd, not_var0);
	} else if (Cudd_PickAllTerms(dd->dd, pick_from_set, vars, vars_dim,
				     result) == 1) {
		StreamMgr_print_error(streams,
				      "Error from Cudd_PickAllTerms.\n");
		return 1;
	}
	return 0;
}
...
int bdd_DumpBlif(DDMgr_ptr dd /* manager */,
		 int n /* number of output nodes to be dumped */,
...
int bdd_DumpBlifBody(DDMgr_ptr dd /* manager */,
		     int n /* number of output nodes to be dumped */,
		     bdd_ptr *f /* array of output nodes to be dumped */,
		     char **inames /* array of input names (or NULL) */,
		     char **onames /* array of output names (or NULL) */,
...
int bdd_ref_count(DDMgr_ptr dd, bdd_ptr n)
{
	...
	common_error(errmgr, N,
		     "Could not make node regular, while getting ref-count.");
	result = N->ref;
	return (result);
}
...
int calculate_bdd_value(DDMgr_ptr mgr, bdd_ptr f, int *values)
{
	...
	while (!Cudd_IsConstant(f)) {
		if (values[(Cudd_Regular(f))->index])
			f = Cudd_T(f);
		else
			f = Cudd_E(f);
...
int Dd_dynamic_var_ordering(NuSMVEnv_ptr env, DDMgr_ptr dd_manager,
			    int dynOrderingMethod, DdDynVarOrderAction action)
{
	...
	case DD_DYN_VAR_ORDER_ACTION_DISABLE: {
		...
		if (currentMethod == REORDER_NONE) {
			StreamMgr_print_output(
				streams,
				"Dynamic variable ordering is already disabled.\n");
		} else {
			StreamMgr_print_output(
				streams,
   ...
   */
	case DD_DYN_VAR_ORDER_ACTION_FORCE: {
		StreamMgr_print_output(streams,
				       "Dynamic variable ordering forced ");
		StreamMgr_print_output(
			streams, "with method %s....\n",
			DynOrderTypeConvertToString(dynOrderingMethod));
