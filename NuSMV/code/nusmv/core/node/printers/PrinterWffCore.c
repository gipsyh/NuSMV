...
int printer_wff_core_print_node(PrinterBase_ptr self, node_ptr n, int priority)
{
	...
	case NUMBER_UNSIGNED_WORD: {
		...
		return _PRINT(WordNumber_to_based_string(
			WORD_NUMBER(car(n)), get_output_word_format(opts),
			false));
	}
	...
	case UWCONST:
		return _PRINT("uwconst(") && _THROW(car(n), 0) &&
		       _PRINT(", ") && _THROW(cdr(n), 0) && _PRINT(")");

	case SWCONST:
		return _PRINT("swconst(") && _THROW(car(n), 0) &&
		       _PRINT(", ") && _THROW(cdr(n), 0) && _PRINT(")");
    ...
    */
	...
	case CONST_ARRAY:
		return _PRINT("CONSTARRAY(") && _THROW(car(n), 0) &&
		       _PRINT(", ") && _THROW(cdr(n), 0) && _PRINT(")");
	...
	case WORDARRAY_TYPE:
		return _PRINT("array word[") && _THROW(car(n), 0) &&
		       _PRINT("] of ") && _THROW(cdr(n), 0);
	...
	default:
		ErrorMgr_internal_error(
			errmgr,
			"printer_wff_core_print_node: not supported type = %d",
			node_get_type(n));
	}
...
static int printer_wff_core_print_case_body(PrinterWffCore_ptr self, node_ptr n)
{
	...
	nusmv_assert(cdr(n) !=
		     Nil); /* Now there is always a last(default) case */

	/* continue to print the tail of the case list */
	if (node_get_type(cdr(n)) == CASE ||
	    node_get_type(cdr(n)) == IFTHENELSE) {
