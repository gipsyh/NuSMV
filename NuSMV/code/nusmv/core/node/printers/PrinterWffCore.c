...
static int printer_wff_core_print_case_body(PrinterWffCore_ptr self,
                                            node_ptr n);
...
PrinterWffCore_ptr PrinterWffCore_create(const NuSMVEnv_ptr env,
  ...
  printer_wff_core_init(self, env, name, NUSMV_CORE_SYMBOL_FIRST,
...
int printer_wff_core_print_node(PrinterBase_ptr self, node_ptr n,
                                int priority) {
  ...
  case FAILURE: {
    ...
    return _PRINT("FAILURE(\"") &&
           _PRINT(ErrorMgr_failure_get_msg(errmgr, n)) && _PRINT(buf);
  }

  case ATTIME:
    return _PRINT("@") && _THROW(cdr(n), 0) && _PRINT("{") &&
  ...
  case SELF:
    return _PRINT("self");
  case BOOLEAN:
    return _PRINT("boolean");
  case SCALAR:
    return _PRINT("{") && _THROW(car(n), 0) && _PRINT("}");
  case REAL:
    return _PRINT("real");
  case INTEGER:
    return _PRINT("integer");
  case CONTINUOUS:
    return _PRINT("continuous");
  ...
  case NUMBER_UNSIGNED_WORD: {
    ...
    return _PRINT(WordNumber_to_based_string(
  ...
  case DOT:
    ...
    return _THROW(car(n), 0) && _PRINT(".") && _THROW(cdr(n), 0);
  ...
  case CONTEXT:
    return _THROW(cdr(n), 0) &&
           ((car(n) == Nil) || (_PRINT(" IN ") && _THROW(car(n), 0)));

  case CONS:
    return _THROW(car(n), 0) &&
           ((cdr(n) == Nil) || (_PRINT(", ") && _THROW(cdr(n), 0)));
  ...
  case IFTHENELSE:
    return _PRINT(" (") && _THROW(car(car(n)), 0) &&
           (_PRINT(" ? ") && _THROW(cdr(car(n)), 0) && _PRINT(" : ") &&
            _THROW(cdr(n), 0)) &&
           _PRINT(") ");

  case ARRAY: /* array access */
    return _THROW(car(n), 0) && _PRINT("[") && _THROW(cdr(n), 0) && _PRINT("]");
  ...
  case BIT_SELECTION:
    ...
    return _THROW(car(n), 18) && _PRINT("[") && _THROW(car(cdr(n)), 0) &&
           _PRINT(" : ") && _THROW(cdr(cdr(n)), 0) && _PRINT("]");

  case CONSTANTS:
    return _PRINT("CONSTANTS ") && _THROW(car(n), 0) && _PRINT(";");
  ...
  case SIGNED_WORD: {
    ...
    if (node_get_type(car(n)) == NUMBER) {
      return ((UNSIGNED_WORD == node_get_type(n)) ? _PRINT("word[")
                                                  : _PRINT("unsigned word[")) &&
             _THROW(car(n), 0) && _PRINT("]");
    }
    ...
    if (pr_tmp <= priority && !_PRINT("("))
      return 0;
    if (!_PRINT("word1("))
      return 0;
    if (!_THROW(car(iter), pr_tmp))
      return 0;
    if (!_PRINT(")"))
      return 0;
  ...
  case WAWRITE:
    return _PRINT("WRITE(") && _THROW(car(n), 0) && _PRINT(", ") &&
           _THROW(car(cdr(n)), 0) && _PRINT(", ") && _THROW(cdr(cdr(n)), 0) &&
           _PRINT(")");

  case WAREAD:
    return _PRINT("READ(") && _THROW(car(n), 0) && _PRINT(", ") &&
           _THROW(cdr(n), 0) && _PRINT(")");

  case CONST_ARRAY:
    return _PRINT("CONSTARRAY(") && _THROW(car(n), 0) && _PRINT(", ") &&
           _THROW(cdr(n), 0) && _PRINT(")");

  case TYPEOF:
    return _PRINT("typeof(") && _THROW(car(n), 0) && _PRINT(")");

  case INTARRAY_TYPE:
    return _PRINT("array integer of ") && _THROW(car(n), 0);
  ...
  case WORDARRAY_TYPE:
    return _PRINT("array word[") && _THROW(car(n), 0) && _PRINT("] of ") &&
           _THROW(cdr(n), 0);
  ...
  case OP_GLOBAL:
    if (Nil != cdr(n)) {
      if (!_PRINT(" G ["))
        return 0;
      if (!_THROW(car(cdr(n)), 0))
        return 0;
      if (!_PRINT(","))
        return 0;
      if (!_THROW(cdr(cdr(n)), 0))
        return 0;
      if (!_PRINT("] "))
        return 0;
      op = "";
  ...
  default:
    ErrorMgr_internal_error(
        errmgr, "printer_wff_core_print_node: not supported type = %d",
        node_get_type(n));
  }

  if (brckts == 1 && pr_tmp < priority && !_PRINT(" [ "))
    return 0;
  if (brckts == 0 && pr_tmp <= priority && !_PRINT("("))
    return 0;
  ...
  case 2:
    ...
    if (!_PRINT(op))
      return 0; /* EF */
    if (!_THROW(car(cdr(n)), pr_tmp))
      return 0; /* a */
    if (!_PRINT(".."))
      return 0;
    if (!_THROW(cdr(cdr(n)), pr_tmp))
      return 0; /* b */
    if (!_PRINT(" "))
      return 0;
    if (!_THROW(car(n), pr_tmp))
...
*/
static int printer_wff_core_print_case(PrinterWffCore_ptr self, node_ptr n) {
  return _PRINT("case\n") && printer_wff_core_print_case_body(self, n) &&
         _PRINT("esac");
}
