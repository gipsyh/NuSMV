...
{
	boolean success;
	MasterPrinter_set_stream_type(mp, STREAM_TYPE_STRING,
	...
	do {
		...
		default: {
			...
			ErrorMgr_internal_error(
				errmgr,
				"MasterPrinter::print_string: Invalid stream type\n");
		}
		}
		...
		if (pad) {
			...
			for (i = 0; i <= padding; i++) {
				...
				case STREAM_TYPE_FUNCTION: {
					void *arg = self->stream_arg.function
							    .argument;
					if (!self->stream_arg.function.func_ptr(
						    tmp, arg)) {
						res = 0; /* failure */
						goto leave;
					}
...

	{
		...
		case STREAM_TYPE_STDOUT:
			self->stream_arg.file =
				StreamMgr_get_output_stream(self->streams);
			break;

		case STREAM_TYPE_STDERR:
			self->stream_arg.file =
				StreamMgr_get_error_stream(self->streams);
			break;
		...
		case STREAM_TYPE_FILE:
			self->stream_arg.file =
				((STREAM_TYPE_ARG_UNUSED != arg) ? arg->file :
								   NULL);
			break;
      ...
      */
		...
		default:
			ErrorMgr_internal_error(
				errmgr,
				"MasterPrinter::set_stream_type: Invalid stream type\n");
		}
...
int master_printer_print_node(MasterPrinter_ptr self, node_ptr n, int priority)
{
	...
	while (!ListIter_is_end(iter)) {
		PrinterBase_ptr pr = PRINTER_BASE(NodeList_get_elem_at(
	...
	if (0 != len) {
		...
		if (self->sstream_len >= self->sstream_cap) {
			...
			self->sstream_cap = self->sstream_len + 1 +
					    (self->sstream_grow_sum /
					     self->sstream_grow_num);

			self->sstream = (char *)REALLOC(char, self->sstream,
							self->sstream_cap);
...
int master_printer_deindent(MasterPrinter_ptr self)
{
	...
	if (NULL == head) {
		...
		ErrorMgr_internal_error(
			errmgr, "printout failure: empty indentation stack!");
	}
...
static int master_printer_get_level(MasterPrinter_ptr self)
{
	...
	return (NULL != head) ?
		       NODE_TO_INT(
			       NodeList_get_elem_at(self->indent_stack, head)) :
		       0;
}
