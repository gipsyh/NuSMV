...
PrinterPsl_ptr PrinterPsl_create(const NuSMVEnv_ptr env, const char *name) {
  ...
  printer_psl_init(self, env, name, NUSMV_PSL_SYMBOL_FIRST,
...
int printer_psl_print_node(PrinterBase_ptr self, node_ptr n, int priority) {
  ...
  case PSL_SERE: {
    ...
    if (!psl_node_is_sere(psl_node_get_left(psl)))
      retval = _PRINT("{");
    else
      retval = 1;
  ...
  case PSL_SERECONCAT:
    return _PRINT("{") && _THROW(psl_node_get_left(psl)) && _PRINT("; ") &&
  ...
  case PSL_SERECOMPOUND: {
    ...
    default:
      ErrorMgr_internal_error(errmgr,
                              "printer_psl_print_node: Unsupported sere "
                              "compound operator");
    }
  ...
  case PSL_REPLPROP:
    return _PRINT("( ") && _THROW(psl_node_get_left(psl)) &&   /* replicator */
           _PRINT(" : ") && _THROW(psl_node_get_right(psl)) && /* property */
           _PRINT(" )");
  ...
  case PSL_FORANY: {
    ...
    if (psl_node_get_op(psl) == PSL_FORALL)
      res = _PRINT("forall ");
    else
      res = _PRINT("forany ");
    ...
    res = res && _PRINT(" in ");
    if (psl_node_is_boolean_type(rvs))
      res = res && _THROW(rvs);
    else {
  ...
  case PSL_PIPEMINUSGT: {
    int res =
        _PRINT("(") && _THROW(psl_node_get_left(psl_node_get_left(psl))) &&
  ...
  case PSL_WITHIN_: {
    ...
    case PSL_WITHIN_:
      ...
      break;
    default:
      error_unreachable_code(); /* no other cases here */
    }

    return res && _PRINT(" (") &&
           _THROW(psl_node_get_left(psl_node_get_left(psl))) && _PRINT(", ") &&
           _THROW(psl_node_get_right(psl_node_get_left(psl))) && _PRINT(") ") &&
  ...
  case PSL_WHILENOT_: {
    ...
    case PSL_WHILENOT:
      ...
      break;
    case PSL_WHILENOTBANG_:
      res = _PRINT("whilenot!_");
      break;
    case PSL_WHILENOT_:
      res = _PRINT("whilenot_");
      break;
    default:
      error_unreachable_code(); /* no other cases here */
    }

    return res && _PRINT(" (") && _THROW(psl_node_get_left(psl)) &&
  ...
  case PSL_NEXT_EVENT_E: {
    ...
    switch (psl_node_get_op(psl)) {
    case PSL_NEXT_EVENT_ABANG:
      res = _PRINT("next_event_a!");
      break;
    case PSL_NEXT_EVENT_A:
      res = _PRINT("next_event_a");
      break;
    case PSL_NEXT_EVENT_EBANG:
      res = _PRINT("next_event_e!");
      break;
    case PSL_NEXT_EVENT_E:
      res = _PRINT("next_event_e");
      break;
    ...
    return res && _PRINT(" (") &&
           _THROW(psl_node_get_right(psl_node_get_right(psl))) && _PRINT(")") &&
           _PRINT(" [") && _THROW(psl_node_get_left(psl_node_get_right(psl))) &&
           _PRINT("]") && _PRINT(" (") && _THROW(psl_node_get_left(psl)) &&
           _PRINT(")");
  }
  ...
  case PSL_NEXT_EVENT: {
    ...
    return res && _PRINT(" (") && _THROW(psl_node_get_left(psl)) && _PRINT(")");
  }
  ...
  case PSL_AMPERSANDAMPERSAND: {
    ...
    switch (psl_node_get_op(psl)) {
    case PSL_BEFOREBANG:
      res = res && _PRINT("before!");
      break;
    case PSL_BEFORE:
      res = res && _PRINT("before");
      break;
    case PSL_BEFOREBANG_:
      res = res && _PRINT("before!_");
      break;
    case PSL_BEFORE_:
      res = res && _PRINT("before_");
      break;
    case PSL_UNTILBANG:
      res = res && _PRINT("until!");
      break;
    case PSL_UNTIL:
      res = res && _PRINT("until");
      break;
    case PSL_UNTILBANG_:
      res = res && _PRINT("until!_");
      break;
    case PSL_UNTIL_:
      res = res && _PRINT("until_");
      break;
    case PSL_ABORT:
      res = res && _PRINT("abort");
      break;
    case PSL_W:
      res = res && _PRINT("W");
      break;
  ...
  case PSL_ITE:
    return _THROW(psl_node_get_ite_cond(psl)) && _PRINT(" ? ") &&
           _THROW(psl_node_get_ite_then(psl)) && _PRINT(" : ") &&
           _THROW(psl_node_get_ite_else(psl));

  case PSL_WSELECT:
    return _PRINT("select(") && _THROW(psl_node_get_left(psl)) &&
           _PRINT(", ") && _THROW(psl_node_get_left(psl_node_get_right(psl))) &&
           _PRINT(", ") &&
           _THROW(psl_node_get_right(psl_node_get_right(psl))) && _PRINT(")");
  default:
    ErrorMgr_internal_error(errmgr,
                            "printer_psl_print_node: not supported type");
  }
