...
#define _THROW(n)                      \
	printer_base_throw_print_node( \
		PRINTER_BASE(self), PslNode_convert_to_node_ptr(n), priority)
...
int printer_psl_print_node(PrinterBase_ptr self, node_ptr n, int priority)
{
	...
	case PSL_SERECONCAT:
		return _PRINT("{") && _THROW(psl_node_get_left(psl)) &&
	...
	case PSL_NEXT_EVENT: {
		...
		if (psl_node_get_left(psl_node_get_right(psl)) != PSL_NULL) {
			res = res && _PRINT(" [") &&
			      _THROW(psl_node_get_left(
				      psl_node_get_right(psl))) &&
			      _PRINT("]");
		}
	...
	case PSL_AMPERSANDAMPERSAND: {
		...
		res = _PRINT("(") && _THROW(psl_node_get_left(psl)) &&
		      _PRINT(" ");
		...
		return _PRINT(" ") && _THROW(psl_node_get_right(psl)) &&
		       _PRINT(")");
	}
	...
	case PSL_XBANG: {
		...
		if (psl_node_get_right(psl) !=
		    PSL_NULL) { /* simple next expression */
			res = res && _PRINT(" [") &&
			      _THROW(psl_node_get_left(
				      psl_node_get_right(psl))) &&
			      _PRINT("] ");
	...
	default:
		ErrorMgr_internal_error(
			errmgr, "printer_psl_print_node: not supported type");
	}
