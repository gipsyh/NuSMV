...
node_ptr NodeMgr_find_atom(NodeMgr_ptr self, node_ptr a) {
  if (a == Nil)
    return (a);
  return NodeMgr_find_node(self, a->type, a->left.nodetype, a->right.nodetype);
}
...
void NodeMgr_self_check(NodeMgr_ptr self, boolean check_repeated) {
  ...
  for (i = 0; i < csize; ++i) {
    ...
    for (el = self->nodelist[i], prev = (node_ptr)NULL; (node_ptr)NULL != el;
         prev = el, el = el->link) {
      int j;
      ...
      if (check_repeated) {
        ...
        for (j = i + 1; j < csize; ++j) {
          ...
          for (el2 = self->nodelist[j]; (node_ptr)NULL != el2;
               el2 = el2->link) {
  ...
  { /* first allocation of nodelist */
    ...
    if ((node_ptr *)NULL == self->nodelist) {
      ErrorMgr_internal_error(
          errmgr, "node_init: Out of Memory in allocating the node hash\n");
    }
...
static int profile_info_cmp(const void *a, const void *b) {
  long res =
      (*((node_profile_info_ptr)b)).load - (*((node_profile_info_ptr)a)).load;
