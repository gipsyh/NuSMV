...
static const unsigned int node_primes[] = {
	...
	201326611, 402653189, 805306457, 1610612741, 3221225081u
};
...
void NodeMgr_show_profile_stats(NodeMgr_ptr self, FILE *out)
{
	...
	fprintf(out, "current load: %ld (%.3f%%)\n", current_load,
	...
	qsort(profile_info, csize, sizeof(node_profile_info), profile_info_cmp);

	fprintf(out, "\n10 most loaded buckets:\n");
	fprintf(out, "----------------------------------------------\n");
	for (i = 0; i < 10; i++) {
		fprintf(out, "%2d. index = %5d (@%8lx), load = %ld\n", i + 1,
...
void NodeMgr_free_node(NodeMgr_ptr self, node_ptr node)
{
...

	{
		...
		while ((node_ptr)NULL != looking) {
         ...
         node cannot be in the list */
			if (0 > node_cmp_fun(node, looking))
				break; /* found limit */
#endif
  ...
  */
	...
	{
		node_ptr iter;
		for (iter = self->nextFree->link; iter != Nil;
		     iter = iter->link) {
			if (iter == node) {
				NuSMVEnv_ptr env;

				env = EnvObject_get_environment(
					ENV_OBJECT(self));
				...
				ErrorMgr_internal_error(
					errmgr,
					"The same node_ptr is freed twice.");
			}
		}
	}
...
node_ptr NodeMgr_find_node(NodeMgr_ptr self, int type, node_ptr left,
			   node_ptr right)
{
	extern int nusmv_yylineno;
	node_rec node;
...
node_ptr NodeMgr_find_atom(NodeMgr_ptr self, node_ptr a)
{
	if (a == Nil)
		return (a);
	return NodeMgr_find_node(self, a->type, a->left.nodetype,
				 a->right.nodetype);
}
...
void NodeMgr_self_check(NodeMgr_ptr self, boolean check_repeated)
{
	...
	for (i = 0; i < csize; ++i) {
		...
		for (el = self->nodelist[i], prev = (node_ptr)NULL;
		     (node_ptr)NULL != el; prev = el, el = el->link) {
			int j;

			/* ordering is guaranteed in each list */
			if (prev != (node_ptr)NULL)
				nusmv_assert(node_cmp_fun(prev, el) < 0);

			if (check_repeated) {
				/* there are no repeated elements in the hash */
				for (j = i + 1; j < csize; ++j) {
					register node_ptr el2;
					for (el2 = self->nodelist[j];
					     (node_ptr)NULL != el2;
					     el2 = el2->link) {
						nusmv_assert(
							node_cmp_fun(el, el2) !=
...
static void node_manager_init(NodeMgr_ptr self, const NuSMVEnv_ptr env)
{
	...
	{ /* first allocation of nodelist */
		...
		if ((node_ptr *)NULL == self->nodelist) {
			ErrorMgr_internal_error(
				errmgr,
				"node_init: Out of Memory in allocating the node hash\n");
		}
...
#elif INSERT_NODE_SORTED
{
	...
	while ((node_ptr)NULL != looking) {
		...
		if (0 == cmp)
			return looking;
		if (0 > cmp)
			break; /* found greater element, insert before it */
		prev = looking;
		looking = looking->link;
	}
...
static inline unsigned int node_hash_fun(node_ptr node)
{
	...
	return (unsigned int)((
		...
		(((size_t)node->right.nodetype)
		 << (NUSMV_SIZEOF_VOID * 4 - 2))));

#else /* big-endian 1 (TO BE TESTED) */
	return (unsigned int)((((size_t)node->type)) +
			      ((((size_t)node->left.nodetype) << 17) ^
			       (((size_t)node->left.nodetype) >> 16)) +
			      ((((size_t)node->right.nodetype) << 17) ^
			       (((size_t)node->right.nodetype) >> 16)));
...
static int node_cmp_fun(node_ptr node1, node_ptr node2)
{
	if (node1->type == node2->type) {
		if (node1->left.nodetype == node2->left.nodetype) {
			if (node1->right.nodetype == node2->right.nodetype)
				return 0;
			return (node1->right.nodetype > node2->right.nodetype) ?
				       1 :
				       -1;
		}
	...
	if (self->nextFree == (node_ptr)NULL) { /* memory is full */
		...
		if (mem == (node_ptr *)NULL) { /* out of memory */
			...
			StreamMgr_print_error(streams,
					      "node_alloc: out of memory\n");
			ErrorMgr_internal_error(
				errmgr, "Memory in use for nodes = %ld\n",
				self->memused);
		} else { /* Adjust manager data structure */
			self->memused +=
				(NODE_MEM_CHUNK + 1) * sizeof(node_rec);
...
static void _node_realloc_nodelist(NodeMgr_ptr self)
{
	...
	if ((node_ptr *)NULL != nodelist) {
...

		{
			...
			unsigned int i;
			for (i = 0; i < csize; ++i) {
				for (el = cnodelist[i]; (node_ptr)NULL != el;
				     el = cnodelist[i]) {
					cnodelist[i] =
						el->link; /* removes the element */
             ...
             alternatively */
               ...
               point in the new list instead of appending */
						...
						while ((node_ptr)NULL != el2) {
							...
							nusmv_assert(
								0 !=
								cmp); /* el cannot be in the list already */
							if (0 > cmp)
								break; /* found limit */
							prev = el2;
							el2 = el2->link;
...
#error "Insert node type unrecognized"
		...
		nodelist =
			(node_ptr *)REALLOC(node_ptr, self->nodelist, newsize);
...
static int profile_info_cmp(const void *a, const void *b)
{
	long res = (*((node_profile_info_ptr)b)).load -
