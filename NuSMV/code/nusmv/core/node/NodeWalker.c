	...
	if (len == 1) {
		...
		for (idx = 0; idx < len; ++idx) {
			if (idx != transf_handle) {
				NodeTransformation *ntp = array_fetch_p(
					NodeTransformation,
					self->node_transformations, idx);
				array_insert_last(NodeTransformation, _new,
						  *ntp);
			}
		}
		      ...
		      boolean can_handle_null)
{
	...
	if (opt_verbose_level_gt(opts, 8)) {
		...
		Logger_log(
			...
			name, low, low + num - 1);
		if (can_handle_null)
			Logger_log(logger, " (and NULL nodes)\n");
		else
			Logger_log(logger, "\n");
	}
}
					      ...
					      node_ptr node)
{
	if (NULL != self->node_transformations) {
		...
		int nt_idx;
		arrayForEachItemP(NodeTransformation,
				  self->node_transformations, nt_idx, ntp)
		{
