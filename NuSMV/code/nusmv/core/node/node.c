...
node_ptr car(node_ptr x) { return (x->left.nodetype); }

node_ptr cdr(node_ptr x) { return (x->right.nodetype); }
...
node_ptr Node_find_error_type(NodeMgr_ptr nodemgr) { return Nil; }
...
node_ptr node_subtract(NodeMgr_ptr nodemgr, node_ptr set1, node_ptr set2) {
  if (set2 == Nil) {
    return Nil;
  } else if (Nil == set1) {
