...
int NodeAnonymizerBase_print_map(NodeAnonymizerBase_ptr self, FILE *stream)
{
	...
	anon_print = MasterPrinter_create(env);
	{
		...
		sta.file = stream;
		MasterPrinter_set_stream_type(anon_print, STREAM_TYPE_FILE,
					      &sta);
	}
	...
	while (NULL != line) {
		...
		/* convert them to nodes and add to the bimap */
		{
			...
			if (NULL == norig)
				goto error;
			nanonymous =
				nab_convert_id_str_to_node(self, &sanonymous);
			if (NULL == nanonymous)
				goto error;
			       ...
			       size_t memoization_threshold)
{
	...
	OVERRIDE(NodeAnonymizerBase, translate) =
		node_anonymizer_base_translate;
					...
					node_ptr id, const char *prefix)
{
	...
	if (NULL == retval) {
		...
		do {
			anonymous_name =
				self->build_anonymous(self, id, prefix);
...
boolean node_anonymizer_base_is_leaf(NodeAnonymizerBase_ptr self, node_ptr node)
{
	boolean retval;
					    ...
					    node_ptr anonymous_expr)
{
	...
	replaced = LRUCache_insert(self->orig2anon, (void *)expr,
				   (void *)anonymous_expr);
					   ...
					   node_ptr expr)
{
	...
	replaced = LRUCache_insert(self->anon2orig, (void *)anonymous_expr,
				   (void *)expr);
					...
					BiMap_ptr map)
{
	...
	{
		node_ptr domain =
			NODE_PTR(BiMap_iter_get_domain_element(map, &iter));
		node_ptr codomain =
			NODE_PTR(BiMap_iter_get_codomain_element(map, &iter));

		nusmv_assert(self->is_id(self, domain));
		nusmv_assert(self->is_id(self, codomain));

		if (BiMap_domain_contains(self->map, domain)) {
			node_ptr current_codomain =
				BiMap_get(self->map, domain);
			...
			node_anonymizer_base_insert_mapping(self, domain,
							    codomain);
		}
	}
	...
	if (self->is_id(self, expr)) {
		...
		if (NULL == retval) {
			...
			retval = find_node(
				...
				Nil != car(expr) ?
					nab_map_expr_rec(self, car(expr)) :
					Nil,
				Nil != cdr(expr) ?
					nab_map_expr_rec(self, cdr(expr)) :
					Nil);

			/* update cache */
			node_anonymizer_base_insert_expr_cache(self, expr,
							       retval);
		}
	}
	...
	if (self->is_id(self, expr)) {
		...
		if (NULL == retval) {
			...
			retval = find_node(
				nodemgr, node_get_type(expr),
				Nil != car(expr) ?
					nab_map_back_rec(self, car(expr)) :
					Nil,
				Nil != cdr(expr) ?
					nab_map_back_rec(self, cdr(expr)) :
					Nil);

			/* update cache */
			node_anonymizer_base_insert_anon2orig(self, expr,
							      retval);
		}
	}
					   ...
					   char **id)
{
	...
	default: {
		/* It is an identifier */
		span = strcspn(*id, NODE_ANONYMIZER_DOT_STR
			       "" NODE_ANONYMIZER_SEPARATOR_STR);
