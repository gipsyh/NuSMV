...
node_ptr node_anonymizer_st_translate(NodeAnonymizerBase_ptr self, node_ptr id,
     ...
     first lookup, then resolve if not found, and then lookup again */
  {
    ...
    resolved_name =
        ResolveSymbol_resolve(resolve_symbol, _self->symb_table, id, Nil);
...
const char *node_anonymizer_st_build_anonymous(NodeAnonymizerBase_ptr self,
  ...
  if (0 != strcmp("", prefix)) {
    ...
    anonymous_name =
        (char *)node_anonymizer_base_build_anonymous(self, id, prefix);
  } else {
    ...
    if (SymbTable_is_symbol_var(_self->symb_table, id)) {
      ...
      int chars =
          snprintf(anonymous_name, NAST_MAX_ID_LEN, "c_%d", _self->counters[4]);
      ...
      chars =
          snprintf(anonymous_name, NAST_MAX_ID_LEN, "a%d", _self->counters[6]);
      SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);
      ...
      dtype = TypeChecker_get_expression_type(
          type_checker, SymbTable_get_define_body(_self->symb_table, id),
          SymbTable_get_define_context(_self->symb_table, id));
      ...
      if (SymbType_is_word(dtype)) {
        ...
        int chars = snprintf(anonymous_name, NAST_MAX_ID_LEN, "d%s%d_%d", t,
                             size, _self->counters[5]);
        SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);
      } else {
        int chars = snprintf(anonymous_name, NAST_MAX_ID_LEN, "d%s_%d", t,
                             _self->counters[5]);
        SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);
      }
      ...
      chars =
          snprintf(anonymous_name, NAST_MAX_ID_LEN, "m%d", _self->counters[7]);
      SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);
      ...
      int chars =
          snprintf(anonymous_name, NAST_MAX_ID_LEN, "f_%d", _self->counters[9]);
      SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);

      _self->counters[9]++;
    } else
      error_unreachable_code_msg("Unsupported symbol category");
  }
