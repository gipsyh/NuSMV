					     ...
					     SymbTable_ptr symb_table)
{
	...
	node_anonymizer_st_init(self, env, default_prefix,
				memoization_threshold, symb_table);
	return self;
}
				      ...
				      const char *prefix)
{
     ...
     first lookup, then resolve if not found, and then lookup again */
	{
		...
		resolve_symbol =
			SymbTable_resolve_symbol(_self->symb_table, id, Nil);
		resolved_name = ResolveSymbol_resolve(
			resolve_symbol, _self->symb_table, id, Nil);
		...
		if (ResolveSymbol_is_undefined(resolve_symbol)) {
         ...
         model, no need to anonymize it */
			...
			retval = node_anonymizer_base_search_mapping(
				self, resolved_name);
		}
	}
	...
	if (NULL == retval) {
		...
		do {
			anonymous_name = self->build_anonymous(
				self, resolved_name, prefix);
		...
		node_anonymizer_base_insert_mapping(self, resolved_name,
						    retval);
	}
					       ...
					       node_ptr id, const char *prefix)
{
	...
	if (0 != strcmp("", prefix)) {
		...
		anonymous_name = (char *)node_anonymizer_base_build_anonymous(
			self, id, prefix);
	} else {
		...
		if (SymbTable_is_symbol_var(_self->symb_table, id)) {
			...
			if (SymbTable_is_symbol_input_var(_self->symb_table,
							  id)) {
				anonymous_name[0] = 'i';
         ...
         defines */
			{
				SymbType_ptr type;
				type = SymbTable_get_var_type(_self->symb_table,
							      id);
				...
				if (SymbType_is_word(type)) {
					...
					chars = snprintf(&anonymous_name[1],
							 NAST_MAX_ID_LEN - 1,
							 "w%d_%d", size,
							 _self->counters[0]);
					SNPRINTF_CHECK(chars,
						       NAST_MAX_ID_LEN - 1);
             ...
             function */
					...
					ErrorMgr_rpterr(
						errmgr,
						"Unknown type for obfuscation.");
					error_unreachable_code();
				}
			}
		} else if (SymbTable_is_symbol_constant(_self->symb_table,
							id)) {
			int chars = snprintf(anonymous_name, NAST_MAX_ID_LEN,
					     "c_%d", _self->counters[4]);
			SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);

			_self->counters[4] = _self->counters[4] + 1;
		} else if (SymbTable_is_symbol_variable_array(_self->symb_table,
							      id)) {
			int chars;
			...
			dtype = TypeChecker_get_expression_type(
				type_checker,
			...
			if (SymbType_is_word(dtype)) {
				...
				int chars = snprintf(anonymous_name,
						     NAST_MAX_ID_LEN,
						     "d%s%d_%d", t, size,
						     _self->counters[5]);
				SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);
				...
				int chars = snprintf(anonymous_name,
						     NAST_MAX_ID_LEN, "d%s_%d",
						     t, _self->counters[5]);
				SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);
			}

			_self->counters[5] = _self->counters[5] + 1;
		} else if (SymbTable_is_symbol_array_define(_self->symb_table,
							    id)) {
			int chars;
		...
		} else if (SymbTable_is_symbol_function(_self->symb_table,
							id)) {
			int chars = snprintf(anonymous_name, NAST_MAX_ID_LEN,
					     "f_%d", _self->counters[9]);
			SNPRINTF_CHECK(chars, NAST_MAX_ID_LEN);

			_self->counters[9]++;
		} else
			error_unreachable_code_msg(
				"Unsupported symbol category");
	}
