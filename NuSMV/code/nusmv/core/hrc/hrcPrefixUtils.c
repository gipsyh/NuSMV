...
Set_t hrc_prefix_utils_get_prefix_symbols(Set_t symbol_set, node_ptr prefix)
{
	...
	SET_FOREACH(symbol_set, iter)
	{
		...
		if (NODE_PTR(Nil) != symbol) {
			if (hrc_prefix_utils_is_subprefix(prefix,
							  car(symbol))) {
				      ...
				      node_ptr expression)
{
	...
	if (DOT != node_get_type(expression) &&
		...
		return_expression = find_node(nodemgr, DOT, context,
					 ...
					 node_ptr identifier, node_ptr context)
{
	...
	if (Nil == identifier) {
		...
		new_context = hrc_prefix_utils_remove_context(
			nodemgr, car(identifier), context);
					     ...
					     node_ptr instance_name)
{
	...
	new_name_chr = ALLOC(char, strlen(module_name_chr) +
					   strlen(instance_name_chr) + 2);

	if (NODE_PTR(Nil) == instance_name) {
		/* main module */
		sprintf(new_name_chr, "%s", module_name_chr);
	} else {
		sprintf(new_name_chr, "%s_%s", module_name_chr,
			instance_name_chr);
	}
					     ...
					     node_ptr expr, node_ptr context)
{
	...
	if (CONTEXT == node_get_type(expr)) {
		...
		context = hrc_prefix_utils_concat_context(env, context,
							  car(expr));
		_expr = cdr(expr);
	} else {
		_expr = expr;
