...
void HrcFlattener_flatten_hierarchy(HrcFlattener_ptr self) {
...

  {
    ...
    FlatHierarchy_set_assign(self->hierarchy,
                             cons(nodemgr, cons(nodemgr, Nil, Nil), Nil));
  }
  ...
  Compile_ProcessHierarchy(self->environment, self->symb_table, self->layer,
                           self->hierarchy, Nil, true, true);
}
...
static void hrc_flattener_populate_model(HrcFlattener_ptr self,
  ...
  /* INIT */
  {
    ...
    FlatHierarchy_set_init(
        self->hierarchy,
        EXPR_AND(new_init, FlatHierarchy_get_init(self->hierarchy)));
  }
  /* INVAR */
  {
    ...
    FlatHierarchy_set_invar(
        self->hierarchy,
        EXPR_AND(new_invar, FlatHierarchy_get_invar(self->hierarchy)));
  }
  /* TRANS */
  {
    ...
    FlatHierarchy_set_trans(
        self->hierarchy,
        EXPR_AND(new_trans, FlatHierarchy_get_trans(self->hierarchy)));
  }
  ...
  /* ASSIGN */
  {
    ...
    instance_assign = hrc_flattener_proc_assign_list(
        self, node, HrcNode_get_init_assign_exprs_iter(node), instance_assign,
        SMALLINIT);
    instance_assign = hrc_flattener_proc_assign_list(
        self, node, HrcNode_get_next_assign_exprs_iter(node), instance_assign,
        NEXT);
    instance_assign = hrc_flattener_proc_assign_list(
        self, node, HrcNode_get_invar_assign_exprs_iter(node), instance_assign,
        INVAR);
    ...
    if (Nil != instance_assign) {
      instance_assign =
          hrc_prefix_utils_contextualize_expr(env, instance_assign, context);
  ...
  /* proces properties */
  {
    ...
    for (idx = 0; idx < sizeof(to_process) / sizeof(to_process[0]); ++idx) {
      ...
      node_ptr prop_ctx = hrc_flattener_build_properties(
          self, prop_iter, context, to_process[idx].prop_type);
      ...
      to_process[idx].fh_setter(self->hierarchy,
                                append_ns(nodemgr, prop_ctx, old_prop));
    }
...
static void hrc_flattener_flatten_recur(HrcFlattener_ptr self, HrcNode_ptr node,
  ...
  hrc_symb_table_utils_populate_symb_table(self->symb_table, self->layer, node,
                                           context);
  ...
  /* Instances: do recursion */
  {
    ...
    SLIST_FOREACH(HrcNode_get_child_hrc_nodes(node), iter) {
      ...
      node_ptr new_context =
          hrc_prefix_utils_concat_context(env, context, name);
...
static node_ptr hrc_flattener_proc_assign_list(HrcFlattener_ptr self,
  ...
  for (; !Oiter_is_end(assign_iter); assign_iter = Oiter_next(assign_iter)) {
    ...
    case NEXT:
      assign =
          find_node(nodemgr, EQDEF, find_node(nodemgr, NEXT, car(assign), Nil),
                    cdr(assign));
      break;
    case INVAR:
