...
/* Macro declarations                                                        */
/*---------------------------------------------------------------------------*/
#define _PRINT(txt) StreamMgr_print_error(streams, txt)

#define _PRINTLN(txt) StreamMgr_print_error(streams, "%s\n", txt)

#define _NPRINT(node) StreamMgr_nprint_error(streams, wffprint, "%N", node)
...
/*!
  ...
  \todo Missing description
*/
#define EXPR_AND(l, r) find_node(nodemgr, AND, l, r)
...
static void hrc_flattener_flatten_recur(HrcFlattener_ptr self, HrcNode_ptr node,
					node_ptr context);
...
SexpFsm_ptr HrcToSexpFsm(NuSMVEnv_ptr env, HrcNode_ptr node,
			 SymbTable_ptr symb_table, SymbLayer_ptr layer)
{
	...
	sexp = SexpFsm_create(hierarchy, FlatHierarchy_get_vars(hierarchy));
...
void HrcFlattener_flatten_hierarchy(HrcFlattener_ptr self)
{
	...
	{
		const NodeMgr_ptr nodemgr = NODE_MGR(
			NuSMVEnv_get_value(self->environment, ENV_NODE_MGR));
		/* Start from an empty assignment */
		FlatHierarchy_set_assign(self->hierarchy,
					 cons(nodemgr, cons(nodemgr, Nil, Nil),
					      Nil));
	}
	...
	/* Add all variables to the hierarchy */
	{
		...
		{
			node_ptr var =
				SymbLayer_iter_get_symbol(self->layer, &iter);
	...
	Compile_ProcessHierarchy(self->environment, self->symb_table,
				 self->layer, self->hierarchy, Nil, true, true);
}
...
void HrcFlattener_write_flatten_model(HrcFlattener_ptr self, FILE *out)
{
	...
	Compile_WriteFlattenModel(self->environment, out, self->symb_table,
				  names, "MODULE main", self->hierarchy, true);
}
					 ...
					 HrcNode_ptr node, node_ptr context)
{
	...
	/* INIT */
	{
		node_ptr new_init = hrc_flattener_expression_and(
			nodemgr, HrcNode_get_init_exprs_iter(node));
		new_init = hrc_prefix_utils_contextualize_expr(env, new_init,
							       context);
	...
	/* INVAR */
	{
		node_ptr new_invar = hrc_flattener_expression_and(
			nodemgr, HrcNode_get_invar_exprs_iter(node));
		new_invar = hrc_prefix_utils_contextualize_expr(env, new_invar,
								context);
	...
	/* TRANS */
	{
		node_ptr new_trans = hrc_flattener_expression_and(
			nodemgr, HrcNode_get_trans_exprs_iter(node));
		new_trans = hrc_prefix_utils_contextualize_expr(env, new_trans,
								context);
	...
	/* COMPASSION */
	{
		...
		node_ptr new_compassion =
			FlatHierarchy_get_compassion(self->hierarchy);

		for (; !Oiter_is_end(comp_iter);
		     comp_iter = Oiter_next(comp_iter)) {
			node_ptr comp = NODE_PTR(Oiter_element(comp_iter));

			comp = hrc_prefix_utils_contextualize_expr(env, comp,
								   context);
	...
	/* JUSTICE */
	{
		...
		node_ptr new_justice =
			FlatHierarchy_get_justice(self->hierarchy);
		for (; !Oiter_is_end(justices);
		     justices = Oiter_next(justices)) {
			node_ptr just = NODE_PTR(Oiter_element(justices));

			just = hrc_prefix_utils_contextualize_expr(env, just,
								   context);
	...
	/* ASSIGN */
	{
		node_ptr assign_list =
			FlatHierarchy_get_assign(self->hierarchy);
		node_ptr instance_assign = Nil;

		/* -- Step 1 -> Create an unique big and of all assignments -- */
		instance_assign = hrc_flattener_proc_assign_list(
			self, node, HrcNode_get_init_assign_exprs_iter(node),
			instance_assign, SMALLINIT);
		instance_assign = hrc_flattener_proc_assign_list(
			self, node, HrcNode_get_next_assign_exprs_iter(node),
			instance_assign, NEXT);
		instance_assign = hrc_flattener_proc_assign_list(
			self, node, HrcNode_get_invar_assign_exprs_iter(node),
			instance_assign, INVAR);

		/* -- Step 2 -> Contextualize all assignments and add it to the
       hierarchy assignment field -- */
		if (Nil != instance_assign) {
			instance_assign = hrc_prefix_utils_contextualize_expr(
				env, instance_assign, context);

			instance_assign = EXPR_AND(instance_assign,
						   cdr(car(assign_list)));
	...
	/* proces properties */
	{
       ...
       properties */
		struct {
			...
			/* LTL */
			{ HrcNode_get_ltl_properties_iter,
			  FlatHierarchy_get_ltlspec, FlatHierarchy_set_ltlspec,
			  LTLSPEC },
			/* CTL */
			{ HrcNode_get_ctl_properties_iter,
			  FlatHierarchy_get_spec, FlatHierarchy_set_spec,
			  SPEC },
			/* PSL */
			{ HrcNode_get_psl_properties_iter,
			  FlatHierarchy_get_pslspec, FlatHierarchy_set_pslspec,
			  PSLSPEC },
			/* COMPUTE */
			{ HrcNode_get_compute_properties_iter,
			  FlatHierarchy_get_compute, FlatHierarchy_set_compute,
			  COMPUTE },
		};
     ...
     */
		for (idx = 0; idx < sizeof(to_process) / sizeof(to_process[0]);
		     ++idx) {
			Oiter prop_iter = to_process[idx].hrc_getter(node);

			/* build a list of flattened properties */
			node_ptr prop_ctx = hrc_flattener_build_properties(
				self, prop_iter, context,
				to_process[idx].prop_type);

			/* set the properties in the hierarchy appending the list of
         flattend properties */
			node_ptr old_prop =
				to_process[idx].fh_getter(self->hierarchy);
			to_process[idx].fh_setter(self->hierarchy,
						  append_ns(nodemgr, prop_ctx,
							    old_prop));
		}
					...
					node_ptr context)
{
	...
	/* Instances: do recursion */
	{
		...
		SLIST_FOREACH(HrcNode_get_child_hrc_nodes(node), iter)
		{
			...
			node_ptr new_context = hrc_prefix_utils_concat_context(
				env, context, name);
					       ...
					       short int type)
{
	...
	for (; !Oiter_is_end(prop_iter); prop_iter = Oiter_next(prop_iter)) {
		...
		if (Nil != prop_name) {
			...
			rs = SymbTable_resolve_symbol(self->symb_table,
						      prop_name, ctx);
			prop_name = ResolveSymbol_get_resolved_name(rs);

			if (!FlatHierarchy_add_property_name(self->hierarchy,
							     prop_name)) {
				ErrorMgr_internal_error(
					errmgr,
					"Property named %s already declared",
			       ...
			       SymbLayer_ptr layer)
{
	...
	if (SYMB_LAYER(NULL) == layer) {
		...
		SymbTable_set_default_layers_class_name(self->symb_table,
							MODEL_LAYERS_CLASS);
	}
	...
	for (; !Oiter_is_end(assign_iter);
		...
		case SMALLINIT:
			assign = find_node(nodemgr, EQDEF,
					   find_node(nodemgr, SMALLINIT,
						     car(assign), Nil),
					   cdr(assign));
			break;
		case NEXT:
			assign = find_node(nodemgr, EQDEF,
					   find_node(nodemgr, NEXT, car(assign),
						     Nil),
					   cdr(assign));
			break;
		case INVAR:
			assign = find_node(nodemgr, EQDEF, car(assign),
					   cdr(assign));
			break;
		default:
