...

	{
		...
		case HDS_HRC_TOP:
			...
			if (info->stage & HRC_STAGE_BEGIN) {
				c = snprintf(buf, buf_size, "<%s xmlns=\"%s\">",
					     tag, SMV_XSD_NS);
		...
		case HDS_MOD_FORMAL_PARAM:
			...
			if (info->stage & HRC_STAGE_BEGIN) {
				...
				_HRC_DUMP_XML_NODE_BEGIN_END("name",
							     info->n1.name);
				nusmv_assert(
					Nil ==
					info->n2.type); /* type not supported yet! */
				_HRC_DUMP_XML_NODE_BEGIN_END("type",
							     info->n2.type);
			}
			if (info->stage & HRC_STAGE_END) {
				_HRC_DUMP_XML_TAG_END(tag);
		...
		case HDS_LIST_SYMBOLS:
			...
			default:
				ErrorMgr_internal_error(
					errmgr,
					"Unexpected type of list of symbols");
			}
		...
		case HDS_SYMBOL:
			if (info->stage & HRC_STAGE_BEGIN) {
				...
				case SYMBOL_FROZEN_VAR:
					...
					_HRC_DUMP_XML_NODE_BEGIN_END(
						"name", info->n1.name);
					_HRC_DUMP_XML_TAG_BEGIN("type");
					hrc_dumper_dump_var_type(self,
								 info->n2.type);
			...
			if (info->stage & HRC_STAGE_END) {
				...
				nusmv_assert(
					info->stage &
					HRC_STAGE_BEGIN); /* begin also processed */
			}
		...
		case HDS_ASSIGN_INIT:
			...
			if (info->stage & HRC_STAGE_BEGIN) {
				...
				_HRC_DUMP_XML_NODE_BEGIN_END("name",
							     info->n1.name);
				_HRC_DUMP_XML_NODE_BEGIN_END("value",
							     info->n2.expr);
			}
			if (info->stage & HRC_STAGE_END) {
				_HRC_DUMP_XML_TAG_END(tag);
		...
		case HDS_CONSTRAINT_INIT:
			...
			if (info->stage & HRC_STAGE_BEGIN) {
				...
				SNPRINTF_CHECK(c, buf_size);
				_HRC_DUMP_STR(buf);
				_HRC_DUMP_XML_NODE_BEGIN_END("expr",
							     info->n1.expr);
			}
			if (info->stage & HRC_STAGE_END) {
				_HRC_DUMP_XML_TAG_END(tag);
		...
		case HDS_COMPASSION:
			...
			if (info->stage & HRC_STAGE_BEGIN) {
				...
				SNPRINTF_CHECK(c, buf_size);
				_HRC_DUMP_STR(buf);
				_HRC_DUMP_XML_NODE_BEGIN_END("expr1",
							     info->n1.expr);
				_HRC_DUMP_XML_NODE_BEGIN_END("expr2",
							     info->n2.expr);
			}
			if (info->stage & HRC_STAGE_END) {
				_HRC_DUMP_XML_TAG_END(tag);
		...
		case HDS_ERROR:
			...
			if (info->stage & HRC_STAGE_BEGIN) {
				...
				_HRC_DUMP_XML_TAG_BEGIN_END(
					"filename", info->error.filename);
				_HRC_DUMP_XML_TAG_BEGIN_END("lineno", buf);
				_HRC_DUMP_XML_TAG_BEGIN_END("token",
							    info->error.token);
				_HRC_DUMP_XML_TAG_BEGIN_END(
		...
		default:
			...
			hrc_dumper_dump_snippet(HRC_DUMPER(self), snippet,
						info);
		}
	}
}
