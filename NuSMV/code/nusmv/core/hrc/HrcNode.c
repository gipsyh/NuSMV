...
void HrcNode_add_constants(HrcNode_ptr self, node_ptr constant)
{
	...
	while (Nil != constant) {
		nusmv_assert(Nil == constant ||
			     CONS == node_get_type(constant));
#if HRC_NODE_AVOID_DUPLICATION
...
void HrcNode_set_child_hrc_nodes(HrcNode_ptr self, Slist_ptr list)
{
...

	{
		...
		for (iter = Slist_first(list); (false == Siter_is_end(iter));
		     iter = Siter_next(iter)) {
			nusmv_assert(HrcNode_get_parent(HRC_NODE(
					     Siter_element(iter))) == self);
		}
	}
					      ...
					      const node_ptr mod_type)
{
	...
	if (HrcNode_get_name(self) == mod_type)
		...
		for (iter = Slist_first(self->childs);
		     false == Siter_is_end(iter); iter = Siter_next(iter)) {
			r = HrcNode_find_hrc_node_by_mod_type(
	...
	if (HrcNode_get_name(self) == mod_type) {
		...
		SLIST_FOREACH(self->childs, iter)
		{
			Olist_ptr child_list =
				HrcNode_find_hrc_nodes_by_mod_type(
					(HrcNode_ptr)Siter_element(iter),
					mod_type);
...
HrcNode_ptr HrcNode_copy(const HrcNode_ptr self)
{
	...
	hrc_copy->array_defines =
		hrc_node_copy_list(nodemgr, self->array_defines);
	hrc_copy->init_assign = hrc_node_copy_list(nodemgr, self->init_assign);
	hrc_copy->invar_assign =
		hrc_node_copy_list(nodemgr, self->invar_assign);
...
node_ptr HrcNode_find_var(HrcNode_ptr self, node_ptr var_name, int type)
{
	...
	default:
		ErrorMgr_internal_error(
			errmgr, "HrcNode: %d is not a valid variable type!",
			type);
	}

	/* Search normalizing node name! */
	normalized_var_name =
		MasterNormalizer_normalize_node(normalizer, var_name);
...
{
	HRC_NODE_CHECK_INSTANCE(self);
	return HrcNode_get_state_vars_num(self) +
...
static Olist_ptr hrc_node_copy_list(NodeMgr_ptr nodemgr, Olist_ptr src_list)
{
	...
	OLIST_FOREACH(src_list, iter)
	{
		...
		new_element = new_node(nodemgr, node_get_type(element),
...
*/
static assoc_retval hrc_node_free_cons_map_fun(char *key, char *data, char *arg)
{
