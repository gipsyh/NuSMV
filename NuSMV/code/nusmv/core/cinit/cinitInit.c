...
#if SETVBUF_REVERSED
		setvbuf(StreamMgr_get_output_stream(streams), _IOLBF,
			(char *)NULL, 0);
		setvbuf(StreamMgr_get_error_stream(streams), _IONBF,
			(char *)NULL, 0);
#else
		setvbuf(StreamMgr_get_output_stream(streams), (char *)NULL,
			_IOLBF, 0);
		setvbuf(StreamMgr_get_error_stream(streams), (char *)NULL,
			_IONBF, 0);
#endif
	}
#endif
...
void init_preprocessors(const NuSMVEnv_ptr env)
{
	...
	if (cpp_call == (char *)NULL) {
		...
		StreamMgr_print_error(
			streams, "The pre-proprocessor could not be found.\n");
	}
...
static const char *user_preprocessor(const NuSMVEnv_ptr env, const char *name)
{
  ...
  */
	...
	for (i = 0; i < size_user_pp_stack; i++) {
		if (strncmp(name, user_pp_stack[i].name,
			    strlen(user_pp_stack[i].name)) == 0) {
			res = user_pp_stack[i].get_pp(opts);
			if (NULL != res) {
				if (true == Utils_file_exists(res))
					return res;
				else
					ErrorMgr_error_set_preprocessor(
						errmgr, res, false);
			}
		}
	}
