...
void NuSMVCore_init_data(void)
{
	...
	NuSMVCore_set_website("http://nusmv.fbk.eu");
	{
		...
		char *tmp = ALLOC(char,
				  strlen(fmt) +
					  strlen(NUSMV_PACKAGE_BUGREPORT) + 1);
...
void NuSMVCore_init(NuSMVEnv_ptr env, FP_V_E funs[][2], int num_funs)
{
	...
	if (num_funs > 0) {
		...
		NuSMVEnv_set_value(env, ENV_INIT_FUNS_NUM,
				   PTR_FROM_INT(void *, num_funs));
	}
...
void NuSMVCore_init_cmd_options(NuSMVEnv_ptr env)
{
...

	{
		...
		char *tmp = ALLOC(char,
				  strlen(fmt) + strlen(libraryName) +
					  (2 * strlen(data->tool_rcfile)) + 1);

		sprintf(tmp, fmt, libraryName, data->tool_rcfile,
			data->tool_rcfile);
		NuSMVCore_add_env_command_line_option("-s", tmp, NULL,
...

	{
		...
		NuSMVCore_add_env_command_line_option(
			"-ojeba", tmp, "str",
			OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM, true, false, NULL,
			NULL);
		FREE(tmp);
	}

	{
		...
		char *preps_tmp;
		const char *fmt =
			"defines a space-separated list of pre-processors "
		...
		if (get_preprocessors_num(env) > 0) {
			const char *preps_fmt =
				"The available preprocessors are: %s";
			char *preps = get_preprocessor_names(env);
			preps_tmp = ALLOC(char, strlen(preps_fmt) +
							strlen(preps) + 1);
			sprintf(preps_tmp, preps_fmt, preps);
			FREE(preps);
		} else {
			const char *preps_fmt =
				"Warning: there are no available preprocessors";
...

	{
		...
		NuSMVCore_add_env_command_line_option("-sat_solver", tmp, "str",
						      A_SAT_SOLVER, true, false,
						      NULL, NULL);
	...
	NuSMVCore_add_command_line_option("-iwls95",
					  ...
					  "cp_t",
					  nusmv_core_set_iwls95_partition, true,
					  false, NULL, "-thresh -mono -cp");
	...
	NuSMVCore_add_command_line_option(
		...
		"cp_t", nusmv_core_set_thresh_partition, true, false, NULL,
		"-iwls95 -mono -cp");
	...
	NuSMVCore_add_command_line_option(
		...
		"cp_t", nusmv_core_set_thresh_partition, true, true, NULL,
		"-iwls95 -mono -thresh");
...
#endif
		,
		NULL, nusmv_core_set_cpp, true, true, NULL, NULL);
	/* --------------------------------------------------------------- */
#endif

	NuSMVCore_add_env_command_line_option(
		"-r", "enables printing of reachable states", NULL,
		PRINT_REACHABLE, true, false, NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option(
		...
		NULL, OPT_CHECK_FSM, true, false, NULL, NULL);

	NuSMVCore_add_env_command_line_option(
		"-lp", "lists all properties in SMV model", NULL,
		LIST_PROPERTIES, true, false, NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option("-is", "does not check SPEC",
					      NULL, IGNORE_SPEC, true, false,
					      NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option("-ii", "does not check INVARSPEC",
					      NULL, IGNORE_INVAR, true, false,
					      NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option(
		...
		"from file \"tv_file\"",
		"tv_file", TRANS_ORDER_FILE, true, false, NULL, NULL);

	NuSMVCore_add_env_command_line_option("-AG", "enables AG only search",
					      NULL, AG_ONLY_SEARCH, true, false,
					      NULL, NULL);

	NuSMVCore_add_env_command_line_option("-reorder",
					      "enables reordering of "
					      "variables before exiting",
					      NULL, ENABLE_REORDER, true, false,
					      NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option(
		...
		"heuristics", BDD_STATIC_ORDER_HEURISTICS, true, false, NULL,
		NULL);

	NuSMVCore_add_env_command_line_option(
		"-coi", "enables cone of influence reduction", NULL,
		CONE_OF_INFLUENCE, true, false, NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option("-obm",
					      ...
					      " file \"bn_file\"",
					      "bn_file",
					      OUTPUT_BOOLEAN_MODEL_FILE, true,
					      false, NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option("-int",
					      "enables interactive mode", NULL,
					      BATCH, true, false, NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option(
		"-load", "executes NuSMV commands from file", "sc_file",
		SCRIPT_FILE, true, true, NULL, "-source");

	NuSMVCore_add_env_command_line_option(
		"-source", "executes NuSMV commands from file", "sc_file",
		SCRIPT_FILE, true, false, NULL, "-load");

	NuSMVCore_add_env_command_line_option("-quiet", "Quiet mode", NULL,
					      QUIET_MODE, false, false, NULL,
					      NULL);

	NuSMVCore_add_env_command_line_option("-v",
					      "sets verbose level to \"vl\"",
					      "vl", VERBOSE_LEVEL, true, false,
					      NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option(
		...
		NULL, DISABLE_SYNTACTIC_CHECKS, true, false, NULL, NULL);
	...
	NuSMVCore_add_env_command_line_option(
		...
		"constant DEFINEs",
		NULL, KEEP_SINGLE_VALUE_VARS, true, false, NULL, NULL);
	...
	*status = nusmv_core_parse_line_options(env, argc, argv);
	...
	if ((*status) == RET_SUCCESS) {
		if (!opt_batch(opts)) { /* interactive mode */
			...
			quit_flag =
				nusmv_core_start_interactive_shell_loop(env);
			*status = RET_SUCCESS;
		} else {
			if (!opt_get_quiet_mode(opts)) {
				data->print_banner(outstream);
					   ...
					   char *dependency, char *conflict)
{
	...
	if ((char *)NULL != dependency) {
		opt->dependency =
			UStringMgr_find_string(data->string_mgr, dependency);
	}
...
static void nusmv_core_print_usage(NuSMVEnv_ptr env, boolean print_banner)
{
	...
	avl_tree *avl =
		avl_init_table((int (*)(char *, char *))Utils_strcasecmp);
	avl_generator *gen;
	char *option_name;
	...
	ASSOC_FOREACH(data->line_options, iter, &key, &opt)
	{
       ...
       the user */
		if (opt->public) {
			avl_insert(avl, (char *)UStringMgr_get_string_text(key),
				   (char *)opt);
		}
	}
	...
	StreamMgr_print_error(
		streams, "Usage:  %s [-h | -help] [options]* [input_file]\n",
		data->tool_name);
	...
	while (avl_gen(gen, &option_name, &option_struct_ptr)) {
		CmdLineOpt_ptr option_struct =
			(CmdLineOpt_ptr)option_struct_ptr;
		char *tmp;
		...
		if ((char *)NULL != option_struct->parameter) {
			tmp = ALLOC(char,
				    strlen(option_name) +
					    strlen(option_struct->parameter) +
					    2);
			sprintf(tmp, "%s %s", option_name,
				option_struct->parameter);
		} else {
		...
		if ((string_ptr)NULL != option_struct->dependency) {
			...
			tmp = REALLOC(
				char, tmp,
				strlen(fmt) +
					strlen(UStringMgr_get_string_text(
						option_struct->dependency)) +
					1);
		...
		if (!Olist_is_empty(option_struct->conflicts)) {
			...
			tmp = REALLOC(char, tmp,
				      strlen(fmt) + strlen(conf) + 2);

			sprintf(tmp, fmt,
				(Olist_get_size(option_struct->conflicts) > 1 ?
					 "s" :
					 ""),
				conf);
		...
		if (option_struct->deprecated) {
			...
			tmp = REALLOC(char, tmp,
				      strlen(fmt) + strlen(option_name) + 1);
					 ...
					 char **argv)
{
	...
	while (argc > 0) {
		...
		/* The input file (last argument) is hardcoded */
			...
			string_ptr opt_name = UStringMgr_find_string(
				data->string_mgr, opt_string);
			CmdLineOpt_ptr opt = (CmdLineOpt_ptr)find_assoc(
				data->line_options, NODE_PTR(opt_name));
			...
			if (Olist_contains(line_options, opt_name)) {
				...
				if ((CmdLineOpt_ptr)NULL != opt) {
					...
					insert_assoc(required,
						     NODE_PTR(opt_name),
						     NODE_PTR(opt->dependency));

					/* Add to the conflicting table the options conflicting with the
             current one */
					insert_assoc(conflicting,
						     NODE_PTR(opt_name),
               ...
               check&apply fun.*/
						...
						nusmv_assert(
							OptsHandler_is_option_registered(
								opts,
								opt->env_option));

						/* If the option is boolean, invert the value. */
						if (OptsHandler_is_bool_option(
							    opts,
							    opt->env_option)) {
							boolean res;
							boolean curr;
							...
							res = OptsHandler_set_option_value(
								opts,
								opt->env_option,
								param);
							if (!res) {
	...
	if (status == RET_SUCCESS) {
		...
		OLIST_FOREACH(line_options, iter)
		{
			...
			Olist_ptr conflicts = (Olist_ptr)find_assoc(
				conflicting, NODE_PTR(option));

			string_ptr dependency = (string_ptr)find_assoc(
				required, NODE_PTR(option));
			...
			if (!Olist_is_empty(conflicts) &&
				...
				Olist_ptr intersect =
					nusmv_core_olist_intersection(
						conflicts, line_options);

				nusmv_core_olist_union(printed, intersect);

				if (!Olist_is_empty(intersect)) {
					const char *fmt =
						"Option %s cannot be used with option%s %s";
					char *conf =
						nusmv_core_merge(intersect);
					char *tmp;
					...
					sprintf(tmp, fmt,
						UStringMgr_get_string_text(
							option),
						(Olist_get_size(intersect) > 1 ?
							 "s" :
							 ""),
						conf);
					FREE(conf);

					nusmv_core_print_string(errstream, tmp,
								0);
...
static char *nusmv_core_merge(Olist_ptr set)
{
	...
	OLIST_FOREACH(set, iter)
	{
		...
		result =
			REALLOC(char, result, strlen(result) + strlen(str) + 2);
...
static Olist_ptr nusmv_core_split(UStringMgr_ptr strmgr, char *string)
{
	...
	if (next_length > 0) {
		...
		if (j > 0) {
			tmp[j] = '\0';
			Olist_append(result,
				     UStringMgr_find_string(strmgr, tmp));
		}
...
{
	if (strcmp(val, "off") == 0) {
		return OptsHandler_set_bool_option_value(opt, SYMB_INLINING,
							 false);
...
{
	if (strcmp(val, "off") == 0) {
		return OptsHandler_set_bool_option_value(opt, RBC_INLINING,
							 false);
	} else if (strcmp(val, "on") == 0) {
		return OptsHandler_set_bool_option_value(opt, RBC_INLINING,
							 true);
...
static int nusmv_core_start_interactive_shell_loop(NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "Interactive shell not available, use batch mode\n");
	quit_flag = -5; /* require immediate quit */
