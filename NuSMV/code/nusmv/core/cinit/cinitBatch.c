	...
	CATCH(errmgr)
	{
		...
		if (opt_list_properties(opts)) {
			res = PropDb_show_property(
				prop_db, false /* print_props_num */,
				PROPDB_PRINT_FMT_DEFAULT, Prop_NoType,
				Prop_NoStatus, -1 /* prop_no */,
		...
		{ /* 3: Builds the encodings */
			res = Compile_encode_variables(
				env, NULL /*input_order_file_name*/,
...

		{
			...
			if (NULL != fname) {
				...
				res |= Compile_write_model_flat_bool(env, fname,
								     _file);
				fclose(_file);
		...
		if (opt_bmc_mode(opts)) {
         ...
         boolean model was specified in the argument list. */
			if (Compile_check_if_bool_model_was_built(env, NULL,
								  false)) {
				res = Compile_create_boolean_model(env);
				if (res)
					goto batch_exit_fail;
         ...
         determinization layers: */
			...
			if (get_prop_no(opts) != -1) {
				...
				case Prop_Invar:
					...
					Bmc_GenSolveInvar(env, prop, TRUE,
							  BMC_DUMP_NONE, NULL);
					break;
				...
				if (!opt_ignore_ltlspec(opts)) {
					...
					if (opt_verbose_level_gt(opts, 0)) {
						...
						Logger_log(
							logger,
							"Verifying the LTL properties...\n");
					}

					props = PropDb_get_props_of_type(
						prop_db, Prop_Ltl);
				...
				if (!opt_ignore_pslspec(opts)) {
					...
					lsDestroy(
						props,
						NULL); /* the list is no longer needed */
				}
				...
				if (!opt_ignore_invar(opts)) {
					...
					if (opt_verbose_level_gt(opts, 0)) {
						...
						Logger_log(
							logger,
							"Verifying the INVAR properties...\n");
					}
		...
		{ /* check required properties */
			...
			if (-1 != prop_no) {
				res = PropDb_check_property(
					prop_db, Prop_NoType, NULL, prop_no);
			} else {
				/* Evaluates the Specifications */
				if (!opt_ignore_spec(opts)) {
					res = PropDb_check_property(prop_db,
								    Prop_Ctl,
								    NULL,
								    prop_no);
				}
		...
		if (opt_reorder(opts)) {
			...
			{ /* write order */
				BddEnc_ptr bdd_enc = BDD_ENC(NuSMVEnv_get_value(
					env, ENV_BDD_ENCODER));

				res = BddEnc_write_var_ordering(
					bdd_enc, get_output_order_file(opts),
