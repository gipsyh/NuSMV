...
void CInit_batch_main(NuSMVEnv_ptr env) {
  ...
  CATCH(errmgr) {
    ...
    if (opt_list_properties(opts)) {
      res = PropDb_show_property(prop_db, false /* print_props_num */,
    ...
    { /* 3: Builds the encodings */
      res = Compile_encode_variables(env, NULL /*input_order_file_name*/,
    ...
    if (opt_bmc_mode(opts)) {
      ...
      if (get_prop_no(opts) != -1) {
        ...
        if ((prop_no < 0) || (prop_no >= PropDb_get_size(prop_db))) {
        ...
        case Prop_Ltl:
          ...
          if (!opt_ignore_ltlspec(opts)) {
            ...
            Bmc_GenSolveLtl(env, prop, get_bmc_pb_length(opts), rel_loop,
                            /*increasing length*/ TRUE, TRUE, BMC_DUMP_NONE,
                            NULL);
          }
          break;
        ...
        case Prop_Psl: {
          ...
          Bmc_Gen_check_psl_property(env, prop, false, false, false,
                                     get_bmc_pb_length(opts), rel_loop);
          break;
        }
        ...
        if (!opt_ignore_pslspec(opts)) {
          ...
          lsForEachItem(props, iterator, prop) {
            if (Prop_is_psl_ltl(prop)) {
              ...
              Bmc_Gen_check_psl_property(env, prop, false, false, false, len,
                                         rel_loop);
            }
          }
        ...
        if (!opt_ignore_invar(opts)) {
          ...
          props = PropDb_get_props_of_type(prop_db, Prop_Invar);
          nusmv_assert(props != LS_NIL);
    ...
    if (opt_reorder(opts)) {
      StreamMgr_print_output(streams,
                             "\n========= starting reordering ============\n");
      dd_reorder(dd, get_reorder_method(opts), DEFAULT_MINSIZE);
      StreamMgr_print_output(streams,
                             "\n========= after reordering ============\n");
      if (opt_verbose_level_gt(opts, 0)) {
        res = Compile_print_usage(env, outstream);
    ...
    if (opt_print_reachable(opts)) {
      BddFsm_ptr fsm = NULL;
      if (opt_cone_of_influence(opts)) {
        StreamMgr_print_error(streams, "WARNING: Statistics of reachable "
      ...
      BddFsm_print_reachable_states_info(fsm, false, /* do not print states */
