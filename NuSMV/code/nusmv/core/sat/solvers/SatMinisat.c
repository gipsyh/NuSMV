...
int sat_minisat_cnfLiteral2minisatLiteral(SatMinisat_ptr self, int cnfLiteral) {
  ...
  minisatVar =
      NODE_TO_INT(find_assoc(self->cnfVar2minisatVar, NODE_FROM_INT(cnfVar)));
...
void sat_minisat_set_polarity(const SatSolver_ptr solver,
  ...
  minisatLiteral = sat_minisat_cnfLiteral2minisatLiteral(self, cnfLiteral);
...
void sat_minisat_set_preferred_variables(const SatSolver_ptr solver,
  ...
  SLIST_FOREACH(cnfVars, genVar) {
    ...
    minisatVar = NODE_TO_INT(
        find_assoc(self->cnfVar2minisatVar, NODE_FROM_INT((int)cnfVar)));
...
sat_minisat_solve_permanent_group_assume(const SatSolver_ptr sol,
  ...
  if (Olist_contains(solver->unsatisfiableGroups, (void *)permanentGroup)) {
    ...
    minisatResult =
        MiniSat_Solve_Assume(self->minisatSolver, numberOfAssumptions, lits);
    FREE(lits);
  }
...
SatSolverResult sat_minisat_solve_groups(const SatIncSolver_ptr solver,
  ...
  /* try to solver (MiniSat_Solve will invoke internal simplifyDB) */
  minisatResult =
      MiniSat_Solve_Assume(self->minisatSolver, numberOfGroups, assumptions);
  FREE(assumptions);
  if (1 == minisatResult) {
    return SAT_SOLVER_SATISFIABLE_PROBLEM;
  } else
...
void sat_minisat_enlarge_minisatClause(const SatMinisat_ptr self,
  ...
  while (self->minisatClauseSize < minSize) {
    ...
    self->minisatClause =
        REALLOC(int, self->minisatClause, self->minisatClauseSize);
...
sat_minisat_extract_interpolant(SatSolver_ptr solver, int nof_ga_groups,
  ...
  while (!STACK_IS_EMPTY(to_process)) {
    ...
    if (MiniSat_Proof_IsRes(self->minisatSolver, p)) {
      ...
      if (of_b) {
        for (i = 0, n = MiniSat_Proof_Size(self->minisatSolver, p); i < n;
             ++i) {
          int l = MiniSat_Proof_Lit(self->minisatSolver, p, i);
          Sset_key k = PTR_FROM_INT(Sset_key, l < 0 ? -l : l);
  ...
  while (!STACK_IS_EMPTY(to_process)) {
    ...
    if (MiniSat_Proof_IsRes(self->minisatSolver, p)) {
      ...
      if (done) {
        ...
        for (i = 1, n = MiniSat_Proof_Size(self->minisatSolver, p); i < n;
             ++i) {
          Term cur;
          bool of_a;
          Sset_key k;
