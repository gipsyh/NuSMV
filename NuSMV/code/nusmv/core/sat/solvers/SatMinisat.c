...
int sat_minisat_cnfLiteral2minisatLiteral(SatMinisat_ptr self, int cnfLiteral)
{
	...
	minisatVar = NODE_TO_INT(
		find_assoc(self->cnfVar2minisatVar, NODE_FROM_INT(cnfVar)));

	if (0 == minisatVar) {
		/* create a new internal var and associate with cnf */
		minisatVar = MiniSat_New_Variable(self->minisatSolver);

		insert_assoc(self->cnfVar2minisatVar, NODE_FROM_INT(cnfVar),
			     NODE_FROM_INT(minisatVar));

		insert_assoc(self->minisatVar2cnfVar, NODE_FROM_INT(minisatVar),
					  ...
					  int minisatLiteral)
{
	...
	int cnfVar = NODE_TO_INT(
		find_assoc(self->minisatVar2cnfVar, NODE_FROM_INT(minisatVar)));
...
void sat_minisat_add(const SatSolver_ptr solver, const Be_Cnf_ptr cnfProb,
		     SatSolverGroup group)
{
	...
	SLIST_FOREACH(Be_Cnf_GetClausesList(cnfProb), genClause)
	{
		...
		if (sat_minisat_get_minisatClauseSize(self) - 4 <=
		    clause_size) {
			sat_minisat_enlarge_minisatClause(self,
							  clause_size + 5);
			minisatClause = sat_minisat_get_minisatClause(self);
		}
		i = 0;
		while (clause[i] != 0) {
			...
			minisatClause[literalNumber] =
				sat_minisat_cnfLiteral2minisatLiteral(self,
								      literal);
			++literalNumber;
			i++;
		}
			      ...
			      SatSolverGroup group)
{
	...
	if (Be_Cnf_GetFormulaLiteral(cnfProb) == INT_MAX) {
       ...
       (see SatSolver.c, SatSolver_set_polarity) */
		nusmv_assert(
			(Slist_get_size(Be_Cnf_GetClausesList(cnfProb)) == 0 ?
				 1 :
				 -1) *
				polarity !=
			1);
	...
	minisatLiteral =
		sat_minisat_cnfLiteral2minisatLiteral(self, cnfLiteral);
					 ...
					 const Slist_ptr cnfVars)
{
	...
	SLIST_FOREACH(cnfVars, genVar)
	{
		...
		minisatVar = NODE_TO_INT(find_assoc(
			self->cnfVar2minisatVar, NODE_FROM_INT((int)cnfVar)));
		/* skip var, if not existing */
		if (0 != minisatVar) {
			MiniSat_Set_Preferred_Variable(self->minisatSolver,
						       minisatVar - 1);
		}
	}
}
...
Slist_ptr sat_minisat_make_model(const SatSolver_ptr solver)
{
	...
	for (index = 1; index <= varNumber; ++index) {
		int cnfLiteral =
			sat_minisat_minisatLiteral2cnfLiteral(self, index);
		...
		if (cnfLiteral > 0) { /* it is a real variable */
			...
			case 1: /* positive polarity => do nothing */
				/* appends the model: */
				Slist_push(model,
					   PTR_FROM_INT(void *, cnfLiteral));
				break;

			case -1:
				break; /* does not store unassigned vars */
			default:
...
int sat_minisat_get_cnf_var(const SatSolver_ptr solver, int var)
{
	...
	return sat_minisat_minisatLiteral2cnfLiteral(self,
						     /*1 + */ var); /* AG */
}
...
void sat_minisat_destroy_group(const SatIncSolver_ptr solver,
			       SatSolverGroup group)
{
	SatMinisat_ptr self = SAT_MINISAT(solver);
	int minisatClause[2];
	...
	const SatIncSolver_ptr solver, SatSolverGroup group)
{
	...
	if (Olist_contains(SAT_SOLVER(self)->unsatisfiableGroups,
	    ...
	    !Olist_contains(SAT_SOLVER(self)->unsatisfiableGroups,
			    (void *)permamentGroup)) {
		Olist_prepend(SAT_SOLVER(self)->unsatisfiableGroups,
			      (void *)permamentGroup);
	}
...
SatSolverResult sat_minisat_solve_groups(const SatIncSolver_ptr solver,
					 const Olist_ptr groups)
{
   ...
   We check it here because the input list may not contain permanent group */
	...
	{
		size_t i, sz;
		for (i = 0, sz = Stack_get_size(self->minisat_itp_groups);
		     i < sz; ++i) {
...
SatSolverResult sat_minisat_solve_without_groups(const SatIncSolver_ptr solver,
						 const Olist_ptr groups)
{
	SatMinisat_ptr self = SAT_MINISAT(solver);
...
Slist_ptr sat_minisat_make_conflicts(const SatMinisat_ptr self)
{
	...
	for (index = 0; index < litNumber; index++) {
		int cnfLiteral = sat_minisat_minisatLiteral2cnfLiteral(
			self, lits[index]);
				       ...
				       unsigned int minSize)
{
	...
	while (self->minisatClauseSize < minSize) {
		...
		self->minisatClause = REALLOC(int, self->minisatClause,
				...
				TermFactoryCallbacksUserData_ptr user_data)
{
	...
	for (i = 0, n = Stack_get_size(self->minisat_itp_groups); i < n; ++i) {
		Sset_insert(b_groups,
			    PTR_FROM_INT(Sset_key,
					 self->minisat_itp_groups->array[i]),
			    PTR_FROM_INT(void *, 1));
	}
	for (i = 0; i < nof_ga_groups; ++i) {
		Sset_delete(b_groups, PTR_FROM_INT(Sset_key, ga_groups[i]),
			    NULL);
	}
	...
	{
		Sset_insert(b_lits, Oiter_element(g_it),
			    PTR_FROM_INT(void *, 1));
	}
		...
		if (MiniSat_Proof_IsRes(self->minisatSolver, p)) {
			...
			for (i = 0,
			    n = MiniSat_Proof_Size(self->minisatSolver, p);
			     i < n; ++i) {
				MiniSat_Proof_Res(self->minisatSolver, p, i,
						  &pivot, &c);
			...
			for (i = 0; i < n; ++i) {
				int l = MiniSat_Proof_Lit(self->minisatSolver,
							  p, i);
				Sset_key k =
					PTR_FROM_INT(Sset_key, l < 0 ? -l : l);
			...
			if (of_b) {
				for (i = 0, n = MiniSat_Proof_Size(
						    self->minisatSolver, p);
				     i < n; ++i) {
					int l = MiniSat_Proof_Lit(
						self->minisatSolver, p, i);
					Sset_key k = PTR_FROM_INT(
						Sset_key, l < 0 ? -l : l);

					Sset_insert(b_lits, k,
						    PTR_FROM_INT(void *, 1));
				}
			}
		}
	...
	while (!STACK_IS_EMPTY(to_process)) {
		...
		if (MiniSat_Proof_IsRes(self->minisatSolver, p)) {
			...
			if (done) {
				Term f;
				Stack_pop(to_process);
				MiniSat_Proof_Res(self->minisatSolver, p, 0,
						  &pivot, &c);
				f = (Term)find_assoc(cache, (node_ptr)c);

				for (i = 1, n = MiniSat_Proof_Size(
						    self->minisatSolver, p);
				     i < n; ++i) {
					Term cur;
					bool of_a;
					...
					MiniSat_Proof_Res(self->minisatSolver,
							  p, i, &pivot, &c);

					cur = (Term)find_assoc(cache,
							       (node_ptr)c);
					k = PTR_FROM_INT(Sset_key,
							 pivot < 0 ? -pivot :
								     pivot);
					of_a = !Ssiter_is_valid(
						Sset_find(b_lits, k));
					if (of_a) {
						f = callbacks->make_or(
							f, cur, user_data);
					} else {
						f = callbacks->make_and(
							f, cur, user_data);
					}
				}
           ...
           which we have to treat as coming from b */
			...
			if (of_b) {
				insert_assoc(cache, (node_ptr)p,
					     callbacks->make_true(user_data));
			} else {
					...
					if (Ssiter_is_valid(
						...
						int cnf_lit =
							sat_minisat_minisatLiteral2cnfLiteral(
								self, idx);
						if (cnf_lit > 0) {
							Term v = callbacks->make_var(
								idx, user_data);
							if (l < 0) {
								v = callbacks->make_not(
									v,
									user_data);
							}
							b_part = callbacks->make_or(
								b_part, v,
								user_data);
						}
					}
		      ...
		      const char *name, boolean enable_proof_logging)
{
	...
	OVERRIDE(SatSolver, extract_interpolant) =
