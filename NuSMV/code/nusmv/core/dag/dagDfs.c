...
void Dag_Dfs(Dag_Vertex_t *current, Dag_DfsFunctions_t *dfs_fun, char *dfs_data)
{
	int set;
	int dfsCode;
	...
	while (true) {
		...
		if ((-1 == set) ||
			...
			if (!IS_LEAF(current_ref)) {
				Stack_push(parent_stack,
					   DagId(current_ref, current_bit));
				current = current_ref->outList[0];
				current_bit = Dag_VertexIsSet(current);
				current_ref = Dag_VertexGetRef(current);
				continue;
			} else {
				dfs_fun->LastVisit(current_ref, dfs_data,
						   current_bit);
			}
		}
		...
		while (true) {
			...
			if (processed ==
			    parent_ref->outList[parent_ref->numSons - 1]) {
				/* Subtree processed. Back on the parent, if any. */
				dfs_fun->LastVisit(parent_ref, dfs_data,
						   parent_bit);
				processed =
					(Dag_Vertex_t *)Stack_pop(parent_stack);
...
void Dag_Dfs(Dag_Vertex_t *dfsRoot, Dag_DfsFunctions_t *dfsFun, char *dfsData)
{
	...
	DFS(Dag_VertexGetRef(dfsRoot), dfsFun, dfsData,
		...
		nusmv_ptrint vBit)
{
	...
	if (v->outList != (Dag_Vertex_t **)NULL) {
		for (gen = 0; gen < v->numSons; gen++) {
			...
			DFS(Dag_VertexGetRef(vSon), dfsFun, dfsData,
