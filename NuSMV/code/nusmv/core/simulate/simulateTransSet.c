       ...
       that have been committed to the passed encoding */
		...
		if (state_frozen_var_exist) {
			res = BddEnc_pick_all_terms_states(
				enc, next_states_set, self->next_states_array,
				self->next_states_num);
			nusmv_assert(
				!res); /* res is true if an error occurred */
		} else {
	...
	if (from_state != (bdd_ptr)NULL) {
		...
		{ /* calculates the number of input variables in the default
         class */
			...
			num_inputs = 0;
			layers = SymbTable_get_class_layer_names(
				self->symb_table, (const char *)NULL);
			arrayForEachItem(char *, layers, i, name)
			{
				num_inputs += SymbLayer_get_input_vars_num(
					SymbTable_get_layer(self->symb_table,
							    name));
			}
		}
		...
		for (s = 0; s < next_states_count; ++s) {
			if (num_inputs > 0) {
				...
				bdd_and_accumulate(self->dd, &inputs_per_state,
						   inputs_mask);

				self->inputs_num_per_state[s] =
					BddEnc_count_inputs_of_bdd(
						enc, inputs_per_state);

				array_of_inputs = ALLOC(
					bdd_ptr, self->inputs_num_per_state[s]);
				nusmv_assert(array_of_inputs !=
					     (bdd_ptr *)NULL);

				res = BddEnc_pick_all_terms_inputs(
					enc, inputs_per_state, array_of_inputs,
					self->inputs_num_per_state[s]);
				nusmv_assert(
					!res); /* res is true if an error occurred */
	...
	if (self->inputs_num_per_state != (int *)NULL) {
		for (s = 0; s < self->next_states_num; ++s) {
			...
			for (i = 0; i < self->inputs_num_per_state[s]; ++i) {
				if (self->inputs_per_state[s][i] !=
				    (bdd_ptr)NULL) {
					bdd_free(self->dd,
						 self->inputs_per_state[s][i]);
				}
			}
...
{
	SIMULATE_TRANS_SET_CHECK_INSTANCE(self);
	nusmv_assert((state_index >= 0) &&
		     (state_index < self->next_states_num));
					    ...
					    int state_index, int input_index)
{
	...
	nusmv_assert((input_index >= 0) &&
		     (input_index < SimulateTransSet_get_inputs_num_at_state(
					    self, state_index)));
	...
	for (s = 0; s < states_num; ++s) {
		...
		if (inputs_num > 0) {
			if (index < count + inputs_num) {
				...
				*state = SimulateTransSet_get_next_state(self,
									 s);
				*input = SimulateTransSet_get_input_at_state(
					self, s, index - count);
				break;
			}
			count += inputs_num;
					   ...
					   bdd_ptr *state, bdd_ptr *input)
{
	...
	*state = SimulateTransSet_get_next_state(self, s);
	...
	if (SimulateTransSet_get_inputs_num_at_state(self, s) > 0) {
		int i;
		i = utils_random() %
		    SimulateTransSet_get_inputs_num_at_state(self, s);
			   ...
			   boolean show_changes_only, OStream_ptr output)
{
...

	{
		...
		array_t *ls = SymbTable_get_class_layer_names(
			self->symb_table, (const char *)NULL);
		NodeList_ptr unfiltered_state_symbs =
			SymbTable_get_layers_sf_symbols(self->symb_table, ls);
		NodeList_ptr state_symbs = TracePkg_get_filtered_symbols(
			tm, unfiltered_state_symbs);

		NodeList_ptr unfiltered_input_symbs =
			SymbTable_get_layers_i_symbols(self->symb_table, ls);
		NodeList_ptr input_symbs = TracePkg_get_filtered_symbols(
			tm, unfiltered_input_symbs);
		...
		BddEnc_print_bdd_begin(self->enc, state_symbs,
				       show_changes_only);
		...
		for (s = 0; s < states_num; ++s) {
			...
			inputs_num = SimulateTransSet_get_inputs_num_at_state(
				self, s);
			...
			BddEnc_print_bdd(self->enc, state, (VPFBEFNNV)NULL,
					 output, NULL);
			...
			if (inputs_num > 0) {
				int i;
				OStream_inc_indent_size(output);
				BddEnc_print_bdd_begin(self->enc, input_symbs,
						       show_changes_only);
				...
				for (i = 0; i < inputs_num; ++i) {
					...
					BddEnc_print_bdd(self->enc, input,
							 (VPFBEFNNV)NULL,
							 output, NULL);
