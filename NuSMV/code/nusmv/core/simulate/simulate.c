				...
				Simulation_Mode mode, int n, int display_all)
{
	...
	while (i <= n) {
       ...
       on state vars, leave constraint as it is otherwise */
		if (time_shift) {
			next_constraint = BddEnc_state_var_to_next_state_var(
				enc, constraint);
		} else {
		...
		if (bdd_is_false(dd, next_constr_set)) {
			StreamMgr_print_error(streams,
					      "No future state exists");
			StreamMgr_print_error(streams,
					      (llength(result) == 1 ?
						       ": trace not built.\n" :
						       "."));
			StreamMgr_print_error(
				streams, "Simulation stopped at step %d.\n", i);
		...
		Simulate_ChooseOneStateInput(env, fsm, current_state_bdd,
					     next_constr_set, mode, display_all,
					     &input, &state);
...
{
	simulate_choose_next(env, fsm, from_state, next_set, mode, display_all,
			     input, state);
}
			     ...
			     bdd_ptr bdd_constraints)
{
	...
	if (i == 0) {
		StreamMgr_print_error(streams,
				      "The set of initial states is EMPTY. "
				      "No state can be chosen.\n");
		bdd_free(dd, initial_set);
		return -1;
	}

	chosen = Simulate_ChooseOneState(env, fsm, initial_set, mode,
					 display_all);
	bdd_free(dd, initial_set);
			...
			const boolean verbose, bdd_ptr bdd_constraints)
{
     ...
     * trace that is going to be registered. */
		...
		tr_number = Simulate_CmdPickOneState(
			env, bdd_fsm, mode, display_all, bdd_constraints);
	}
...
int Simulate_simulate(const NuSMVEnv_ptr env, const boolean time_shift,
		      ...
		      const boolean only_changes, const bdd_ptr bdd_constraints)
{
	/* needed for trace lanugage */
	const SexpFsm_ptr const sexp_fsm =
		SEXP_FSM(NuSMVEnv_get_value(env, ENV_SEXP_FSM));
	const BddFsm_ptr const fsm =
		BDD_FSM(NuSMVEnv_get_value(env, ENV_BDD_FSM));
	...
	current_trace = Simulate_MultipleSteps(env, fsm, bdd_constraints,
					       time_shift, mode, steps,
					       display_all);
	...
	/* Update the current state. */
	{
		...
		new_label = TraceLabel_create(nodemgr, trace_id,
...
int Simulate_goto_state(NuSMVEnv_ptr env, TraceLabel label)
{
	...
	state = TraceUtils_fetch_as_bdd(from_trace, iter, TRACE_ITER_SF_VARS,
					enc);
...
int Simulate_print_current_state(NuSMVEnv_ptr env, boolean Verbosely)
{
	...
	if (Verbosely == 0) {
		bdd_ptr const current_state_bdd =
			SimulateState_get_bdd(current_state);
		OStream_ptr outstream = StreamMgr_get_output_ostream(streams);

		BddEnc_print_bdd_begin(enc, SexpFsm_get_vars_list(sexp_fsm),
				       false);
		BddEnc_print_bdd(enc, current_state_bdd, (VPFBEFNNV)NULL,
				 outstream, NULL);
		BddEnc_print_bdd_end(enc);
	}
					     ...
					     boolean allow_inputs)
{
	...
	if ((Parser_ReadNextExprFromString(env, constr_str, &parsed) == 0) &&
		...
		boolean is_wff = TypeChecker_is_expression_wellformed(
			tc, constraints, Nil);

		if (is_wff) {
			/* keep NEXT nodes */
			vars = Formula_GetDependenciesByType(
				st, constraints, Nil, VFT_CNIF, true);

			/* verify wrt input that are not allowed in constraints: */
			CATCH(errmgr)
			{
				const NodeList_ptr vars_list =
					Set_Set2List(vars);
	...
	*which_input = NULL;
	...
	if (mode == Interactive) {
		...
		if (SETJMP(simulate_jmp_buff, 1) == 0) {
			...
			states_count = BddEnc_count_states_of_bdd(
				enc, constrained_next_set);
			trans_set = SimulateTransSet_create(
				fsm, enc, from_state, constrained_next_set,
				states_count);
			bdd_free(dd, constrained_next_set);

			max_choice = SimulateTransSet_print(
				trans_set, (display_all == 0),
			...
			if (max_choice > 0) {
				...
				while (NIL(char) != (fgets(line, CHOICE_LENGTH,
							   instream)) ||
				       (line[0] != '\n')) {
			...
			SimulateTransSet_get_state_input_at(
				trans_set, choice, which_state, which_input);
		} /* setjump */
       ...
       being destroyed: */
		...
		if (mode == Random) {
			next_state =
				BddEnc_pick_one_state_rand(enc, next_state_set);

			if (from_state != (bdd_ptr)NULL) {
				bdd_ptr inputs, masked_inputs;

				inputs = BddFsm_states_to_states_get_inputs(
					fsm, from_state, next_state);
				masked_inputs =
					BddEnc_apply_input_vars_mask_bdd(
						enc, inputs);
				bdd_free(dd, inputs);

				input = BddEnc_pick_one_input_rand(
					enc, masked_inputs);
			...
			if (from_state != (bdd_ptr)NULL) {
				...
				input = BddEnc_pick_one_input(enc,
							      masked_inputs);
				bdd_free(dd, masked_inputs);
			}
		}
					       ...
					       bdd_ptr bdd, int max_size)
{
	...
	while (size > max_size) {
		...
		masked_bdd = BddEnc_apply_state_frozen_vars_mask_bdd(
			enc, constrained_bdd);
...
static bdd_ptr simulate_request_constraints(NuSMVEnv_ptr env, BddEnc_ptr enc)
{
	...
	while (true) {
		...
		while (NIL(char) != fgets(str,
					  simulation_buffer_size - read_bytes,
					  instream)) {
			size_t new_read_bytes = strlen(str);
			if (new_read_bytes > 0 &&
			    str[new_read_bytes - 1] == '\n')
				break;
			...
			if (read_bytes == simulation_buffer_size - 1) {
				...
				simulation_buffer =
					REALLOC(char, simulation_buffer,
						simulation_buffer_size);
			}
			str = simulation_buffer + read_bytes;
		} /* end of while(fgets) */

		result = simulate_get_constraints_from_string(
			env, simulation_buffer, enc, false, /* no NEXTs */
			false /*only states*/);
					     ...
					     NodeList_ptr symbols)
{
	...
	trace = TraceMgr_get_trace_at_index(
		tm, TraceMgr_get_current_trace_number(tm));
