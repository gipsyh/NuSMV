...
bdd_ptr SimulateState_get_bdd(SimulateState_ptr const self) {
  ...
  if ((bdd_ptr)NULL != self->bdd && TRACE_LABEL_INVALID != self->trace_label) {
...
void SimulateState_set_all(SimulateState_ptr const self, bdd_ptr const state,
  ...
  if ((bdd_ptr)NULL != self->bdd) {
    bdd_free(self->dd_mgr, self->bdd);
    self->bdd = (bdd_ptr)NULL;
  }
...
SimulateState_ptr SimulateState_set_in_env(NuSMVEnv_ptr const env,
  ...
  if (NuSMVEnv_has_value(env, ENV_SIMULATE_STATE)) {
    current_state = SIMULATE_STATE(NuSMVEnv_get_value(env, ENV_SIMULATE_STATE));
