					 ...
					 PredicateNormaliser_ptr normaliser)
{
	...
	FlatHierarchy_set_justice(
		copy->hierarchy, PredicateNormaliser_normalise_expr(
					 normaliser, FlatHierarchy_get_justice(
							     copy->hierarchy)));
...
NodeList_ptr SexpFsm_get_symbols_list(const SexpFsm_ptr self)
{
	...
	if (NODE_LIST(NULL) == self->symbols) {
		...
		{
			node_ptr def =
				SymbTable_iter_get_symbol(self->st, &iter);
...
void SexpFsm_apply_synchronous_product(SexpFsm_ptr self, SexpFsm_ptr other)
{
	...
	self->const_var_fsm = var_fsm_synchronous_product(
		self, self->const_var_fsm, other->const_var_fsm);
   ...
   * assigns) is not preserved; MR said that's ok */
	...
	SET_FOREACH(self->vars_set, iter)
	{
		...
		fsm_prod =
			var_fsm_synchronous_product(self, fsm_self, fsm_other);
...
void SexpFsm_self_check(const SexpFsm_ptr self)
{
	...
	if (!Set_Contains(self->vars_set,
			  FlatHierarchy_get_vars(self->hierarchy))) {
	...
	*(self->family_counter) = 1; /* this is adam for this family */
     ...
     * assings */
		...
		FlatHierarchy_set_init(
			self->hierarchy,
			ExprMgr_and_nil(
				exprs, FlatHierarchy_get_init(self->hierarchy),
				var_fsm_get_init(self, self->const_var_fsm)));
		/* invars */
		FlatHierarchy_set_invar(
			self->hierarchy,
			ExprMgr_and_nil(
				exprs, FlatHierarchy_get_invar(self->hierarchy),
				var_fsm_get_invar(self, self->const_var_fsm)));
		/* next */
		FlatHierarchy_set_trans(
			self->hierarchy,
			ExprMgr_and_nil(
				exprs, FlatHierarchy_get_trans(self->hierarchy),
		...
		SET_FOREACH(self->vars_set, iter)
		{
			...
			VarFsm_ptr varfsm =
				sexp_fsm_hash_var_fsm_lookup_var(self, var);
			...
			if (varfsm != VAR_FSM(NULL)) {
				...
				FlatHierarchy_set_init(
					self->hierarchy,
					ExprMgr_and_nil(exprs,
							FlatHierarchy_get_init(
								self->hierarchy),
							tmp));
				/* invars */
	...
	if (self->inlining) {
       ...
       equivalences still hold */
		...
		(void)SexpInliner_force_equivalences(
			inliner, InlineRes_get_equivalences(invar_res));
		(void)SexpInliner_force_invariants(
			inliner, InlineRes_get_invariants(invar_res));
		...
		FlatHierarchy_set_invar(self->hierarchy,
					InlineRes_get_result(invar_res));
		FlatHierarchy_set_init(self->hierarchy,
				       InlineRes_get_result(init_res));
		FlatHierarchy_set_trans(self->hierarchy,
					InlineRes_get_result(trans_res));
...
void sexp_fsm_copy_aux(const SexpFsm_ptr self, SexpFsm_ptr copy)
{
	...
	copy->const_var_fsm = var_fsm_create(
		self, var_fsm_get_init(self, self->const_var_fsm),
		var_fsm_get_invar(self, self->const_var_fsm),
		var_fsm_get_next(self, self->const_var_fsm));
...
static void sexp_fsm_const_var_fsm_init(SexpFsm_ptr self, hash_ptr simp_hash)
{
	...
	node_ptr invar_const = FlatHierarchy_lookup_constant_constrains(
		self->hierarchy, INVAR);
	node_ptr trans_const = FlatHierarchy_lookup_constant_constrains(
		self->hierarchy, TRANS);
	node_ptr init_const =
	...
	invar_const =
		sexp_fsm_simplify_expr(self, simp_hash, invar_const, INVAR);
	init_const = sexp_fsm_simplify_expr(self, simp_hash, init_const, INIT);
	trans_const =
		sexp_fsm_simplify_expr(self, simp_hash, trans_const, TRANS);
...
static void sexp_fsm_hash_var_fsm_init(SexpFsm_ptr self, hash_ptr simp_hash)
{
	...
	SET_FOREACH(self->vars_set, iter)
	{
		...
		node_ptr var_name_i =
			find_node(nodemgr, SMALLINIT, var_name, Nil);
		node_ptr var_name_n = ExprMgr_next(exprs, var_name, symb_table);

		node_ptr init_a = FlatHierarchy_lookup_assign(self->hierarchy,
							      var_name_i);

		node_ptr invar_a =
			FlatHierarchy_lookup_assign(self->hierarchy, var_name);

		node_ptr next_a = FlatHierarchy_lookup_assign(self->hierarchy,
							      var_name_n);

		node_ptr init_c = FlatHierarchy_lookup_constrains(
			self->hierarchy, var_name_i);

		node_ptr invar_c = FlatHierarchy_lookup_constrains(
			self->hierarchy, var_name);

		node_ptr next_c = FlatHierarchy_lookup_constrains(
			self->hierarchy, var_name_n);
		...
		if (Nil != invar_c)
			invar_sexp =
				ExprMgr_and(exprs, invar_sexp, EXPR(invar_c));
		if (Nil != next_c)
			trans_sexp =
				ExprMgr_and(exprs, trans_sexp, EXPR(next_c));

		/* add all assignments */
		if (Nil != init_a) {
			nusmv_yylineno = init_a->lineno;
			init_sexp = ExprMgr_and(exprs, init_sexp,
						EXPR(find_node(nodemgr, EQDEF,
							       var_name_i,
							       init_a)));
		}
		...
		if (Nil != invar_a) {
			...
			invar_sexp =
				ExprMgr_and(exprs, invar_sexp,
					    EXPR(new_node(nodemgr, EQDEF,
							  var_name, invar_a)));
		}
		...
		/* simplification */
		init_sexp = sexp_fsm_simplify_expr(self, simp_hash, init_sexp,
						   INIT);
		invar_sexp = sexp_fsm_simplify_expr(self, simp_hash, invar_sexp,
						    INVAR);
		trans_sexp = sexp_fsm_simplify_expr(self, simp_hash, trans_sexp,
						    TRANS);

		/* inserts the var fsm inside the hash table */
		var_fsm =
			var_fsm_create(self, init_sexp, invar_sexp, trans_sexp);
				       ...
				       Expr_ptr expr, const int group)
{
	...
	if ((expr == EXPR(NULL)) ||
	    simplifier_hash_query_expr(hash, expr, group)) {
		result = ExprMgr_true(exprs);
	} else {
		switch (node_get_type(NODE_PTR(expr))) {
		case AND: {
			Expr_ptr left = sexp_fsm_simplify_expr(
				self, hash, car(NODE_PTR(expr)), group);
			Expr_ptr right = sexp_fsm_simplify_expr(
				self, hash, cdr(NODE_PTR(expr)), group);
					      ...
					      VarFsm_ptr fsm2)
{
	...
	if (Nil == fsm1 && Nil == fsm2) {
		...
		prod_init = NODE_PTR(ExprMgr_and_nil(
			exprs, EXPR(var_fsm_get_init(self, fsm1)),
			EXPR(var_fsm_get_init(self, fsm2))));
		prod_invar = NODE_PTR(ExprMgr_and_nil(
			exprs, EXPR(var_fsm_get_invar(self, fsm1)),
			EXPR(var_fsm_get_invar(self, fsm2))));
		prod_next = NODE_PTR(ExprMgr_and_nil(
			exprs, EXPR(var_fsm_get_next(self, fsm1)),
