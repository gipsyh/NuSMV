			 ...
			 JusticeList_ptr justice, CompassionList_ptr compassion)
{
	...
	bdd_fsm_init(self, encoding, init, invar_states, invar_inputs, trans,
		     justice, compassion);
...
BddStates BddFsm_get_reachable_states(BddFsm_ptr self)
{
	...
	/* Otherwise compute them, if necessary */
		...
		if (CACHE_GET(reachable.diameter) > 0) {
			res = CACHE_GET(
				reachable.layers[CACHE_GET(reachable.diameter) -
						 1]);
		} else {
...
int BddFsm_get_distance_of_states(BddFsm_ptr self, BddStates states)
{
	...
	for (i = 0; i < diameter; ++i) {
		bdd_ptr Ri = (bdd_ptr)BddFsm_get_reachable_states_at_distance(
			self, i);
...
BddStates BddFsm_get_not_successor_states(BddFsm_ptr self)
{
	...
	if (CACHE_IS_EQUAL(not_successor_states, BDD_STATES(NULL))) {
		...
		bdd_ptr no_succ_constr =
			bdd_and(self->dd, not_succ, self->invar_states);
	...
	BddStatesInputsNexts constraints)
{
	...
	res = BDD_STATES_INPUTS(
		BddEnc_next_state_var_to_state_var(self->enc, tmp));
	bdd_free(self->dd, tmp);
					       ...
					       BddStates states)
{
	...
	result = BddTrans_get_backward_image_state_input(self->trans,
							 constr_trans);
	bdd_free(self->dd, constr_trans);
						     ...
						     BddFsm_dir dir)
{
	...
	if (dir == BDD_FSM_DIR_BWD) {
		...
		result = BddFsm_get_forward_image_states_inputs(self,
								all_states);
	}
	bdd_free(self->dd, all_states);
	return result;
					      ...
					      int max_seconds)
{
	...
	/* Get the cache */
	completed =
		BddFsm_get_cached_reachable_states(self, &layers, &diameter);
	...
	if (diameter > 0) {
		...
		if (completed) {
         ...
         last state */
			...
			/* Regen the list */
			for (i = 0; i < diameter; i++) {
				reachable_states_layers = cons(
					nodemgr, (node_ptr)bdd_dup(layers[i]),
			...
			if (diameter > 1) {
				...
				from_lower_bound = bdd_and(
					self->dd, reachable_states_bdd, tmp);
				bdd_free(self->dd, tmp);
			} else {
		...
		if (bdd_isnot_false(self->dd, reachable_states_bdd)) {
			reachable_states_layers =
				cons(nodemgr,
				     (node_ptr)bdd_dup(reachable_states_bdd),
				     reachable_states_layers);
	...
	while ((bdd_isnot_false(self->dd, from_lower_bound)) && (0 != k) &&
		...
		if (opt_verbose_level_gt(opts, 2)) {
			...
			Logger_log(
				...
				BddEnc_count_states_of_bdd(
					self->enc, reachable_states_bdd));
		}

		/* Get the forward image */
		img = BddFsm_get_forward_image(self,
					       BDD_STATES(from_lower_bound));
       ...
       old reachables with the new reachables. See also issue 4475 */
		from_lower_bound = bdd_and(self->dd, reachable_states_bdd,
    ...
    */
		...
		BddFsm_update_cached_reachable_states(
			self, reachable_states_layers, diameter, true);
	} else {
		/*
    ...
    */
		BddFsm_update_cached_reachable_states(
			self, reachable_states_layers, diameter, false);
	}
...
void BddFsm_check_machine(const BddFsm_ptr self)
{
	...
	if (!BddFsm_is_total(self)) {
		...
		vars = SymbTable_get_layers_sf_i_vars(
			self->symb_table,
			SymbTable_get_class_layer_names(self->symb_table,
							(const char *)NULL));
		BddEnc_print_bdd_begin(self->enc, vars, false);
		BddEnc_print_bdd(self->enc, ds, (VPFBEFNNV)NULL, outstream,
				 NULL);
		...
		if (CACHE_IS_EQUAL(reachable.computed, true) ||
			...
			if (!BddFsm_is_deadlock_free(self)) {
				bdd_ptr deadlockStates =
					BddFsm_get_deadlock_states(self);
				bdd_ptr ds = BddEnc_pick_one_state(
					self->enc, deadlockStates);
				...
				StreamMgr_print_output(
					streams, "A deadlock state is:\n");
				BddEnc_print_bdd(self->enc, ds, (VPFBEFNNV)NULL,
						 outstream, NULL);
			} else {
				StreamMgr_print_output(
					streams,
			...
			StreamMgr_print_output(
				streams,
				"NOTE: No-successor states could be non-reachable, so\n");
			StreamMgr_print_output(
				streams,
				"      the machine could still be deadlock-free.\n");
			StreamMgr_print_output(
				streams,
						     ...
						     bdd_ptr next_vars_cube)
{
	...
	/* invars */
	bdd_and_accumulate(self->dd, &(self->invar_states),
			   other->invar_states);
	...
	CompassionList_apply_synchronous_product(self->compassion,
						 other->compassion);
...
double BddFsm_count_transitions(const BddFsm_ptr self, BddStatesInputs bdd)
{
	...
	{ /* computes all pairs */
		...
		array_size =
			BddEnc_count_states_inputs_of_bdd(self->enc, mask_bdd);
		nusmv_assert(array_size >= 0);
		array = ALLOC(bdd_ptr, array_size);
		nusmv_assert(NULL != array);

		res = BddEnc_pick_all_terms_states_inputs(self->enc, mask_bdd,
							  array, array_size);
		nusmv_assert(!res);
	...
	{ /* loop over the pairs, accumulate the number of transitions */
		...
		for (j = 0; j < array_size; ++j) {
			bdd_ptr image = BddFsm_get_constrained_forward_image(
				self, one, array[j]);
			bdd_ptr mask_image =
				BddEnc_apply_state_frozen_vars_mask_bdd(
					self->enc, image);

			num += BddEnc_count_states_of_bdd(self->enc,
							  mask_image);
		    ...
		    FILE *outfile)
{
	...
	/* here we construct the input for BddEnc_dump_addarray_dot */
	{
		struct {
			...
			{ trans, "Trans",
			  BddFsm_get_monolithic_trans_bdd(fsm) },
			{ fair, "Fair", BddFsm_get_fair_states(fsm) },
			{ reachable, "Reachables",
			  BddFsm_get_reachable_states(fsm) },
		};
		int i, idx;
		...
		for (idx = 0, i = 0;
		     i < sizeof(info) / sizeof(info[0]) && idx < entries; ++i) {
			if (info[i].enabled) {
				labels[idx] = util_strsav(info[i].label);
				AddArray_set_n(addarray, idx,
					       bdd_to_add(dd, info[i].bdd));
				++idx;
			}
		}
		...
		if ((AddArray_ptr)NULL != aar_expr) {
			...
			const int digits =
				(int)log10(AddArray_get_size(aar_expr));
			...
			for (i = idx; i < entries; ++i) {
				...
				c = snprintf(oname, oname_len, oname_fmt,
					     str_constr, digits, i - idx);
				SNPRINTF_CHECK(c, oname_len);

				labels[i] = oname;
				AddArray_set_n(addarray, i,
					       add_dup(AddArray_get_n(
						       aar_expr, i - idx)));
			}
		}

		res = BddEnc_dump_addarray_dot(bdd_enc, addarray, labels,
					       outfile);
...
static void bdd_fsm_copy(const BddFsm_ptr self, BddFsm_ptr copy)
{
	...
	copy->compassion =
		COMPASSION_LIST(Object_copy(OBJECT(self->compassion)));
					 ...
					 BddStatesInputs si, BddFsm_dir dir)
{
	...
	if (opt_use_reachable_states(opts)) {
		bdd_ptr reachable_states_bdd =
			BddFsm_get_reachable_states(self);
	...
	while (bdd_isnot_false(self->dd, newY)) {
		...
		if (opt_verbose_level_gt(opts, 5)) {
			...
			Logger_log(
				...
				i,
				BddEnc_count_states_inputs_of_bdd(self->enc,
								  resY),
						    ...
						    BddFsm_dir dir)
{
	...
	while (res != old) {
		...
		/* See issue 4476 */
		_new = bdd_fsm_compute_EL_SI_subset_aux(
			self, BDD_STATES_INPUTS(res), subspace, dir);
				 ...
				 BddStatesInputs subspace, BddFsm_dir dir)
{
	...
	while (!FairnessListIterator_is_end(iter)) {
		...
		temp = bdd_fsm_EUorES_SI(self, subspace, constrained_state,
					 dir);
	...
	if ((dir == BDD_FSM_DIR_BWD &&
       ...
       there benchmarks?) */
		...
		if (dir == BDD_FSM_DIR_BWD) {
			...
			CACHE_SET_BDD(revfair_states_inputs,
				      fair_or_revfair_si);
		}
