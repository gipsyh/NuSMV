...
static void BddFsmTrans_print(const NuSMVEnv_ptr env,
...
void BddFsm_print_info(const BddFsm_ptr self, OStream_ptr file)
{
	...
	if ((init_bdd != NULL)) {
		OStream_printf(
			file, "BDD nodes representing init set of states: %d\n",
	...
	if ((state_bdd != NULL)) {
		OStream_printf(file,
			       "BDD nodes representing state constraints: %d\n",
	...
	if ((input_bdd != NULL)) {
		OStream_printf(file,
			       "BDD nodes representing input constraints: %d\n",
			       bdd_size(self->dd, input_bdd));
		bdd_free(self->dd, input_bdd);
	}

	BddTrans_print_short_info(BddFsm_get_trans(self),
				  OStream_get_stream(file));
}
					...
					OStream_ptr file)
{
	...
	OStream_printf(file, "reachable states: %g (2^%g) out of %g (2^%g)\n",
		       ...
		       log(search_space_cardinality) / log(2.0));

	/* one of these flags can be enabled, not both */
	nusmv_assert(!print_states || !print_formula);
	if (print_states) {
		BddEnc_print_set_of_states(self->enc, reachable, false,
					   print_defines, (VPFBEFNNV)NULL, file,
					   NULL);
	} else if (print_formula) {
		BoolEnc_ptr benc =
			BoolEncClient_get_bool_enc(BOOL_ENC_CLIENT(self->enc));
		...
		NodeList_ptr all_vars =
			SymbTable_get_layers_sf_vars(st, layer_names);
		...
		OStream_printf(file, "\nFORMULA = \n");
		BddEnc_print_bdd_wff(self->enc, reachable, scalar_vars, true,
				     false, 0, file);
...
void BddFsm_print_fair_states_info(const BddFsm_ptr self,
				   const boolean print_states, OStream_ptr file)
{
	bdd_ptr fair_states;
	bdd_ptr mask;
	double reached_cardinality;
	...
	reached_cardinality =
		BddEnc_count_states_of_bdd(self->enc, fair_states);
	search_space_cardinality = BddEnc_count_states_of_bdd(self->enc, mask);
	bdd_free(self->dd, mask);
	...
	OStream_ptr file)
{
	...
	StreamMgr_print_output(streams,
			       ...
			       fairtrans_cardinality,
			       log(fairtrans_cardinality) / log(2.0),
			       search_space_cardinality,
			       log(search_space_cardinality) / log(2.0));
	...
	if (BDD_FSM_TRANS_PRINTER_SILENT != format) {
		...
		BddFsm_trans_iterator_begin(self, fair_trans, &iter,
					    &trans_info);

		if (BDD_FSM_TRANS_PRINTER_SMV == format) {
			OStream_inc_indent_size(file);
			BddEnc_print_bdd_begin(self->enc, iter.sf_symb_list,
					       false);
		}

		BddFsmTrans_print(env, &iter, &trans_info, format,
				  BDD_FSM_TRANS_PRINTER_SECTION_HEADER, file);
		...
		while (true) {
			...
			if (!BddFsm_trans_iter_next(self, &iter, &trans_info,
						    &inner))
		...
		BddFsmTrans_print(env, &iter, &trans_info, format,
				  BDD_FSM_TRANS_PRINTER_SECTION_FOOTER, file);
					      ...
					      OStream_ptr file)
{
	...
	fairstates_cardinality =
		BddEnc_get_minterms_of_bdd(self->enc, fair_trans);
	search_space_cardinality = BddEnc_get_minterms_of_bdd(self->enc, mask);
	bdd_free(self->dd, mask);

	OStream_printf(file,
		       "Fair state-input pairs: %g (2^%g) out of %g (2^%g)\n",
		       fairstates_cardinality,
		       log(fairstates_cardinality) / log(2.0),
		       search_space_cardinality,
		       log(search_space_cardinality) / log(2.0));

	if (print_transitions) {
		BddEnc_print_set_of_state_input_pairs(self->enc, fair_trans,
						      false, (VPFBEFNNV)NULL,
						      file, NULL);
	}
				  ...
				  boolean print_defines, boolean formula)
{
	...
	BddFsm_print_reachable_states_info(
		BDD_FSM(NuSMVEnv_get_value(env, ENV_BDD_FSM)), verbose,
		print_defines, formula, stream);
	StreamMgr_print_output(streams, HEADER_SEPARATOR);
...
int BddFsm_print_fair_states(BddFsm_ptr self, const NuSMVEnv_ptr env,
			     const OStream_ptr outstream, const boolean verbose)
{
	const StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_output(streams, HEADER_SEPARATOR);
...
BddFsm_trans_printer_format_from_string(const char *format_str)
{
	...
	for (i = 0; i < (sizeof(bddfsm_trans_printer_formats) /
	     ...
	     ++i) {
		if (0 ==
		    strcmp(format_str, bddfsm_trans_printer_formats[i].name))
...
static void BddFsmTrans_print_dot(const NuSMVEnv_ptr env,
				  const struct BddFsmTransIterator *iter,
				  const struct BddFsmTrans *trans_info,
				  const enum BddFsmTransPrinterSection section,
				  OStream_ptr file, void *arg)
{
	...
	case BDD_FSM_TRANS_PRINTER_SECTION_BODY:
		OStream_printf(file, "%d -> %d\n", trans_info->source_id,
			       trans_info->dest_id);
		break;

	case BDD_FSM_TRANS_PRINTER_SECTION_FOOTER: { /* dump legends for states and
                                                  labels, to explain IDs */
		const char *lagend_header =
			"{ rank = sink;\n"
		...
		const char *state_prefix =
			"<TR><TD><FONT COLOR=\"blue\">%d:</FONT></TD><TD><FONT "
			"COLOR=\"black\">";
		...
		for (i = 0; i < VECTOR_SIZE(iter->id2bdd); ++i) {
			...
			node_ptr values = BddEnc_assign_symbols(
				iter->enc, bdd, iter->sf_symb_list, true, NULL);
			node_ptr l;
...
/* this is the printer for the CSV format */
static void BddFsmTrans_print_csv(const NuSMVEnv_ptr env,
				  const struct BddFsmTransIterator *iter,
				  const struct BddFsmTrans *trans_info,
				  const enum BddFsmTransPrinterSection section,
				  OStream_ptr file, void *arg)
{
	...
	const struct {
		...
		{ iter->sf_symb_list,
		  trans_info->source }, /* state and frozen */
		{ iter->i_symb_list, trans_info->source }, /* inputs */
		{ iter->n_symb_list,
		  trans_info->dest_next }, /* next and frozen */
	};

	switch (section) {
	case BDD_FSM_TRANS_PRINTER_SECTION_HEADER: { /* print headers: states, inputs,
                                                  next states, IDs */
		const int pairs =
			sizeof(symb_bdd_pairs) / sizeof(*symb_bdd_pairs);
		int i;
		for (i = 0; i < pairs; ++i) {
			ListIter_ptr iter;
			NODE_LIST_FOREACH(symb_bdd_pairs[i].list, iter)
			{
				node_ptr var = NodeList_get_elem_at(
					symb_bdd_pairs[i].list, iter);
				OStream_nprintf(file, wffprint, "\"%N\"%s", var,
						sep);
			}
		}
	...
	case BDD_FSM_TRANS_PRINTER_SECTION_BODY: { /* print currs, inputs, nexts */
		const int pairs =
			sizeof(symb_bdd_pairs) / sizeof(*symb_bdd_pairs);
		int i;
		for (i = 0; i < pairs; ++i) {
			node_ptr values = BddEnc_assign_symbols(
				trans_info->enc, symb_bdd_pairs[i].bdd,
				symb_bdd_pairs[i].list, false, NULL);
			node_ptr l;

			for (l = values; Nil != l; l = cdr(l)) {
				OStream_nprintf(file, wffprint, "\"%N\"%s",
						cdr(car(l)), sep);
			}
			free_list(nodemgr, values);
		}
		...
		OStream_printf(file, "%d%s%d", trans_info->source_id, sep,
			       trans_info->dest_id);
...
static void BddFsmTrans_print_smv(const NuSMVEnv_ptr env,
				  const struct BddFsmTransIterator *iter,
				  const struct BddFsmTrans *trans_info,
				  const enum BddFsmTransPrinterSection section,
				  OStream_ptr file, void *arg)
{
	...
	OStream_printf(
		file,
		"------- Trans: %4.f (Curr State ID: %d, Next State ID: %d) ------\n",
		iter->trans_counter, trans_info->source_id,
		trans_info->dest_id);
	/* prints the set of states... */
	BddEnc_print_bdd(iter->enc, trans_info->source, (VPFBEFNNV)NULL, file,
			 NULL);
	...
	BddEnc_print_bdd(iter->enc, trans_info->dest_next, (VPFBEFNNV)NULL,
			 file, NULL);
	BddEnc_print_bdd_end(iter->enc);
...
static void BddFsmTrans_print(const NuSMVEnv_ptr env,
			      ...
			      OStream_ptr file)
{
	void (*printer)(const NuSMVEnv_ptr env,
			const struct BddFsmTransIterator *iter,
			const struct BddFsmTrans *trans_info,
			const enum BddFsmTransPrinterSection section,
			OStream_ptr file, void *arg);
	...
	default: {
		...
		ErrorMgr_internal_error(
			errmgr, "Invalid value for format: %d\n", format);
	}
	}
...
static bdd_ptr
BddFsmMinterArrayIterator_get(const struct BddFsmMinterArrayIterator *self)
{
...
static int
BddFsmMinterArrayIterator_begin(struct BddFsmMinterArrayIterator *self,
				...
				bdd_ptr *first_bdd)
{
	...
	if (contains_inputs)
		res = BddEnc_pick_all_terms_states_inputs(enc, bdd, self->array,
							  self->array_size);
	else
		res = BddEnc_pick_all_terms_states(enc, bdd, self->array,
						   self->array_size);
...
static void
BddFsmMinterArrayIterator_end(struct BddFsmMinterArrayIterator *self,
			      BddEnc_ptr enc)
{
	int j;
...
/* returned bdd is NOT referenced */
static int
BddFsmMinterArrayIterator_next(struct BddFsmMinterArrayIterator *self,
			       bdd_ptr *bdd)
{
...
static int BddFsm_trans_iterator_get_bdd_id(struct BddFsmTransIterator *iter,
					    bdd_ptr bdd)
...
static void
BddFsm_trans_iterator_compute_dest_image(const BddFsm_ptr self,
					 struct BddFsmTransIterator *iter,
					 struct BddFsmTrans *trans)
{
	bdd_ptr bdd;
					...
					struct BddFsmTrans *trans)
{
	...
	trans->source_id =
		BddFsm_trans_iterator_get_bdd_id(iter, trans->source_curr);
	...
	{ /* prepare symbol lists */
		...
		{ /* next vars */
			...
			{
				node_ptr v = NodeList_get_elem_at(
					iter->sf_symb_list, list_iter);
				nusmv_assert(
					SymbTable_is_symbol_state_frozen_var(
						st, v));

				NodeList_append(iter->n_symb_list,
...
static enum st_retval bdd_fsm_trans_iter_free_bdd_num_cache(char *key,
							    char *val, char *dd)
{
	if (NULL != key) {
				      ...
				      struct BddFsmTransIterator *iter)
{
	...
	clear_assoc_and_free_entries_arg(
		iter->bdd2id, bdd_fsm_trans_iter_free_bdd_num_cache,
				  ...
				  boolean *inside_inner)
{
	...
	if (res) {
		...
		if (source != trans->source) {
			trans->enc = iter->enc;
			trans->source = bdd_dup(BddFsmMinterArrayIterator_get(
				&(iter->source_iter)));

			trans->source_curr = BddFsm_states_inputs_to_states(
				self, trans->source);
			trans->source_id = BddFsm_trans_iterator_get_bdd_id(
				iter, trans->source_curr);
		}

		trans->dest_curr = bdd_dup(next_dest);
		trans->dest_next = BddEnc_state_var_to_next_state_var(
			iter->enc, next_dest);
		trans->dest_id =
			BddFsm_trans_iterator_get_bdd_id(iter, next_dest);
       ...
       * the inner one */
			...
			BddFsm_trans_iterator_compute_dest_image(self, iter,
								 trans);
...
static void BddFsmTrans_deinit(struct BddFsmTrans *self)
{
	if (NULL != self->enc) {
		if (NULL != self->source) {
			bdd_free(BddEnc_get_dd_manager(self->enc),
				 self->source);
		}
