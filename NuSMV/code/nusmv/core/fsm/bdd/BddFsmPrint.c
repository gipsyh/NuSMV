...
void BddFsm_print_reachable_states_info(const BddFsm_ptr self,
  ...
  if (print_states) {
    ...
    OStream_printf(file, "\nFORMULA = \n");
    BddEnc_print_bdd_wff(self->enc, reachable, scalar_vars, true, false, 0,
                         file);
...
void BddFsm_print_fair_transitions_info(
  ...
  StreamMgr_print_output(
      streams, "Fair transitions: %g (2^%g) out of %g (2^%g)\n",
  ...
  if (BDD_FSM_TRANS_PRINTER_SILENT != format) {
    ...
    while (true) {
      ...
      BddFsmTrans_print(env, &iter, &trans_info, format,
                        BDD_FSM_TRANS_PRINTER_SECTION_BODY, file);
      ...
      if (iter.trans_counter > MAX_ELEMENTS_PRINTABLE) {
        StreamMgr_print_error(streams, "warning: stopped printing earlier\n");
        break;
      }
    }
...
int BddFsm_print_reachable_states(BddFsm_ptr self, NuSMVEnv_ptr env,
  ...
  BddFsm_print_reachable_states_info(
      BDD_FSM(NuSMVEnv_get_value(env, ENV_BDD_FSM)), verbose, print_defines,
      formula, stream);
  StreamMgr_print_output(streams, HEADER_SEPARATOR);
...
static void BddFsmTrans_print_dot(const NuSMVEnv_ptr env,
  ...
  case BDD_FSM_TRANS_PRINTER_SECTION_FOOTER: { /* dump legends for states and
    ...
    for (i = 0; i < VECTOR_SIZE(iter->id2bdd); ++i) {
      ...
      node_ptr values =
          BddEnc_assign_symbols(iter->enc, bdd, iter->sf_symb_list, true, NULL);
      node_ptr l;
...
static void BddFsmTrans_print_smv(const NuSMVEnv_ptr env,
  ...
  BddEnc_print_bdd(iter->enc, trans_info->dest_next, (VPFBEFNNV)NULL, file,
                   NULL);
  BddEnc_print_bdd_end(iter->enc);
...
BddFsmMinterArrayIterator_begin(struct BddFsmMinterArrayIterator *self,
  ...
  if (contains_inputs)
    ...
    res = BddEnc_pick_all_terms_states(enc, bdd, self->array, self->array_size);
...
static int BddFsm_trans_iterator_get_bdd_id(
    struct BddFsmTransIterator *iter,
    bdd_ptr bdd) { /* computes the source id if needed */
...
static int BddFsm_trans_iter_next(const BddFsm_ptr self,
  ...
  if (res) {
    ...
    if (source != trans->source) {
      trans->enc = iter->enc;
      trans->source =
          bdd_dup(BddFsmMinterArrayIterator_get(&(iter->source_iter)));
    ...
    *inside_inner = false;
    ...
    res = BddFsmMinterArrayIterator_next(&(iter->source_iter), &next_source);
