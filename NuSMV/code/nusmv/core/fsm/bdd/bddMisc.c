...
*/
#define BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_BWD_STRING "EL_bwd"
...
/*!
  ...
  \todo Missing description
*/
#define BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_FWD_STRING "EL_fwd"
...
Bdd_BddOregJusticeEmptinessBddAlgorithmType_from_string(const char *name) {
  ...
  if (strcmp(name, BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_BWD_STRING) ==
      0) {
    res = BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_BWD;
  } else if (strcmp(name,
                    BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_FWD_STRING) ==
             0) {
    res = BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_FWD;
  } else
    res = BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_INVALID;
...
void Bdd_print_available_BddOregJusticeEmptinessBddAlgorithms(FILE *file) {
  ...
  for (alg = (int)BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_MIN_VALID;
       alg <= (int)BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_MAX_VALID; alg++) {
    fprintf(file, "%s ",
...
boolean Bdd_elfwd_check_options(NuSMVEnv_ptr env, unsigned int which_options,
  ...
  if ((which_options & BDD_ELFWD_OPT_COUNTER_EXAMPLES) &&
    ...
    if (on_fail_print) {
      StreamMgr_print_error(
          streams, "Forward Emerson-Lei must be used with counterexamples "
...
void Bdd_elfwd_restore_options(NuSMVEnv_ptr env, unsigned int which_options,
  ...
  if (which_options & BDD_ELFWD_OPT_FORWARD_SEARCH) {
    if (saved_options->forward_search)
      set_forward_search(opts);
    else
      unset_forward_search(opts);
  }
  if (which_options & BDD_ELFWD_OPT_LTL_TABLEAU_FORWARD_SEARCH) {
    if (saved_options->ltl_tableau_forward_search) {
      set_ltl_tableau_forward_search(opts);
    } else
      unset_ltl_tableau_forward_search(opts);
  }
  if (which_options & BDD_ELFWD_OPT_USE_REACHABLE_STATES) {
    if (saved_options->use_reachable_states)
      set_use_reachable_states(opts);
    else
      unset_use_reachable_states(opts);
  }
  if (which_options & BDD_ELFWD_OPT_COUNTER_EXAMPLES) {
    if (saved_options->counter_examples)
