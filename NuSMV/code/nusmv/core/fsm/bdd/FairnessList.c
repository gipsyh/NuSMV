...
*/
#define FAIRNESS_LIST_ITERATOR(x) ((FairnessListIterator_ptr)x)
...
/*!
  ...
  \todo Missing description
*/
#define END_ITERATOR FAIRNESS_LIST_ITERATOR(Nil)
...
static void fairness_list_init(FairnessList_ptr self, DDMgr_ptr dd_manager);
...
void JusticeList_append_p(JusticeList_ptr self, BddStates p) {
  const NuSMVEnv_ptr env =
      EnvObject_get_environment(ENV_OBJECT(FAIRNESS_LIST(self)->dd));
  ...
  FAIRNESS_LIST(self)->first =
      cons(nodemgr, (node_ptr)_new, FAIRNESS_LIST(self)->first);
}
...
void CompassionList_append_p_q(CompassionList_ptr self, BddStates p,
  ...
  FAIRNESS_LIST(self)->first =
      cons(nodemgr, couple, FAIRNESS_LIST(self)->first);
  ...
  if (!FairnessListIterator_is_end(iter)) {
    ...
    default:
      ErrorMgr_internal_error(
          errmgr, "fairness_list_deinit_aux: unexpected  %d-type node.",
...
static node_ptr fairness_list_copy_aux(const FairnessList_ptr self,
  ...
  if (!FairnessListIterator_is_end(iter)) {
    ...
    case BDD:
      res = new_node(nodemgr, BDD, NODE_PTR(bdd_dup((bdd_ptr)car(iter2))), Nil);
      break;

    default:
      ErrorMgr_internal_error(
          errmgr, "fairness_list_copy_aux: unexpected  %d-type node.",
          node_get_type(iter2));
    }
