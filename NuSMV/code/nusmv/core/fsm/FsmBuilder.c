...
fsm_builder_clusterize_expr(FsmBuilder_ptr self, BddEnc_ptr enc, Expr_ptr expr);

static void fsm_builder_clusterize_expr_aux(const FsmBuilder_ptr self,
					    BddEnc_ptr enc,
					    ClusterList_ptr clusters,
					    Expr_ptr expr_trans,
					    boolean is_inside_and, hash_ptr h);

static JusticeList_ptr
fsm_builder_justice_sexp_to_bdd(FsmBuilder_ptr self, BddEnc_ptr enc,
				node_ptr justice_sexp_list);

static CompassionList_ptr
fsm_builder_compassion_sexp_to_bdd(FsmBuilder_ptr self, BddEnc_ptr enc,
...
static BddFsm_ptr fsm_builder_lookup_bdd_fsm(
	const FsmBuilder_ptr self, const SexpFsm_ptr sexp_fsm, const Set_t vars,
	TransType trans_type, BddVarSet_ptr state_vars_cube,
	BddVarSet_ptr input_vars_cube, BddVarSet_ptr next_state_vars_cube);

#if __BDDENC_LAZY_COMMIT_LAYER__
static void fsm_builder_check_bddenc(const FsmBuilder_ptr self, BddEnc_ptr enc,
				     Set_t vars);
#endif

static void fsm_builder_insert_bdd_fsm(const FsmBuilder_ptr self,
				       const SexpFsm_ptr sexp_fsm,
				       const Set_t vars, TransType trans_type,
				       BddVarSet_ptr state_cube,
				       BddVarSet_ptr input_cube,
				       BddVarSet_ptr next_cube,
...
fsm_builder_lookup_bool_sexp_fsm(const FsmBuilder_ptr self,
				 const FlatHierarchy_ptr fh, const Set_t vars);
...
static Expr_ptr fsm_builder_compute_scalar_fsm_id(const NuSMVEnv_ptr env,
						  const SexpFsm_ptr sexp);

static Expr_ptr fsm_builder_compute_hierarchy_id(const NuSMVEnv_ptr env,
						 const FlatHierarchy_ptr fh);
...
BoolSexpFsm_ptr FsmBuilder_create_boolean_sexp_fsm(
	...
	const Set_t vars, BddEnc_ptr bdd_enc, /* cannot be NULL */
				     ...
				     const TransType trans_type)
{
	...
	bddfsm = FsmBuilder_create_bdd_fsm_of_vars(self, sexp_fsm, trans_type,
						   enc, state_vars_cube,
						   input_vars_cube,
					     ...
					     BddVarSet_ptr next_state_vars_cube)
{
	...
	{ /* here the trans is constructed out of the vars fsm, to make
       it ordered wrt the obtained ordered vars set */
		...
		sorted_vars = fsm_builder_order_vars_list(
			self, enc, SexpFsm_get_vars_list(sexp_fsm));

		trans_expr = SexpFsm_get_trans(sexp_fsm);
		printf("===================== expr = ");
		debug_print_node(EnvObject_get_environment(ENV_OBJECT(enc)),
				 trans_expr);
		SET_FOREACH(sorted_vars, iter)
		{
			Expr_ptr vtrans = SexpFsm_get_var_trans(
				sexp_fsm,
		...
		{
			Set_t deps =
				Formula_GetDependencies(st, trans_expr, Nil);

			if (fsm_builder_set_contains_infinite_variables(st,
									deps)) {
				Set_ReleaseSet(deps);
				ErrorMgr_rpterr(
					errmgr,
					"Impossible to build a BDD FSM"
					" with infinite precision variables");
			}
			Set_ReleaseSet(deps);
		}
		...
		trans = BddTrans_create(dd_manager, clusters,
					(bdd_ptr)state_vars_cube,
					(bdd_ptr)input_vars_cube,
					(bdd_ptr)next_state_vars_cube,
					trans_type, cluster_options);

		ClusterList_destroy(clusters);
		ClusterOptions_destroy(
			cluster_options); /* this is no longer needed */
	}

	/* ---------------------------------------------------------------------- */
	/* Bdd Fsm construction                                                   */
	/* ---------------------------------------------------------------------- */
	justice = fsm_builder_justice_sexp_to_bdd(
		self, enc, SexpFsm_get_justice(sexp_fsm));

	compassion = fsm_builder_compassion_sexp_to_bdd(
		self, enc, SexpFsm_get_compassion(sexp_fsm));
	...
	bddfsm = BddFsm_create(enc, BDD_STATES(init_bdd),
			       BDD_INVAR_STATES(invar_bdd),
			       BDD_INVAR_INPUTS(input_bdd), trans, justice,
			       compassion);
...
#if SHOW_BUG_ABOUT_4870
	fsm_builder_insert_bdd_fsm(self, sexp_fsm, SexpFsm_get_vars(sexp_fsm),
				   trans_type, state_vars_cube, input_vars_cube,
...
fsm_builder_clusterize_expr(FsmBuilder_ptr self, BddEnc_ptr enc, Expr_ptr expr)
{
	...
	clear_assoc_and_free_entries_arg(h, hash_bdd_key_free,
					 (char *)dd_manager);
					    ...
					    boolean is_inside_and, hash_ptr h)
{
	...
	if (node != Nil) {
		...
		case AND:
			fsm_builder_clusterize_expr_aux(self, enc, clusters,
							car(node), true, h);
			fsm_builder_clusterize_expr_aux(self, enc, clusters,
							cdr(node), true, h);

			if (!is_inside_and &&
			    (ClusterList_length(clusters) == 0)) {
           ...
           the call is over). Adds a single true cluster */
				...
				if (opt_verbose_level_gt(opts, 2)) {
					...
					Logger_log(
						logger,
						"FsmBuilder: created cluster for expression: True\n");
				}
			}
		...
		case EQUAL: {
			...
			if ((SYMB_TYPE_UNSIGNED_WORD ==
             ...
             check for absence of FAILURE nodes */
					bdd_ptr lhsbddi = add_to_bdd(
						dd_manager,
						AddArray_get_n(lhs, i));
					bdd_ptr rhsbddi = add_to_bdd(
						dd_manager,
						AddArray_get_n(rhs, i));
					bdd_ptr ci = bdd_iff(dd_manager,
							     lhsbddi, rhsbddi);
					...
					if (!(bdd_is_true(dd_manager, ci))) {
						...
						if (opt_verbose_level_gt(opts,
							...
							Logger_log(
								logger,
								"FsmBuilder: created cluster for word expression [%d:%d] : ",
								i, i);
							if (opt_verbose_level_gt(
								    opts, 5)) {
								Logger_nlog(
									logger,
									wffprint,
									"FsmBuilder: expression : %N[%d:%d]",
									expr_trans,
									i, i);
							}
							Logger_log(logger,
...
#endif
				if (!(bdd_is_true(dd_manager, tmp) &&
				      is_inside_and)) {
					Cluster_ptr cluster =
						Cluster_create(dd_manager);
					Cluster_set_trans(cluster, dd_manager,
							  tmp);
					ClusterList_append_cluster(clusters,
								   cluster);
			   ...
			   const NodeList_ptr list)
{
	...
	while (!ListIter_is_end(iter)) {
		char *name =
			sprint_node(printer, NodeList_get_elem_at(list, iter));
		int l = strlen(name) + 1;
		if (len + l > 70) {
			fprintf(fout, "\n");
			len = 0;
					 ...
					 const NodeList_ptr vars)
{
	...
	if (opt_trans_order_file(opts)) {
		...
		if (fl == (FILE *)NULL) {
			...
			ErrorMgr_error_file_not_found(
				errmgr, get_trans_order_file(opts));
		}
		...
		FAIL(errmgr)
		{
			...
			ErrorMgr_nusmv_exit(errmgr,
					    1); /* re-throws the exception */
		}

		fclose(fl);

		res = Compile_make_sorted_vars_list_from_order(
			BaseEnc_get_symb_table(BASE_ENC(enc)), vars,
			ParserIdList_get_id_list(parser));
...
static Expr_ptr fsm_builder_remove_dupl(FsmBuilder_ptr self, Expr_ptr expr)
{
	...
	case AND: {
		Expr_ptr left =
			fsm_builder_remove_dupl(self, car(NODE_PTR(expr)));
		Expr_ptr right =
			fsm_builder_remove_dupl(self, cdr(NODE_PTR(expr)));
					       ...
					       const Set_t vars)
{
	...
	ASSOC_FOREACH(self->sexp_fsm_hash, aiter, &couple, NULL)
	{
		...
		node_ptr scalar_id =
			fsm_builder_compute_hierarchy_id(env, scalar);
		Set_t cone = (Set_t)cdr(couple);

		if ((fh_id == scalar_id) && Set_Equals(cone, vars)) {
			result = SEXP_FSM(
				find_assoc(self->sexp_fsm_hash, couple));
			_break = true;
			break;
		}
	}
	...
	if (SEXP_FSM(NULL) != result) {
		if (opt_verbose_level_ge(opts, 2)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger, "FsmBuilder: Create scalar fsm -> "
					...
					const SexpFsm_ptr sexp_fsm)
{
	...
	if (opt_verbose_level_ge(opts, 2)) {
		...
		Logger_log(
			logger,
			"FsmBuilder: Create scalar fsm -> Caching new FSM\n");
	}
				 ...
				 const FlatHierarchy_ptr fh, const Set_t vars)
{
	...
	ASSOC_FOREACH(self->bool_fsm_hash, aiter, &couple, NULL)
	{
		...
		if ((fh_id == scalar_id) && Set_Equals(cone, vars)) {
			result = BOOL_SEXP_FSM(
				find_assoc(self->bool_fsm_hash, couple));
			_break = true;
			break;
		}
	}
	...
	if (BOOL_SEXP_FSM(NULL) != result) {
		if (opt_verbose_level_ge(opts, 2)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger, "FsmBuilder: Create boolean fsm -> "
					   "Returning previously cached FSM\n");
		}
					     ...
					     const BoolSexpFsm_ptr sexp_fsm)
{
	...
	if (opt_verbose_level_ge(opts, 2)) {
		...
		Logger_log(
			logger,
			"FsmBuilder: Create boolean fsm -> Caching new FSM\n");
	}
	...
	BddVarSet_ptr input_vars_cube, BddVarSet_ptr next_state_vars_cube)
{
	...
	ASSOC_FOREACH(self->bdd_fsm_hash, aiter, &mem, NULL)
	{
		...
		if ((sexp_fsm_id == scalar_fsm_id) &&
			...
			tmp_fsm = BDD_FSM(
				find_assoc(self->bdd_fsm_hash, NODE_PTR(mem)));
			_break = true;
			break;
		}
	}
	...
	if (BDD_FSM(NULL) != tmp_fsm) {
		if (opt_verbose_level_ge(opts, 2)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger, "FsmBuilder: Create bdd fsm -> "
					   "Returning previously cached FSM\n");
		}
				       ...
				       const BddFsm_ptr bdd_fsm)
{
	...
	if ((BddFsmMemoize *)NULL == mem) {
		StreamMgr_print_error(
			streams,
			"Cannot allocate memory for BDD FSM memoization\n");
	...
	if (opt_verbose_level_ge(opts, 2)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "FsmBuilder: Create bdd fsm -> Caching new FSM\n");
	}
					    ...
					    const Set_t vars)
{
	...
	SET_FOREACH(vars, iter)
	{
		...
		if (SymbType_is_infinite_precision(type) ||
		    SymbType_is_wordarray(type) || SymbType_is_intarray(type)) {
			return true;
		}
	}
