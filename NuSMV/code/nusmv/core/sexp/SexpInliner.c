...
static void sexp_inliner_init(SexpInliner_ptr self, SymbTable_ptr st,
...
static array_t *sexp_inliner_extract_candidate_equals(
	const SexpInliner_ptr self, const Set_t equals, const Set_t imp_vars,
	const hash_ptr var2invar, Set_t *rem_equals);

static hash_ptr sexp_inliner_remove_loops(const SexpInliner_ptr self,
					  array_t *good_equals,
					  hash_ptr hash_invars, Set_t *good,
					  Set_t *rem);

static node_ptr sexp_inliner_substitute(SexpInliner_ptr self, node_ptr expr,
					hash_ptr var2expr, hash_ptr var2invar,
					boolean *changed);

static boolean sexp_inliner_force_equivalence(SexpInliner_ptr self,
					      node_ptr var, Expr_ptr expr);

static boolean sexp_inliner_force_invariant(SexpInliner_ptr self, node_ptr var,
					    Expr_ptr expr);

static boolean sexp_inliner_expr_is_var(const SexpInliner_ptr self,
					node_ptr expr);
...
static boolean sexp_inliner_is_expr_deterministic(const SexpInliner_ptr self,
						  node_ptr expr);
				      ...
				      Expr_ptr expr)
{
	...
	expr2 = sexp_inliner_move_time_to_leaves(self, expr,
...
boolean SexpInliner_force_equivalences(SexpInliner_ptr self, Set_t equivs)
{
	...
	SET_FOREACH(equivs, iter)
	{
		...
		nusmv_assert(EQUAL == node_get_type(eq) ||
			     IFF == node_get_type(eq) ||
...
boolean SexpInliner_force_invariants(SexpInliner_ptr self, Set_t invars)
{
	...
	SET_FOREACH(invars, iter)
	{
		...
		res |= SexpInliner_force_invariant(self, car(invar),
						   cdr(invar));
	}
				 ...
				 boolean *changed)
{
	...
	psi = sexp_inliner_move_time_to_leaves(self, expr,
					       EXPR_UNTIMED_CURRENT);
	...
	do {
		...
		psi = sexp_inliner_extract_equals_invars(self, &info, psi,
							 false);
		...
		{ /* 12. extract hash out of auto-extracted invars set */
			...
			SET_FOREACH(info.invars, invars_iter)
			{
             ...
             general with hand-made formulae) */
					insert_assoc(hash_invars, car(invar),
						     cdr(invar));
				} else {
		...
		good_equals = sexp_inliner_extract_candidate_equals(
			self, info.equals, info.vars, hash_invars, &rem_equals);

		/* 16. removes loops from good_equals, obtaining kep_equals and
       possibly growing rem_equals with equals causing loops */
		hash_equals = sexp_inliner_remove_loops(self, good_equals,
							hash_invars,
							&kept_equals,
							&rem_equals);
		...
		if (opt_verbose_level_gt(info.opts, 6)) {
			...
			SET_FOREACH(rem_equals, iter)
			{
				Logger_nlog(info.logger, info.wffprint, "%N",
					    (node_ptr)Set_GetMember(rem_equals,
								    iter));
		...
		_changed = false;
		psi = sexp_inliner_substitute(self, psi, hash_equals,
					      hash_invars, &_changed);
					...
					boolean *changed)
{
	...
	do {
		...
		psi = sexp_inliner_substitute(
			self, psi, hash_equals,
			(hash_ptr)NULL /* no extracted invars */, &_changed);
		...
		if (opt_verbose_level_gt(opts, 6)) {
			...
			Logger_log(logger,
				   "SexpInliner: Done inlining iteration...\n");
		}
...
Expr_ptr InlineRes_get_result(const InlineRes_ptr self)
{
	...
	return ExprMgr_and(exprs, InlineRes_get_inlined_expr(self),
			   ExprMgr_and(exprs,
				       InlineRes_get_equivalences_expr(self),
...
Expr_ptr InlineRes_get_result_unique(const InlineRes_ptr self)
{
	...
	for (iter = InlineRes_get_equivalences_expr(self);
		...
		if (AND != node_get_type(iter)) {
			...
			array_insert_last(Expr_ptr, arr, car(iter));
	}

	/* accumulates invariants */
	for (iter = InlineRes_get_invariant_expr(self); (node_ptr)NULL != iter;
	     iter = cdr(iter)) {
		if (AND != node_get_type(iter)) {
			array_insert_last(Expr_ptr, arr, iter);
			break;
...
Expr_ptr InlineRes_get_original_expr(const InlineRes_ptr self)
{
	...
	return ((node_ptr)NULL != self->orig) ? self->orig :
						ExprMgr_true(exprs);
}
...
Expr_ptr InlineRes_get_inlined_expr(const InlineRes_ptr self)
{
	...
	return ((node_ptr)NULL != self->inlined) ? self->inlined :
						   ExprMgr_true(exprs);
}
...
Expr_ptr InlineRes_get_equivalences_expr(const InlineRes_ptr self)
{
	...
	SET_FOREACH(self->equivs, iter)
	{
		res = ExprMgr_and_nil(exprs, res,
				      (Expr_ptr)Set_GetMember(self->equivs,
							      iter));
	}
	return res;
}
...
Expr_ptr InlineRes_get_invariant_expr(const InlineRes_ptr self)
{
	...
	{
		res = ExprMgr_and_nil(exprs, res,
				      (Expr_ptr)Set_GetMember(self->invars,
							      iter));
	}
	return res;
}
						 ...
						 node_ptr expr, int time)
{
	...
	case CONTEXT:
		ErrorMgr_internal_error(errmgr,
					"SexpInliner::mode_time_to_leaves: "
	...
	case ARRAY: {
		...
		if (ResolveSymbol_is_define(rs)) {
			...
			node_ptr body = SymbTable_get_define_flatten_body(
				self->st, symb);
			res = sexp_inliner_move_time_to_leaves(self, body,
							       time);
#else
			res = sexp_inliner_move_time_to_leaves(self, symb,
							       time);
#endif
		}
		...
		/* is it a formal param? */
			...
			node_ptr actual =
				SymbTable_get_flatten_actual_parameter(self->st,
								       symb);
			res = sexp_inliner_move_time_to_leaves(self, actual,
							       time);
		}
	...
	case NEXT:
		...
		if (time == EXPR_UNTIMED_CURRENT) {
			res = sexp_inliner_move_time_to_leaves(
				self, car(expr), EXPR_UNTIMED_NEXT);
		} else {
			/* here it is timed */
			nusmv_assert(time >= 0);
	...
	case ATTIME:
		...
		res = sexp_inliner_move_time_to_leaves(
			self, ExprMgr_attime_get_untimed(exprs, expr),
	...
	case ABU:
		nusmv_assert(Nil == cdr(expr) ||
			     TWODOTS == node_get_type(cdr(expr)));
	...
	default: /* all other uninteresting cases */
		res = ExprMgr_resolve(
			exprs, self->st, node_get_type(expr),
			sexp_inliner_move_time_to_leaves(self, car(expr), time),
			sexp_inliner_move_time_to_leaves(self, cdr(expr),
							 time));
	}

	insert_assoc(self->hash_timed2leaves, key,
		     res); /* memoizes the result */
	return res;
}
				   ...
				   node_ptr expr, boolean is_neg)
{
	...
	case NOT:
		res = ExprMgr_not(info->exprmgr,
				  sexp_inliner_extract_equals_invars(
					  self, info, car(expr), !is_neg));
		break;
		...
		if (!is_neg) { /* negated AND becomes OR which is useless for us */
			res = ExprMgr_and(
				...
				sexp_inliner_extract_equals_invars(
					self, info, car(expr), is_neg),
				sexp_inliner_extract_equals_invars(
					self, info, cdr(expr), is_neg));
		}
		/* otherwise just resolve */
		else
			res = ExprMgr_resolve(info->exprmgr, self->st,
					      node_get_type(expr), car(expr),
					      cdr(expr));
		break;

	case OR:
		if (is_neg) { /* negated OR becomes AND */
			res = ExprMgr_or(info->exprmgr,
					 sexp_inliner_extract_equals_invars(
						 self, info, car(expr), is_neg),
					 sexp_inliner_extract_equals_invars(
						 self, info, cdr(expr),
	...
	case NOTEQUAL:
		if (is_neg) {
			res = sexp_try_acquiring_equality(self, info, expr,
							  is_neg);
		}
		/* otherwise just resolve */
		else
	...
	case EQDEF:
		if (!is_neg) {
			if (NEXT != node_get_type(car(expr)) &&
				...
				const OptsHandler_ptr opts =
					OPTS_HANDLER(NuSMVEnv_get_value(
						ENV_OBJECT(self)->environment,
						ENV_OPTS_HANDLER));
           ...
           this case keep the formula */
				...
				if ((node_ptr)NULL != hit) {
					res = sexp_inliner_assign_to_setin(
						self, expr);
					break;
				}
           ...
           this case keep the formula */
				...
				if (!sexp_inliner_is_expr_deterministic(
					    self, cdr(expr))) {
					res = sexp_inliner_assign_to_setin(
						self, expr);
					break;
				...
				if (opt_verbose_level_gt(opts, 6)) {
					Logger_nlog(
						info->logger, info->wffprint,
						"SexpInliner: acquiring invar '%N'\n",
						expr);
				}

				info->invars = Set_AddMember(
					info->invars, (Set_Element_t)expr);
	...
	case NEXT:
       ...
       next(v) <-> neg ? F:T */
		if (SymbTable_is_symbol_var(self->st, car(expr))) {
			nusmv_assert(SymbTable_is_symbol_bool_var(self->st,
								  car(expr)));
         ...
         expressions into expr or !expr thus breaking the expected input. */
			res = sexp_try_acquiring_equality(
				self, info,
				(is_neg ? find_node(info->nodemgr, IFF, expr,
						    ExprMgr_false(
							    info->exprmgr)) :
					  find_node(
						  info->nodemgr, IFF, expr,
						  ExprMgr_true(info->exprmgr))),
				is_neg);
		}
		/* otherwise just resolve */
		else
	...
	case ARRAY:
		...
		if (SymbTable_is_symbol_var(self->st, expr)) {
			nusmv_assert(
				SymbTable_is_symbol_bool_var(self->st, expr));
			/* [MR] Here we cannot use Expr_iff since it will simplify the
         expressions into expr or !expr thus breaking the expected input. */
			res = sexp_try_acquiring_equality(
				self, info,
...
static enum st_retval sexp_inliner_fill_good_equals(char *key, char *data,
						    char *arg)
{
	...
	const hash_ptr var2invar, Set_t *rem_equals)
{
	...
	good_equals =
		array_alloc(node_ptr, Set_GiveCardinality(equals) -
					      Set_GiveCardinality(*rem_equals));
	...
	if (opt_verbose_level_gt(opts, 6)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "In sexp_inliner_extract_candidate_equals:\n");
					  ...
					  Set_t *rem)
{
	...
	for (i = 0; i < array_n(good_equals); ++i) {
		...
		if ((node_ptr)NULL != coli) {
           ...
           the following equals if loops are detected */
				...
				node_ptr colj =
					array_fetch(node_ptr, good_equals, j);
             ...
             may be introducing loops */
					tj_inlined = sexp_inliner_substitute(
						self, tj, hash_equals,
						hash_invars, (boolean *)NULL);

					/* now checks if a loop has been introduced */
					deps = Formula_GetDependenciesByType(
						self->st, tj_inlined, Nil,
						VFT_CNIF,
						true /*preserve time*/);
               ...
               corresponding entry within the array */
						*rem = Set_AddMember(
						...
						array_insert(node_ptr,
							     good_equals, j,
							     (node_ptr)NULL);
					} else { /* inlines tj */
						node_ptr new_colj = new_node(
							nodemgr, COLON,
							find_node(
								nodemgr,
								node_get_type(car(
	...
	for (i = array_n(good_equals) - 1; i >= 0; --i) {
		...
		if ((node_ptr)NULL != col) {
			...
			if (i < array_n(good_equals) - 1) {
				...
				t = sexp_inliner_substitute(self, t,
							    hash_equals,
							    hash_invars,
							    &tchanged);
				if (tchanged)
					e = find_node(nodemgr, node_get_type(e),
						      v, t);
			}
	...
	if (opt_verbose_level_gt(opts, 7)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "Removing loops, after re-traversing bottom-up:\n");
		sexp_inliner_print_equality_array(env, good_equals, errstream);
	}
			    ...
			    node_ptr equal, boolean is_neg)
{
	...
	/*     [MR??]: an equality among constants and thus we do not need to even
     proceed and [MR??]: we can simplify the expression if possible w.r.t. the
     values [MR??]: either to TRUE, FALSE or leave as it is if we cannot perfrm
     the test. */
	...
	if (Nil != var) {
		...
		var_type =
			TypeChecker_get_expression_type(type_checker, var, Nil);

		if (SymbType_is_integer(var_type) ||
		    SymbType_is_pure_int_enum(var_type)) {
			SymbType_ptr expr_type;

			expr_type = TypeChecker_get_expression_type(
				type_checker, expr, Nil);

			if (SymbType_is_real(expr_type) ||
			    SymbType_is_continuous(expr_type)) {
				if (Nil != left && Nil != right &&
				    left != right) {
					node_ptr tmp;
	...
	if (Nil != var && Nil != expr &&
       ...
       this case keep the formula */
		...
		info->equals = Set_AddMember(
			info->equals, (Set_Element_t)find_node(
					      info->nodemgr, type, var, expr));
		...
		res = is_neg ? ExprMgr_false(info->exprmgr) :
			       ExprMgr_true(info->exprmgr);
	} else
		res = equal; /* not a variable equality */
					...
					node_ptr expr)
{
	...
	if (NEXT == node_get_type(expr) || SMALLINIT == node_get_type(expr) ||
	    ATTIME == node_get_type(expr))
		symb = car(expr);
	else
		symb = expr;

	if (BIT == node_get_type(symb) || DOT == node_get_type(symb) ||
	    ATOM == node_get_type(symb) || ARRAY == node_get_type(symb)) {
					...
					boolean *changed)
{
	...
	if (res != (node_ptr)NULL) {
		...
		if (res_in != expr) {
			res_in = sexp_inliner_substitute(self, res_in, var2expr,
							 var2invar, changed);
		}

		if ((boolean *)NULL == changed || *changed) {
			insert_assoc(
				self->hash_subst, expr,
		...
		if ((hash_ptr)NULL != var2invar) {
			...
			if ((node_ptr)NULL != hit) {
				...
				StreamMgr_print_output(streams,
						       "Substituting v2i \"");
				StreamMgr_nprint_output(streams, wffprint, "%N",
							expr);
				StreamMgr_print_output(streams, "\" with \"");
				StreamMgr_nprint_output(streams, wffprint, "%N",
							hit);
       ...
       auto-extracted, then user-provided) */
		if ((hash_ptr)NULL != var2invar) {
			...
			if ((node_ptr)NULL != hit) {
				...
				hit = sexp_inliner_move_time_to_leaves(
					self, hit, EXPR_UNTIMED_CURRENT);
				return hit;
			}
		}
	...
	case ATTIME: { /* attime was moved to leaves */
       ...
       auto-extracted, then user-provided) */
		...
		if ((node_ptr)NULL != hit) {
			...
			error_unreachable_code();
			/* [MRRC]: should be returned assuring at-time is moved to leaves */
			return ExprMgr_attime(exprs, hit,
					      ExprMgr_attime_get_time(exprs,
								      expr),
					      symb_table);
		}
		/* no hit in the invariants */
	...
	case EQDEF:
		...
		res = ExprMgr_resolve(
			exprs, self->st, node_get_type(expr), car(expr),
			sexp_inliner_substitute(self, cdr(expr), var2expr,
						var2invar, changed));
		break;
	...
	if (sexp_inliner_is_expr_deterministic(self, expr)) {
		...
		if (!Set_IsMember(deps, var)) {
			...
			if (opt_verbose_level_gt(opts, 6)) {
				...
				Logger_log(
					logger,
					"SexpInliner: accepted user-provided equivalence '");
					    ...
					    Expr_ptr expr)
{
	...
	if (sexp_inliner_is_expr_deterministic(self, expr)) {
		Set_t deps = Formula_GetDependenciesByType(
			self->st, expr, Nil, VFT_CNIF, true /*preserve time*/);
		if (!Set_IsMember(deps, var)) {
			...
			if (expr != old_invar) {
				...
				self->invars = Set_AddMember(
					self->invars,
					(Set_Element_t)ExprMgr_equal(
						exprs, var, expr, symb_table));
				res = true;
				...
				if (opt_verbose_level_gt(opts, 6)) {
					...
					Logger_log(
						logger,
						"SexpInliner: accepted invariant '");
...
static int sexp_inliner_expr_ptr_compare(const void *c1, const void *c2)
{
	Expr_ptr a = *((Expr_ptr *)c1);
	Expr_ptr b = *((Expr_ptr *)c2);
	if (a < b)
		return -1;
	if (a == b)
		return 0;
						  ...
						  node_ptr expr)
{
	...
	case IFTHENELSE:
		...
		is_deterministic = (sexp_inliner_is_expr_deterministic(
					    self, cdr(car(expr))) /*then*/
				    && sexp_inliner_is_expr_deterministic(
					       self, cdr(expr)) /*else*/);
					     ...
					     Expr_ptr assign)
{
	...
	if (SMALLINIT == node_get_type(car(assign))) {
		/* gets rid of init */
		return ExprMgr_setin(exprs, caar(assign), cdr(assign),
				     symb_table);
	}
