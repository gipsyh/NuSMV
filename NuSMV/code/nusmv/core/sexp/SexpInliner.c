...
boolean SexpInliner_force_invariants(SexpInliner_ptr self, Set_t invars) {
  ...
  SET_FOREACH(invars, iter) {
    ...
    nusmv_assert(EQUAL == node_get_type(invar) || IFF == node_get_type(invar) ||
...
InlineRes_ptr SexpInliner_inline(SexpInliner_ptr self, Expr_ptr expr,
  ...
  do {
    ...
    { /* 12. extract hash out of auto-extracted invars set */
      ...
      SET_FOREACH(info.invars, invars_iter) {
        node_ptr invar = (node_ptr)Set_GetMember(info.invars, invars_iter);
    ...
    _changed = false;
    psi =
        sexp_inliner_substitute(self, psi, hash_equals, hash_invars, &_changed);
  ...
  { /* constructs the result */
    ...
    res->invars = Set_Union(Set_Union(res->invars, info.invars), self->invars);
...
Expr_ptr InlineRes_get_result_unique(const InlineRes_ptr self) {
  ...
  for (iter = InlineRes_get_equivalences_expr(self); (node_ptr)NULL != iter;
       iter = cdr(iter)) {
...
static node_ptr sexp_inliner_move_time_to_leaves(const SexpInliner_ptr self,
  ...
  case NEXT:
    ...
    if (time == EXPR_UNTIMED_CURRENT) {
      res =
          sexp_inliner_move_time_to_leaves(self, car(expr), EXPR_UNTIMED_NEXT);
    } else {
      /* here it is timed */
      nusmv_assert(time >= 0);
  ...
  case ABU:
    ...
    res = ExprMgr_resolve(
        ...
        sexp_inliner_move_time_to_leaves(self, car(expr), time), cdr(expr));
    break;
...
sexp_inliner_extract_equals_invars(const SexpInliner_ptr self,
  ...
  switch (node_get_type(expr)) {
  case CONTEXT:
    ErrorMgr_internal_error(info->errmgr, "SexpInliner::auto_learn_equals: "
                                          "CONTEXT in expression "
                                          " assumed to be already flattened");

  case NOT:
    res = ExprMgr_not(info->exprmgr, sexp_inliner_extract_equals_invars(
    ...
    if (SymbTable_is_symbol_var(self->st, car(expr))) {
      ...
      res = sexp_try_acquiring_equality(
          ...
          (is_neg ? find_node(info->nodemgr, IFF, expr,
                              ExprMgr_false(info->exprmgr))
                  : find_node(info->nodemgr, IFF, expr,
...
static array_t *sexp_inliner_extract_candidate_equals(
  ...
  SET_FOREACH(equals, iter) {
    ...
    nusmv_assert(EQUAL == node_get_type(equal) || IFF == node_get_type(equal) ||
    ...
    /* skip init(var) := ... */
    if (EQDEF == node_get_type(equal) && SMALLINIT == node_get_type(var))
      continue;
    ...
    deps = Formula_GetDependenciesByType(self->st, expr, Nil, VFT_CNIF,
                                         true /* preserve time */);
    ideps = Set_Copy(deps);
      ...
      *rem_equals = Set_AddMember(*rem_equals, (Set_Element_t)equal);
      ...
      if ((node_ptr)NULL != prev_expr) {
        ...
        if (prev_deps > expr_deps) {
          ...
          *rem_equals =
              Set_AddMember(*rem_equals, (Set_Element_t)car(prev_expr));
        ...
        insert_assoc(
            hash, var,
            new_node(nodemgr, COLON, equal, PTR_FROM_INT(node_ptr, expr_deps)));
#ifdef IN_MOD
  ...
  good_equals = array_alloc(node_ptr, Set_GiveCardinality(equals) -
    ...
    if ((node_ptr)NULL != coli) {
      ...
      for (j = i + 1; j < array_n(good_equals); ++j) {
        ...
        if ((node_ptr)NULL != colj) {
          ...
          tj_inlined = sexp_inliner_substitute(self, tj, hash_equals,
                                               hash_invars, (boolean *)NULL);

          /* now checks if a loop has been introduced */
          deps = Formula_GetDependenciesByType(
              self->st, tj_inlined, Nil, VFT_CNIF, true /*preserve time*/);
          if (Set_IsMember(deps, vj)) {
            ...
            *rem = Set_AddMember(*rem, (Set_Element_t)car(colj));
            ...
            node_ptr new_colj = new_node(
                nodemgr, COLON,
                find_node(nodemgr, node_get_type(car(colj)), vj, tj_inlined),
                cdr(colj));
...
sexp_try_acquiring_equality(const SexpInliner_ptr self,
  ...
  if (Nil != var) {
    ...
    var_type = TypeChecker_get_expression_type(type_checker, var, Nil);

    if (SymbType_is_integer(var_type) || SymbType_is_pure_int_enum(var_type)) {
      SymbType_ptr expr_type;

      expr_type = TypeChecker_get_expression_type(type_checker, expr, Nil);
...
static node_ptr sexp_inliner_substitute(SexpInliner_ptr self, node_ptr expr,
  ...
  if (res != (node_ptr)NULL) {
    ...
    if (res_in != expr) {
      res_in =
          sexp_inliner_substitute(self, res_in, var2expr, var2invar, changed);
    }
  ...
  case CONTEXT:
    ErrorMgr_internal_error(
        errmgr, "SexpInliner::substitute: CONTEXT in expression assumed"
                " to be already flattened");
  ...
  case NEXT: { /* next was moved to leaves */
    ...
    nusmv_assert(
        ...
        (BIT == node_get_type(name)) || (ARRAY == node_get_type(name)));
  ...
  case AND:
    res = ExprMgr_and(
        exprs,
        sexp_inliner_substitute(self, car(expr), var2expr, var2invar, changed),
        sexp_inliner_substitute(self, cdr(expr), var2expr, var2invar, changed));
    break;
...
static boolean sexp_inliner_force_equivalence(SexpInliner_ptr self,
  ...
  if (sexp_inliner_is_expr_deterministic(self, expr)) {
    Set_t deps = Formula_GetDependenciesByType(self->st, expr, Nil, VFT_CNIF,
                                               true /* preserve time */);
    ...
    if (!Set_IsMember(deps, var)) {
      ...
      if (expr != old_invar) {
        ...
        self->invars = Set_AddMember(
            self->invars,
            (Set_Element_t)ExprMgr_equal(exprs, var, expr, symb_table));
        res = true;
