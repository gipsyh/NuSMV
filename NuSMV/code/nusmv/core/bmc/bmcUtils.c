...
static be_ptr bmc_utils_costraint_from_string(BeEnc_ptr be_enc,
					      BddEnc_ptr bdd_enc,
					      const char *str,
...
int Bmc_Utils_ConvertLoopFromString(const char *strValue, Outcome *result)
{
	...
	if (strValue == NIL(char)) {
       ...
       corresponds to a reserved value:: */
		...
		res = OUTCOME_GENERIC_ERROR; /* bad string value */

	/* This implements the auto-check (to simplify coding): */
	if (result == NULL) {
		nusmv_assert(res == OUTCOME_SUCCESS);
	} else
		*result = res;
...
int Bmc_Utils_GetNoLoopback()
{
	return BMC_NO_LOOP;
}

int Bmc_Utils_GetAllLoopbacks()
{
	return BMC_ALL_LOOPS;
}
...
int Bmc_Utils_RelLoop2AbsLoop(const int upov_loop, const int k)
{
	if ((Bmc_Utils_IsNoLoopback(upov_loop)) ||
	    (Bmc_Utils_IsAllLoopbacks(upov_loop)) || (upov_loop >= 0)) {
		return upov_loop;
	} else
		return k + upov_loop;
}
...
Outcome Bmc_Utils_Check_k_l(const int k, const int l)
{
	...
	if ((k >= 0) && /* k has to be non-negative in all cases */
	    (Bmc_Utils_IsNoLoopback(l) /* the no-loop case */
	     ...
	     || ((l >= 0) &&
		 (l < k)) /* the single-loop case with the new semantics */
	     ))
		ret = OUTCOME_SUCCESS;
				      ...
				      char *filename_expanded, size_t buf_len)
{
	...
	for (i = 0; i < table_len; ++i) {
		apply_string_macro_expansion(table_ptr + i, filename_expanded,
					     buf_len);
	...
	const int k, const char *trace_name, NodeList_ptr symbols)
{
	...
	Trace_ptr trace = Bmc_Utils_generate_cntexample(be_enc, solver, be_prob,
...
{
	return Bmc_fill_trace_from_cnf_model(
		be_enc, SatSolver_get_model(solver), k, trace);
}
					      ...
					      Prop_ptr invarprop)
{
	...
	return Bmc_Utils_get_vars_list_for_uniqueness_fsm(be_enc,
							  bool_sexp_fsm);
}
...

	{
		...
		NODE_LIST_FOREACH(vars, iter)
		{
			...
			if (SymbTable_is_symbol_state_var(st, sexp_var)) {
				...
				if (SymbTable_is_symbol_bool_var(st,
								 sexp_var)) {
					be_var = BeEnc_name_to_untimed(
						be_enc, sexp_var);
					status = lsNewEnd(crnt_state_be_vars,
							  (lsGeneric)be_var, 0);
...
be_ptr Bmc_Utils_simple_costraint_from_string(BeEnc_ptr be_enc,
					      ...
					      Expr_ptr *node_expr)
{
	return bmc_utils_costraint_from_string(be_enc, bdd_enc, str, false,
					       node_expr);
}
...
be_ptr Bmc_Utils_next_costraint_from_string(BeEnc_ptr be_enc,
					    ...
					    Expr_ptr *node_expr)
{
	return bmc_utils_costraint_from_string(be_enc, bdd_enc, str, true,
					       node_expr);
}
					      ...
					      Expr_ptr *node_expr)
{
	...
	int (*parse_fun)(NuSMVEnv_ptr, const char *, node_ptr *) =
		accept_next_expr ? Parser_ReadNextExprFromString :
				   Parser_ReadSimpExprFromString;
	int node_type = accept_next_expr ? NEXTWFF : SIMPWFF;

	if (0 == parse_fun(env, str, &parsed_expr) && Nil != parsed_expr &&
		...
		CATCH(errmgr)
		{
			...
			bool_constraints =
				Compile_detexpr2bexpr(bdd_enc, constraints);

			result = Bmc_Conv_Bexp2Be(be_enc, bool_constraints);
			if ((Expr_ptr *)NULL != node_expr)
				*node_expr = constraints;
		}
		FAIL(errmgr)
		{
			result = (be_ptr)NULL;
			if ((Expr_ptr *)NULL != node_expr)
				*node_expr = (Expr_ptr)NULL;
		}
	}
	...
	if ((be_ptr)NULL == result) {
		ErrorMgr_rpterr(
			errmgr,
			"Conversion from expression to BE (aka RBC) failed.");
	}
