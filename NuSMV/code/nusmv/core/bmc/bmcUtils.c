...
void Bmc_Utils_ConvertLoopFromInteger(const int iLoopback, char *szLoopback,
  ...
  if (Bmc_Utils_IsAllLoopbacks(iLoopback)) {
    iCheck = snprintf(szLoopback, bufsize, "%s", BMC_ALL_LOOPS_USERSIDE_SYMBOL);
...
Trace_ptr Bmc_Utils_generate_and_print_cntexample(
  ...
  Trace_ptr trace = Bmc_Utils_generate_cntexample(be_enc, solver, be_prob, k,
                                                  trace_name, symbols);
...
Trace_ptr Bmc_Utils_fill_cntexample(BeEnc_ptr be_enc, SatSolver_ptr solver,
  ...
  return Bmc_fill_trace_from_cnf_model(be_enc, SatSolver_get_model(solver), k,
                                       trace);
}
...
be_ptr Bmc_Utils_apply_inlining(Be_Manager_ptr be_mgr, be_ptr f) {
  ...
  return Be_apply_inlining(
      be_mgr, f, (!opt_rbc_inlining_lazy(opts) && opt_counter_examples(opts)));
}
...
be_ptr Bmc_Utils_next_costraint_from_string(BeEnc_ptr be_enc,
                                            ...
                                            Expr_ptr *node_expr) {
  return bmc_utils_costraint_from_string(be_enc, bdd_enc, str, true, node_expr);
}
  ...
  if (0 == parse_fun(env, str, &parsed_expr) && Nil != parsed_expr &&
    ...
    CATCH(errmgr) {
      if (!TypeChecker_is_expression_wellformed(
              BaseEnc_get_type_checker(BASE_ENC(bdd_enc)), constraints, Nil)) {
