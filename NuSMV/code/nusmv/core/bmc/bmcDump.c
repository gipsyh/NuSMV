			   ...
			   const char *dump_fname_template)
{
	...
	case BMC_DUMP_DIMACS:
		...
		if (Prop_get_type(prop) == Prop_Invar) {
			Bmc_Dump_DimacsInvarProblemFilename(
				be_enc, cnf, dumpFilenameExpanded);
		} else {
			Bmc_Dump_DimacsProblemFilename(be_enc, cnf,
						       dumpFilenameExpanded, k);
		}
		break;
	...
	case BMC_DUMP_DA_VINCI: {
		...
		Be_DumpDavinci(BeEnc_get_be_manager(be_enc),
			       Be_Cnf_GetOriginalProblem(cnf), davinci_file);
	...
	case BMC_DUMP_GDL: {
		...
		Be_DumpGdl(BeEnc_get_be_manager(be_enc),
			   Be_Cnf_GetOriginalProblem(cnf), gdl_file);
		if (opt_verbose_level_gt(opts, 1)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger, dumpFilenameExpanded);
			    ...
			    const int k, FILE *dimacsfile)
{
	...
	for (time = 0; time <= k; ++time) {
		...
		iter = BeEnc_get_first_untimed_var_index(
			be_enc, BE_VAR_TYPE_CURR | BE_VAR_TYPE_FROZEN |
					BE_VAR_TYPE_INPUT);
		while (BeEnc_is_var_index_valid(be_enc, iter)) {
         ...
         frozen variable at time > 0 (since such variables do not exist) */
			if (!((BeEnc_is_index_input_var(be_enc, iter) &&
			       time == k) ||
			      (BeEnc_is_index_frozen_var(be_enc, iter) &&
			       time > 0))) {
				int cnf_index;
				...
				if (cnf_index != 0) {
					...
					fprintf(dimacsfile,
						"c CNF variable %d => Time %d, Model Variable ",
						cnf_index, time);
					print_node(wffprint, dimacsfile,
						   BeEnc_index_to_name(be_enc,
								       iter));
					fprintf(dimacsfile, "\n");
				}
			}

			iter = BeEnc_get_next_var_index(
				be_enc, iter,
