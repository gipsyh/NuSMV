...
be_ptr sbmc_equal_vectors_formula(const BeEnc_ptr be_enc, lsList vars,
  ...
  lsForEachItem(vars, iterator, node_sv) {
    ...
    be_constraint =
        Be_And(be_mgr, be_constraint, Be_Iff(be_mgr, be_sv_i, be_sv_j));
  }
...
hash_ptr sbmc_init_LTL_info(const NuSMVEnv_ptr env, SymbLayer_ptr layer,
  ...
  while (lsLength(unprocessed_nodes) > 0) {
    ...
    case IFF:
      ...
      if (opt_do_virtual_unrolling)
        sbmc_node_info_set_past_depth(
            info, max(sbmc_node_info_get_past_depth(lsf_info),
    ...
    case NOT:
      ...
      if (opt_do_virtual_unrolling)
        sbmc_node_info_set_past_depth(info,
    ...
    case OP_FUTURE:
      ...
      if (sbmc_node_info_get_aux_F_node(lsf_info) == 0) {
        sbmc_node_info_set_aux_F_node(
            lsf_info, sbmc_1_fresh_state_var(env, layer, &new_var_index));
        lsNewBegin(state_vars_formula_aux,
                   (lsGeneric)sbmc_node_info_get_aux_F_node(lsf_info), LS_NH);
        sbmc_print_Fvarmap(env, errstream,
                           sbmc_node_info_get_aux_F_node(lsf_info), lsf);
      }
      break;
    ...
    case OP_GLOBAL:
      ...
      if (sbmc_node_info_get_aux_G_node(lsf_info) == 0) {
        ...
        sbmc_print_Gvarmap(env, errstream,
                           sbmc_node_info_get_aux_G_node(lsf_info), lsf);
      }
      break;
    ...
    case OP_ONCE:
      ...
      if (opt_do_virtual_unrolling)
        sbmc_node_info_set_past_depth(
            info, sbmc_node_info_get_past_depth(lsf_info) + 1);
      else
        sbmc_node_info_set_past_depth(info, 0);
    ...
    case UNTIL:
      ...
      if (sbmc_node_info_get_aux_F_node(rsf_info) == 0) {
        sbmc_node_info_set_aux_F_node(
            rsf_info, sbmc_1_fresh_state_var(env, layer, &new_var_index));
        lsNewBegin(state_vars_formula_aux,
                   (lsGeneric)sbmc_node_info_get_aux_F_node(rsf_info), LS_NH);
        sbmc_print_Fvarmap(env, errstream,
                           sbmc_node_info_get_aux_F_node(rsf_info), rsf);
      }
      break;
    ...
    case RELEASES:
      ...
      if (sbmc_node_info_get_aux_G_node(rsf_info) == 0) {
        ...
        sbmc_print_Gvarmap(env, errstream,
                           sbmc_node_info_get_aux_G_node(rsf_info), rsf);
      }
      break;
...
void sbmc_init_state_vector(const BeEnc_ptr be_enc, const node_ptr ltlspec,
  ...
  while (lsLength(unprocessed_nodes) > 0) {
    ...
    if ((lsFirstItem(unprocessed_nodes, (lsGeneric *)&f_node, LS_NH) !=
         LS_OK) ||
    ...
    if ((array_t *)NULL == past_array) {
      past_array =
          array_alloc(be_ptr, sbmc_node_info_get_past_depth(f_info) + 1);
      for (d = 0; d <= sbmc_node_info_get_past_depth(f_info); d++)
        array_insert(be_ptr, past_array, d, (be_ptr)NULL);
      array_insert(array_t *, sbmc_node_info_get_trans_bes(f_info), i_real,
                   past_array);
    }
    ...
    case ARRAY: {
      ...
      if (sbmc_node_info_get_trans_vars(f_info) != (array_t *)NULL) {
        ...
        node_p =
            array_fetch(node_ptr, sbmc_node_info_get_trans_vars(f_info), 0);
        ...
        if (LastState_var == (node_ptr)NULL ||
          ...
          be_LastState_i = BeEnc_name_to_timed(be_enc, LastState_var, i_real);
          be_result = Be_And(
              be_mgr, BeEnc_name_to_timed(be_enc, f_node, i_real),
              Be_Or(be_mgr, Be_Not(be_mgr, be_LastState_i), be_LoopExists));
          nusmv_assert((be_ptr)NULL != be_result);
          array_insert(be_ptr, past_array, 0, be_result);
      ...
      if (sbmc_node_info_get_trans_vars(f_info) != (array_t *)NULL) {
        ...
        for (d = 0; d <= sbmc_node_info_get_past_depth(f_info); d++) {
          if (LastState_var == (node_ptr)NULL ||
            ...
            if (opt_verbose_level_ge(opts, 6)) {
              Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
              Logger_log(logger, "  [!f]]_%s^%d := ![[f]]_%s^%d: ", i_str, d,
                         i_str, d);
              Be_DumpSexpr(be_mgr, be_result, errstream);
              Logger_log(logger, "\n");
            }
...
be_ptr sbmc_build_InLoop_i(const BeEnc_ptr be_enc,
  ...
  if (opt_verbose_level_ge(opts, 2))
    StreamMgr_print_error(streams,
                          "Created the constraint (InLoop_%d => !l_%d)",
                          i_model - 1, i_model);
  if (opt_verbose_level_ge(opts, 6)) {
    Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
    Logger_log(logger, ": ");
...
lsList sbmc_SimplePaths(const BeEnc_ptr be_enc,
  ...
  for (j = 0; j < k; j++) {
    ...
    be_equal_pd0_translation_vars = sbmc_equal_vectors_formula(
        be_enc, sbmc_state_vars_get_translation_vars_pd0(state_vars),
        sbmc_real_k(j), sbmc_real_k(k));

    be_equal_pdx_translation_vars = sbmc_equal_vectors_formula(
        be_enc, sbmc_state_vars_get_translation_vars_pdx(state_vars),
        sbmc_real_k(j), sbmc_real_k(k));

    be_equal_aux_translation_vars = sbmc_equal_vectors_formula(
        be_enc, sbmc_state_vars_get_translation_vars_aux(state_vars),
        sbmc_real_k(j), sbmc_real_k(k));
    ...
    be_constraint =
        Be_Or(be_mgr, be_constraint, Be_Not(be_mgr, be_equal_InLoops));
    ...
    be_c2 = Be_And(be_mgr, Be_And(be_mgr, be_InLoop_j, be_InLoop_k),
                   Be_Or(be_mgr, Be_Not(be_mgr, be_equal_pdx_translation_vars),
