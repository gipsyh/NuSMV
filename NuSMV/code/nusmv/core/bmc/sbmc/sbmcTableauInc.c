				  ...
				  const unsigned int i, const unsigned int j)
{
	...
	lsForEachItem(vars, iterator, node_sv)
	{
		...
		be_constraint = Be_And(be_mgr, be_constraint,
			    ...
			    const int opt_do_virtual_unrolling)
{
	...
	while (lsLength(unprocessed_nodes) > 0) {
		node_ptr node = (node_ptr)NULL, lsf = (node_ptr)NULL,
			 rsf = (node_ptr)NULL;
		...
		case FALSEEXP:
			...
			if (opt_force_state_vars) {
				...
				sbmc_allocate_trans_vars(env, info, layer,
							 state_vars_formula_pd0,
							 state_vars_formula_pdx,
							 &new_var_index);
		...
		case IFF:
			...
			if (opt_do_virtual_unrolling)
				sbmc_node_info_set_past_depth(
					info, max(sbmc_node_info_get_past_depth(
							  lsf_info),
		...
		case NOT:
			...
			if (opt_do_virtual_unrolling)
				sbmc_node_info_set_past_depth(
					info, sbmc_node_info_get_past_depth(
						      lsf_info));
			else
				sbmc_node_info_set_past_depth(info, 0);
		...
		case OP_FUTURE:
			...
			if (sbmc_node_info_get_aux_F_node(lsf_info) == 0) {
				sbmc_node_info_set_aux_F_node(
					lsf_info,
					sbmc_1_fresh_state_var(env, layer,
							       &new_var_index));
				lsNewBegin(
					state_vars_formula_aux,
					(lsGeneric)sbmc_node_info_get_aux_F_node(
						lsf_info),
					LS_NH);
				sbmc_print_Fvarmap(
					env, errstream,
		...
		case OP_GLOBAL:
			...
			if (sbmc_node_info_get_aux_G_node(lsf_info) == 0) {
				...
				lsNewBegin(
					state_vars_formula_aux,
					(lsGeneric)sbmc_node_info_get_aux_G_node(
						lsf_info),
					LS_NH);
				sbmc_print_Gvarmap(
					env, errstream,
		...
		case OP_NEXT:
         ...
         make sure it can be free i.e. force it to be translated via var */
			if (sbmc_node_info_get_trans_vars(lsf_info) == 0) {
				sbmc_allocate_trans_vars(env, lsf_info, layer,
							 state_vars_formula_pd0,
							 state_vars_formula_pdx,
							 &new_var_index);
				sbmc_print_varmap(env, errstream, lsf,
						  lsf_info);
			}
			break;
			    ...
			    const be_ptr be_LoopExists)
{
	...
	while (lsLength(unprocessed_nodes) > 0) {
		...
		if ((lsFirstItem(unprocessed_nodes, (lsGeneric *)&f_node,
				 LS_NH) != LS_OK) ||
		...
		/* Get past_array */
		{
			...
			while (array_n(bearray) <= i_real)
				array_insert(array_t *, bearray,
					     array_n(bearray), (array_t *)NULL);
		...
		if (sbmc_set_is_in(visit_cache, f_node)) {
			...
			if (lsDelBegin(unprocessed_nodes,
				       (lsGeneric *)&f_node) != LS_OK)
				ErrorMgr_internal_error(errmgr, sbmc_SNH_text,
							__FILE__, __LINE__);
			continue;
		}
		...
		if ((array_t *)NULL == past_array) {
			past_array = array_alloc(
				be_ptr,
				sbmc_node_info_get_past_depth(f_info) + 1);
			for (d = 0; d <= sbmc_node_info_get_past_depth(f_info);
			     d++)
				array_insert(be_ptr, past_array, d,
					     (be_ptr)NULL);
			array_insert(array_t *,
				     sbmc_node_info_get_trans_bes(f_info),
				     i_real, past_array);
		}

		for (d = 0; d <= sbmc_node_info_get_past_depth(f_info); d++)
			nusmv_assert(array_fetch(be_ptr, past_array, d) ==
				     (be_ptr)NULL);
		...
		case ARRAY: {
			...
			nusmv_assert(sbmc_node_info_get_past_depth(f_info) ==
				     0);

			if (sbmc_node_info_get_trans_vars(f_info) !=
			    (array_t *)NULL) {
				/* main_array[i][0] = [[p]]_i */
				node_ptr node_p;
				...
				node_p = array_fetch(
					node_ptr,
					sbmc_node_info_get_trans_vars(f_info),
					0);
				nusmv_assert((node_ptr)NULL != node_p);
				be_result = BeEnc_name_to_timed(be_enc, node_p,
								i_real);
				...
				if (LastState_var == (node_ptr)NULL ||
				    !SymbTable_is_symbol_input_var(st,
								   f_node)) {
					/* main_array[i][0] := p_i */
					be_result = BeEnc_name_to_timed(
						be_enc, f_node, i_real);
					nusmv_assert((be_ptr)NULL != be_result);
					array_insert(be_ptr, past_array, 0,
						     be_result);
					...
					nusmv_assert(LastState_var !=
						     (node_ptr)NULL);

					be_LastState_i = BeEnc_name_to_timed(
						be_enc, LastState_var, i_real);
					be_result = Be_And(
						be_mgr,
						BeEnc_name_to_timed(
							be_enc, f_node, i_real),
					...
					if (opt_verbose_level_ge(opts, 6)) {
						...
						Logger_log(
							logger,
							"  [[p]]_%s^0 := p_%s & (!LastState_%s | LoopExists): ",
							i_str, i_str, i_str);
						Be_DumpSexpr(be_mgr, be_result,
							     errstream);
						Logger_log(logger, "\n");
		...
		case TRUEEXP: {
			...
			if (sbmc_node_info_get_trans_vars(f_info) !=
				...
				node_aux = array_fetch(
					node_ptr,
					sbmc_node_info_get_trans_vars(f_info),
					0);
				nusmv_assert((node_ptr)NULL != node_aux);
				be_result = BeEnc_name_to_timed(
					be_enc, node_aux, i_real);
				nusmv_assert((be_ptr)NULL != be_result);
				array_insert(be_ptr, past_array, 0, be_result);
		...
		case OR: {
			...
			if (!sbmc_set_is_in(visit_cache, lsf_node)) {
				lsNewBegin(unprocessed_nodes,
					   (lsGeneric)lsf_node, LS_NH);
				has_unprocessed_children = 1;
			}
			if (!sbmc_set_is_in(visit_cache, rsf_node)) {
				lsNewBegin(unprocessed_nodes,
					   (lsGeneric)rsf_node, LS_NH);
				has_unprocessed_children = 1;
			}
			if (has_unprocessed_children)
				break;
			...
			lsf_info =
				sbmc_node_info_assoc_find(info_map, lsf_node);
			...
			rsf_info =
				sbmc_node_info_assoc_find(info_map, rsf_node);
			...
			if (sbmc_node_info_get_trans_vars(f_info) !=
				...
				/* Formula variable translation */
				for (d = 0;
				     d <= sbmc_node_info_get_past_depth(f_info);
				     d++) {
					...
					if (LastState_var == (node_ptr)NULL ||
						...
						nusmv_assert(
							array_fetch(be_ptr,
								    ...
								    d) ==
							(be_ptr)NULL);
						array_insert(be_ptr, past_array,
							     d, be_result);
		...
		/* Remove node from unprocessed stack */
		if (lsDelBegin(unprocessed_nodes, (lsGeneric *)&f_node) !=
		    LS_OK)
			ErrorMgr_internal_error(errmgr, sbmc_SNH_text, __FILE__,
						__LINE__);
			   ...
			   array_t *InLoop_array, const unsigned int i_model)
{
	...
	if (0 == i_model) {
		...
		if ((be_ptr)NULL == be_InLoop_i) {
			...
			array_insert(be_ptr, InLoop_array, i_model,
				     be_InLoop_i);
			if (opt_verbose_level_ge(opts, 2)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_log(logger,
					   "Constructed InLoop_%d := FALSE",
					   i_model);
			}
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_ptr logger = LOGGER(
	...
	be_constraint =
		Be_Implies(be_mgr, be_InLoop_i_minus_1, Be_Not(be_mgr, be_l_i));

	if (opt_verbose_level_ge(opts, 2))
		StreamMgr_print_error(
			streams, "Created the constraint (InLoop_%d => !l_%d)",
			i_model - 1, i_model);
	if (opt_verbose_level_ge(opts, 6)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger, ": ");
	...
	if ((be_ptr)NULL == be_InLoop_i) {
		...
		if (opt_verbose_level_ge(opts, 2))
			StreamMgr_print_error(
				streams,
				"Constructed InLoop_%d := InLoop_%d | l_%d",
			...
			array_t *InLoop_array, const unsigned int k)
{
	...
	for (j = 0; j < k; j++) {
       ...
       implicitly always keep their valus from state to state */
		be_equal_system_vars = sbmc_equal_vectors_formula(
			be_enc,
			sbmc_state_vars_get_simple_path_system_vars(state_vars),
			sbmc_real_k(j), sbmc_real_k(k));

		be_equal_pd0_translation_vars = sbmc_equal_vectors_formula(
			be_enc,
			sbmc_state_vars_get_translation_vars_pd0(state_vars),
			sbmc_real_k(j), sbmc_real_k(k));

		be_equal_pdx_translation_vars = sbmc_equal_vectors_formula(
			be_enc,
			sbmc_state_vars_get_translation_vars_pdx(state_vars),
			sbmc_real_k(j), sbmc_real_k(k));

		be_equal_aux_translation_vars = sbmc_equal_vectors_formula(
			be_enc,
		...
		be_constraint = Be_Or(be_mgr, be_constraint,
				      Be_Not(be_mgr, be_equal_InLoops));

		/* (s_j != s_k) | (InLoop_j != InLoop_k) | ([[f]]_j^0 != [[f]]_k^0) */
		be_constraint =
			Be_Or(be_mgr, be_constraint,
			      Be_Not(be_mgr, be_equal_pd0_translation_vars));

		be_c2 = Be_And(
			be_mgr, Be_And(be_mgr, be_InLoop_j, be_InLoop_k),
			Be_Or(be_mgr,
			      Be_Not(be_mgr, be_equal_pdx_translation_vars),
			      Be_Not(be_mgr, be_equal_aux_translation_vars)));
