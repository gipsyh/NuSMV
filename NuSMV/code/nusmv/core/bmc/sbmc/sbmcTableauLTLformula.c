				    ...
				    const int k, const int l)
{
	...
	tableau = get_f_at_time(be_enc, ltl_wff, formulatable, memoiz, 0, k, l,
				0);
	...
	if (!opt_bmc_sbmc_il_opt(opts) || k == 0) { /**normal translation*/
		...
		if (node_get_type(ltl_wff) == OP_FUTURE) {
			if (opt_bmc_sbmc_gf_fg_opt(opts) &&
				...
				/**FG p*/
				return bmc_tableauGetGloballyIL_opt(
					be_enc, ltl_wff, k, l, pastdepth, table,
					memoiz);
			}
			/** Fp*/
			return bmc_tableauGetEventuallyIL_opt(be_enc, ltl_wff,
							      k, l, pastdepth,
							      table, memoiz);
			...
			if (opt_bmc_sbmc_gf_fg_opt(opts) &&
				...
				/**GF p*/
				return bmc_tableauGetEventuallyIL_opt(
					be_enc, ltl_wff, k, l, pastdepth, table,
					memoiz);
			}
			/**G p*/
			return bmc_tableauGetGloballyIL_opt(be_enc, ltl_wff, k,
							    l, pastdepth, table,
							    memoiz);
		} else {
		     ...
		     const unsigned pastdepth)
{
	...
	if (Bmc_Utils_IsAllLoopbacks(l)) {
		...
		/**f(k):=g(l)*/
		return get_g_at_time(be_enc, ltl_wff, table, memoiz, l, k, l,
				     pastdepth);
	} else { /**No loopback*/
		/** f(k):= false*/
		     ...
		     const unsigned pastdepth)
{
	...
	if (Bmc_Utils_IsAllLoopbacks(l)) {
		...
		/**f(k):=f1(l)*/
		return get_f_at_time(be_enc, ltl_wff, table, memoiz, l, k, l,
				     pastdepth);
	} else { /**No loopback*/
		/** f(k):= false*/
		return Be_Falsity(be_mgr);
		...
		case OP_GLOBAL:
			if (opt_bmc_sbmc_gf_fg_opt(opts)) {
				if (node_get_type(car(formula)) == OP_FUTURE) {
					Bmc_Hash_insert(table, formula,
							TLcount++);
					Bmc_Stack_push(thestack,
						       car(car(formula)));
					break;
				}
			}
			    ...
			    const int k, const int l, const unsigned pastdepth)
{
	...
	case AND:
		result = Be_And(be_mgr,
				get_f_at_time(be_enc, car(ltl_wff), table,
					      memoiz, time, k, l, pastdepth),
				get_f_at_time(be_enc, cdr(ltl_wff), table,
					      memoiz, time, k, l, pastdepth));
		break;
	...
	case NOT:
		...
		if (!SymbTable_is_symbol_declared(st, car(ltl_wff))) {
			ErrorMgr_internal_error(
				errmgr,
				"Unexpected scalar or undefined node\n");
		}
		...
		return Be_Not(be_mgr,
			      BeEnc_name_to_timed(be_enc, car(ltl_wff), time));

	case OP_NEXT:
		if (time < k) {
			/**f(time):=f(time+1)*/
			result = get_f_at_time(be_enc, car(ltl_wff), table,
					       memoiz, time + 1, k, l,
					       pastdepth);
		} else { /**time=k*/
			result = last_f(be_enc, car(ltl_wff), table, memoiz, l,
					k,
	...
	case OP_NOTPRECNOT:
	case OP_PREC:
		result = get_ZY_at_time(be_enc, ltl_wff, table, memoiz, time, k,
					l, pastdepth);
		break;
	...
	case OP_FUTURE:
		result = bmc_cache_insert_f(
			ltl_wff, time, k, pastdepth, table,
			get_Eventually_at_time(be_enc, ltl_wff, table, memoiz,
					       time, k, l, pastdepth));
		break;
	...
	case UNTIL:
		result = bmc_cache_insert_f(
			ltl_wff, time, k, pastdepth, table,
			get_Until_at_time(be_enc, ltl_wff, table, memoiz, time,
					  k, l, pastdepth));
		break;

	case RELEASES:
		result = bmc_cache_insert_f(ltl_wff, time, k, pastdepth, table,
					    get_V_at_time(be_enc, ltl_wff,
							  table, memoiz, time,
							  k, l, pastdepth));
		break;
	...
	case SINCE:
		result = bmc_cache_insert_f(
			ltl_wff, time, k, pastdepth, table,
			get_Since_at_time(be_enc, ltl_wff, table, memoiz, time,
					  k, l, pastdepth));
		break;
	...
	case OP_ONCE:
		result = bmc_cache_insert_f(
			ltl_wff, time, k, pastdepth, table,
			get_Once_at_time(be_enc, ltl_wff, table, memoiz, time,
					 k, l, pastdepth));
		break;
	...
	case OP_HISTORICAL:
		result = bmc_cache_insert_f(
			ltl_wff, time, k, pastdepth, table,
			get_Historically_at_time(be_enc, ltl_wff, table, memoiz,
						 time, k, l, pastdepth));
		break;

	case IMPLIES:
		ErrorMgr_internal_error(
			errmgr, "'Implies' should had been nnf-ed away!\n");
	case ATOM:
	case EX:
	case AX:
	case EF:
	...
	case ABG:
		ErrorMgr_internal_error(
			errmgr, "f: Unexpected CTL operator, node type %d\n",
			node_get_type(ltl_wff));
	default:
		ErrorMgr_internal_error(
			errmgr, "f: Unexpected operator, node type %d\n",
			node_get_type(ltl_wff));
				   ...
				   const int l, const unsigned pastdepth)
{
	...
	if (time < k) {
		if (opt_bmc_sbmc_gf_fg_opt(opts) &&
		    (node_get_type(car(ltl_wff)) == OP_FUTURE)) {
			/**f(i):=f(k)*/
			return get_f_at_time(be_enc, ltl_wff, table, memoiz, k,
					     k, l, pastdepth);
		}
	...
	return Be_And(be_mgr,
		      get_f_at_time(be_enc, car(ltl_wff), table, memoiz, time,
				    k, l, pastdepth),
		      (pastdepth < bmc_past_depth(ltl_wff)) ?
			      last_f(be_enc, ltl_wff, table, memoiz, l, k,
				     pastdepth + 1) :
				...
				const unsigned pastdepth)
{
	...
	if (time < k) {
		...
		return Be_Or(
			be_mgr,
			get_f_at_time(be_enc, cdr(ltl_wff), table, memoiz, time,
				      k, l, pastdepth),
			Be_And(be_mgr,
			       get_f_at_time(be_enc, car(ltl_wff), table,
					     memoiz, time, k, l, pastdepth),
				...
				const unsigned pastdepth)
{
	...
	if ((time == 0) && (pastdepth == 0)) {
		return get_f_at_time(be_enc, cdr(ltl_wff), table, memoiz, time,
				     k, l, pastdepth);
		...
		return Be_Or(be_mgr,
			     ...
			     Be_And(be_mgr,
				    ...
				    Be_Ite(be_mgr,
					   ...
					   get_f_at_time(be_enc, ltl_wff, table,
							 memoiz, k, k, l,
							 pastdepth - 1),
					   get_f_at_time(be_enc, ltl_wff, table,
							 memoiz, time - 1, k, l,
				       ...
				       const unsigned pastdepth)
{
	...
	nusmv_assert((node_get_type(ltl_wff) == OP_HISTORICAL) &&
		     (time < k + 1));
	...
	if ((time == 0) && (pastdepth == 0)) {
		/**f1(i)*/
		return get_f_at_time(be_enc, car(ltl_wff), table, memoiz, time,
				     k, l, pastdepth);
	} else if ((pastdepth == 0) && (time > 0) && (time <= k)) {
			       ...
			       const unsigned pastdepth)
{
	...
	if ((time == 0) && (pastdepth == 0)) {
		...
		return Be_Or(be_mgr,
			     get_f_at_time(be_enc, car(ltl_wff), table, memoiz,
					   time, k, l, pastdepth),
			     get_f_at_time(be_enc, ltl_wff, table, memoiz, k, k,
					   l, pastdepth - 1));
	}
	error_unreachable_code();
	return 0; /**Should not be reached*/
			     ...
			     const int k, const int l, const unsigned pastdepth)
{
	...
	if ((time == 0) && (pastdepth == 0)) {
		...
		return get_f_at_time(be_enc, car(ltl_wff), table, memoiz, k, k,
				     l, pastdepth - 1);
	}
	error_unreachable_code();
	return 0; /**Should not be reached*/
			    ...
			    const int k, const int l, const unsigned pastdepth)
{
	...
	if (data >= 0) { /**A TL subformula, check cache*/
		if (bmc_cache_fetch_g(ltl_wff, time, k, pastdepth, table) !=
		    0) {
			return bmc_cache_fetch_g(ltl_wff, time, k, pastdepth,
						 table);
		}
	}
	...
	case ABG:
		ErrorMgr_internal_error(
			errmgr, "g: Unexpected CTL operator, node type %d\n",
			node_get_type(ltl_wff));
	default:
		/* no other type are available here: */
		error_unreachable_code();
...
static be_ptr AtMostOnce(const BeEnc_ptr be_enc, const int k)
{
	...
	for (i = 1; i < k; i++) {
		...
		smaller_exists_i = Be_Or(be_mgr, smaller_exists_i,
				...
				hashPtr table)
{
	...
	if (bmc_cache_f != (be_ptr *)NULL) {
		int i = (pastdepth + (bmc_tab_past_depth + 1) * data) *
				(k + 1) +
			time;
