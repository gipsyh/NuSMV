...
/*---------------------------------------------------------------------------*/
static hash_ptr sbmc_bmc_inc_setup_layer(
	const NuSMVEnv_ptr env, BeFsm_ptr be_fsm, BddEnc_ptr bdd_enc,
	const char *layer_name, state_vars_struct *state_vars,
	node_ptr bltlspec, const int opt_do_virtual_unrolling,
	const int opt_do_completeness_check, const int opt_force_state_vars,
	node_ptr *ass_SimplePath_node);

static void sbmc_bmc_inc_shutdown_layer(BeFsm_ptr be_fsm, BddEnc_ptr bdd_enc,
		     ...
		     const int opt_do_completeness_check)
{
   ...
   */
	...
	info_map = sbmc_bmc_inc_setup_layer(
		env, be_fsm, bdd_enc, SBMC_INC_LAYER_NAME, state_vars, bltlspec,
		opt_do_virtual_unrolling, opt_do_completeness_check,
		opt_force_state_vars, &ass_SimplePath_node);
   ...
   */
	be_LoopExists = BeEnc_name_to_timed(
		be_enc, sbmc_state_vars_get_LoopExists_var(state_vars), 0);
   ...
   */
	{
		lsList new_constraints =
			sbmc_unroll_base(be_enc, bltlspec, info_map,
   ...
   */
	...
	for (current_k = 0; current_k <= max_k && !found_solution;
	     current_k++) {
		int i = 0;
     ...
     */
		for (i = max(previous_k + 2, 0); i <= current_k + 1; i++) {
			sbmc_init_state_vector(
				be_enc, bltlspec, info_map, sbmc_real_k(i),
				sbmc_state_vars_get_LastState_var(state_vars),
				be_LoopExists);
		}
     ...
     */
		for (i = max(previous_k, 0); i < current_k; i++) {
			be_ptr be_TR = Bmc_Model_GetUnrolling(
				be_fsm, sbmc_real_k(i), sbmc_real_k(i + 1));

			sbmc_MS_force_true(zolver, be_TR, cnf_alg);

			if (opt_verbose_level_gt(opts, 1))
				StreamMgr_print_error(
					streams, "Forced T(%d,%d)", i, i + 1);
     ...
     */
		for (i = max(previous_k + 1, 0); i <= current_k; i++) {
			...
			if (i == 0) {
				/* l_0 <=> FALSE */
				be_constraint = Be_Iff(be_mgr, be_l_i,
						       Be_Falsity(be_mgr));
				if (opt_verbose_level_ge(opts, 6)) {
					Logger_ptr logger = LOGGER(
					...
					Logger_log(logger,
						   "Created (l_0 <=> false): ");
					Be_DumpSexpr(be_mgr, be_constraint,
						     errstream);
					Logger_log(logger, "\n");
				}
			} else {
				be_constraint = sbmc_equal_vectors_formula(
					be_enc,
					sbmc_state_vars_get_simple_path_system_vars(
						state_vars),
					sbmc_real_k(i - 1), sbmc_E_state());
				be_constraint = Be_Implies(be_mgr, be_l_i,
							   be_constraint);
				if (opt_verbose_level_ge(opts, 6)) {
					Logger_ptr logger = LOGGER(
						NuSMVEnv_get_value(env,
     ...
     */
		for (i = max(previous_k, 0); i <= current_k - 1; i++) {
			...
			be_constraint = Be_Iff(be_mgr, be_LastState_i,
					       Be_Falsity(be_mgr));
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_log(logger,
					   "Created (LastState_%d <=> false): ",
					   i);
     ...
     */
		for (i = max(previous_k + 1, 0); i <= current_k; i++) {
			be_ptr be_constraint = sbmc_build_InLoop_i(
				be_enc, state_vars, InLoop_array, i);
			if (be_constraint) {
     ...
     */
		{
			lsList new_constraints = sbmc_unroll_invariant(
				be_enc, bltlspec, previous_k, current_k,
				state_vars, InLoop_array, info_map,
				be_LoopExists, opt_do_optimization);
     ...
     */
       ...
       */
			...
			for (i = max(previous_k + 1, 0); i <= current_k; i++) {
				lsList new_constraints = sbmc_SimplePaths(
					be_enc, state_vars, InLoop_array, i);
				sbmc_MS_force_constraint_list(
					zolver, new_constraints, cnf_alg);
				lsDestroy(new_constraints, NULL);
				new_constraints = (lsList)NULL;
			}
#endif
       ...
       */
...
#ifdef BENCHMARKING
			{
				times(&time_temp);
				StreamMgr_print_error(
					streams, ":UTIME = %.4f secs.\n",
					time_diff(&time_solver_start,
						  &time_temp));
				StreamMgr_print_error(
					streams,
					":STOP:benchmarking Solving\n");
				StreamMgr_print_error(
					streams,
			...
			default:
				ErrorMgr_internal_error(
					errmgr,
					"Sbmc_zigzag_incr: Unexpected value in satResult");
     ...
     */
		{
			...
			be_LastState_k = BeEnc_name_to_timed(
				be_enc,
				sbmc_state_vars_get_LastState_var(state_vars),
				sbmc_real_k(current_k));

			be_constraint = Be_Iff(be_mgr, be_LastState_k,
					       Be_Truth(be_mgr));
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
     ...
     */
		{
			lsList new_constraints = sbmc_dependent(
				be_enc, bltlspec, current_k, state_vars,
				InLoop_array, be_LoopExists, info_map);
			sbmc_MS_force_constraint_list(zolver, new_constraints,
						      cnf_alg);
			lsDestroy(new_constraints, NULL);
			new_constraints = (lsList)NULL;
     ...
     */
		...
		case SAT_SOLVER_SATISFIABLE_PROBLEM:
			StreamMgr_print_output(
				streams, "-- terminating with bound %d.\n",
				current_k);
			StreamMgr_print_output(streams, "-- ");
	...
	sbmc_bmc_inc_shutdown_layer(be_fsm, bdd_enc, SBMC_INC_LAYER_NAME,
				    info_map);
   ...
   */
	{
		...
		SLIST_FOREACH(assumptions, iterator)
		{
			...
			ass_be = BeEnc_name_to_timed(be_enc, ass_name,
						     sbmc_real_k(0));
			if (NOT == node_get_type(assumption)) {
				lsNewEnd(assumptions_be,
					 (lsGeneric)Be_Not(be_mgr, ass_be),
					 LS_NH);
			} else {
				lsNewEnd(assumptions_be, (lsGeneric)ass_be,
					 LS_NH);
			}
		...
		ass_SimplePath_be =
			BeEnc_name_to_timed(be_enc, ass_SimplePath_node, 0);
       ...
       */
			...
			for (i = max(previous_k + 1, 0); i <= current_k; i++) {
				...
				/* prefix SimplePath constraints with assumption */
				{
					...
					lsForEachItem(new_constraints, iterator,
						      constraint)
					{
						lsNewEnd(
							assumed_constraints,
			...
			if (0 == ass_SimplePath_cnflit) {
				/* try to set and append ass_SimplePath_cnflit to assumptions_cnf */
				int ass_sp_beind =
					Be_Var2Index(be_mgr, ass_SimplePath_be);
				nusmv_assert(0 != ass_sp_beind);
				ass_SimplePath_cnflit = Be_BeIndex2CnfLiteral(
					be_mgr, ass_sp_beind);
				if (0 != ass_SimplePath_cnflit) {
					Slist_push(
						assumptions_cnf,
			...
			/* Actually solve the problem */
			satResult =
				sbmc_MS_solve_assume(zolver, assumptions_cnf);
			/*
      ...
      */
			...
			case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
				...
				/* obtain conflict */
				{
					...
					{
						conf_cnflit = PTR_TO_INT(
							Siter_element(
								iterator));

						if (ass_SimplePath_cnflit ==
						    conf_cnflit) {
							sp_in_conflict = true;
							if (opt_verbose_level_gt(
								    opts, 1)) {
								Logger_ptr logger =
									LOGGER(NuSMVEnv_get_value(
										env,
										ENV_LOGGER));
								Logger_log(
									logger,
					...
					*conflict = Slist_create();
					...
					{ /*1*/
						...
						{ /*2*/
							int conf_belit,
								conf_beind;
							be_ptr conf_be_timed,
								conf_be_untimed;
							node_ptr conf;

							conf_belit = Be_CnfLiteral2BeLiteral(
								be_mgr,
								conf_cnflit);
							conf_beind = Be_BeLiteral2BeIndex(
								be_mgr,
								abs(conf_belit));
							conf_be_timed = Be_Index2Var(
								be_mgr,
								conf_beind);
							conf_be_untimed =
								BeEnc_var_to_untimed(
									be_enc,
									conf_be_timed);
							conf = BeEnc_var_to_name(
								be_enc,
								conf_be_untimed);
							if (Be_BeLiteral_IsSignPositive(
								    be_mgr,
								    conf_belit)) {
								Slist_push(
									*conflict,
	...
	node_ptr *ass_SimplePath_node)
{
	...
	sbmc_state_vars_set_l_var(state_vars,
				  sbmc_add_new_state_variable(env, ltl_layer,
							      "#LTL_l"));
	...
	*ass_SimplePath_node = sbmc_add_new_state_variable(
		env, ltl_layer, "#LTL_ass_SimplePath");
   ...
   */
	layer_info_map = sbmc_init_LTL_info(
		env, ltl_layer, bltlspec,
		...
		opt_force_state_vars, opt_do_virtual_unrolling);
