...
static hash_ptr sbmc_bmc_inc_setup_layer(
    ...
    const char *layer_name, state_vars_struct *state_vars, node_ptr bltlspec,
    const int opt_do_virtual_unrolling, const int opt_do_completeness_check,
    const int opt_force_state_vars, node_ptr *ass_SimplePath_node);
...
int Sbmc_zigzag_incr(NuSMVEnv_ptr env, Prop_ptr ltlprop, const int max_k,
   ...
   */
  {
    ...
    lsList tr_be_vars = Bmc_Utils_get_vars_list_for_uniqueness(be_enc, ltlprop);
  ...
  info_map = sbmc_bmc_inc_setup_layer(
      ...
      opt_do_virtual_unrolling, opt_do_completeness_check, opt_force_state_vars,
      &ass_SimplePath_node);
   ...
   */
  {
    lsList new_constraints = sbmc_unroll_base(
        be_enc, bltlspec, info_map, be_LoopExists, opt_do_optimization);
  ...
  for (current_k = 0; current_k <= max_k && !found_solution; current_k++) {
    ...
    for (i = max(previous_k, 0); i < current_k; i++) {
      be_ptr be_TR =
          Bmc_Model_GetUnrolling(be_fsm, sbmc_real_k(i), sbmc_real_k(i + 1));
    ...
    for (i = max(previous_k + 1, 0); i <= current_k; i++) {
      ...
      be_l_i = BeEnc_name_to_timed(
          be_enc, sbmc_state_vars_get_l_var(state_vars), sbmc_real_k(i));
      ...
      if (i == 0) {
        ...
        if (opt_verbose_level_ge(opts, 6)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(logger, "Created (l_%d => (s_%d = s_E)): ", i, i - 1);
          Be_DumpSexpr(be_mgr, be_constraint, errstream);
          Logger_log(logger, "\n");
        }
      }
    ...
    for (i = max(previous_k + 1, 0); i <= current_k; i++) {
      be_ptr be_constraint =
          sbmc_build_InLoop_i(be_enc, state_vars, InLoop_array, i);
      if (be_constraint) {
        sbmc_MS_force_true(zolver, be_constraint, cnf_alg);
      }
    }
    ...
    if (opt_do_optimization) {
      ...
      for (i = max(previous_k + 1, 0); i <= current_k; i++) {
        ...
        if (opt_verbose_level_ge(opts, 6)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(logger, "Created (l_%d => LoopExists): ", current_k);
          Be_DumpSexpr(be_mgr, be_constraint, errstream);
          Logger_log(logger, "\n");
        }
      }
     ...
     */
    {
      lsList new_constraints = sbmc_unroll_invariant(
          be_enc, bltlspec, previous_k, current_k, state_vars, InLoop_array,
          info_map, be_LoopExists, opt_do_optimization);
      sbmc_MS_force_constraint_list(zolver, new_constraints, cnf_alg);
      lsDestroy(new_constraints, NULL);
      new_constraints = (lsList)NULL;
    }
    ...
    if (opt_do_completeness_check) {
...
#ifdef BENCHMARKING
      {
        ...
        StreamMgr_print_error(
            ...
            current_k, time_diff(&time_solver_start, &time_temp),
      ...
      case SAT_SOLVER_SATISFIABLE_PROBLEM:
        StreamMgr_print_output(
            streams, "-- could not terminate yet with bound %d", current_k);
      ...
      default:
        ErrorMgr_internal_error(
            errmgr, "Sbmc_zigzag_incr: Unexpected value in satResult");
    ...
    {
      lsList new_constraints =
          sbmc_dependent(be_enc, bltlspec, current_k, state_vars, InLoop_array,
                         be_LoopExists, info_map);
      sbmc_MS_force_constraint_list(zolver, new_constraints, cnf_alg);
      lsDestroy(new_constraints, NULL);
      new_constraints = (lsList)NULL;
    }
    ...
    case SAT_SOLVER_UNSATISFIABLE_PROBLEM: {
      StreamMgr_print_output(
          streams, "-- no counterexample found with bound %d", current_k);
      if (opt_verbose_level_gt(opts, 2)) {
        Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
        Logger_log(logger, " for ");
        print_spec(Logger_get_ostream(logger), inputprop,
    ...
    case SAT_SOLVER_SATISFIABLE_PROBLEM:
      ...
      if (opt_counter_examples(opts)) {
        ...
        trace = Sbmc_Utils_generate_and_print_cntexample(
            be_enc, tm, zolver, sbmc_state_vars_get_l_var(state_vars),
...
int Sbmc_zigzag_incr_assume(NuSMVEnv_ptr env, Prop_ptr ltlprop, const int max_k,
   ...
   */
  {
    ...
    ass_SimplePath_be = BeEnc_name_to_timed(be_enc, ass_SimplePath_node, 0);
  ...
  for (current_k = 0; current_k <= max_k && !found_solution; current_k++) {
    ...
    for (i = max(previous_k + 1, 0); i <= current_k; i++) {
      ...
      if (i == 0) {
        ...
        if (opt_verbose_level_ge(opts, 6)) {
          Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
          Logger_log(logger, "Created (l_%d => (s_%d = s_E)): ", current_k,
                     current_k - 1);
          Be_DumpSexpr(be_mgr, be_constraint, errstream);
          Logger_log(logger, "\n");
        }
        ...
        /* prefix SimplePath constraints with assumption */
        {
          ...
          lsForEachItem(new_constraints, iterator, constraint) {
            lsNewEnd(assumed_constraints,
                     Be_Implies(be_mgr, ass_SimplePath_be, constraint), LS_NH);
          }
          lsDestroy(new_constraints, NULL);
      ...
      if (0 == ass_SimplePath_cnflit) {
        ...
        if (0 != ass_SimplePath_cnflit) {
          Slist_push(assumptions_cnf,
                     PTR_FROM_INT(void *, ass_SimplePath_cnflit));
        }
      }
...
#ifndef SBMC_HAVENOSIMPLEPATH
          SLIST_FOREACH(!sp_in_conflict ? conflict_cnf : assumptions_cnf,
                        iterator) { /*1*/
            conf_cnflit = PTR_TO_INT(Siter_element(iterator));
          ...
          SLIST_FOREACH(conflict_cnf, iterator) { /*1*/
            ...
            { /*2*/
              ...
              if (Be_BeLiteral_IsSignPositive(be_mgr, conf_belit)) {
                ...
                Slist_push(*conflict,
                           (void *)find_node(nodemgr, NOT, conf, Nil));
              }
            } /*2*/
          }   /*1*/
        }     /* obtain conflict */
...
static hash_ptr sbmc_bmc_inc_setup_layer(
  ...
  SymbTable_layer_add_to_class(symb_table, layer_name, ARTIFACTS_LAYERS_CLASS);

  /* Allocate the l_i variable */
  sbmc_state_vars_set_l_var(
      state_vars, sbmc_add_new_state_variable(env, ltl_layer, "#LTL_l"));
  ...
  *ass_SimplePath_node =
   ...
   */
  layer_info_map =
      sbmc_init_LTL_info(env, ltl_layer, bltlspec,
