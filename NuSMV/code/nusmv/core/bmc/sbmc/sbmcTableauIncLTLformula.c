			...
			const int do_optimization)
{
	...
	while (lsLength(unprocessed_nodes) > 0) {
		...
		array_t *E_past_array = (array_t *)NULL,
			*L_past_array = (array_t *)NULL;
		int has_unprocessed_children;

		/* Get node */
		if ((lsFirstItem(unprocessed_nodes, (lsGeneric *)&node,
				 LS_NH) != LS_OK) ||
		    ((node_ptr)NULL == node))
			ErrorMgr_internal_error(errmgr, sbmc_SNH_text, __FILE__,
						__LINE__);
		...
		if (sbmc_node_info_get_trans_bes(info)) {
			nusmv_assert(array_n(sbmc_node_info_get_trans_bes(
					     info)) >= 2);
			L_past_array = array_fetch(
				array_t *, sbmc_node_info_get_trans_bes(info),
				sbmc_L_state());
		...
		if (sbmc_set_is_in(visit_cache, node)) {
			...
			if (lsDelBegin(unprocessed_nodes, (lsGeneric *)&node) !=
			    LS_OK)
				ErrorMgr_internal_error(errmgr, sbmc_SNH_text,
							__FILE__, __LINE__);
			continue;
		}
		...
		case ARRAY: {
			if (do_optimization &&
			    !SymbTable_is_symbol_input_var(st, node)) {
				/* Optimization:
           If p is not an input variable, add [[p]]_E^d <=> p_E */
				be_ptr be_result = 0;
				nusmv_assert(sbmc_node_info_get_past_depth(
						     info) == 0);
				if (sbmc_node_info_get_trans_vars(info) ==
				    (array_t *)NULL) {
					/* Definitional translation.
             Already done by sbmc_init_state_vector. */
					nusmv_assert(array_fetch(be_ptr,
								 E_past_array,
								 0) !=
						     (be_ptr)NULL);
				} else {
					...
					be_p_E_0 = array_fetch(be_ptr,
							       E_past_array, 0);
					nusmv_assert((be_ptr)NULL != be_p_E_0);
					be_realp_E = BeEnc_name_to_timed(
						be_enc, node, sbmc_E_state());
					nusmv_assert((be_ptr)NULL !=
						     be_realp_E);

					be_result = Be_Iff(be_mgr, be_p_E_0,
							   be_realp_E);
					nusmv_assert((be_ptr)NULL != be_result);

					/* Save the created constraint */
					lsNewBegin(created_constraints,
						   (lsGeneric)be_result, LS_NH);
		...
		case TRUEEXP: {
			if (do_optimization) {
				...
				if (sbmc_node_info_get_trans_vars(info) ==
					...
					be_ptr be_TRUE_E_0 = array_fetch(
						be_ptr, E_past_array, 0);

					nusmv_assert((be_ptr)NULL !=
						     be_TRUE_E_0);

					be_result = Be_Iff(be_mgr, be_TRUE_E_0,
							   Be_Truth(be_mgr));
					nusmv_assert((be_ptr)NULL != be_result);
		...
		case FALSEEXP: {
			if (do_optimization) {
				...
				if (sbmc_node_info_get_trans_vars(info) ==
					...
					be_ptr be_FALSE_E_0 = array_fetch(
						be_ptr, E_past_array, 0);

					nusmv_assert((be_ptr)NULL !=
						     be_FALSE_E_0);

					be_result = Be_Iff(be_mgr, be_FALSE_E_0,
							   Be_Falsity(be_mgr));
					nusmv_assert((be_ptr)NULL != be_result);
		...
		case OR: {
			...
			if (!sbmc_set_is_in(visit_cache, lsf)) {
				lsNewBegin(unprocessed_nodes, (lsGeneric)lsf,
					   LS_NH);
				has_unprocessed_children = 1;
			}
			if (!sbmc_set_is_in(visit_cache, rsf)) {
				lsNewBegin(unprocessed_nodes, (lsGeneric)rsf,
					   LS_NH);
				has_unprocessed_children = 1;
			}
			...
			if (do_optimization) {
				...
				if (sbmc_node_info_get_trans_vars(info) ==
					...
					/* Definitional translation already done by state vector init */
					for (d = 0;
					     d <= sbmc_node_info_get_past_depth(
							  info);
					     d++)
						nusmv_assert(
							array_fetch(be_ptr,
					...
					for (d = 0;
					     d <= sbmc_node_info_get_past_depth(
							  info);
					     d++) {
						be_ptr be_fORg_E_d, be_f_E_d,
							be_g_E_d, be_result;
						const unsigned int d_lsf = min(
							d,
							sbmc_node_info_get_past_depth(
								lsf_info));
						const unsigned int d_rsf = min(
							d,
							sbmc_node_info_get_past_depth(
								rsf_info));

						be_fORg_E_d = array_fetch(
							be_ptr, E_past_array,
							d);
						nusmv_assert((be_ptr)NULL !=
							     be_fORg_E_d);
						be_f_E_d = array_fetch(
							be_ptr, past_array_f_E,
							d_lsf);
						nusmv_assert((be_ptr)NULL !=
							     be_f_E_d);
						be_g_E_d = array_fetch(
							be_ptr, past_array_g_E,
							d_rsf);
						nusmv_assert((be_ptr)NULL !=
							     be_g_E_d);
						be_result = Be_Iff(
							be_mgr, be_fORg_E_d,
							Be_Or(be_mgr, be_f_E_d,
							      be_g_E_d));
						nusmv_assert((be_ptr)NULL !=
							     be_result);
		...
		case NOT: {
			...
			if (do_optimization &&
			    !SymbTable_is_symbol_input_var(st, lsf)) {
				/* Optimization:
				...
				if (sbmc_node_info_get_trans_vars(info) ==
					...
					for (d = 0;
					     d <= sbmc_node_info_get_past_depth(
							  info);
					     d++) {
						be_ptr be_notf_E_d, be_f_E_d,
							be_result;

						be_notf_E_d = array_fetch(
							be_ptr, E_past_array,
							d);
						nusmv_assert((be_ptr)NULL !=
							     be_notf_E_d);
         ...
         */
				for (d = 0;
				     d <= sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_aux, be_lsf, be_aux_next,
						be_result;

					be_aux = array_fetch(be_ptr,
							     E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_aux);
					be_lsf = array_fetch(
						be_ptr, lsf_E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_lsf);
					be_aux_next = array_fetch(
						be_ptr, L_past_array,
						min(d + 1,
						    sbmc_node_info_get_past_depth(
							    info)));
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Ff_E_d, be_Ff_E_dP1,
						be_result;

					be_Ff_E_d = array_fetch(
						be_ptr, E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_Ff_E_d);
					be_Ff_E_dP1 = array_fetch(
						be_ptr, E_past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Ff_E_dP1);
					be_result = Be_Implies(
						be_mgr, be_Ff_E_dP1, be_Ff_E_d);
         ...
         */
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Gf_E_d, be_Gf_E_dP1,
						be_result;

					be_Gf_E_d = array_fetch(
						be_ptr, E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_Gf_E_d);
					be_Gf_E_dP1 = array_fetch(
						be_ptr, E_past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Gf_E_dP1);
					be_result = Be_Implies(
						be_mgr, be_Gf_E_d, be_Gf_E_dP1);
         ...
         * [[g]]_E^d | ([[f]]_E^d  & [[fUg]]_L^min(d+1,pd(fUg))) */
				for (d = 0;
				     d <= sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_fUg_E_d, be_f_E_d, be_g_E_d,
						be_fUg_L_dP1, be_result;

					be_fUg_E_d = array_fetch(
						be_ptr, E_past_array, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fUg_E_d);
         ...
         * [[g]]_E^d & ([[f]]_E^d  | [[fRg]]_L^min(d+1,pd(fUg))) */
				for (d = 0;
				     d <= sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_fRg_E_d, be_f_E_d, be_g_E_d,
						be_fRg_L_dP1, be_result;

					be_fRg_E_d = array_fetch(
						be_ptr, E_past_array, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fRg_E_d);
			...
			if (do_optimization) {
				...
				for (d = 0;
					...
					be_aux = array_fetch(be_ptr,
							     E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_aux);
					be_lsf_next = array_fetch(
						be_ptr, lsf_L_past_array,
						min(d + 1,
						    sbmc_node_info_get_past_depth(
							    info)));
					nusmv_assert((be_ptr)NULL !=
		...
		case OP_HISTORICAL: {
			...
			if (do_optimization) {
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Hf_E_d, be_Hf_E_dP1,
						be_result;

					be_Hf_E_d = array_fetch(
						be_ptr, E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_Hf_E_d);
					be_Hf_E_dP1 = array_fetch(
						be_ptr, E_past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Hf_E_dP1);
					be_result = Be_Implies(
						be_mgr, be_Hf_E_dP1, be_Hf_E_d);
		...
		case OP_ONCE: {
			...
			if (do_optimization) {
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Of_E_d, be_Of_E_dP1,
						be_result;

					be_Of_E_d = array_fetch(
						be_ptr, E_past_array, d);
					nusmv_assert((be_ptr)NULL != be_Of_E_d);
					be_Of_E_dP1 = array_fetch(
						be_ptr, E_past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Of_E_dP1);
					be_result = Be_Implies(
						be_mgr, be_Of_E_d, be_Of_E_dP1);
		...
		default:
			...
			ErrorMgr_internal_error(errmgr, sbmc_SNYI_text,
						__FILE__, __LINE__);
			break;
		}
		if (has_unprocessed_children)
     ...
     */
       ...
       */
			int d;
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
			     d++) {
				...
				be_result = Be_Implies(
					be_mgr, Be_Not(be_mgr, be_LoopExists),
			       ...
			       const int do_optimization)
{
	...
	if (opt_verbose_level_ge(opts, 2)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "Unrolling k-invariant future stuff at time %u\n",
			   i_model);
		fflush(errstream);
	}
	...
	while (lsLength(unprocessed_nodes) > 0) {
		...
		nusmv_assert((array_t *)NULL !=
			     sbmc_node_info_get_trans_bes(info));
		nusmv_assert(array_n(sbmc_node_info_get_trans_bes(info)) >=
			     i_real + 1);
		past_array = array_fetch(
			array_t *, sbmc_node_info_get_trans_bes(info), i_real);
		nusmv_assert((array_t *)NULL != past_array);
		...
		if (sbmc_set_is_in(visit_cache, node)) {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
			     d++)
				nusmv_assert(array_fetch(be_ptr, past_array,
							 d) != (be_ptr)NULL);
			/* Remove node from unprocessed stack */
			if (lsDelBegin(unprocessed_nodes, (lsGeneric *)&node) !=
			    LS_OK)
		...
		case ARRAY: {
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_nlog(logger, wffprint, str_debug1, node,
					    i_model);
			}
			...
			if (sbmc_node_info_get_trans_vars(info) ==
				...
				if (SymbTable_is_symbol_input_var(st, node)) {
					...
					be_ptr be_p_i_0, be_realp_i, be_result;
					nusmv_assert(be_LastState_i !=
						     (be_ptr)NULL);

					be_p_i_0 = array_fetch(be_ptr,
							       past_array, 0);
					nusmv_assert((be_ptr)NULL != be_p_i_0);
					be_realp_i = BeEnc_name_to_timed(
						be_enc, node, i_real);
					nusmv_assert((be_ptr)NULL !=
						     be_realp_i);
					be_result = Be_Iff(
					...
					nusmv_assert((be_ptr)NULL !=
						     be_realp_i);
					be_result = Be_Iff(be_mgr, be_p_i_0,
							   be_realp_i);
					nusmv_assert((be_ptr)NULL != be_result);
					/* Save the created constraint */
					lsNewBegin(created_constraints,
         ...
         */
				...
				be_TRUE_i_0 =
					array_fetch(be_ptr, past_array, 0);
				nusmv_assert((be_ptr)NULL != be_TRUE_i_0);
				be_true = Be_Truth(be_mgr);
				nusmv_assert((be_ptr)NULL != be_true);
				be_result =
					Be_Iff(be_mgr, be_TRUE_i_0, be_true);
				nusmv_assert((be_ptr)NULL != be_result);
         ...
         */
				...
				nusmv_assert((be_ptr)NULL != be_false);
				be_result =
					Be_Iff(be_mgr, be_FALSE_i_0, be_false);
				nusmv_assert((be_ptr)NULL != be_result);
		...
		case OR: {
			...
			rsf_past_array = array_fetch(
				array_t *,
				sbmc_node_info_get_trans_bes(rsf_info), i_real);
			nusmv_assert((array_t *)NULL != rsf_past_array);
			...
			if (sbmc_node_info_get_trans_vars(info) ==
				...
				for (d = 0;
				     ...
				     d++) {
					/* [[f | g]]_i^d <=> [[f]]_i^d | [[g]]_i^d */
					be_ptr be_fORg_i_d, be_f_i_d, be_g_i_d,
						be_result;
					const unsigned int d_lsf = min(
						d,
						sbmc_node_info_get_past_depth(
					...
					be_fORg_i_d = array_fetch(
						be_ptr, past_array, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fORg_i_d);
					be_f_i_d = array_fetch(
						be_ptr, lsf_past_array, d_lsf);
					nusmv_assert((be_ptr)NULL != be_f_i_d);
					be_g_i_d = array_fetch(
						be_ptr, rsf_past_array, d_rsf);
					nusmv_assert((be_ptr)NULL != be_g_i_d);
					be_result =
						Be_Iff(be_mgr, be_fORg_i_d,
		...
		case AND: {
			...
			if (sbmc_node_info_get_trans_vars(info) ==
				...
				for (d = 0;
				     ...
				     d++) {
					/* [[f & g]]_i^d <=> [[f]]_i^d & [[g]]_i^d */
					be_ptr be_fANDg_i_d, be_f_i_d, be_g_i_d,
						be_result;
					const unsigned int d_lsf = min(
						d,
						sbmc_node_info_get_past_depth(
		...
		case NOT: {
			...
			if (sbmc_node_info_get_trans_vars(info) ==
				...
				for (d = 0;
				     d <= sbmc_node_info_get_past_depth(info);
				     d++) {
					if (SymbTable_is_symbol_input_var(
						    st, lsf)) {
						/* An input variable. */
						/* [[!f]]_i^0 <=> ![[f]]_i^d & (!LastState_i | LoopExists) */
						be_ptr be_notf_i_d, be_f_i_d,
							be_result;
						nusmv_assert(be_LastState_i !=
							     (be_ptr)NULL);

						be_notf_i_d = array_fetch(
							be_ptr, past_array, d);
						nusmv_assert((be_ptr)NULL !=
							     be_notf_i_d);
						be_f_i_d = array_fetch(
							be_ptr, lsf_past_array,
							d);
						nusmv_assert((be_ptr)NULL !=
							     be_f_i_d);
						be_result = Be_Iff(
		...
		case OP_FUTURE: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				nusmv_assert((be_ptr)NULL != be_lsf);
				be_aux_next =
					array_fetch(be_ptr, next_past_array, d);
			...
			if (do_optimization) {
				...
				be_Ff_i_pd = array_fetch(
					be_ptr, past_array,
					sbmc_node_info_get_past_depth(info));
				nusmv_assert((be_ptr)NULL != be_Ff_i_pd);

				be_result = Be_Implies(be_mgr, be_auxFf_i,
						       be_Ff_i_pd);
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Ff_i_dP1, be_Ff_i_d,
						be_result;

					be_Ff_i_dP1 = array_fetch(
						be_ptr, past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Ff_i_dP1);

					be_Ff_i_d = array_fetch(be_ptr,
								past_array, d);
					nusmv_assert((be_ptr)NULL != be_Ff_i_d);

					be_result = Be_Implies(
						be_mgr, be_InLoop_i,
						Be_Implies(be_mgr, be_Ff_i_dP1,
							   be_Ff_i_d));
				...
				}
				{
					...
					for (d = 0;
					     d < sbmc_node_info_get_past_depth(
							 info);
					     d++) {
						be_ptr be_Ff_i_d, be_Ff_E_d,
							be_result;

						be_Ff_i_d = array_fetch(
							be_ptr, past_array, d);
						nusmv_assert((be_ptr)NULL !=
							     be_Ff_i_d);

						be_Ff_E_d = array_fetch(
							be_ptr, past_array_Ff_E,
							d);
						nusmv_assert((be_ptr)NULL !=
							     be_Ff_E_d);
		...
		case OP_GLOBAL: {
			...
			if (do_optimization) {
				...
				be_Gf_i_pd = array_fetch(
					be_ptr, past_array,
					sbmc_node_info_get_past_depth(info));
				nusmv_assert(be_Gf_i_pd);

				be_result = Be_Implies(be_mgr, be_Gf_i_pd,
						       be_auxGf_i);
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Gf_i_d, be_Gf_i_dP1,
						be_result;

					be_Gf_i_d = array_fetch(be_ptr,
								past_array, d);
					nusmv_assert(be_Gf_i_d);

					be_Gf_i_dP1 = array_fetch(
						be_ptr, past_array, d + 1);
					nusmv_assert(be_Gf_i_dP1);

					be_result = Be_Implies(
						be_mgr, be_InLoop_i,
						Be_Implies(be_mgr, be_Gf_i_d,
							   be_Gf_i_dP1));
				...
				}
				{
					...
					for (d = 0;
					     d < sbmc_node_info_get_past_depth(
							 info);
					     d++) {
						be_ptr be_Gf_i_d, be_Gf_E_d,
							be_result;

						be_Gf_i_d = array_fetch(
							be_ptr, past_array, d);
						nusmv_assert((be_ptr)NULL !=
							     be_Gf_i_d);

						be_Gf_E_d = array_fetch(
							be_ptr, past_array_Gf_E,
							d);
						nusmv_assert((be_ptr)NULL !=
							     be_Gf_E_d);
		...
		case UNTIL: {
			...
			nusmv_assert((sbmc_node_info *)NULL != lsf_info);
			lsf_past_array = array_fetch(
				be_ptr, sbmc_node_info_get_trans_bes(lsf_info),
				i_real);
			nusmv_assert((array_t *)NULL != lsf_past_array);
			...
			nusmv_assert((sbmc_node_info *)NULL != rsf_info);
			rsf_past_array = array_fetch(
				be_ptr, sbmc_node_info_get_trans_bes(rsf_info),
				i_real);
			nusmv_assert((array_t *)NULL != rsf_past_array);
         ...
         */
				be_ptr be_fUg_i_d, be_f_i_d, be_g_i_d,
					be_fUg_iP1_d, be_result;
				const unsigned int d_lsf =
					min(d, sbmc_node_info_get_past_depth(
						       lsf_info));
		...
		case RELEASES: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				be_ptr be_fRg_i_d, be_f_i_d, be_g_i_d,
					be_fRg_iP1_d, be_result;
				const unsigned int d_lsf =
					min(d, sbmc_node_info_get_past_depth(
						       lsf_info));
				const unsigned int d_rsf =
		...
		if ((node_ptr)NULL != sbmc_node_info_get_aux_F_node(info)) {
			...
			if (opt_verbose_level_ge(opts, 6))
				sbmc_print_node(
					env, errstream,
					" Doing <<F f>> translation for ", node,
					"\n");

			be_aux_i = BeEnc_name_to_timed(
				be_enc, sbmc_node_info_get_aux_F_node(info),
				i_real);
			nusmv_assert((be_ptr)NULL != be_aux_i);
			...
			if (i_model == 0) {
				...
				be_aux_i_minus_1 = BeEnc_name_to_timed(
					...
					i_real - 1);
				nusmv_assert((be_ptr)NULL != be_aux_i_minus_1);
				be_f_i_pd = array_fetch(
					be_ptr, past_array,
					sbmc_node_info_get_past_depth(info));
				nusmv_assert((be_ptr)NULL != be_f_i_pd);
				be_result =
					Be_Iff(be_mgr, be_aux_i,
					       Be_Or(be_mgr, be_aux_i_minus_1,
						     Be_And(be_mgr, be_InLoop_i,
							    be_f_i_pd)));
			...
			/* Save the created constraint */
			lsNewBegin(created_constraints, (lsGeneric)be_result,
				   LS_NH);
		}
		if (((node_ptr)NULL != sbmc_node_info_get_aux_F_node(info)) &&
		    do_optimization) {
		...
		if ((node_ptr)NULL != sbmc_node_info_get_aux_G_node(info)) {
			...
			if (opt_verbose_level_ge(opts, 6))
				sbmc_print_node(
					env, errstream,
					" Doing <<G f>> translation for ", node,
					"\n");

			be_aux_i = BeEnc_name_to_timed(
				be_enc, sbmc_node_info_get_aux_G_node(info),
				i_real);
			nusmv_assert((be_ptr)NULL != be_aux_i);
       ...
       */
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				be_f_L_d =
					array_fetch(be_ptr, f_L_past_array, d);
				nusmv_assert((be_ptr)NULL != be_f_L_d);

				be_result = Be_Implies(be_mgr, be_l_i,
						       Be_Iff(be_mgr, be_f_L_d,
							      be_f_i_d));
			       ...
			       const int do_optimization)
{
	...
	if (opt_verbose_level_gt(opts, 1)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "Unrolling k-invariant past stuff at time %u\n",
			   i_model);
		fflush(errstream);
	}
	...
	while (lsLength(unprocessed_nodes) > 0) {
		...
		case OP_ONCE: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				if (i_model == 0) {
					/* Add [[O f]]_0^d <=> [[f]]_0^0 */
					be_ptr be_f_0_0 = array_fetch(
						be_ptr, lsf_past_array, 0);
					nusmv_assert((be_ptr)NULL != be_f_0_0);
					be_result = Be_Iff(be_mgr, be_Of_i_d,
							   be_f_0_0);
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_Of_iM1);
					be_Of_iM1_d = array_fetch(
						be_ptr, past_array_Of_iM1, d);
					nusmv_assert((be_ptr)NULL !=
						     be_Of_iM1_d);
           ...
           */
					...
					nusmv_assert((array_t *)NULL !=
						     E_past_array);
					be_E = array_fetch(be_ptr, E_past_array,
							   d - 1);
           ...
           */
			...
			if (do_optimization) {
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Of_i_d, be_Of_i_dP1,
						be_Of_E_d, be_result;

					be_Of_i_d = array_fetch(be_ptr,
								past_array, d);
					nusmv_assert((be_ptr)NULL != be_Of_i_d);

					be_Of_i_dP1 = array_fetch(
						be_ptr, past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Of_i_dP1);
		...
		case OP_HISTORICAL: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				if (i_model == 0) {
					...
					be_result = Be_Iff(be_mgr, be_Hf_i_d,
							   be_f_0_0);
				} else if (i_model >= 1 && d == 0) {
         ...
         * stabilized at the last unrolling depth */
           ...
           */
					...
					nusmv_assert((array_t *)NULL !=
						     Hf_iM1_past_array);
					be_Hf_iM1_d = array_fetch(
						be_ptr, Hf_iM1_past_array, d);
					nusmv_assert((be_ptr)NULL !=
						     be_Hf_iM1_d);
			...
			if (do_optimization) {
				...
				for (d = 0;
				     d < sbmc_node_info_get_past_depth(info);
				     d++) {
					be_ptr be_Hf_i_d, be_Hf_i_dP1,
						be_Hf_E_d, be_result;

					be_Hf_i_d = array_fetch(be_ptr,
								past_array, d);
					nusmv_assert((be_ptr)NULL != be_Hf_i_d);

					be_Hf_i_dP1 = array_fetch(
						be_ptr, past_array, d + 1);
					nusmv_assert((be_ptr)NULL !=
						     be_Hf_i_dP1);
					...
					be_Hf_E_d = array_fetch(
						be_ptr, past_array_Hf_E, d);
					nusmv_assert((be_ptr)NULL != be_Hf_E_d);

					be_result = Be_Implies(
						be_mgr, be_InLoop_i,
						Be_Implies(be_mgr, be_Hf_E_d,
							   be_Hf_i_d));
		...
		case SINCE: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				if (i_model == 0) {
					...
					be_ptr be_g_0_0 = array_fetch(
						be_ptr, rsf_past_array, 0);
					...
					be_f_i_d = array_fetch(
						be_ptr, lsf_past_array, d);
					nusmv_assert((be_ptr)NULL != be_f_i_d);

					be_g_i_d = array_fetch(
						be_ptr, rsf_past_array, d);
					nusmv_assert((be_ptr)NULL != be_g_i_d);
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_fSg_iM1);
					be_fSg_iM1_d = array_fetch(
						be_ptr, past_array_fSg_iM1, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fSg_iM1_d);
           ...
           */
					be_ptr be_f_i_d, be_g_i_d, be_fSg_E_dM1,
						be_fSg_iM1_d;
					array_t *past_array_fSg_iM1,
						*past_array_fSg_E;
					const unsigned int d_lsf = min(
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_fSg_E);
					be_fSg_E_dM1 = array_fetch(
						be_ptr, past_array_fSg_E,
						d - 1);
					nusmv_assert((be_ptr)NULL !=
						     be_fSg_E_dM1);
         ...
         * stabilized at the last unrolling depth */
           ...
           */
					be_ptr be_f_i_d, be_g_i_d, be_fSg_E_d,
						be_fSg_iM1_d;
					array_t *past_array_fSg_iM1,
						*past_array_fSg_E;
					const unsigned int d_lsf = min(
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_fSg_E);
					be_fSg_E_d = array_fetch(
						be_ptr, past_array_fSg_E, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fSg_E_d);
		...
		case TRIGGERED: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
             ...
             [[g]]_i^0 & ([[f]]_i^0 | [[f T g]]_{i-1}^0)*/
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_fTg_iM1);
					be_fTg_iM1_d = array_fetch(
						be_ptr, past_array_fTg_iM1, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fTg_iM1_d);
           ...
           */
					be_ptr be_f_i_d, be_g_i_d, be_fTg_E_dM1,
						be_fTg_iM1_d;
					array_t *past_array_fTg_iM1,
						*past_array_fTg_E;
					const unsigned int d_lsf = min(
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_fTg_E);
					be_fTg_E_dM1 = array_fetch(
						be_ptr, past_array_fTg_E,
						d - 1);
					nusmv_assert((be_ptr)NULL !=
						     be_fTg_E_dM1);
         ...
         * stabilized at the last unrolling depth */
           ...
           */
					be_ptr be_f_i_d, be_g_i_d, be_fTg_E_d,
						be_fTg_iM1_d;
					array_t *past_array_fTg_iM1,
						*past_array_fTg_E;
					const unsigned int d_lsf = min(
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_fTg_E);
					be_fTg_E_d = array_fetch(
						be_ptr, past_array_fTg_E, d);
					nusmv_assert((be_ptr)NULL !=
						     be_fTg_E_d);
		...
		case OP_PREC: {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				if (i_model == 0) {
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_f_iM1);
					be_f_iM1_0 = array_fetch(
						be_ptr, past_array_f_iM1, d);
					nusmv_assert((be_ptr)NULL !=
						     be_f_iM1_0);
					be_result = Be_Iff(be_mgr, be_Yf_i_d,
							   be_f_iM1_0);
           ...
           */
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_f_E);
					be_f_E_dM1 = array_fetch(
						be_ptr, past_array_f_E, d - 1);
					nusmv_assert((be_ptr)NULL !=
						     be_f_E_dM1);
					...
					nusmv_assert((array_t *)NULL !=
						     past_array_f_iM1);
					be_f_iM1_d = array_fetch(
						be_ptr, past_array_f_iM1,
						d_lsf);
					nusmv_assert((be_ptr)NULL !=
						     be_f_iM1_d);
	...
	while (lsLength(unprocessed_nodes) > 0) {
		...
		case OP_FUTURE:
			...
			if (!sbmc_set_is_in(visit_cache, car(node))) {
				lsNewBegin(unprocessed_nodes,
					   (lsGeneric)car(node), LS_NH);
				has_unprocessed_children = 1;
			}
			break;
		...
		default:
			...
			ErrorMgr_internal_error(
				errmgr, "%s:%d: Something not implemented",
				__FILE__, __LINE__);
			break;
		}
		if (has_unprocessed_children)
		...
		if ((array_t *)NULL != sbmc_node_info_get_trans_vars(info)) {
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				be_f_E_d =
					array_fetch(be_ptr, f_E_past_array, d);
				nusmv_assert((be_ptr)NULL != be_f_E_d);
				be_f_k_d =
					array_fetch(be_ptr, f_k_past_array, d);
		...
		if ((array_t *)NULL != sbmc_node_info_get_trans_vars(info)) {
			...
			int d;
			array_t *f_kP1_past_array = array_fetch(
				array_t *, sbmc_node_info_get_trans_bes(info),
			...
			for (d = 0; d <= sbmc_node_info_get_past_depth(info);
				...
				be_f_kP1_d = array_fetch(be_ptr,
							 f_kP1_past_array, d);
				nusmv_assert((be_ptr)NULL != be_f_kP1_d);
				be_f_L_dP1 = array_fetch(
					be_ptr, f_L_past_array,
					min(d + 1,
					    sbmc_node_info_get_past_depth(
						    info)));
				nusmv_assert((be_ptr)NULL != be_f_L_dP1);
				be_result =
					Be_Iff(be_mgr, be_f_kP1_d, be_f_L_dP1);
		...
		if ((node_ptr)NULL != sbmc_node_info_get_aux_F_node(info)) {
			...
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_log(logger, "  <<Ff>>_E <=> <<Ff>>_%d: ",
					   k_model);
				Be_DumpSexpr(be_mgr, be_result, errstream);
				Logger_log(logger, "\n");
			}
		...
		if (sbmc_node_info_get_aux_G_node(info)) {
			...
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_log(logger, "  <<Gf>>_E <=> <<Gf>>_%d: ",
					   k_model);
				Be_DumpSexpr(be_mgr, be_result, errstream);
				Logger_log(logger, "\n");
			}
			     ...
			     const int opt_do_optimization)
{
	...
	for (i = max(previous_k + 1, 0); i <= new_k; i++) {
		...
		be_l_i = BeEnc_name_to_timed(
			be_enc, sbmc_state_vars_get_l_var(state_vars),
			sbmc_real_k(i));
		nusmv_assert((be_ptr)NULL != be_l_i);

		be_LastState_i = BeEnc_name_to_timed(
			be_enc, sbmc_state_vars_get_LastState_var(state_vars),
			sbmc_real_k(i));
...

		{
			...
			lsList new_constraints = sbmc_unroll_invariant_f(
				be_enc, bltlspec, i, info_map, be_InLoop_i,
				be_l_i, be_LastState_i, be_LoopExists,
				opt_do_optimization);
		...
		{
			/* Past fragment */
			lsList new_constraints = sbmc_unroll_invariant_p(
				be_enc, bltlspec, i, info_map, be_InLoop_i,
   ...
   */
	{
		...
		if (opt_verbose_level_gt(opts, 1)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			Logger_log(logger, "Forced (LoopExists <=> InLoop_%d)",
				   k);
			if (opt_verbose_level_ge(opts, 6)) {
				Logger_log(logger, ": ");
				Be_DumpSexpr(be_mgr, be_constraint, errstream);
