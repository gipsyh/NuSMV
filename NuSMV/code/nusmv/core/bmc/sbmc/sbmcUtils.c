...
void sbmc_increment_unique_id(const NuSMVEnv_ptr env)
{
	if (!NuSMVEnv_has_value(env, ENV_SBMC_UNIQUE_ID)) {
		...
		int currval =
			PTR_TO_INT(NuSMVEnv_get_value(env, ENV_SBMC_UNIQUE_ID));
		NuSMVEnv_set_or_replace_value(env, ENV_SBMC_UNIQUE_ID,
				     ...
				     SymbLayer_ptr layer, const char *name)
{
	...
	if (snprintf(uname, uname_size, "%d_%s", sbmc_get_unique_id(env),
		     name) < 0) {
		FREE(uname);
		ErrorMgr_internal_error(errmgr,
					"%s:%d: Unable to create unique string",
					__FILE__, __LINE__);
	}
	...
	if (opt_verbose_level_ge(opts, 2)) {
		...
		for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
			Logger_nlog(logger, wffprint, "[[%N]]^%u = %N;\n", node,
				    d, array_fetch(node_ptr, trans_vars, d));
		}
	}
}
				 ...
				 const unsigned int n, unsigned int *index)
{
	...
	for (i = 0; i < n; i++) {
		array_insert(node_ptr, array, i,
			     sbmc_1_fresh_state_var(env, layer, index));
	}
	return array;
}
			      ...
			      unsigned int *new_var_index)
{
	...
	lsNewBegin(state_vars_formula_pd0,
		   (lsGeneric)array_fetch(node_ptr, array, 0), LS_NH);
	for (d = 1; d <= pd; d++)
...
node_ptr sbmc_make_boolean_formula(BddEnc_ptr bdd_enc, Prop_ptr ltlprop)
{
	...
	fltlspec = Wff_make_not(
		nodemgr,
		Compile_detexpr2bexpr(bdd_enc, Prop_get_expr_core(ltlprop)));
   ...
   */
	{
		...
		justice_list = Bmc_CheckFairnessListForPropositionalFormulae(
			env, j_list);
		j_list = justice_list;
			     ...
			     node_ptr bltlspec)
{
	...
	if (lsLength(sbmc_state_vars_get_simple_path_system_vars(svs)) > 0) {
		lsDestroy(sbmc_state_vars_get_simple_path_system_vars(svs),
			  NULL);
						   ...
						   NodeList_ptr symbols)
{
	...
	Trace_ptr trace = Sbmc_Utils_generate_cntexample(
		be_enc, solver, l_var, k, trace_name, symbols);
...
Trace_ptr Sbmc_Utils_fill_cntexample(BeEnc_ptr be_enc, sbmc_MetaSolver *solver,
				     node_ptr l_var, const int k, Trace_ptr res)
{
	/* local refs */
	...
	for (i = 1; i <= k; ++i) {
		...
		insert_assoc(time_2_step, NODE_FROM_INT(sbmc_real_k(i)),
			     (node_ptr)(step));
	}
  ...
  */
	...
	SLIST_FOREACH(cnf_model, genLiteral)
	{
		...
		beLiteral = (nusmv_ptrint)Be_CnfLiteral2BeLiteral(be_mgr,
								  cnfLiteral);
		...
		if (BeEnc_is_index_frozen_var(be_enc, index) ||
         ...
         the scalar variable which this bit belongs to */
			if (BoolEnc_is_var_bit(bool_enc, var)) {
				...
				scalar_var = BoolEnc_get_scalar_var_from_bit(
					bool_enc, var);
				key = find_node(nodemgr, ATTIME, scalar_var,
						NODE_FROM_INT(vtime));

				bv = BIT_VALUES(find_assoc(tvar_2_bval, key));
				if (BIT_VALUES(NULL) == bv) {
					bv = BitValues_create(bool_enc,
							      scalar_var);
					insert_assoc(tvar_2_bval, key,
						     (node_ptr)(bv));
				}

				/* set the bit value */
				BitValues_set(bv,
					      BoolEnc_get_index_from_bit(
						      bool_enc, var),
					      (beLiteral >= 0) ?
						      BIT_VALUE_TRUE :
						      BIT_VALUE_FALSE);
	...
	{ /* phase 2: iterate over elements of the hash table (i.e. scalar
       vars) and populate the trace accordingly. */
		...
		ASSOC_FOREACH(tvar_2_bval, aiter, &ts_var, &bitValues)
		{
			...
			node_ptr value = BoolEnc_get_value_from_var_bits(
				bool_enc, bitValues);
			...
			nusmv_assert(TRACE_END_ITER != timed_step);
			Trace_step_put_value(res, timed_step, car(ts_var),
					     value);
...
sbmc_MetaSolver *sbmc_MS_create(BeEnc_ptr be_enc)
{
	...
	ms->permanent_group =
		SatSolver_get_permanent_group(SAT_SOLVER(ms->solver));
...
{
	METASOLVERCHECK(ms);
	return SatSolver_solve_all_groups_assume(SAT_SOLVER(ms->solver),
						 assumptions);
}
...
void sbmc_add_loop_variable(BddEnc_ptr bdd_enc, BeFsm_ptr be_fsm)
{
	...
	loop_var_name =
		sbmc_add_new_state_variable(env, ltl_layer, "#SBMC_LTL_l");
