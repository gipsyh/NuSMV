			  ...
			  boolean crossComparison, int k, int l)
{
	...
	if (wff == NULL) {
		...
		switch (wff_operator) {
		case GWO_None:
			wff = bmc_test_gen_wff(be_enc, max_depth, max_conns,
					       usePastOperators);
			break;
		...
		case GWO_Globally:
			wff = Wff_make_globally(
				nodemgr,
				bmc_test_gen_wff(be_enc, max_depth, max_conns,
						 usePastOperators));
			break;
		...
		case GWO_Once:
			wff = Wff_make_once(nodemgr,
					    bmc_test_gen_wff(be_enc, max_depth,
							     max_conns,
							     usePastOperators));
			break;
	...
	if (!crossComparison) {
		...
		tableau_test = bmc_test_gen_tableau(be_fsm, wff, k, l,
						    usePastOperators);
		...
		Bmc_Utils_ConvertLoopFromInteger(l, szLoopback,
						 sizeof(szLoopback));
		fprintf(f,
			"\n\n-- Property %d (k=%d, l=%s, max_depth=%d, max_conns=%d): \n",
			generated_formulas, k, szLoopback, max_depth,
			max_conns);
		fprintf(f, "LTLSPEC ");
		...
		wff = Wff_make_implies(
			nodemgr, bmc_test_mk_loopback_ltl(be_enc, k, l), wff);
					 ...
					 const int l)
{
	...
	bigand_vars = Wff_make_iff(
		nodemgr, var, Wff_make_opnext_times(nodemgr, var, loop_len));
	...
	while (BeEnc_is_var_index_valid(be_enc, idx)) {
		...
		single_var_eq = Wff_make_iff(nodemgr, var,
					     Wff_make_opnext_times(nodemgr, var,
								   loop_len));
				     ...
				     const int l, boolean usePastOperators)
{
	...
	if (Bmc_Utils_IsNoLoopback(l)) {
		implies_formula =
			Wff_make_implies(nodemgr, tableau_as_wff, ltl_nnf_wff);
	} else {
		nusmv_assert(!Bmc_Utils_IsAllLoopbacks(
			l)); /* all loops are not allowed nowadays */
				 ...
				 int max_conns, boolean usePastOperators)
{
	...
	do {
		...
		rnd = (int)floor((GEN_WFF_CONSES_OP_NUMBER +
	...
	if ((max_depth < 0) || (max_conns < 0)) {
		...
		idx = BeEnc_get_var_index_with_offset(
			be_enc,
			BeEnc_get_first_untimed_var_index(be_enc,
							  BE_VAR_TYPE_CURR),
	...
	case 1:
		return Wff_make_not(
			nodemgr, bmc_test_gen_wff(be_enc, max_depth, max_conns,
						  usePastOperators));
				  ...
				  const node_ptr bexp, const int output_type)
{
	...
	case FALSEEXP: /* FALSEEXP  */
		fprintf(f, "%s",
			(output_type == BMC_BEXP_OUTPUT_SMV) ? "FALSE" :
							       "false");
		break;

	case TRUEEXP: /* TRUEEXP   */
		fprintf(f, "%s",
			(output_type == BMC_BEXP_OUTPUT_SMV) ? "TRUE" : "true");
		break;
	...
	case AND: /* AND       */
		...
		fprintf(f, " %s ",
			(output_type == BMC_BEXP_OUTPUT_SMV) ? "&" : "/\\");
	...
	default: /* (default action) */
	{
       ...
       of correspondent be variable */
		if (r != (be_ptr)NULL) {
			fprintf(f, "p%d",
				Be_Var2Index(BeEnc_get_be_manager(be_enc), r));
		} else {
