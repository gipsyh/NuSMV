...
be_ptr BmcInt_Tableau_GetAtTime(const BeEnc_ptr be_enc, const node_ptr ltl_wff,
  ...
  case UNTIL:
    result = bmc_tableauGetUntilAtTime(be_enc, car(ltl_wff), cdr(ltl_wff), time,
                                       k, l);
    break;
...
hash_ptr Bmc_Tableau_get_handled_hash(SymbTable_ptr symb_table,
  ...
  if (!strcmp(ST_BMC_TABLEAU_LTL_HASH, hash_str)) {
    return SymbTable_get_handled_hash_ptr(
        symb_table, ST_BMC_TABLEAU_LTL_HASH, (ST_PFICPCP)NULL, (ST_PFICPI)NULL,
...
static be_ptr bmc_tableauGetUntilAtTime_aux(const BeEnc_ptr be_enc,
  ...
  if (steps > 1) {
    tableau_following = bmc_tableauGetUntilAtTime_aux(
        be_enc, p, q, Bmc_Utils_GetSuccTime(time, k, l), k, l, steps - 1);

    tableau_temp =
        Be_Or(be_mgr, tableau_temp,
              Be_And(be_mgr, BmcInt_Tableau_GetAtTime(be_enc, p, time, k, l),
                     tableau_following));
  }
  return tableau_temp;
}
...
static be_ptr bmc_tableauGetReleasesAtTime_aux(const BeEnc_ptr be_enc,
  ...
  if (steps == 1) {
    if (Bmc_Utils_IsNoLoopback(l)) { /* q & p */
      tableau_result =
          Be_And(BeEnc_get_be_manager(be_enc), tableau_p, tableau_q);
    } else { /* q */
    ...
    be_ptr tableau_following = bmc_tableauGetReleasesAtTime_aux(
        be_enc, p, q, Bmc_Utils_GetSuccTime(time, k, l), k, l, steps - 1);
    tableau_result = Be_And(
        BeEnc_get_be_manager(be_enc), tableau_q,
        Be_Or(BeEnc_get_be_manager(be_enc), tableau_p, tableau_following));
  }
