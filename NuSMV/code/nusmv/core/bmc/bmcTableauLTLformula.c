...
/*---------------------------------------------------------------------------*/
static be_ptr bmc_tableauGetUntilAtTime_aux(const BeEnc_ptr be_enc,
					    const node_ptr p, const node_ptr q,
					    const int time, const int k,
					    const int l, const int steps);

static be_ptr bmc_tableauGetReleasesAtTime_aux(const BeEnc_ptr be_enc,
					       const node_ptr p,
				...
				const int time, const int k, const int l)
{
	...
	case DOT:
		...
		return BeEnc_name_to_timed(be_enc, ltl_wff,
					   time); /* not memoized */
	...
	case AND:
		result = Be_And(be_mgr,
				BmcInt_Tableau_GetAtTime(be_enc, car(ltl_wff),
							 time, k, l),
				BmcInt_Tableau_GetAtTime(be_enc, cdr(ltl_wff),
							 time, k, l));
		break;
	...
	case OP_NEXT:
		result = bmc_tableauGetNextAtTime(be_enc, car(ltl_wff), time, k,
						  l);
		break;

	case OP_GLOBAL:
		result = bmc_tableauGetGloballyAtTime(be_enc, car(ltl_wff),
						      time, k, l);
		break;

	case OP_FUTURE: /* EVENTUALLY */
		result = bmc_tableauGetEventuallyAtTime(be_enc, car(ltl_wff),
							time, k, l);

		break;

	case UNTIL:
		result = bmc_tableauGetUntilAtTime(be_enc, car(ltl_wff),
						   cdr(ltl_wff), time, k, l);
		break;

	case RELEASES:
		result = bmc_tableauGetReleasesAtTime(be_enc, car(ltl_wff),
						      cdr(ltl_wff), time, k, l);
		break;
	...
	case ABG:
		ErrorMgr_internal_error(
			errmgr, "Unexpected CTL operator, node type %d\n",
				...
				const int time, const int k, const int l)
{
	...
	if (!Bmc_Utils_IsNoLoopback(succtime)) {
		tableau = BmcInt_Tableau_GetAtTime(be_enc, ltl_wff, succtime, k,
						   l);
	} else {
				      ...
				      const int k, const int l)
{
	...
	nusmv_assert((intime < k) ||
		     (intime == k && Bmc_Utils_IsNoLoopback(l)));
	...
	for (time = start_time; time >= stop_time; --time) {
		/* lazy evaluation: */
		be_ptr tableau_at_time =
			BmcInt_Tableau_GetAtTime(be_enc, ltl_wff, time, k, l);

		if (Be_IsTrue(be_mgr, tableau_at_time)) {
			tableau = tableau_at_time;
			break;
		}
		tableau = Be_Or(be_mgr, tableau_at_time, tableau);
	} /* loop */
				    ...
				    const int k, const int l)
{
	...
	if (Bmc_Utils_IsNoLoopback(l)) {
		...
		for (time = k - 1; time >= stop_time; --time) {
			/* lazy evaluation: */
			be_ptr tableau_at_time = BmcInt_Tableau_GetAtTime(
				be_enc, ltl_wff, time, k, l);
			if (Be_IsFalse(be_mgr, tableau_at_time)) {
				tableau = tableau_at_time;
				break;
			}
			tableau = Be_And(be_mgr, tableau_at_time, tableau);
		}
	}
				    ...
				    const int k, const int l)
{
	...
	return bmc_tableauGetReleasesAtTime_aux(be_enc, p, q, time, k, l,
						steps);
}
...
hash_ptr Bmc_Tableau_get_handled_hash(SymbTable_ptr symb_table, char *hash_str)
{
	if (!strcmp(ST_BMC_TABLEAU_LTL_HASH, hash_str)) {
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_BMC_TABLEAU_LTL_HASH, (ST_PFICPCP)NULL,
	...
	return find_node(nodemgr, CONS, wff,
			 find_node(nodemgr, CONS, PTR_FROM_INT(node_ptr, time),
				   find_node(nodemgr, CONS,
					     PTR_FROM_INT(node_ptr, k),
					     PTR_FROM_INT(node_ptr, l))));
}

void bmc_tableau_memoization_insert(hash_ptr tableau_ltl_hash, node_ptr key,
				    be_ptr be)
{
...
be_ptr bmc_tableau_memoization_lookup(hash_ptr tableau_ltl_hash, node_ptr key)
{
	nusmv_assert(tableau_ltl_hash != (hash_ptr)NULL);
					    ...
					    const int l, const int steps)
{
	...
	if (steps > 1) {
		tableau_following = bmc_tableauGetUntilAtTime_aux(
			be_enc, p, q, Bmc_Utils_GetSuccTime(time, k, l), k, l,
			steps - 1);
					       ...
					       const int steps)
{
	...
	if (steps == 1) {
		...
		be_ptr tableau_following = bmc_tableauGetReleasesAtTime_aux(
			be_enc, p, q, Bmc_Utils_GetSuccTime(time, k, l), k, l,
			steps - 1);
		tableau_result = Be_And(BeEnc_get_be_manager(be_enc), tableau_q,
