	...
	for (increasingK = k_min; (increasingK <= k_max) && !found_solution;
       ...
       if it were relative: */
		...
		StreamMgr_print_output(streams,
				       ":START:benchmarking Generation\n");
		start_time = clock();
#endif

		prob = Bmc_Gen_LtlProblem(be_fsm, bltlspec, increasingK, l);
		prob = Bmc_Utils_apply_inlining(be_mgr,
						prob); /* inline if needed */
		...
		if (dump_type != BMC_DUMP_NONE) {
			...
			Bmc_Dump_WriteProblem(be_enc, cnf, ltlprop, increasingK,
					      l, dump_type,
					      dump_fname_template);
		}
		...
		if (must_solve) {
			...
			/* Sat construction */
			solver = Sat_CreateNonIncSolver(env,
							get_sat_solver(opts));
			...
			case SAT_SOLVER_UNSATISFIABLE_PROBLEM: {
				char szLoopMsg
					[16]; /* for loopback part of message */
			...
			case SAT_SOLVER_SATISFIABLE_PROBLEM:
				...
				if (opt_counter_examples(opts)) {
					...
					bsexp_fsm =
						Prop_get_bool_sexp_fsm(ltlprop);
					...
					trace = Bmc_Utils_generate_and_print_cntexample(
						be_enc, tm, solver, prob,
						increasingK,
						"BMC Counterexample",
						SexpFsm_get_symbols_list(
							SEXP_FSM(bsexp_fsm)));

					Prop_set_trace(ltlprop,
						       Trace_get_id(trace));
				}
		      ...
		      const char *dump_fname_template)
{
	...
	if (dump_type != BMC_DUMP_NONE) {
		...
		Bmc_Dump_WriteProblem(be_enc, cnf, invarprop, 1,
				      Bmc_Utils_GetNoLoopback(), dump_type,
				      dump_fname_template);
	...
	case SAT_SOLVER_SATISFIABLE_PROBLEM:
		...
		if (opt_counter_examples(opts)) {
			...
			*trace = Bmc_Utils_generate_cntexample(
				be_enc, solver, prob, 1, "BMC Failed Induction",
				symbols);
		}
		result = BMC_UNKNOWN;
		break;
				   ...
				   boolean use_extra_step)
{
	...
	result = Bmc_een_sorensson_algorithm(
		env, be_fsm, Prop_get_bool_sexp_fsm(invarprop), binvarspec,
		max_k, dump_type, dump_fname_template, invarprop, oldprop, true,
		use_extra_step, &trace);
	...
	case BMC_FALSE:
		...
		if (opt_counter_examples(opts)) {
			...
			TraceMgr_register_trace(TRACE_MGR(NuSMVEnv_get_value(
							env, ENV_TRACE_MGR)),
						trace);
...
Bmc_result Bmc_een_sorensson_algorithm_without_dump(
	...
	node_ptr binvarspec, int max_k, boolean use_extra_step,
	Trace_ptr *trace)
{
	return Bmc_een_sorensson_algorithm(env, be_fsm, bool_fsm, binvarspec,
					   max_k, BMC_DUMP_NONE, NULL,
					   PROP(NULL), PROP(NULL), false,
					   use_extra_step, trace);
}
	...
	boolean print_steps, boolean use_extra_step, Trace_ptr *trace)
{
	...
	crnt_state_be_vars = Bmc_Utils_get_vars_list_for_uniqueness_fsm(
		be_enc, (SexpFsm_ptr)bool_fsm);
	...
	while (!solved && (k <= max_k)) {
		...
		if (dump_type != BMC_DUMP_NONE) {
			...
			strncat(template_name, "_base",
				sizeof(template_name) - strlen(template_name) -
					1);
			template_name[sizeof(template_name) - 1] =
				'\0'; /* terms the string */

			Bmc_Dump_WriteProblem(be_enc, cnf, pp, 1,
		...
		if (!solved) {
         ...
         ^ loopFree(0..i)
      */
			be_ptr be_steps[3] = { (be_ptr)NULL, (be_ptr)NULL,
					       (be_ptr)NULL };
			be_ptr be_unique;
			int j;
			...
			if (use_extra_step) {
				...
				be_steps[1] =
					Be_And(be_mgr, be_steps[0], be_init);
			}
       ...
       * in frame 0 */
			...
			for (i = 0; i < k; i++) {
				for (j = 0; j < i; j++) {
					...
					lsForEachItem(crnt_state_be_vars, gen,
						      be_var)
					{
						be_ptr be_xor = Be_Xor(
							...
							BeEnc_untimed_expr_to_timed(
								be_enc, be_var,
								j));
						not_equal = Be_Or(be_mgr,
								  not_equal,
								  be_xor);
					}

					be_unique = Be_And(be_mgr, be_unique,
							   not_equal);
				}
			} /* for i */
			be_steps[0] = Be_And(be_mgr, be_steps[0], be_unique);
			be_steps[0] =
				Bmc_Utils_apply_inlining(be_mgr, be_steps[0]);

			if (use_extra_step) {
				be_steps[1] =
					Be_And(be_mgr, be_steps[1], be_unique);
				be_steps[1] = Bmc_Utils_apply_inlining(
					be_mgr, be_steps[1]);
			}

			nusmv_assert(use_extra_step ||
				     (be_ptr)NULL == be_steps[1]);

			/* SAT problem solving */
			for (i = 0; ((be_ptr)NULL != be_steps[i]) && !solved;
			     ++i) {
				SatSolver_ptr solver;
				SatSolverResult sat_res;
				...
				if (dump_type != BMC_DUMP_NONE) {
					...
					cnf = Be_ConvertToCnf(be_mgr,
							      be_steps[i], 1,
							      cnf_alg);

					strncpy(template_name,
						dump_fname_template,
						sizeof(template_name) - 2);
					template_name[sizeof(template_name) - 1] =
						'\0'; /* terms the string */
				...
				/* Sat construction */
				solver = Sat_CreateNonIncSolver(
					env, get_sat_solver(opts));
				if (solver == SAT_SOLVER(NULL)) {
					StreamMgr_print_error(
						streams,
						"Non-incremental sat solver '%s' is not available.\n",
