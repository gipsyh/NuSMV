...
static be_ptr Bmc_TableauLTL_GetSingleLoopWithFairness(const BeFsm_ptr be_fsm,
  ...
  be_ptr tableau =
      BmcInt_Tableau_GetAtTime(BeFsm_get_be_encoding(be_fsm), ltl_wff, 0, k, l);
  return Be_And(BeEnc_get_be_manager(BeFsm_get_be_encoding(be_fsm)), tableau,
                fairness);
}
...
static be_ptr Bmc_TableauLTL_GetSingleLoop(const BeFsm_ptr be_fsm,
                                           ...
                                           const int l) {
  be_ptr loopback =
      Bmc_Tableau_GetLoopCondition(BeFsm_get_be_encoding(be_fsm), k, l);

  be_ptr tableau =
      Bmc_TableauLTL_GetSingleLoopWithFairness(be_fsm, ltl_wff, k, l);

  return Be_And(BeEnc_get_be_manager(BeFsm_get_be_encoding(be_fsm)), loopback,
                tableau);
}
...
static be_ptr Bmc_TableauLTL_GetAllLoopsDepth1(const BeFsm_ptr be_fsm,
  ...
  case OP_NEXT:
    if (k == 0) {
      result = Be_And(
          ...
          Bmc_TableauLTL_GetSingleLoopWithFairness(be_fsm, car(ltl_wff), k, 0),
  ...
  case RELEASES: /* RELEASES     */
    result =
        Be_And(BeEnc_get_be_manager(be_enc),
               ...
               bmc_tableauGetGloballyAtTime(be_enc, cdr(ltl_wff), 0, k, 0));
    break;
  ...
  case ARRAY:
    ErrorMgr_internal_error(
        errmgr, "Unexpected formula with depth zero had been found.\n");
...
static be_ptr Bmc_TableauPLTL_GetAllLoopsDepth1(const BeFsm_ptr be_fsm,
  ...
  case RELEASES:
    result =
        Be_And(be_mgr, Bmc_Tableau_GetAllLoopsDisjunction(be_enc, k),
  ...
  default:
    result = Be_And(be_mgr, Bmc_Tableau_GetAllLoopsDisjunction(be_enc, k),
...
static be_ptr Bmc_Tableau_GetAllLoopsDisjunction(const BeEnc_ptr be_enc,
  ...
  for (l = 0; l < k; l++) {
    result = Be_Or(be_mgr, Bmc_Tableau_GetLoopCondition(be_enc, k, l), result);
  }
