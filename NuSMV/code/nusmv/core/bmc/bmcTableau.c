...
static be_ptr Bmc_Tableau_GetLoopCondition(const BeEnc_ptr be_enc, const int k,
					   const int l);

static be_ptr Bmc_Tableau_GetAllLoopsDisjunction(const BeEnc_ptr be_enc,
						 const int k);

/* Tableaux for LTL: */
static be_ptr Bmc_TableauLTL_GetNoLoop(const BeFsm_ptr be_fsm,
				       const node_ptr ltl_wff, const int k);
static be_ptr Bmc_TableauLTL_GetSingleLoop(const BeFsm_ptr be_fsm,
					   const node_ptr ltl_wff, const int k,
					   const int l);
static be_ptr Bmc_TableauLTL_GetAllLoops(const BeFsm_ptr be_fsm,
					 const node_ptr ltl_wff, const int k,
					 const int l);
static be_ptr Bmc_TableauLTL_GetAllLoopsDepth1(const BeFsm_ptr be_fsm,
					       const node_ptr ltl_wff,
					       const int k);

/* Tableaux for PLTL: */
static be_ptr Bmc_TableauPLTL_GetNoLoop(const BeFsm_ptr be_fsm,
					const node_ptr ltl_wff, const int k);
static be_ptr Bmc_TableauPLTL_GetSingleLoop(const BeFsm_ptr be_fsm,
					    const node_ptr ltl_wff, const int k,
					    const int l);
static be_ptr Bmc_TableauPLTL_GetAllLoops(const BeFsm_ptr be_fsm,
					  const node_ptr ltl_wff, const int k,
					  const int l);
static be_ptr Bmc_TableauPLTL_GetAllLoopsDepth1(const BeFsm_ptr be_fsm,
						const node_ptr ltl_wff,
						const int k);
				 ...
				 const int k, const int l)
{
	...
	if (Bmc_Utils_IsAllLoopbacks(l)) {
		...
		be_ptr tableau_noloop =
			Bmc_Tableau_GetNoLoop(be_fsm, ltl_wff, k);
		...
		if (k == 0) {
			...
			if ((fairness == NULL) && (depth == 0)) {
				...
				tableau_loops = Bmc_Tableau_GetAllLoopsDepth1(
					be_fsm, ltl_wff, k);
			} else {
				tableau_loops = Bmc_Tableau_GetAllLoops(
					be_fsm, ltl_wff, k, 0);
			}
		} else {
			/* do not use depth1 optimization: */
			tableau_loops =
				Bmc_Tableau_GetAllLoops(be_fsm, ltl_wff, k, 0);
		}
...
static be_ptr Bmc_TableauLTL_GetSingleLoopWithFairness(const BeFsm_ptr be_fsm,
						       const node_ptr ltl_wff,
						       const int k, const int l)
{
...
static be_ptr Bmc_TableauLTL_GetSingleLoop(const BeFsm_ptr be_fsm,
					   ...
					   const int l)
{
	be_ptr loopback = Bmc_Tableau_GetLoopCondition(
		BeFsm_get_be_encoding(be_fsm), k, l);

	be_ptr tableau =
		Bmc_TableauLTL_GetSingleLoopWithFairness(be_fsm, ltl_wff, k, l);
				       ...
				       const node_ptr ltl_wff, const int k)
{
	...
	be_ptr fairness_k =
		Bmc_Model_GetFairness(be_fsm, k, Bmc_Utils_GetNoLoopback());

	/* lazy evaluation: */
	if (Be_IsFalse(BeEnc_get_be_manager(be_enc), fairness_k)) {
		return fairness_k;
	} else {
		be_ptr tableau_k = BmcInt_Tableau_GetAtTime(
			be_enc, ltl_wff, 0, k, Bmc_Utils_GetNoLoopback());
		return Be_And(BeEnc_get_be_manager(be_enc), fairness_k,
			      tableau_k);
	}
}
					 ...
					 const int l)
{
	...
	for (j = l; j < k; ++j) {
		be_ptr tableau_k_j = Bmc_TableauLTL_GetSingleLoopWithFairness(
			be_fsm, ltl_wff, k, j);
					       ...
					       const int k)
{
	...
	case ARRAY:
		ErrorMgr_internal_error(
			errmgr,
			"Unexpected formula with depth zero had been found.\n");

	default:
		result = Be_And(BeEnc_get_be_manager(be_enc),
				Bmc_Tableau_GetAllLoopsDisjunction(be_enc, k),
				BmcInt_Tableau_GetAtTime(be_enc, ltl_wff, 0, k,
							 0));
	}
					...
					const node_ptr ltl_wff, const int k)
{
	...
	be_ptr tableau_k = Be_IsFalse(be_mgr, fairness_k) ?
				   Be_Falsity(be_mgr) :
				   Bmc_TableauPLTL_GetTableau(be_enc, ltl_wff,
							      k, noLoopBack);
						...
						const int k)
{
	...
	case OP_NEXT:
		result = (k > 1) ?
				 Be_Falsity(be_mgr) :
				 Bmc_TableauPLTL_GetSingleLoop(be_fsm, pltl_wff,
							       k, 0);
		break;
	...
	default:
		result = Be_And(
			be_mgr, Bmc_Tableau_GetAllLoopsDisjunction(be_enc, k),
