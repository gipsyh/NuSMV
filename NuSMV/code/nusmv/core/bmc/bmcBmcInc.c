...
bmc_add_be_into_solver(SatSolver_ptr solver, SatSolverGroup group, be_ptr prob,
		       int polarity, Be_CnfAlgorithm cnf_alg, BeEnc_ptr be_enc);
...
static be_ptr bmc_build_uniqueness(const BeFsm_ptr be_fsm,
		       ...
		       const int relative_loop, const boolean must_inc_length)
{
	...
	int k_min = 0;
	int increasingK;
	int previousIncreasingK =
		0; /* used to create Be of execution from time 0 */
	...
	bmc_add_be_into_solver_positively(
		SAT_SOLVER(solver),
		SatSolver_get_permanent_group(SAT_SOLVER(solver)), beInit,
		be_enc, cnf_alg);
	...
	for (increasingK = k_min; (increasingK <= k_max && !found_solution);
		...
		SatSolverGroup additionalGroup =
			SatIncSolver_create_group(solver);
       ...
       if it were relative: */
		...
		Bmc_Utils_ConvertLoopFromInteger(relative_loop, szLoop,
						 sizeof(szLoop));
		...
		if (Bmc_Utils_IsSingleLoopback(l) &&
		    ((l >= increasingK) || (l < 0))) {
			StreamMgr_print_error(
				streams,
		...
		if (previousIncreasingK < increasingK) {
			beProb = Bmc_Model_GetUnrolling(
				be_fsm, previousIncreasingK, increasingK);
		...
		beProb = Bmc_Tableau_GetLtlTableau(be_fsm, bltlspec,
						   increasingK, l);
		bmc_add_be_into_solver_positively(SAT_SOLVER(solver),
						  additionalGroup, beProb,
						  be_enc, cnf_alg);
		...
		case SAT_SOLVER_UNSATISFIABLE_PROBLEM: {
			...
			if (Bmc_Utils_IsAllLoopbacks(l)) {
				...
				strncpy(szLoopMsg, " and no loop",
					sizeof(szLoopMsg) - 1);
			} else {
				/* loop is Natural: */
				strncpy(szLoopMsg, " and loop at ",
					sizeof(szLoopMsg) - 1);
				strncat(szLoopMsg, szLoop,
					sizeof(szLoopMsg) - 1 -
						strlen(szLoopMsg));
			}
			...
			if (opt_verbose_level_gt(opts, 2)) {
				...
				print_spec(Logger_get_ostream(logger),
					   inputprop,
					   (Prop_PrintFmt)get_prop_print_method(
						   opts));
			}
		...
		case SAT_SOLVER_SATISFIABLE_PROBLEM:
            ...
            */
					Be_And(BeEnc_get_be_manager(be_enc),
					       beInit, beProb),
					increasingK, "BMC Counterexample",
		...
		default:
			ErrorMgr_internal_error(
				errmgr,
				"%s:%d:%s: Unexpected value in satResult (%d)",
			    ...
			    const int max_k)
{
	...
	binvarspec = Wff2Nnf(
		env,
		Compile_detexpr2bexpr(bdd_enc, Prop_get_expr_core(invarprop)));
	...
	/* start of ZifZag algorithm */
	{
		...
		SatIncSolver_ptr solver;
		SatSolverGroup
			group_init; /* a SAT group containing initial state CNF */
       ...
       state uniqueness, taking into account of coi if enabled. */
		crnt_state_be_vars = Bmc_Utils_get_vars_list_for_uniqueness(
			be_enc, invarprop);
		...
		for (stepN = 0; stepN <= max_k; ++stepN) {
			...
			prob_k = BeEnc_untimed_expr_to_timed(be_enc, be_invar,
							     stepN);
			...
			SatSolver_set_polarity(SAT_SOLVER(solver), cnf_prob_k,
					       -1, additionalGroup);

			/* SOLVE (withtout the initial state) */
			satResult = SatIncSolver_solve_without_groups(
				solver, group_list_init);
			...
			case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
				...
				SatIncSolver_destroy_group(solver,
							   additionalGroup);
			...
			satResult =
				SatSolver_solve_all_groups(SAT_SOLVER(solver));
			...
			case SAT_SOLVER_SATISFIABLE_PROBLEM:
				...
				if (opt_counter_examples(opts)) {
					...
					bsexp_fsm = Prop_get_bool_sexp_fsm(
						invarprop);
					if (BOOL_SEXP_FSM(NULL) == bsexp_fsm) {
						bsexp_fsm = BOOL_SEXP_FSM(
							NuSMVEnv_get_value(
			...
			case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
				if (opt_verbose_level_gt(opts, 0)) {
					...
					Logger_log(
						logger,
						"No counter-example path of length %d found\n",
						stepN);
				}
				break;
			...
			{ /* Insert transition relation (stepN,stepN+1) permanently */
				be_ptr unrolling = Bmc_Model_GetUnrolling(
					be_fsm, stepN, stepN + 1);
         ...
         stepN permanently */
			...
			for (l = 0; l < stepN; ++l) {
				...
				{
					be_ptr be_xor = Be_Xor(
						be_mgr,
						BeEnc_untimed_expr_to_timed(
							be_enc, be_var, l),
						BeEnc_untimed_expr_to_timed(
							be_enc, be_var, stepN));
					not_equal = Be_Or(be_mgr, not_equal,
							  be_xor);
				}
			...
			StreamMgr_print_output(
				streams,
				"-- no proof or counterexample found with bound %d",
				stepN);
			if (opt_verbose_level_gt(opts, 2)) {
				Logger_ptr logger = LOGGER(
					NuSMVEnv_get_value(env, ENV_LOGGER));
				Logger_log(logger, " for ");
				print_invar(
					Logger_get_ostream(logger), oldprop,
					(Prop_PrintFmt)get_prop_print_method(
						opts));
			}
			StreamMgr_print_output(streams, "\n");
			  ...
			  bmc_invar_closure_strategy strategy)
{
	...
	if (opt_verbose_level_gt(opts, 2)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger, "The invariant solving algorithm is Dual\n");
		switch (strategy) {
		case BMC_INVAR_BACKWARD_CLOSURE:
			Logger_log(logger, "using backward closure strategy\n");
			break;
		case BMC_INVAR_FORWARD_CLOSURE:
			Logger_log(logger, "using forward closure strategy\n");
			break;
		default:
			error_unreachable_code(); /* unexpected */
		}
	}
	...
	/* start of Dual algorithm */
	{
		...
		{
			solver_step =
				Sat_CreateIncSolver(env, get_sat_solver(opts));
       ...
       state uniqueness, taking into account of coi if enabled. */
		...
		case BMC_INVAR_BACKWARD_CLOSURE: {
			...
			be_ptr invar = BeEnc_untimed_expr_to_timed(be_enc,
								   be_invar, 0);
		...
		for (stepN = 0; stepN <= max_k; ++stepN) {
			...
			/* create a new group in solver */
			additionalGroup =
				SatIncSolver_create_group(solver_base);
			...
			/* SOLVE (base) */
			satResult = SatSolver_solve_all_groups(
				SAT_SOLVER(solver_base));
			...
			case SAT_SOLVER_SATISFIABLE_PROBLEM:
				...
				if (opt_counter_examples(opts)) {
					...
					trace = Bmc_Utils_generate_and_print_cntexample(
						be_enc, tm,
						SAT_SOLVER(solver_base), prob_k,
						stepN, "BMC Counterexample",
						SexpFsm_get_symbols_list(
							SEXP_FSM(bsexp_fsm)));
				...
				SatIncSolver_destroy_group(solver_base,
							   additionalGroup);
			...
			/* Insert transition relation (stepN,stepN+1) permanently */
			unrolling = Bmc_Model_GetUnrolling(be_fsm, stepN,
							   stepN + 1);
           ...
           state stepN */
				...
				case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
					StreamMgr_print_output(streams, "-- ");
					print_invar(
						StreamMgr_get_output_ostream(
							streams),
						oldprop,
						(Prop_PrintFmt)
							get_prop_print_method(
								opts));
					StreamMgr_print_output(streams,
							       "  is true\n");
					Prop_set_status(invarprop, Prop_True);
             ...
             see Bmc_Model_GetUnrolling) */
					...
					trans = BeEnc_untimed_expr_to_times(
						be_enc, BeFsm_get_trans(be_fsm),
						stepN + 1, stepN + 1, stepN + 1,
						stepN);
					/* invars at step stepN */
					invar = BeEnc_untimed_expr_to_timed(
						be_enc, BeFsm_get_invar(be_fsm),
						stepN);
           ...
           to all other states, which are good */
				if (1 < stepN) {
					be_ptr not_equal = bmc_build_uniqueness(
						be_fsm, crnt_state_be_vars, 1,
						1 + stepN);
              ...
              if (stepN == max_k) {
                ...
                StreamMgr_print_output(streams,
					...
					case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
						...
						StreamMgr_print_output(
							streams, "  is true\n");
						Prop_set_status(invarprop,
								Prop_True);

						/* freeing all existing object in the function */
						SatIncSolver_destroy_group(
							solver_step, bugGroup);
						SatIncSolver_destroy(
							solver_step);
						SatIncSolver_destroy(
							solver_base);
						lsDestroy(crnt_state_be_vars,
							  NULL);
					...
					SatIncSolver_destroy_group(solver_step,
								   bugGroup);
				}
	...
	/* start of Falsification algorithm */
	{
       ...
       state uniqueness, taking into account of coi if enabled. */
		...
		for (stepN = 0; stepN <= max_k; stepN += step_k) {
			...
			limit_k = (stepN + step_k <= max_k ? stepN + step_k :
							     max_k + 1);
			/* ---------------------- */
			/* --- Extending base --- */
			/* ---------------------- */
			...
			for (cur_k = stepN; cur_k < limit_k; ++cur_k) {
				/* BE of the invariant at time k */
				be_ptr cur_prob_k = BeEnc_untimed_expr_to_timed(
					be_enc, be_invar, cur_k);
				...
				prob_k = Be_Or(be_mgr, prob_k,
					       Be_Not(be_mgr, cur_prob_k));
			}
			if (limit_k - stepN > 1) {
				be_ptr unrolling = Bmc_Model_GetUnrolling(
					be_fsm, stepN, stepN + limit_k - 1);
			...
			case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
				if (opt_verbose_level_gt(opts, 0)) {
					...
					if (step_k == 1) {
						...
						Logger_log(
							logger,
							"No counter-example path of length between %d and %d found\n",
							stepN, limit_k - 1);
					}
				}
				break;
	...
	for (l = init_state; l < last_state; ++l) {
		...
		lsForEachItem(state_vars, gen, be_var)
		{
			be_ptr be_xor = Be_Xor(
				...
				BeEnc_untimed_expr_to_timed(be_enc, be_var,
							    last_state));
