...
be_ptr Bmc_Gen_InvarBaseStep(const BeFsm_ptr be_fsm, const node_ptr wff) {
  ...
  be_ptr P_0 =
      BeEnc_untimed_expr_to_timed(be_enc, Bmc_Conv_Bexp2Be(be_enc, wff), 0);

  return Be_Implies(be_mgr,
                    Be_And(be_mgr, Bmc_Model_GetInit0(be_fsm),
...
be_ptr Bmc_Gen_InvarInductStep(const BeFsm_ptr be_fsm, const node_ptr wff) {
  ...
  be_ptr trans_01_invar_01_P0 = Be_And(
      be_mgr, trans_01_invar_01, BeEnc_untimed_expr_to_timed(be_enc, P, 0));
...
be_ptr Bmc_Gen_UnrollingFragment(BeFsm_ptr be_fsm, const int i) {
  ...
  return Be_And(be_mgr, Bmc_Model_GetInvarAtTime(be_fsm, i - 1),
...
int Bmc_Gen_check_psl_property(NuSMVEnv_ptr env, Prop_ptr prop,
  ...
  if (!Prop_is_psl_ltl(prop)) {
    StreamMgr_print_error(streams,
                          "BMC can be used only with Psl/ltl properties.\n");
    return 1;
  }
  ...
  return Bmc_GenSolveLtl(
      env, prop, k, rel_loop, !single_prob, /* incrementally */
      BMC_HAS_TO_SOLVE, dump_prob ? BMC_DUMP_DIMACS : BMC_DUMP_NONE,
