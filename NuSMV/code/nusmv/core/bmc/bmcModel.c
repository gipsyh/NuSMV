...
static be_ptr bmc_model_getSingleFairness(const BeEnc_ptr be_enc,
					  const be_ptr one_fairness,
					  const int k, const int l);

static be_ptr bmc_model_getFairness_aux(const BeEnc_ptr be_enc,
					const node_ptr list, const int k,
					const int l);
...
be_ptr Bmc_Model_GetInit0(const BeFsm_ptr be_fsm)
{
	...
	be_ptr init0 = BeEnc_untimed_expr_to_timed(
		be_enc,
		Be_And(BeEnc_get_be_manager(be_enc), BeFsm_get_init(be_fsm),
...
{
	BeEnc_ptr be_enc = BeFsm_get_be_encoding(be_fsm);
	be_ptr init = BeEnc_untimed_expr_to_timed(
		be_enc,
		Be_And(BeEnc_get_be_manager(be_enc), BeFsm_get_init(be_fsm),
		       BeFsm_get_invar(be_fsm)),
		i);
...
be_ptr Bmc_Model_GetUnrolling(const BeFsm_ptr be_fsm, const int j, const int k)
{
	...
	return BeEnc_untimed_to_timed_and_interval(be_enc, trans_invar, j,
						   k - 1);
}
...
be_ptr Bmc_Model_GetFairness(const BeFsm_ptr be_fsm, const int k, const int l)
{
	...
	return bmc_model_getFairness_aux(BeFsm_get_be_encoding(be_fsm), list, k,
					 l);
}
					...
					const int l)
{
	...
	if (list == LS_NIL)
		...
		res = Be_Falsity(be_mgr);
	else {
		be_ptr singleFairness = bmc_model_getSingleFairness(
			be_enc, (be_ptr)car(list), k, l);

		res = Be_And(be_mgr, singleFairness,
			     bmc_model_getFairness_aux(be_enc, cdr(list), k,
						       l));
	}
					  ...
					  const int k, const int l)
{
	...
	return BeEnc_untimed_to_timed_or_interval(be_enc, one_fairness, l,
						  k - 1);
}
