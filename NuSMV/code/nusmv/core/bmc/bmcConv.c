...
void Bmc_Conv_get_BeModel2SymbModel(const BeEnc_ptr be_enc,
  ...
  *inputs = array_alloc(node_ptr, k + 1);
  ...
  SLIST_FOREACH(be_model, genLit) {
    ...
    if (lit < 0)
      var_expr = ExprMgr_not(exprs, var_expr); /* negate if needed: */
      ...
      arrayForEachItem(node_ptr, array, array_iter, expr_list) {
        ...
        for (expr_iter = expr_list; Nil != expr_iter;
          ...
          node_ptr var = BoolEnc_is_var_bit(bool_enc, bit)
                             ? BoolEnc_get_scalar_var_from_bit(bool_enc, bit)
                             : bit;
          int index;
          ...
          if (var == bit) {
            expr = find_node(nodemgr, EQUAL, var,
                             isNegation ? ExprMgr_false(exprs)
                                        : ExprMgr_true(exprs));
        ...
        { /* iterate over elements of the hash table (i.e. scalar
          ...
          ASSOC_FOREACH(var2BitValues, aiter, &var, &bitValues) {
            node_ptr value =
                BoolEnc_get_value_from_var_bits(bool_enc, bitValues);

            new_expr_list =
                new_node(nodemgr, AND, new_node(nodemgr, EQUAL, var, value),
                         new_expr_list);
...
static void Be2bexp_Last(be_ptr be, char *Be2bexpData, nusmv_ptrint sign) {
  ...
  if (Rbc_is_top(rbc)) {
    ...
    if (BE_CURRENT_UNTIMED == time)
      time = 0;
    var_id =
        BeEnc_index_to_untimed_index(be_enc, BeEnc_var_to_index(be_enc, rbc));
      ...
      Be2bexpDfsData_push(
          ...
          Wff_make_not(
              ...
              Wff_make_opnext_times(
                  nodemgr, BeEnc_index_to_name(be_enc, var_id), time)));
    } else {
      Be2bexpDfsData_push(
          (Be2bexpDfsData *)Be2bexpData,
          Wff_make_opnext_times(nodemgr, BeEnc_index_to_name(be_enc, var_id),
                                time));
    }
  }
...
static be_ptr bmc_apply_time_on_expr(hash_ptr bexpr2be, BeEnc_ptr be_enc,
  ...
  case IMPLIES:
    result =
        Be_Implies(BeEnc_get_be_manager(be_enc),
                   bmc_apply_time_on_expr(bexpr2be, be_enc, car(bexp), time),
                   bmc_apply_time_on_expr(bexpr2be, be_enc, cdr(bexp), time));
    break;
  ...
  case ATOM: /* Variable */
    ErrorMgr_internal_error(errmgr,
                            "Not DOT node as variable has been found!\n");
  ...
  case FAILURE: {
    if (ErrorMgr_failure_get_kind(errmgr, bexp) ==
      ...
      ErrorMgr_report_failure_node(errmgr,
  ...
  case NUMBER:
    ...
    ErrorMgr_internal_error(
        errmgr, "bmc_apply_time_on_expr: Unexpected number value [%d]\n",
        node_get_int(bexp));
...
static be_ptr bmc_conv_bexp2be_recur(hash_ptr bexpr2be, BeEnc_ptr be_enc,
  ...
  case ATTIME: {
    ...
    result = bmc_apply_time_on_expr(bexpr2be, be_enc, car(bexp), time_val);
    break;
  }

    /* no other values are allowed */
    ErrorMgr_internal_error(
        errmgr, "bmc_conv_bexp2be_recur: Unexpected number value [%d]\n",
        node_get_int(bexp));
