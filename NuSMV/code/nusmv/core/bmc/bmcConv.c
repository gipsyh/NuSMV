...
static be_ptr bmc_conv_bexp2be_recur(hash_ptr, BeEnc_ptr be_enc, node_ptr bexp);
...
node_ptr Bmc_Conv_Be2Bexp(BeEnc_ptr be_enc, be_ptr be)
{
	...
	/* Calling DFS on f. */
	Rbc_Dfs_exported(rbc, &Be2bexpFunctions, (void *)(&Be2bexpData),
			 rbc_manager);
...
be_ptr Bmc_Conv_Bexp2Be(BeEnc_ptr be_enc, node_ptr bexp)
{
	...
	hash_ptr bexpr2be = bmc_conv_get_handled_hash(
		symb_table, ST_BMC_CONV_BEXPR2BE_HASH);
...
node_ptr Bmc_Conv_BexpList2BeList(BeEnc_ptr be_enc, node_ptr bexp_list)
{
	...
	if (bexp_list == Nil) {
		...
		return cons(nodemgr,
			    (node_ptr)Bmc_Conv_Bexp2Be(be_enc, car(bexp_list)),
				    ...
				    array_t **inputs)
{
	...
	*inputs = array_alloc(node_ptr, k + 1);
	...
	SLIST_FOREACH(be_model, genLit)
	{
		...
		if (lit < 0)
			var_expr = ExprMgr_not(
				exprs, var_expr); /* negate if needed: */
		...
		if (BeEnc_is_index_untimed_curr(be_enc, ut_index)) {
			...
			nusmv_assert(
				BeEnc_is_index_untimed_frozen(be_enc, var_idx));
    ...
    */
		...
		for (all_array_iter = 0; all_array_iter < 3; ++all_array_iter) {
			...
			{
				node_ptr new_expr_list =
					Nil; /* a scalar list instead of bits expr_list */
				node_ptr expr_iter;
				hash_ptr var2BitValues = new_assoc();

				/* -- iterate over a list of expressions */
				for (expr_iter = expr_list; Nil != expr_iter;
				     expr_iter = cdr(expr_iter)) {
					node_ptr expr = car(expr_iter);
					boolean isNegation =
						node_get_type(expr) == NOT;
					node_ptr bit = isNegation ? car(expr) :
								    expr;
					node_ptr var =
						BoolEnc_is_var_bit(bool_enc,
								   bit) ?
					...
					index = BoolEnc_get_index_from_bit(
						bool_enc, bit);

					/* get/create BitValues for a given var */
					bitValues = BIT_VALUES(
						find_assoc(var2BitValues, var));
					if (NULL == bitValues) {
						bitValues = BitValues_create(
							bool_enc, var);
						insert_assoc(
							var2BitValues, var,
							NODE_PTR(bitValues));
             ...
             vars) and add to the new list "var=value" */
					...
					ASSOC_FOREACH(var2BitValues, aiter,
						      &var, &bitValues)
					{
						node_ptr value =
							BoolEnc_get_value_from_var_bits(
								bool_enc,
								bitValues);

						new_expr_list = new_node(
							nodemgr, AND,
							new_node(nodemgr, EQUAL,
								 var, value),
							new_expr_list);
				...
				array_insert(node_ptr, array, array_iter,
					     new_expr_list);
...
/*!
  ...
  \se None
*/
static void Be2bexpDfsData_push(Be2bexpDfsData *data, node_ptr value)
{
	(data->head)++;
	...
	if (Rbc_is_top(rbc)) {
		if (sign == RBC_FALSE) {
			Be2bexpDfsData_push((Be2bexpDfsData *)Be2bexpData,
					    Wff_make_falsity(nodemgr));
		} else {
			Be2bexpDfsData_push((Be2bexpDfsData *)Be2bexpData,
					    Wff_make_truth(nodemgr));
		}
	}
       ...
       state or frozen variable */
		...
		time = BeEnc_index_to_time(be_enc,
					   BeEnc_var_to_index(be_enc, rbc));
		/* frozen var has time BE_CURRENT_UNTIMED => make it 0 to avoid adding NEXTs
     */
		if (BE_CURRENT_UNTIMED == time)
			time = 0;
		var_id = BeEnc_index_to_untimed_index(
			be_enc, BeEnc_var_to_index(be_enc, rbc));

		if (sign == RBC_FALSE) {
			Be2bexpDfsData_push(
				(Be2bexpDfsData *)Be2bexpData,
				Wff_make_not(nodemgr,
					     Wff_make_opnext_times(
						     nodemgr,
						     BeEnc_index_to_name(
							     be_enc, var_id),
						     time)));
			...
			Be2bexpDfsData_push(
				...
				Wff_make_opnext_times(
					nodemgr,
					BeEnc_index_to_name(be_enc, var_id),
					time));
		}
	}
		...
		if (sign == RBC_FALSE) {
			Be2bexpDfsData_push(
				(Be2bexpDfsData *)Be2bexpData,
				Wff_make_not(nodemgr,
					     find_node(nodemgr, identifier,
						       left, right)));
		} else {
			Be2bexpDfsData_push((Be2bexpDfsData *)Be2bexpData,
					    find_node(nodemgr, identifier, left,
						      right));
		}
	}
				     ...
				     node_ptr bexp, int time)
{
	...
	/* Nil value can be used in AND sequences, so a true value must
     be returned */
	if (bexp == Nil)
		return Be_Truth(BeEnc_get_be_manager(be_enc));
	...
	case NEXT: {
		...
		nusmv_assert(
			SymbTable_is_symbol_state_frozen_var(st, car(bexp)));
	...
	case NOT:
		...
		result = Be_Not(BeEnc_get_be_manager(be_enc),
				bmc_apply_time_on_expr(bexpr2be, be_enc,
						       car(bexp), time));
		break;
    ...
    */
		...
		be_ptr cond = bmc_apply_time_on_expr(bexpr2be, be_enc,
						     caar(bexp), time);
		...
		/* no simplification possible since the condition is not a constant */
			...
			result =
				Be_Ite(BeEnc_get_be_manager(be_enc), cond,
				       bmc_apply_time_on_expr(bexpr2be, be_enc,
							      cdar(bexp), time),
				       bmc_apply_time_on_expr(bexpr2be, be_enc,
							      cdr(bexp), time));
	...
	case ARRAY:
		...
		nusmv_assert(SymbTable_is_symbol_bool_var(
			BaseEnc_get_symb_table(BASE_ENC(be_enc)), bexp));
	...
	case ATOM: /* Variable */
		ErrorMgr_internal_error(
			errmgr, "Not DOT node as variable has been found!\n");

	case UNSIGNED_WORD:
		ErrorMgr_internal_error(
			errmgr,
			"Words cannot be met in boolean expressions!\n");

	case FAILURE: {
		if (ErrorMgr_failure_get_kind(errmgr, bexp) ==
		    FAILURE_CASE_NOT_EXHAUSTIVE) {
			ErrorMgr_warning_case_not_exhaustive(errmgr, bexp);
			/* forces a default */
			result = Be_Truth(BeEnc_get_be_manager(be_enc));
			break;
		} else if (ErrorMgr_failure_get_kind(errmgr, bexp) ==
			   FAILURE_DIV_BY_ZERO) {
			...
			ErrorMgr_warning_possible_array_out_of_bounds(errmgr,
								      bexp);
			/* forces a default */
			result = Be_Truth(BeEnc_get_be_manager(be_enc));
			break;
		} else {
			ErrorMgr_report_failure_node(
				errmgr,
				bexp); /* some error in the input expr */
		}
	}
	...
	case NUMBER:
       ...
       values. Other values fall down to an error */
		...
		ErrorMgr_internal_error(
			errmgr,
			"bmc_apply_time_on_expr: Unexpected number value [%d]\n",
			node_get_int(bexp));
	...
	default: {
		...
		ErrorMgr_internal_error(
			errmgr,
			"bmc_apply_time_on_expr: Unexpected case value. Node type = %d\n",
			node_get_type(bexp));
	}
	}
	return result;
				     ...
				     node_ptr bexp)
{
	...
	case NOT:
		/* NOT arg is converted into an be, and its negation is result =ed */
		result = Be_Not(BeEnc_get_be_manager(be_enc),
				bmc_conv_bexp2be_recur(bexpr2be, be_enc,
						       car(bexp)));
		break;
    ...
    */
		...
		be_ptr cond =
			bmc_conv_bexp2be_recur(bexpr2be, be_enc, caar(bexp));
		if (cond == Be_Truth(BeEnc_get_be_manager(be_enc))) {
			result = bmc_conv_bexp2be_recur(bexpr2be, be_enc,
							cdar(bexp));
		} else if (cond == Be_Falsity(BeEnc_get_be_manager(be_enc))) {
			result = bmc_conv_bexp2be_recur(bexpr2be, be_enc,
							cdr(bexp));
		...
		/* no simplification possible since the condition is not a constant */
			...
			result = Be_Ite(BeEnc_get_be_manager(be_enc), cond,
					bmc_conv_bexp2be_recur(bexpr2be, be_enc,
							       cdar(bexp)),
					bmc_conv_bexp2be_recur(bexpr2be, be_enc,
							       cdr(bexp)));
	...
	case ATTIME: {
		...
		result = bmc_apply_time_on_expr(bexpr2be, be_enc, car(bexp),
						time_val);
		break;
	}
		...
		ErrorMgr_internal_error(
			errmgr,
			"bmc_conv_bexp2be_recur: Unexpected number value [%d]\n",
			node_get_int(bexp));
	...
	default: {
		...
		ErrorMgr_internal_error(
			errmgr,
			"bmc_conv_bexp2be_recur: Unexpected case value. Node type = %d\n",
			node_get_type(bexp));
	}
	}
					  ...
					  char *hash_str)
{
	if (!strcmp(ST_BMC_CONV_BEXPR2BE_HASH, hash_str)) {
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_BMC_CONV_BEXPR2BE_HASH, (ST_PFICPCP)NULL,
