...
int Bmc_Pkg_bmc_setup(NuSMVEnv_ptr env, boolean forced) {
...

  {
    ...
    TraceMgr_register_complete_trace_executor(
        TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), "sat",
        "SAT complete trace execution",
        COMPLETE_TRACE_EXECUTOR(
            SATCompleteTraceExecutor_create(fsm, enc, bdd_enc)));

    TraceMgr_register_partial_trace_executor(
        TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), "sat",
        "SAT partial trace execution (no restart)",
        PARTIAL_TRACE_EXECUTOR(
            SATPartialTraceExecutor_create(fsm, enc, bdd_enc, false)));

    TraceMgr_register_partial_trace_executor(
        TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), "sat_r",
        "SAT partial trace execution (restart)",
        PARTIAL_TRACE_EXECUTOR(
...
int Bmc_check_if_model_was_built(NuSMVEnv_ptr env, FILE *err, boolean forced) {
  ...
  if (cmp_struct_get_bmc_init(cmps) && opt_cone_of_influence(opts) && !forced)
    return 0;
