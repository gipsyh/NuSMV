...
void Bmc_Quit(NuSMVEnv_ptr env)
{
	...
	if (NuSMVEnv_has_value(env, ENV_BE_FSM)) {
		BeFsm_ptr be_fsm =
			BE_FSM(NuSMVEnv_remove_value(env, ENV_BE_FSM));
...
void Bmc_Pkg_build_master_be_fsm(const NuSMVEnv_ptr env)
{
	...
	if (SymbTable_get_layer(BaseEnc_get_symb_table(BASE_ENC(be_enc)),
		...
		if (!BaseEnc_layer_occurs(BASE_ENC(be_enc),
					  DETERM_LAYER_NAME)) {
			BaseEnc_commit_layer(BASE_ENC(be_enc),
					     DETERM_LAYER_NAME);
		}
	...
	fsm_be = BeFsm_create_from_sexp_fsm(
		be_enc, BOOL_SEXP_FSM(NuSMVEnv_get_value(env, ENV_BOOL_FSM)));
...
int Bmc_Pkg_bmc_setup(NuSMVEnv_ptr env, boolean forced)
{
...

	{
		...
		TraceMgr_register_complete_trace_executor(
			...
			COMPLETE_TRACE_EXECUTOR(SATCompleteTraceExecutor_create(
				fsm, enc, bdd_enc)));

		TraceMgr_register_partial_trace_executor(
			TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)),
			"sat", "SAT partial trace execution (no restart)",
			PARTIAL_TRACE_EXECUTOR(SATPartialTraceExecutor_create(
				fsm, enc, bdd_enc, false)));

		TraceMgr_register_partial_trace_executor(
			TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)),
			"sat_r", "SAT partial trace execution (restart)",
			PARTIAL_TRACE_EXECUTOR(SATPartialTraceExecutor_create(
				fsm, enc, bdd_enc, true)));
	}
...
int Bmc_check_if_model_was_built(NuSMVEnv_ptr env, FILE *err, boolean forced)
{
	...
	if (err != (FILE *)NULL) {
		...
		if (forced && opt_cone_of_influence(opts)) {
			fprintf(err,
				"bmc_setup -f\" as Cone Of Influence is enabled.\n");
		} else
			fprintf(err, "bmc_setup\".\n");
	}
