...
static void bmc_simulate_print_state(const NuSMVEnv_ptr env, Trace_ptr trace,
				     TraceIter step, int state_num,
				     boolean show_inputs,
...
static Trace_ptr
bmc_simulate_interactive_step(SatSolver_ptr solver, BeEnc_ptr be_enc,
			      BddEnc_ptr bdd_enc, NodeList_ptr symbols,
			      boolean in_simulation, boolean display_all);
...
int Bmc_Simulate(NuSMVEnv_ptr env, const BeFsm_ptr be_fsm, BddEnc_ptr bdd_enc,
		 be_ptr be_constraints, boolean time_shift, const int k,
		 ...
		 const Simulation_Mode mode)
{
	...
	case Interactive:
		ErrorMgr_internal_error(
			errmgr, "%s: Interactive mode not supported yet",
			__func__);
		break;
	case Deterministic:
		/* Do nothing */
		break;
	default:
		ErrorMgr_internal_error(errmgr, "%s: Invalid mode given",
					__func__);
	}

	/* starting state taken from the last node of the current sim trace */
	init = BeEnc_untimed_expr_to_timed(
		be_enc,
	...
	if (!Be_IsTrue(be_mgr, be_constraints)) {
		...
		for (i = 0; i <= k; ++i) {
			...
			if (time_shift) {
				be_timed = BeEnc_shift_curr_to_next(be_enc,
								    be_timed);
			}

			be_timed = BeEnc_untimed_expr_to_timed(be_enc, be_timed,
							       i);
	...
	case SAT_SOLVER_SATISFIABLE_PROBLEM: {
		...
		extension = Bmc_create_trace_from_cnf_model(
			be_enc, SexpFsm_get_symbols_list(SEXP_FSM(bsexp_fsm)),
			NIL(char), TRACE_TYPE_UNSPECIFIED,
		...
		if (print_trace) {
			TraceMgr_execute_plugin(TRACE_MGR(NuSMVEnv_get_value(
							env, ENV_TRACE_MGR)),
						TRACE_OPT(NULL),
	...
	default:
		ErrorMgr_internal_error(
			errmgr,
			" Bmc_Simulate: Unexpected value in sat result");
	} /* switch */
...
int Bmc_StepWiseSimulation(NuSMVEnv_ptr env, BeFsm_ptr be_fsm,
			   ...
			   boolean time_shift, boolean print_trace,
			   ...
			   boolean display_all)
{
	...
	be_trans = Be_And(be_mgr, be_trans,
	...
	Bmc_Utils_add_be_into_inc_solver_positively(
		...
		SatSolver_get_permanent_group(SAT_SOLVER(solver)), be_trans,
		be_enc, cnf_alg);
	...
	/* 2. Iteration adding last computed state as src state and
        ...
        next_state */
	{
		for (steps = 0; ((steps < target_steps) && no_deadlock);
		     steps++) {
			...
			if (opt_verbose_level_gt(opts, 1)) {
				...
				Logger_log(logger,
					   "Performing simulation step %d ...",
					   steps + 1);
			}

			/* 2.0. Extraction from the so far compute trace the last
         state. */
			be_src_state = BeEnc_untimed_expr_to_timed(
				be_enc,
				TraceUtils_fetch_as_be(
					trace, Trace_last_iter(trace),
					TRACE_ITER_SF_VARS, be_enc, bdd_enc),
				0);
           ...
           last state to the solver */
			...
			Bmc_Utils_add_be_into_inc_solver_positively(
				SAT_INC_SOLVER(solver), satGrp, be_src_state,
				be_enc, cnf_alg);
						  ...
						  be_ptr from_state)
{
	...
	be_init = (from_state == (be_ptr)NULL) ? BeFsm_get_init(be_fsm) :
						 from_state;
	be_init = Be_And(be_mgr, be_init, BeFsm_get_invar(be_fsm));

	be_trans = Be_And(be_mgr, BeFsm_get_trans(be_fsm),
			  BeEnc_shift_curr_to_next(be_enc,
	...
	Bmc_Utils_add_be_into_inc_solver_positively(
		SAT_INC_SOLVER(solver),
		SatSolver_get_permanent_group(SAT_SOLVER(solver)), be_prob,
		be_enc, cnf_alg);
	...
	{ /* loops over the constraints list */
		...
		OLIST_FOREACH(constraints, iter)
		{
			...
			be_constr = BeEnc_untimed_expr_to_timed(be_enc,
								be_constr, 0);

			/* pushes the constraint */
			satGrp = SatIncSolver_create_group(solver);
			Bmc_Utils_add_be_into_inc_solver_positively(
				SAT_INC_SOLVER(solver), satGrp, be_constr,
				be_enc, cnf_alg);
			       ...
			       Simulation_Mode mode, boolean display_all)
{
	...
	prob = Be_And(be_mgr,
		      Be_And(be_mgr, init, Be_And(be_mgr, invar, constr)),
		      constr);
	prob = BeEnc_untimed_expr_to_timed(be_enc, prob, 0);

	Bmc_Utils_add_be_into_non_inc_solver_positively(SAT_SOLVER(solver),
							prob, be_enc, cnf_alg);
	...
	case SAT_SOLVER_SATISFIABLE_PROBLEM: {
		...
		{ /* Build language for the FSM */
			...
			bsexp_fsm = BOOL_SEXP_FSM(
				NuSMVEnv_get_value(env, ENV_BOOL_FSM));
			BOOL_SEXP_FSM_CHECK_INSTANCE(bsexp_fsm);
			symbols = SexpFsm_get_symbols_list(SEXP_FSM(bsexp_fsm));

			if (Interactive == mode) {
				trace = bmc_simulate_interactive_step(
					solver, be_enc, bdd_enc, symbols, false,
					display_all);
			} else {
				trace = Bmc_create_trace_from_cnf_model(
					be_enc, symbols,
	...
	default:
		ErrorMgr_internal_error(
			errmgr, "%s: Unexpected value in sat result", __func__);
	}
	...
	const boolean human_readable)
{
	...
	while (!Olist_is_empty(str_constraints)) {
		...
		{
			Expr_ptr constr;
			be_ptr be_constr = Bmc_Utils_next_costraint_from_string(
				be_enc, bdd_enc, str_constr, &constr);
	...
	{ /* does the actual work, and presents the result */
		...
		be_ptr curr_state = TraceUtils_fetch_as_be(
			curr_trace, Trace_last_iter(curr_trace),
			TRACE_ITER_SF_VARS, be_enc, bdd_enc);

		Olist_ptr res = Bmc_simulate_check_feasible_constraints(
			env, BE_FSM(NuSMVEnv_get_value(env, ENV_BE_FSM)),
			bdd_enc, be_constraints, curr_state);
		{
			Oiter iter_c;
			Oiter iter_r;
			int idx = 0;
			...
			for (iter_c = Olist_first(expr_constraints),
			    iter_r = Olist_first(res), idx = 0;
			     ...
			     iter_c = Oiter_next(iter_c),
			    iter_r = Oiter_next(iter_r), ++idx) {
				Expr_ptr constr =
					(Expr_ptr)Oiter_element(iter_c);
				...
				if (human_readable) {
					...
					if (idx == 0)
						StreamMgr_print_output(
							streams,
							"#num\ttruth\tconstraint\n");
					StreamMgr_print_output(streams,
							       "%d\t%d\t", idx,
							       truth);
					StreamMgr_nprint_output(streams,
								wffprint, "%N",
								constr);
					StreamMgr_print_output(streams, "\n");
				} else {
				...
				const int display_all, const boolean verbose)
{
	...
	if (label != NULL) {
		...
		Trace_ptr from_trace = TraceMgr_get_trace_at_index(
			gtm, TraceLabel_get_trace(label));
		...
		tr_number = Bmc_pick_state_from_constr(
			env, be_fsm, bdd_enc, be_constr, mode, display_all);
	}
				     ...
				     int idx)
{
	...
	NuSMVEnv_set_or_replace_value(
		env, ENV_BMC_SIM_TRACE_IDX,
       ...
       the sat solver */
		be_tr = Be_Not(be_mgr,
			       TraceUtils_fetch_as_be(
				       ...
				       TRACE_ITER_SF_VARS, be_enc, bdd_enc));
		be_ass = BeEnc_untimed_expr_to_timed(be_enc, be_tr,
						     (in_simulation ? 1 : 0));

		if (in_simulation) {
			iSatGrp = SatIncSolver_create_group(
				SAT_INC_SOLVER(solver));
			Bmc_Utils_add_be_into_inc_solver_positively(
				SAT_INC_SOLVER(solver), iSatGrp, be_ass, be_enc,
				cnf_alg);
			Slist_push(groups, (void *)iSatGrp);
		} else {
			Bmc_Utils_add_be_into_non_inc_solver_positively(
				solver, be_ass, be_enc, cnf_alg);
		}
		...
		for (j = 0; j < i; ++j) {
			if (in_simulation) {
				bmc_simulate_print_state(
					env, traces[j],
					Trace_last_iter(traces[j]), j,
					in_simulation, shown_assignments);
			} else {
				bmc_simulate_print_state(
					env, traces[j],
					Trace_first_iter(traces[j]), j,
					in_simulation, shown_assignments);
			}
		}
				     ...
				     hash_ptr shown_assignments)
{
	...
	bmc_simulate_trace_step_print(env, trace, step, TRACE_ITER_SF_VARS,
				      shown_assignments);
	...
	if (has_inputs && show_inputs) {
		...
		bmc_simulate_trace_step_print(
			env, trace, step, TRACE_ITER_I_VARS, shown_assignments);
	}
					  ...
					  hash_ptr shown_assignments)
{
	...
	TRACE_STEP_FOREACH(trace, step, it_type, iter, symb, val)
	{
		if ((hash_ptr)NULL != shown_assignments) {
			if (val == find_assoc(shown_assignments, symb))
				continue;
	...
	if (max_choice > 0) {
		...
		while (NIL(char) != (fgets(line, READ_BUF_SIZE, instream)) ||
			...
			if ((sscanf(line, "%d", &choice) != 1) ||
			    (choice < 0) || (choice > max_choice)) {
				StreamMgr_print_output(
					streams,
					"Choose a state from the above (0-%d): ",
					max_choice);
				continue;
			} else
				break;
