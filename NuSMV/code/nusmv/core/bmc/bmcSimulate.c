...
int Bmc_Simulate(NuSMVEnv_ptr env, const BeFsm_ptr be_fsm, BddEnc_ptr bdd_enc,
  ...
  case Random:
    ...
    break;
  case Interactive:
    ErrorMgr_internal_error(errmgr, "%s: Interactive mode not supported yet",
                            __func__);
    break;
  case Deterministic:
    /* Do nothing */
  ...
  init = BeEnc_untimed_expr_to_timed(
      be_enc,
      TraceUtils_fetch_as_be(trace, Trace_last_iter(trace), TRACE_ITER_SF_VARS,
                             be_enc, bdd_enc),
      0);
  ...
  case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
    StreamMgr_print_output(
        streams, "The model deadlocks before requested length %d!\n", k);
    break;
  ...
  case SAT_SOLVER_SATISFIABLE_PROBLEM: {
    ...
    extension = Bmc_create_trace_from_cnf_model(
        be_enc, SexpFsm_get_symbols_list(SEXP_FSM(bsexp_fsm)), NIL(char),
        TRACE_TYPE_UNSPECIFIED, SatSolver_get_model(solver), k);
    ...
    if (print_trace) {
      TraceMgr_execute_plugin(TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)),
                              TRACE_OPT(NULL), (changes_only) ? 0 : 1, tr_num);
    }
...
int Bmc_StepWiseSimulation(NuSMVEnv_ptr env, BeFsm_ptr be_fsm,
  ...
  be_trans =
      Be_And(be_mgr, be_trans, BeEnc_shift_curr_to_next(be_enc, be_trans));
  ...
  Bmc_Utils_add_be_into_inc_solver_positively(
      SAT_INC_SOLVER(solver), SatSolver_get_permanent_group(SAT_SOLVER(solver)),
        ...
        next_state */
  {
    for (steps = 0; ((steps < target_steps) && no_deadlock); steps++) {
      ...
      Bmc_Utils_add_be_into_inc_solver_positively(
          SAT_INC_SOLVER(solver), satGrp, be_src_state, be_enc, cnf_alg);
      ...
      case SAT_SOLVER_SATISFIABLE_PROBLEM:
        if (Interactive == mode) {
          Trace_ptr iTrace = bmc_simulate_interactive_step(
              SAT_SOLVER(solver), be_enc, bdd_enc, Trace_get_symbols(trace),
              true, display_all);
          Trace_concat(trace, &iTrace);
        } else {
          /* Append current computed state to the trace */
          bmc_trace_utils_append_input_state(
              trace, be_enc, SatSolver_get_model(SAT_SOLVER(solver)));
        }
      ...
      case SAT_SOLVER_UNSATISFIABLE_PROBLEM:
        StreamMgr_print_error(
            streams, "The model reached a deadlock state: iteration %d.\n",
            steps);
...
Olist_ptr Bmc_simulate_check_feasible_constraints(NuSMVEnv_ptr env,
  ...
  { /* loops over the constraints list */
    ...
    OLIST_FOREACH(constraints, iter) {
      ...
      satGrp = SatIncSolver_create_group(solver);
      Bmc_Utils_add_be_into_inc_solver_positively(
          SAT_INC_SOLVER(solver), satGrp, be_constr, be_enc, cnf_alg);
...
int Bmc_pick_state_from_constr(NuSMVEnv_ptr env, BeFsm_ptr fsm,
  ...
  Be_Manager_ptr be_mgr = BeEnc_get_be_manager(be_enc);
  SatSolver_ptr solver = Sat_CreateNonIncSolver(env, get_sat_solver(opts));
  SatSolverResult sat_res;
  int tr_num = -1;
  ...
  Bmc_Utils_add_be_into_non_inc_solver_positively(SAT_SOLVER(solver), prob,
                                                  be_enc, cnf_alg);
  ...
  case SAT_SOLVER_SATISFIABLE_PROBLEM: {
    ...
    {                            /* Build language for the FSM */
      ...
      if (Interactive == mode) {
        trace = bmc_simulate_interactive_step(solver, be_enc, bdd_enc, symbols,
                                              false, display_all);
      } else {
        trace = Bmc_create_trace_from_cnf_model(
            be_enc, symbols, BMC_PICK_TRACE_DESCRIPTION, TRACE_TYPE_SIMULATION,
            SatSolver_get_model(solver), has_constraints ? 1 : 0);
      }
    }
  ...
  default:
    ErrorMgr_internal_error(errmgr, "%s: Unexpected value in sat result",
                            __func__);
  }
...
int Bmc_Simulate_bmc_simulate_check_feasible_constraints(
  ...
  { /* does the actual work, and presents the result */
        ...
        be_constraints, curr_state);
    {
      ...
      for (iter_c = Olist_first(expr_constraints), iter_r = Olist_first(res),
          idx = 0;
        ...
        if (human_readable) {
          /* header */
          if (idx == 0)
            StreamMgr_print_output(streams, "#num\ttruth\tconstraint\n");
...
int Bmc_Simulate_bmc_pick_state(const NuSMVEnv_ptr env, TraceLabel label,
  ...
  if (label != NULL) {
    ...
    tr_number = Bmc_pick_state_from_constr(env, be_fsm, bdd_enc, be_constr,
                                           mode, display_all);
  }
  ...
  if (tr_number != -1) {
    ...
    if ((be_ptr)NULL == be_constr) {
      ...
      StreamMgr_print_error(
          streams, "No trace: constraint and initial state are inconsistent\n");
    }
    return 1;
  }
...
bmc_simulate_interactive_step(SatSolver_ptr solver, BeEnc_ptr be_enc,
  ...
  Bmc_fill_trace_from_cnf_model(be_enc, curr_model, (in_simulation ? 1 : 0),
                                curr_trace);
  ...
  while ((i < max_states) && (!found_all_states)) {
    ...
    /* Retrieve the variables assignments from the very latest
       ...
       the sat solver */
    be_tr = Be_Not(be_mgr, TraceUtils_fetch_as_be(
                               curr_trace,
                               (in_simulation ? Trace_last_iter(curr_trace)
                                              : Trace_first_iter(curr_trace)),
                               TRACE_ITER_SF_VARS, be_enc, bdd_enc));
    be_ass =
        BeEnc_untimed_expr_to_timed(be_enc, be_tr, (in_simulation ? 1 : 0));

    if (in_simulation) {
      iSatGrp = SatIncSolver_create_group(SAT_INC_SOLVER(solver));
      Bmc_Utils_add_be_into_inc_solver_positively(
          SAT_INC_SOLVER(solver), iSatGrp, be_ass, be_enc, cnf_alg);
      Slist_push(groups, (void *)iSatGrp);
    } else {
      Bmc_Utils_add_be_into_non_inc_solver_positively(solver, be_ass, be_enc,
                                                      cnf_alg);
    }
  ...
  { /* Start printing found states */
    ...
    for (j = 0; j < i; ++j) {
      if (in_simulation) {
        bmc_simulate_print_state(env, traces[j], Trace_last_iter(traces[j]), j,
                                 in_simulation, shown_assignments);
      } else {
        bmc_simulate_print_state(env, traces[j], Trace_first_iter(traces[j]), j,
                                 in_simulation, shown_assignments);
      }
    }
...
static void bmc_simulate_print_state(const NuSMVEnv_ptr env, Trace_ptr trace,
  ...
  if (has_inputs && show_inputs) {
    ...
    bmc_simulate_trace_step_print(env, trace, step, TRACE_ITER_I_VARS,
                                  shown_assignments);
  }
