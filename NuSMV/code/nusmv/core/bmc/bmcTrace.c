...
/*---------------------------------------------------------------------------*/
static enum st_retval trace_utils_release_bv(char *key, char *data, char *arg);
  ...
  */
     ...
     in the next step */
       ...
       the scalar variable which this bit belongs */
		if (BoolEnc_is_var_bit(bool_enc, var)) {
			node_ptr scalar_var =
				BoolEnc_get_scalar_var_from_bit(bool_enc, var);
			BitValues_ptr bv;
			...
			/* set the bit value */
			BitValues_set(bv,
				      BoolEnc_get_index_from_bit(bool_enc, var),
				      (beLiteral >= 0) ? BIT_VALUE_TRUE :
							 BIT_VALUE_FALSE);
			...
			Trace_step_put_value(trace, timed_step, var,
					     beLiteral >= 0 ?
						     ExprMgr_true(exprs) :
						     ExprMgr_false(exprs));
		}
	...
	{ /* phase 2: iterate over elements of the hash table (i.e. scalar
      vars) and populate the trace accordingly. */
		...
		ASSOC_FOREACH(tvar_2_bval, aiter, &ts_var, &bitValues)
		{
			...
			nusmv_assert(TRACE_END_ITER != timed_step);
			Trace_step_put_value(trace, timed_step, car(ts_var),
					     value);
...
#if BMC_MODEL_DEBUG
	{
		...
		TRACE_FOREACH(trace, step)
		{
			...
			if (1 != i) {
				bmc_model_trace_step_print(env, trace, step,
							   TRACE_ITER_I_VARS,
							   "I", i);
			}
	...
	{ /* build defaults for scalar vars */
		...
		NODE_LIST_FOREACH(symbols, lst_iter)
		{
			...
			if (!SymbTable_is_symbol_var(st, symb) ||
			    SymbType_is_boolean(
				    SymbTable_get_var_type(st, symb))) {
	...
	TRACE_FOREACH(trace, step)
	{
		...
		TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_ALL_VARS, sym_iter, var)
		{
			...
			if (Nil == val) { /* not assigned */
				...
				if (SymbTable_is_symbol_input_var(st, var) &&
				    (Trace_first_iter(trace) == step)) {
					continue;
				}

				if (!SymbType_is_boolean(
					    SymbTable_get_var_type(st, var))) {
					/* default scalar value */
					Trace_step_put_value(
						trace, step, var,
						BoolEnc_get_value_from_var_bits(
							bool_enc,
							BIT_VALUES(find_assoc(
								defaults,
								var))));
				} else {
  ...
  */
       ...
       the scalar variable which this bit belongs to */
		if (BoolEnc_is_var_bit(bool_enc, var)) {
			node_ptr scalar_var =
				BoolEnc_get_scalar_var_from_bit(bool_enc, var);
			BitValues_ptr bv =
				BIT_VALUES(find_assoc(values, scalar_var));
	...
	{ /* phase 2: iterate over elements of the values hash table and
       populate the trace accordingly. */
		...
		{
			boolean is_scalar = (!SymbType_is_boolean(
				SymbTable_get_var_type(st, var)));
