...
#define END_ITERATOR           \
	CLUSTER_LIST_ITERATOR( \
		Nil) /* Represent the end of ClusterList iterator      \
                              */
...
void ClusterList_prepend_cluster(ClusterList_ptr self, Cluster_ptr cluster)
{
...

	{
		...
		self->first = CLUSTER_LIST_ITERATOR(cons(
			nodemgr, (node_ptr)cluster, (node_ptr)self->first));
	}
...
{
	CLUSTER_LIST_CHECK_INSTANCE(self);
	return cluster_list_get_k_image(
		self, s, k, &Cluster_get_quantification_state_input);
}
	...
	const ClusterOptions_ptr cl_options)
{
	...
	if (ClusterOptions_is_iwls95_preorder(cl_options)) {
		...
		if (opt_verbose_level_gt(opts, 1)) {
			...
			Logger_log(logger,
				   "...Performing clusters preordering...");
		}
		source = cluster_list_iwls95_order(self, state_vars_cube,
						   input_vars_cube,
					   ...
					   const ClusterList_ptr other)
{
	...
	for (iter = ClusterList_begin(other); !ClusterListIterator_is_end(iter);
		...
		ClusterList_append_cluster(
			self, CLUSTER(Object_copy(OBJECT(cluster))));
	} /* loop on clusters */
		       ...
		       bdd_ptr (*cluster_getter)(const Cluster_ptr cluster))
{
	...
	while (!ClusterListIterator_is_end(iter)) {
		...
		if (opt_verbose_level_gt(opts, 5)) {
			Logger_ptr logger =
				LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
			long intermediateSizeProduct =
				bdd_size(self->dd, new_p);
			  ...
			  const ClusterOptions_ptr cl_options)
{
	...
	while (!ClusterListIterator_is_end(iter)) {
		...
		clusters_iwls = cluster_list_apply_iwls95_info(
			weak_copy, state_vars_cube, input_vars_cube,
			next_state_vars_cube, cl_options);
		...
		while (!ClusterListIterator_is_end(iter_iwls)) {
			...
			cluster_iwls = CLUSTER_IWLS95(ClusterList_get_cluster(
				clusters_iwls, iter_iwls));
			       ...
			       const ClusterOptions_ptr cl_options)
{
...

	{
		...
		bdd_ptr acc_next_state_vars = bdd_cube_intersection(
			self->dd, acc, next_state_vars_cube);
	...
	while (!ClusterListIterator_is_end(iter)) {
		...
		bdd_ptr sti_ns = bdd_cube_intersection(self->dd, sti,
						       next_state_vars_cube);

		bdd_ptr supp_Q_Ci = cluster_list_get_supp_Q_Ci(self, cluster);
		bdd_ptr state_input =
			bdd_cube_diff(self->dd, sti_pspi, supp_Q_Ci);

		/* constructor limits values to be greater or equal to zero: */
		cluster_iwls = ClusterIwls95_create(
			self->dd, cl_options,
			bdd_size(self->dd, state_input) - 1,
			bdd_size(self->dd, sti_pspi) - 1, x_c,
			bdd_size(self->dd, sti_ns) - 1, z_c,
			bdd_get_lowest_index(self->dd, sti_pspi), max_c);
		...
		tmp = Cluster_get_quantification_state(cluster);
		Cluster_set_quantification_state(CLUSTER(cluster_iwls),
						 self->dd, tmp);
		bdd_free(self->dd, tmp);

		Cluster_set_quantification_state_input(CLUSTER(cluster_iwls),
						       self->dd, state_input);
		/* -------------------------------------------------------------------- */
					     ...
					     bdd_ptr *acc_s, bdd_ptr *acc_si)
{
	if (ClusterListIterator_is_end(iter)) {
		...
		*acc_si = bdd_true(self->dd);
		...
		clusterlist_build_schedule_recur(self,
						 ClusterListIterator_next(iter),
						 s_cube, si_cube, &tacc_s,
						 &tacc_si);
						    ...
						    const boolean append)
{
	...
	while (!is_last_cluster) {
		...
		if (can_accumulate) {
			bdd_and_accumulate(self->dd, &bdd_cluster,
					   bdd_relation);
			...
			if (append)
				...
				ClusterList_prepend_cluster(result,
							    new_cluster);
...
cluster_list_apply_threshold_affinity(const ClusterList_ptr self,
				      const int threshold, const boolean append)
{
	const NuSMVEnv_ptr env =
		EnvObject_get_environment(ENV_OBJECT(self->dd));
	...
	n = clusterlist_affinity_move_clusters(self, result, threshold, append,
					       &list, _heap);
	...
	while (n > 1) {
		...
		if (ASLE_exists(ASPair_get_c1(pair)) &&
			...
			t1 = Cluster_get_trans(
				ASLE_get_cluster(ASPair_get_c1(pair)));
			t2 = Cluster_get_trans(
				ASLE_get_cluster(ASPair_get_c2(pair)));
			...
			if (bdd_size(self->dd, t12) > threshold) {
				...
				list = support_list_heap_add(
					list, _heap, self->dd, new_cluster,
	...
	if (n == 1) {
		...
		if (iter != Nil) {
			...
			cluster = Cluster_create(self->dd);
			t = Cluster_get_trans(ASLE_get_cluster(
				(af_support_list_entry *)car(iter)));
			...
			StreamMgr_print_output(
				streams,
				"Affinity Optimized Inconsistency!!!\n");
					      ...
					      node_ptr *list_ref, heap _heap)
{
	...
	while (!ClusterListIterator_is_end(iter)) {
		...
		if (bdd_size(self->dd, trans) > threshold) {
			...
			if (append)
				ClusterList_append_cluster(new_list,
							   new_cluster);
			else
				ClusterList_prepend_cluster(new_list,
							    new_cluster);
		} else {
			*list_ref = support_list_heap_add(
				*list_ref, _heap, self->dd, cluster,
