...
typedef struct BddTrans_TAG {
	...
	/* takes a bdd and computes an image, i.e. conjunct
     the bdd with transition and abstract away some of state/input/next
     variables depending on the image kind. For example,
     TRANS_IMAGE_FORWARD_STATE abstract away input and next variables, leaving
     only current state ones. Depending on the transition data structure the BDD
     may have some limitations, e.g. for forward images provided BDD may have
     only current state and input variables. If there are such limitation the
     user is responsible to guarantee that algorithms invoking BddTrans
     functions computing images respect such limitations. Computed BDD is
     returned and has to be freed by invoker. */
	bdd_ptr (*trans_compute_image)(void *transition, bdd_ptr bdd,
				       TransImageKind kind);

	/* the same as compute_image but k-image is computed. */
	bdd_ptr (*trans_compute_k_image)(void *transition, bdd_ptr bdd, int k,
					 TransImageKind kind);
...
static void
bdd_trans_init(BddTrans_ptr self, const NuSMVEnv_ptr env,
	       const TransType trans_type, void *transition,
	       void *(*trans_copy)(void *transition),
	       void (*trans_destroy)(void *transition),
	       bdd_ptr (*trans_compute_image)(void *transition, bdd_ptr bdd,
					      TransImageKind kind),
	       bdd_ptr (*trans_compute_k_image)(void *transition, bdd_ptr bdd,
						int k, TransImageKind kind),
	       bdd_ptr (*trans_get_monolithic_bdd)(void *transition),
	       void (*trans_synchronous_product)(void *transition1,
						 void *const transition2),
	       void (*trans_print_short_info)(void *transition, FILE *file),
...
static boolean bdd_trans_debug_partitioned(const ClusterBasedTrans_ptr self,
			     ...
			     const ClusterOptions_ptr cl_options)
{
	const NuSMVEnv_ptr env =
		EnvObject_get_environment(ENV_OBJECT(dd_manager));
	...
	case TRANS_TYPE_MONOLITHIC:
		trans->forward_trans =
			ClusterList_apply_monolithic(clusters_bdd);
		trans->backward_trans = ClusterList_copy(trans->forward_trans);
		break;

	case TRANS_TYPE_THRESHOLD:
		trans->forward_trans =
			ClusterList_apply_threshold(clusters_bdd, cl_options);
		trans->backward_trans = ClusterList_copy(trans->forward_trans);
		break;

	case TRANS_TYPE_IWLS95:
		trans->forward_trans = ClusterList_apply_iwls95_partition(
			clusters_bdd, state_vars_cube, input_vars_cube,
			next_state_vars_cube, cl_options);

		trans->backward_trans = ClusterList_apply_iwls95_partition(
			clusters_bdd, next_state_vars_cube, input_vars_cube,
			state_vars_cube, cl_options);
		break;
	...
	ClusterList_build_schedule(trans->forward_trans, state_vars_cube,
				   input_vars_cube);

	ClusterList_build_schedule(trans->backward_trans, next_state_vars_cube,
				   input_vars_cube);
	...
	self = BddTrans_generic_create(
		env, trans_type, trans, bdd_trans_clusterlist_copy,
	...
	ClusterList_ptr (*trans_get_clusterlist)(void *transition))
{
	...
	bdd_trans_init(self, env, trans_type, transition, trans_copy,
		       trans_destroy, trans_compute_image,
		       trans_compute_k_image, trans_get_monolithic_bdd,
		       trans_synchronous_product, trans_print_short_info,
					   ...
					   FILE *file)
{
	...
	tmp = ClusterList_check_equality(self->backward_trans, basic_clusters);
	fprintf(file, "\t  Backward : %d\n", tmp);
	res = res && tmp;

	tmp = ClusterList_check_equality(self->forward_trans, basic_clusters);
						   ...
						   TransImageKind kind)
{
	...
	case TRANS_IMAGE_FORWARD_STATE_INPUT:
		return ClusterList_get_image_state_input(trans->forward_trans,
							 bdd);
	...
	case TRANS_IMAGE_BACKWARD_STATE_INPUT:
		return ClusterList_get_image_state_input(trans->backward_trans,
							 bdd);
	...
	default: {
		...
		NuSMVEnv_ptr const env =
			EnvObject_get_environment(ENV_OBJECT(dd_mgr));
		...
		ErrorMgr_internal_error(
			errmgr,
			"impossible code in bdd_trans_clusterlist_compute_image");
	}
		return NULL;
	} /* switch */
}
						     ...
						     TransImageKind kind)
{
	...
	switch (kind) {
	case TRANS_IMAGE_FORWARD_STATE:
		return ClusterList_get_k_image_state(trans->forward_trans, bdd,
						     k);

	case TRANS_IMAGE_FORWARD_STATE_INPUT:
		return ClusterList_get_k_image_state_input(trans->forward_trans,
							   bdd, k);

	case TRANS_IMAGE_BACKWARD_STATE:
		return ClusterList_get_k_image_state(trans->backward_trans, bdd,
						     k);

	case TRANS_IMAGE_BACKWARD_STATE_INPUT:
		return ClusterList_get_k_image_state_input(
			trans->backward_trans, bdd, k);
...
/*!
  ...
  See struct BddTrans_TAG for specification of this function
*/
static void bdd_trans_clusterlist_print_short_info(void *transition, FILE *file)
{
	ClusterBasedTrans_ptr trans = (ClusterBasedTrans_ptr)transition;
