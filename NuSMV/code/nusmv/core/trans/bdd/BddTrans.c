...
typedef struct BddTrans_TAG {
  ...
  void (*trans_synchronous_product)(void *transition1, void *const transition2);
...
BddTrans_ptr BddTrans_create(DDMgr_ptr dd_manager,
  ...
  case TRANS_TYPE_IWLS95:
    trans->forward_trans = ClusterList_apply_iwls95_partition(
        clusters_bdd, state_vars_cube, input_vars_cube, next_state_vars_cube,
        cl_options);

    trans->backward_trans = ClusterList_apply_iwls95_partition(
        clusters_bdd, next_state_vars_cube, input_vars_cube, state_vars_cube,
        cl_options);
    break;
  ...
  self = BddTrans_generic_create(
      ...
      bdd_trans_clusterlist_destroy, bdd_trans_clusterlist_compute_image,
...
BddTrans_ptr BddTrans_generic_create(
  ...
  bdd_trans_init(self, env, trans_type, transition, trans_copy, trans_destroy,
                 trans_compute_image, trans_compute_k_image,
                 trans_get_monolithic_bdd, trans_synchronous_product,
                 trans_print_short_info, trans_get_clusterlist);
...
static bdd_ptr bdd_trans_clusterlist_compute_image(void *transition,
  ...
  default: {
    ...
    ErrorMgr_internal_error(
        errmgr, "impossible code in bdd_trans_clusterlist_compute_image");
  }
    return NULL;
  } /* switch */
}
...
static bdd_ptr bdd_trans_clusterlist_compute_k_image(void *transition,
  ...
  case TRANS_IMAGE_FORWARD_STATE_INPUT:
    return ClusterList_get_k_image_state_input(trans->forward_trans, bdd, k);

  case TRANS_IMAGE_BACKWARD_STATE:
    return ClusterList_get_k_image_state(trans->backward_trans, bdd, k);

  case TRANS_IMAGE_BACKWARD_STATE_INPUT:
    return ClusterList_get_k_image_state_input(trans->backward_trans, bdd, k);
