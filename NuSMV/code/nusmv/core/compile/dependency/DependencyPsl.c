...
static Set_t dependency_psl_get_dependencies(DependencyBase_ptr self,
...
DependencyPsl_ptr DependencyPsl_create(const NuSMVEnv_ptr env,
  ...
  dependency_psl_init(self, env, name, NUSMV_PSL_SYMBOL_FIRST,
...
void dependency_psl_init(DependencyPsl_ptr self, const NuSMVEnv_ptr env,
  ...
  OVERRIDE(DependencyBase, get_dependencies) = dependency_psl_get_dependencies;
}
...
static Set_t dependency_psl_get_dependencies(DependencyBase_ptr self,
  ...
  case PSL_WSELECT: {
    ...
    Set_t object = _THROW(symb_table, psl_node_get_left(formula), context,
                          filter, preserve_time, time, dependencies_hash);
    Set_t high_bit = _THROW(symb_table, psl_node_get_left(right), context,
                            filter, preserve_time, time, dependencies_hash);
    Set_t low_bit = _THROW(symb_table, psl_node_get_right(right), context,
                           filter, preserve_time, time, dependencies_hash);
  ...
  case PSL_DIAMONDMINUSGT: {
    Set_t con =
        _THROW(symb_table, psl_node_suffix_implication_get_consequence(formula),
               context, filter, preserve_time, time, dependencies_hash);

    result =
        _THROW(symb_table, psl_node_suffix_implication_get_premise(formula),
               context, filter, preserve_time, time, dependencies_hash);
  ...
  case PSL_WITHIN_: {
    Set_t n2 =
        _THROW(symb_table, psl_node_get_right(psl_node_get_left(formula)),
               context, filter, preserve_time, time, dependencies_hash);
    Set_t n3 = _THROW(symb_table, psl_node_get_right(formula), context, filter,
                      preserve_time, time, dependencies_hash);

    result = _THROW(symb_table, psl_node_get_left(psl_node_get_left(formula)),
                    context, filter, preserve_time, time, dependencies_hash);
  ...
  case PSL_X:
  case PSL_XBANG: {
    Set_t n2 = _THROW(symb_table, psl_node_extended_next_get_when(formula),
                      context, filter, preserve_time, time, dependencies_hash);
    Set_t n3 = _THROW(symb_table, psl_node_extended_next_get_condition(formula),
                      context, filter, preserve_time, time, dependencies_hash);
  ...
  case PSL_ITE: {
    Set_t then_arg = _THROW(symb_table, psl_node_get_ite_then(formula), context,
                            filter, preserve_time, time, dependencies_hash);
    Set_t else_arg = _THROW(symb_table, psl_node_get_ite_else(formula), context,
                            filter, preserve_time, time, dependencies_hash);

    result = _THROW(symb_table, psl_node_get_ite_cond(formula), context, filter,
                    preserve_time, time, dependencies_hash);
