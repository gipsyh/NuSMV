					 ...
					 node_ptr formula, node_ptr context)
{
	return FormulaDependency_get_dependencies_by_type(
		self, symb_table, formula, context, VFT_CNIF, false);
}
						  ...
						  node_ptr compassion)
{
	...
	result1 = FormulaDependency_get_dependencies(self, symb_table, formula,
						     Nil);
	result2 = FormulaDependency_get_dependencies(self, symb_table, justice,
						     Nil);
	result3 = FormulaDependency_get_dependencies(self, symb_table,
						     compassion, Nil);
					  ...
					  hash_ptr dependencies_hash)
{
	...
	while (!ListIter_is_end(iter)) {
		DependencyBase_ptr pr = DEPENDENCY_BASE(NodeList_get_elem_at(
		...
		if (NodeWalker_can_handle(NODE_WALKER(pr), formula)) {
			{
				...
				formula_dependency_mk_hash_key(formula, context,
							       filter,
							       preserve_time,
							       time, &key);
			...
			return DependencyBase_get_dependencies(
				pr, symb_table, formula, context, filter,
	...
	hash_ptr dependencies_hash)
{
			...
			if (preserve_time) {
				if (EXPR_UNTIMED_NEXT == time &&
				    !SymbTable_is_symbol_input_var(symb_table,
								   formula) &&
				    !SymbTable_is_symbol_parameter(symb_table,
								   formula)) {
	...
	if (SymbTable_is_symbol_define(symb_table, formula)) {
		...
		formula_dependency_mk_hash_key(formula, Nil, filter,
					       preserve_time, time, &key);

		result = formula_dependency_lookup_hash(dependencies_hash,
							NODE_PTR(&key));
		...
		if (result == (Set_t)NULL) {
         ...
         dependencies. */
			node_ptr nformula;
			formula_dependency_insert_hash(dependencies_hash,
						       NODE_PTR(&key),
						       BUILDING_DEP_SET);
         ...
         dependencies for further use. */
			if (Set_IsEmpty(result)) {
				formula_dependency_close_define_hash(
					dependencies_hash, NODE_PTR(&key),
					EMPTY_DEP_SET);
			} else
				formula_dependency_close_define_hash(
       ...
       filter */
		...
		for (i = low; i <= high; ++i) {
			...
			ret = formula_dependency_get_definition_dependencies(
				self, symb_table, arr_var, filter,
				preserve_time, time, dependencies_hash);
		...
		result = (VFT_CONSTANTS & filter) ? Set_MakeSingleton(formula) :
						    Set_MakeEmpty();
	} else {
