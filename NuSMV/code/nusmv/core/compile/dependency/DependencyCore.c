...
static Set_t dependency_core_get_dependencies(
					 ...
					 const char *name)
{
	...
	dependency_core_init(self, env, name, NUSMV_CORE_SYMBOL_FIRST,
	...
	int time, hash_ptr dependencies_hash)
{
   ...
   */
         ...
         value on the same key*/
			if (name == formula &&
				...
				nusmv_assert((Set_t)NULL !=
					     formula_dependency_lookup_hash(
						     dependencies_hash,
						     NODE_PTR(&key)));
	...
	case NEXT: /* next(alpha), with alpha possibly complex, thus ... */
       ...
       next-operator */
		result = _THROW(symb_table, car(formula), context, filter,
				preserve_time, EXPR_UNTIMED_NEXT,
				dependencies_hash);
		break;
	...
	case CASE:
	case IFTHENELSE: {
		Set_t then_arg = _THROW(symb_table, cdr(car(formula)), context,
					filter, preserve_time, time,
					dependencies_hash);

		Set_t else_arg = _THROW(symb_table, cdr(formula), context,
					filter, preserve_time, time,
					dependencies_hash);
	...
	case BIT_SELECTION: {
		Set_t high_bit = _THROW(symb_table, car(cdr(formula)), context,
					filter, preserve_time, time,
					dependencies_hash);
		Set_t low_bit = _THROW(symb_table, cdr(cdr(formula)), context,
				       filter, preserve_time, time,
				       dependencies_hash);
	...
	case COUNT: {
		...
		while (Nil != list) {
			Set_t tmp = _THROW(symb_table, car(list), context,
					   filter, preserve_time, time,
					   dependencies_hash);
	...
	case WAWRITE: {
		Set_t location = _THROW(symb_table, car(cdr(formula)), context,
					filter, preserve_time, time,
					dependencies_hash);
		Set_t address = _THROW(symb_table, cdr(cdr(formula)), context,
				       filter, preserve_time, time,
				       dependencies_hash);
	...
	case CAST_TO_UNSIGNED_WORD: {
		Set_t location = _THROW(symb_table, car(formula), context,
					filter, preserve_time, time,
					dependencies_hash);
	...
	case ARRAY: {
		...
		if (ResolveSymbol_is_defined(rs)) {
         ...
         value on the same key */
			...
			result = _THROW(symb_table, car(formula), context,
					filter, preserve_time, time,
					dependencies_hash);
