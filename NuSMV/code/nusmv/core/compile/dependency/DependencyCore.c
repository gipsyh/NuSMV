...
void dependency_core_init(DependencyCore_ptr self, const NuSMVEnv_ptr env,
  ...
  OVERRIDE(DependencyBase, get_dependencies) = dependency_core_get_dependencies;
}
...
static Set_t dependency_core_get_dependencies(
  ...
  case ATOM: {
    ...
    if (ResolveSymbol_is_parameter(rs)) {
      ...
      result = _THROW(symb_table, param, context, filter, preserve_time, time,
                      dependencies_hash);
    } else if (ResolveSymbol_is_constant(rs)) {
      result = formula_dependency_get_definition_dependencies(
          FORMULA_DEPENDENCY(NODE_WALKER(self)->master), symb_table, name,
      ...
      if (name == formula && SymbTable_is_symbol_define(symb_table, name)) {
        nusmv_assert((Set_t)NULL != formula_dependency_lookup_hash(
  ...
  case NFUNCTION: {
    ...
    fname = _THROW(symb_table, car(formula), context, filter, preserve_time,
                   time, dependencies_hash);
    result = _THROW(symb_table, cdr(formula), context, filter, preserve_time,
                    time, dependencies_hash);
  ...
  case COUNT: {
    ...
    while (Nil != list) {
      Set_t tmp = _THROW(symb_table, car(list), context, filter, preserve_time,
                         time, dependencies_hash);
  ...
  case ARRAY: {
    ...
    if (ResolveSymbol_is_defined(rs)) {
      ...
      tmp = _THROW(symb_table, cdr(formula), context, filter, preserve_time,
                   time, dependencies_hash);
  ...
  case ARRAY_DEF: {
    ...
    for (iter = car(formula); iter != Nil; iter = cdr(iter)) {
      ...
      tmp = _THROW(symb_table, car(iter), context, filter, preserve_time, time,
                   dependencies_hash);
      result = Set_Union(result, tmp);
      Set_ReleaseSet(tmp);
    }
