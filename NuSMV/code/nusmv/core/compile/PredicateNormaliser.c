					    ...
					    node_ptr expr)
{
  ...
  */
	if (AND == node_get_type(expr) || CONS == node_get_type(expr)) {
		node_ptr left =
			PredicateNormaliser_normalise_expr(self, car(expr));
		node_ptr right =
			PredicateNormaliser_normalise_expr(self, cdr(expr));
		result = ExprMgr_resolve(exprs, self->st, node_get_type(expr),
					 left, right);
	} else {
					     ...
					     node_ptr expr)
{
  ...
  */
	if (AND == node_get_type(expr) || CONS == node_get_type(expr)) {
		node_ptr left = PredicateNormaliser_normalise_expr_no_expand(
			self, car(expr));
		node_ptr right = PredicateNormaliser_normalise_expr_no_expand(
			self, cdr(expr));
		result = ExprMgr_resolve(exprs, self->st, node_get_type(expr),
					 left, right);
	} else {
...
static void pred_norm_init(PredicateNormaliser_ptr self, SymbTable_ptr st)
{
	...
	self->expr2normalisedPredicate =
		predicate_normalizer_get_handled_hash(self);
#else
				    ...
				    node_ptr context, boolean expand_defines)
{
	...
	case NUMBER_EXP:
	case TWODOTS:
		result = ExprMgr_resolve(exprs, self->st, node_type, car(expr),
					 cdr(expr));
		break;
           ...
           thus conversion to integer is enough*/
				nusmv_assert(
					...
					SYMB_TYPE_SET_INT_SYMB ==
						SymbType_get_tag(type));
				if (pred_norm_is_true_bool_exp(self, result,
							       Nil)) {
					result = pred_norm_bool2int(self,
								    result);
				}
			}
		}
		...
		/* check whether this symbol is a parameter */
           ...
           possibility */
				ErrorMgr_internal_error(
					errmgr,
					"Symbol is unexpectedly undefined: %s",
    ...
    */
	...
	case CONS: {
		...
		node_ptr op2 = Nil == cdr(expr) ?
				       ExprMgr_true(exprs) :
				       pred_norm_normalise(self, cdr(expr),
							   context,
							   expand_defines);
	...
	case GE: {
		...
		node_ptr op1 = pred_norm_normalise(self, car(expr), context,
						   expand_defines);
		node_ptr op2 = pred_norm_normalise(self, cdr(expr), context,
						   expand_defines);
		...
		else if ((SymbType_is_intarray(type1) &&
			  SymbType_is_intarray(type2)) ||
			 (SymbType_is_wordarray(type1) &&
			  SymbType_is_wordarray(type2))) {
			result = pred_norm_find_node(self, node_type, op1, op2);
		}
		/* left is boolean and the right is word[1] => convert word1 to bool */
		else if (SymbType_is_boolean(type1) &&
			 SymbType_is_word_1(type2)) {
			result = pred_norm_find_node(
				self, node_type, op1,
				ExprMgr_resolve(exprs, self->st, CAST_BOOL, op2,
						Nil));
		}
		/* left is word[1] and the right is boolean => convert word1 to bool */
		else if (SymbType_is_word_1(type1) &&
			 SymbType_is_boolean(type2)) {
			result = pred_norm_find_node(
				self, node_type,
				ExprMgr_resolve(exprs, self->st, CAST_BOOL, op1,
						Nil),
				op2);
		}
    ...
    */
	...
	case CONST_ARRAY: { /* casts, extend and these two WordArray
       ...
       into "((a!=b)?1:0) +5 = 6" and then obtain "(a!=b) ? 1+5=6 : 0+5=0" */
		...
		if (op2 != Nil &&
		    pred_norm_is_true_bool_exp(self, op2, context)) {
			op2 = pred_norm_bool2int(self, op2);
		}
	...
	case WAWRITE: {
		node_ptr op1 = pred_norm_normalise(self, car(expr), context,
						   expand_defines);
		node_ptr op2 = pred_norm_normalise(self, car(cdr(expr)),
						   context, expand_defines);
		node_ptr op3 = pred_norm_normalise(self, cdr(cdr(expr)),
						   context, expand_defines);
	...
	case BIT_SELECTION: {
		...
		node_ptr hbit = pred_norm_normalise(self, car(cdr(expr)),
						    context, true);
		node_ptr lbit = pred_norm_normalise(self, cdr(cdr(expr)),
						    context, true);
	...
	case COUNT: {
		...
		while (Nil != list) {
			node_ptr elem = pred_norm_normalise(
				self, car(list), context, expand_defines);
		...
		while (Nil != new_list) {
			...
			find_list = find_node(nodemgr, CONS,
					      find_atom(nodemgr, tmp),
					      find_list);
    ...
    */
	...
	case CAST_WORD1:
		result = pred_norm_normalise(self, car(expr), context,
					     expand_defines);
    ...
    */
	...
	case CASE: {
		...
		if (SymbType_is_boolean(type)) {
			...
			result = ExprMgr_resolve(
				exprs, self->st, CASE,
				ExprMgr_resolve(exprs, self->st, COLON, cond,
						then),
				tail);
		} else
			result =
				pred_norm_normalise_ite(self, cond, then, tail);
	...
	case NFUNCTION: {
		...
		while (Nil != params) {
			p = pred_norm_normalise(self, car(params), context,
						expand_defines);
		...
		while (Nil != tmp) {
			...
			new_params =
				find_node(nodemgr, CONS, car(tmp), new_params);
				     ...
				     node_ptr expr)
{
	...
	if (ExprMgr_is_true(exprs, expr) || ExprMgr_is_number(exprs, expr, 1)) {
		return ExprMgr_resolve(
			...
			(node_ptr)WordNumberMgr_integer_to_word_number(words, 1,
								       1),
			Nil);
	}
	if (ExprMgr_is_false(exprs, expr) ||
	    ExprMgr_is_number(exprs, expr, 0)) {
		return ExprMgr_resolve(
			exprs, self->st, NUMBER_UNSIGNED_WORD,
			(node_ptr)WordNumberMgr_integer_to_word_number(words, 0,
								       1),
			Nil);
	}
				      ...
				      node_ptr op1, node_ptr op2)
{
  ...
  */
    ...
    */
		return pred_norm_push_ite_up_conditioned(
			self, cond, op, cdr(car(op1)), op2, tail);
    ...
    */
		...
		node_ptr first_expr =
			pred_norm_push_ite_up(self, op, op1, cdr(car(op2)));
		node_ptr second_expr =
			FAILURE == node_get_type(cdr(op2)) ?
				cdr(op2) :
					  ...
					  hash_ptr already_processed)
{
	...
	case OP_FUTURE:
	case OP_ONCE:
		pred_norm_get_predicates_only(self, preds, car(expr),
					      already_processed);
		break;
	...
	case CONS:
		pred_norm_get_predicates_only(self, preds, car(expr),
					      already_processed);
		pred_norm_get_predicates_only(self, preds, cdr(expr),
					      already_processed);
		break;
	...
	case CAST_BOOL: {
       ...
       added to the set of predicates "expr == 0d1_1" */
		expr = car(expr);
		type = TypeChecker_get_expression_type(self->checker, expr,
						       Nil);
	...
	case GE: {
		SymbType_ptr type1 = TypeChecker_get_expression_type(
			self->checker, car(expr), Nil);
		SymbType_ptr type2 = TypeChecker_get_expression_type(
			self->checker, cdr(expr), Nil);
	...
	case UNION:
		...
		nusmv_assert(SYMB_TYPE_SET_BOOL ==
			     SymbType_get_tag(TypeChecker_get_expression_type(
				     self->checker, expr, Nil)));
		pred_norm_get_predicates_only(self, preds, car(expr),
					      already_processed);
		pred_norm_get_predicates_only(self, preds, cdr(expr),
	...
	default: {
		...
		ErrorMgr_internal_error(
			errmgr, "unknown kind of an expression (see above)");
	}
	} /* switch */
...
predicate_normalizer_get_handled_hash(PredicateNormaliser_ptr self)
{
	...
	result = SymbTable_get_handled_hash_ptr(
		self->st, PREDICATE_NORMALIZER_HASH, (ST_PFICPCP)NULL,
		(ST_PFICPI)NULL, predicate_normalizer_hash_free,
...
static node_ptr pred_normalizer_make_key(const NodeMgr_ptr nodemgr,
					 ...
					 node_ptr context, node_ptr expr)
{
	return find_node(nodemgr, CONTEXT + (expand_define ? 1 : 0), context,
			 expr);
}
