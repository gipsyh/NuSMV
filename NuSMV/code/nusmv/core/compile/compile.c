...
int Compile_print_usage(NuSMVEnv_ptr env, OStream_ptr file)
{
	...
	if (opt_cone_of_influence(opts) == false) {
		if (!Compile_check_if_model_was_built(env, (FILE *)NULL,
						      false)) {
			BddFsm_ptr fsm =
				     ...
				     boolean forced)
{
	...
	if (Compile_check_if_encoding_was_built(env, err))
		return 1;

	if (cmp_struct_get_build_model(cmps) == 0 &&
	    opt_cone_of_influence(opts) && !forced)
		return 0;
	...
	if (err != (FILE *)NULL) {
		if (cmp_struct_get_build_bool_model(cmps) == 0) {
			...
			if (opt_cone_of_influence(opts) && forced) {
				fprintf(err,
					"(Use the command \"build_boolean_model -f\" "
					"as Cone Of Influence is enabled.)\n");
			} else
				fprintf(err,
					"(Use the command \"build_boolean_model\")\n");
		}
	}
					  ...
					  boolean forced)
{
	...
	if (Compile_check_if_flattening_was_built(env, err))
		return 1;

	if (err != (FILE *)NULL) {
		if (cmp_struct_get_build_flat_model(cmps) == 0) {
			fprintf(err,
				"The flat model must be built before "
...
int Compile_check_if_encoding_was_built(const NuSMVEnv_ptr env, FILE *err)
{
	...
	if (err != (FILE *)NULL) {
		fprintf(err, "The variables must be built before. Use the "
...
int Compile_create_boolean_model(NuSMVEnv_ptr env)
{
	...
	BddEnc_ptr benc = BDD_ENC(NuSMVEnv_get_value(env, ENV_BDD_ENCODER));
	BoolEnc_ptr bool_enc =
	...
	CATCH(errmgr)
	{
		bool_fsm = BoolSexpFsm_create_from_scalar_fsm(scalar_fsm, benc,
							      det_layer);
	}
	FAIL(errmgr)
	{
		...
		{ /* register BDD complete and partial executors */
			...
			TraceMgr_register_complete_trace_executor(
				...
				COMPLETE_TRACE_EXECUTOR(
					BDDCompleteTraceExecutor_create(fsm,
									enc)));
			...
			TraceMgr_register_partial_trace_executor(
				...
				PARTIAL_TRACE_EXECUTOR(
					BDDPartialTraceExecutor_create(fsm,
								       enc)));
		}
...
Expr_ptr Compile_remove_assignments(const NuSMVEnv_ptr env, Expr_ptr expr)
{
	...
	if (AND == node_get_type(expr)) {
		return ExprMgr_and(exprs,
				   Compile_remove_assignments(env, car(expr)),
			    ...
			    OStream_ptr output_file)
{
	...
	OStream_printf(output_file, "Propert%s", (multiple ? "ies\n" : "y "));
	...
	if (multiple) {
		...
		Prop_print(prop, output_file, get_prop_print_method(opts));
	}
				...
				OStream_ptr output_file)
{
	...
	array_t *layers =
		SymbTable_get_class_layer_names(symb_table, (const char *)NULL);
	SexpFsm_ptr fsm;
	...
	/* Do some post-processings over the COI hierarchy */
	{
		...
		/* 1. -> INIT, TRANS, INVAR, INPUT, JUSTICE, COMPASSION. */
		{
			...
			coi_hierarchy = FlatHierarchy_create_from_members(
				symb_table,
         ...
         it's relative assignments  */
			...
			SET_FOREACH(cone, iter)
			{
...

				{
					...
					node_ptr init_var = find_node(
						nodemgr, SMALLINIT, var, Nil);
					node_ptr next_var = find_node(
						nodemgr, NEXT, var, Nil);
					node_ptr tmp;

					/* First try with normal assignments */
					tmp = FlatHierarchy_lookup_assign(
						fsm_fh, var);
					if (Nil != tmp) {
			...
			SET_FOREACH(props, iter)
			{
				...
				case Prop_Ltl:
					FlatHierarchy_set_ltlspec(
						coi_hierarchy,
						cons(nodemgr,
						     find_node(nodemgr, LTLSPEC,
							       prop_expr,
							       prop_name),
						     FlatHierarchy_get_ltlspec(
							     coi_hierarchy)));
					break;
				case Prop_Ctl:
				...
				case Prop_Psl:
					FlatHierarchy_set_pslspec(
						coi_hierarchy,
						cons(nodemgr,
						     find_node(nodemgr, PSLSPEC,
							       prop_expr,
							       prop_name),
						     FlatHierarchy_get_pslspec(
							     coi_hierarchy)));
					break;
				case Prop_Compute:
					FlatHierarchy_set_compute(
						coi_hierarchy,
						cons(nodemgr,
						     find_node(nodemgr, COMPUTE,
							       prop_expr,
							       prop_name),
						     FlatHierarchy_get_compute(
							     coi_hierarchy)));
					break;
				case Prop_Invar:
	...
	Compile_WriteRestrictedFlattenModel(env,
					    OStream_get_stream(output_file),
					    symb_table, layers, "MODULE main",
					    coi_hierarchy, true);
...
int Compile_get_bits(const SymbTable_ptr st, const NodeList_ptr lst)
{
	...
	NODE_LIST_FOREACH(lst, iter)
	{
		...
		if (SymbType_is_boolean(type)) {
			...
			res += Utils_log2_round(max(
				llength(SymbType_get_enum_type_values(type)) -
					1,
				1));
         ...
         node_ptr cons list of subtypes */
			NodeList_ptr subtype_lst = NodeList_create_from_element(
				  ...
				  Prop_Type prop_type, OStream_ptr output_file)
{
	...
	for (prop_no = 0; prop_no < PropDb_get_size(prop_db); ++prop_no) {
		...
		if ((prop_type == Prop_NoType) ||
		    (prop_type == Prop_get_type(prop))) {
			Set_t cone =
				Prop_compute_cone(prop, hierarchy, symb_table);
			coi_union = Set_Union(coi_union, cone);
			properties =
				Set_AddMember(properties, (Set_Element_t)prop);
			Set_ReleaseSet(cone);
		}
	}

	Compile_write_coi_prop_fsm(env, hierarchy, coi_union, properties,
				   output_file);
				 ...
				 const char *file_name)
{
	...
	NodeList_ptr order = PropDb_get_coi_grouped_properties(
	...
	NODE_LIST_FOREACH(order, iter)
	{
		...
		if (Prop_NoType == prop_type) {
			...
			SET_FOREACH(props, iter)
			{
				...
				if (Prop_get_type(prop) == prop_type) {
					filtered = Set_AddMember(
						filtered, (Set_Element_t)prop);
				}
			}
		}
		...
		if (!Set_IsEmpty(filtered)) {
			if (NIL(char) != file_name) {
				...
				chars = snprintf(new_file_name, max_len,
						 "%s.%d", file_name, prop_no);
				SNPRINTF_CHECK(chars, max_len);

				output_file = OStream_create_file(new_file_name,
								  false);
			...
			if (only_dump_coi) {
				Compile_write_coi_prop(env, cone, filtered,
						       output_file);
			} else {
			...
			node_ptr ntype, int threshold)
{
	...
	case SCALAR: {
         ...
         all other elements must be skipped. */
           ...
           is not necessary.. */
             ...
             string can be printed within the threshold length.. */
					Olist_ptr rev = Olist_copy_reversed(
						values_list);
					Oiter ri;
		...
		if (missing > 0) {
			OStream_printf(file, fmt, missing);
		}
...
node_ptr Compile_get_var_type(const NuSMVEnv_ptr env, SymbType_ptr type)
{
	...
	case SYMB_TYPE_SIGNED_WORD:
		...
		ntype = find_node(nodemgr, SIGNED_WORD,
				  PTR_FROM_INT(node_ptr, j), Nil);
		break;

		/* word is like an arrary of booleans + unsigned arithmetic */
	case SYMB_TYPE_UNSIGNED_WORD:
		j = SymbType_get_word_width(type);
		ntype = find_node(nodemgr, UNSIGNED_WORD,
				  PTR_FROM_INT(node_ptr, j), Nil);
		break;
		       ...
		       const boolean verbose)
{
	...
	if (!total_only && !defs_only) {
		...
		{ /* extracts the default class layer names */
			array_t *layer_names =
				SymbTable_get_class_layer_names(st, NULL);
			char *layer_name;
			int i;
			...
			{
				SymbLayer_ptr layer =
					SymbTable_get_layer(st, layer_name);
				SymbLayerIter iter;
				...
				SYMB_LAYER_FOREACH(layer, iter, STT_VAR)
				{
					...
					if (SymbTable_is_symbol_state_var(
						    st, var)) {
						if (!vars_only) {
							NodeList_append(alls,
									var);
						}
						if (statevars) {
							NodeList_append(list,
									var);
						}
					} else if (SymbTable_is_symbol_frozen_var(
							   st, var)) {
						if (!vars_only) {
							NodeList_append(allf,
									var);
						}
						if (frozenvars) {
							NodeList_append(list,
		...
		{ /* prints the variables */
			...
			{
				node_ptr name =
					NodeList_get_elem_at(list, iter);
				...
				ntype = Compile_get_var_type(
					env, SymbTable_get_var_type(st, name));
				Compile_print_type(
					env, ostream, ntype,
					(verbose ? 0 :
						   (80 - strlen(s_name) - 3)));
		...
		{ /* prints the defines */
			...
			{
				node_ptr name =
					SymbTable_iter_get_symbol(st, &iter);
				SymbType_ptr type =
					TypeChecker_get_expression_type(
						tc, name, Nil);

				nusmv_assert(
					SymbTable_is_symbol_define(st, name));

				OStream_nprintf(ostream, wffprint,
						"%N : ", name);
		...
		arrayForEachItem(char *, layer_names, i, layer_name)
		{
			...
			SYMB_LAYER_FOREACH(layer, iter, STT_VAR)
			{
				...
				} else if (SymbTable_is_symbol_input_var(st,
									 var)) {
					if (inputvars) {
						NodeList_append(list, var);
					}
  ...
  */
	...
	arrayForEachItem(const char *, layers_name, i, a_layer_name)
	{
		...
		SYMB_LAYER_FOREACH(layer, iter, STT_VAR)
		{
			...
			node_ptr init_expr = FlatHierarchy_lookup_assign(
				hierarchy, init_name);
			node_ptr next_expr = FlatHierarchy_lookup_assign(
				hierarchy, next_name);

			if (invar_expr != Nil) {
				tmp = PredicateNormaliser_normalise_expr(
					normalizer,
					find_node(nodemgr, EQDEF, name,
						  invar_expr));
				PredicateNormaliser_get_predicates_only(
					normalizer, &predicates, tmp);
			}
			if (init_expr != Nil) {
				tmp = PredicateNormaliser_normalise_expr(
					normalizer,
					find_node(nodemgr, EQDEF, init_name,
						  init_expr));
				PredicateNormaliser_get_predicates_only(
					normalizer, &predicates, tmp);
			}
			if (next_expr != Nil) {
				tmp = PredicateNormaliser_normalise_expr(
					normalizer,
					find_node(nodemgr, EQDEF, next_name,
						  next_expr));
				PredicateNormaliser_get_predicates_only(
					normalizer, &predicates, tmp);
			}
				  ...
				  const char *output_file, FILE *ofileid)
{
	...
	CATCH(errmgr)
	{
		...
		SymbTable_ptr st = BaseEnc_get_symb_table(BASE_ENC(bool_enc));
       ...
       been committed */
		arrayForEachItem(const char *,
				 SymbTable_get_class_layer_names(st, NULL), i,
				 name)
		{
			SymbLayer_ptr layer = SymbTable_get_layer(st, name);
			if (NodeList_belongs_to(BaseEnc_get_committed_layers(
							BASE_ENC(bool_enc)),
						(node_ptr)layer)) {
			...
			{ /* gets and add the corresponding boolean layer if exists */
				...
				if (layer != SYMB_LAYER(NULL) &&
				    !NodeList_belongs_to(layers,
							 (node_ptr)layer) &&
				    NodeList_belongs_to(
					    BaseEnc_get_committed_layers(
						    BASE_ENC(bool_enc)),
					    (node_ptr)layer)) {
					NodeList_append(layers,
							(node_ptr)layer);
				}
			}
		}
		...
		Compile_WriteBoolModel(
			env, ofileid, enc, layers, "MODULE main",
		...
		if ((BddEnc_get_reordering_count(enc) > 0) &&
			...
			BddEnc_write_var_ordering(
				enc, get_output_order_file(opts),
