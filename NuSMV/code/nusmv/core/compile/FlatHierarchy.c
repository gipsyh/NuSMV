...
struct FlatHierarchy {
	...
	hash_ptr assign_hash; /* a hash table that for every variable
                           'var_name' the hash contains the following
                           associations: init(var_name) -> CONS(rhs, init_list)
                           where rhs is the right handsides of init-assignments
                           of the given variable init-list is a list of INIT
                           expressions of this hierarchy, which contain the
                           variable next(var_name) -> CONS( rhs, trans-list)
                           where rhs can be
                           ...
                           var_name : the structure is the same as for
                           init(var_name) except that the rhs-n are the right
                           handside of invar-assignment, and init-list is a list
                           of INVAR expressions  */
...
static void flat_hierarchy_mergeinto(FlatHierarchy_ptr self,
				     FlatHierarchy_ptr other);

static void flat_hierarchy_calc_var_const_aux(FlatHierarchy_ptr self,
					      node_ptr expr, int type);

static void flat_hierarchy_calc_var_const_recur(FlatHierarchy_ptr self,
						node_ptr expr, int type);

static boolean flat_hierarchy_check_const_deps(FlatHierarchy_ptr self,
					       node_ptr expr, int type);

static void flat_hiearchy_self_check_expr(const FlatHierarchy_ptr self,
					  node_ptr expr);
...
static assoc_retval flat_hierarchy_free_assign_fun(char *key, char *data,
						   char *arg);
/*---------------------------------------------------------------------------*/
...
void FlatHierarchy_set_init(FlatHierarchy_ptr self, node_ptr n)
{
	...
	self->vars_constr_built = self->vars_constr_built &&
				  (self->init_expr == n);
	self->init_expr = n;
}
...
/* Warning: after flattening is done this function returns has a list
   ...
   actual assignments used in FSM it is necessary to access
   FlatHierarchy_lookup_assign as it is done in, for example,
...
boolean FlatHierarchy_add_property_name(FlatHierarchy_ptr self, node_ptr name)
{
					    ...
					    hash_ptr *outbound_edges)
{
	...
	ASSOC_FOREACH(self->assign_hash, iter, &var, &assignment)
	{
		...
		if (is_invar_assignment) {
			...
			nodes = Set_AddMember(
				nodes, (Set_Element_t)ExprMgr_next(exprs, var,
								   symb_table));
		}

		{
			...
			SET_FOREACH(deps, iter)
			{
				...
				/* Create the edge */
				if (dep != var) {
					out = Set_AddMember(out,
							    (Set_Element_t)var);
					if (is_invar_assignment) {
						/* invar (normal) Assignment: We add both init(x) and next(x) */
						out = Set_AddMember(
							out,
							(Set_Element_t)ExprMgr_next(
								exprs, var,
								symb_table));
					}
				}
				insert_assoc(outbounds, dep,
					     (Set_Element_t)out);
				nodes = Set_AddMember(nodes,
						      (Set_Element_t)dep);
			}
		}
	...
	/* Topological sort */
	{
		...
		SET_FOREACH(nodes, iter)
		{
			...
			flat_hierarchy_visit_dag(outbounds, visited, var,
						 result);
		}
...
node_ptr FlatHierarchy_lookup_assign(FlatHierarchy_ptr self, node_ptr name)
{
	...
	nusmv_assert(
		SMALLINIT == node_get_type(name) ||
		NEXT == node_get_type(name) || DOT == node_get_type(name) ||
		ATOM == node_get_type(name) || ARRAY == node_get_type(name) ||
				 ...
				 node_ptr assign)
{
	...
	if (Nil ==
	    cont) { /* there was no container before => create a new one */
...
node_ptr FlatHierarchy_lookup_constrains(FlatHierarchy_ptr self, node_ptr name)
{
	...
	nusmv_assert(SMALLINIT == node_get_type(name) ||
		     ...
		     BIT == node_get_type(name) || ATOM == node_get_type(name));
				  ...
				  node_ptr expr)
{
	...
	if (Nil == cdr(cont))
		...
		setcdr(cont, find_node(self->nodemgr, AND, cdr(cont), expr));
}
...
void FlatHierarchy_calculate_vars_constrains(FlatHierarchy_ptr self)
{
	...
	flat_hierarchy_calc_var_const_aux(self, FlatHierarchy_get_init(self),
					  INIT);
	flat_hierarchy_calc_var_const_aux(self, FlatHierarchy_get_invar(self),
					  INVAR);
	flat_hierarchy_calc_var_const_aux(self, FlatHierarchy_get_trans(self),
					  TRANS);
...
void FlatHierarchy_type_check(FlatHierarchy_ptr self) THROWS_EXCEPTION
{
	NuSMVEnv_ptr const env =
		EnvObject_get_environment(ENV_OBJECT(self->st));
	...
	isOk = isOk && TypeChecker_check_constrains(
			       type_checker, FlatHierarchy_get_init(self),
			       FlatHierarchy_get_trans(self),
			       FlatHierarchy_get_invar(self), assign_list,
				...
				FlatHierarchy_ptr other)
{
	...
	clear_assoc(other->assign_hash);
	{
		...
		{
			node_ptr new_cons = cons(self->nodemgr, car(old_cons),
						 cdr(old_cons));
				     ...
				     const FlatHierarchy_ptr other)
{
	...
	self->invar_expr = NODE_PTR(ExprMgr_and_nil(
		exprs, EXPR(self->invar_expr), EXPR(other->invar_expr)));
	self->trans_expr = NODE_PTR(ExprMgr_and_nil(
		exprs, EXPR(self->trans_expr), EXPR(other->trans_expr)));
	self->input_expr = NODE_PTR(ExprMgr_and_nil(
		exprs, EXPR(self->input_expr), EXPR(other->input_expr)));
   ...
   * append simply the module assigments to self */
	{
       ...
       size of other+self */
		...
		for (iter = other->assign_expr; iter != Nil; iter = cdr(iter)) {
			...
			if (inst_assgn != Nil &&
			    !ExprMgr_is_true(exprs, inst_assgn)) {
				/* stores the assignement to be used later when traversing self */
				node_ptr inst_name = car(assgn);
				node_ptr old_assgn = ExprMgr_and_nil(
					exprs,
					find_assoc(other_assgns, inst_name),
					inst_assgn);
				insert_assoc(other_assgns, inst_name,
					     old_assgn);
			}
		}
     ...
     * found in other */
		for (iter = self->assign_expr; iter != Nil; iter = cdr(iter)) {
			...
			if (other_assgn != Nil) {
				...
				inst_assgn = ExprMgr_and_nil(exprs, inst_assgn,
							     other_assgn);
		...
		{
			self->assign_expr = cons(
				self->nodemgr, cons(self->nodemgr, name, assgn),
				self->assign_expr);
		}
	...
	{
		node_ptr tmp =
			FlatHierarchy_lookup_constant_constrains(other, INIT);
	...
	self->justice_expr =
		append_ns(nodemgr, self->justice_expr, other->justice_expr);
	self->compassion_expr = append_ns(nodemgr, self->compassion_expr,
					  other->compassion_expr);
	self->spec_expr = append_ns(nodemgr, self->spec_expr, other->spec_expr);
	self->ltlspec_expr =
		append_ns(nodemgr, self->ltlspec_expr, other->ltlspec_expr);
	self->invarspec_expr =
		append_ns(nodemgr, self->invarspec_expr, other->invarspec_expr);
	self->pslspec_expr =
		append_ns(nodemgr, self->pslspec_expr, other->pslspec_expr);
	self->compute_expr =
		append_ns(nodemgr, self->compute_expr, other->compute_expr);

	/* pred_list and mirror_list : append other to self */
	self->pred_list = append_ns(nodemgr, self->pred_list, other->pred_list);

	/* [MD] Mirror list was not merged, I think that it was a bug */
	self->mirror_list =
		append_ns(nodemgr, self->mirror_list, other->mirror_list);
     ...
     above. */
	{
		...
		SET_FOREACH(intersect, iter)
		{
			...
			node_ptr init_var =
				find_node(self->nodemgr, SMALLINIT, var, Nil);
			node_ptr next_var =
				find_node(self->nodemgr, NEXT, var, Nil);
			boolean self_has_init, self_has_next, self_has_invar;
			boolean other_has_init, other_has_next, other_has_invar;

			self_has_init = (Nil != FlatHierarchy_lookup_assign(
							self, init_var));
			self_has_next = (Nil != FlatHierarchy_lookup_assign(
							self, next_var));
			self_has_invar =
				(Nil != FlatHierarchy_lookup_assign(self, var));

			other_has_init = (Nil != FlatHierarchy_lookup_assign(
							 other, init_var));
			other_has_next = (Nil != FlatHierarchy_lookup_assign(
							 other, next_var));
			other_has_invar = (Nil != FlatHierarchy_lookup_assign(
							  other, var));
...

	{
		...
		{
			nusmv_assert(NODE_FROM_INT(1) !=
				     find_assoc(self->property_hash, name));
			insert_assoc(self->property_hash, name,
				     NODE_FROM_INT(1));
		}
	}
}
	...
	if (node_get_type(expr) == AND) {
       ...
       constrains. */
		...
		SET_FOREACH(deps, iter)
		{
			...
			switch (type) {
			case INIT:
				index = find_node(self->nodemgr, SMALLINIT, var,
						  Nil);
				break;
			case INVAR:
				index = var;
				break;
			case TRANS:
				index = find_node(self->nodemgr, NEXT, var,
						  Nil);
				break;
			default: {
					       ...
					       node_ptr expr, int type)
{
	...
	if (opt_verbose_level_gt(opts, 1)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "Constant expression found in a %s statement in",
					  ...
					  node_ptr expr)
{
	...
	if (!Set_Contains(self->var_set, deps)) {
		...
		StreamMgr_nprint_error(streams, wffprint,
				       "The dependency set of expr\n%N\n",
				       expr);

		Set_PrintSet(wffprint, StreamMgr_get_error_stream(streams),
			     deps, NULL, NULL);

		StreamMgr_print_error(
			streams,
			"is not cointained in the set of vars of the flat hierarchy");

		Set_PrintSet(wffprint, StreamMgr_get_error_stream(streams),
			     self->var_set, NULL, NULL);

		ErrorMgr_internal_error(errmgr,
					"FlatHierachy failed self-check.");
	}
				     ...
				     node_ptr var, NodeList_ptr result)
{
	...
	if (Nil == find_assoc(visited, var)) {
		...
		SET_FOREACH(outb, iter)
		{
			...
			flat_hierarchy_visit_dag(outbounds, visited, out,
						 result);
		}
