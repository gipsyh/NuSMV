...
void FlatHierarchy_add_invar(FlatHierarchy_ptr self, node_ptr n) {
  ...
  self->vars_constr_built =
      self->vars_constr_built && (self->invar_expr == old);
}
...
void FlatHierarchy_add_trans(FlatHierarchy_ptr self, node_ptr n) {
  ...
  self->vars_constr_built =
      self->vars_constr_built && (self->trans_expr == old);
}
...
NodeList_ptr FlatHierarchy_get_ordered_vars(const FlatHierarchy_ptr self,
  ...
  ASSOC_FOREACH(self->assign_hash, iter, &var, &assignment) {
    ...
    deps = Formula_GetDependenciesByType(
        symb_table, MasterNormalizer_normalize_node(normalizer, assignment),
        Nil, VFT_CNIF, true);
    ...
    if (is_invar_assignment) {
      ...
      nodes = Set_AddMember(
          nodes, (Set_Element_t)ExprMgr_next(exprs, var, symb_table));
    }
      ...
      SET_FOREACH(deps, iter) {
        ...
        if (dep != var) {
          ...
          if (is_invar_assignment) {
            ...
            out = Set_AddMember(
                out, (Set_Element_t)ExprMgr_next(exprs, var, symb_table));
          }
        }
  ...
  /* If the caller did not request the hash map, free it */
    ...
    ASSOC_FOREACH(outbounds, iter, &tmp, &set) { Set_ReleaseSet(set); }
  }
...
void FlatHierarchy_insert_assign(FlatHierarchy_ptr self, node_ptr name,
  ...
  nusmv_assert(
      (node_get_type(name) != SMALLINIT && node_get_type(name) != DOT) ||
      Nil == car(cont));
...
void FlatHierarchy_type_check(FlatHierarchy_ptr self) THROWS_EXCEPTION {
  ...
  isOk =
      isOk && TypeChecker_check_constrains(
                  type_checker, FlatHierarchy_get_init(self),
                  FlatHierarchy_get_trans(self), FlatHierarchy_get_invar(self),
                  assign_list, FlatHierarchy_get_justice(self),
...
static void flat_hierarchy_deinit(FlatHierarchy_ptr self) {
  ...
  clear_assoc_and_free_entries_arg(
      self->assign_hash, flat_hierarchy_free_assign_fun, (char *)self->nodemgr);
...
static void flat_hierarchy_mergeinto(FlatHierarchy_ptr self,
  ...
  self->init_expr = NODE_PTR(
      ExprMgr_and_nil(exprs, EXPR(self->init_expr), EXPR(other->init_expr)));
  self->invar_expr = NODE_PTR(
      ExprMgr_and_nil(exprs, EXPR(self->invar_expr), EXPR(other->invar_expr)));
  self->trans_expr = NODE_PTR(
      ExprMgr_and_nil(exprs, EXPR(self->trans_expr), EXPR(other->trans_expr)));
  self->input_expr = NODE_PTR(
      ExprMgr_and_nil(exprs, EXPR(self->input_expr), EXPR(other->input_expr)));
   ...
   * append simply the module assigments to self */
  {
    ...
    for (iter = other->assign_expr; iter != Nil; iter = cdr(iter)) {
      ...
      if (inst_assgn != Nil && !ExprMgr_is_true(exprs, inst_assgn)) {
        ...
        node_ptr old_assgn = ExprMgr_and_nil(
            exprs, find_assoc(other_assgns, inst_name), inst_assgn);
    ...
    ASSOC_FOREACH(other_assgns, aiter, &name, &assgn) {
      self->assign_expr = cons(self->nodemgr, cons(self->nodemgr, name, assgn),
                               self->assign_expr);
    }
  ...
  self->compassion_expr =
      append_ns(nodemgr, self->compassion_expr, other->compassion_expr);
  ...
  self->mirror_list = append_ns(nodemgr, self->mirror_list, other->mirror_list);

  self->property_patterns =
      append_ns(nodemgr, self->property_patterns, other->property_patterns);
     ...
     above. */
  {
    ...
    SET_FOREACH(intersect, iter) {
       ...
       */
      nusmv_assert((!other_has_invar) | ((!self_has_init) && (!self_has_next)));
...
static void flat_hierarchy_calc_var_const_recur(FlatHierarchy_ptr self,
  ...
  if (node_get_type(expr) == AND) {
    ...
    SET_FOREACH(deps, iter) {
      ...
      switch (type) {
      case INIT:
        index = find_node(self->nodemgr, SMALLINIT, var, Nil);
        break;
      case INVAR:
        index = var;
        break;
      case TRANS:
        index = find_node(self->nodemgr, NEXT, var, Nil);
        break;
      default: {
...
static void flat_hiearchy_self_check_expr(const FlatHierarchy_ptr self,
  ...
  if (!Set_Contains(self->var_set, deps)) {
    ...
    Set_PrintSet(wffprint, StreamMgr_get_error_stream(streams), deps, NULL,
                 NULL);

    StreamMgr_print_error(
        streams, "is not cointained in the set of vars of the flat hierarchy");

    Set_PrintSet(wffprint, StreamMgr_get_error_stream(streams), self->var_set,
                 NULL, NULL);
