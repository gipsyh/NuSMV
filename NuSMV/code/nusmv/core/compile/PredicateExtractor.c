	...
	PredicateExtractor_ptr self, FlatHierarchy_ptr fh)
{
	...
	node_ptr (*fh_access[])(
		FlatHierarchy_ptr) = { FlatHierarchy_get_init,
  ...
  */
	layers_name =
		SymbTable_get_class_layer_names(self->st, (const char *)NULL);

	arrayForEachItem(const char *, layers_name, i, a_layer_name)
	{
		SymbLayer_ptr layer =
			SymbTable_get_layer(self->st, a_layer_name);
		SymbLayerIter iter;
		...
		SYMB_LAYER_FOREACH(layer, iter, STT_VAR)
		{
			...
			node_ptr invar_expr =
				FlatHierarchy_lookup_assign(fh, name);
			node_ptr init_expr =
				FlatHierarchy_lookup_assign(fh, init_name);
			node_ptr next_expr =
				FlatHierarchy_lookup_assign(fh, next_name);

			if (invar_expr != Nil) {
				expr = find_node(nodemgr, EQDEF, name,
						 invar_expr);
				PredicateExtractor_compute_preds(self, expr);
			}
			if (init_expr != Nil) {
				expr = find_node(nodemgr, EQDEF, init_name,
						 init_expr);
				PredicateExtractor_compute_preds(self, expr);
			}
			if (next_expr != Nil) {
				expr = find_node(nodemgr, EQDEF, next_name,
						 next_expr);
				PredicateExtractor_compute_preds(self, expr);
			}
		}
...
Set_t PredicateExtractor_get_all_clusters(const PredicateExtractor_ptr self)
{
	...
	if (!Set_IsEmpty(self->unclustered_preds)) {
		...
		{
			node_ptr predicate = Set_GetMember(
				self->unclustered_preds, pred_iter);
			Set_t deps = Formula_GetDependencies(self->st,
							     predicate, Nil);
      ...
      */
           ...
           existing one */
				...
				Set_t cluster = (Set_t)find_assoc(
					self->var2cluster, var);

				/* if there is no cluster => create a new one */
				if (NULL == cluster) {
					cluster = Set_MakeSingleton(var);
					insert_assoc(self->var2cluster, var,
						     NODE_PTR(cluster));
					self->all_clusters = Set_AddMember(
						self->all_clusters,
						NODE_PTR(cluster));
					...
					insert_assoc(
						self->cluster2preds,
						NODE_PTR(cluster),
						NODE_PTR(Set_MakeSingleton(
							NODE_PTR(predicate))));
				}
				/* cluster already exist => insert the predicate into existing
           cluster2preds associated */
				else {
					Set_t cl_preds = (Set_t)find_assoc(
						self->cluster2preds,
						NODE_PTR(cluster));
					Set_t tmp;
					nusmv_assert(
						NULL !=
						cl_preds); /* every cluster has some predicate */
					tmp = Set_AddMember(cl_preds,
							    predicate);
					nusmv_assert(
						tmp ==
						cl_preds); /* debug: the pointer did not change */
				}
           ...
           the clusters */
             ...
             hash: var -> cluster and hash: cluster->preds */
               ...
               only cluster associations have to be changed */
						...
						cl_preds = (Set_t)find_assoc(
							self->cluster2preds,
							NODE_PTR(cluster));
						other_preds = (Set_t)find_assoc(
							self->cluster2preds,
							NODE_PTR(
								another_cluster));
			      ...
			      boolean printPredicates, boolean printClusters)
{
	...
	SET_FOREACH(set, iter)
	{
		...
		SET_FOREACH(cluster, sit)
		{
			...
			fprintf(stream, " : ");
			SymbType_print(SymbTable_get_var_type(self->st, var),
				       wffprint, stream);
			fprintf(stream, "\n");
		}
		fprintf(stream, " \t ]\n");
					...
					node_ptr expr, node_ptr context)
{
	...
	case CONTEXT: {
		node_ptr new_ctx =
			CompileFlatten_concat_contexts(env, context, car(expr));
	...
	case SWCONST:
	case TWODOTS:
		tmp = ExprMgr_resolve(exprs, self->st, node_type, car(expr),
				      cdr(expr));
	...
	case NFUNCTION: {
       ...
       consider the expression */
		while (Nil != list) {
			Set_t elem = pred_extract_process_recur(self, car(list),
								context);
	...
	case ARRAY: {
		...
		if (ResolveSymbol_is_undefined(rs)) {
         ...
         predicate extractor is to be invoked */
			node_ptr tmp =
				Compile_FlattenSexp(self->st, expr, context);
			nusmv_assert(
				tmp !=
    ...
    */
		...
		/* check whether is a define */
			...
			node_ptr def = SymbTable_get_define_body(self->st,
								 resolvedName);
			node_ptr ctx = SymbTable_get_define_context(
				self->st, resolvedName);
      ...
      */
			if (ARRAY == node_type && !SymbType_is_boolean(type) &&
			    SymbType_is_boolean(TypeChecker_get_expression_type(
				    self->checker, def, ctx))) {
       ...
       takes care of simple/complex constants */
		...
		/* check whether this symbol is a parameter */
         ...
         possibility */
			...
			new_ctx = SymbTable_get_actual_parameter_context(
				self->st, resolvedName);

			result = pred_extract_process_recur(self, param,
							    new_ctx);
		}
	...
	case NOT:
		nusmv_assert(
			Nil ==
			cdr(expr)); /* checking that indeed no right child */
		result = PREDICATES_OVERAPPROX;
		...
		if (!IS_OVER_APPROX(left)) {
			if (SymbType_is_boolean(type)) {
				...
				if (!IS_FLAG_VALID_PREDICATES(left)) {
					result = pred_extract_apply_unary(
						self, NOT, left);
				}
			}
		}
	...
	case EU:
		...
		if (!IS_OVER_APPROX(left)) {
			if (Nil != cdr(expr)) {
				right = pred_extract_process_recur(
           ...
           i.e.  apply the binary operator to results. */
				...
				if (!IS_OVER_APPROX(right)) {
					if (!IS_FLAG_VALID_PREDICATES(left) &&
						...
						result =
							pred_extract_apply_binary(
								self, node_type,
								left, right);
					} else {
						result =
							Set_MakeSingleton(expr);
					}
				}
			}
		}
    ...
    */
		if (!IS_FLAG_PREDICATES(result)) {
			...
			result = pred_extract_apply_unary(self, node_type,
							  result);
		} else {
	...
	case LE:
	case GE: {
		SymbType_ptr type1 = TypeChecker_get_expression_type(
			self->checker, car(expr), context);
		SymbType_ptr type2 = TypeChecker_get_expression_type(
			self->checker, cdr(expr), context);
		...
		if ((SymbType_is_boolean(type1) ||
			...
			case SETIN:
			case EQUAL:
				result = (left == PREDICATES_ARBITRARY ||
					  right == PREDICATES_ARBITRARY) ?
						 PREDICATES_ARBITRARY :
						 (left == right ?
							  PREDICATES_TRUE :
							  PREDICATES_FALSE);
         ...
         operator */
			if (!IS_FLAG_VALID_PREDICATES(left) &&
			    !IS_FLAG_VALID_PREDICATES(right)) {
				result = pred_extract_apply_binary(
					self, node_type, left, right);
			} else {
				result = Set_MakeSingleton(expr);
			}
	...
	case CONST_ARRAY: {
		...
		left = pred_extract_process_recur(self, car(expr), context);
		if (!IS_OVER_APPROX(left)) {
			right = pred_extract_process_recur(self, cdr(expr),
							   context);
			if (!IS_FLAG_VALID_PREDICATES(left) &&
			    !IS_FLAG_VALID_PREDICATES(right)) {
				result = pred_extract_apply_binary(
	...
	case WAREAD: {
		result = PREDICATES_OVERAPPROX;
		if (!SymbType_is_boolean(type)) {
			left = pred_extract_process_recur(self, car(expr),
							  context);
			if (!IS_OVER_APPROX(left)) {
				right = pred_extract_process_recur(
					self, cdr(expr), context);
	...
	case WAWRITE: {
		...
		if (!IS_OVER_APPROX(left) && !IS_FLAG_VALID_PREDICATES(left)) {
			Set_t index, value;
			index = pred_extract_process_recur(self, car(cdr(expr)),
							   context);
			if (!IS_OVER_APPROX(index)) {
				value = pred_extract_process_recur(
					self, cdr(cdr(expr)), context);
				if (!IS_OVER_APPROX(value)) {
					if (!IS_FLAG_VALID_PREDICATES(index) &&
					    !IS_FLAG_VALID_PREDICATES(value)) {
						right = pred_extract_apply_binary(
	...
	case BIT_SELECTION: {
		...
		if (!IS_OVER_APPROX(left)) {
			if (!IS_FLAG_VALID_PREDICATES(left)) {
				right = Set_MakeSingleton(find_node(
					nodemgr, COLON,
	...
	case CAST_TOINT: {
		...
		if (!IS_OVER_APPROX(left)) {
			if (!IS_FLAG_VALID_PREDICATES(left)) {
				...
				node_ptr fexpr = Compile_FlattenSexp(
					self->st, expr, context);
    ...
    */
	...
	case CAST_BOOL: {
		...
		if (!IS_OVER_APPROX(left) && !IS_FLAG_VALID_PREDICATES(left)) {
			result = pred_extract_apply_binary(
				self, EQUAL, left, self->special_word_preds[1]);
		}
		/* remember the results */
		result = pred_extract_fix_any_preds(self, result);
		break;
	...
	case CAST_WORD1:
		...
		if (!IS_OVER_APPROX(left)) {
			...
			result = Set_Copy(
				result); /* create a copy of special predicate set */
		}
		break;
			...
			if (!IS_OVER_APPROX(right)) {
				if (SymbType_is_boolean(type) ||
					...
					if (left == right) {
					   ...
					   !IS_FLAG_VALID_PREDICATES(right)) {
					result = Set_Union(Set_Copy(left),
							   right);
				} else {
					result = Set_MakeSingleton(expr);
				}
       ...
       also if tail is FAILURE => ignore it. */
		if (cond == PREDICATES_TRUE || FAILURE == node_get_type(e)) {
			...
			if (!IS_OVER_APPROX(then)) {
				tail = pred_extract_process_recur(self, e,
								  context);
...
#else
	{
		const MasterPrinter_ptr sexpprinter = MASTER_PRINTER(
					  ...
					  Set_t result)
{
	...
	SET_FOREACH(result, iter)
	{
		...
		node_ptr unnexted = (node_get_type(expr) == NEXT ||
				     node_get_type(expr) == SMALLINIT) ?
					    car(expr) :
					    expr;
					...
					Set_t result)
{
	...
	nusmv_assert(!IS_FLAG_PREDICATES(
		result)); /* only proper sets are expected */
	...
	SET_FOREACH(result, iter)
	{
		...
		if (FALSEEXP == node_get_type(expr)) {
			...
			if (!Set_IsMember(self->all_preds, expr)) {
				self->all_preds =
					Set_AddMember(self->all_preds, expr);
				self->unclustered_preds = Set_AddMember(
					self->unclustered_preds, expr);
			}
		}
	} /* loop */
				       ...
				       Set_t leftResult, Set_t rightResult)
{
	...
	if (self->use_approx && (((size_t)Set_GiveCardinality(leftResult) *
				  (size_t)Set_GiveCardinality(rightResult)) >
				 OVER_APPROX_THRESHOLD)) {
		/* too-big: gives up */
		return PREDICATES_OVERAPPROX;
	}
	...
	SET_FOREACH(leftResult, l_iter)
	{
		...
		SET_FOREACH(rightResult, r_iter)
		{
			...
			nusmv_assert(Nil !=
				     r_expr); /* expression is well-formed */

			result = Set_AddMember(
				result, ExprMgr_resolve(exprs, self->st, type,
							l_expr, r_expr));
		}
	}
	return result;
