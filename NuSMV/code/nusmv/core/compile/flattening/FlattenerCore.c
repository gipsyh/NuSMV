...
node_ptr flattener_core_flatten(FlattenerBase_ptr self,
                   ...
                   as they are handled only at flattening time */
  {
    ...
    if ((node_ptr)NULL == width) {
      ErrorMgr_rpterr(errmgr, "width specifier of swconst/uwconst operator is "
                              "not a constant");
    }
    ...
    result = find_node(
        nodemgr, (UWCONST == type) ? NUMBER_UNSIGNED_WORD : NUMBER_SIGNED_WORD,
  ...
  case ATOM: {
    ...
    result = MasterCompileFlattener_get_definition(
        MASTER_COMPILE_FLATTENER((NODE_WALKER(self))->master), symb_table, name,
        mode);
  ...
  case SELF: {
    ...
    StreamMgr_print_error(streams, "flattener_core_flatten: invalid usage "
                                   "of identifier \"self\"\n");
  ...
  case ARRAY: {
    ...
    */
    ...
    if (IFTHENELSE == node_get_type(array) || CASE == node_get_type(array)) {
      ...
      */
      result = flattener_core_push_array_index_down(self, array, index, is_next,
                                                    symb_table, def_hash, mode);
      break;
    }
    ...
    if (NUMBER != node_get_type(index)) {
      ...
      */
      result = flattener_core_construct_array_multiplexer(
          self, array, index, is_next, symb_table, def_hash, mode);
    ...
    /* we have a constant index => check that it is in the range */
    {
      ...
      if (val < lower_bound || val > upper_bound) {
        ErrorMgr_error_array_out_of_bounds(errmgr, val, lower_bound,
                                           upper_bound);
      }
    }
  ...
  case CAST_TOINT: {
    ...
    if (SymbType_is_infinite_precision(type) ||
      ...
      result =
          new_node(nodemgr, CASE, new_node(nodemgr, COLON, body, one), zero);
  ...
  case COUNT: {
    ...
    do {
      ...
      toint = _THROW(new_node(nodemgr, CAST_TOINT, cur_expr, Nil), symb_table,
  ...
  case CONST_ARRAY: {
    ...
    const_array_value = _THROW(cdr(sexp), symb_table, def_hash, context, mode);

    type = _compute_constarray_type(symb_table, array_type);

    if (SymbType_is_intarray(type) || SymbType_is_wordarray(type)) {
      result = new_node(nodemgr, CONST_ARRAY, array_type, const_array_value);
      break;
    }
    ...
    if (SymbType_is_array(type)) {
      ...
      result = find_node(nodemgr, INTERNAL_ARRAY_STRUCT,
                         find_node(nodemgr, TWODOTS, NODE_FROM_INT(lower_bound),
  ...
  case NOTEQUAL: {
    ...
    if (SymbTable_is_symbol_variable_array(symb_table, left) ||
      ...
      if (SymbTable_is_symbol_variable_array(symb_table, left)) {
        SymbType_ptr type = SymbTable_get_variable_array_type(symb_table, left);
      ...
      /* array may be an array define */
        ...
        node_ptr body = SymbTable_get_array_define_body(symb_table, left);
...

      {
        ...
        for (i = left_upper_bound; i >= left_lower_bound; --i) {
          ...
          cond = ((node_get_type(sexp) == EQUAL)
                      ? ExprMgr_equal(exprs, left_body, right_body, symb_table)
                      : ExprMgr_notequal(exprs, left_body, right_body,
                                         symb_table));
    ...
    if (node_get_type(array) == INTERNAL_ARRAY_STRUCT) {
      ...
      if (Nil != tmp) { /* The current index is constant */
        ...
        if (i < lower_bound || i > upper_bound) {
          ErrorMgr_error_array_out_of_bounds(errmgr, i, lower_bound,
                                             upper_bound);
        }
      ...
      /* multiplexer case */
        ...
        for (i = upper_bound; i >= lower_bound; --i) {
          ...
          result = ExprMgr_ite(exprs, eq, local_array[i - lower_bound], result,
                               symb_table);
        }
      }
  ...
  case WAWRITE: {
    ...
    if (SymbType_is_intarray(arraytype) || SymbType_is_wordarray(arraytype)) {
      ...
      if (tmp != Nil) { /* The current index is constant */
...

        {
          ...
          if (j < lower_bound || j > upper_bound) {
            ErrorMgr_error_array_out_of_bounds(errmgr, j, lower_bound,
                                               upper_bound);
          }
        }
      }
    ...
    if (!is_local_array) {
      ...
      for (i = upper_bound; i >= lower_bound; --i) {
        ...
        if (SymbType_is_word(indextype)) {
          ...
          if (SymbType_is_signed_word(indextype)) {
            num =
                find_node(nodemgr, NUMBER_SIGNED_WORD,
                          (node_ptr)WordNumberMgr_signed_integer_to_word_number(
                              words, (WordNumberValue)i, size),
                          Nil);
          } else {
      ...
      for (i = upper_bound; i >= lower_bound; --i) {
        if (is_index_constant) { /* constant case */
          ...
          neq = ExprMgr_notequal(exprs, index, num, symb_table);
          local_array[i - lower_bound] = ExprMgr_ite(
              exprs, neq, local_array[i - lower_bound], value, symb_table);
        }
      }
    }
  ...
  case WRESIZE: {
    ...
    { /* just check */
      node_ptr value =
          CompileFlatten_resolve_number(symb_table, right, context);
  ...
  case EXTEND: {
    ...
    {
      node_ptr value =
          CompileFlatten_resolve_number(symb_table, delta, context);
  ...
  case OP_HISTORICAL: {
    ...
    node_ptr right = ((Nil != cdr(sexp)) ? _THROW(cdr(sexp), symb_table,
                                                  def_hash, context, mode)
                                         : Nil);
  ...
  case EQDEF: {
    ...
    if ((IFTHENELSE == node_get_type(name)) || (CASE == node_get_type(name))) {
      /* Note about array defines: Dynamic indexes are not allowed
           in assignment. Thus no if-then-else expression is
           possible here. */
      ErrorMgr_rpterr(errmgr, "Expressions not allowed in array subscripts on "
    ...
    if (SymbTable_is_symbol_variable_array(symb_table, name)) {
      result = _flatten_resolve_variable_array_eqdef(self, symb_table, def_hash,
  ...
  case NFUNCTION: {
    ...
    if (ResolveSymbol_is_parameter(rs)) {
      ...
      param = SymbTable_get_flatten_actual_parameter(symb_table, resolvedName);
      result = _THROW(param, symb_table, def_hash, context, mode);
...

  {
    ...
    if (SymbType_is_word(indextype)) {
      ...
      if (SymbType_is_unsigned_word(indextype) && lower_bound < 0) {
        lower_bound = 0;
      } else if (SymbType_is_signed_word(indextype) && lower_bound < 0 &&
...
static node_ptr flattener_core_push_array_index_down(
  ...
  case IFTHENELSE: {
    node_ptr left, right;
    left = flattener_core_push_array_index_down(
        self, car(array), index, is_array_next, st, def_hash, mode);
    right = flattener_core_push_array_index_down(
        self, cdr(array), index, is_array_next, st, def_hash, mode);
