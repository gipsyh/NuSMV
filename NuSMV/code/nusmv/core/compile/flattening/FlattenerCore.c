...
static node_ptr flattener_core_construct_array_multiplexer(
	FlattenerBase_ptr self, node_ptr array, node_ptr index,
	boolean is_array_next, SymbTable_ptr symb_table, hash_ptr def_hash,
	const MasterCompileFlattener_def_mode mode);

static node_ptr flattener_core_push_array_index_down(
	FlattenerBase_ptr self, node_ptr array, node_ptr index,
	boolean is_array_next, SymbTable_ptr st, hash_ptr def_hash,
	const MasterCompileFlattener_def_mode mode);

static SymbType_ptr _compute_constarray_type(SymbTable_ptr symb_table,
					     node_ptr array_type);

static node_ptr _resolve_name_or_nop(SymbTable_ptr, node_ptr, node_ptr);

static node_ptr _flatten_resolve_variable_array_eqdef(
	FlattenerBase_ptr, SymbTable_ptr, hash_ptr, node_ptr, int, node_ptr,
...
FlattenerCore_ptr FlattenerCore_create(const NuSMVEnv_ptr env, const char *name)
{
	...
	flattener_core_init(self, env, name, NUSMV_CORE_SYMBOL_FIRST,
...

{
	...
	flattener_base_init(FLATTENER_BASE(self), env, name, low, num,
			    true /*handles NULL*/);
				...
				MasterCompileFlattener_def_mode mode)
{
                   ...
                   as they are handled only at flattening time */
	{
		...
		value = CompileFlatten_resolve_number(symb_table, value,
						      context);
		width = CompileFlatten_resolve_number(symb_table, width,
						      context);
		...
		case NUMBER_UNSIGNED_WORD:
			tmp = WordNumber_get_unsigned_value(
				WORD_NUMBER(car(width)));
			width_int = tmp;
			if (tmp != width_int) {
				ErrorMgr_rpterr(
					errmgr,
		...
		case NUMBER_SIGNED_WORD:
			tmp = WordNumber_get_signed_value(
				WORD_NUMBER(car(width)));
			width_int = tmp;
			if (tmp != width_int) {
				ErrorMgr_rpterr(
		...
		if (width_int <= 0 || width_int > WordNumber_max_width()) {
			ErrorMgr_rpterr(
				errmgr,
				"width specifier is out of range [0, %i]",
       ...
       overflow. */
		if ((value_int > 0 &&
		     ((UWCONST == type &&
		       value_int >> (width_int - 1) >> 1 != 0) ||
		      (SWCONST == type &&
		       value_int >> (width_int - 2) >> 1 != 0))) ||
		...
		value_word =
			(value_int >= 0) ?
				WordNumberMgr_integer_to_word_number(
					words, (WordNumberValue)value_int,
					width_int) :
		...
		result = find_node(nodemgr,
				   (UWCONST == type) ? NUMBER_UNSIGNED_WORD :
						       NUMBER_SIGNED_WORD,
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_parameter(rs)) {
			...
			result = _THROW(param, symb_table, def_hash, context,
					mode);
			break;
		}

		/* It can be a defined symbol, a running condition or a variable */
		result = MasterCompileFlattener_get_definition(
			MASTER_COMPILE_FLATTENER((NODE_WALKER(self))->master),
			symb_table, name, mode);
    ...
    */
       ...
       then return the results */
		if (Nil != name && ResolveSymbol_is_defined(rs)) {
			result = MasterCompileFlattener_get_definition(
				MASTER_COMPILE_FLATTENER(
					(NODE_WALKER(self))->master),
				symb_table, name, mode);
			break;
		}
		...
		if (NEXT ==
		    node_get_type(array)) { /* exp of form "next(m)[i]" */
			is_next = true;
			array = car(array);
		}
		...
		while (SymbTable_is_symbol_define(symb_table, array)) {
			node_ptr body =
				SymbTable_get_define_body(symb_table, array);
			node_ptr ctx =
				SymbTable_get_define_context(symb_table, array);
       ...
       and then try to get constant value if it is possible */
		...
		if (tmp != Nil)
			index = tmp; /* The current index is constant */
		...
		if (NUMBER_SIGNED_WORD == node_get_type(index) ||
			...
			WordNumberValue val = WordNumber_get_unsigned_value(
				(WordNumber_ptr)car(index));
			index = find_node(nodemgr, NUMBER,
					  NODE_FROM_INT((int)val), Nil);
      ...
      */
			result = flattener_core_push_array_index_down(
				self, array, index, is_next, symb_table,
				def_hash, mode);
			break;
		}
      ...
      */
		...
		/* we have a constant index => check that it is in the range */
		{
			SymbType_ptr type = TypeChecker_get_expression_type(
				SymbTable_get_type_checker(symb_table), array,
				Nil);
			int lower_bound, upper_bound, val;

			if (!SymbType_is_array(type)) {
				ErrorMgr_error_lhs_of_index_is_not_array(
					errmgr);
			}
       ...
       expand if required.*/
		...
		if (Flattener_Expand_Def_Mode == mode) {
			array = MasterCompileFlattener_get_definition(
				MASTER_COMPILE_FLATTENER(
					(NODE_WALKER(self))->master),
				symb_table, array, mode);
		}
		/* add the final next if required */
		if (is_next)
			array = find_node(nodemgr, NEXT, array, Nil);
		result = array;
		break;
	}
	...
	case CONTEXT:
		/* (CONTEXT (cxt . expr)) */
		result = _THROW(cdr(sexp), symb_table, def_hash, car(sexp),
				mode);
		break;
	...
	case FLOOR: {
		...
		if (SymbType_is_integer(type)) {
			...
			result = new_node(nodemgr, node_get_type(sexp), body,
					  Nil);
		break;
	}
	...
	case CAST_BOOL: {
		...
		if (SymbType_is_boolean(type)) {
			...
			/* int = 0 ? FALSE : TRUE */
			node_ptr zero = find_node(nodemgr, NUMBER,
						  NODE_FROM_INT(0), Nil);
			result = new_node(
				nodemgr, CASE,
	...
	case CAST_TOINT: {
		...
		if (SymbType_is_infinite_precision(type) ||
			...
			node_ptr one = find_node(nodemgr, NUMBER,
						 NODE_FROM_INT(1), Nil);
			node_ptr zero = find_node(nodemgr, NUMBER,
						  NODE_FROM_INT(0), Nil);

			result = new_node(nodemgr, CASE,
					  new_node(nodemgr, COLON, body, one),
					  zero);
		} else if (SymbType_is_word(type)) {
			result = compile_flatten_rewrite_word_toint_cast(
				env, body, type);
		} else {
       ...
       parameter. */
		do {
         ...
         not a good practice. */
			...
			type = TypeChecker_get_expression_type(tc, cur_expr,
							       context);
			if (!SymbType_is_boolean(type)) {
				ErrorMgr_error_invalid_count_operator(errmgr,
								      cur_expr);
			}

			/* Rewrite the expression as toint(expr) */
			toint = _THROW(new_node(nodemgr, CAST_TOINT, cur_expr,
						Nil),
			...
			if (Nil == new_expr) {
				...
				new_expr = new_node(nodemgr, PLUS, toint,
						    new_expr);
			}
	...
	case CONST_ARRAY: {
		...
		if (SymbType_is_intarray(type) || SymbType_is_wordarray(type)) {
			result = new_node(nodemgr, CONST_ARRAY, array_type,
					  const_array_value);
			break;
		}
		...
		if (SymbType_is_array(type)) {
			...
			SymbType_ptr const_array_type =
				SymbTable_get_variable_array_type(
					symb_table, car(array_type));
			int lower_bound = SymbType_get_array_lower_bound(
				const_array_type);
			int upper_bound = SymbType_get_array_upper_bound(
				const_array_type);
			...
			result =
				find_node(nodemgr, INTERNAL_ARRAY_STRUCT,
	...
	case NOTEQUAL: {
       ...
       left is internal array structure */
		if (SymbTable_is_symbol_variable_array(symb_table, left) ||
		    (node_get_type(left) == NEXT &&
		     SymbTable_is_symbol_variable_array(symb_table,
							car(left))) ||
         ...
         First, check if the id is an array variable */
			if (SymbTable_is_symbol_variable_array(symb_table,
							       left)) {
				SymbType_ptr type =
					SymbTable_get_variable_array_type(
						symb_table, left);
			...
			else if (SymbTable_is_symbol_array_define(symb_table,
								  left)) {
				node_ptr body = SymbTable_get_array_define_body(
					symb_table, left);
			...
			/* internal array structure case */
				...
				nusmv_assert(TWODOTS ==
					     node_get_type(array_size));
        ...
        */
           ...
           or array define can be removed */
			...
			left_res = ErrorMgr_failure_make(
				errmgr, "array access out of bounds",
				FAILURE_ARRAY_OUT_OF_BOUNDS, nusmv_yylineno);
         ...
         First, check if the id is an array variable */
			...
			/* internal array structure case */
				...
				right_lower_bound =
					NODE_TO_INT(car(array_size));
				right_upper_bound =
					NODE_TO_INT(cdr(array_size));
			} else {
				/* array has to be array var or array define but it is not.
           Actually, since type checking is not done yet LHS of
           array subscripting can be invalid expression.
        */
           ...
           or array define can be removed */
			...
			/* Build the out of bounds case */
			right_res = ErrorMgr_failure_make(
				errmgr, "array access out of bounds",
				FAILURE_ARRAY_OUT_OF_BOUNDS, nusmv_yylineno);
			...
			{
				int i;
				for (i = left_upper_bound;
				     i >= left_lower_bound; --i) {
					node_ptr num;
					node_ptr cond;
               ...
               which are always resolved in ARRAY expression. */
						left_body = _THROW(left_body,
								   symb_table,
								   def_hash,
								   Nil, mode);
					} else {
						left_body = left_local_array
							[i - left_lower_bound];
					}
               ...
               2) the array ID (left child of ARRAY) may be a define
               which are always resolved in ARRAY expression. */
						right_body = _THROW(right_body,
								    symb_table,
								    def_hash,
								    Nil, mode);
					} else {
						right_body = right_local_array
							[i - left_lower_bound];
					}
           ...
           * further expand it */
			...
			result = new_node(nodemgr, node_get_type(sexp), left,
					  right);
		}
	...
	case WAREAD: {
		...
		node_ptr index =
			_THROW(cdr(sexp), symb_table, def_hash, context, mode);
		SymbType_ptr arraytype = TypeChecker_get_expression_type(
		...
		if (node_get_type(array) == INTERNAL_ARRAY_STRUCT) {
			...
			/* simple case */
			tmp = CompileFlatten_resolve_number(symb_table, index,
							    context);
			if (Nil != tmp) { /* The current index is constant */
				int i;
				index = tmp;
				/* convert word constant to integer constant */
				if (NUMBER_SIGNED_WORD ==
					    node_get_type(index) ||
				    NUMBER_UNSIGNED_WORD ==
					    node_get_type(index)) {
					WordNumberValue val =
						WordNumber_get_unsigned_value(
							(WordNumber_ptr)car(
								index));
					index = find_node(
						nodemgr, NUMBER,
			...
			/* multiplexer case */
				...
				const WordNumberMgr_ptr words =
					WORD_NUMBER_MGR(NuSMVEnv_get_value(
						env, ENV_WORD_NUMBER_MGR));
				const ExprMgr_ptr exprs = EXPR_MGR(
				...
				for (i = upper_bound; i >= lower_bound; --i) {
					...
					eq = ExprMgr_equal(exprs, index, num,
							   symb_table);
					result = ExprMgr_ite(
						exprs, eq,
	...
	case WAWRITE: {
		...
		node_ptr array =
			_THROW(car(sexp), symb_table, def_hash, context, mode);
		node_ptr index = _THROW(car(cdr(sexp)), symb_table, def_hash,
					context, mode);
		node_ptr value = _THROW(cdr(cdr(sexp)), symb_table, def_hash,
					context, mode);
		SymbType_ptr arraytype = TypeChecker_get_expression_type(
			SymbTable_get_type_checker(symb_table), array, Nil);
		SymbType_ptr indextype;
		...
		if (SymbType_is_intarray(arraytype) ||
			...
			result = new_node(nodemgr, WAWRITE, array,
					  new_node(nodemgr, WAWRITE, index,
						   value));
			break;
		}
    ...
    bounded arrays are treated as now arrays (subrange) type */
       ...
       First, check if the id is an array variable */
		if (SymbTable_is_symbol_variable_array(symb_table, array)) {
			SymbType_ptr type = SymbTable_get_variable_array_type(
				symb_table, array);
     ...
     * local array */
		if (!is_local_array) {
			...
			local_array =
				ALLOC(node_ptr, upper_bound - lower_bound + 1);
		}

		indextype = TypeChecker_get_expression_type(
       ...
       we may limit the range of indexes to word domain */
		if (SymbType_is_word(indextype)) {
			...
			if (SymbType_is_signed_word(indextype)) {
				maxWord = WordNumberMgr_max_signed_value(words,
									 size);
				maxValue = WordNumber_get_signed_value(maxWord);
			} else {
				maxWord = WordNumberMgr_max_unsigned_value(
					words, size);
				maxValue =
		...
		{ /* check index is constant */
			node_ptr tmp = CompileFlatten_resolve_number(
				symb_table, index, context);
		...
		if (!is_local_array) {
			/* Build the out of bounds case */
			node_ptr res = ErrorMgr_failure_make(
				errmgr, "array access out of bounds",
				FAILURE_ARRAY_OUT_OF_BOUNDS, nusmv_yylineno);
			int i;
			...
			for (i = upper_bound; i >= lower_bound; --i) {
				...
				if (SymbType_is_word(indextype)) {
					int size = SymbType_get_word_width(
						indextype);
					if (SymbType_is_signed_word(
						    indextype)) {
						num = find_node(
           ...
           2) the array ID (left child of ARRAY) may be a define
              which are always resolved in ARRAY expression. */
				body = _THROW(body, symb_table, def_hash, Nil,
					      mode);
				...
				if (is_index_constant) { /* constant case */
					...
					neq = ExprMgr_notequal(exprs, index,
							       num, symb_table);
					res = ExprMgr_ite(exprs, neq, body,
							  value, symb_table);
				}
	...
	case WRESIZE: {
		...
		{ /* just check */
			node_ptr value = CompileFlatten_resolve_number(
				symb_table, right, context);

			if ((Nil == value) ||
			    (NUMBER != node_get_type(value))) {
				ErrorMgr_error_not_constant_resize_width(
					errmgr, cdr(sexp));
			}
		}
		result = new_node(nodemgr, node_get_type(sexp), left, right);
	...
	case WSIZEOF: {
...

		{
			...
			result = find_node(nodemgr, NUMBER,
	...
	case EXTEND: {
		...
		{
			node_ptr value = CompileFlatten_resolve_number(
				symb_table, delta, context);

			if ((Nil == value) || NUMBER != node_get_type(value)) {
				ErrorMgr_error_not_constant_extend_width(
					errmgr, cdr(sexp));
			}
		}
	...
	case OP_HISTORICAL: {
		...
		node_ptr right = ((Nil != cdr(sexp)) ?
					  _THROW(cdr(sexp), symb_table,
						 def_hash, context, mode) :
					  Nil);
	...
	case EQDEF: {
		...
		case NEXT:
			...
			name = _THROW(car(left), symb_table, def_hash, context,
				      mode);
			res_left = new_node(nodemgr, node_get_type(left), name,
					    Nil);
			res_right = _THROW(right, symb_table, def_hash, context,
					   mode);
			break;

		default:
			/* we are dealing with x := simple_expr */
			name = res_left = _THROW(left, symb_table, def_hash,
						 context, mode);
			res_right = _THROW(right, symb_table, def_hash, context,
					   mode);
			break;
       ...
       Here only assertions may be. */
		...
		if (SymbTable_is_symbol_variable_array(symb_table, name)) {
			result = _flatten_resolve_variable_array_eqdef(
				self, symb_table, def_hash, name,
				node_get_type(left), res_right, context, mode);
			break;
		}
	...
	case NFUNCTION: {
		node_ptr params =
			_THROW(cdr(sexp), symb_table, def_hash, context, mode);
		ResolveSymbol_ptr rs = SymbTable_resolve_symbol(
			symb_table, car(sexp), context);
	...
	const MasterCompileFlattener_def_mode mode)
{
	...
	/* array may be an array define */
		...
		node_ptr body =
			SymbTable_get_array_define_body(symb_table, array);
    ...
    */
...

	{
		...
		node_ptr res = ErrorMgr_failure_make(
			...
			FAILURE_ARRAY_OUT_OF_BOUNDS, nusmv_yylineno);
		int i;
		SymbType_ptr indextype = TypeChecker_get_expression_type(
			SymbTable_get_type_checker(symb_table), index, Nil);
       ...
       we may limit the range of indexes to word domain */
		if (SymbType_is_word(indextype)) {
         ...
         the singed one has minimal value -(2^(size-1)), i.e. -(max-value+1) */
			if (SymbType_is_unsigned_word(indextype) &&
			    lower_bound < 0) {
				lower_bound = 0;
				...
				nusmv_assert(
					lower_bound <
					0); /* no overflow is possible here */
			}

			/* upper bound cannot be greater than max word value */
			if (upper_bound >
				    0 /* to avoid signed/unsigned cmp and casts */
			    && upper_bound > maxValue) {
				upper_bound = maxValue;
				nusmv_assert(
					maxValue ==
					upper_bound); /* overflow detection */
			}
		}
...
*/
static node_ptr
flattener_core_push_array_index_down(FlattenerBase_ptr self, node_ptr array,
				     node_ptr index, boolean is_array_next,
				     SymbTable_ptr st, hash_ptr def_hash,
				     const MasterCompileFlattener_def_mode mode)
{
   ...
   * flattener_core_flatten */
	...
	case IFTHENELSE: {
		node_ptr left, right;
		left = flattener_core_push_array_index_down(self, car(array),
							    index,
							    is_array_next, st,
							    def_hash, mode);
		right = flattener_core_push_array_index_down(self, cdr(array),
							     index,
							     is_array_next, st,
							     def_hash, mode);
	...
	case NEXT: /* next is passed as parameter */
		...
		return flattener_core_push_array_index_down(
			self, car(array), index, true, st, def_hash, mode);
	...
	default: /* a normal expression */
	{
       ...
       whole array expression. */
		if (NUMBER ==
		    node_get_type(index)) /* words were converted to int */ {
			expr = find_node(nodemgr, ARRAY, array, index);
			if (is_array_next)
				expr = find_node(nodemgr, NEXT, expr, Nil);
		} else {
			expr = array;
			if (is_array_next)
				expr = find_node(nodemgr, NEXT, array, Nil);
...
static node_ptr _resolve_name_or_nop(SymbTable_ptr st, node_ptr expr,
				     node_ptr context)
{
	const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(st));
	ResolveSymbol_ptr rs;
...
static node_ptr _flatten_resolve_variable_array_eqdef(
	FlattenerBase_ptr self, SymbTable_ptr symb_table, hash_ptr def_hash,
	node_ptr left, int left_type, node_ptr right, node_ptr context,
	MasterCompileFlattener_def_mode mode)
{
	SymbType_ptr type;
	int left_lower_bound, left_upper_bound;
	...
	/* array may be an array define */
		...
		node_ptr body =
			SymbTable_get_array_define_body(symb_table, right);
    ...
    */
...

	{
		...
		for (i = left_upper_bound; i >= left_lower_bound; --i) {
			...
			if (right_is_local_array) {
				right_body =
					right_local_array[i - left_lower_bound];
			} else {
				right_body =
					find_node(nodemgr, ARRAY, right, num);
				/* it is necessary to fully resolve newly created ARRAY
           expression for 2 reasons:
           1) the expression can be a define and the flatting mode
              is set to define-expansion.
           2) the array ID (left child of ARRAY) may be a define
              which are always resolved in ARRAY expression. */
				right_body = _THROW(right_body, symb_table,
						    def_hash, Nil, mode);
			}

			/* appending next/init */
			left_body =
				find_node(nodemgr, left_type, left_body, Nil);

			if (right_is_next) {
				right_body = find_node(nodemgr, NEXT,
						       right_body, Nil);
			}
         ...
         variables in the left and side */
			cond = _THROW(cond, symb_table, def_hash, context,
				      mode);
