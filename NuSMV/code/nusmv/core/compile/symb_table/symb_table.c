...
void SymbTablePkg_init(NuSMVEnv_ptr env) {
  ...
  */
  ...
  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_STATEMENT_TYPE,
  ...
  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_BOOLEAN_TYPE,
  ...
  NuSMVEnv_set_value(env, ENV_SYMB_TABLE_PURE_SYMBOL_ENUM_TYPE,
                     SymbType_create_memory_sharing_type(
                         env, SYMB_TYPE_ENUM, cons(nodemgr, atom, Nil)));
  ...
  NuSMVEnv_set_value(
      ...
      SymbType_create_memory_sharing_type(
          env, SYMB_TYPE_ENUM, cons(nodemgr, ExprMgr_number(exprs, 0), Nil)));

  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_INTEGER_TYPE,
  ...
  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_CONTINUOUS_TYPE,
      SymbType_create_memory_sharing_type(env, SYMB_TYPE_CONTINUOUS, Nil));

  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_BOOLEAN_SET_TYPE,
      SymbType_create_memory_sharing_type(env, SYMB_TYPE_SET_BOOL, Nil));

  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_INTEGER_SET_TYPE,
      SymbType_create_memory_sharing_type(env, SYMB_TYPE_SET_INT, Nil));

  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_SYMBOLIC_SET_TYPE,
      SymbType_create_memory_sharing_type(env, SYMB_TYPE_SET_SYMB, Nil));

  NuSMVEnv_set_value(
      env, ENV_SYMB_TABLE_INTEGER_SYMBOLIC_SET_TYPE,
...
void SymbTablePkg_quit(NuSMVEnv_ptr env) {
  ...
  hash_ptr symb_table_width_to_word_type_hash = (hash_ptr)NuSMVEnv_remove_value(
...
SymbType_ptr SymbTablePkg_unsigned_word_type(const NuSMVEnv_ptr env,
  ...
  type = SYMB_TYPE(
      find_assoc(symb_table_width_to_word_type_hash, NODE_FROM_INT(width)));
  if (SYMB_TYPE(NULL) == type) {
    ...
    insert_assoc(symb_table_width_to_word_type_hash, NODE_FROM_INT(width),
                 (node_ptr)type);
  }
...
SymbType_ptr SymbTablePkg_array_type(SymbType_ptr subtype, int lower_bound,
  ...
  if (SYMB_TYPE(NULL) == type) {
    type = SymbType_create_memory_sharing_array_type(subtype, lower_bound,
                                                     upper_bound);
...
SymbType_ptr SymbTablePkg_intarray_type(const NuSMVEnv_ptr env,
                                        SymbType_ptr subtype) {
  const NodeMgr_ptr nodemgr = NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
  hash_ptr symb_table_intarray_subtype_hash = (hash_ptr)NuSMVEnv_get_value(
      env, ENV_SYMB_TABLE_SUBTYPE_TO_INTARRAY_TYPE_HASH);
