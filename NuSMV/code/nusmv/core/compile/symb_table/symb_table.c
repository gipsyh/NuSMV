...
#define RM_TYPE(e, t)                         \
	SymbType_destroy_memory_sharing_type( \
		SYMB_TYPE(NuSMVEnv_remove_value(e, t)))
...
/*!
  ...
  \todo Missing description
*/
#define GET_TYPE(e, t) SYMB_TYPE(NuSMVEnv_get_value(e, t))
...
/*!
  ...
  \todo Missing description
*/
#define ENV_SYMB_TABLE_INTEGER_SYMBOLIC_SET_TYPE \
...
/*!
  ...
  \todo Missing description
*/
#define ENV_SYMB_TABLE_WIDTH_TO_WORD_TYPE_HASH \
	"symb_table_width_to_word_type_hash"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_SYMB_TABLE_WIDTHS_TO_WORDARRAY_TYPE_HASH \
	"symb_table_widths_to_wordarray_type_hash"
...
#define ENV_SYMB_TABLE_SUBTYPE_TO_INTARRAY_TYPE_HASH \
	"symb_table_intarray_subtypes"
...
static void symb_table_init_symb_table(NuSMVEnv_ptr env)
{
	...
	if (opt_verbose_level_gt(opts, 4)) {
		...
		Logger_log(logger, "Instantiating the SymbTable instance "
...
{
	SymbTable_ptr st =
		SYMB_TABLE(NuSMVEnv_remove_value(env, ENV_SYMB_TABLE));
	OptsHandler_ptr opts =
		OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));

	if (opt_verbose_level_gt(opts, 4)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger, "Clearing the SymbTable instance in "
...
static enum st_retval symb_table_types_hash_cleaner(char *c1, char *c2,
						    char *c3);
...
void SymbTablePkg_init(NuSMVEnv_ptr env)
{
  ...
  */
	...
	NuSMVEnv_set_value(
		env, ENV_SYMB_TABLE_PURE_SYMBOL_ENUM_TYPE,
		SymbType_create_memory_sharing_type(env, SYMB_TYPE_ENUM,
						    cons(nodemgr, atom, Nil)));

	NuSMVEnv_set_value(
		env, ENV_SYMB_TABLE_INT_SYMBOLIC_ENUM_TYPE,
		SymbType_create_memory_sharing_type(
			env, SYMB_TYPE_ENUM,
			cons(nodemgr, atom,
	...
	NuSMVEnv_set_value(
		env, ENV_SYMB_TABLE_PURE_INT_ENUM_TYPE,
		SymbType_create_memory_sharing_type(
			env, SYMB_TYPE_ENUM,
			cons(nodemgr, ExprMgr_number(exprs, 0), Nil)));
	...
	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_INTEGER_TYPE,
			   SymbType_create_memory_sharing_type(
				   env, SYMB_TYPE_INTEGER, Nil));

	NuSMVEnv_set_value(
		env, ENV_SYMB_TABLE_REAL_TYPE,
		SymbType_create_memory_sharing_type(env, SYMB_TYPE_REAL, Nil));

	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_CONTINUOUS_TYPE,
			   SymbType_create_memory_sharing_type(
				   env, SYMB_TYPE_CONTINUOUS, Nil));

	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_BOOLEAN_SET_TYPE,
			   SymbType_create_memory_sharing_type(
				   env, SYMB_TYPE_SET_BOOL, Nil));

	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_INTEGER_SET_TYPE,
			   SymbType_create_memory_sharing_type(
				   env, SYMB_TYPE_SET_INT, Nil));

	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_SYMBOLIC_SET_TYPE,
			   SymbType_create_memory_sharing_type(
				   env, SYMB_TYPE_SET_SYMB, Nil));

	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_INTEGER_SYMBOLIC_SET_TYPE,
			   SymbType_create_memory_sharing_type(
				   env, SYMB_TYPE_SET_INT_SYMB, Nil));

	NuSMVEnv_set_value(
		env, ENV_SYMB_TABLE_ERROR_TYPE,
		SymbType_create_memory_sharing_type(env, SYMB_TYPE_ERROR, Nil));

	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_WIDTH_TO_WORD_TYPE_HASH,
			   new_assoc());
	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_WIDTHS_TO_WORDARRAY_TYPE_HASH,
			   new_assoc());
	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_SUBTYPE_BOUND_TO_ARRAY_TYPE_HASH,
			   new_assoc());
	NuSMVEnv_set_value(env, ENV_SYMB_TABLE_SUBTYPE_TO_INTARRAY_TYPE_HASH,
			   new_assoc());
}
...
void SymbTablePkg_quit(NuSMVEnv_ptr env)
{
	...
	hash_ptr symb_table_width_to_word_type_hash =
		(hash_ptr)NuSMVEnv_remove_value(
			env, ENV_SYMB_TABLE_WIDTH_TO_WORD_TYPE_HASH);
	hash_ptr symb_table_widths_to_wordarray_type_hash =
		(hash_ptr)NuSMVEnv_remove_value(
			env, ENV_SYMB_TABLE_WIDTHS_TO_WORDARRAY_TYPE_HASH);
	hash_ptr symb_table_subtype_bound_to_array_type_hash =
		(hash_ptr)NuSMVEnv_remove_value(
			env, ENV_SYMB_TABLE_SUBTYPE_BOUND_TO_ARRAY_TYPE_HASH);
	hash_ptr symb_table_subtype_to_intarray_type_hash =
		(hash_ptr)NuSMVEnv_remove_value(
			env, ENV_SYMB_TABLE_SUBTYPE_TO_INTARRAY_TYPE_HASH);
	...
	clear_assoc_and_free_entries(
		symb_table_subtype_bound_to_array_type_hash,
		symb_table_types_hash_cleaner);
...
SymbType_ptr SymbTablePkg_unsigned_word_type(const NuSMVEnv_ptr env, int width)
{
	...
	if (SYMB_TYPE(NULL) == type) {
		type = SymbType_create_memory_sharing_type(
			env, SYMB_TYPE_UNSIGNED_WORD,
					 ...
					 SymbType_ptr subtype)
{
	...
	type = SYMB_TYPE(
		find_assoc(symb_table_widths_to_wordarray_type_hash, key));
	if (SYMB_TYPE(NULL) == type) {
		type = SymbType_create_memory_sharing_type(
			env, SYMB_TYPE_WORDARRAY, key);
		insert_assoc(symb_table_widths_to_wordarray_type_hash, key,
			     (node_ptr)type);
	}
				     ...
				     int upper_bound)
{
	...
	node_ptr key =
		find_node(nodemgr, CONS, NODE_PTR(subtype),
			  find_node(nodemgr, CONS, NODE_FROM_INT(lower_bound),
				    NODE_FROM_INT(upper_bound)));

	SymbType_ptr type = SYMB_TYPE(
		find_assoc(symb_table_subtype_bound_to_array_type_hash, key));

	if (SYMB_TYPE(NULL) == type) {
		type = SymbType_create_memory_sharing_array_type(
			subtype, lower_bound, upper_bound);
					...
					SymbType_ptr subtype)
{
	...
	if (SYMB_TYPE(NULL) == type) {
		type = SymbType_create_memory_sharing_type(
			env, SYMB_TYPE_INTARRAY, key);
		insert_assoc(symb_table_intarray_subtype_hash, key,
			     (node_ptr)type);
	}
