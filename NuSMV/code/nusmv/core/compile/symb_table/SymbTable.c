...
/*!
  ...
  \todo Missing description
*/
typedef enum { ST_HANDLED_STRUCTS_HASH } SymbTableHandledType;
...
static boolean symb_table_layer_exists(const SymbTable_ptr self,
				       const char *layer_name);

static NodeList_ptr symb_table_filter_layers_symbols(SymbTable_ptr self,
...
static node_ptr symb_table_flatten_array_define(const SymbTable_ptr self,
						const node_ptr body,
				     ...
				     const LayerInsertPolicy ins_policy)
{
	...
	if (layer_name == (char *)NULL) {
		int c = snprintf(tmp_name,
				 sizeof(tmp_name) / sizeof(tmp_name[0]),
				 SYMB_TABLE_TEMP_LAYER_NAME,
	...
	while (!ListIter_is_end(iter)) {
		if (SymbLayer_must_insert_before(
			    layer, SYMB_LAYER(NodeList_get_elem_at(self->layers,
								   iter)))) {
			NodeList_insert_before(self->layers, iter,
					       (node_ptr)layer);
			break;
		}
	...
	if (ListIter_is_end(iter))
		NodeList_append(self->layers, (node_ptr)layer);
...
void SymbTable_remove_layer(SymbTable_ptr self, SymbLayer_ptr layer)
{
	...
	{ /* removes the layer from all existing layers' classes: */
		...
		{
			const char *class_name =
				(const char *)NodeList_get_elem_at(
					self->class_names, iter);

			symb_table_layer_rename_in_class(
				self, class_name, SymbLayer_get_name(layer),
				(const char *)NULL);
		}
	}
	...
	nusmv_assert(
		!ListIter_is_end(iter)); /* This layer had not been found */
}
				  ...
				  const char *layer_name)
{
	...
	return SYMB_LAYER(find_assoc(
		self->name2layer, (node_ptr)UStringMgr_find_string(
					  self->strings, (char *)layer_name)));
}
					     ...
					     const char *class_name)
{
	...
	if (class_name != (const char *)NULL) {
		...
		if (opt_verbose_level_gt(self->options, 3)) {
			...
			Logger_log(
				logger,
				"SymbTable: default layers class set to '%s'\n",
				class_name);
		}
	}
}
				  ...
				  const char *class_name)
{
	...
	/* adds to the array */
	array_insert_last(const char *, _class,
			  util_strsav((char *)layer_name));

	if (opt_verbose_level_gt(self->options, 3)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger, "SymbTable: Added layer '%s' to class ",
			   layer_name);
...
{
	SYMB_TABLE_CHECK_INSTANCE(self);
	symb_table_layer_rename_in_class(self, class_name, layer_name,
					 (const char *)NULL);
}
					 ...
					 const char *layer_name)
{
	...
	NODE_LIST_FOREACH(self->class_names, iter)
	{
		const char *class_name = (const char *)NodeList_get_elem_at(
			self->class_names, iter);
					     ...
					     const array_t *layer_names)
{
	...
	SymbTable_iter_set_filter(self, &iter, SymbTable_iter_filter_sf_symbols,
				  NULL);
					    ...
					    const array_t *layer_names)
{
	...
	SymbTable_gen_iter(self, &iter, STT_ALL);
	SymbTable_iter_set_filter(self, &iter, SymbTable_iter_filter_i_symbols,
				  NULL);
				       ...
				       const node_ptr name)
{
	...
	if (SymbCache_is_symbol_var(self->cache, name)) {
		...
		node_ptr expansion =
			Compile_FlattenSexpExpandDefine(self, name, NULL);
		...
		if (SymbCache_is_symbol_declared(self->cache, expansion) &&
			...
			retval = TypeChecker_get_expression_type(
				self->tc, expansion, NULL);
	} else
		retval = NULL;
...
{
	SYMB_TABLE_CHECK_INSTANCE(self);
	return SymbType_get_nfunction_type(
		SymbTable_get_function_type(self, name));
}
						 ...
						 const node_ptr name)
{
	...
	return symb_table_flatten_array_define(
		self, SymbCache_get_array_define_body(self->cache, name),
	...
	if (SymbTable_is_symbol_constant(self, symbol)) {
       ...
       otherwise it is undefined symbol. */
		if (ARRAY == node_get_type(symbol)) {
			...
			nusmv_assert(DOT == node_get_type(symbol) ||
				     ATOM == node_get_type(symbol));
			res = SYMBOL_INVALID;
		}
	}
...
boolean SymbTable_is_var_finite(const SymbTable_ptr self, const node_ptr name)
{
	...
	return !(SymbType_is_infinite_precision(vtype) ||
		 SymbType_is_intarray(vtype) || SymbType_is_wordarray(vtype));
}
...
node_ptr SymbTable_get_determinization_var_name(const SymbTable_ptr self)
{
	...
	while (true) {
		int c = snprintf(name, buf_size, "%s%" PRIuPTR, INT_VAR_PREFIX,
				 self->det_counter++);
		SNPRINTF_CHECK(c, buf_size);
		vname = NodeMgr_find_node(self->nodes, DOT, Nil,
					  sym_intern(env, name));
		if (!SymbTable_is_symbol_declared(self, vname))
			break;
	}
				       ...
				       const char *symbol_str)
{
	...
	return NodeMgr_find_node(
		self->nodes, DOT, Nil,
		NodeMgr_find_node(self->nodes, ATOM,
				  (node_ptr)UStringMgr_find_string(
					  self->strings, symbol_str),
				  Nil));
}
...
SymbTable_ptr SymbTable_copy(const SymbTable_ptr self, Set_t blacklist)
{
	...
	SymbTable_set_default_layers_class_name(
		new_st, SymbTable_get_default_layers_class_name(self));
	...
	NODE_LIST_FOREACH(layers, layer_iter)
	{
		...
		old_layer =
			(SymbLayer_ptr)NodeList_get_elem_at(layers, layer_iter);

		new_layer = SymbTable_create_layer(
			new_st, SymbLayer_get_name(old_layer),
			SymbLayer_get_insert_policy(old_layer));

		class_name = SymbTable_get_class_of_layer(
			self, SymbLayer_get_name(old_layer));
		if ((char *)NULL != class_name) {
		...
		{
			node_ptr symbol =
				SymbLayer_iter_get_symbol(old_layer, &iter);
			...
			if (!Set_IsMember(blacklist, symbol)) {
				...
				if (SymbTable_is_symbol_var(self, symbol)) {
						...
						while (value_list != Nil) {
							...
							if (DOT ==
								...
								rs = SymbTable_resolve_symbol(
									...
									name,
									Nil);
								name = ResolveSymbol_get_resolved_name(
									rs);
							} else
								name = find_atom(
									nodemgr,
							...
							if (SymbLayer_can_declare_constant(
								    new_layer,
								    name)) {
					...
					if (SymbTable_is_symbol_state_var(
						...
						ErrorMgr_rpterr(
							errmgr,
							"SymbTable_copy: Symbol %s not handled.\n",
							sprint_node(wffprint,
								    symbol));
				...
				/* Not variables */
					...
					if (SymbTable_is_symbol_constant(
						    self, symbol)) {
						...
						if (SymbLayer_can_declare_constant(
							...
							SymbLayer_declare_constant(
								new_layer,
								symbol);
					} else if (SymbTable_is_symbol_variable_array(
							   self, symbol)) {
						SymbType_ptr type =
						...
						SymbLayer_declare_variable_array(
							new_layer, symbol,
							type);
					} else if (SymbTable_is_symbol_array_define(
							   self, symbol)) {
						node_ptr body =
               ...
               is returned (eg: WriteFlattenModel) */
						...
						SymbLayer_declare_define(
							new_layer, symbol, ctx,
							body);
					} else if (SymbTable_is_symbol_parameter(
							   self, symbol)) {
						node_ptr actual =
						...
						SymbLayer_declare_parameter(
							new_layer, symbol, ctx,
							actual);
					} else if (SymbTable_is_symbol_function(
							   self, symbol)) {
						node_ptr ctx =
							SymbTable_get_function_context(
								self, symbol);
						SymbType_ptr type =
							SymbTable_get_function_type(
								self, symbol);
						type = SymbType_copy(type);
					 ...
					 const node_ptr sym, void *arg)
{
	if (SymbTable_is_symbol_define(self, sym)) {
		...
		return cat == SYMBOL_STATE_NEXT_DEFINE ||
		       cat == SYMBOL_STATE_DEFINE || cat == SYMBOL_CONSTANT;
	}
...
boolean SymbTable_iter_filter_out_var_array_elems(const SymbTable_ptr self,
						  const node_ptr sym, void *arg)
{
	if (SymbTable_is_symbol_array_var_element(self, sym))
		return false;
	else
		return true;
}
...
Set_t SymbTable_iter_to_set(const SymbTable_ptr self, SymbTableIter iter)
{
	...
	while (!SymbTable_iter_is_end(self, &iter)) {
		res = Set_AddMember(res,
				    SymbTable_iter_get_symbol(self, &iter));
...
hash_ptr SymbTable_get_handled_hash_ptr(SymbTable_ptr self, const char *key,
					...
					SymbTableTriggerFun redeclare_action)
{
	...
	if (PAIR(NULL) == p) {
		...
		insert_assoc(self->handled_structures, NODE_PTR(strkey),
			     NODE_PTR(p));
       ...
       always the same */
		if ((SymbTableTriggerFun)NULL != add_action) {
			SymbTable_add_trigger(self, add_action,
					      ST_TRIGGER_SYMBOL_ADD,
					      (void *)res, false);
		}
		if ((SymbTableTriggerFun)NULL != remove_action) {
			AssocAndDestroy_ptr remove_action_arg =
				ALLOC(AssocAndDestroy, 1);
			nusmv_assert(NULL != remove_action_arg);
			remove_action_arg->assoc = res;
			remove_action_arg->destroy_func =
				(PF_STCPCPCP)destroy_func;

			SymbTable_add_trigger(self, remove_action,
						...
						void *arg)
{
	...
	if (assoc_get_size(hash) > 0) {
		if (assoc_and_destroy->destroy_func) {
			clear_assoc_and_free_entries(
				hash, assoc_and_destroy->destroy_func);
		} else {
			clear_assoc(hash);
		}
	}
...
SymbTable_contains_infinite_precision_variables(const SymbTable_ptr self)
{
	...
	if (SymbCache_get_functions_num(self->cache) > 0)
		return true;
...
Set_t SymbTable_get_type_tags(const SymbTable_ptr self)
{
	...
	SYMB_TABLE_FOREACH(self, iter, STT_VAR | STT_FUNCTION)
	{
		...
		res = Set_AddMember(res, PTR_FROM_INT(Set_Element_t,
				  ...
				  NodeAnonymizerBase_ptr anonymizer)
{
	...
	NODE_LIST_FOREACH(layers, layer_iter)
	{
		...
		SYMB_LAYER_FOREACH(old_layer, iter, STT_ALL)
		{
			...
			anonymous_symbol =
				NodeAnonymizerBase_map_expr(anonymizer, symbol);
			...
			if (!Set_IsMember(blacklist, symbol)) {
               ...
               is returned (eg: WriteFlattenModel) */
						...
						ErrorMgr_rpterr(
							errmgr,
							"SymbTable_anonymize: Symbol %s not handled.\n",
							n);
						FREE(n);
						error_unreachable_code();
					}
...
boolean SymbTable_is_symbol_array_var_element(const SymbTable_ptr self,
					      const node_ptr name)
{
  ...
  */
	if (node_get_type(name) == ARRAY &&
		...
		if (SymbTable_is_symbol_variable_array(self, car(name))) {
			...
			return SymbTable_is_symbol_array_var_element(self,
								     car(name));
	}
					      ...
					      node_ptr element)
{
	...
	if (SymbTable_is_symbol_array_var_element(self, car(element))) {
		retval = SymbTable_get_var_array_from_element(self,
							      car(element));
	} else {
		nusmv_assert((SymbTable_is_symbol_variable_array(
			self, car(element))));
		retval = car(element);
	}
					     ...
					     node_ptr array)
{
	...
	if (SymbTable_is_symbol_variable_array(self, array)) {
		node_ptr lbound =
			SymbTable_get_array_lower_bound_variable(self, array);

		if (SymbTable_is_symbol_frozen_var(self, lbound))
			return true;
		else
			return false;
	} else
					    ...
					    node_ptr array)
{
	...
	if (SymbTable_is_symbol_variable_array(self, array)) {
		...
		if (SymbTable_is_symbol_state_var(self, lbound))
			return true;
		else
			return false;
	} else
					    ...
					    node_ptr array)
{
	...
	if (SymbTable_is_symbol_variable_array(self, array)) {
		...
		if (SymbTable_is_symbol_input_var(self, lbound))
			return true;
		else
			return false;
	} else
						  ...
						  node_ptr array)
{
...

	{
		...
		NodeMgr_ptr const nodemgr =
			NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
		ExprMgr_ptr const exprs =
			EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
		SymbType_ptr type =
			SymbTable_get_variable_array_type(self, array);
		...
		if (SymbTable_is_symbol_variable_array(self, retval)) {
			retval = SymbTable_get_array_lower_bound_variable(
				self, retval);
		}
	}
						  ...
						  node_ptr array)
{
...

	{
		...
		retval = find_node(nodemgr, ARRAY, array,
				   ExprMgr_number(exprs, ubound));
		if (SymbTable_is_symbol_variable_array(self, retval)) {
			retval = SymbTable_get_array_upper_bound_variable(
				self, retval);
		}
	}
...
static void symb_table_deinit(SymbTable_ptr self)
{
	...
	clear_assoc_and_free_entries(self->class_layers,
	...
	{
		SymbTableHandledType t =
			(SymbTableHandledType)Pair_get_second(v);
						     ...
						     SymbTableIter *iter)
{
	...
	while (!SymbTable_iter_is_end(self, iter)) {
		...
		arrayForEachItem(const char *, (array_t *)layer_names, idx,
				 layer_name)
		{
			SymbLayer_ptr layer =
				SymbTable_get_layer(self, layer_name);
		...
		if (strcmp(name, old_layer_name) == 0) {
			...
			if (new_layer_name != (const char *)NULL) {
				...
				for (j = i + 1; j < array_n(_class); ++j) {
					array_insert(const char *, _class,
						     j - 1,
						     array_fetch(const char *,
								 _class, j));
				}
				_class->num -= 1;
			}
					       ...
					       const char *class_name)
{
	...
	if (_class == (array_t *)NULL) {
		...
		insert_assoc(self->class_layers, (node_ptr)key,
			     (node_ptr)_class);
	...
	case CONS:
		return NodeMgr_find_node(self->nodes, node_get_type(body),
					 symb_table_flatten_array_define(
						 self, car(body), context),
					 symb_table_flatten_array_define(
						 self, cdr(body), context));
	...
	if (NEXT == node_get_type(expr)) {
		...
		if (SymbTable_is_symbol_constant(st, expr)) {
			...
			if (SymbTable_is_symbol_state_var_array(st, expr) ||
				...
				nusmv_assert(
					SymbTable_is_symbol_input_var_array(
						st, expr));
				cat = SYMBOL_INPUT_DEFINE;
			}
		}
		...
		if (has_next && has_input && has_state) {
			...
			nusmv_assert(SYMBOL_INVALID == l_cat &&
				     SYMBOL_INVALID == r_cat);
