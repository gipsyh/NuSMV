...
#define CLEAR_PRINTER_INFO(pinfo)               \
	if (STRING_OUTPUT == pinfo.output_type) \
	FREE(pinfo.string_buffer)
...
static void str_print(const NuSMVEnv_ptr env,
		      SymbTypePrinterInfo_ptr printer_info, const char *fmt,
		      ...);
...
SymbType_ptr SymbType_copy(const SymbType_ptr self)
{
	...
	if (SYMB_TYPE_ARRAY == self->tag) {
		...
		return SymbType_create_array(
			SymbType_copy(SymbType_get_array_subtype(self)),
	...
	if (SYMB_TYPE_NFUNCTION == self->tag) {
		...
		return SymbType_create_nfunction(
			env, NFunction_copy(SymbType_get_nfunction_type(self)));
	}

	return SymbType_create(env, self->tag,
			       self->tag == SYMB_TYPE_ENUM ? car(self->body) :
							     self->body);
}
...
int SymbType_calculate_type_size(const SymbType_ptr self)
{
	...
	SYMB_TYPE_CHECK_INSTANCE(self);
	nusmv_assert(
		!self->isMemorySharingInstance); /* must not be memory-shared */
...
int SymbType_get_word_line_number(const SymbType_ptr self)
{
	...
	nusmv_assert(((SYMB_TYPE_UNSIGNED_WORD == self->tag ||
		     ...
		     (SYMB_TYPE_WORDARRAY == self->tag &&
		      node_get_type(self->body) == CONS));
	nusmv_assert(
		!self->isMemorySharingInstance); /* type is not memory-shared */
...
boolean SymbType_is_back_comp(const SymbType_ptr type)
{
	...
	return SYMB_TYPE_BOOLEAN == type->tag || SYMB_TYPE_ENUM == type->tag ||
				  ...
				  const SymbType_ptr type2)
{
	...
	if (returnType)
		return returnType; /* conversion was successful */
					    ...
					    SymbType_ptr rightType)
{
  ...
  */
	...
	if (leftType->tag == SYMB_TYPE_REAL) {
		return (rightType->tag == SYMB_TYPE_INTEGER) ? leftType :
							       nullType;
	}
	...
	if (SymbType_is_array(leftType) || SymbType_is_array(rightType)) {
		...
		{
			SymbType_ptr subLeft =
				SymbType_get_array_subtype(leftType);
			SymbType_ptr subRight =
				SymbType_get_array_subtype(rightType);
			SymbType_ptr subtype = SymbType_convert_right_to_left(
				subLeft, subRight);
...
SymbType_ptr SymbType_get_minimal_common(SymbType_ptr type1, SymbType_ptr type2)
{
  ...
  */
	...
	if ((type1->tag == SYMB_TYPE_INTEGER || type1->tag == SYMB_TYPE_REAL) &&
	    (type2->tag == SYMB_TYPE_INTEGER || type2->tag == SYMB_TYPE_REAL)) {
     ...
     compatible subtypes */
...

		{
			...
			SymbType_ptr subtype =
				SymbType_get_minimal_common(sub1, sub2);
			if (subtype == NULL)
				return SYMB_TYPE(NULL);

			/* create a proper memory shared array type */
			return SymbTablePkg_array_type(
				subtype, SymbType_get_array_lower_bound(type1),
				SymbType_get_array_upper_bound(type1));
		}
	}
...
SymbType_ptr SymbType_make_memory_shared(const SymbType_ptr self)
{
	...
	case SYMB_TYPE_UNSIGNED_WORD:
		res = SymbTablePkg_unsigned_word_type(
			env, SymbType_get_word_width(self));
		break;

	case SYMB_TYPE_SIGNED_WORD:
		res = SymbTablePkg_signed_word_type(
			env, SymbType_get_word_width(self));
		break;

	case SYMB_TYPE_WORDARRAY:
		res = SymbTablePkg_wordarray_type(
			env, SymbType_get_wordarray_awidth(self),
			SymbType_make_memory_shared(
				SymbType_get_wordarray_subtype(self)));
		break;
	...
	case SYMB_TYPE_ARRAY:
		res = SymbTablePkg_array_type(
			SymbType_make_memory_shared(
				SymbType_get_array_subtype(self)),
			SymbType_get_array_lower_bound(self),
			SymbType_get_array_upper_bound(self));
		break;

	case SYMB_TYPE_INTARRAY:
		res = SymbTablePkg_intarray_type(
			env, SymbType_make_memory_shared(
				     SymbType_get_intarray_subtype(self)));
		break;

	case SYMB_TYPE_NFUNCTION:
		error_unreachable_code_msg(
			"Uninterpreted functions should be not "
			"converted to a memory shared type\n");
		break;
...
boolean SymbType_equals(SymbType_ptr self, SymbType_ptr oth)
{
	...
	if (self->tag == oth->tag) {
      ...
      */
			...
			return (SymbType_get_word_width(self) ==
				SymbType_get_word_width(oth));
			...
			return NFunction_equals(
				SymbType_get_nfunction_type(self),
				SymbType_get_nfunction_type(oth));
		} else {
			ErrorMgr_internal_error(
				errmgr,
				"SymbType_equals: Unknown type combination.");
			error_unreachable_code();
			return true;
		}
	}
...
node_ptr SymbType_to_node(const SymbType_ptr self, NodeMgr_ptr nodemgr)
{
	...
	case SYMB_TYPE_ARRAY: {
		...
		return find_node(
			...
			find_node(nodemgr, TWODOTS,
				  find_node(nodemgr, NUMBER,
					    NODE_FROM_INT(lindex), Nil),
				  find_node(nodemgr, NUMBER,
					    NODE_FROM_INT(rindex), Nil)),
			SymbType_to_node(subtype, nodemgr));
	} break;

	case SYMB_TYPE_INTARRAY: {
		return find_node(
			nodemgr, INTARRAY_TYPE,
	...
	case SYMB_TYPE_NFUNCTION: {
		...
		node_ptr rtype = SymbType_to_node(
			NFunction_get_return_type(nfunction), nodemgr);
	...
	if (node_get_type(n1) == node_get_type(n2)) {
		...
		case ATOM:
			if ((Nil != car(n1)) && (Nil != car(n2))) {
				...
				return !strcmp(UStringMgr_get_string_text(
						       node_get_lstring(n1)),
...
/*!
  ...
  \sa SymbType_print, SymbType_sprint
*/
static void str_print(const NuSMVEnv_ptr env, SymbTypePrinterInfo_ptr pinfo,
		      const char *fmt, ...)
{
				 ...
				 SymbTypePrinterInfo_ptr pinfo)
{
	...
	case SYMB_TYPE_WORDARRAY:
		...
		str_print(env, pinfo, "%d",
			  SymbType_get_wordarray_awidth(self));
		str_print(env, pinfo, "]");

		str_print(env, pinfo, " of ");

		symb_type_sprint_aux(SymbType_get_wordarray_subtype(self),
				     wffprint, pinfo);
		break;

	case SYMB_TYPE_SET_BOOL:
		str_print(env, pinfo, "boolean-set");
		break;
	case SYMB_TYPE_SET_INT:
		str_print(env, pinfo, "integer-set");
		break;
	case SYMB_TYPE_SET_SYMB:
		str_print(env, pinfo, "symbolic-set");
		break;
	case SYMB_TYPE_SET_INT_SYMB:
		str_print(env, pinfo, "integer-symbolic-set");
		break;
	...
	case SYMB_TYPE_ARRAY: {
		str_print(env, pinfo, "array %d..%d of ",
			  SymbType_get_array_lower_bound(self),
			  SymbType_get_array_upper_bound(self));
		symb_type_sprint_aux(SymbType_get_array_subtype(self), wffprint,
				     pinfo);
		break;
	}
	...
	case SYMB_TYPE_INTARRAY: {
		...
		symb_type_sprint_aux(SymbType_get_intarray_subtype(self),
				     wffprint, pinfo);
		break;
	}
