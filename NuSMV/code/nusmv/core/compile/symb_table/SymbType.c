    ...
    */
    ...
    while (iter != Nil) {
               ...
               */
      {
        ...
        StreamMgr_nprint_error(streams, wffprint, "%N", body);
    ...
    enumType = (thereIsInt && thereIsSymbolic) ? ENUM_TYPE_INT_SYMBOLIC
...
SymbType_ptr SymbType_create_array(SymbType_ptr subtype, int lower_bound,
  ...
  self->body = cons(
      nodemgr, NODE_PTR(subtype),
      cons(nodemgr, NODE_FROM_INT(lower_bound), NODE_FROM_INT(upper_bound)));
...
boolean SymbType_is_set(const SymbType_ptr self) {
  SYMB_TYPE_CHECK_INSTANCE(self);
  return SYMB_TYPE_SET_BOOL == self->tag || SYMB_TYPE_SET_INT == self->tag ||
         SYMB_TYPE_SET_SYMB == self->tag || SYMB_TYPE_SET_INT_SYMB == self->tag;
}
...
int SymbType_get_word_line_number(const SymbType_ptr self) {
  ...
  nusmv_assert(
      ((SYMB_TYPE_UNSIGNED_WORD == self->tag ||
        SYMB_TYPE_SIGNED_WORD == self->tag) &&
       node_get_type(self->body) == NUMBER) ||
      (SYMB_TYPE_WORDARRAY == self->tag && node_get_type(self->body) == CONS));
...
SymbType_ptr SymbType_make_memory_shared(const SymbType_ptr self) {
  ...
  case SYMB_TYPE_INTARRAY:
    res = SymbTablePkg_intarray_type(
        env, SymbType_make_memory_shared(SymbType_get_intarray_subtype(self)));
    break;

  case SYMB_TYPE_NFUNCTION:
    error_unreachable_code_msg("Uninterpreted functions should be not "
...
node_ptr SymbType_to_node(const SymbType_ptr self, NodeMgr_ptr nodemgr) {
  ...
  case SYMB_TYPE_INTARRAY: {
    return find_node(
        ...
        SymbType_to_node(SymbType_get_intarray_subtype(self), nodemgr), Nil);
  }

  case SYMB_TYPE_NFUNCTION: {
    NFunction_ptr nfunction = SymbType_get_nfunction_type(self);
    node_ptr rtype =
        SymbType_to_node(NFunction_get_return_type(nfunction), nodemgr);
  ...
  if (node_get_type(n1) == node_get_type(n2)) {
    ...
    case ATOM:
      if ((Nil != car(n1)) && (Nil != car(n2))) {
        ...
        return false;
    default:
      return (node_equal(car(n1), car(n2)) && node_equal(cdr(n1), cdr(n2)));
    }
  }
      ...
      if (n > -1 && n < size) {
        ...
        if (NIL(char) == pinfo->string_buffer) {
          ...
          if (pinfo->remaining_space <= n) {
            ...
            tmp = REALLOC(char, pinfo->string_buffer,
