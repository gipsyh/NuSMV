				     ...
				     const SymbLayer_ptr other)
{
	...
	default:
		ErrorMgr_internal_error(self->errors,
					"Unexpected layer insertion policy");
	}
			      ...
			      node_ptr context, node_ptr definition)
{
	...
	if (!SymbLayer_can_declare_define(self, name)) {
		...
		ErrorMgr_internal_error(
			self->errors,
			"SymbLayer_declare_define: name already declared\n");
	}
			...
			const unsigned int mask)
{
	...
	if (index != self->symbols_index) {
		...
		type = (Nil != sym) ?
			       SymbCache_get_symbol_type(self->cache, sym) :
			       STT_NONE;
...
Set_t SymbLayer_iter_to_set(const SymbLayer_ptr self, SymbLayerIter iter)
{
	...
	while (!SymbLayer_iter_is_end(self, &iter)) {
		res = Set_AddMember(res,
				    SymbLayer_iter_get_symbol(self, &iter));
...
static void symb_layer_deinit(SymbLayer_ptr self, boolean clean_cache)
{
	...
	if (clean_cache) {
		SymbCache_remove_symbols(self->cache, self->symbols,
					 self->symbols_index);
	}
...
*/
static inline void symb_layer_new_symbol(SymbLayer_ptr self, const node_ptr sym)
{
...
{
	unsigned int pos =
		NODE_TO_INT(remove_assoc(self->symbol2position, sym));
	...
	if ((self->symbols_allocated > INITIAL_SYMBOLS_ARRAY_SIZE) &&
		...
		for (i = 0, j = 0; j < self->symbols_index; ++j) {
           ...
           it is possible to check for NULL */
				insert_assoc(self->symbol2position, sym,
					     NODE_FROM_INT(i + 1));
