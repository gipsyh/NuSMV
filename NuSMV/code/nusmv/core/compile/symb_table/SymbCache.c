...
/*!
  ...
  \todo Missing description
*/
#define SI_CATEGORY(si) si->category
...
/*!
  ...
  \todo Missing description
*/
#define SI_GET_CONSTANT_COUNT(si) NODE_TO_INT(si->field1)
...
/*!
  ...
  \todo Missing description
*/
#define SI_SET_CONSTANT_COUNT(si, val) si->field1 = NODE_FROM_INT(val)
...
/*!
  ...
  \todo Missing description
*/
#define SI_VAR_TYPE(si) si->field1
...
/*!
  ...
  \todo Missing description
*/
#define SI_DEFINE_CONTEXT(si) si->field1
...
/*!
  ...
  \todo Missing description
*/
#define SI_DEFINE_BODY(si) si->field2
...
/*!
  ...
  \todo Missing description
*/
#define SI_DEFINE_FLATTEN_BODY(si) si->field3
...
/*!
  ...
  \todo Missing description
*/
#define SI_ARRAY_DEFINE_CONTEXT(si) si->field1
...
/*!
  ...
  \todo Missing description
*/
#define SI_ARRAY_DEFINE_BODY(si) si->field2
...
/*!
  ...
  \todo Missing description
*/
#define SI_ARRAY_DEFINE_FLAT_BODY(si) si->field3
...
/*!
  ...
  \todo Missing description
*/
#define SI_PARAMETER_CONTEXT(si) si->field1
...
/*!
  ...
  \todo Missing description
*/
#define SI_PARAMETER_ACTUAL(si) si->field2
...
/*!
  ...
  \todo Missing description
*/
#define SI_PARAMETER_FLATTEN_ACTUAL(si) si->field3
...
/*!
  ...
  \todo Missing description
*/
#define SI_FUNCTION_CONTEXT(si) si->field1
...
/*!
  ...
  \todo Missing description
*/
#define SI_FUNCTION_TYPE(si) si->field2
...
/*!
  ...
  \todo Missing description
*/
#define GET_SYMBOL(self, s) ((SymbolInfo *)find_assoc(self->symbol_hash, s))
...
/*!
  ...
  \todo Missing description
*/
#define SI_IS_REMOVED(si) (SI_SYMBOL_REMOVED == si)
...
static inline SymbolInfo *symb_cache_alloc_symb_info(const SymbCache_ptr self);

static inline void symb_cache_free_symb_info(const SymbCache_ptr self,
					     SymbolInfo *si);

static inline void symb_cache_new_symbol(SymbCache_ptr self, const node_ptr sym,
					 SymbolInfo *symbol_info);

static inline void symb_cache_remove_symbol(SymbCache_ptr self,
					    const node_ptr sym,
					    const boolean shrink_if_needed);

static inline void symb_cache_check_and_shrink_symbols(SymbCache_ptr self);
...
void SymbCache_new_define(SymbCache_ptr self, node_ptr name, node_ptr ctx,
			  node_ptr definition)
{
	SymbolInfo *si;
...
void SymbCache_new_parameter(SymbCache_ptr self, node_ptr formal, node_ptr ctx,
			     node_ptr actual)
{
	SymbolInfo *si;
...
void SymbCache_new_function(SymbCache_ptr self, node_ptr name, node_ptr ctx,
			    SymbType_ptr type)
{
	SymbolInfo *si;
					  ...
					  const NodeList_ptr var_list)
{
	...
	NODE_LIST_FOREACH(var_list, iter)
	{
		...
		if (SymbCache_is_symbol_input_var(self, sym))
			return true;
	}
						 ...
						 const NodeList_ptr var_list)
{
	...
	{
		node_ptr sym = NodeList_get_elem_at(var_list, iter);
		if (SymbCache_is_symbol_state_frozen_var(self, sym))
			return true;
	}
...
void SymbCache_next_iter(const SymbCache_ptr self, SymbTableIter *iter)
{
	...
	do {
		...
		nusmv_assert(SI_IS_DECLARED(si));
		 ...
		 ((SI_CATEGORY(si) & iter->mask) == 0)); /* Not a match */
}
			   ...
			   boolean must_free_arg)
{
	SymbCacheRemoveSymbolStruct *srss =
		ALLOC(SymbCacheRemoveSymbolStruct, 1);
	...
	case ST_TRIGGER_SYMBOL_REDECLARE:
		...
		if (!NodeList_belongs_to(self->redef_triggers,
					 NODE_PTR(srss))) {
			      ...
			      SymbTableTriggerAction action)
{
	...
	if (NODE_LIST(NULL) != triggers) {
		NODE_LIST_FOREACH(triggers, iter)
		{
			SymbCacheRemoveSymbolStruct *srss =
				(SymbCacheRemoveSymbolStruct *)
					NodeList_get_elem_at(triggers, iter);
...
static assoc_retval sym_hash_free_vars(char *key, char *data, char *arg)
{
	...
	if (STT_VAR & SI_CATEGORY(si) || STT_VARIABLE_ARRAY & SI_CATEGORY(si)) {
					 ...
					 SymbolInfo *symbol_info)
{
	...
	if (index == self->symbols_allocated) {
		...
		self->symbols = REALLOC(node_ptr, self->symbols,
	...
	if (NODE_LIST(NULL) != self->add_triggers) {
		...
		NODE_LIST_FOREACH(self->add_triggers, iter)
		{
			SymbCacheRemoveSymbolStruct *srss =
				(SymbCacheRemoveSymbolStruct *)
					NodeList_get_elem_at(self->add_triggers,
							     iter);
			srss->trigger(self->symb_table, sym,
				      ST_TRIGGER_SYMBOL_ADD, srss->arg);
		}
	}
...
static inline void symb_cache_check_and_shrink_symbols(SymbCache_ptr self)
{
	...
	if ((self->symbols_allocated > INITIAL_SYMBOLS_ARRAY_SIZE) &&
	    (((double)self->symbols_empty / (double)self->symbols_allocated) >
	     0.75)) {
		unsigned int i, j;
		...
		for (i = 0, j = 0; j < self->symbols_index; ++j) {
			...
			if (sym != Nil) {
				SymbolInfo *si = (SymbolInfo *)find_assoc(
					self->symbol_hash, sym);
				nusmv_assert(SI_IS_DECLARED(si));
					    ...
					    const boolean shrink_if_needed)
{
	...
	if (NODE_LIST(NULL) != self->rem_triggers) {
		...
		NODE_LIST_FOREACH(self->rem_triggers, iter)
		{
			SymbCacheRemoveSymbolStruct *srss =
				(SymbCacheRemoveSymbolStruct *)
					NodeList_get_elem_at(self->rem_triggers,
							     iter);
			srss->trigger(self->symb_table, sym,
