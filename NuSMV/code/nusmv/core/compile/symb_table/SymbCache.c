...
node_ptr SymbCache_get_flatten_actual_parameter(const SymbCache_ptr self,
  ...
  if (Nil == SI_PARAMETER_FLATTEN_ACTUAL(si)) {
    ...
    flat = NodeMgr_find_node(self->nodes, CONTEXT, SI_PARAMETER_CONTEXT(si),
...
static inline void symb_cache_new_symbol(SymbCache_ptr self, const node_ptr sym,
  ...
  if (redefined && NODE_LIST(NULL) != self->redef_triggers) {
    ...
    NODE_LIST_FOREACH(self->redef_triggers, iter) {
      ...
      srss->trigger(self->symb_table, sym, ST_TRIGGER_SYMBOL_REDECLARE,
                    srss->arg);
    }
  }
}
...
static inline void symb_cache_remove_symbol(SymbCache_ptr self,
  ...
  if (NODE_LIST(NULL) != self->rem_triggers) {
    ...
    NODE_LIST_FOREACH(self->rem_triggers, iter) {
      SymbCacheRemoveSymbolStruct *srss =
          (SymbCacheRemoveSymbolStruct *)NodeList_get_elem_at(
              self->rem_triggers, iter);
      srss->trigger(self->symb_table, sym, ST_TRIGGER_SYMBOL_REMOVE, srss->arg);
    }
  }
