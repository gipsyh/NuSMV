				       ...
				       const char *suffix)
{
	...
	{
		int c = snprintf(result_name, result_size, "%s%s%s", prefix,
				 expr_string, suffix);
			    ...
			    node_ptr name2)
{
	...
	case ATOM:
		return (car(name1) == car(name2) ||
			(0 == strcmp(UStringMgr_get_string_text(
					     (string_ptr)car(name1)),
				     UStringMgr_get_string_text(
					     (string_ptr)car(name2)))));
	...
	default:
		ErrorMgr_internal_error(
			...
			"Invalid case %d in sym_names_are_equal",
			node_get_type(name1));
	}
...
node_ptr Compile_remove_ltl_bop(const NuSMVEnv_ptr env, node_ptr prop)
{
	...
	if (opt_verbose_level_gt(opts, 6)) {
		...
		if (prop == result) {
			Logger_log(
				logger,
				"-- No bounded operators have been rewritten\n");
		} else {
			Logger_nlog(logger, wffprint,
					       ...
					       const NodeList_ptr vars_order)
{
	...
	NODE_LIST_FOREACH(vars_order, iter)
	{
		...
		rs = SymbTable_resolve_symbol(
			st, NodeList_get_elem_at(vars_order, iter), Nil);
	...
	NODE_LIST_FOREACH(vars, iter)
	{
		...
		rs = SymbTable_resolve_symbol(
			st, NodeList_get_elem_at(vars, iter), Nil);
		name = ResolveSymbol_get_resolved_name(rs);
						...
						node_ptr prop)
{
	...
	case CONTEXT:
		return find_node(nodemgr, CONTEXT, car(prop),
				 compile_pop_distrib_ops_recurse(nodemgr,
								 cdr(prop)));
	...
	if (IS_AND_DISTRIB_OP(prop) || IS_OR_DISTRIB_OP(prop)) {
		...
		if (AND == op || OR == op) {
			node_ptr l = compile_pop_distrib_ops_recurse(
				nodemgr, car(car(prop)));
			node_ptr r = compile_pop_distrib_ops_recurse(
				nodemgr, cdr(car(prop)));
			...
			if (((AND == op) && ARE_AND_DISTRIB_OPS(prop, l) &&
				...
				return compile_pop_distrib_ops_recurse(
					nodemgr,
					find_node(nodemgr, node_get_type(prop),
						  find_node(nodemgr, op, car(l),
							    car(r)),
						  Nil));
			} else if (((AND == op) &&
				    ARE_AND_DISTRIB_OPS(prop, l)) ||
				   ((OR == op) &&
				    ARE_OR_DISTRIB_OPS(prop, l))) { /* 4 */
				return compile_pop_distrib_ops_recurse(
					nodemgr,
					find_node(nodemgr, node_get_type(prop),
						  find_node(nodemgr, op, car(l),
				    ...
				    ARE_AND_DISTRIB_OPS(prop, r)) ||
				   ((OR == op) &&
				    ARE_OR_DISTRIB_OPS(prop, r))) { /* 3 */
				return compile_pop_distrib_ops_recurse(
					nodemgr,
					find_node(nodemgr, node_get_type(prop),
						  find_node(nodemgr, op, l,
							    car(r)),
						  Nil));
			}
					       ...
					       node_ptr prop)
{
	...
	case CONTEXT:
		return find_node(nodemgr, CONTEXT, car(prop),
				 compile_remove_ltl_bop_recurse(nodemgr,
								cdr(prop)));
	case OP_GLOBAL:
	case OP_FUTURE:
	case OP_ONCE:
	case OP_HISTORICAL:
		...
		if (Nil != cdr(prop)) {
			node_ptr condition = compile_remove_ltl_bop_recurse(
				nodemgr, car(prop));
			...
			for (j = t2 - t1; j > 0; j--) {
				...
				result = find_node(nodemgr, OP, condition,
						   result);
			}
			/* TOP^{t1} (body) */
			for (j = t1; j > 0; j--) {
