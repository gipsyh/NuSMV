...
#define ENV_SET_INT(env, key, val)                                     \
	{                                                              \
		nusmv_assert(-5 != val);                               \
		NuSMVEnv_set_or_replace_value(env, key,                \
					      NODE_FROM_INT(val + 5)); \
	}
...
void insert_module_hash(const NuSMVEnv_ptr env, node_ptr x, node_ptr y)
{
	hash_ptr module_hash =
		(hash_ptr)NuSMVEnv_get_value(env, ENV_MODULE_HASH);
...
{
	hash_ptr module_hash =
		(hash_ptr)NuSMVEnv_remove_value(env, ENV_MODULE_HASH);
...
static void clear_flatten_def_hash(const NuSMVEnv_ptr env)
{
	if (NuSMVEnv_has_value(env, ENV_FLATTEN_DEF_HASH)) {
		hash_ptr flatten_def_hash = (hash_ptr)NuSMVEnv_remove_value(
			env, ENV_FLATTEN_DEF_HASH);
	...
	HrcNode_ptr hrc_result /* hrc node to be populated*/)
{
	...
	if (HRC_NODE(NULL) != hrc_result) {
		node_ptr mod_def = lookup_module_hash(
			env, find_atom(nodemgr, module_name));
		if (NODE_PTR(Nil) == mod_def) {
			ErrorMgr_error_undefined(
				errmgr,
	...
	Compile_ConstructHierarchy(env, symb_table, layer, module_name,
				   inst_name, actual, result, hrc_result,
			      ...
			      boolean calc_vars_constr)
{
  ...
  */
	...
	tmp = Compile_FlattenSexp(symb_table, FlatHierarchy_get_init(hierarchy),
				  name);
	FlatHierarchy_set_init(hierarchy, tmp);

	tmp = Compile_FlattenSexp(symb_table,
				  FlatHierarchy_get_trans(hierarchy), name);
	FlatHierarchy_set_trans(hierarchy, tmp);

	tmp = Compile_FlattenSexp(symb_table,
				  FlatHierarchy_get_invar(hierarchy), name);
	...
	tmp = Compile_FlattenSexp(
		symb_table, reverse(FlatHierarchy_get_compassion(hierarchy)),
		name);
	...
	FlatHierarchy_set_pslspec(hierarchy,
				  tmp /*compile_fix_nested_context(tmp)*/);

	tmp = reverse(FlatHierarchy_get_compute(hierarchy));
	FlatHierarchy_set_compute(hierarchy,
				  tmp /*compile_fix_nested_context(tmp)*/);
  ...
  */
	...
	if (opt_syntactic_checks_disabled(opts)) {
		...
		Compile_check_next(symb_table,
				   FlatHierarchy_get_init(hierarchy), Nil,
				   false);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_invar(hierarchy), Nil,
				   false);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_trans(hierarchy), Nil,
				   true);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_justice(hierarchy), Nil,
				   false);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_compassion(hierarchy), Nil,
				   false);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_compute(hierarchy), Nil,
				   false);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_spec(hierarchy), Nil,
				   false);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_invarspec(hierarchy), Nil,
				   true);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_ltlspec(hierarchy), Nil,
				   true);
		Compile_check_next(symb_table,
				   FlatHierarchy_get_pslspec(hierarchy), Nil,
				   false);
		...
		/* Check [m]define bodies for nested nexts */
		{
			...
			SYMB_TABLE_FOREACH(symb_table, iter,
					   STT_DEFINE | STT_ARRAY_DEFINE)
			{
				node_ptr define = SymbTable_iter_get_symbol(
					symb_table, &iter);
				Compile_check_next(symb_table, define, Nil,
						   true);
				Compile_check_input_next(symb_table, define,
							 Nil);
			}
		}
		...
		/* TODO[MD] Here could be used the function FlatHierarchy_type_check */
		{
			...
			if (!isOk)
				ErrorMgr_error_type_system_violation(
					errmgr); /* error */
		}
			     ...
			     node_ptr context)
{
	...
	CATCH(errmgr)
	{
		MasterCompileFlattener_ptr flattener = MASTER_COMPILE_FLATTENER(
			NuSMVEnv_get_value(env, ENV_FLATTENER));
		result = MasterCompileFlattener_flatten(flattener, symb_table,
							sexp, context);
	}
	FAIL(errmgr)
	{
					 ...
					 node_ptr sexp, node_ptr context)
{
	...
	CATCH(errmgr)
	{
		MasterCompileFlattener_ptr flattener = MASTER_COMPILE_FLATTENER(
			NuSMVEnv_get_value(env, ENV_FLATTENER));
		result = MasterCompileFlattener_flatten_expand_define(
			flattener, symb_table, sexp, context);
	}
	FAIL(errmgr)
	{
		ErrorMgr_rpterr(errmgr, NULL); /* rethrow */
			       ...
			       const boolean expand_defines)
{
	...
	result = MasterCompileFlattener_get_definition(flattener, symb_table,
						       atom, mode);
	return result;
}
					...
					node_ptr ctx2)
{
	...
	if (op == ATOM || op == NUMBER) {
		return MasterNormalizer_normalize_node(
			normalizer, find_node(nodemgr, DOT, ctx1, ctx2));
	}

	if (op == BIT) {
		return find_node(nodemgr, BIT,
				 CompileFlatten_concat_contexts(env, ctx1,
								car(ctx2)),
				 cdr(ctx2));
	}
  ...
  */
	{
       ...
       the class to be the default */
		SymbTable_layer_add_to_class(st, MODEL_LAYER_NAME,
					     MODEL_LAYERS_CLASS);
		...
		CATCH(errmgr)
		{
			hierarchy = Compile_FlattenHierarchy(
				env, st, layer, sym_intern(env, "main"), Nil,
				Nil, true, calc_vars_constrains,
				expand_bounded_arrays, hrc_node);
		}
		FAIL(errmgr)
		{
			NuSMVCore_reset(env);
		...
		{
			propErr = PropDb_fill(
				db, st, FlatHierarchy_get_spec(hierarchy),
	...
	if (Parser_get_syntax_errors_list(env) != Nil) {
		...
		StreamMgr_print_error(streams,
				      "This allows you to dump the HRC.\n");
...
flattening_failed:
	...
	cmp_struct_unset_read_model(
		cmps); /* resets also the command read_model */
	return 1; /* error */
}
						  ...
						  node_ptr name)
{
   ...
   * implemented */
	...
	StreamMgr_print_error(streams, "':\n");
	StreamMgr_print_error(
		streams, "Error: Current version does not support assignment "
			 "of bit selections.\n");
						 ...
						 SymbType_ptr type)
{
	...
	if (SymbType_is_unsigned_word(type)) {
       ...
       bits. */
		if (width > 31) {
			ErrorMgr_error_out_of_bounds_word_toint_cast(errmgr,
								     body);
		}

		result = compile_flatten_build_word_toint_ith_bit_case(
			env, body, 0, false);
		...
		node_ptr w0 =
			find_node(nodemgr, NUMBER_UNSIGNED_WORD,
				  ...
				  Nil);
		node_ptr msb = find_node(nodemgr, NUMBER,
					 NODE_FROM_INT(width - 1), Nil);
       ...
       bits. */
		...
		positive = compile_flatten_build_word_toint_ith_bit_case(
			env, body, 0, false);
		negative = compile_flatten_build_word_toint_ith_bit_case(
			env, body, 0, true);
				      ...
				      FlatHierarchy_ptr flattenHierarchy)
{
  ...
  */
	...
	for (l = proc_assign_list; l != Nil;
	     l = cdr(l)) { /* Loops over processes */
		...
		rs = SymbTable_resolve_symbol(symb_table, running,
					      process_name);
		running_name = ResolveSymbol_get_resolved_name(rs);

		result = cons(nodemgr,
			      cons(nodemgr, process_name, process_assignments),
			      result);

		compileFlattenProcessRecur(env, symb_table, process_assignments,
					   Nil, running_name, flattenHierarchy);
	}
					     ...
					     node_ptr old_values)
{
	...
	node_ptr values =
		compile_flatten_normalise_value_list(nodemgr, old_values);
				       ...
				       node_ptr context)
{
	...
	node_ptr num =
		compile_flatten_eval_number(flattener, symb_table, n, context);
					      ...
					      node_ptr expr, node_ptr context)
{
    ...
    */
		...
		/* is it parameter => resolve and continue */
			...
			context = SymbTable_get_actual_parameter_context(
				symb_table, expr);
			expr = SymbTable_get_actual_parameter(symb_table, expr);

			/* Flatten expression if needed */
			if (context != Nil) {
				expr = Compile_FlattenSexp(symb_table, expr,
							   context);
				context = Nil;
			}
		...
		/* is it define => resolve and continue */
			...
			context =
				SymbTable_get_define_context(symb_table, expr);
    ...
    */
			...
			node_ptr tmp =
				Compile_FlattenSexp(symb_table, expr, context);
...
void CompileFlatten_quit_flattener(NuSMVEnv_ptr env)
{
	...
	if (NuSMVEnv_has_value(env, ENV_PROC_SELECTOR_VNAME)) {
		...
		proc_selector_vname = NODE_PTR(
			NuSMVEnv_remove_value(env, ENV_PROC_SELECTOR_VNAME));
		free_node(nodemgr, proc_selector_vname);
	}

	/* other vars: */
	set_variable_instantiation_mode(env,
					State_Variables_Instantiation_Mode);
	set_function_instantiation_mode(env,
					Frozen_Functions_Instantiation_Mode);
				     ...
				     boolean expand_bounded_arrays)
{
	...
	case TWODOTS: {
       ...
       with the constraint that "b >= a" */
		...
		symbolicType =
			SymbType_create(env, SYMB_TYPE_ENUM, expanded_range);
		break;
	}

	case SCALAR: {
		node_ptr value_list =
			CompileFlatten_normalise_value_list(env, car(type));
		node_ptr iter;
    ...
    */
	...
	case INTEGER:
		symbolicType =
			SymbType_create(env, SYMB_TYPE_INTEGER, Nil /*body*/);
		break;

	case REAL:
		symbolicType =
			SymbType_create(env, SYMB_TYPE_REAL, Nil /*body*/);
		break;

	/* [SM] We have to handle the CONTINUOUS type here */
	case CONTINUOUS: {
		symbolicType = SymbType_create(env, SYMB_TYPE_CONTINUOUS,
					       Nil /*body*/);
		break;
	}
    ...
    */
		node_ptr num =
			CompileFlatten_resolve_number(st, car(type), context);
		if ((node_ptr)NULL == num || NUMBER != node_get_type(num)) {
			ErrorMgr_error_not_constant_width_of_word_type(errmgr,
								       name);
		}
		symbolicType =
			SymbType_create(env,
    ...
    */
		...
		if (NUMBER != node_get_type(dim1)) {
			ErrorMgr_error_not_constant_width_of_word_array_type(
				errmgr, name);
		}

		subtype = Compile_InstantiateType(
			st, layer, find_node(nodemgr, WORDARRAY, name, dim1),
         ...
         once done remove the below error and assert */
			...
			{ /* below is the array expansion trick */
				...
				WordNumber_ptr maxWord =
					WordNumberMgr_max_unsigned_value(words,
									 size);
				unsigned long long maxValue =
					WordNumber_get_unsigned_value(maxWord);
				symbolicType = SymbType_create_array(
					subtype, 0, maxValue - 1);
			}
		}
		break;
	}
	...
	case INTARRAY_TYPE: {
		...
		symbolicType = SymbType_create(
			env, SYMB_TYPE_INTARRAY,
	...
	case ARRAY_TYPE: {
		...
		subtype = Compile_InstantiateType(
			st, layer, find_node(nodemgr, ARRAY, name, lower),
			cdr(type), context, false);

		symbolicType =
			SymbType_create_array(subtype, lower_num, upper_num);
		break;
	}
	...
	case NFUNCTION_TYPE: {
		...
		SymbType_ptr ret = Compile_InstantiateType(
			st, layer, name, rtype, context, false);
		...
		NODE_LIST_FOREACH(args_list, iter)
		{
			...
			args[i++] = Compile_InstantiateType(
				st, layer, name, arg, context, false);
		}
	...
	default:
		ErrorMgr_internal_error(errmgr,
					"Compile_InstantiateType: type = %d",
					node_get_type(type));
		break;
	} /* switch */
	...
	case SYMB_TYPE_ENUM: {
       ...
       instead of a var */
		...
		if (!(opt_keep_single_value_vars(opts)) &&
			...
			SymbLayer_declare_define(layer, name, context,
						 car(values));
			SymbType_destroy(type);
			type = SYMB_TYPE(NULL);

			StreamMgr_print_error(
				streams, "WARNING: single-value variable '");
			StreamMgr_nprint_error(streams, wffprint, "%N", name);
			StreamMgr_print_error(
				streams, "' has been stored as a constant\n");
		}
		break;
	}
	...
	case SYMB_TYPE_ARRAY: {
       ...
       The recursively declare array elements.*/
		...
		for (i = lower; i <= upper; ++i) {
			node_ptr index = find_node(nodemgr, NUMBER,
						   NODE_FROM_INT(i), Nil);
				...
				Instantiation_Functions_Mode_Type mode)
{
	...
	if (SYMB_TYPE(NULL) != type) {
		...
		case Frozen_Functions_Instantiation_Mode:
			SymbLayer_declare_function(layer, local_name, context,
						   type);
			break;
		default:
			error_unreachable_code(); /* impossible mode */
			     ...
			     HrcNode_ptr hrc_result)
{
   ...
   */
	nusmv_assert(
		Oiter_is_end(HrcNode_get_actual_parameters_iter(hrc_result)));
	nusmv_assert(
		Oiter_is_end(HrcNode_get_formal_parameters_iter(hrc_result)));
   ...
   */
	...
	while (formal_list) {
		...
		if (!actual_list) {
			if (basename != (node_ptr)NULL) {
				...
				StreamMgr_print_error(
					streams, "While creating instance ");
				StreamMgr_nprint_error(streams, wffprint, "%N",
						       basename);
				...
				ErrorMgr_rpterr(
					errmgr,
					"module 'main' cannot have formal parameters");
			}
		}
     ...
     * the type */
		formal_parameter_node =
			cons(nodemgr, formal_parameter_hrc, Nil);
					  ...
					  FlatHierarchy_ptr fh)
{
	...
	for (i = lower; i <= upper; ++i) {
		node_ptr index =
			find_node(nodemgr, NUMBER, NODE_FROM_INT(i), Nil);
		node_ptr new_name = find_node(nodemgr, ARRAY, name, index);
		if (SymbType_is_array(
			    subtype)) { /* still array => recursively go down */
				    ...
				    boolean expand_bounded_arrays)
{
	...
	node_ptr name_mod = find_node(nodemgr, MODTYPE,
				      find_atom(nodemgr, context),
	...
	case INTARRAY_TYPE: {
		...
		boolean dv = Compile_DeclareVariable(
			st, layer, name, symbolicType, context,
		...
		if (HRC_NODE(NULL) != hrc_result) {
			if (true == dv) {
				...
				node_ptr hrc_var =
					cons(nodemgr, hrc_var_name, type);

				switch (mode) {
				case State_Variables_Instantiation_Mode:
					HrcNode_add_state_variable(hrc_result,
								   hrc_var);
					break;
				case Input_Variables_Instantiation_Mode:
					HrcNode_add_input_variable(hrc_result,
								   hrc_var);
					break;
				case Frozen_Variables_Instantiation_Mode:
					HrcNode_add_frozen_variable(hrc_result,
								    hrc_var);
					break;
				default:
         ...
         declared also into the HRC node */
				...
				if (NUMBER != node_get_type(body)) {
					HrcNode_add_constants(hrc_result,
							      cons(nodemgr,
								   body, Nil));
	...
	case ARRAY_TYPE: {
		SymbType_ptr symbolicType = Compile_InstantiateType(
			st, layer, name, type, context, false);
	...
	case MODTYPE: {
		...
		if (HRC_NODE(NULL) == hrc_result) {
			compile_instantiate_by_name(env, st, layer, car(type),
						    name, actual, assign,
						    result, HRC_NODE(NULL),
						    instances,
						    expand_bounded_arrays);
		} else {
	...
	case PROCESS: {
		...
		Compile_ConstructHierarchy(env, st, layer, pmod_name, name,
					   actual, result, hrc_result,
					   instances, expand_bounded_arrays);
		free_list(nodemgr, actual);
		break;
	}
	...
	default:
		ErrorMgr_internal_error(errmgr,
					"compile_instantiate_var: type = %d",
					node_get_type(type));
		break;
	}
}
				    ...
				    HrcNode_ptr hrc_result, hash_ptr instances)
{
	...
	case NFUNCTION_TYPE: {
       ...
       Compile_DeclareVariable */
		...
		if (HRC_NODE(NULL) != hrc_result) {
			...
			switch (mode) {
			case Frozen_Functions_Instantiation_Mode:
				HrcNode_add_frozen_function(hrc_result,
							    hrc_fun);
				break;
			default:
				ErrorMgr_internal_error(
	...
	default:
		ErrorMgr_internal_error(errmgr,
					"compile_instantiate_fun: type = %d",
					node_get_type(type));
		break;
	}
}
				     ...
				     boolean expand_bounded_arrays)
{
	...
	while (iter != Nil) {
		...
		compile_instantiate_var(env, st, layer, name, type, mod_name,
				     ...
				     HrcNode_ptr hrc_result, hash_ptr instances)
{
	...
	while (iter != Nil) {
		...
		compile_instantiate_fun(env, st, layer, name, type, mod_name,
					assign, result, hrc_result, instances);
   ...
   */
	...
	while (formal_list) {
		...
		formal_parameter_flat =
			find_node(nodemgr, DOT, basename,
				  find_atom(nodemgr, car(formal_list)));

		formal_parameter_flat = MasterNormalizer_normalize_node(
			normalizer, formal_parameter_flat);

		actual_parameter = car(actual_list);

		if (!SymbLayer_can_declare_parameter(layer,
						     formal_parameter_flat)) {
			nusmv_yylineno = node_get_lineno(car(formal_list));
			ErrorMgr_error_redefining(errmgr,
						  formal_parameter_flat);
		}
	...
	for (mod_body_decls = mod_body; mod_body_decls != Nil;
		...
		case DEFINE: {
			...
			for (define_iter = car(cur_decl); define_iter != Nil;
				...
				rs = SymbTable_resolve_symbol(st, local_name,
							      mod_name);
				name = ResolveSymbol_get_resolved_name(rs);
             ...
             distinguished by ARRAY_DEF at top of the expression */
                 ...
                 declaration, so the resulting list is in order */
					...
					nusmv_yylineno = node_get_lineno(
						local_name); /* set correct line info */
					/* more precise error message */
					if (SymbTable_is_symbol_parameter(st,
									  name))
						ErrorMgr_error_shadowing(errmgr,
									 name);
					else
						ErrorMgr_error_redefining(
							errmgr, name);
				}
			} /* loop on defines */
		} break;
  ...
  */
	for (mod_body_decls = mod_body; mod_body_decls != Nil;
		...
		case ISA:
			...
			if (HRC_NODE(NULL) != hrc_result) {
				HrcNode_ptr root =
					get_hrc_root_node(hrc_result);
				/* Set a flag so we know that the HRC hierarchy is not usable */
				HrcNode_set_undef(root, (void *)~0);
				hrc_result = HRC_NODE(NULL);
			}
			compile_instantiate_by_name(env, st, layer,
						    car(cur_decl), mod_name,
						    Nil, assign, result,
		...
		case FUN:
			...
			compile_instantiate_funs(env, st, layer, car(cur_decl),
						 mod_name, assign, result,
						 hrc_result, instances);
			break;

		case VAR:
			compile_instantiate_vars(env, st, layer, car(cur_decl),
						 mod_name, assign, result,
						 hrc_result, instances,
						 expand_bounded_arrays);
			break;
		...
		case TRANS:
			tmp = find_node(nodemgr, AND,
					FlatHierarchy_get_trans(result),
					find_node(nodemgr, CONTEXT, mod_name,
						  car(cur_decl)));
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_trans_expr(hrc_result,
						       car(cur_decl));
		...
		case INIT:
			tmp = find_node(nodemgr, AND,
					FlatHierarchy_get_init(result),
					find_node(nodemgr, CONTEXT, mod_name,
						  car(cur_decl)));
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_init_expr(hrc_result,
						      car(cur_decl));
		...
		case INVAR:
			tmp = find_node(nodemgr, AND,
					FlatHierarchy_get_invar(result),
					find_node(nodemgr, CONTEXT, mod_name,
						  car(cur_decl)));
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_invar_expr(hrc_result,
						       car(cur_decl));
       ...
       */
		...
		case SPEC: {
			...
			if (node_get_type(car(cur_decl)) == CONTEXT) {
				...
				node_ptr new_ctx =
					CompileFlatten_concat_contexts(
						env, mod_name, caar(cur_decl));
				tmp = find_node(nodemgr, CONTEXT, new_ctx,
						cdr(car(cur_decl)));
			} else
				tmp = find_node(nodemgr, CONTEXT, mod_name,
						car(cur_decl));

			/* Support for property names */
			if (Nil != property_name) {
				property_name = CompileFlatten_concat_contexts(
					env, mod_name, property_name);
				if (!FlatHierarchy_add_property_name(
					    result, property_name)) {
					ErrorMgr_error_redefining(
						errmgr, property_name);
				}
			}

			tmp = find_node(nodemgr, SPEC, tmp, property_name);
			tmp = cons(nodemgr, tmp,
				   FlatHierarchy_get_spec(result));
			FlatHierarchy_set_spec(result, tmp);
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_ctl_property_expr(hrc_result,
							      cur_decl);
		} break;
		...
		case LTLSPEC: {
			...
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_ltl_property_expr(hrc_result,
							      cur_decl);
		} break;
		...
		case PSLSPEC: {
			...
			if (node_get_type(car(cur_decl)) == CONTEXT) {
				...
				node_ptr new_ctx =
					CompileFlatten_concat_contexts(
						env, mod_name, caar(cur_decl));
				tmp = PslNode_new_context(nodemgr, new_ctx,
							  cdr(car(cur_decl)));
			} else
				tmp = find_node(nodemgr, CONTEXT, mod_name,
						car(cur_decl));
			...
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_psl_property_expr(hrc_result,
							      cur_decl);
		} break;
		...
		case INVARSPEC: {
			...
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_invar_property_expr(hrc_result,
								cur_decl);
		} break;
		...
		case COMPUTE: {
			...
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_compute_property_expr(hrc_result,
								  cur_decl);
		} break;
			/* ----------------------------------------------------------------------
       */
		...
		case JUSTICE:
			...
			if (HRC_NODE(NULL) != hrc_result)
				HrcNode_add_justice_expr(hrc_result,
							 car(cur_decl));
			break;
		...
		case COMPASSION:
			...
			if (HRC_NODE(NULL) != hrc_result) {
				node_ptr hrc_compassion =
					cons(nodemgr, car(car(cur_decl)),
					     cdr(car(cur_decl)));

				HrcNode_add_compassion_expr(hrc_result,
							    hrc_compassion);
			}
		...
		case ASSIGN: {
			...
			if (HRC_NODE(NULL) != hrc_result) {
				compile_add_assign_hrc(nodemgr, hrc_result,
						       car(cur_decl));
			}
		} break;
		...
		case CONSTANTS:
			...
			flatten_declare_constants_within_list(
				st, layer, reverse_ns(nodemgr, car(cur_decl)));
		...
		case DEFINE_PROPERTY: {
			...
			tmp = find_node(nodemgr, DEFINE_PROPERTY, tmp,
					property_name);

			tmp = cons(nodemgr, tmp,
				   FlatHierarchy_get_property_patterns(result));
	...
	hash_ptr instances, boolean expand_bounded_arrays)
{
	...
	node_ptr s;
	node_ptr mod_name =
		find_atom(nodemgr, module_name); /* find module name */
	...
	if (NuSMVEnv_has_value(env, ENV_MODULE_STACK)) {
		module_stack =
			NODE_PTR(NuSMVEnv_get_value(env, ENV_MODULE_STACK));
	}
	...
	while (s != Nil) {
		if (car(s) == mod_name) {
			ErrorMgr_rpterr(errmgr,
					"module \"%s\" is recursively defined",
					UStringMgr_get_string_text(
						(string_ptr)car(module_name)));
		}
		s = cdr(s);
	}
				       ...
				       FlatHierarchy_ptr flatHierarchy)
{
	...
	case CONS:
	case AND:
		compileFlattenProcessRecur(env, symb_table, car(assign),
					   context, running, flatHierarchy);
		compileFlattenProcessRecur(env, symb_table, cdr(assign),
					   context, running, flatHierarchy);
		break;

	case CONTEXT:
		compileFlattenProcessRecur(env, symb_table, cdr(assign),
					   car(assign), running, flatHierarchy);
		break;
	...
	case EQDEF: {
		...
		case SMALLINIT: /* init assignement */ {
			rs = SymbTable_resolve_symbol(symb_table, car(left),
						      context);
			vname = ResolveSymbol_get_resolved_name(rs);
			lhsa = find_node(nodemgr, node_get_type(left), vname,
					 Nil);
			stored = FlatHierarchy_lookup_assign(flatHierarchy,
							     lhsa);
		...
		case NEXT: /* next assignement */
		{
			...
			if (NuSMVEnv_has_value(env, ENV_PROC_SELECTOR_VNAME)) {
				...
				if (Nil != stored)
					ErrorMgr_error_reassigning(errmgr,
								   vname);
			}
		} break;
		...
		default:
			...
			{
				rs = SymbTable_resolve_symbol(symb_table, left,
							      context);
				vname = lhsa =
					ResolveSymbol_get_resolved_name(rs);
				stored = FlatHierarchy_lookup_assign(
					flatHierarchy, lhsa);
	...
	default:
		ErrorMgr_internal_error(errmgr,
					"compileFlattenProcessRecur: type = %d",
					      ...
					      node_ptr process_name_list)
{
    ...
    */
	...
	proc_selector_vname = find_node(
		nodemgr, DOT, Nil, sym_intern(env, PROCESS_SELECTOR_VAR_NAME));
	...
	if (SymbTable_is_symbol_declared(symb_table, proc_selector_vname)) {
		ErrorMgr_error_redefining_operational_symbol(
			errmgr, proc_selector_vname);
	}
	...
	/* declare the "process-selector" symbol with a proper values */
	{
		...
		flatten_declare_constants_within_list(symb_table, layer,
						      process_name_list);
		symbolicType =
			SymbType_create(env, SYMB_TYPE_ENUM, process_name_list);
		SymbLayer_declare_input_var(layer, proc_selector_vname,
					    symbolicType);
	}
		...
		for (iter = process_name_list; iter != Nil; iter = cdr(iter)) {
			...
			if (module_name == main_atom) {
				...
				rs = SymbTable_resolve_symbol(
					symb_table, running_atom, Nil);
			} else {
				rs = SymbTable_resolve_symbol(
					symb_table, running_atom, module_name);
			}
			...
			def_body = find_node(nodemgr, EQUAL,
					     proc_selector_vname, module_name);

			/* The flatten hash has to be filled with the flattened
         body of the newly defined symbol.
      */
			insert_flatten_def_hash(
				env, def_name,
				Compile_FlattenSexp(symb_table, def_body, Nil));

			/* declare the define: */
			SymbLayer_declare_define(layer, def_name,
						 Nil /*context*/, def_body);
		} /* for */
	}
}
			    ...
			    SymbTable_ptr st, node_ptr n, node_ptr context)
{
	...
	case CONTEXT:
		...
		return compile_flatten_eval_number(flattener, st, cdr(n),
						   car(n));
	...
	case ARRAY: {
      ...
      */
			...
			node_ptr tmp = MasterCompileFlattener_flatten(
				flattener, st, n, context);

			/* it is impossible that flattening returned undefined identifier. */
			nusmv_assert(tmp != n);
			return compile_flatten_eval_number(flattener, st, tmp,
							   Nil);
	...
	case BIT: {
		...
		if (name != Nil && ResolveSymbol_is_define(rs)) {
			...
			node_ptr body =
				SymbTable_get_define_flatten_body(st, name);
			return compile_flatten_eval_number(flattener, st, body,
							   (node_ptr)NULL);
		}
		if (ResolveSymbol_is_parameter(rs)) {
         ...
         parameter */
			...
			return compile_flatten_eval_number(flattener, st,
							   actual, Nil);
		}
		return name;
	}
				     ...
				     node_ptr definition)
{
	...
	case ARRAY_DEF: {
		...
		/* Declare this symbol */
		SymbLayer_declare_array_define(layer, name, mod_name,
					       definition);
       ...
       with first index = 0 */
		for (idx = 0, iter = car(definition); iter != Nil;
			...
			index = find_node(nodemgr, NUMBER, NODE_FROM_INT(idx),
					  Nil);
			instantiate_array_define(st, layer,
						 find_node(nodemgr, ARRAY, name,
							   index),
						 mod_name, car(iter));
		}
		break;
			    ...
			    node_ptr assign_list)
{
	...
	while (!Slist_is_empty(stack)) {
		...
		case SMALLINIT: {
			/* init assign */
			node_ptr assign_node = new_node(
				nodemgr, ASSIGN, car(left_expr), right_expr);
						     ...
						     node_ptr old_value_list)
{
	...
	/* normalise the tail */
	new_tail = compile_flatten_normalise_value_list(nodemgr,
							cdr(old_value_list));
	...
	const NuSMVEnv_ptr env, node_ptr wexpr, int bit, boolean is_neg)
{
	...
	/* cond ? 2^bit : 0 */
	res = new_node(nodemgr, IFTHENELSE, new_node(nodemgr, COLON, cond, mul),
		       zero);
					    ...
					    node_ptr name)
{
	...
	{
		SymbType_ptr t =
			SYMB_TYPE(NodeList_get_elem_at(arg_types, iter));
