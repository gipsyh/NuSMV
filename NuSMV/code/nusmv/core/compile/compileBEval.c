				  ...
				  boolean in_next, hash_ptr expr2bexpr)
{
...

	{
		...
		for (i = 0; i < width; ++i) {
			...
			res = find_node(nodemgr, CONS, bit,
					res); /* msb at the top */
		} /* for i */
					...
					boolean in_next, hash_ptr expr2bexpr)
{
	...
	case NOTEQUAL: /* !(a=b) is smaller than (a!=b) */
		res = expr2bexpr_recur(
			...
			find_node(nodemgr, NOT,
				  find_node(nodemgr, EQUAL, left, right), Nil),
			context, in_next, expr2bexpr);
		break;
    ...
    */
       ...
       chains of define definitions to actually get the expression */
		{
			...
			left = CompileFlatten_resolve_define_chains(st, left,
								    context);
			right = CompileFlatten_resolve_define_chains(st, right,
								     context);
		}

		if (CASE == node_get_type(right) ||
			...
			if (CASE == node_get_type(right) ||
				...
				if (FAILURE != node_get_type(iter))
					iter = find_node(nodemgr, kind, left,
							 iter);
			} else {
				iter = left;
				while (node_get_type(iter) == CASE ||
				...
				if (FAILURE != node_get_type(iter))
					iter = find_node(nodemgr, kind, iter,
							 right);
			}
			/* revert the list, find_node it and add CASE */
			for (new_exp = iter, iter = list; iter != Nil;
			     iter = cdr(iter)) {
				new_exp = find_node(nodemgr, CASE, car(iter),
						    new_exp);
			}
			free_list(nodemgr, list);
       ...
       * resolved, so Nil has to be passed for context now */
			res = expr2bexpr_recur(enc, det_layer, new_exp, Nil,
					       in_next, expr2bexpr);
		} else if (ATTIME == node_get_type(left) &&
			   ATTIME == node_get_type(right)) {
			res = expr2bexpr_resolve_timed_equal_expr(enc, expr);
		} else {
			Utils_set_mode_for_range_check(
				env, false); /* issue just a warning */
			CATCH(errmgr)
			{
				res = scalar_atom2bexpr(enc, det_layer, expr,
							context, in_next);
			}
			FAIL(errmgr)
			{
...
static boolean contain_nested_attime(BddEnc_ptr enc, node_ptr expr)
{
	boolean res;
	SymbTable_ptr symb_table = BaseEnc_get_symb_table(BASE_ENC(enc));
	hash_ptr memoiz = compile_beval_get_handled_hash(
		symb_table, ST_CHECK_NESTED_ATTIME_HASH);
				 ...
				 boolean in_next, hash_ptr expr2bexpr)
{
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_parameter(rs)) {
			...
			node_ptr param = SymbTable_get_flatten_actual_parameter(
				symb_table, name);
			res = expr2bexpr_recur(enc, det_layer, param, context,
					       in_next, expr2bexpr);
			break;
		}
		...
		if (ResolveSymbol_is_define(rs)) {
			...
			node_ptr body = SymbTable_get_define_flatten_body(
				symb_table, name);
			if (body == (node_ptr)NULL)
				ErrorMgr_error_undefined(errmgr, name);

			res = expr2bexpr_recur(enc, det_layer, body, context,
					       in_next, expr2bexpr);
		}
		/* variable */
			...
			if (SymbType_is_boolean(vtype)) {
				...
				ErrorMgr_rpterr(
					errmgr,
					"Unexpected non boolean variable");
			}
		}
	...
	case CAST_SIGNED:
		...
		if (node_get_type(res) == NUMBER_UNSIGNED_WORD) {
			res = find_node(nodemgr, NUMBER_SIGNED_WORD, car(res),
					cdr(res));
		}
		break;
	...
	case CAST_UNSIGNED:
		...
		if (node_get_type(res) == NUMBER_SIGNED_WORD) {
			res = find_node(nodemgr, NUMBER_UNSIGNED_WORD, car(res),
					cdr(res));
		}
		break;
       ...
       semantics of the word resize operator */
		...
		if (n == m) {
			...
			if (SymbType_is_signed_word(orig_type)) {
				...
				node_ptr rightmost_sel = find_node(
					nodemgr, COLON,
					find_node(nodemgr, NUMBER,
				...
				node_ptr cnexpr = find_node(
					nodemgr, CAST_SIGNED, nexpr, Nil);

				res = expr2bexpr_recur(enc, det_layer, cnexpr,
						       context, in_next,
						       expr2bexpr);
			} /* signed words */
			else {
				...
				res = expr2bexpr_recur(enc, det_layer, nexpr,
						       context, in_next,
						       expr2bexpr);
			} /* unsigned */
		} else {
	...
	case EXTEND: {
		...
		a = expr2bexpr_recur(enc, det_layer, car(expr), context,
				     in_next, expr2bexpr);
		if (node_get_type(a) == NUMBER_UNSIGNED_WORD) {
			WordNumber_ptr wn = WORD_NUMBER(car(a));
			...
			/* not a constant */
			res = node_word_extend(
				a, delta, SymbType_is_signed_word(ta), env);
		}
		break;
	}
	...
	case BIT_SELECTION: {
       ...
       that the range is numeric. */
		hbit = CompileFlatten_resolve_number(symb_table, car(cdr(expr)),
						     context);
		lbit = CompileFlatten_resolve_number(symb_table, cdr(cdr(expr)),
						     context);
		...
		res = node_word_selection(
			a, find_node(nodemgr, COLON, hbit, lbit), env);
		break;
	}
	...
	case EQDEF: {
		...
		{ /* Resolve the symbol */
			ResolveSymbol_ptr rs;
			rs = SymbTable_resolve_symbol(symb_table, var_name,
						      context);
			var_name = ResolveSymbol_get_resolved_name(rs);
		}

		nusmv_assert(
			SymbTable_is_symbol_declared(symb_table, var_name));
		...
		if (SymbType_is_boolean(lhs_type) &&
			...
			res = ExprMgr_iff(exprs, var_name,
					  expr2bexpr_recur(enc, det_layer, rhs,
							   context, in_next,
							   expr2bexpr));
			...
			res = expr2bexpr_recur_binary(enc, det_layer,
						      fixed_expr, context,
						      in_next, expr2bexpr);
		}
		break;
	...
	case ATTIME: {
		...
		node_ptr bexpr =
			Wff2Nnf(env, Compile_detexpr2bexpr(enc, car(expr)));
		/* no nested ATTIME */
	...
	case NOT:
	case UMINUS:
		res = expr2bexpr_recur_unary(enc, det_layer, expr, context,
					     in_next, expr2bexpr);
		break;
	...
	case DIVIDE:
	case MOD:
		res = expr2bexpr_recur_binary(enc, det_layer, expr, context,
					      in_next, expr2bexpr);
		break;
	...
	case COMPUTE:
		res = expr2bexpr_recur(enc, det_layer, cdr(car(expr)),
				       car(car(expr)), in_next, expr2bexpr);
		break;

	case CONTEXT:
		res = expr2bexpr_recur(enc, det_layer, cdr(expr), car(expr),
				       in_next, expr2bexpr);
		break;
	...
	if (res != (node_ptr)NULL) {
		expr2bexpr_hash_insert_entry(expr2bexpr, nodemgr, expr, context,
					     res, in_next,
					     det_layer != SYMB_LAYER(NULL));
	}
				  ...
				  boolean in_next)
{
	...
	add_ptr bool_add = BddEnc_expr_to_add(
		enc, (in_next) ? find_node(nodemgr, NEXT, expr, Nil) : expr,
		context);
					       ...
					       char *hash_str)
{
	if (!strcmp(ST_BEVAL_EXPR2BEXPR_HASH, hash_str)) {
		...
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_CHECK_NESTED_ATTIME_HASH,
			(ST_PFICPCP)NULL, (ST_PFICPI)NULL, (ST_PFSR)NULL,
			(SymbTableTriggerFun)NULL,
			SymbTable_clear_handled_remove_action_hash,
			(SymbTableTriggerFun)NULL);
	} else
