...
static node_ptr expr2bexpr_recur_binary(BddEnc_ptr enc, SymbLayer_ptr det_layer,
  ...
  if (SymbType_is_boolean(t_left) && SymbType_is_boolean(t_right)) {
    ...
    default:
      /* all other expressions are impossible. they cannot have boolean
         ...
         (parent) expression */
      ErrorMgr_internal_error(
          errmgr, "Unexpected operator while booleanizing an expression");
    }
    return res;
  }
  ...
  case NOTEQUAL: /* !(a=b) is smaller than (a!=b) */
    res =
        expr2bexpr_recur(enc, det_layer, find_node(nodemgr, EQUAL, left, right),
                         context, in_next, expr2bexpr);
...
static node_ptr expr2bexpr_recur(BddEnc_ptr enc, SymbLayer_ptr det_layer,
...

  {
    ...
    case CAST_WORD1:
      res = expr2bexpr_cast_word1(enc, det_layer, expr, context, in_next,
                                  expr2bexpr);
      break;
    ...
    case WRESIZE:
      res =
          expr2bexpr_resize(enc, det_layer, expr, context, in_next, expr2bexpr);
      break;

    case EXTEND:
      res =
          expr2bexpr_extend(enc, det_layer, expr, context, in_next, expr2bexpr);
      break;
    ...
    case RSHIFT:
      res =
          expr2bexpr_shift(enc, det_layer, expr, context, in_next, expr2bexpr);
      break;

    case LROTATE:
    case RROTATE:
      res =
          expr2bexpr_rotate(enc, det_layer, expr, context, in_next, expr2bexpr);
      break;

    case BIT_SELECTION:
      res = expr2bexpr_bitselect(enc, det_layer, expr, context, in_next,
                                 expr2bexpr);
      break;

    case EQDEF:
      res =
          expr2bexpr_eqdef(enc, det_layer, expr, context, in_next, expr2bexpr);
      break;
...
#else
{
...

  {
    ...
    hash_ptr expr2bexpr =
        compile_beval_get_handled_hash(st, ST_BEVAL_EXPR2BEXPR_HASH);
...
#warning TODO: Consider to use an ST Handled Hash to perform this memoization
    ...
    r_left =
        expr2bexpr_recur(enc, det_layer, left, context, in_next, expr2bexpr);
    r_right =
        expr2bexpr_recur(enc, det_layer, right, context, in_next, expr2bexpr);
...
static Expr_ptr expr2bexpr_extend(BddEnc_ptr enc, SymbLayer_ptr det_layer,
  ...
  nusmv_assert(SymbType_is_word(ta));
  node_ptr a =
      expr2bexpr_recur(enc, det_layer, car(expr), context, in_next, expr2bexpr);
  if (node_get_type(a) == NUMBER_UNSIGNED_WORD) {
    WordNumber_ptr wn = WORD_NUMBER(car(a));
  ...
  if (n == m) {
    ...
    if (SymbType_is_signed_word(orig_type)) {
      node_ptr msb_sel = find_node(
          nodemgr, COLON, find_node(nodemgr, NUMBER, NODE_FROM_INT(m - 1), Nil),
          find_node(nodemgr, NUMBER, NODE_FROM_INT(m - 1), Nil));

      node_ptr rightmost_sel = find_node(
          nodemgr, COLON, find_node(nodemgr, NUMBER, NODE_FROM_INT(n - 2), Nil),
          find_node(nodemgr, NUMBER, NODE_FROM_INT(0), Nil));
      ...
      node_ptr rightmost_sel = find_node(
          nodemgr, COLON, find_node(nodemgr, NUMBER, NODE_FROM_INT(n - 1), Nil),
          find_node(nodemgr, NUMBER, NODE_FROM_INT(0), Nil));
    ...
    res = expr2bexpr_recur(enc, det_layer, nexpr, context, in_next, expr2bexpr);
  }
...
static Expr_ptr expr2bexpr_eqdef(BddEnc_ptr enc, SymbLayer_ptr det_layer,
  ...
  if (SymbType_is_boolean(lhs_type) &&
    ...
    res = expr2bexpr_recur_binary(enc, det_layer, fixed_expr, context, in_next,
                                  expr2bexpr);
  }
...
static Expr_ptr expr2bexpr_leaf(BddEnc_ptr enc, SymbLayer_ptr det_layer,
  ...
  case ATOM: {
    ...
    if (ResolveSymbol_is_define(rs)) {
      ...
      res = expr2bexpr_recur(enc, det_layer, body, Nil, in_next, expr2bexpr);
    }
    /* constant */
    else if (ResolveSymbol_is_constant(rs)) {
      res = name;
...
static node_ptr _mk_key(BStructInfo_ptr aux, EPFMEE fn, Expr_ptr a,
                        Expr_ptr b) {
