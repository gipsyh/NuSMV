...
#define _IS_LEAF(a)                                                         \
	((NULL != a) &&                                                     \
	 ((NUMBER == node_get_type(a)) || (FALSEEXP == node_get_type(a)) || \
	  (TRUEEXP == node_get_type(a)) || (ATOM == node_get_type(a)) ||    \
	  (DOT == node_get_type(a)) || (CONS == node_get_type(a)) ||        \
	  (FAILURE == node_get_type(a))))

#define _IS_ITE(a)                                                           \
	(((IFTHENELSE == node_get_type(a)) || (CASE == node_get_type(a))) && \
...
static Expr_ptr expr_plus_(const ExprMgr_ptr self, const Expr_ptr a,
			   const Expr_ptr b);
static Expr_ptr expr_times_(const ExprMgr_ptr self, const Expr_ptr a,
			    const Expr_ptr b);
static Expr_ptr expr_divide_(const ExprMgr_ptr self, const Expr_ptr a,
			     const Expr_ptr b);
static Expr_ptr expr_minus_(const ExprMgr_ptr self, const Expr_ptr a,
			    const Expr_ptr b);
static Expr_ptr expr_mod_(const ExprMgr_ptr self, const Expr_ptr a,
			  const Expr_ptr b);
static Expr_ptr expr_equal_(const ExprMgr_ptr self, const Expr_ptr a,
			    const Expr_ptr b);
static Expr_ptr expr_notequal_(const ExprMgr_ptr self, const Expr_ptr a,
			       const Expr_ptr b);
static Expr_ptr expr_le_(const ExprMgr_ptr self, const Expr_ptr a,
			 const Expr_ptr b);
static Expr_ptr expr_ge_(const ExprMgr_ptr self, const Expr_ptr a,
			 const Expr_ptr b);
static Expr_ptr expr_lt_(const ExprMgr_ptr self, const Expr_ptr a,
			 const Expr_ptr b);
static Expr_ptr expr_gt_(const ExprMgr_ptr self, const Expr_ptr a,
			 const Expr_ptr b);
static Expr_ptr expr_eqdef_(const ExprMgr_ptr self, const Expr_ptr a,
			    const Expr_ptr b);
static Expr_ptr expr_union_(const ExprMgr_ptr self, const Expr_ptr a,
			    const Expr_ptr b);
static Expr_ptr expr_setin_(const ExprMgr_ptr self, const Expr_ptr a,
			    const Expr_ptr b);
...
Expr_ptr Compile_detexpr2bexpr_list(BddEnc_ptr enc, Expr_ptr expr)
{
  ...
  */
	...
	for (iter = list; Nil != iter; iter = cdr(iter)) {
		...
		elem = expr2bexpr_recur(enc, SYMB_LAYER(NULL), car(iter), Nil,
					is_next, expr2bexpr);
				      ...
				      int numWidth, hash_ptr expr2bexpr)
{
	...
	if (numWidth > 0)
		...
		maxShift =
			~0U; /* value for sure bigger than any possible width */
	...
	for (i = 0; i < width; ++i) {
		...
		for (k = MIN(i, maxShift); k >= 0; --k) {
			...
			if (numWidth > 0) { /* b is a word */
				node_ptr wk = node_word_create_from_integer(
					k, numWidth, env);
			...
			bit = ExprMgr_ite(exprs, beqk,
					  array_fetch(node_ptr, va, i - k), bit,
					  symb_table);
		}
			       ...
			       hash_ptr expr2bexpr)
{
	...
	if (numWidth > 0) { /* b is a word */
		...
		if ((width >= (2ULL << (numWidth - 1)) -
				      1)) { /* width >= (2^numWidth - 1). */
         ...
         a check that number<=width is not required. */
			...
			node_ptr cond = node_word_unsigned_less_equal(
				b,
				node_word_create_from_integer(width, numWidth,
							      env),
				env);
			res = ExprMgr_ite(exprs, cond, defaultBit, error,
					  symb_table);
		}
	} else { /* b is not a word */
				 ...
				 boolean in_next, hash_ptr expr2bexpr)
{
	...
	if (ta == SYMB_TYPE(NULL) || SymbType_is_error(ta)) {
		ErrorMgr_internal_error(
			errmgr, "expr2bexpr_shift: operand has invalid type");
	}
	...
	if (SymbType_is_unsigned_word(tb)) {
		b = expr2bexpr_recur(enc, det_layer, cdr(expr), context,
				     in_next, expr2bexpr);
		_CHECK_WORD(b);
		if (node_get_type(b) == NUMBER_UNSIGNED_WORD) {
			...
			b = find_node(
				nodemgr, NUMBER,
				NODE_FROM_INT(
					(int)WordNumber_get_unsigned_value(
						WORD_NUMBER(car(b)))),
				Nil);
			numWidth = -1;
		} else
	...
	if (node_get_type(a) == NUMBER_UNSIGNED_WORD ||
		...
		case NUMBER_UNSIGNED_WORD:
			bits = WordNumber_get_unsigned_value(
				WORD_NUMBER(car(b)));
			break;
		...
		if (bits > 0) {
			...
			if (etype == LSHIFT) {
				ws = WordNumberMgr_left_shift(
					words, WORD_NUMBER(car(a)), bits);
			} else {
				ws = (node_get_type(a) ==
				      NUMBER_UNSIGNED_WORD) ?
					     WordNumberMgr_unsigned_right_shift(
						     words, WORD_NUMBER(car(a)),
						     bits) :
					     WordNumberMgr_signed_right_shift(
						     words, WORD_NUMBER(car(a)),
						     bits);
			}
			return find_node(nodemgr, node_get_type(a),
					 (node_ptr)ws, Nil);
		}
	...
	{ /* construct the default value.  For right shift of signed
       ...
       other case default value is 0.  */
		...
		if (etype == LSHIFT) {
			res = expr2bexpr_shift_left(enc, det_layer, context,
						    in_next, a, b, def_case,
						    numWidth, expr2bexpr);
		} else {
			res = expr2bexpr_shift_right(enc, det_layer, context,
						     in_next, a, b, def_case,
						     numWidth, expr2bexpr);
		}
		return res;
				  ...
				  boolean in_next, hash_ptr expr2bexpr)
{
	...
	if (ta == SYMB_TYPE(NULL) || SymbType_is_error(ta)) {
		ErrorMgr_internal_error(
			errmgr, "expr2bexpr_rotate: operand has invalid type");
	}
	...
	if (SymbType_is_word(tb)) {
		...
		if (node_get_type(b) == NUMBER_UNSIGNED_WORD) {
			...
			b = find_node(
				nodemgr, NUMBER,
				NODE_FROM_INT((int)WordNumber_get_signed_value(
					WORD_NUMBER(car(b)))),
				Nil);
			numWidth = -1;
		} else
	...
	if (node_get_type(a) == NUMBER_UNSIGNED_WORD ||
		...
		if (bits > 0) {
			WordNumber_ptr ws;
			if (etype == LROTATE) {
				ws = WordNumberMgr_left_rotate(
					words, WORD_NUMBER(car(a)), bits);
			} else
				ws = WordNumberMgr_right_rotate(
					words, WORD_NUMBER(car(a)), bits);
			return find_node(nodemgr, node_get_type(a),
					 (node_ptr)ws, Nil);
		}
				       ...
				       boolean in_next, hash_ptr expr2bexpr)
{
	...
	if (SymbType_is_word(ta)) {
		node_ptr a = expr2bexpr_recur(enc, det_layer, car(expr),
					      context, in_next, expr2bexpr);
		...
		if (node_get_type(a) == NUMBER_UNSIGNED_WORD ||
			...
			case CAST_BOOL:
				...
				if (WordNumber_get_unsigned_value(wn) != 0) {
					return find_node(nodemgr, TRUEEXP, Nil,
							 Nil);
				} else
					return find_node(nodemgr, FALSEEXP, Nil,
							 Nil);
	...
	{ /* not a leaf, applies the standard encoding */
		node_ptr v = expr2bexpr_recur(enc, det_layer, car(expr),
					      context, in_next, expr2bexpr);
					...
					boolean in_next, hash_ptr expr2bexpr)
{
	...
	node_ptr res =
		NODE_PTR(-1); /* for sure incorrect value to catch bugs */
	...
	if (SymbType_is_word(t_left) || SymbType_is_word(t_right)) {
       ...
       (it is required by below word functions) */
		...
		if ((node_get_type(a) == NUMBER_UNSIGNED_WORD ||
			...
			WordNumber_ptr (*wop)(WordNumberMgr_ptr, WordNumber_ptr,
					      WordNumber_ptr) = NULL;
			...
			case TIMES:
				...
				break;
			case DIVIDE:
				wop = SymbType_is_unsigned_word(t_left) ?
					      WordNumberMgr_unsigned_divide :
					      WordNumberMgr_signed_divide;
				break;
			case MOD:
				wop = SymbType_is_unsigned_word(t_left) ?
					      WordNumberMgr_unsigned_mod :
			...
			if (NULL != bop) {
				...
				return (bop(WORD_NUMBER(car(a)),
					    WORD_NUMBER(car(b)))) ?
					       find_node(nodemgr, TRUEEXP, Nil,
							 Nil) :
					       find_node(nodemgr, FALSEEXP, Nil,
							 Nil);
			}
         ...
         the left operand */
			...
			if (node_get_type(a) == NUMBER_UNSIGNED_WORD ||
			    node_get_type(a) == NUMBER_SIGNED_WORD) {
				a = node_word_create_from_wordnumber(
					WORD_NUMBER(car(a)), env);
			} else if (node_get_type(b) == NUMBER_UNSIGNED_WORD ||
				   node_get_type(b) == NUMBER_SIGNED_WORD) {
				b = node_word_create_from_wordnumber(
					WORD_NUMBER(car(b)), env);
			}
			...
			case LT:
				op = SymbType_is_unsigned_word(t_left) ?
					     node_word_unsigned_less :
					     node_word_signed_less;
				break;
			case LE:
				op = SymbType_is_unsigned_word(t_left) ?
					     node_word_unsigned_less_equal :
					     node_word_signed_less_equal;
				break;
			case GT:
				op = SymbType_is_unsigned_word(t_left) ?
					     node_word_unsigned_greater :
			...
			case DIVIDE:
				op = SymbType_is_unsigned_word(t_left) ?
					     node_word_unsigned_divide :
					     node_word_signed_divide;
				break;
			case MOD:
				op = SymbType_is_unsigned_word(t_left) ?
					     node_word_unsigned_mod :
	...
	if (SymbType_is_boolean(t_left) && SymbType_is_boolean(t_right)) {
		...
		case LT:
			res = ExprMgr_and(
				exprs,
				ExprMgr_not(exprs, expr2bexpr_recur(
							   enc, det_layer, left,
							   context, in_next,
							   expr2bexpr)),
		...
		case AND: {
			node_ptr a = expr2bexpr_recur(enc, det_layer, left,
						      context, in_next,
						      expr2bexpr);
		...
		case MOD:
		case TIMES:
			res = scalar_atom2bexpr(enc, det_layer, expr, context,
						in_next);
			break;
		case UNION: {
			node_ptr tres = scalar_atom2bexpr(enc, det_layer, expr,
							  context, in_next);
			if ((CONS == node_get_type(tres)) &&
			    (SYMB_LAYER(NULL) != det_layer)) {
		...
		default:
         ...
         (parent) expression */
			ErrorMgr_internal_error(
				errmgr,
				"Unexpected operator while booleanizing an expression");
		}
		return res;
	}
			       ...
			       hash_ptr expr2bexpr)
{
	...
	if (SymbType_is_word(TypeChecker_get_expression_type(
		...
		res = expr2bexpr_word_ite_aux(enc, det_layer, expr, context,
					      in_next, expr2bexpr);
	} else {
		c = expr2bexpr_recur(enc, det_layer, car(car(expr)), context,
				     in_next, expr2bexpr);
		if (ExprMgr_is_true(exprs, c)) {
			res = expr2bexpr_recur(enc, det_layer, cdr(car(expr)),
					       context, in_next, expr2bexpr);
		} else if (ExprMgr_is_false(exprs, c)) {
			res = expr2bexpr_recur(enc, det_layer, cdr(expr),
					       context, in_next, expr2bexpr);
		}
					...
					boolean in_next, hash_ptr expr2bexpr)
{
	...
	iter = expr;
	while (true) {
		node_ptr bcond = expr2bexpr_recur(enc, det_layer,
						  car(car(iter)), context,
						  in_next, expr2bexpr);
     ...
     */
		if (!ExprMgr_is_false(exprs, bcond)) {
			node_ptr bthen =
				expr2bexpr_recur(enc, det_layer, cdr(car(iter)),
						 context, in_next, expr2bexpr);
			_CHECK_WORD(bthen);
			...
			if ((node_get_type(bthen) == NUMBER_UNSIGNED_WORD ||
				...
				bthen = node_word_create_from_wordnumber(
					WORD_NUMBER(car(bthen)), env);
			}
			...
			if (size == -1)
				...
				nusmv_assert(node_word_get_width(bthen) ==
					     size);
			}
		...
		if (node_get_type(iter) == FAILURE) {
			...
			failure = expr2bexpr_recur(enc, det_layer, iter,
						   context, in_next,
						   expr2bexpr);
			...
			nusmv_assert(node_word_get_width(failure) ==
				     size); /* of proper size */
			break;
		}
	}
	...
	{ /* creates the encoding */
		...
		int i;
		node_ptr wbits = Nil;
		node_ptr failure_iter =
			failure; /* used if failure is a list of bits */
      ...
      */
			if (FAILURE == node_get_type(failure))
				...
				nusmv_assert(
					CONS ==
					node_get_type(
						failure_iter)); /* a list of bits */
				cases = car(failure_iter);
				failure_iter = cdr(
					failure_iter); /* a bit for next iteration */
			}

			for (iter = bcase; iter != Nil; iter = cdr(iter)) {
				nusmv_assert(node_get_type(car(iter)) == COLON);
				node_ptr bcond = car(car(iter));
				node_ptr biti = array_fetch(
					node_ptr, (array_t *)cdr(car(iter)), i);

				cases = ExprMgr_ite(exprs, bcond, biti, cases,
						    symb_table);
			}
				 ...
				 boolean in_next, hash_ptr expr2bexpr)
{
...

	{
		...
		case NUMBER_EXP:
			ErrorMgr_rpterr(
				errmgr,
				"Real constant cannot be casted to boolean");
			break;
		...
		case TWODOTS: {
			...
			int dim1, dim2, i;
			node_ptr t = Nil;
			node_ptr ndim1 = CompileFlatten_resolve_number(
				st, car(expr), context);
			node_ptr ndim2 = CompileFlatten_resolve_number(
				st, cdr(expr), context);
			...
			if (t == Nil) {
				ErrorMgr_rpterr(errmgr, "empty range: %d..%d",
						dim1, dim2);
			}
			res = t; // res = expr;
			break;
		...
		case SWCONST:
         ...
         rid of SWCONST and UWCONST */
			res = Compile_FlattenSexp(
				BaseEnc_get_symb_table(BASE_ENC(enc)), expr,
				context);
			break;
		...
		case DOT:
		case ATOM:
			res = expr2bexpr_leaf(enc, det_layer, expr, context,
					      in_next, expr2bexpr);
			break;
		...
		case CAST_BOOL: {
         ...
         rid of CAST_BOOL, toint, sizeof */
			node_ptr tmp = Compile_FlattenSexp(
				BaseEnc_get_symb_table(BASE_ENC(enc)), expr,
				context);
			res = expr2bexpr_recur(enc, det_layer, tmp, Nil,
					       in_next, expr2bexpr);
			break;
		}

		case CAST_WORD1:
			res = expr2bexpr_cast_word1(enc, det_layer, expr,
						    context, in_next,
						    expr2bexpr);
			break;

		case CAST_SIGNED:
			res = expr2bexpr_recur(enc, det_layer, car(expr),
					       context, false, expr2bexpr);
			_CHECK_WORD(res);
		...
		case WRESIZE:
			res = expr2bexpr_resize(enc, det_layer, expr, context,
						in_next, expr2bexpr);
			break;

		case EXTEND:
			res = expr2bexpr_extend(enc, det_layer, expr, context,
						in_next, expr2bexpr);
			break;
		...
		case BIT_SELECTION:
			res = expr2bexpr_bitselect(enc, det_layer, expr,
						   context, in_next,
						   expr2bexpr);
			break;

		case EQDEF:
			res = expr2bexpr_eqdef(enc, det_layer, expr, context,
					       in_next, expr2bexpr);
			break;

		case CASE:
		case IFTHENELSE:
			res = expr2bexpr_ite(enc, det_layer, expr, context,
					     in_next, expr2bexpr);
			break;
		...
		case NOT:
		case UMINUS:
			res = expr2bexpr_recur_unary(enc, det_layer, expr,
						     context, in_next,
						     expr2bexpr);
			break;
		...
		case DIVIDE:
		case MOD:
			res = expr2bexpr_recur_binary(enc, det_layer, expr,
						      context, in_next,
						      expr2bexpr);
			break;
		...
		case COMPUTE:
			res = expr2bexpr_recur(enc, det_layer, cdr(car(expr)),
					       car(car(expr)), in_next,
					       expr2bexpr);
			break;

		case CONTEXT:
			res = expr2bexpr_recur(enc, det_layer, cdr(expr),
					       car(expr), in_next, expr2bexpr);
			break;
		...
		if (res != (node_ptr)NULL) {
			expr2bexpr_hash_insert_entry(
				expr2bexpr, nodemgr, expr, context, res,
...
#else
{
...

	{
		...
		const ExprMgr_ptr exprmgr =
			EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
		const SymbTable_ptr st = BaseEnc_get_symb_table(BASE_ENC(enc));
		hash_ptr expr2bexpr = compile_beval_get_handled_hash(
			st, ST_BEVAL_EXPR2BEXPR_HASH);
...
#warning TODO: Consider to use an ST Handled Hash to perform this memoization
		...
		r_left = expr2bexpr_recur(enc, det_layer, left, context,
					  in_next, expr2bexpr);
		r_right = expr2bexpr_recur(enc, det_layer, right, context,
					   in_next, expr2bexpr);
  ...
  printf("======================================================================\n");
	...
	/*!
      ...
      Private service of compile_is_booleanizable.
                           To represent 'true' in cache we use the constant 2
      for 'false' we use 1 to avoid representation problems wrt Nil

      \se cache can be updated
    */
	static boolean compile_is_booleanizable_aux(
		node_ptr expr, node_ptr context,
		const CompileIsBooleanizableAuxParams *params)
{
	...
	if (CONTEXT == node_get_type(expr)) {
		return compile_is_booleanizable_aux(car(expr), cdr(expr),
						    params);
	}
	...
	case ARRAY: {
		ResolveSymbol_ptr rs =
			SymbTable_resolve_symbol(st, expr, context);
		...
		if (ResolveSymbol_is_var(rs)) {
			...
			SymbType_ptr ty = TypeChecker_get_expression_type(
				tc, resolved, Nil);
	...
	default: {
		...
		if (res) {
			...
			res = compile_is_booleanizable_aux(cdr(expr), context,
							   params);
		}
	}
	}
					       ...
					       char *hash_str)
{
	if (!strcmp(ST_BEVAL_EXPR2BEXPR_HASH, hash_str)) {
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_BEVAL_EXPR2BEXPR_HASH, (ST_PFICPCP)NULL,
...
/*!
  \brief Auxiliary function to handle the cast of a boolean expression to a word
  of size 1.
*/
static Expr_ptr expr2bexpr_cast_word1(BddEnc_ptr enc, SymbLayer_ptr det_layer,
				      node_ptr expr, node_ptr context,
				  ...
				  boolean in_next, hash_ptr expr2bexpr)
{
	...
	const NodeMgr_ptr nodemgr =
		NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
	node_ptr delta = CompileFlatten_resolve_number(
		BaseEnc_get_symb_table(BASE_ENC(enc)), cdr(expr), context);
	nusmv_assert(Nil != delta && NUMBER == node_get_type(delta));

	SymbType_ptr ta = TypeChecker_get_expression_type(
		BaseEnc_get_type_checker(BASE_ENC(enc)), car(expr), context);
	nusmv_assert(SymbType_is_word(ta));
	node_ptr a = expr2bexpr_recur(enc, det_layer, car(expr), context,
				      in_next, expr2bexpr);
	if (node_get_type(a) == NUMBER_UNSIGNED_WORD) {
		WordNumber_ptr wn = WORD_NUMBER(car(a));
		...
		int times = node_get_int(delta);
		res = find_node(nodemgr, NUMBER_SIGNED_WORD,
				(node_ptr)WordNumberMgr_signed_extend(words, wn,
								      times),
				Nil);
	} else {
		/* not a constant */
...
/*!
  \brief Auxiliary function to handle the bitselection operation among two
  words.
*/
static Expr_ptr expr2bexpr_bitselect(BddEnc_ptr enc, SymbLayer_ptr det_layer,
				     node_ptr expr, node_ptr context,
				     boolean in_next, hash_ptr expr2bexpr)
{
	...
	const NodeMgr_ptr nodemgr =
		NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
	const SymbTable_ptr st = BaseEnc_get_symb_table(BASE_ENC(enc));
	const SymbType_ptr ta = TypeChecker_get_expression_type(
		BaseEnc_get_type_checker(BASE_ENC(enc)), car(expr), context);
	nusmv_assert(SymbType_is_word(ta));
	...
	res = node_word_selection(a, find_node(nodemgr, COLON, hbit, lbit),
				  env);
				  ...
				  boolean in_next, hash_ptr expr2bexpr)
{
	...
	const NodeMgr_ptr nodemgr =
		NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));
	SymbType_ptr orig_type = TypeChecker_get_expression_type(
		BaseEnc_get_type_checker(BASE_ENC(enc)), car(expr), context);
	node_ptr _size = CompileFlatten_resolve_number(
		BaseEnc_get_symb_table(BASE_ENC(enc)), cdr(expr), context);
	...
	if (n == m) {
		res = expr2bexpr_recur(enc, det_layer, car(expr), context,
				       in_next, expr2bexpr);
	} else if (n < m) {
		if (SymbType_is_signed_word(orig_type)) {
			...
			node_ptr rightmost_sel =
				find_node(nodemgr, COLON,
					  find_node(nodemgr, NUMBER,
						    NODE_FROM_INT(n - 2), Nil),
					  find_node(nodemgr, NUMBER,
						    NODE_FROM_INT(0), Nil));

			node_ptr nexpr =
				find_node(nodemgr, CONCATENATION,
					  find_node(nodemgr, BIT_SELECTION,
						    car(expr), msb_sel),
					  find_node(nodemgr, BIT_SELECTION,
			...
			res = expr2bexpr_recur(enc, det_layer, cnexpr, context,
					       in_next, expr2bexpr);
		} /* signed words */
		else {
			node_ptr rightmost_sel =
			...
			res = expr2bexpr_recur(enc, det_layer, nexpr, context,
					       in_next, expr2bexpr);
		} /* unsigned */
	} else {
				 ...
				 boolean in_next, hash_ptr expr2bexpr)
{
	...
	const TypeChecker_ptr tc = BaseEnc_get_type_checker(BASE_ENC(enc));
	const SymbTable_ptr st = BaseEnc_get_symb_table(BASE_ENC(enc));
	const SymbType_ptr lhs_type =
		TypeChecker_get_expression_type(tc, lhs, context);
	const SymbType_ptr rhs_type =
	...
	if (SymbType_is_boolean(lhs_type) &&
		...
		res = expr2bexpr_recur(enc, det_layer, rhs, context, in_next,
				       expr2bexpr);
		res = ExprMgr_iff(
			EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER)),
			var_name, res);
		...
		if (SMALLINIT == node_get_type(lhs))
			lhs = car(lhs);
		fixed_expr = find_node(
			NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR)), EQDEF,
			lhs, rhs);

		res = expr2bexpr_recur_binary(enc, det_layer, fixed_expr,
					      context, in_next, expr2bexpr);
	}
				...
				boolean in_next, hash_ptr expr2bexpr)
{
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_parameter(rs)) {
			...
			node_ptr param = SymbTable_get_flatten_actual_parameter(
				st, name);
			res = expr2bexpr_recur(enc, det_layer, param, Nil,
					       in_next, expr2bexpr);
			break;
		}
		...
		if (ResolveSymbol_is_define(rs)) {
			...
			res = expr2bexpr_recur(enc, det_layer, body, Nil,
					       in_next, expr2bexpr);
		}
		/* constant */
		else if (ResolveSymbol_is_constant(rs)) {
			...
			if (SymbType_is_boolean(vtype)) {
				const ExprMgr_ptr exprs = EXPR_MGR(
					NuSMVEnv_get_value(env,
							   ENV_EXPR_MANAGER));
				if (in_next)
					res = find_node(nodemgr, NEXT, name,
							Nil);
				else
					res = name;
				...
				_CHECK_WORD(res);
				if (in_next)
					res = node_word_apply_unary(res, NEXT,
								    env);
			} else {
				BoolEnc_ptr benc = BoolEncClient_get_bool_enc(
					BOOL_ENC_CLIENT(enc));
