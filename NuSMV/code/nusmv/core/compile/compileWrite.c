...
#define PRINT_DAG_STATS()                                                      \
  if (opt_get_daggifier_statistics(opts)) {                                    \
    ...
    StreamMgr_print_error(streams, "\n DAG Statistics:\n");                    \
    StreamMgr_print_error(streams, "\tNumber of introduced defines: %d\n",     \
                          st_count(defines));                                  \
...
static boolean compile_write_obfuscated_flatten_vars_aux(
    const NuSMVEnv_ptr env, const CompileWriteData *cwd, const node_ptr name,
    hash_ptr printed);
...
node_ptr Compile_make_dag_info(const NuSMVEnv_ptr env, node_ptr expr,
  ...
  return compile_make_dag_info_aux(
      nodemgr, MasterNormalizer_normalize_node(normalizer, expr), hash);
}
...
void Compile_write_dag_defines(const NuSMVEnv_ptr env, FILE *out,
  ...
  compile_visit_dag_defines(defines, &compile_write_dag_defines_cb, &cb_arg);
}
...
static boolean compile_declare_dag_defines_in_layer_cb(node_ptr define_eqdef,
  ...
  if (count > 0) {
    ...
    SymbLayer_declare_define(layer, car(define_eqdef),
                             Nil /*context is assumed*/, cdr(define_eqdef));
  }
...
{
  compile_visit_dag_defines(defines, &compile_declare_dag_defines_in_layer_cb,
                            layer);
}
...
node_ptr Compile_convert_to_dag(const NuSMVEnv_ptr env,
  ...
  return compile_convert_to_dag_aux(
      ...
      opt_get_daggifier_depth_threshold(opts), opt_traces_hiding_prefix(opts));
}
...
static int compile_write_obfuscated_flat_define(const NuSMVEnv_ptr env,
  ...
  SYMB_LAYER_FOREACH(layer, iter, STT_DEFINE) {
    ...
    compile_write_obfuscated_flat_define_aux(env, cwd, name, printed_arrays);
  }
...
static int compile_write_flat_define_aux(const NuSMVEnv_ptr env,
  ...
  if (is_array_define_element(cwd->symb_table, name)) {
    ...
    compile_write_flat_define_aux(env, cwd, car(name), printed_arrays);
  } else {
    node_ptr fdef;
    ...
    print_node(wffprint, cwd->out,
               Compile_convert_to_dag(env, cwd->symb_table, fdef, cwd->dag_info,
                                      cwd->defines));
    fprintf(cwd->out, ";\n");
  }
  return 1;
...
static int compile_write_obfuscated_flat_define_aux(const NuSMVEnv_ptr env,
  ...
  if (is_array_define_element(cwd->symb_table, name)) {
    ...
    /* get rid of chains of cwd->defines. useful only for normal cwd->defines */
    fdef = CompileFlatten_resolve_define_chains(cwd->symb_table, fdef, Nil);
    fdef = Compile_convert_to_dag(env, cwd->symb_table, fdef, cwd->dag_info,
                                  cwd->defines);
...
static int compile_write_flatten_vars(const NuSMVEnv_ptr env,
  ...
  while (!SymbLayer_iter_is_end(layer, iter)) {
    ...
    if (SymbTable_is_symbol_var(cwd->symb_table, name)) {
      ...
      compile_write_flatten_vars_aux(env, cwd, name, printed_arrays);
    }
...
static int compile_write_flatten_funs(const NuSMVEnv_ptr env,
  ...
  while (!SymbLayer_iter_is_end(layer, iter)) {
    ...
    if (SymbTable_is_symbol_function(cwd->symb_table, name)) {
      ...
      compile_write_flatten_funs_aux(env, cwd, name, printed_arrays);
    }
...
static int compile_write_obfuscated_flatten_vars(const NuSMVEnv_ptr env,
  ...
  SYMB_LAYER_FOREACH(layer, iter, type) {
    ...
    if (SymbTable_is_symbol_var(cwd->symb_table, name)) {
      ...
      /* print the var declaration */
      compile_write_obfuscated_flatten_vars_aux(env, cwd, name, printed_arrays);
    }
...
static int compile_write_obfuscated_flatten_funs(const NuSMVEnv_ptr env,
  ...
  SYMB_LAYER_FOREACH(layer, iter, type) {
    ...
    if (SymbTable_is_symbol_function(cwd->symb_table, name)) {
      ...
      fprintf(cwd->out, "FUN\n");
      /* print the fun declaration */
      compile_write_obfuscated_flatten_funs_aux(env, cwd, name, printed_arrays);
    }
  ...
  if (SymbTable_is_symbol_array_var_element(cwd->symb_table, name)) {
    ...
    if (SymbTable_is_symbol_variable_array(cwd->symb_table, name)) {
      if (find_assoc(printed, name) == Nil) {
        ...
        SymbType_ptr type =
            SymbTable_get_variable_array_type(cwd->symb_table, name);
      ...
      print_node(wffprint, cwd->out, name);
      fprintf(cwd->out, " : ");
      SymbType_print(SymbTable_get_var_type(cwd->symb_table, name), wffprint,
                     cwd->out);
...
static boolean compile_write_obfuscated_flatten_vars_aux(
  ...
  if (SymbTable_is_symbol_array_var_element(cwd->symb_table, name)) {
    /* this identifier is a subpart of array => print the outer array only */
    return compile_write_obfuscated_flatten_vars_aux(env, cwd, car(name),
                                                     printed);
  } else {
    /* this identifier is an array or an individual identifier
       declared with index subscript in its name */
    if (SymbTable_is_symbol_variable_array(cwd->symb_table, name)) {
      ...
      compile_symbtype_obfuscated_print(
          SymbTable_get_var_type(cwd->symb_table, name), cwd);
      fprintf(cwd->out, ";\n"); /* end of the variable output */
    }
  }
...
static int compile_write_flat_asgn(const NuSMVEnv_ptr env,
  ...
  NODE_LIST_FOREACH(vars, iter) {
    ...
    node_ptr init_expr = compile_get_rid_of_define_chain(
        cwd, FlatHierarchy_lookup_assign(cwd->hierarchy, init_name));
    node_ptr next_expr = compile_get_rid_of_define_chain(
        cwd, FlatHierarchy_lookup_assign(cwd->hierarchy, next_name));
...
static void compile_print_assign(const NuSMVEnv_ptr env,
  ...
  fprintf(cwd->out, " := ");
  print_node(wffprint, cwd->out,
             Compile_convert_to_dag(env, cwd->symb_table, rhs, cwd->dag_info,
                                    cwd->defines));
  fprintf(cwd->out, ";\n");
}
...
compile_write_obfuscated_flatten_spec_split(const NuSMVEnv_ptr env,
  ...
  case CONS:
  case AND:
    compile_write_obfuscated_flatten_spec_split(env, cwd, car(n), s);

    compile_write_obfuscated_flatten_spec_split(env, cwd, cdr(n), s);
    break;
...
static int compile_write_flatten_spec(const NuSMVEnv_ptr env,
  ...
  print_node(wffprint, cwd->out,
             Compile_convert_to_dag(env, cwd->symb_table, expr, cwd->dag_info,
                                    cwd->defines));
  fprintf(cwd->out, ";\n\n");
  return 1;
}
...
compile_write_obfuscated_flatten_expr_split(const NuSMVEnv_ptr env,
  ...
  case CONS:
  case AND:
    compile_write_obfuscated_flatten_expr_split(env, cwd, car(n), s);

    compile_write_obfuscated_flatten_expr_split(env, cwd, cdr(n), s);
    break;
...
static int compile_write_obfuscated_flatten_expr(const NuSMVEnv_ptr env,
  ...
  n_anonymous = NodeAnonymizerBase_map_expr(
      cwd->anonymizer, Compile_convert_to_dag(env, cwd->symb_table, n,
                                              cwd->dag_info, cwd->defines));
...
static int compile_write_flatten_psl(const NuSMVEnv_ptr env,
  ...
  default: {
    ...
    expr = car(n);
    name = cdr(n);
    dagged = Compile_convert_to_dag(env, cwd->symb_table, expr, cwd->dag_info,
                                    cwd->defines);
...
static int compile_write_flatten_bfexpr(const NuSMVEnv_ptr env,
  ...
  default: {
    ...
    expr = Compile_convert_to_dag(
        env, cwd->symb_table,
        Compile_expr2bexpr(cwd->enc, cwd->det_layer, expr), cwd->dag_info,
        cwd->defines);
...
static int compile_write_flatten_expr_pair(const NuSMVEnv_ptr env,
  ...
  while (l) {
    ...
    if (node_get_type(n) == CONTEXT) {
      ...
      fn = Compile_convert_to_dag(env, cwd->symb_table, fn, cwd->dag_info,
                                  cwd->defines);
      print_node(wffprint, cwd->out, fn);
    } else
      print_node(wffprint, cwd->out,
                 Compile_convert_to_dag(env, cwd->symb_table, car(n),
                                        cwd->dag_info, cwd->defines));
    ...
    if (node_get_type(n) == CONTEXT) {
      ...
      print_node(wffprint, cwd->out,
                 Compile_convert_to_dag(env, cwd->symb_table, cdr(n),
                                        cwd->dag_info, cwd->defines));
...
static void compile_write_obfuscated_flat_fsm(const NuSMVEnv_ptr env,
  ...
  arrayForEachItem(const char *, cwd->layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      if (SymbLayer_get_input_vars_num(layer) > 0) {
        compile_write_obfuscated_flatten_vars(env, cwd, layer, STT_INPUT_VAR);
      }
    }
  }
  ...
  arrayForEachItem(const char *, cwd->layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      if (SymbLayer_get_state_vars_num(layer) > 0) {
        compile_write_obfuscated_flatten_vars(env, cwd, layer, STT_STATE_VAR);
      }
    }
  }
  ...
  arrayForEachItem(const char *, cwd->layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      if (SymbLayer_get_frozen_vars_num(layer) > 0) {
        compile_write_obfuscated_flatten_vars(env, cwd, layer, STT_FROZEN_VAR);
      }
    }
  }
  ...
  if (compile_write_obfuscated_flatten_expr_split(
          env, cwd, FlatHierarchy_get_init(cwd->hierarchy), "INIT\n")) {
    fprintf(cwd->out, "\n");
  }

  if (compile_write_obfuscated_flatten_expr_split(
          env, cwd, FlatHierarchy_get_invar(cwd->hierarchy), "INVAR\n")) {
    fprintf(cwd->out, "\n");
  }

  if (compile_write_obfuscated_flatten_expr_split(
          env, cwd, FlatHierarchy_get_trans(cwd->hierarchy), "TRANS\n")) {
    fprintf(cwd->out, "\n");
  }
...
static void compile_write_restricted_flat_fsm(const NuSMVEnv_ptr env,
  ...
  arrayForEachItem(const char *, cwd->layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      ...
      SymbLayer_gen_iter(layer, &iter, STT_INPUT_VAR);
      SymbLayer_iter_set_filter(layer, &iter, compile_write_is_var_in_set,
                                restrict_on);
      compile_write_flatten_vars(env, cwd, layer, &iter);
    }
  }
...

  {
    ...
    arrayForEachItem(const char *, cwd->layer_names, i, name) {
      ...
      if (layer != SYMB_LAYER(NULL)) {
        ...
        compile_write_flat_define(env, cwd, restricted_defines);
...
static void compile_write_flat_specs(const NuSMVEnv_ptr env,
  ...
  { /* PSL specifications are not supported at the moment */
    ...
    if (pslspec != Nil) {
      ...
      fprintf(cwd->out, "--- Dumping of PSL properties is not supported by "
...
static void compile_write_obfuscated_flat_specs(const NuSMVEnv_ptr env,
  ...
  compile_write_obfuscated_flat_spec(
      env, cwd, FlatHierarchy_get_compute(cwd->hierarchy), "COMPUTE\n");
  compile_write_obfuscated_flat_spec(
      env, cwd, FlatHierarchy_get_ltlspec(cwd->hierarchy), "LTLSPEC\n");
  compile_write_obfuscated_flat_spec(
      env, cwd, FlatHierarchy_get_invarspec(cwd->hierarchy), "INVARSPEC\n");
...
static void compile_write_bool_fsm(const NuSMVEnv_ptr env,
  ...
  while (!ListIter_is_end(iter)) {
    ...
    fprintf(cwd->out, "-- Input variables from layer '%s'\n",
            SymbLayer_get_name(layer));
    if (SymbLayer_get_input_vars_num(layer) > 0) {
      compile_write_flatten_bool_vars(env, cwd, benc, layer, STT_INPUT_VAR);
    }
    iter = ListIter_get_next(iter);
  }
  ...
  while (!ListIter_is_end(iter)) {
    ...
    fprintf(cwd->out, "-- State variables from layer '%s'\n",
            SymbLayer_get_name(layer));
    if (SymbLayer_get_state_vars_num(layer) > 0) {
      compile_write_flatten_bool_vars(env, cwd, benc, layer, STT_STATE_VAR);
    }
    iter = ListIter_get_next(iter);
  }
  ...
  while (!ListIter_is_end(iter)) {
    ...
    fprintf(cwd->out, "-- Frozen variables from layer '%s'\n",
            SymbLayer_get_name(layer));
    if (SymbLayer_get_frozen_vars_num(layer) > 0) {
      compile_write_flatten_bool_vars(env, cwd, benc, layer, STT_FROZEN_VAR);
    }
    iter = ListIter_get_next(iter);
  }
  ...
  /* CONSTRAINS */
  if (compile_write_flatten_expr_split(
          env, cwd, SexpFsm_get_init(SEXP_FSM(cwd->bool_sexp_fsm)), "INIT\n")) {
    fprintf(cwd->out, "\n");
  }
  ...
  if (compile_write_flatten_expr_pair(
          env, cwd, SexpFsm_get_compassion(SEXP_FSM(cwd->bool_sexp_fsm)),
          "COMPASSION\n")) {
    fprintf(cwd->out, "\n\n");
  }
}
...
static void compile_write_bool_specs(const NuSMVEnv_ptr env,
  ...
  /* dumps the properties */
  compile_write_bool_spec(env, cwd, FlatHierarchy_get_spec(cwd->hierarchy),
                          "CTLSPEC\n");
  ...
  { /* PSL specifications are not supported at the moment */
    ...
    if (pslspec != Nil) {
      /* see issue https://essvn.fbk.eu/bugs/view.php?id=2626 */
      StreamMgr_print_error(streams, "\n********   WARNING   ********\n");
      StreamMgr_print_error(streams, "This version does not support the "
  ...
  if (SymbLayer_get_input_vars_num(cwd->det_layer) > 0) {
    ...
    compile_write_flatten_bool_vars(
        env, cwd, BoolEncClient_get_bool_enc(BOOL_ENC_CLIENT(cwd->enc)),
...
compile_convert_to_dag_aux(const NuSMVEnv_ptr env, const CompileWriteData *cwd,
    ...
    if (info != Nil) {
      ...
      if (admissible &&
        ...
        node_ptr left = compile_convert_to_dag_aux(
            env, cwd, car(expr), num_thres, dep_thres, defines_prefix);
        node_ptr right = compile_convert_to_dag_aux(
            env, cwd, cdr(expr), num_thres, dep_thres, defines_prefix);
  ...
  default: {
    node_ptr left = compile_convert_to_dag_aux(env, cwd, car(expr), num_thres,
                                               dep_thres, defines_prefix);
    node_ptr right = compile_convert_to_dag_aux(env, cwd, cdr(expr), num_thres,
                                                dep_thres, defines_prefix);
...
compile_create_dag_info_from_hierarchy(const CompileWriteData *cwd) {
  ...
  Compile_make_dag_info(env,
                        compile_get_rid_of_define_chain(
                            cwd, FlatHierarchy_get_init(cwd->hierarchy)),
                        dag_info);
  Compile_make_dag_info(env,
                        compile_get_rid_of_define_chain(
                            cwd, FlatHierarchy_get_invar(cwd->hierarchy)),
                        dag_info);
  Compile_make_dag_info(env,
                        compile_get_rid_of_define_chain(
                            cwd, FlatHierarchy_get_trans(cwd->hierarchy)),
                        dag_info);
  Compile_make_dag_info(env,
                        compile_get_rid_of_define_chain(
...

  {
    ...
    SYMB_TABLE_FOREACH(cwd->symb_table, iter, STT_DEFINE) {
      ...
      {
        node_ptr b = compile_get_rid_of_define_chain(cwd, body);
        if (b != body)
          body = b;
      }
...
static void compile_symbtype_obfuscated_print(SymbType_ptr type,
  ...
  if (SymbType_is_enum(type) && (!SymbType_is_boolean(type))) {
    ...
    fprintf(cwd->out, "{");
    while (l != Nil) {
      node_ptr l_anonymous =
          NodeAnonymizerBase_map_expr(cwd->anonymizer, car(l));
    ...
    fprintf(cwd->out, "array %d..%d of ", SymbType_get_array_lower_bound(type),
  ...
  st_foreach_item(cwd->defines, gen, &key, &value) {
    ...
    if (define != Nil) {
      ...
      if (count > 0) {
        ...
        fprintf(cwd->out, "DEFINE ");
        print_node(wffprint, cwd->out,
                   NodeAnonymizerBase_map_expr(cwd->anonymizer, car(define)));
        fprintf(cwd->out, " ;\n\n");
      }
    }
  }
