...
/*!
  ...
  \todo Missing description
*/
#define PRINT_DAG_STATS()                                                     \
	if (opt_get_daggifier_statistics(opts)) {                             \
		int i;                                                        \
		for (i = 0; i < 80; i++)                                      \
		...
		StreamMgr_print_error(streams,                                \
				      ...
				      st_count(defines));                     \
		StreamMgr_print_error(streams, "\tNumber of hits: %d\n",      \
				      GET_DAG_HITS_NUMBER());                 \
		for (i = 0; i < 80; i++)                                      \
			StreamMgr_print_error(streams, "*");                  \
		StreamMgr_print_error(streams, "\n");                         \
...
static int compile_write_obfuscated_flat_define(const NuSMVEnv_ptr env,
						const CompileWriteData *cwd,
						SymbLayer_ptr layer);
static int compile_write_flat_define_aux(const NuSMVEnv_ptr env,
					 const CompileWriteData *cwd,
					 node_ptr name,
					 hash_ptr printed_arrays);
static int compile_write_obfuscated_flat_define_aux(const NuSMVEnv_ptr env,
						    const CompileWriteData *cwd,
						    node_ptr name,
						    hash_ptr printed_arrays);
...
static int compile_write_flatten_vars_aux(const NuSMVEnv_ptr env,
					  ...
					  const node_ptr name,
					  hash_ptr printed);
static boolean compile_write_obfuscated_flatten_vars_aux(
	const NuSMVEnv_ptr env, const CompileWriteData *cwd,
	const node_ptr name, hash_ptr printed);
...
static boolean compile_write_obfuscated_flatten_funs_aux(
	const NuSMVEnv_ptr env, const CompileWriteData *cwd,
	const node_ptr name, hash_ptr printed);
...
static void compile_print_assign(const NuSMVEnv_ptr env,
				 const CompileWriteData *cwd, node_ptr lhs,
				 node_ptr rhs);
...
static int compile_write_flatten_expr(const NuSMVEnv_ptr env,
				      const CompileWriteData *cwd, node_ptr n,
				      const char *s);
...
static int compile_write_flatten_expr_split(const NuSMVEnv_ptr env,
					    const CompileWriteData *cwd,
					    node_ptr n, const char *s);
...
static int compile_write_flatten_spec_split(const NuSMVEnv_ptr env,
					    const CompileWriteData *cwd,
					    node_ptr n, const char *s);
...
static boolean compile_write_is_var_in_set(const SymbLayer_ptr layer,
...
static void compile_write_flat_spec(const NuSMVEnv_ptr env,
				    const CompileWriteData *cwd, node_ptr spec,
				    const char *msg);
...
static node_ptr compile_pack_dag_info(NodeMgr_ptr nodemgr, unsigned int count,
				      unsigned int depth, boolean admissible);

static void compile_unpack_dag_info(node_ptr info, unsigned int *count,
				    unsigned int *depth, boolean *admissible);

static void compile_set_dag_info(node_ptr info, unsigned int count,
				 unsigned int depth, boolean admissible);

static node_ptr
compile_convert_to_dag_aux(const NuSMVEnv_ptr env, const CompileWriteData *cwd,
			   node_ptr expr, unsigned int num_thres,
			   unsigned int dep_thres, const char *defines_prefix);

static node_ptr compile_make_dag_info_aux(NodeMgr_ptr nodemgr, node_ptr expr,
					  hash_ptr hash);
...
static node_ptr __create_define_name(SymbTable_ptr st, const char *prefix,
				     node_ptr body);

static void compile_write_data_init(CompileWriteData *cwd);

static void compile_write_data_deinit(CompileWriteData *cwd);

static void compile_visit_dag_defines(hash_ptr defines,
			       ...
			       boolean force_flattening)
{
	...
	if ((hash_ptr)NULL != cwdata.defines)
		free_assoc(cwdata.defines);
	if ((hash_ptr)NULL != cwdata.dag_info)
		free_assoc(cwdata.dag_info);
	...
	boolean force_flattening, NodeAnonymizerBase_ptr anonymizer)
{
	...
	if (print_map && !NodeAnonymizerBase_is_map_empty(anonymizer)) {
       ...
       for debugging  */
		NodeAnonymizerBase_print_map(
			anonymizer, StreamMgr_get_output_stream(streams));
	}
...
void Compile_WriteFlattenFsm(const NuSMVEnv_ptr env, FILE *out,
			     const SymbTable_ptr st, const array_t *layer_names,
			     const char *fsm_name, FlatHierarchy_ptr hierarchy,
			     boolean force_flattening)
{
	hash_ptr cdh;
	CompileWriteData cwdata;
			       ...
			       hash_ptr hash)
{
	...
	return compile_make_dag_info_aux(
		nodemgr, MasterNormalizer_normalize_node(normalizer, expr),
		hash);
}
			      ...
			      hash_ptr defines)
{
	...
	clear_assoc_and_free_entries_arg(dag_info, compile_free_node,
					 (char *)nodemgr);
	clear_assoc_and_free_entries_arg(defines, compile_free_define,
					 (char *)nodemgr);
}
...
static boolean compile_write_dag_defines_cb(node_ptr define_eqdef,
					    unsigned int count, void *_arg)
{
	if (count > 0) {
		struct WriteDagDefinesCbArg *arg =
			(struct WriteDagDefinesCbArg *)_arg;
...
static boolean compile_declare_dag_defines_in_layer_cb(node_ptr define_eqdef,
						       unsigned int count,
						       void *_arg)
{
	if (count > 0) {
...
{
	compile_visit_dag_defines(
		defines, &compile_declare_dag_defines_in_layer_cb, layer);
}
				...
				hash_ptr dag_hash, hash_ptr defines)
{
	...
	return compile_convert_to_dag_aux(
		env, &cwdata, MasterNormalizer_normalize_node(normalizer, expr),
...
*/
static boolean is_array_define_element(const SymbTable_ptr symb_table,
				       const node_ptr name)
{
  ...
  */
	if (ARRAY == node_get_type(name) &&
		...
		node_ptr body =
			SymbTable_get_array_define_body(symb_table, car(name));
				     ...
				     const NodeList_ptr names)
{
	...
	if (NodeList_get_length(names) == 0)
		return 1;
	...
	while (!ListIter_is_end(iter)) {
		...
		compile_write_flat_define_aux(env, cwd, name, printed_arrays);
						...
						SymbLayer_ptr layer)
{
	...
	if (SymbLayer_get_defines_num(layer) == 0)
		return 1;
	...
	SYMB_LAYER_FOREACH(layer, iter, STT_DEFINE)
	{
		...
		compile_write_obfuscated_flat_define_aux(env, cwd, name,
							 printed_arrays);
	}
...
static int compile_write_flat_define_aux(const NuSMVEnv_ptr env,
					 const CompileWriteData *cwd,
					 node_ptr name, hash_ptr printed_arrays)
{
	const MasterPrinter_ptr wffprint =
		MASTER_PRINTER(NuSMVEnv_get_value(env, ENV_WFF_PRINTER));
	...
	if (is_array_define_element(cwd->symb_table, name)) {
		...
		compile_write_flat_define_aux(env, cwd, car(name),
					      printed_arrays);
	} else {
		node_ptr fdef;
		...
		if (SymbTable_is_symbol_define(cwd->symb_table, name)) {
			...
			ctx = SymbTable_get_define_context(cwd->symb_table,
							   name);
			if (cwd->force_flattening || Nil != ctx) {
				fdef = Compile_FlattenSexp(cwd->symb_table,
							   fdef, ctx);
			}
		} else {
			node_ptr ctx;
			nusmv_assert(SymbTable_is_symbol_array_define(
				cwd->symb_table, name));
			...
			fdef = SymbTable_get_array_define_body(cwd->symb_table,
							       name);
		...
		fdef = CompileFlatten_resolve_define_chains(cwd->symb_table,
							    fdef, Nil);

		print_node(wffprint, cwd->out,
			   Compile_convert_to_dag(env, cwd->symb_table, fdef,
						    ...
						    hash_ptr printed_arrays)
{
	...
	if (is_array_define_element(cwd->symb_table, name)) {
		/* output the outer array but not this identifier */
		compile_write_obfuscated_flat_define_aux(env, cwd, car(name),
							 printed_arrays);
	} else {
		node_ptr fdef;
		...
		name_anonymous =
			NodeAnonymizerBase_map_expr(cwd->anonymizer, name);
		...
		fdef = Compile_convert_to_dag(env, cwd->symb_table, fdef,
					      cwd->dag_info, cwd->defines);
		fdef_anonymous =
			NodeAnonymizerBase_map_expr(cwd->anonymizer, fdef);
				      ...
				      SymbLayerIter *iter)
{
	...
	while (!SymbLayer_iter_is_end(layer, iter)) {
		...
		if (SymbTable_is_symbol_var(cwd->symb_table, name)) {
			if (SymbTable_is_symbol_state_var(cwd->symb_table,
							  name) &&
						 ...
						 const SymbTableType type)
{
	...
	SYMB_LAYER_FOREACH(layer, iter, type)
	{
		...
		if (SymbTable_is_symbol_function(cwd->symb_table, name)) {
			fprintf(cwd->out, "FUN\n");
			/* print the fun declaration */
			compile_write_obfuscated_flatten_funs_aux(
				env, cwd, name, printed_arrays);
		}
					  ...
					  const node_ptr name, hash_ptr printed)
{
	...
	if (SymbTable_is_symbol_array_var_element(cwd->symb_table, name)) {
		...
		if (SymbTable_is_symbol_variable_array(cwd->symb_table, name)) {
			...
			print_node(wffprint, cwd->out, name);
			fprintf(cwd->out, " : ");
			SymbType_print(SymbTable_get_var_type(cwd->symb_table,
							      name),
				       wffprint, cwd->out);
			fprintf(cwd->out,
					  ...
					  const node_ptr name, hash_ptr printed)
{
	...
	if (SymbTable_is_symbol_function(cwd->symb_table, name)) {
		if (find_assoc(printed, name) == Nil) {
			...
			NFunction_ptr function =
				SymbTable_get_function(cwd->symb_table, name);
			ListIter_ptr iter;
			...
			for (iter = NodeList_get_first_iter(args);
			     !ListIter_is_end(iter);) {
				node_ptr arg = NodeList_get_elem_at(args, iter);
				SymbType_print(SYMB_TYPE(arg), wffprint,
					       cwd->out);
				iter = ListIter_get_next(iter);
					  ...
					  const node_ptr name, hash_ptr printed)
{
	...
	if (SymbTable_is_symbol_array_var_element(cwd->symb_table, name)) {
		/* this identifier is a subpart of array => print the outer array only */
		return compile_write_obfuscated_flatten_vars_aux(
			env, cwd, car(name), printed);
	} else {
		/* this identifier is an array or an individual identifier
       declared with index subscript in its name */
		if (SymbTable_is_symbol_variable_array(cwd->symb_table, name)) {
			...
			node_ptr name_anonymous = NodeAnonymizerBase_map_expr(
				cwd->anonymizer, name);
			print_node(wffprint, cwd->out, name_anonymous);
			fprintf(cwd->out, " : ");
			compile_symbtype_obfuscated_print(
				SymbTable_get_var_type(cwd->symb_table, name),
				cwd);
			fprintf(cwd->out,
				";\n"); /* end of the variable output */
		}
				   ...
				   const NodeList_ptr vars)
{
	...
	NODE_LIST_FOREACH(vars, iter)
	{
		...
		node_ptr invar_expr = compile_get_rid_of_define_chain(
			cwd, FlatHierarchy_lookup_assign(cwd->hierarchy, name));
		node_ptr init_expr = compile_get_rid_of_define_chain(
			cwd,
			FlatHierarchy_lookup_assign(cwd->hierarchy, init_name));
		node_ptr next_expr = compile_get_rid_of_define_chain(
			cwd,
		...
		if (init_expr != (node_ptr)NULL)
			compile_print_assign(env, cwd, init_name, init_expr);
		if (invar_expr != (node_ptr)NULL)
			compile_print_assign(env, cwd, name, invar_expr);
		if (next_expr != (node_ptr)NULL)
			compile_print_assign(env, cwd, next_name, next_expr);
					      ...
					      const SymbLayer_ptr layer)
{
	...
	SYMB_LAYER_FOREACH(layer, iter, STT_VAR)
	{
		...
		node_ptr init_name = find_node(nodemgr, SMALLINIT, name, Nil);
		node_ptr next_name = find_node(nodemgr, NEXT, name, Nil);
		node_ptr invar_expr = NodeAnonymizerBase_map_expr(
			cwd->anonymizer,
			compile_get_rid_of_define_chain(
				cwd, FlatHierarchy_lookup_assign(cwd->hierarchy,
								 name)));
		node_ptr init_expr = NodeAnonymizerBase_map_expr(
			cwd->anonymizer,
			compile_get_rid_of_define_chain(
				cwd, FlatHierarchy_lookup_assign(cwd->hierarchy,
								 init_name)));

		node_ptr next_expr = NodeAnonymizerBase_map_expr(
			cwd->anonymizer,
			compile_get_rid_of_define_chain(
				cwd, FlatHierarchy_lookup_assign(cwd->hierarchy,
								 next_name)));
		...
		init_name =
			NodeAnonymizerBase_map_expr(cwd->anonymizer, init_name);
		next_name =
			NodeAnonymizerBase_map_expr(cwd->anonymizer, next_name);
					   ...
					   const SymbTableType type)
{
	...
	SYMB_LAYER_FOREACH(layer, iter, type)
	{
		...
		if (SymbTable_is_symbol_var(cwd->symb_table, name)) {
			if (!SymbTable_is_symbol_bool_var(cwd->symb_table,
							  name)) {
					    ...
					    node_ptr n, const char *s)
{
	...
	case CONS:
	case AND:
		compile_write_obfuscated_flatten_spec_split(env, cwd, car(n),
							    s);

		compile_write_obfuscated_flatten_spec_split(env, cwd, cdr(n),
							    s);
		break;

	default:
		compile_write_obfuscated_flatten_spec(env, cwd, n, s);
	} /* switch */
				      ...
				      const char *s)
{
	...
	if (CONTEXT == node_get_type(expr)) {
		if (cwd->force_flattening || (Nil != car(expr))) {
			expr = Compile_FlattenSexp(cwd->symb_table, cdr(expr),
						   car(expr));
		}
	}
					    ...
					    node_ptr n, const char *s)
{
	...
	case CONS:
	case AND:
		compile_write_obfuscated_flatten_expr_split(env, cwd, car(n),
							    s);

		compile_write_obfuscated_flatten_expr_split(env, cwd, cdr(n),
							    s);
		break;

	default:
		compile_write_obfuscated_flatten_expr(env, cwd, n, s);
	} /* switch */
				      ...
				      const char *s)
{
	...
	if (CONTEXT == node_get_type(n)) {
		if (cwd->force_flattening || (Nil != car(n))) {
			n = Compile_FlattenSexp(cwd->symb_table, cdr(n),
						car(n));
		}
	}
	{
						 ...
						 node_ptr n, const char *s)
{
	...
	n_anonymous = NodeAnonymizerBase_map_expr(
		cwd->anonymizer,
		Compile_convert_to_dag(env, cwd->symb_table, n, cwd->dag_info,
				       cwd->defines));
				     ...
				     const CompileWriteData *cwd, node_ptr n)
{
	...
	default: {
		...
		expr = car(n);
		name = cdr(n);
		dagged = Compile_convert_to_dag(env, cwd->symb_table, expr,
						cwd->dag_info, cwd->defines);
					   ...
					   node_ptr l, const char *s)
{
	...
	while (l) {
		...
		if (node_get_type(n) == CONTEXT) {
			...
			if (cwd->force_flattening || (Nil != car(n))) {
				/* flats the context */
				fn = Compile_FlattenSexp(cwd->symb_table,
							 car(n), Nil);
			}
...
static int compile_write_obfuscated_constants(const CompileWriteData *cwd)
{
	...
	{
		node_ptr name =
			SymbTable_iter_get_symbol(cwd->symb_table, &iter);
		node_ptr name_anonymous = NULL;
		...
		if (node_get_type(name) == ATOM || node_get_type(name) == DOT) {
			...
			name_anonymous = NodeAnonymizerBase_map_expr(
				cwd->anonymizer, name);
			print_node(wffprint, cwd->out, name_anonymous);
		}
	}
					      ...
					      const CompileWriteData *cwd)
{
	...
	fprintf(cwd->out,
		"-- Obfuscated and flattened FSM model generated from %s\n"
		"-- Dumped layers are: ",
	...
	{
		SymbLayer_ptr layer =
			SymbTable_get_layer(cwd->symb_table, name);
	...
	arrayForEachItem(const char *, cwd->layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL)) {
			if (SymbLayer_get_functions_num(layer) > 0) {
				compile_write_obfuscated_flatten_funs(
					env, cwd, layer, STT_FUNCTION);
			}
		}
	}
	...
	arrayForEachItem(const char *, cwd->layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL)) {
			compile_write_obfuscated_flat_define(env, cwd, layer);
		}
	}
	...
	arrayForEachItem(const char *, cwd->layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL)) {
			compile_write_obfuscated_flat_asgn(env, cwd, layer);
		}
	}

	/* CONSTRAINS (except assigns) */
	if (compile_write_obfuscated_flatten_expr_split(
		    env, cwd, FlatHierarchy_get_init(cwd->hierarchy),
		    "INIT\n")) {
		fprintf(cwd->out, "\n");
	}

	if (compile_write_obfuscated_flatten_expr_split(
		    env, cwd, FlatHierarchy_get_invar(cwd->hierarchy),
		    "INVAR\n")) {
		fprintf(cwd->out, "\n");
	}

	if (compile_write_obfuscated_flatten_expr_split(
		    env, cwd, FlatHierarchy_get_trans(cwd->hierarchy),
		    "TRANS\n")) {
		fprintf(cwd->out, "\n");
	}

	{
		...
		while (Nil != fc) {
			if (compile_write_obfuscated_flatten_expr(
				    env, cwd, car(fc),
				    /* For backward compatibility */
				    are_there_compassion ? "JUSTICE\n" :
							   "FAIRNESS\n")) {
				fprintf(cwd->out, "\n");
			}
			fc = cdr(fc);
	...
	if (compile_write_obfuscated_flatten_expr_pair(
		    env, cwd, FlatHierarchy_get_compassion(cwd->hierarchy),
		    "COMPASSION\n")) {
		fprintf(cwd->out, "\n");
	}
	return;
				   ...
				   const CompileWriteData *cwd)
{
	...
	fprintf(cwd->out,
		"-- Flattened FSM model generated from %s\n"
		"-- Dumped layers are: ",
		((char *)NULL != ifile ? ifile : "stdin"));
	...
	arrayForEachItem(const char *, cwd->layer_names, i, name)
	{
		...
		fprintf(cwd->out, "-- Defines from layer '%s'\n", name);
		if (layer != SYMB_LAYER(NULL)) {
			compile_write_flat_define(env, cwd, defines_list);
		}
	...
	/* CONSTRAINS (except assigns) */
	if (compile_write_flatten_expr_split(
		    env, cwd, FlatHierarchy_get_init(cwd->hierarchy),
		    "INIT\n")) {
		fprintf(cwd->out, "\n");
	}
	...
	if (compile_write_flatten_expr_pair(
		    env, cwd, FlatHierarchy_get_compassion(cwd->hierarchy),
		    "COMPASSION\n")) {
		fprintf(cwd->out, "\n");
	}
...

	{
		...
		arrayForEachItem(const char *, cwd->layer_names, i, name)
		{
           ...
           expressions refering to undeclare variables */
				...
				NodeList_ptr restricted_defines =
					NodeList_create();
				SymbLayerIter iter;
				...
				compile_write_flat_define(env, cwd,
							  restricted_defines);
...
{
	if (compile_write_obfuscated_flatten_spec_split(env, cwd, spec, msg)) {
		fprintf(cwd->out, "\n");
	}
}
				     ...
				     const CompileWriteData *cwd)
{
	...
	compile_write_flat_spec(
		env, cwd, FlatHierarchy_get_spec(cwd->hierarchy), "CTLSPEC\n");
	compile_write_flat_spec(env, cwd,
	...
	{ /* PSL specifications are not supported at the moment */
		...
		if (pslspec != Nil) {
			...
			StreamMgr_print_error(
				...
				"However, for user's convenience all the PSL properties will be "
				"dumped\n"
			...
			fprintf(cwd->out,
				"--- Dumping of PSL properties is not supported by "
				"this version of the system.\n"
				"--- However, the PSL properties had been dumped here "
						...
						const CompileWriteData *cwd)
{
	...
	compile_write_obfuscated_flat_spec(
		env, cwd, FlatHierarchy_get_spec(cwd->hierarchy), "CTLSPEC\n");
	compile_write_obfuscated_flat_spec(
		env, cwd, FlatHierarchy_get_compute(cwd->hierarchy),
		"COMPUTE\n");
	compile_write_obfuscated_flat_spec(
		env, cwd, FlatHierarchy_get_ltlspec(cwd->hierarchy),
		"LTLSPEC\n");
	compile_write_obfuscated_flat_spec(
		env, cwd, FlatHierarchy_get_invarspec(cwd->hierarchy),
		"INVARSPEC\n");
				   ...
				   const CompileWriteData *cwd)
{
	...
	while (!ListIter_is_end(iter)) {
		SymbLayer_ptr layer = SYMB_LAYER(
			NodeList_get_elem_at(cwd->nodelist_layer, iter));
	...
	/* while (!ListIter_is_end(iter)) { */
	/*   SymbLayer_ptr layer = SYMB_LAYER(NodeList_get_elem_at(cwd->layers,
   * iter)); */
	/*   NodeList_ptr defines_list; */
   ...
   */
	...
	while (!ListIter_is_end(iter)) {
		...
		fprintf(cwd->out, "-- Input variables from layer '%s'\n",
			SymbLayer_get_name(layer));
		if (SymbLayer_get_input_vars_num(layer) > 0) {
			compile_write_flatten_bool_vars(env, cwd, benc, layer,
							STT_INPUT_VAR);
		}
		iter = ListIter_get_next(iter);
	}
	...
	/* CONSTRAINS */
	if (compile_write_flatten_expr_split(
		    env, cwd, SexpFsm_get_init(SEXP_FSM(cwd->bool_sexp_fsm)),
		    "INIT\n")) {
		fprintf(cwd->out, "\n");
	}

	if (compile_write_flatten_expr_split(
		    env, cwd, SexpFsm_get_invar(SEXP_FSM(cwd->bool_sexp_fsm)),
		    "INVAR\n")) {
		fprintf(cwd->out, "\n");
	}

	if (compile_write_flatten_expr_split(
		    env, cwd, SexpFsm_get_trans(SEXP_FSM(cwd->bool_sexp_fsm)),
		    "TRANS\n")) {
		fprintf(cwd->out, "\n");
	}
	...
	{ /* PSL specifications are not supported at the moment */
		...
		if (pslspec != Nil) {
			...
			StreamMgr_print_error(
				...
				"booleanization of PSL properties.\n"
				"However, for user's convenience all the "
				"PSL properties will be dumped\n"
				"as comments in the output file.\n");
			StreamMgr_print_error(
				streams, "******** END WARNING ********\n\n");
			   ...
			   unsigned int dep_thres, const char *defines_prefix)
{
	...
	if (cwd->defines != (hash_ptr)NULL) {
		...
		if (define != Nil) {
			...
			/* this counter keeps track of really used cwd->defines, for later
         dumping. setcdr can be used here as the node was created with new_node
       */
	...
	if (cwd->dag_info != (hash_ptr)NULL) {
		...
		if (info != Nil) {
			...
			compile_unpack_dag_info(info, &count, &depth,
						&admissible);
			if (admissible && (count >= num_thres ||
					   (count > 1 && depth >= dep_thres))) {
				/* simplifies to a new dag node: continue on children */
				node_ptr name;
				node_ptr left = compile_convert_to_dag_aux(
					env, cwd, car(expr), num_thres,
					dep_thres, defines_prefix);
				node_ptr right = compile_convert_to_dag_aux(
					env, cwd, cdr(expr), num_thres,
					dep_thres, defines_prefix);
				node_ptr body = find_node(nodemgr,
							  node_get_type(expr),
							  left, right);

				name = __create_define_name(
					cwd->symb_table, defines_prefix, body);
...
compile_create_dag_info_from_hierarchy(const CompileWriteData *cwd)
{
	...
	Compile_make_dag_info(
		env,
		compile_get_rid_of_define_chain(
			cwd, FlatHierarchy_get_justice(cwd->hierarchy)),
		dag_info);
	Compile_make_dag_info(
		env,
		compile_get_rid_of_define_chain(
			cwd, FlatHierarchy_get_compassion(cwd->hierarchy)),
		dag_info);
	...
	{ /* learn from ASSIGNs */
		...
		SET_FOREACH(vars, iter)
		{
			...
			Compile_make_dag_info(
				env,
				compile_get_rid_of_define_chain(
					cwd, FlatHierarchy_lookup_assign(
						     cwd->hierarchy, name)),
				dag_info);
			Compile_make_dag_info(
				env,
...

	{
		...
		{
			node_ptr define = SymbTable_iter_get_symbol(
				cwd->symb_table, &iter);
			node_ptr body = SymbTable_get_define_body(
				cwd->symb_table, define);
			node_ptr ctx = SymbTable_get_define_context(
				cwd->symb_table, define);
			if (cwd->force_flattening || (Nil != ctx)) {
				body = Compile_FlattenSexp(cwd->symb_table,
							   body, ctx);
			}

			{
				node_ptr b = compile_get_rid_of_define_chain(
					cwd, body);
				if (b != body)
					body = b;
			}
	...
	for (i = 0; i < sizeof(specs) / sizeof(specs[0]); ++i) {
		...
		if (cwd->det_layer != SYMB_LAYER(NULL)) {
			spec = Compile_expr2bexpr(cwd->enc, cwd->det_layer,
						  specs[i]);
		} else {
			node_ptr ctx;
			...
			if ((Nil != specs[i]) && (Nil != car(specs[i])) &&
				...
				ctx = caar(specs[i]);
				if (cwd->force_flattening || (Nil != ctx)) {
					spec = Compile_FlattenSexp(
						cwd->symb_table, specs[i], Nil);
				} else {
					spec = specs[i];
				}
			} else {
		...
		{
			node_ptr s = compile_get_rid_of_define_chain(cwd, spec);
			if (s != spec)
				spec = s;
		}
					  ...
					  hash_ptr hash)
{
	...
	case NEXT: {
		...
		node_ptr left =
			compile_make_dag_info_aux(nodemgr, car(expr), hash);
		node_ptr right =
			compile_make_dag_info_aux(nodemgr, cdr(expr), hash);
		nusmv_assert(left != Nil ||
			     right != Nil); /* cannot be a leaf */
		if (left != Nil) {
			compile_unpack_dag_info(left, &count, &depth,
						&ladmissible);
		}
		if (right != Nil) {
			unsigned int rdepth;
			compile_unpack_dag_info(right, &count, &rdepth,
						&radmissible);
			depth = MAX(rdepth, depth);
		}

		return compile_pack_dag_info(nodemgr, 1, depth + 1,
					     radmissible && ladmissible);
	}
	...
	default: {
		...
		info = compile_pack_dag_info(nodemgr, 1, depth + 1,
					     ladmissible && radmissible);
				      ...
				      unsigned int depth, boolean admissible)
{
	return new_node(nodemgr, COLON,
			new_node(nodemgr, COLON, PTR_FROM_INT(node_ptr, count),
					      ...
					      const CompileWriteData *cwd)
{
	...
	if (SymbType_is_enum(type) && (!SymbType_is_boolean(type))) {
		...
		fprintf(cwd->out, "{");
		while (l != Nil) {
			node_ptr l_anonymous = NodeAnonymizerBase_map_expr(
				cwd->anonymizer, car(l));
		...
		compile_symbtype_obfuscated_print(
			SymbType_get_array_subtype(type), cwd);
	} else {
		SymbType_print(type, wffprint, cwd->out);
	}
}
						...
						node_ptr expr)
{
	...
	case NFUNCTION: {
		...
		if (ResolveSymbol_is_define(rs)) {
			res = CompileFlatten_resolve_define_chains(
				cwd->symb_table, name, Nil);
...
*/
node_ptr __create_define_name(SymbTable_ptr st, const char *prefix,
			      node_ptr body)
{
