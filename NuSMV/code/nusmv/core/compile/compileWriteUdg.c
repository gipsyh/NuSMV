...
*/
#define EDGE_ATTS \
	"a(\"EDGEPATTERN\",\"single;solid;1;0\"),\
...
#define _PRINT_WS(code, str, style, color)                                  \
	((Nil != find_assoc(visited, code) && (shared)) ?                   \
		 ...
		 (insert_assoc(visited, code, (void *)1),                   \
		  fprintf(out,                                              \
			  NODE_1_UDG color NODE_2_UDG "%s" style NODE_3_UDG \
						      "%s",                 \
			  (void *)code, str, (close ? ",[]))" : ",["))))
...
#define _PRINT_NS(code, str, style, color)                                     \
	(insert_assoc(visited, code, (void *)1),                               \
	 fprintf(out, NODE_1E_UDG color NODE_2_UDG "%s" style NODE_3_UDG "%s", \
		 str, (close ? ",[]))" : ",[")))
...
static void compile_print_assign_udg(const NuSMVEnv_ptr env, SymbTable_ptr st,
...
static int compile_write_udg_flat_define(const NuSMVEnv_ptr env,
					 ...
					 const SymbTableType sym_mask, hash_ptr,
					 hash_ptr);
...
static int compile_write_udg_flat_asgn(const NuSMVEnv_ptr env,
				       ...
				       FlatHierarchy_ptr hierarchy, hash_ptr,
				       hash_ptr);
...
static int compile_write_udg_flatten_bool_vars(const NuSMVEnv_ptr env,
					       const SymbTable_ptr symb_table,
...
static int compile_write_udg_flatten_expr(const NuSMVEnv_ptr env,
					  const SymbTable_ptr symb_table,
					  FILE *out, node_ptr l,
					  ModelSectionTag mst, hash_ptr,
					  hash_ptr);

static int compile_write_udg_flatten_expr_split(
	const NuSMVEnv_ptr env, const SymbTable_ptr symb_table, FILE *out,
	node_ptr n, ModelSectionTag mst, hash_ptr dag_info, hash_ptr defines);
...
static int compile_write_udg_flatten_spec_split(
	const NuSMVEnv_ptr env, const SymbTable_ptr symb_table, FILE *out,
	node_ptr n, ModelSectionTag mst, hash_ptr dag_info, hash_ptr defines);
...
static int compile_write_udg_flatten_bfexpr(const NuSMVEnv_ptr env,
					    BddEnc_ptr enc,
					    const SymbTable_ptr symb_table,
					    SymbLayer_ptr det_layer, FILE *out,
					    node_ptr n, const char *s, hash_ptr,
					    hash_ptr);

static int compile_write_udg_flatten_psl(const NuSMVEnv_ptr env,
					 const SymbTable_ptr symb_table,
					 FILE *out, node_ptr n, hash_ptr,
					 hash_ptr);

static void compile_write_udg_flat_fsm(const NuSMVEnv_ptr env, FILE *out,
				       const SymbTable_ptr symb_table,
...
static void compile_write_udg_bool_fsm(const NuSMVEnv_ptr env, FILE *out,
				       const SymbTable_ptr symb_table,
				       NodeList_ptr layers,
				       const char *fsm_name,
...
static void compile_write_udg_bool_spec(const NuSMVEnv_ptr env, FILE *out,
					BddEnc_ptr enc, node_ptr spec,
					const char *msg,
					SymbLayer_ptr det_layer,
					hash_ptr dag_info, hash_ptr defines);

static void compile_write_udg_bool_specs(const NuSMVEnv_ptr env, FILE *out,
					 BddEnc_ptr enc,
					 SymbLayer_ptr det_layer,
					 FlatHierarchy_ptr hierarchy,
					 hash_ptr dag_info, hash_ptr defines);
...
static hash_ptr compile_create_dag_info_from_hierarchy_udg(
	SymbTable_ptr st, FlatHierarchy_ptr hierarchy, SymbLayer_ptr det_layer,
	BddEnc_ptr enc);

static node_ptr compile_pack_dag_info_udg(NodeMgr_ptr nodemgr,
					  unsigned int count,
					  unsigned int depth);
static void compile_unpack_dag_info_udg(node_ptr info, unsigned int *count,
					unsigned int *depth);

static void compile_set_dag_info_udg(node_ptr info, unsigned int count,
				     unsigned int depth);

static node_ptr
compile_convert_to_dag_aux_udg(const NuSMVEnv_ptr env, SymbTable_ptr symb_table,
			       node_ptr expr, hash_ptr hash,
			       unsigned int num_thres, unsigned int dep_thres,
			       hash_ptr defines, const char *defines_prefix);
...
static int compile_write_flat_array_define_udg(
	const NuSMVEnv_ptr env, const SymbTable_ptr symb_table, FILE *out,
	const SymbLayer_ptr layer, const SymbTableType sym_mask,
	hash_ptr dag_info, hash_ptr defines);

static boolean is_array_define_cell_udg(const SymbTable_ptr st,
					const node_ptr name);

static node_ptr compile_write_udg_flatten_array_define(SymbTable_ptr st,
						       node_ptr body,
						       node_ptr context);

static int compile_write_udg_print_node(const NuSMVEnv_ptr env, FILE *out,
					node_ptr n, boolean close,
					boolean shared, const char *style);

static inline int insert_assoc_w(hash_ptr, node_ptr, node_ptr);

static inline int compile_write_udg_print_2_ary(
	const NuSMVEnv_ptr env, FILE *buffer, node_ptr code, const char *str,
	boolean close, boolean shared, const char *color1, const char *color2);

static inline int compile_write_udg_print_2_arya(const NuSMVEnv_ptr env,
						 FILE *buffer, node_ptr code,
						 const char *str, boolean close,
						 boolean shared);
...
static inline int compile_write_udg_print_3_aryc(const NuSMVEnv_ptr env,
						 FILE *buffer, node_ptr code,
						 const char *str, node_ptr fst,
						 node_ptr snd, node_ptr trd,
						 boolean close, boolean shared);

static inline int compile_write_udg_print_3_aryc_color(
	const NuSMVEnv_ptr env, FILE *buffer, node_ptr code, const char *str,
	node_ptr fst, node_ptr snd, node_ptr trd, boolean close, boolean shared,
...
static void Compile_write_dag_defines_udg(const NuSMVEnv_ptr env, FILE *out,
					  hash_ptr defines);
...
#if DAG_ENABLED
	dag_info = compile_create_dag_info_from_hierarchy_udg(
		st, hierarchy, SYMB_LAYER(NULL), BDD_ENC(NULL));
	defines = new_assoc();
	nusmv_assert(defines != (hash_ptr)NULL);
#endif

	/* dumps the FSM */
	compile_write_udg_flat_fsm(env, out, st, layer_names, fsm_name,
				   hierarchy, dag_info, defines);

	/* dumps the specifications */
	compile_write_udg_flat_specs(env, out, st, hierarchy, dag_info,
				     defines);
...
{
	compile_write_udg_flat_fsm(env, out, st, layer_names, fsm_name,
				   hierarchy, (hash_ptr)NULL, (hash_ptr)NULL);
}
...
void Compile_WriteFlattenSpecs_udg(const NuSMVEnv_ptr env, FILE *out,
				   ...
				   FlatHierarchy_ptr hierarchy)
{
	compile_write_udg_flat_specs(env, out, st, hierarchy, (hash_ptr)NULL,
				     (hash_ptr)NULL);
}
				...
				BoolSexpFsm_ptr bool_sexp_fsm)
{
...
#if DAG_ENABLED
	dag_info = compile_create_dag_info_from_hierarchy_udg(st, fh, det_layer,
							      enc);
	defines = new_assoc();
	nusmv_assert(defines != (hash_ptr)NULL);
#endif
	...
	/* dumps the FSM */
	compile_write_udg_bool_fsm(env, out, st, layers, fsm_name,
				   bool_sexp_fsm, dag_info, defines);

	compile_write_udg_bool_specs(env, out, enc, det_layer, fh, dag_info,
				     defines);
...
void Compile_destroy_dag_info_udg(const NuSMVEnv_ptr env, hash_ptr dag_info,
				  hash_ptr defines)
{
				    ...
				    hash_ptr dag_hash, hash_ptr defines)
{
	...
	return compile_convert_to_dag_aux_udg(
		...
		opt_get_daggifier_depth_threshold(opts), defines,
						       ...
						       node_ptr context)
{
	...
	for (iter = car(body); iter != Nil; iter = cdr(iter)) {
		...
		if (ARRAY_DEF == node_get_type(tmp)) {
			/* process sub-array */
			tmp = compile_write_udg_flatten_array_define(st, tmp,
								     context);
		} else {
			/* process an element */
	...
	SYMB_LAYER_FOREACH(layer, iter, sym_mask)
	{
		...
		if (fdef != Nil) {
			...
			if (!is_array_define_cell_udg(symb_table, name)) {
				fprintf(out, ",");
				compile_write_udg_print_node(env, out, name,
							     false, true, "");
				fprintf(out, EDGE_1_UDG EDGE_ATTS);
				compile_write_udg_print_node(
					env, out,
					Compile_convert_to_dag_udg(
						env, symb_table, fdef, dag_info,
						defines),
					true, true, "");
			...
			StreamMgr_print_error(
				streams,
				"compile_write_udg_flat_define: Flattening failed\n");
		}
	}
	...
	hash_ptr dag_info, hash_ptr defines)
{
	...
	SYMB_LAYER_FOREACH(layer, iter, sym_mask)
	{
		node_ptr name = SymbLayer_iter_get_symbol(layer, &iter);
		node_ptr def =
			SymbTable_get_array_define_body(symb_table, name);
		node_ptr ctx =
			SymbTable_get_array_define_context(symb_table, name);

		node_ptr fdef = compile_write_udg_flatten_array_define(
			symb_table, def, ctx);
		...
		if (fdef != Nil) {
			...
			if (node_get_type(name) != ARRAY) {
				...
				compile_write_udg_print_node(env, out, name,
							     false, true, "");
				fprintf(out, EDGE_1_UDG EDGE_ATTS);
				compile_write_udg_print_node(env, out, fdef,
							     true, true, "");
				fprintf(out, CLOSE_NODE);
			}
		} else {
				       ...
				       hash_ptr dag_info, hash_ptr defines)
{
	...
	SYMB_LAYER_FOREACH(layer, iter, sym_mask)
	{
		...
		if (init_expr != (node_ptr)NULL)
			compile_print_assign_udg(env, symb_table, out,
						 init_name, init_expr, dag_info,
						 defines);
		if (invar_expr != (node_ptr)NULL)
			compile_print_assign_udg(env, symb_table, out, name,
						 invar_expr, dag_info, defines);
		if (next_expr != (node_ptr)NULL)
			compile_print_assign_udg(env, symb_table, out,
						 next_name, next_expr, dag_info,
						 defines);
				     ...
				     hash_ptr dag_info, hash_ptr defines)
{
	...
	compile_write_udg_print_node(
		env, out,
		Compile_convert_to_dag_udg(env, st, rhs, dag_info, defines),
		true, true, "");
	fprintf(out, CLOSE_NODE);
}
	...
	while (!SymbLayer_iter_is_end(layer, &iter)) {
		...
		if (SymbTable_is_symbol_var(symb_table, name)) {
			...
			if (SymbTable_is_symbol_state_var(symb_table, name) &&
				...
				last_insert = WFV_STATE;
			} else if (SymbTable_is_symbol_frozen_var(symb_table,
								  name) &&
				   last_insert != WFV_FROZEN) {
				last_insert = WFV_FROZEN;
			} else if (SymbTable_is_symbol_input_var(symb_table,
								 name) &&
			...
			if ((!printed) && (Nil == find_assoc(visited, name))) {
				printed = true;
				fprintf(out, NODE_1_UDG VAR_COLOR NODE_2_UDG_I,
					(void *)name);
				SymbType_print(SymbTable_get_var_type(
						       symb_table, name),
					       wffprint, out);
		...
		if (printed)
			fprintf(out, (SymbLayer_iter_is_end(layer, &iter)) ?
					     "" :
					     ",");
	} /* loop */
	...
	SYMB_LAYER_FOREACH(layer, iter, sym_mask)
	{
		...
		if (SymbTable_is_symbol_var(symb_table, name)) {
			if (!SymbTable_is_symbol_bool_var(symb_table, name)) {
				...
				if (SymbTable_is_symbol_state_var(symb_table,
								  name) &&
				    last_insert != WFV_STATE) {
					fprintf(out, "VAR\n");
					last_insert = WFV_STATE;
				} else if (SymbTable_is_symbol_frozen_var(
						   symb_table, name) &&
					   last_insert != WFV_FROZEN) {
	...
	node_ptr n, ModelSectionTag mst, hash_ptr dag_info, hash_ptr defines)
{
	...
	case AND:
		compile_write_udg_flatten_spec_split(
			env, symb_table, out, car(n), mst, dag_info, defines);

		compile_write_udg_flatten_spec_split(
			env, symb_table, out, cdr(n), mst, dag_info, defines);
		break;
					  ...
					  hash_ptr dag_info, hash_ptr defines)
{
	...
	nusmv_assert(
		(SPEC == node_get_type(n)) || (LTLSPEC == node_get_type(n)) ||
		(INVARSPEC == node_get_type(n)) ||
		(PSLSPEC == node_get_type(n)) || (COMPUTE == node_get_type(n)));
	...
	print_node(wffprint, out,
		   Compile_convert_to_dag_udg(env, symb_table, expr, dag_info,
					      defines));
	fprintf(out, "\n\n");
	return 1;
}
					    ...
					    hash_ptr dag_info, hash_ptr defines)
{
	...
	case CONS:
	case AND:
		compile_write_udg_flatten_bfexpr(env, enc, symb_table,
						 det_layer, out, car(n), s,
						 dag_info, defines);
		compile_write_udg_flatten_bfexpr(env, enc, symb_table,
						 det_layer, out, cdr(n), s,
						 dag_info, defines);
		break;
	...
	default: {
		...
		expr = Compile_convert_to_dag_udg(
			env, symb_table,
			Compile_expr2bexpr(enc, det_layer, expr), dag_info,
			defines);
					  ...
					  hash_ptr dag_info, hash_ptr defines)
{
	...
	if (n == Nil ||
	    (node_get_type(n) == TRUEEXP /*this chech is optimization*/ &&
	     n == find_node(nodemgr, TRUEEXP, Nil, Nil)))
		return 0;
	...
	if (mst == INVAR_T) {
		compile_write_udg_print_node(
			env, out,
			Compile_convert_to_dag_udg(env, symb_table, n, dag_info,
						   defines),
			true, true, INVAR_COLOR);
	} else {
		compile_write_udg_print_node(
			env, out,
			Compile_convert_to_dag_udg(env, symb_table, n, dag_info,
...
*/
static int compile_write_udg_flatten_expr_pair(
	const NuSMVEnv_ptr env, const SymbTable_ptr symb_table, FILE *out,
	node_ptr l, ModelSectionTag mst, hash_ptr dag_info, hash_ptr defines)
{
	const MasterPrinter_ptr wffprint =
		MASTER_PRINTER(NuSMVEnv_get_value(env, ENV_WFF_PRINTER));
	...
	while (l) {
		...
		if (node_get_type(n) == CONTEXT) {
			/* flats the context */
			node_ptr fn =
				Compile_FlattenSexp(symb_table, car(n), Nil);
			fn = Compile_convert_to_dag_udg(env, symb_table, fn,
							dag_info, defines);
			print_node(wffprint, out, fn);
		} else
			print_node(wffprint, out,
				   Compile_convert_to_dag_udg(env, symb_table,
							      car(n), dag_info,
							      defines));
		...
		if (node_get_type(n) == CONTEXT) {
			...
			node_ptr fn =
				Compile_FlattenSexp(symb_table, cdr(n), Nil);
			fn = Compile_convert_to_dag_udg(env, symb_table, fn,
							dag_info, defines);
			print_node(wffprint, out, fn);
		} else
				       ...
				       FILE *out)
{
	...
	return 1;
	{
		...
		SYMB_TABLE_FOREACH(symb_table, iter, STT_CONSTANT)
		{
			...
			if (node_get_type(name) == ATOM ||
			    node_get_type(name) == DOT) {
				       ...
				       hash_ptr dag_info, hash_ptr defines)
{
	...
	arrayForEachItem(const char *, layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL) &&
		    SymbLayer_get_input_vars_num(layer) > 0) {
			printed = compile_write_udg_flatten_vars(
				symb_table, out, layer, STT_INPUT_VAR);
		}
	}
	...
	arrayForEachItem(const char *, layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL) &&
		    SymbLayer_get_state_vars_num(layer) > 0) {
			compile_write_udg_flatten_vars(symb_table, out, layer,
						       STT_STATE_VAR);
		}
	}
	...
	arrayForEachItem(const char *, layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL)) {
			compile_write_flat_array_define_udg(env, symb_table,
							    out, layer,
							    STT_ARRAY_DEFINE,
							    dag_info, defines);
		}
	...
	/* CONSTANTS */
	if (!opt_backward_comp(opts)) {
		if (compile_write_udg_constants(symb_table, out))
			fprintf(out, "\n");
	}
	...
	arrayForEachItem(const char *, layer_names, i, name)
	{
		...
		if (layer != SYMB_LAYER(NULL)) {
			compile_write_udg_flat_asgn(env, symb_table, out, layer,
						    STT_VAR, hierarchy,
						    dag_info, defines);
		}
	}
	...
	if (compile_write_udg_flatten_expr_split(
		    env, symb_table, out, FlatHierarchy_get_init(hierarchy),
		    INIT_T, dag_info, defines))
		fprintf(out, "\n");

	if (compile_write_udg_flatten_expr_split(
		    env, symb_table, out, FlatHierarchy_get_invar(hierarchy),
		    INVAR_T, dag_info, defines))
		fprintf(out, "\n");

	if (compile_write_udg_flatten_expr_split(
		    env, symb_table, out, FlatHierarchy_get_trans(hierarchy),
...

	{
		...
		while (Nil != fc) {
			if (compile_write_udg_flatten_expr(
				    ...
				    are_there_compassion ? JUSTICE_T :
							   FAIRNESS_T,
				    dag_info, defines)) {
				fprintf(out, "\n");
			}
	...
	if (compile_write_udg_flatten_expr_pair(
		    env, symb_table, out,
		    FlatHierarchy_get_compassion(hierarchy), COMPASSION_T,
		    dag_info, defines)) {
		fprintf(out, "\n");
	}
	return;
					 ...
					 hash_ptr dag_info, hash_ptr defines)
{
	...
	{ /* PSL specifications are not supported at the moment (see issue 2626) */
		...
		if (pslspec != Nil) {
			...
			compile_write_udg_flatten_psl(env, st, out, pslspec,
						      dag_info, defines);
		}
	}
}
				       ...
				       hash_ptr dag_info, hash_ptr defines)
{
	...
	while (!ListIter_is_end(iter)) {
		SymbLayer_ptr layer =
			SYMB_LAYER(NodeList_get_elem_at(layers, iter));
	...
	while (!ListIter_is_end(iter)) {
		...
		if (SymbLayer_get_input_vars_num(layer) > 0) {
			compile_write_udg_flatten_bool_vars(env, symb_table,
							    benc, out, layer,
							    STT_INPUT_VAR);
		}
		iter = ListIter_get_next(iter);
	...
	if (compile_write_udg_flatten_expr_split(
		    env, symb_table, out,
		    SexpFsm_get_init(SEXP_FSM(bool_sexp_fsm)), INIT_T, dag_info,
		    defines)) {
		fprintf(out, "\n");
	}
	...
	if (compile_write_udg_flatten_expr_split(
		    env, symb_table, out,
		    SexpFsm_get_invar(SEXP_FSM(bool_sexp_fsm)), INVAR_T,
		    dag_info, defines)) {
		fprintf(out, "\n");
	}
	...
	if (compile_write_udg_flatten_expr_split(
		    env, symb_table, out,
		    SexpFsm_get_trans(SEXP_FSM(bool_sexp_fsm)), TRANS_T,
		    dag_info, defines)) {
		fprintf(out, "\n");
	}
...
static void compile_write_udg_bool_spec(const NuSMVEnv_ptr env, FILE *out,
					...
					hash_ptr dag_info, hash_ptr defines)
{
	if (compile_write_udg_flatten_bfexpr(
		    env, enc, BaseEnc_get_symb_table(BASE_ENC(enc)), det_layer,
		    out, spec, msg, dag_info, defines))
		fprintf(out, "\n");
}
					 ...
					 hash_ptr dag_info, hash_ptr defines)
{
	...
	if (SymbLayer_get_input_vars_num(det_layer) > 0) {
		fprintf(out,
			"-- Determinization variables of specifications:\n");
		compile_write_udg_flatten_bool_vars(
			env, st,
			BoolEncClient_get_bool_enc(BOOL_ENC_CLIENT(enc)), out,
			det_layer, STT_INPUT_VAR);
	}
}
			       ...
			       hash_ptr defines, const char *defines_prefix)
{
			...
			if (count >= num_thres ||
				...
				{ /* finds a good name for the define */
					...
					char *buf = ALLOC(
						char,
						strlen(form) +
							strlen(defines_prefix) +
							10);
					sprintf(buf, form, defines_prefix,
						++defines_count);
					name = find_node(
						nodemgr, ATOM,
						(node_ptr)UStringMgr_find_string(
							strings, buf),
						Nil);
					FREE(buf);
				}
				name = SymbTable_get_fresh_symbol_name(
					symb_table, "__expr");

				define = new_node(nodemgr, COLON,
						  new_node(nodemgr, EQDEF, name,
							   body),
	...
	default: {
		node_ptr left = compile_convert_to_dag_aux_udg(
			env, symb_table, car(expr), hash, num_thres, dep_thres,
			defines, defines_prefix);
		node_ptr right = compile_convert_to_dag_aux_udg(
			env, symb_table, cdr(expr), hash, num_thres, dep_thres,
			defines, defines_prefix);
	...
	BddEnc_ptr enc)
{
	...
	Compile_make_dag_info(env, FlatHierarchy_get_invar(hierarchy),
			      dag_info);
	Compile_make_dag_info(env, FlatHierarchy_get_trans(hierarchy),
			      dag_info);
	Compile_make_dag_info(env, FlatHierarchy_get_justice(hierarchy),
			      dag_info);
	...
	{ /* learn from ASSIGNs */
		...
		SET_FOREACH(vars, iter)
		{
			...
			node_ptr init_name =
				find_node(nodemgr, SMALLINIT, name, Nil);
			node_ptr next_name =
				find_node(nodemgr, NEXT, name, Nil);
			Compile_make_dag_info(
				env,
				FlatHierarchy_lookup_assign(hierarchy, name),
				dag_info);
			Compile_make_dag_info(env,
					      ...
					      node_ptr expr, hash_ptr hash)
{
	...
	case NEXT: {
		...
		node_ptr left =
			compile_make_dag_info_aux_udg(env, car(expr), hash);
		node_ptr right =
			compile_make_dag_info_aux_udg(env, cdr(expr), hash);
		nusmv_assert(left != Nil ||
			     right != Nil); /* cannot be a leaf */
		if (left != Nil) {
...
static node_ptr compile_pack_dag_info_udg(NodeMgr_ptr nodemgr,
					  ...
					  unsigned int depth)
{
	return new_node(nodemgr, COLON, PTR_FROM_INT(node_ptr, count),
					...
					boolean shared, const char *style)
{
	...
	case FAILURE: {
		...
		return _PRINT(n, "FAILURE", NODE_STYLE_1, CONS_COLOR);
		{
			...
			int chars = snprintf(buf, 20, "\", line %d)",
					     ErrorMgr_failure_get_lineno(errmgr,
									 n));
	...
	case ATTIME:
		error_unreachable_code();
		return _PRINT(n, "@", "", "") && _THROW(cdr(n)) &&
		       _PRINT(n, "{", "", "") && _THROW(car(n)) &&
		       _PRINT(n, "}", "", "");

	case TRUEEXP:
		return _PRINT_NS(n, "T", NODE_STYLE_1, CONS_COLOR);
	case FALSEEXP:
		return _PRINT_NS(n, "F", NODE_STYLE_1, CONS_COLOR);
	case SELF:
		return _PRINT(n, "self", "", "");
	case BOOLEAN:
		return _PRINT(n, "boolean", "", "");

	case ATOM:
		if (!_PRINT_NS(n,

			       UStringMgr_get_string_text((string_ptr)car(n)),
			       NODE_STYLE_1, CONS_COLOR))
			return 0;
		if (cdr(n)) {
			char buf[20];
			int chars =
	...
	case NUMBER_UNSIGNED_WORD:
		return _PRINT(n,
			      WordNumber_to_based_string(
				      WORD_NUMBER(car(n)),
				      get_output_word_format(opts), false),
			      NODE_STYLE_1, CONS_COLOR);
	case NUMBER_SIGNED_WORD:
		return _PRINT(n,
			      WordNumber_to_based_string(
				      WORD_NUMBER(car(n)),
				      get_output_word_format(opts), true),
			      NODE_STYLE_1, CONS_COLOR);
	...
	case UWCONST:
		return _PRINT(n, "uwconst(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ", ", "", "") && _THROW(cdr(n)) &&
		       _PRINT(n, ")", "", "");

	case SWCONST:
		return _PRINT(n, "swconst(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ", ", "", "") && _THROW(cdr(n)) &&
		       _PRINT(n, ")", "", "");

	case WRESIZE:
		return _PRINT(n, "resize(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ", ", "", "") && _THROW(cdr(n)) &&
		       _PRINT(n, ")", "", "");
	case WSIZEOF:
		return _PRINT(n, "sizeof(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ")", "", "");
	case CAST_TOINT:
		return _PRINT(n, "toint(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ")", "", "");
		...
		if (Nil != car(n)) {
			if (FAILURE == node_get_type(cdr(n)))
				...
				compile_write_udg_print_3_aryc_color(
					env, out, n, "case", car(car(n)),
					cdr(car(n)), cdr(n), true, shared,
					INPUT_CASE_COLOR, CONTROL_CASE_COLOR,
					STANDARD_COLOR);
		} else {
			compile_write_udg_print_1_ary(env, out, n, "case", true,
						      shared, style);
		}
		break;
		break;
	...
	case IFTHENELSE:
		compile_write_udg_print_3_aryc_color(
			env, out, n, "ite", car(car(n)), cdr(car(n)), cdr(n),
			true, shared, INPUT_CASE_COLOR, CONTROL_CASE_COLOR,
			STANDARD_COLOR);
		break;
	...
	case BIT_SELECTION: {
		...
		arg = strcat(arg, "]");
		compile_write_udg_print_3_aryc(env, out, n, arg, car(n), NULL,
					       NULL, true, shared);
		return 1;
	}
	case CONSTANTS:
    ...
    */
	case UNSIGNED_WORD: {
		assert(false);
		{
			node_ptr iter = car(n);
			nusmv_assert(CONS ==
				     node_get_type(iter)); /* a list of bits */
	...
	case WAWRITE:
		return _PRINT(n, "WRITE(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ", ", "", "") && _THROW(car(cdr(n))) &&
		       _PRINT(n, ", ", "", "") && _THROW(cdr(cdr(n))) &&
		       _PRINT(n, ")", "", "");

	case WAREAD:
		return _PRINT(n, "READ(", "", "") && _THROW(car(n)) &&
		       _PRINT(n, ", ", "", "") && _THROW(cdr(n)) &&
		       _PRINT(n, ")", "", "");
	...
	case SMALLINIT:
		...
		break;
	case CAST_WORD1:
		compile_write_udg_print_1_ary(env, out, n, "word1", true,
					      shared, style);
		break;

	case CAST_BOOL:
		compile_write_udg_print_1_ary(env, out, n, "bool", true, shared,
					      style);
		break;

	case CAST_SIGNED:
		compile_write_udg_print_1_ary(env, out, n, "signed", true,
					      shared, style);
		break;
	case CAST_UNSIGNED:
	...
	case AND:
		...
		break;
	case EX:
		compile_write_udg_print_1_ary(env, out, n, "EX", true, shared,
					      style);
		break;
	case AX:
		compile_write_udg_print_1_ary(env, out, n, "AX", true, shared,
					      style);
		break;
	case EF:
		compile_write_udg_print_1_ary(env, out, n, "EF", true, shared,
					      style);
		break;
	case AF:
		compile_write_udg_print_1_ary(env, out, n, "AF", true, shared,
					      style);
		break;
	case EG:
		compile_write_udg_print_1_ary(env, out, n, "EG", true, shared,
					      style);
		break;
	case AG:
		compile_write_udg_print_1_ary(env, out, n, "AG", true, shared,
					      style);
		break;
	case OP_NEXT:
		compile_write_udg_print_1_ary(env, out, n, "X", true, shared,
					      style);
		break;
	case OP_PREC:
		compile_write_udg_print_1_ary(env, out, n, "Y", true, shared,
					      style);
		break;
	case OP_NOTPRECNOT:
		compile_write_udg_print_1_ary(env, out, n, "Z", true, shared,
					      style);
		break;
	case OP_GLOBAL:
		compile_write_udg_print_1_ary(env, out, n, "G", true, shared,
					      style);
		break;
	case OP_HISTORICAL:
		compile_write_udg_print_1_ary(env, out, n, "H", true, shared,
					      style);
		break;
	case OP_FUTURE:
		compile_write_udg_print_1_ary(env, out, n, "F", true, shared,
					      style);
		break;
	case OP_ONCE:
		compile_write_udg_print_1_ary(env, out, n, "O", true, shared,
					      style);
		break;
	case UNTIL:
	...
	case EQUAL:
		if ((SMALLINIT == node_get_type(car(n))) ||
			...
			if (Nil != find_assoc(visited, car(n))) {
				compile_write_udg_print_node(
					env, out, car(n), true, false, style);
			} else {
				compile_write_udg_print_node(
					env, out, car(n), false, false, style);
				fprintf(out,
					EDGE_1_UDG EDGE_ATTS_C "%s" CLOSE_EDGE,
					style);
				compile_write_udg_print_node(env, out, cdr(n),
							     true, true, style);
				fprintf(out, CLOSE_NODE);
			}
		} else if ((SMALLINIT == node_get_type(cdr(n))) ||
			   (NEXT == node_get_type(cdr(n)))) {
			if (Nil != find_assoc(visited, cdr(n))) {
				compile_write_udg_print_node(
					env, out, cdr(n), true, false, style);
			} else {
				compile_write_udg_print_node(
					env, out, cdr(n), false, false, style);
				fprintf(out,
					EDGE_1_UDG EDGE_ATTS_C "%s" CLOSE_EDGE,
					style);
				compile_write_udg_print_node(env, out, cdr(n),
			...
			compile_write_udg_print_2_ary(env, out, n, "=", true,
						      shared, style, style);
		break;
	case NOTEQUAL:
		compile_write_udg_print_2_ary(env, out, n, "!=", true, shared,
	...
	case SETIN:
		...
		break;
	case UNION:
		compile_write_udg_print_2_ary(env, out, n, "union", true,
					      shared, style, style);
		break;
	case LSHIFT:
		compile_write_udg_print_2_ary(env, out, n, "<<", true, shared,
	...
	case PLUS:
		if (cdr(n) == (node_ptr)NULL) { /* checks if unary */
			compile_write_udg_print_1_ary(env, out, n, "+", true,
						      shared, style);
		} else {
			compile_write_udg_print_2_ary(env, out, n, "+", true,
						      shared, style, style);
	...
	case UMINUS:
		compile_write_udg_print_1_ary(env, out, n, "-", true, shared,
					      style);
		break;

	case FLOOR:
		compile_write_udg_print_1_ary(env, out, n, "floor", true,
					      shared, style);
		break;
	...
	case CONCATENATION:
		...
		break;
	case NOT:
		compile_write_udg_print_1_ary(env, out, n, "not", true, shared,
					      style);
		break;
	case VAR:
		break;
	...
	case EQDEF:
		if (Nil != find_assoc(visited, car(n))) {
			compile_write_udg_print_node(env, out, car(n), true,
						     false, style);
		} else {
			compile_write_udg_print_node(env, out, car(n), false,
						     false, style);
			fprintf(out, EDGE_1_UDG EDGE_ATTS);
			compile_write_udg_print_node(env, out, cdr(n), true,
						     true, style);
			fprintf(out, CLOSE_NODE);
		}
		break;
...
static inline int compile_write_udg_print_2_ary(
	...
	boolean close, boolean shared, const char *color1, const char *color2)
{
	return compile_write_udg_print_3_aryc_color(env, buffer, code, str,
						    car(code), cdr(code), NULL,
						    close, shared, color1,
						    color2, color2);
}
						 ...
						 boolean shared)
{
	if (Nil != find_assoc(visited, code)) {
		...
		fprintf(buffer,
			NODE_1_UDG NODE_2_UDG "%s" NODE_3_UDG
					      ",[" EDGE_1_UDG EDGE_ATTS,
			(void *)code, str);
		compile_write_udg_print_node(env, buffer, car(code), close,
					     shared, "");
		fprintf(buffer, ")," EDGE_1_UDG EDGE_ATTS_C CONTROL_CASE_COLOR
					CLOSE_EDGE);
		compile_write_udg_print_node(env, buffer, cdr(code), close,
					     shared, "");
...
static inline int compile_write_udg_print_3_aryc(const NuSMVEnv_ptr env,
						 ...
						 boolean close, boolean shared)
{
	return compile_write_udg_print_3_aryc_color(env, buffer, code, str, fst,
						    snd, trd, close, shared, "",
						    "", "");
}
	...
	const char *color1, const char *color2, const char *color3)
{
	...
	if (!isvisited) {
		if (shared) {
			fprintf(buffer,
				NODE_1_UDG NODE_2_UDG
				"%s" NODE_3_UDG ",[" EDGE_1_UDG EDGE_ATTS_C
				"%s" CLOSE_EDGE,
				(void *)code, str,
				(strcmp(color1, "") ? color1 : STANDARD_COLOR));
		} else {
			fprintf(buffer,
				NODE_1E_UDG NODE_2_UDG
				"%s" NODE_3_UDG ",[" EDGE_1_UDG EDGE_ATTS_C
				"%s" CLOSE_EDGE,
				str,
				(strcmp(color1, "") ? color1 : STANDARD_COLOR));
		}
		compile_write_udg_print_node(env, buffer, fst, close, shared,
					     color3);
		if (NULL != snd) {
			fprintf(buffer,
				")," EDGE_1_UDG EDGE_ATTS_C "%s" CLOSE_EDGE,
				(strcmp(color2, "") ? color2 : STANDARD_COLOR));
			compile_write_udg_print_node(env, buffer, snd, close,
						     shared, color3);
			if (NULL != trd) {
				fprintf(buffer,
					")," EDGE_1_UDG EDGE_ATTS_C
					"%s" CLOSE_EDGE,
					(strcmp(color3, "") ? color3 :
							      STANDARD_COLOR));
				compile_write_udg_print_node(env, buffer, trd,
							     close, shared,
							     color3);
			}
		}
		...
		if (shared) {
			...
			fprintf(buffer,
				...
				str,
				(strcmp(color1, "") ? color1 : STANDARD_COLOR));
			compile_write_udg_print_node(env, buffer, fst, close,
						     shared, color3);
			if (NULL != snd) {
				fprintf(buffer,
					")," EDGE_1_UDG EDGE_ATTS_C
					"%s" CLOSE_EDGE,
					(strcmp(color2, "") ? color2 :
							      STANDARD_COLOR));
				compile_write_udg_print_node(env, buffer, snd,
							     close, shared,
							     color3);
				if (NULL != trd) {
					fprintf(buffer,
...
static void compile_write_print_node_type(FILE *buffer, node_ptr node)
{
	...
	case TRANS:
		fprintf(buffer, "TRANS");
		break; /* 101 */
	case INIT:
		fprintf(buffer, "INIT");
		break;
	case INVAR:
		fprintf(buffer, "INVAR");
		break;
	case ASSIGN:
		fprintf(buffer, "ASSIGN");
		break;
	case FAIRNESS:
		fprintf(buffer, "FAIRNESS");
		break;
	case JUSTICE:
		fprintf(buffer, "JUSTICE");
		break;
	case COMPASSION:
		fprintf(buffer, "COMPASSION");
		break;
	case SPEC:
		fprintf(buffer, "SPEC");
		break;
	case LTLSPEC:
		fprintf(buffer, "LTLSPEC");
		break;
	case PSLSPEC:
		fprintf(buffer, "PSLSPEC");
		break; /* 110 */
	case INVARSPEC:
		fprintf(buffer, "INVARSPEC");
		break;
	case COMPUTE:
		fprintf(buffer, "COMPUTE");
		break;
	case DEFINE:
		fprintf(buffer, "DEFINE");
		break;
	case ISA:
		fprintf(buffer, "ISA");
		break;
	case CONSTRAINT:
		fprintf(buffer, "CONSTRAINT");
		break;
	case MODULE:
		fprintf(buffer, "MODULE");
		break;
	case PROCESS:
		fprintf(buffer, "PROCESS");
		break;
	case MODTYPE:
		fprintf(buffer, "MODTYPE");
		break;
	case LAMBDA:
		fprintf(buffer, "LAMBDA");
		break; /* 120 */
	case CONSTANTS:
		fprintf(buffer, "CONSTANTS");
		break;
	case PRED:
		fprintf(buffer, "PRED");
		break;
	case ATTIME:
		fprintf(buffer, "ATTIME");
		break;
	case PREDS_LIST:
		fprintf(buffer, "PREDS_LIST");
		break;
	case MIRROR:
		fprintf(buffer, "MIRROR");
		break;
	case NUSMV_STATEMENTS_SYMBOL_LAST:
		fprintf(buffer, "NUSMV_STATEMENTS_SYMBOL_LAST");
		break;
	case NUSMV_EXPR_SYMBOL_FIRST:
		fprintf(buffer, "NUSMV_EXPR_SYMBOL_FIRST");
		break;
	case FAILURE:
		fprintf(buffer, "FAILURE");
		break;
	case CONTEXT:
		fprintf(buffer, "CONTEXT");
		break;
	case EU:
		fprintf(buffer, "EU");
		break;
	...
	case VAR:
		...
		break;
	case FROZENVAR:
		fprintf(buffer, "FROZENVAR");
		break;
	case IVAR:
		fprintf(buffer, "IVAR");
		break;
	case BOOLEAN:
		fprintf(buffer, "BOOLEAN");
		break;
	case ARRAY:
		fprintf(buffer, "ARRAY");
		break; /* 140 */
	case OF:
		fprintf(buffer, "OF");
		break;
	case SCALAR:
		fprintf(buffer, "SCALAR");
		break;
	case CONS:
		fprintf(buffer, "CONS");
		break;
	...
	case RCB:
		...
		break;
	case EQDEF:
		fprintf(buffer, "EQDEF");
		break;
	case TWODOTS:
		fprintf(buffer, "TWODOTS");
		break;
	case FALSEEXP:
		fprintf(buffer, "FALSEEXP");
		break;
	case TRUEEXP:
		fprintf(buffer, "TRUEEXP");
		break;
	case SELF:
		fprintf(buffer, "SELF");
		break;
	...
	case COLON:
		...
		break;
	case IFTHENELSE:
		fprintf(buffer, "IFTHENELSE");
		break; /* 160 */
	case INCONTEXT:
		fprintf(buffer, "INCONTEXT");
		break;
	case SIMPWFF:
		fprintf(buffer, "SIMPWFF");
		break;
	case LTLWFF:
		fprintf(buffer, "LTLWFF");
		break;
	case CTLWFF:
		fprintf(buffer, "CTLWFF");
		break;
	case COMPWFF:
		fprintf(buffer, "COMPWFF");
		break;
	case ATOM:
		fprintf(buffer, "ATOM");
		break;
	case NUMBER:
		fprintf(buffer, "NUMBER");
		break;
	case COMMA:
		fprintf(buffer, "COMMA");
		break;
	case IMPLIES:
		fprintf(buffer, "IMPLIES");
		break;
	case IFF:
		fprintf(buffer, "IFF");
		break;
	...
	case EE:
		...
		break;
	case SINCE:
		fprintf(buffer, "SINCE");
		break;
	case UNTIL:
		fprintf(buffer, "UNTIL");
		break;
	case TRIGGERED:
		fprintf(buffer, "TRIGGERED");
		break;
	case RELEASES:
		fprintf(buffer, "RELEASES");
		break;
	case EBF:
		fprintf(buffer, "EBF");
		break;
	...
	case MMAX:
		...
		break;
	case OP_NEXT:
		fprintf(buffer, "OP_NEXT");
		break;
	case OP_GLOBAL:
		fprintf(buffer, "OP_GLOBAL");
		break;
	case OP_FUTURE:
		fprintf(buffer, "OP_FUTURE");
		break;
	case OP_PREC:
		fprintf(buffer, "OP_PREC");
		break;
	case OP_NOTPRECNOT:
		fprintf(buffer, "OP_NOTPRECNOT");
		break;
	case OP_HISTORICAL:
		fprintf(buffer, "OP_HISTORICAL");
		break; /* 200 */
	case OP_ONCE:
		fprintf(buffer, "OP_ONCE");
		break;
	case EQUAL:
		fprintf(buffer, "EQUAL");
		break;
	case NOTEQUAL:
		fprintf(buffer, "NOTEQUAL");
		break;
	case LT:
		fprintf(buffer, "LT");
		break;
	...
	case GE:
		...
		break;
	case UNION:
		fprintf(buffer, "UNION");
		break;
	case SETIN:
		fprintf(buffer, "SETIN");
		break;
	case MOD:
		fprintf(buffer, "MOD");
		break;
	case PLUS:
		...
		break;
	case MINUS:
		fprintf(buffer, "MINUS");
		break;
	case TIMES:
		fprintf(buffer, "TIMES");
		break;
	case DIVIDE:
		fprintf(buffer, "DIVIDE");
		break;
	case UMINUS:
		fprintf(buffer, "UMINUS");
		break;
	case NEXT:
		fprintf(buffer, "NEXT");
		break;
	case SMALLINIT:
		fprintf(buffer, "SMALLINIT");
		break;
	case DOT:
		fprintf(buffer, "DOT");
		break;
	case BIT:
		...
		break;
	case RANGE:
		fprintf(buffer, "RANGE");
		break; /* 220 */
	case UNSIGNED_WORD:
		fprintf(buffer, "UNSIGNED_WORD");
		break;
	case SIGNED_WORD:
		fprintf(buffer, "SIGNED_WORD");
		break;
	case INTEGER:
		fprintf(buffer, "INTEGER");
		break;
	case REAL:
		fprintf(buffer, "REAL");
		break;
	case NUMBER_UNSIGNED_WORD:
		fprintf(buffer, "NUMBER_UNSIGNED_WORD");
		break;
	case NUMBER_SIGNED_WORD:
		fprintf(buffer, "NUMBER_SIGNED_WORD");
		break;
	case NUMBER_FRAC:
		fprintf(buffer, "NUMBER_FRAC");
		break;
	case NUMBER_REAL:
		fprintf(buffer, "NUMBER_REAL");
		break;
	case NUMBER_EXP:
		fprintf(buffer, "NUMBER_EXP");
		break;
	case LSHIFT:
		fprintf(buffer, "LSHIFT");
		break; /* 230 */
	case RSHIFT:
		fprintf(buffer, "RSHIFT");
		break;
	case LROTATE:
		fprintf(buffer, "LROTATE");
		break;
	case RROTATE:
		fprintf(buffer, "RROTATE");
		break;
	case BIT_SELECTION:
		fprintf(buffer, "BIT_SELECTION");
		break;
	case CONCATENATION:
		fprintf(buffer, "CONCATENATION");
		break;
	case CAST_BOOL:
		fprintf(buffer, "CAST_BOOL");
		break;
	case CAST_WORD1:
		fprintf(buffer, "CAST_WORD1");
		break;
	case CAST_SIGNED:
		fprintf(buffer, "CAST_SIGNED");
		break;
	case CAST_UNSIGNED:
		fprintf(buffer, "CAST_UNSIGNED");
		break;
	case EXTEND:
		fprintf(buffer, "EXTEND");
		break;
	case WORDARRAY:
		fprintf(buffer, "WORDARRAY");
		break;
	case WAREAD:
		fprintf(buffer, "WAREAD");
		break;
	case WAWRITE:
		fprintf(buffer, "WAWRITE");
		break;
	case UWCONST:
		fprintf(buffer, "UWCONST");
		break;
	case SWCONST:
		fprintf(buffer, "SWCONST");
		break;
	case WRESIZE:
		fprintf(buffer, "WRESIZE");
		break;
	case WSIZEOF:
		fprintf(buffer, "WSIZEOF");
		break;
	case CAST_TOINT:
		fprintf(buffer, "CAST_TOINT");
		break;
	case COMPID:
		fprintf(buffer, "COMPID");
		break;
	case ARRAY_TYPE:
		fprintf(buffer, "ARRAY_TYPE");
		break;
	case ARRAY_DEF:
		fprintf(buffer, "ARRAY_DEF");
		break;
	case NUSMV_EXPR_SYMBOL_LAST:
		fprintf(buffer, "NUSMV_EXPR_SYMBOL_LAST");
		break;
	case NUSMV_CORE_SYMBOL_LAST:
		fprintf(buffer, "NUSMV_CORE_SYMBOL_LAST");
		break;
	}
	fprintf(buffer, "- ");
}
