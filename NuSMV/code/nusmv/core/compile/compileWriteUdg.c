...
#define _PRINT_WS(code, str, style, color)                                     \
  ((Nil != find_assoc(visited, code) && (shared))                              \
       ...
       : (insert_assoc(visited, code, (void *)1),                              \
          fprintf(out, NODE_1_UDG color NODE_2_UDG "%s" style NODE_3_UDG "%s", \
                  (void *)code, str, (close ? ",[]))" : ",["))))
...
#define _PRINT_NS(code, str, style, color)                                     \
  (insert_assoc(visited, code, (void *)1),                                     \
   fprintf(out, NODE_1E_UDG color NODE_2_UDG "%s" style NODE_3_UDG "%s", str,  \
           (close ? ",[]))" : ",[")))
...
static int compile_write_flat_array_define_udg(
    const NuSMVEnv_ptr env, const SymbTable_ptr symb_table, FILE *out,
    const SymbLayer_ptr layer, const SymbTableType sym_mask, hash_ptr dag_info,
    hash_ptr defines);
...
void Compile_WriteBoolModel_udg(const NuSMVEnv_ptr env, FILE *out,
  ...
  compile_write_udg_bool_specs(env, out, enc, det_layer, fh, dag_info, defines);
...
node_ptr Compile_convert_to_dag_udg(const NuSMVEnv_ptr env,
  ...
  return compile_convert_to_dag_aux_udg(
      env, symb_table, MasterNormalizer_normalize_node(normalizer, expr),
      dag_hash, opt_get_daggifier_counter_threshold(opts),
...
static node_ptr compile_write_udg_flatten_array_define(SymbTable_ptr st,
  ...
  for (iter = car(body); iter != Nil; iter = cdr(iter)) {
    ...
    if (ARRAY_DEF == node_get_type(tmp)) {
      /* process sub-array */
      tmp = compile_write_udg_flatten_array_define(st, tmp, context);
    } else {
      /* process an element */
...
static int compile_write_flat_array_define_udg(
  ...
  SYMB_LAYER_FOREACH(layer, iter, sym_mask) {
    ...
    node_ptr fdef =
        compile_write_udg_flatten_array_define(symb_table, def, ctx);
...
static int compile_write_udg_flat_asgn(const NuSMVEnv_ptr env,
  ...
  SYMB_LAYER_FOREACH(layer, iter, sym_mask) {
    ...
    if ((init_expr != (node_ptr)NULL) || (next_expr != (node_ptr)NULL) ||
        (invar_expr != (node_ptr)NULL)) {
    }

    if (init_expr != (node_ptr)NULL)
      compile_print_assign_udg(env, symb_table, out, init_name, init_expr,
                               dag_info, defines);
    if (invar_expr != (node_ptr)NULL)
      compile_print_assign_udg(env, symb_table, out, name, invar_expr, dag_info,
                               defines);
    if (next_expr != (node_ptr)NULL)
      compile_print_assign_udg(env, symb_table, out, next_name, next_expr,
                               dag_info, defines);
...
static void compile_print_assign_udg(const NuSMVEnv_ptr env, SymbTable_ptr st,
  ...
  compile_write_udg_print_node(
      env, out, Compile_convert_to_dag_udg(env, st, rhs, dag_info, defines),
      true, true, "");
  fprintf(out, CLOSE_NODE);
}
  ...
  while (!SymbLayer_iter_is_end(layer, &iter)) {
    ...
    if (SymbTable_is_symbol_var(symb_table, name)) {
      ...
      if ((!printed) && (Nil == find_assoc(visited, name))) {
        ...
        SymbType_print(SymbTable_get_var_type(symb_table, name), wffprint, out);
...
static int compile_write_udg_flatten_spec_split(
  ...
  default:
    compile_write_udg_flatten_spec(env, symb_table, out, n, mst, dag_info,
                                   defines);
  } /* switch */
...
static int compile_write_udg_flatten_spec(const NuSMVEnv_ptr env,
  ...
  nusmv_assert((SPEC == node_get_type(n)) || (LTLSPEC == node_get_type(n)) ||
  ...
  print_node(
      wffprint, out,
      Compile_convert_to_dag_udg(env, symb_table, expr, dag_info, defines));
  fprintf(out, "\n\n");
  return 1;
}
...
static int compile_write_udg_flatten_psl(const NuSMVEnv_ptr env,
  ...
  case CONS:
  case AND:
    compile_write_udg_flatten_psl(env, symb_table, out, car(n), dag_info,
                                  defines);
    compile_write_udg_flatten_psl(env, symb_table, out, cdr(n), dag_info,
                                  defines);
    break;
...
*/
static int compile_write_udg_flatten_bfexpr(
    const NuSMVEnv_ptr env, BddEnc_ptr enc, const SymbTable_ptr symb_table,
    SymbLayer_ptr det_layer, FILE *out, node_ptr n, const char *s,
    hash_ptr dag_info, hash_ptr defines) {
  const ExprMgr_ptr exprs = EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
  ...
  case CONS:
  case AND:
    compile_write_udg_flatten_bfexpr(env, enc, symb_table, det_layer, out,
                                     car(n), s, dag_info, defines);
    compile_write_udg_flatten_bfexpr(env, enc, symb_table, det_layer, out,
                                     cdr(n), s, dag_info, defines);
    break;
  ...
  default: {
    ...
    /* Support for property names */
    nusmv_assert(SPEC == node_get_type(n) || LTLSPEC == node_get_type(n) ||
                 INVARSPEC == node_get_type(n) || PSLSPEC == node_get_type(n) ||
    ...
    expr = Compile_convert_to_dag_udg(env, symb_table,
...
static int compile_write_udg_flatten_expr_split(
  ...
  default:
    ...
    fprintf(out, ",");
    compile_write_udg_flatten_expr(env, symb_table, out, n, mst, dag_info,
                                   defines);
  } /* switch */
...
static int compile_write_udg_flatten_expr(const NuSMVEnv_ptr env,
  ...
  if (mst == INVAR_T) {
    compile_write_udg_print_node(
        env, out,
        Compile_convert_to_dag_udg(env, symb_table, n, dag_info, defines), true,
        true, INVAR_COLOR);
  } else {
    compile_write_udg_print_node(
        env, out,
  ...
  while (l) {
    ...
    if (node_get_type(n) == CONTEXT) {
      ...
      print_node(wffprint, out,
                 Compile_convert_to_dag_udg(env, symb_table, car(n), dag_info,
                                            defines));
    ...
    if (node_get_type(n) == CONTEXT) {
      ...
      print_node(wffprint, out,
                 Compile_convert_to_dag_udg(env, symb_table, cdr(n), dag_info,
                                            defines));
...
static void compile_write_udg_flat_fsm(const NuSMVEnv_ptr env, FILE *out,
  ...
  arrayForEachItem(const char *, layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL) && SymbLayer_get_input_vars_num(layer) > 0) {
      printed =
          compile_write_udg_flatten_vars(symb_table, out, layer, STT_INPUT_VAR);
    }
  }
  ...
  arrayForEachItem(const char *, layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      compile_write_udg_flat_define(env, symb_table, out, layer, STT_DEFINE,
                                    dag_info, defines);
    }
  }
  ...
  arrayForEachItem(const char *, layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      compile_write_flat_array_define_udg(env, symb_table, out, layer,
                                          STT_ARRAY_DEFINE, dag_info, defines);
    }
  }
  ...
  arrayForEachItem(const char *, layer_names, i, name) {
    ...
    if (layer != SYMB_LAYER(NULL)) {
      compile_write_udg_flat_asgn(env, symb_table, out, layer, STT_VAR,
...

  {
    ...
    while (Nil != fc) {
      if (compile_write_udg_flatten_expr(env, symb_table, out, car(fc),
  ...
  if (compile_write_udg_flatten_expr_pair(
          env, symb_table, out, FlatHierarchy_get_compassion(hierarchy),
...
static void compile_write_udg_bool_fsm(const NuSMVEnv_ptr env, FILE *out,
  ...
  while (!ListIter_is_end(iter)) {
    ...
    if (SymbLayer_get_input_vars_num(layer) > 0) {
      compile_write_udg_flatten_bool_vars(env, symb_table, benc, out, layer,
                                          STT_INPUT_VAR);
    }
    iter = ListIter_get_next(iter);
  }
  ...
  if (compile_write_udg_flatten_expr_split(
          env, symb_table, out, SexpFsm_get_init(SEXP_FSM(bool_sexp_fsm)),
          INIT_T, dag_info, defines)) {
    fprintf(out, "\n");
  }

  if (compile_write_udg_flatten_expr_split(
          env, symb_table, out, SexpFsm_get_invar(SEXP_FSM(bool_sexp_fsm)),
          INVAR_T, dag_info, defines)) {
    fprintf(out, "\n");
  }

  if (compile_write_udg_flatten_expr_split(
          env, symb_table, out, SexpFsm_get_trans(SEXP_FSM(bool_sexp_fsm)),
          TRANS_T, dag_info, defines)) {
    fprintf(out, "\n");
  }
  ...
  if (compile_write_udg_flatten_expr_pair(
          env, symb_table, out, SexpFsm_get_compassion(SEXP_FSM(bool_sexp_fsm)),
          COMPASSION_T, dag_info, defines)) {
    fprintf(out, "\n\n");
  }
}
...
static void compile_write_udg_bool_spec(const NuSMVEnv_ptr env, FILE *out,
  ...
  if (compile_write_udg_flatten_bfexpr(
          env, enc, BaseEnc_get_symb_table(BASE_ENC(enc)), det_layer, out, spec,
...
static void compile_write_udg_bool_specs(const NuSMVEnv_ptr env, FILE *out,
  ...
  compile_write_udg_bool_spec(env, out, enc, FlatHierarchy_get_spec(hierarchy),
                              "CTLSPEC\n", det_layer, dag_info, defines);
  compile_write_udg_bool_spec(env, out, enc,
                              FlatHierarchy_get_compute(hierarchy), "COMPUTE\n",
                              det_layer, dag_info, defines);
  compile_write_udg_bool_spec(env, out, enc,
                              FlatHierarchy_get_ltlspec(hierarchy), "LTLSPEC\n",
                              det_layer, dag_info, defines);
  compile_write_udg_bool_spec(env, out, enc,
  ...
  { /* PSL specifications are not supported at the moment (see issue 2626) */
    ...
    if (pslspec != Nil) {
      StreamMgr_print_error(streams, "\n********   WARNING   ********\n");
      StreamMgr_print_error(
          streams, "This version does not support the booleanization of "
                   "PSL properties.\n"
  ...
  if (SymbLayer_get_input_vars_num(det_layer) > 0) {
    ...
    compile_write_udg_flatten_bool_vars(
        env, st, BoolEncClient_get_bool_enc(BOOL_ENC_CLIENT(enc)), out,
...
compile_convert_to_dag_aux_udg(const NuSMVEnv_ptr env, SymbTable_ptr symb_table,
    ...
    if (info != Nil) {
      ...
      if (count >= num_thres || (count > 1 && depth >= dep_thres)) {
        ...
        node_ptr left = compile_convert_to_dag_aux_udg(
            env, symb_table, car(expr), hash, num_thres, dep_thres, defines,
            defines_prefix);
        node_ptr right = compile_convert_to_dag_aux_udg(
            env, symb_table, cdr(expr), hash, num_thres, dep_thres, defines,
            defines_prefix);
        ...
        { /* finds a good name for the define */
          ...
          name = find_node(nodemgr, ATOM,
                           (node_ptr)UStringMgr_find_string(strings, buf), Nil);
          FREE(buf);
        }
  ...
  default: {
    node_ptr left = compile_convert_to_dag_aux_udg(env, symb_table, car(expr),
                                                   hash, num_thres, dep_thres,
                                                   defines, defines_prefix);
    node_ptr right = compile_convert_to_dag_aux_udg(env, symb_table, cdr(expr),
                                                    hash, num_thres, dep_thres,
                                                    defines, defines_prefix);
...
static hash_ptr compile_create_dag_info_from_hierarchy_udg(
  ...
  { /* learn from ASSIGNs */
    ...
    SET_FOREACH(vars, iter) {
      ...
      node_ptr init_name = find_node(nodemgr, SMALLINIT, name, Nil);
      node_ptr next_name = find_node(nodemgr, NEXT, name, Nil);
      Compile_make_dag_info(env, FlatHierarchy_lookup_assign(hierarchy, name),
                            dag_info);
      Compile_make_dag_info(
          env, FlatHierarchy_lookup_assign(hierarchy, init_name), dag_info);
      Compile_make_dag_info(
          env, FlatHierarchy_lookup_assign(hierarchy, next_name), dag_info);
    }
  }
...
static int compile_write_udg_print_node(const NuSMVEnv_ptr env, FILE *out,
  ...
  case ATTIME:
    error_unreachable_code();
    return _PRINT(n, "@", "", "") && _THROW(cdr(n)) && _PRINT(n, "{", "", "") &&
  ...
  case ATOM:
    if (!_PRINT_NS(n,
                   ...
                   UStringMgr_get_string_text((string_ptr)car(n)), NODE_STYLE_1,
                   CONS_COLOR))
      return 0;
    if (cdr(n)) {
      char buf[20];
  ...
  case NUMBER_UNSIGNED_WORD:
    return _PRINT(n,
                  WordNumber_to_based_string(
                      WORD_NUMBER(car(n)), get_output_word_format(opts), false),
                  NODE_STYLE_1, CONS_COLOR);
  case NUMBER_SIGNED_WORD:
    return _PRINT(n,
                  WordNumber_to_based_string(
                      WORD_NUMBER(car(n)), get_output_word_format(opts), true),
                  NODE_STYLE_1, CONS_COLOR);
  ...
  case UWCONST:
    return _PRINT(n, "uwconst(", "", "") && _THROW(car(n)) &&
           _PRINT(n, ", ", "", "") && _THROW(cdr(n)) && _PRINT(n, ")", "", "");
  ...
  case CONTEXT:
    return _THROW(cdr(n)) &&
           ((car(n) == Nil) || (_PRINT(n, " IN ", "", "") && _THROW(car(n))));

  case CONS:
    return _THROW(car(n)) &&
           ((cdr(n) == Nil) || (_PRINT(n, ", ", "", "") && _THROW(cdr(n))));
    ...
    if (Nil != car(n)) {
      if (FAILURE == node_get_type(cdr(n)))
        ...
        compile_write_udg_print_3_aryc_color(
            env, out, n, "case", car(car(n)), cdr(car(n)), cdr(n), true, shared,
            INPUT_CASE_COLOR, CONTROL_CASE_COLOR, STANDARD_COLOR);
    } else {
  ...
  case COLON:
    compile_write_udg_print_2_ary(env, out, n, ":", true, shared, style, style);
    break;

  case IFTHENELSE:
    compile_write_udg_print_3_aryc_color(
        env, out, n, "ite", car(car(n)), cdr(car(n)), cdr(n), true, shared,
        INPUT_CASE_COLOR, CONTROL_CASE_COLOR, STANDARD_COLOR);
    break;

  case ARRAY:
    compile_write_udg_print_2_ary(env, out, n, "[]", true, shared, style,
                                  style);
    return 1;
    return _THROW(car(n)) && _PRINT(n, "[", "", "") && _THROW(cdr(n)) &&
           _PRINT(n, "]", "", "");
  ...
  case BIT_SELECTION: {
    ...
    arg = strcat(arg, "]");
    compile_write_udg_print_3_aryc(env, out, n, arg, car(n), NULL, NULL, true,
                                   shared);
    return 1;
  }
  case CONSTANTS:
    return _PRINT(n, "CONSTANTS ", "", CONS_COLOR) && _THROW(car(n)) &&
           _PRINT(n, ";", "", "");
  ...
  case CAST_SIGNED:
    ...
    break;
  case CAST_UNSIGNED:
    compile_write_udg_print_1_ary(env, out, n, "unsigned", true, shared, style);
    break;
  case EXTEND: {
  ...
  case IMPLIES:
    ...
    break;
  case IFF:
    compile_write_udg_print_2_ary(env, out, n, "<->", true, true, style, style);
    break;
  case XOR:
    compile_write_udg_print_2_ary(env, out, n, "xor", true, shared, style,
                                  style);
    break;
  case XNOR:
    compile_write_udg_print_2_ary(env, out, n, "xnor", true, shared, style,
                                  style);
    break;
  case OR:
    compile_write_udg_print_2_ary(env, out, n, "|", true, shared, style, style);
  ...
  case OP_ONCE:
    ...
    break;
  case UNTIL:
    compile_write_udg_print_2_ary(env, out, n, "U", true, shared, style, style);
    break;
  case SINCE:
    compile_write_udg_print_2_ary(env, out, n, "S", true, shared, style, style);
    break;
  case RELEASES:
    compile_write_udg_print_2_ary(env, out, n, "V", true, shared, style, style);
    break;
  case TRIGGERED:
    compile_write_udg_print_2_ary(env, out, n, "T", true, shared, style, style);
    break;
  ...
  case EBF:
    compile_write_udg_print_2_ary(env, out, n, "EBF", true, shared, style,
                                  style);
    break;
  case ABF:
    compile_write_udg_print_2_ary(env, out, n, "ABF", true, shared, style,
                                  style);
    break;
  case EBG:
    compile_write_udg_print_2_ary(env, out, n, "EBG", true, shared, style,
                                  style);
    break;
  case ABG:
    compile_write_udg_print_2_ary(env, out, n, "ABG", true, shared, style,
                                  style);
    break;
  ...
  case GE:
    ...
    break;
  case SETIN:
    compile_write_udg_print_2_ary(env, out, n, "in", true, shared, style,
                                  style);
    break;
  case UNION:
    compile_write_udg_print_2_ary(env, out, n, "union", true, shared, style,
                                  style);
    break;
  case LSHIFT:
    compile_write_udg_print_2_ary(env, out, n, "<<", true, shared, style,
  ...
  case RROTATE:
    ...
    break;
  case MOD:
    compile_write_udg_print_2_ary(env, out, n, "mod", true, shared, style,
                                  style);
    break;
...
static inline int compile_write_udg_print_2_ary(
    const NuSMVEnv_ptr env, FILE *buffer, node_ptr code, const char *str,
    boolean close, boolean shared, const char *color1, const char *color2) {
  return compile_write_udg_print_3_aryc_color(env, buffer, code, str, car(code),
                                              cdr(code), NULL, close, shared,
...
static inline int compile_write_udg_print_2_arya(const NuSMVEnv_ptr env,
  ...
  if (Nil != find_assoc(visited, code)) {
    ...
    fprintf(buffer,
            NODE_1_UDG NODE_2_UDG "%s" NODE_3_UDG ",[" EDGE_1_UDG EDGE_ATTS,
            (void *)code, str);
...
*/
static inline int compile_write_udg_print_3_aryc(
    const NuSMVEnv_ptr env, FILE *buffer, node_ptr code, const char *str,
    node_ptr fst, node_ptr snd, node_ptr trd, boolean close, boolean shared) {
  return compile_write_udg_print_3_aryc_color(env, buffer, code, str, fst, snd,
                                              trd, close, shared, "", "", "");
}
...
static inline int compile_write_udg_print_3_aryc_color(
  ...
  if (!isvisited) {
    if (shared) {
      fprintf(buffer,
              NODE_1_UDG NODE_2_UDG "%s" NODE_3_UDG ",[" EDGE_1_UDG EDGE_ATTS_C
                                    "%s" CLOSE_EDGE,
              (void *)code, str,
              (strcmp(color1, "") ? color1 : STANDARD_COLOR));
    } else {
      fprintf(buffer,
              NODE_1E_UDG NODE_2_UDG "%s" NODE_3_UDG ",[" EDGE_1_UDG EDGE_ATTS_C
                                     "%s" CLOSE_EDGE,
              str, (strcmp(color1, "") ? color1 : STANDARD_COLOR));
    }
