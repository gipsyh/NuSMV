...
Set_t Formula_GetDependencies(const SymbTable_ptr symb_table, node_ptr formula,
			      node_ptr context)
{
	FormulaDependency_ptr dep = FORMULA_DEPENDENCY(NuSMVEnv_get_value(
		ENV_OBJECT_GET_ENV(symb_table), ENV_DEPENDENCY));
			       ...
			       node_ptr justice, node_ptr compassion)
{
	...
	return FormulaDependency_formulae_get_dependencies(
		dep, symb_table, formula, justice, compassion);
}
				     ...
				     boolean preserve_time)
{
	...
	return FormulaDependency_formulae_get_dependencies_by_type(
		dep, symb_table, formula, justice, compassion, filter,
		preserve_time);
}
			 ...
			 boolean *reached_fixpoint)
{
	...
	while ((i < steps) && (!Set_IsEmpty(symbols_left))) {
		...
		SET_FOREACH(symbols_left, iter)
		{
         ...
         hash */
			base = _coi_get_var_coi0(symb_table, hierarchy, var,
						 &nonassign, false,
						 (hash_ptr)NULL);
					      ...
					      hash_ptr dependencies_hash)
{
		...
		if (((filter & VFT_INPUT) &&
			...
			if (preserve_time) {
				if (EXPR_UNTIMED_NEXT == time &&
				    !SymbTable_is_symbol_input_var(symb_table,
								   formula)) {
					formula = ExprMgr_next(exprs, formula,
							       symb_table);
				} else if (time >= 0) {
					formula = ExprMgr_attime(exprs, formula,
								 time,
								 symb_table);
				}
			}
	...
	if (SymbTable_is_symbol_define(symb_table, formula)) {
		...
		result = lookup_dependencies_hash(dependencies_hash,
						  NODE_PTR(&key));
		...
		if (result == (Set_t)NULL) {
         ...
         dependencies. */
			node_ptr nformula;
			insert_dependencies_hash(dependencies_hash,
						 NODE_PTR(&key),
						 BUILDING_DEP_SET);
         ...
         dependencies for further use. */
			if (Set_IsEmpty(result)) {
				close_define_dependencies_hash(
		...
		node_ptr nformula =
			SymbTable_get_array_define_body(symb_table, formula);
		result = formulaGetDependenciesRecur(
			symb_table, nformula,
       ...
       filter */
		...
		for (i = low; i <= high; ++i) {
			node_ptr index = find_node(nodemgr, NUMBER,
						   NODE_FROM_INT(i), Nil);
			node_ptr arr_var =
				find_node(nodemgr, ARRAY, formula, index);
			Set_t ret;

			ret = formulaGetDefinitionDependencies(
				symb_table, arr_var, filter, preserve_time,
					 ...
					 hash_ptr dependencies_hash)
{
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_parameter(rs)) {
			...
			param = SymbTable_get_flatten_actual_parameter(
				symb_table, name);
			result = formulaGetDependenciesRecur(
				symb_table, param, context, filter,
			...
			result = formulaGetDefinitionDependencies(
				symb_table, name, filter, preserve_time, time,
				dependencies_hash);

			/* It is possible formulaGetDefinitionDependencies inserted a value on
         the same key */
			if (name == formula &&
			    SymbTable_is_symbol_define(symb_table, name)) {
				nusmv_assert((Set_t)NULL !=
					     lookup_dependencies_hash(
						     dependencies_hash,
						     NODE_PTR(&key)));
	...
	case NEXT: /* next(alpha), with alpha possibly complex, thus ... */
       ...
       next-operator */
		result = formulaGetDependenciesRecur(
			symb_table, car(formula), context, filter,
			preserve_time, EXPR_UNTIMED_NEXT, dependencies_hash);
		break;
	...
	case MINU: {
		Set_t right = formulaGetDependenciesRecur(
			symb_table, cdr(formula), context, filter,
			preserve_time, time, dependencies_hash);
	...
	case IFTHENELSE: {
		Set_t then_arg = formulaGetDependenciesRecur(
			symb_table, cdr(car(formula)), context, filter,
			preserve_time, time, dependencies_hash);
	...
	case BIT_SELECTION: {
		Set_t high_bit = formulaGetDependenciesRecur(
			symb_table, car(cdr(formula)), context, filter,
			preserve_time, time, dependencies_hash);
		Set_t low_bit = formulaGetDependenciesRecur(
			symb_table, cdr(cdr(formula)), context, filter,
			preserve_time, time, dependencies_hash);
	...
	case COUNT: {
		...
		while (Nil != list) {
			Set_t tmp = formulaGetDependenciesRecur(
				symb_table, car(list), context, filter,
				preserve_time, time, dependencies_hash);
	...
	case WAWRITE: {
		Set_t location = formulaGetDependenciesRecur(
			symb_table, car(cdr(formula)), context, filter,
			preserve_time, time, dependencies_hash);
		Set_t address = formulaGetDependenciesRecur(
			symb_table, cdr(cdr(formula)), context, filter,
			preserve_time, time, dependencies_hash);
		...
		if (ResolveSymbol_is_defined(rs)) {
         ...
         the same key */
			...
			result = formulaGetDependenciesRecur(
				symb_table, car(formula), context, filter,
				preserve_time, time, dependencies_hash);
			tmp = formulaGetDependenciesRecur(
				symb_table, cdr(formula), context, filter,
				preserve_time, time, dependencies_hash);
			result = Set_Union(result, tmp);
			Set_ReleaseSet(tmp);
	...
	case ARRAY_DEF: {
		...
		for (iter = car(formula); iter != Nil; iter = cdr(iter)) {
			...
			tmp = formulaGetDependenciesRecur(symb_table, car(iter),
							  context, filter,
							  preserve_time, time,
							  dependencies_hash);
			result = Set_Union(result, tmp);
	...
	case PSL_WSELECT: {
		...
		Set_t object = formulaGetDependenciesRecur(
			symb_table, psl_node_get_left(formula), context, filter,
			preserve_time, time, dependencies_hash);
		Set_t high_bit = formulaGetDependenciesRecur(
			symb_table, psl_node_get_left(right), context, filter,
			preserve_time, time, dependencies_hash);
		Set_t low_bit = formulaGetDependenciesRecur(
			symb_table, psl_node_get_right(right), context, filter,
			preserve_time, time, dependencies_hash);
	...
	case PSL_CONCATENATION: {
		Set_t right = formulaGetDependenciesRecur(
			symb_table, psl_node_get_right(formula), context,
			filter, preserve_time, time, dependencies_hash);
	...
	case PSL_SEREREPEATED:
		result = formulaGetDependenciesRecur(
			symb_table, psl_node_sere_repeated_get_expr(formula),
	...
	case PSL_REPLPROP:
		result = formulaGetDependenciesRecur(
			symb_table, psl_node_repl_prop_get_property(formula),
			context, filter, preserve_time, time,
			dependencies_hash);
		break;
	...
	case PSL_XBANG: {
		Set_t n2 = formulaGetDependenciesRecur(
			symb_table, psl_node_extended_next_get_when(formula),
			context, filter, preserve_time, time,
			dependencies_hash);
		Set_t n3 = formulaGetDependenciesRecur(
			symb_table,
		...
		result = formulaGetDependenciesRecur(
			symb_table, psl_node_extended_next_get_expr(formula),
			context, filter, preserve_time, time,
			dependencies_hash);
	...
	case PSL_ITE: {
		Set_t then_arg = formulaGetDependenciesRecur(
			symb_table, psl_node_get_ite_then(formula), context,
			filter, preserve_time, time, dependencies_hash);
		Set_t else_arg = formulaGetDependenciesRecur(
			symb_table, psl_node_get_ite_else(formula), context,
			filter, preserve_time, time, dependencies_hash);

		result = formulaGetDependenciesRecur(
			symb_table, psl_node_get_ite_cond(formula), context,
			filter, preserve_time, time, dependencies_hash);
		result = Set_Union(Set_Union(result, then_arg), else_arg);
		Set_ReleaseSet(else_arg);
		Set_ReleaseSet(then_arg);
...
static void coiInit(const SymbTable_ptr symb_table, FlatHierarchy_ptr hierarchy)
{
	...
	SYMB_TABLE_FOREACH(symb_table, iter, STT_VAR)
	{
       ...
       from NULL */
		...
		base = _coi_get_var_coi0(symb_table, hierarchy, var, &nonassign,
					 true, coi0_hash);
		...
		if (COI_VERBOSE) {
			...
			Set_PrintSet(wffprint, Logger_get_stream(logger), base,
				     NULL, NULL);
			Logger_log(logger, "\n");
		}
				      ...
				      hash_ptr consts_hash)
{
	...
	switch (node_get_type(formula)) {
	case CONTEXT:
		result = formulaGetConstantsRecur(symb_table, cdr(formula),
						  car(formula), consts_hash);
		break;
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_constant(rs)) {
			...
			param = SymbTable_get_flatten_actual_parameter(
				symb_table, name);
			result = formulaGetConstantsRecur(symb_table, param,
							  context, consts_hash);
		}
		/* it should be a defined symbol, running, or a variable */
			...
			nusmv_assert(
				SymbTable_is_symbol_var(symb_table, name) ||
	...
	case FLOOR:
		result = formulaGetConstantsRecur(symb_table, car(formula),
						  context, consts_hash);
		break;
	...
	case MAXU: /* MIN MAX operators */
	case MINU: {
		Set_t right = formulaGetConstantsRecur(symb_table, cdr(formula),
						       context, consts_hash);
		result = formulaGetConstantsRecur(symb_table, car(formula),
						  context, consts_hash);
		result = Set_Union(result, right);
	...
	case IFTHENELSE: {
		Set_t condition = formulaGetConstantsRecur(
			symb_table, car(car(formula)), context, consts_hash);
		Set_t then_arg = formulaGetConstantsRecur(
			symb_table, cdr(car(formula)), context, consts_hash);
		Set_t else_arg = formulaGetConstantsRecur(
			symb_table, cdr(formula), context, consts_hash);
	...
	case BIT_SELECTION: {
		Set_t object = formulaGetConstantsRecur(
			symb_table, car(formula), context, consts_hash);
		Set_t high_bit = formulaGetConstantsRecur(
			symb_table, car(cdr(formula)), context, consts_hash);
		Set_t low_bit = formulaGetConstantsRecur(
			symb_table, cdr(cdr(formula)), context, consts_hash);
		result = Set_Union(Set_Union(object, high_bit), low_bit);
		Set_ReleaseSet(low_bit);
		Set_ReleaseSet(high_bit);
		break;
	...
	case COUNT: {
		...
		while (Nil != list) {
			Set_t tmp = formulaGetConstantsRecur(
				symb_table, car(list), context, consts_hash);
		...
		/* Operations on WORDARRAYs */
	case WAWRITE: {
		Set_t memory = formulaGetConstantsRecur(
			symb_table, car(formula), context, consts_hash);
		Set_t location = formulaGetConstantsRecur(
			symb_table, car(cdr(formula)), context, consts_hash);
		Set_t address = formulaGetConstantsRecur(
			symb_table, cdr(cdr(formula)), context, consts_hash);
	...
	case PSL_WSELECT: {
		...
		Set_t object = formulaGetConstantsRecur(
			symb_table, psl_node_get_left(formula), context,
			consts_hash);
		Set_t high_bit = formulaGetConstantsRecur(
			symb_table, psl_node_get_left(right), context,
			consts_hash);
		Set_t low_bit = formulaGetConstantsRecur(
			symb_table, psl_node_get_right(right), context,
			consts_hash);
...

	{
		...
		while (!Set_IsEmpty(vars_left)) {
			...
			SET_FOREACH(vars_left, iter)
			{
				...
				vdeps = _coi_get_var_coi0(st, fh, v, &nonassign,
							  true, coi0_hash);
			       ...
			       boolean use_cache, hash_ptr coi0_hash)
{
...

	{
		...
		node_ptr invar_a =
			FlatHierarchy_lookup_assign(hierarchy, invar_var);
		node_ptr invar_c =
			FlatHierarchy_lookup_constrains(hierarchy, invar_var);
		node_ptr init_a =
			FlatHierarchy_lookup_assign(hierarchy, init_var);
		node_ptr init_c =
			FlatHierarchy_lookup_constrains(hierarchy, init_var);
		node_ptr next_a =
			FlatHierarchy_lookup_assign(hierarchy, next_var);
		node_ptr next_c =
			FlatHierarchy_lookup_constrains(hierarchy, next_var);
		...
		if (Nil != invar_a) {
			Set_t deps = Formula_GetDependenciesByType(
				st, invar_a, Nil, VFT_CNIF | VFT_FUNCTION,
				false);
		...
		if (Nil != invar_c) {
			Set_t deps = Formula_GetDependenciesByType(
				st, invar_c, Nil, VFT_CNIF | VFT_FUNCTION,
				false);
		...
		if (Nil != init_a) {
			Set_t deps = Formula_GetDependenciesByType(
				st, init_a, Nil, VFT_CNIF | VFT_FUNCTION,
				false);
			result = Set_Union(result, deps);
			Set_ReleaseSet(deps);
		}
		if (Nil != init_c) {
			Set_t deps = Formula_GetDependenciesByType(
				st, init_c, Nil, VFT_CNIF | VFT_FUNCTION,
				false);
		...
		if (Nil != next_a) {
			Set_t deps = Formula_GetDependenciesByType(
				st, next_a, Nil, VFT_CNIF | VFT_FUNCTION,
				false);
			result = Set_Union(result, deps);
			Set_ReleaseSet(deps);
		}
		if (Nil != next_c) {
			Set_t deps = Formula_GetDependenciesByType(
				st, next_c, Nil, VFT_CNIF | VFT_FUNCTION,
				false);
...
*/
static Set_t lookup_dependencies_hash(hash_ptr dependencies_hash, node_ptr key)
{
	nusmv_assert(dependencies_hash != (hash_ptr)NULL);
					      ...
					      char *hash_str)
{
	if (!strcmp(ST_CONE_COI_HASH, hash_str)) {
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_CONE_COI_HASH, (ST_PFICPCP)NULL,
			(ST_PFICPI)NULL, coi_hash_free,
		...
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_CONE_DEPENDENCIES_HASH,
			(ST_PFICPCP)Tuple5_compare, (ST_PFICPI)Tuple5_hash,
			dependencies_hash_free, (SymbTableTriggerFun)NULL,
			SymbTable_clear_handled_remove_action_hash,
			(SymbTableTriggerFun)NULL);
	} else if (!strcmp(ST_CONE_CONSTS_HASH, hash_str)) {
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_CONE_CONSTS_HASH, (ST_PFICPCP)NULL,
			(ST_PFICPI)NULL, consts_hash_free,
			(SymbTableTriggerFun)NULL,
			SymbTable_clear_handled_remove_action_hash,
			(SymbTableTriggerFun)NULL);
	} else if (!strcmp(ST_CONE_COI0_HASH, hash_str)) {
		return SymbTable_get_handled_hash_ptr(
			symb_table, ST_CONE_COI0_HASH, (ST_PFICPCP)NULL,
			(ST_PFICPI)NULL, coi0_hash_free,
			(SymbTableTriggerFun)NULL,
