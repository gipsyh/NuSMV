...
boolean TypeChecker_check_layer(TypeChecker_ptr self, SymbLayer_ptr layer)
{
	...
	nusmv_assert(SymbTable_get_layer(table, SymbLayer_get_name(layer)) ==
		     layer);
	...
	isOK = true;
	{
		...
		{
			node_ptr varName =
				SymbLayer_iter_get_symbol(layer, &liter);
			SymbType_ptr type =
				SymbTable_get_var_type(table, varName);
			isOK = TypeChecker_is_type_wellformed(self, type,
							      varName) &&
			       isOK;
		}
		...
		SYMB_LAYER_FOREACH(layer, liter, STT_DEFINE | STT_ARRAY_DEFINE)
		{
			...
			isOK = TypeChecker_is_specification_wellformed(
				       self, find_node(nodemgr, DEFINE, defName,
						       Nil)) &&
			       isOK;
		}
	}
...

	{
      ...
      */
			...
			while (!ListIter_is_end(iter)) {
				if ((NodeList_count_elem(
					     list, NodeList_get_elem_at(
							   list, iter)) > 1) &&
				    (type_checker_viol_handler(
					    self,
					    TC_VIOLATION_DUPLICATE_CONSTANTS,
					    new_lined_node(nodemgr, CONS,
							   varName, values,
							   node_get_lineno(
								   values))))) {
					return false;
					     ...
					     node_ptr context)
{
...

	{
		...
		if (Nil != context)
			ctx_expr = find_node(nodemgr, CONTEXT, context,
					     expression);
		else
			ctx_expr = expression;
		...
		if (res == nullType) {
			res = type_checker_check_expression(self, expression,
							    context);
		}
				    ...
				    SymbType_ptr type)
{
	if (type_checker_is_memoizing_enabled(self)) {
		...
		insert_assoc(self->expressionTypeMapping, expression,
			     (node_ptr)type);
...
SymbType_ptr tc_lookup_expr_type(TypeChecker_ptr self, node_ptr expression)
{
	if (type_checker_is_memoizing_enabled(self)) {
		return SYMB_TYPE(
			find_assoc(self->expressionTypeMapping, expression));
	}
	return nullType;
}
					   ...
					   node_ptr context)
{
	...
	while (!ListIter_is_end(iter)) {
		CheckerBase_ptr cb = CHECKER_BASE(NodeList_get_elem_at(
				      ...
				      boolean is_error)
{
	...
	if (node_get_lineno(expr)) {
		StreamMgr_print_error(streams, ": line %d",
...
static void type_checker_init(TypeChecker_ptr self, SymbTable_ptr symbolTable)
{
	...
	master_node_walker_init(
		MASTER_NODE_WALKER(self),
		EnvObject_get_environment(ENV_OBJECT(symbolTable)));
						 ...
						 node_ptr expressions)
{
	...
	exp = car(expressions);
	if (Nil != exp && AND != node_get_type(exp) &&
	    CONS != node_get_type(exp)) {
		...
		isOK = type_checker_check_constrain_list(self, kind, exp) &&
		       isOK;
	}
					       ...
					       void *arg)
{
	...
	if (opt_verbose_level_gt(opts, 4)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "TypeChecker: cache invalidator trigger invoked\n");
	}
