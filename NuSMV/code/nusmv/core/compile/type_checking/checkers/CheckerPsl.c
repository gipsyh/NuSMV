...
static SymbType_ptr checker_psl_check_expr(CheckerBase_ptr self, node_ptr e,
					   node_ptr ctx);

static boolean checker_psl_viol_handler(CheckerBase_ptr self,
					   ...
					   node_ptr ctx)
{
	...
	case PSL_SEREREPEATED: {
		...
		if (count != PSL_NULL) {
			...
			if (SymbType_is_error(tcount))
				return _SET_TYPE(ctx_expr, tcount);
		...
		if (sere != PSL_NULL && is_ok) {
			...
			tsere = SymbTablePkg_error_type(env);

		/* is this a type error ? */
		if (!is_ok &&
		    _VIOLATION(SymbType_is_back_comp(tcount) &&
	...
	case PSL_REPLPROP: {
		...
		rs = SymbTable_resolve_symbol(
			symb_table, PslNode_convert_to_node_ptr(id), ctx);
		...
		if (!psl_node_is_boolean_type(value_set)) {
			...
			SymbType_ptr tvs = SymbType_create(env, SYMB_TYPE_ENUM,
							   evalue_set);
       ...
       property */
		while (evalue_set != PSL_NULL) {
			...
			SymbLayer_ptr layer = SymbTable_create_layer(
				symb_table, NULL, SYMB_LAYER_POS_DEFAULT);
			...
			/* handle violation */
			if (!is_ok &&
			    _VIOLATION(SymbType_is_back_comp(prop_type) ?
					       TC_VIOLATION_TYPE_BACK_COMP :
					       TC_VIOLATION_TYPE_MANDATORY,
				       ctx_expr)) {
				return _SET_TYPE(ctx_expr,
	...
	case PSL_PIPEEQGT: {
		...
		PslNode_ptr con =
			psl_node_suffix_implication_get_consequence(expr);
	...
	case PSL_WITHIN_: {
		...
		if (_VIOLATION(SymbType_is_back_comp(t1) &&
					       SymbType_is_back_comp(t2) &&
					       SymbType_is_back_comp(t3) ?
				       TC_VIOLATION_TYPE_BACK_COMP :
				       TC_VIOLATION_TYPE_MANDATORY,
			       ctx_expr)) {
			return _SET_TYPE(ctx_expr,
	...
	case PSL_XBANG: {
		...
		SymbType_ptr t2 = (n2 != PSL_NULL) ? _THROW(n2, context) :
						     SYMB_TYPE(NULL);

		SymbType_ptr t3 = (n3 != PSL_NULL) ? _THROW(n3, context) :
						     SYMB_TYPE(NULL);
		...
		if (SymbType_is_boolean(t1) &&
			...
			case PSL_NEXT_E:
				...
				if (SymbType_get_tag(t2) == SYMB_TYPE_SET_INT) {
					PslNode_ptr low =
						psl_node_range_get_low(n2);
					PslNode_ptr high =
						psl_node_range_get_high(n2);
					if (!psl_node_is_infinite(high) &&
					    psl_node_number_get_value(low) >=
						    0 &&
					    psl_node_number_get_value(low) <=
	...
	case PSL_ITE: {
		...
		SymbType_ptr condType =
			_THROW(psl_node_get_ite_cond(expr), context);
		SymbType_ptr thenType =
			_THROW(psl_node_get_ite_then(expr), context);
		SymbType_ptr elseType =
			_THROW(psl_node_get_ite_else(expr), context);

		if (SymbType_is_error(condType) ||
		    SymbType_is_error(thenType) ||
		    SymbType_is_error(elseType)) {
			/* earlier error */
			return _SET_TYPE(ctx_expr,
					 SymbTablePkg_error_type(env));
		}
    ...
    */
		{
			SymbType_ptr tmp1 =
				SymbType_is_set(elseType) ?
					SymbType_make_set_type(thenType) :
					thenType;

			SymbType_ptr tmp2 =
				SymbType_is_set(thenType) ?
					SymbType_make_set_type(elseType) :
					elseType;
    ...
    */
	...
	default:
		ErrorMgr_internal_error(
			errmgr, "checker_psl_check_expr: not supported type");
	}
    ...
    */
	...
	case TC_VIOLATION_TYPE_WARNING:
		...
		case PSL_SERECOMPOUND:
			...
			print_node(wffprint, stderr,
				   PslNode_convert_to_node_ptr(
					   psl_node_get_left(expr)));
			StreamMgr_print_error(streams, "} : ");
			checker_base_print_type(
				self, errstream,
				PslNode_convert_to_node_ptr(
					psl_node_get_left(expr)),
		...
		case PSL_SERECONCAT:
			StreamMgr_print_error(
				streams,
				"operand types of sere concatenation: ");
			checker_base_print_type(
				self, errstream,
				PslNode_convert_to_node_ptr(
					psl_node_get_left(expr)),
		...
		case PSL_SEREREPEATED: {
			...
			PslNode_ptr count =
				psl_node_sere_repeated_get_count(expr);

			StreamMgr_print_error(
				streams, "operand types of sere repeated: ");
			if (sere != PSL_NULL) {
				checker_base_print_type(
					self, errstream,
		...
		case PSL_PIPEEQGT: {
			PslNode_ptr pre =
				psl_node_suffix_implication_get_premise(expr);
			PslNode_ptr con =
			...
			StreamMgr_print_error(
				streams,
				"operand types of suffix implication: ");
			checker_base_print_type(
				self, errstream,
		...
		case PSL_WITHIN_: {
			PslNode_ptr n1 =
				psl_node_get_left(psl_node_get_left(expr));
			PslNode_ptr n2 =
				psl_node_get_right(psl_node_get_left(expr));
			PslNode_ptr n3 = psl_node_get_right(expr);
		...
		case PSL_XBANG: {
			...
			PslNode_ptr n2 = psl_node_extended_next_get_when(expr);
			PslNode_ptr n3 =
				psl_node_extended_next_get_condition(expr);
...
static void print_operator(FILE *output_stream, PslNode_ptr expr)
{
	...
	case PSL_ALWAYS:
		fprintf(output_stream, "always");
		return;
	case PSL_NEVER:
		fprintf(output_stream, "never");
		return;
	case PSL_EVENTUALLYBANG:
		fprintf(output_stream, "eventually!");
		return;

	case PSL_WITHINBANG:
		fprintf(output_stream, "within!");
		return;
	case PSL_WITHIN:
		fprintf(output_stream, "within");
		return;
	case PSL_WITHINBANG_:
		fprintf(output_stream, "within!_");
		return;
	case PSL_WITHIN_:
		fprintf(output_stream, "within_");
		return;

	case PSL_NEXT_EVENT_ABANG:
		fprintf(output_stream, "next_event_a!");
		return;
	case PSL_NEXT_EVENT_A:
		fprintf(output_stream, "next_event_a");
		return;
	case PSL_NEXT_EVENT_EBANG:
		fprintf(output_stream, "next_event_e!");
		return;
	case PSL_NEXT_EVENT_E:
		fprintf(output_stream, "next_event_e");
		return;
	case PSL_NEXT_EVENTBANG:
		fprintf(output_stream, "next_event!");
		return;
	case PSL_NEXT_EVENT:
		fprintf(output_stream, "next_event");
		return;
	case PSL_NEXT_ABANG:
		fprintf(output_stream, "next_a!");
		return;
	case PSL_NEXT_EBANG:
		fprintf(output_stream, "next_e!");
		return;
	case PSL_NEXT_A:
		fprintf(output_stream, "next_a");
		return;
	...
	case PSL_BEFOREBANG:
		fprintf(output_stream, "before!");
		return;
	case PSL_BEFORE:
		fprintf(output_stream, "before");
		return;
	case PSL_BEFOREBANG_:
		fprintf(output_stream, "before!_");
		return;
	case PSL_BEFORE_:
		fprintf(output_stream, "before_");
		return;
	case PSL_UNTILBANG:
		fprintf(output_stream, "until!");
		return;
	case PSL_UNTIL:
		fprintf(output_stream, "until");
		return;
	case PSL_UNTILBANG_:
		fprintf(output_stream, "until!_");
		return;
	case PSL_UNTIL_:
		fprintf(output_stream, "until_");
		return;
	case PSL_ABORT:
		fprintf(output_stream, "abort");
		return;
	case PSL_W:
		fprintf(output_stream, "W");
		return;
	case PSL_OR:
		fprintf(output_stream, "or");
		return;
	case PSL_CARET:
		fprintf(output_stream, "^");
		return;
	...
	case PSL_AMPERSANDAMPERSAND:
		...
		return;
	case PSL_WHILENOTBANG:
		fprintf(output_stream, "whilenot!");
		return;
	case PSL_WHILENOT:
		fprintf(output_stream, "whilenot");
		return;
	case PSL_WHILENOTBANG_:
		fprintf(output_stream, "whilenot!_");
		return;
	case PSL_WHILENOT_:
		fprintf(output_stream, "whilenot_");
		return;

	case PSL_ITE:
		fprintf(output_stream, "ITE");
		return;
