...
static SymbType_ptr checker_psl_check_expr(CheckerBase_ptr self, node_ptr e,
  ...
  if (context != PSL_NULL) {
    ctx_expr = PslNode_new_context(
        nodemgr, psl_node_context_to_main_context(nodemgr, context), expr);
  } else
    ctx_expr = expr;
  ...
  case PSL_CONCATENATION: {
    ...
    if (!is_ok &&
        _VIOLATION(SymbType_is_back_comp(left) ? TC_VIOLATION_TYPE_BACK_COMP
                                               : TC_VIOLATION_TYPE_MANDATORY,
                   ctx_expr)) {
      return _SET_TYPE(ctx_expr, SymbTablePkg_error_type(env));
    }
  ...
  case PSL_SEREREPEATED: {
    ...
    if (!is_ok &&
        _VIOLATION(SymbType_is_back_comp(tcount) && SymbType_is_back_comp(tsere)
                       ? TC_VIOLATION_TYPE_BACK_COMP
                       : TC_VIOLATION_TYPE_MANDATORY,
                   ctx_expr)) {
      return _SET_TYPE(ctx_expr, SymbTablePkg_error_type(env));
  ...
  case PSL_REPLPROP: {
    ...
    rs = SymbTable_resolve_symbol(symb_table, PslNode_convert_to_node_ptr(id),
                                  ctx);
    ...
    if (!psl_node_is_boolean_type(value_set)) {
      ...
      if (!TypeChecker_is_type_wellformed(
              TYPE_CHECKER(NODE_WALKER(self)->master), tvs, id_ctx)) {
        return _SET_TYPE(ctx_expr, SymbTablePkg_error_type(env));
      }
    }
    ...
    while (evalue_set != PSL_NULL) {
      ...
      SymbLayer_declare_define(
          layer, id_ctx, ctx,
          PslNode_convert_to_node_ptr(psl_node_cons_get_element(evalue_set)));
  ...
  case PSL_WITHIN_: {
    ...
    /* there is violation */
    if (_VIOLATION(SymbType_is_back_comp(t1) && SymbType_is_back_comp(t2) &&
  ...
  case PSL_XBANG: {
    ...
    SymbType_ptr t2 = (n2 != PSL_NULL) ? _THROW(n2, context) : SYMB_TYPE(NULL);

    SymbType_ptr t3 = (n3 != PSL_NULL) ? _THROW(n3, context) : SYMB_TYPE(NULL);
      ...
      case PSL_XBANG:
        ...
        if ((t2 == SYMB_TYPE(NULL)) ||
            ((SymbType_get_tag(t2) == SYMB_TYPE_INTEGER ||
              SymbType_is_pure_int_enum(t2) || SymbType_is_boolean(t2)) &&
        ...
        if (SymbType_get_tag(t2) == SYMB_TYPE_SET_INT) {
          ...
          if (!psl_node_is_infinite(high) &&
              ...
              psl_node_number_get_value(low) <=
                  psl_node_number_get_value(high)) {
            return _SET_TYPE(ctx_expr, SymbTablePkg_boolean_type(env));
          }
        }
        break;
  ...
  case PSL_ITE: {
    ...
    if (SymbType_is_error(condType) || SymbType_is_error(thenType) ||
  ...
  default:
    ErrorMgr_internal_error(errmgr,
                            "checker_psl_check_expr: not supported type");
  }
  ...
  case TC_VIOLATION_TYPE_WARNING:
    ...
    case PSL_SERECOMPOUND:
      ...
      checker_base_print_type(
          self, errstream, PslNode_convert_to_node_ptr(psl_node_get_left(expr)),
    ...
    case PSL_PIPEEQGT: {
      ...
      checker_base_print_type(self, errstream, PslNode_convert_to_node_ptr(pre),
                              PslNode_convert_to_node_ptr(context));
      ...
      checker_base_print_type(self, errstream, PslNode_convert_to_node_ptr(con),
                              PslNode_convert_to_node_ptr(context));
      break;
    }
    ...
    case PSL_WITHIN_: {
      ...
      print_operator(errstream, expr);
      StreamMgr_print_error(streams, "\" : (");
      checker_base_print_type(self, errstream, PslNode_convert_to_node_ptr(n1),
                              PslNode_convert_to_node_ptr(context));
      StreamMgr_print_error(streams, ", ");
      checker_base_print_type(self, errstream, PslNode_convert_to_node_ptr(n2),
                              PslNode_convert_to_node_ptr(context));
      StreamMgr_print_error(streams, ") ");
      checker_base_print_type(self, errstream, PslNode_convert_to_node_ptr(n3),
                              PslNode_convert_to_node_ptr(context));
      break;
    }
