...
static boolean checker_core_viol_handler(CheckerBase_ptr checker,
...
*/
static SymbType_ptr checker_core_check_expr(CheckerBase_ptr self, node_ptr expr,
					    node_ptr context)
{
  ...
  */
    ...
    */
	...
	case UNSIGNED_WORD: {
       ...
       returned by msat model */
		...
		return _SET_TYPE(ctx_expr, SymbTablePkg_unsigned_word_type(
						   env, wordWidth));
	}
	...
	case SIGNED_WORD: {
       ...
       returned by msat model */
		...
		return _SET_TYPE(ctx_expr,
				 SymbTablePkg_signed_word_type(env, wordWidth));
	}
    ...
    */
		if (ResolveSymbol_is_undefined(rs)) {
			SymbType_ptr type;
			SymbType_ptr l_type = checker_core_check_expr(
				self, car(expr), context);
			SymbType_ptr r_type = checker_core_check_expr(
				self, cdr(expr), context);

			if (SymbType_is_error(l_type) ||
			    SymbType_is_error(r_type)) {
				/* earlier error */
				return _SET_TYPE(ctx_expr,
         ...
         or word */
			if (!SymbType_is_array(l_type) ||
				...
				_VIOLATION(TC_VIOLATION_TYPE_MANDATORY,
					   ctx_expr);
				return _SET_TYPE(ctx_expr,
						 SymbTablePkg_error_type(env));
			}
	...
	case ATOM: {
		...
		if (ResolveSymbol_is_undefined(rs)) {
			SymbType_ptr type =
				_VIOLATION(TC_VIOLATION_UNDEF_IDENTIFIER,
					   resolvedName) ?
		...
		if (ResolveSymbol_is_ambiguous(rs)) {
			SymbType_ptr type =
				_VIOLATION(TC_VIOLATION_AMBIGUOUS_IDENTIFIER,
					   ctx_expr) ?
					SymbTablePkg_error_type(env) /* ERROR */
					:
					SymbTablePkg_pure_symbolic_enum_type(
      ...
      */
			node_ptr bodyDef = SymbTable_get_define_body(
				symb_table, resolvedName);
			node_ptr ctxDef = SymbTable_get_define_context(
				symb_table, resolvedName);
		...
		if (ResolveSymbol_is_array_def(rs)) {
			node_ptr arraydef = SymbTable_get_array_define_body(
				symb_table, resolvedName);
			node_ptr array_ctx = SymbTable_get_array_define_context(
				symb_table, resolvedName);
      ...
      */
		...
		if (ResolveSymbol_is_function(rs)) {
			SymbType_ptr type = SymbTable_get_function_type(
				symb_table, resolvedName);
	...
	case ABG: {
		...
		if (Nil != cdr(expr) &&
		    !Utils_check_subrange_not_negative(cdr(expr))) {
		...
		if (SymbType_is_boolean(type) ||
		    (node_get_type(expr) == NOT && SymbType_is_word(type))) {
			return _SET_TYPE(ctx_expr, type);
		}
	...
	case CAST_WORD1: {
		...
		if (node_get_type(expr) == CAST_WORD1 &&
			...
			return _SET_TYPE(ctx_expr,
					 SymbTablePkg_unsigned_word_type(env,
									 1));
		}
	...
	case CAST_UNSIGNED: {
		...
		if (node_get_type(expr) == CAST_SIGNED &&
			...
			return _SET_TYPE(ctx_expr,
					 SymbTablePkg_signed_word_type(env,
								       width));
		}
		if (node_get_type(expr) == CAST_UNSIGNED &&
			...
			return _SET_TYPE(
				ctx_expr,
				SymbTablePkg_unsigned_word_type(env, width));
		}
	...
	case CAST_TOINT: {
		...
		if (!(SymbType_is_word(type) || SymbType_is_boolean(type) ||
		      (SymbType_get_tag(type) ==
		       SYMB_TYPE_INTEGER))) { /* ERROR */
			if (_VIOLATION(TC_VIOLATION_TYPE_MANDATORY, ctx_expr)) {
				return _SET_TYPE(ctx_expr,
						 SymbTablePkg_error_type(env));
			} else { /* return arbitrary type */
	...
	case WRESIZE: {
        ...
        */
				SymbType_ptr test_type =
					_THROW(cdr(expr), context);
    ...
    */
		...
		if (Nil != context) {
			normalisedExpr = find_node(nodemgr, CONTEXT, context,
						   normalisedExpr);
		}
	...
	case BIT_SELECTION: {
       ...
       can resolve into a NUMBER */
		if ((Nil == l || Nil == h) || (NUMBER != node_get_type(l) ||
					       NUMBER != node_get_type(h))) {
			if (_VIOLATION(TC_VIOLATION_UNCONSTANT_EXPRESSION,
				       ctx_expr)) {
				return _SET_TYPE(ctx_expr,
						 SymbTablePkg_error_type(env));
			} else { /* return arbitrary Word type */
		...
		if (width <= highBound || highBound < lowBound ||
		    lowBound < 0) {
			/* ERROR */
			if (_VIOLATION(TC_VIOLATION_OUT_OF_WORD_WIDTH,
				       ctx_expr)) {
				return _SET_TYPE(ctx_expr,
						 SymbTablePkg_error_type(env));
		...
		if (SymbType_is_intarray(type1)) {
			if (!SymbType_is_integer(type2)) {
				...
				SymbType_ptr subtype =
					SymbType_get_intarray_subtype(type1);
			...
			/* subtype check */
			if (!SymbType_equals(
				    SymbType_get_wordarray_subtype(type1),
				    type3))
				return _SET_TYPE(ctx_expr,
						 SymbTablePkg_error_type(env));
		}
	...
	case WORDARRAY_TYPE: {
		...
		return _SET_TYPE(ctx_expr, SymbTablePkg_wordarray_type(
						   env, index_width, subtype));
	}
	case INTARRAY_TYPE: {
       ...
       I think the enum covers both cases */
		...
		return _SET_TYPE(ctx_expr,
				 SymbTablePkg_intarray_type(env, subtype));
	}
	...
	case UMINUS: {
		...
		if (nullType != type && (SymbType_is_infinite_precision(type) ||
    ...
    */
	...
	case MOD: {
		...
		/* w[N] * w[N] -> w[N] */
		if (type1 ==
			    type2 && /*types are shared => same types == same pointers*/
	...
	case RROTATE: {
		...
		if (SymbType_is_word(type1) &&
			...
			case NUMBER:
				...
				break;
			case NUMBER_UNSIGNED_WORD:
				sval = WordNumber_get_unsigned_value(
					WORD_NUMBER(car(cdr(expr))));
				break;
			case NUMBER_SIGNED_WORD:
				sval = WordNumber_get_signed_value(
					WORD_NUMBER(car(cdr(expr))));
				break;
			default:
				sval = -1;
    ...
    */
	case EQDEF: {
       ...
       operands.
    */
		returnType = SymbType_convert_right_to_left(
			type1, SymbType_make_from_set_type(type2));
    ...
    */
		{
			...
			if (nullType != tmp1 && nullType != tmp2) {
				returnType =
					SymbType_get_minimal_common(tmp1, tmp2);
			} else
				returnType = nullType;
		}
    ...
    */
	case SETIN: {
		...
		if (SymbType_is_error(type1) ||
		    SymbType_is_error(type2)) { /*earlier error*/
			return _SET_TYPE(ctx_expr,
					 SymbTablePkg_error_type(env));
		}
    ...
    */
      ...
      */
			SymbType_ptr tmp1 =
				SymbType_is_set(nextCaseType) ?
					SymbType_make_set_type(expType) :
					expType;

			SymbType_ptr tmp2 =
				SymbType_is_set(expType) ?
					SymbType_make_set_type(nextCaseType) :
					nextCaseType;
    ...
    */
	...
	case MAXU: {
		...
		if (SymbType_is_error(type1) || SymbType_is_error(type2)) {
			return _SET_TYPE(
				ctx_expr,
				SymbTablePkg_error_type(env)); /*earlier error*/
		}
    ...
    */
	...
	case ARRAY_DEF: {
		...
		for (size = 0, iter = car(expr); iter != Nil;
			...
			if (SymbType_is_error(elemType)) { /* error */
				return _SET_TYPE(ctx_expr, elemType);
				   ...
				   type) { /* remember the type of first element */
				type = elemType;
			} else { /* infer the common type from current and previous one */
				SymbType_ptr common =
					SymbType_get_minimal_common(elemType,
								    type);
	...
	case NFUNCTION: {
		...
		if (ResolveSymbol_is_parameter(rs)) {
			...
			if (!SymbType_is_function(funtype)) {
				if (_VIOLATION(TC_VIOLATION_TYPE_MANDATORY,
					       ctx_expr)) {
					return _SET_TYPE(
						ctx_expr,
						SymbTablePkg_error_type(env));
      ...
      */
			node_ptr bodyDef =
				SymbTable_get_define_body(st, res_name);
			node_ptr ctxDef =
				SymbTable_get_define_context(st, res_name);
			/* the context is Nil because expr is already flattened */
       ...
       formal ones */
		while (Nil != actual_args) {
			...
			formal_type = (SymbType_ptr)NodeList_get_elem_at(
				formal_args, formal_iter);
			shared_formal_type =
				SymbType_make_memory_shared(formal_type);
			...
			if (SymbType_is_word(formal_type)) {
				has_bitvector = true;
			} else if (SymbType_is_infinite_precision(
					   formal_type) ||
			...
			/* implicit conversion of one of the operands */
			cmp_type = SymbType_get_greater(actual_type,
							shared_formal_type);

			if (nullType == cmp_type) {
				_VIOLATION(TC_VIOLATION_PARAMS_TYPE_ERROR,
					   expr);
				return _SET_TYPE(ctx_expr,
						 SymbTablePkg_error_type(env));
			}
       ...
       to theory combination issues, only */
		if (has_bitvector && has_real_int) {
			_VIOLATION(TC_VIOLATION_DIFFERENT_TYPE_PARAMS_ERROR,
				   expr);
			return _SET_TYPE(ctx_expr,
					 SymbTablePkg_error_type(env));
		}
...
*/
static boolean checker_core_viol_handler(CheckerBase_ptr self,
					 TypeSystemViolation violation,
					 node_ptr expression)
{
  ...
  */
    ...
    */
	...
	case TC_VIOLATION_OUT_OF_WORD_WIDTH:
		if (BIT_SELECTION == node_get_type(expr)) {
			StreamMgr_print_error(
				streams,
				"out of Word's width index or "
				"first index smaller than the second one ('");
		} else if (EXTEND == node_get_type(expr)) {
			StreamMgr_print_error(
				streams,
				"word width is out of range [1..%d] ('",
				WordNumber_max_width());
		} else if (LSHIFT == node_get_type(expr) ||
			   RSHIFT == node_get_type(expr)) {
			StreamMgr_print_error(
				streams,
				"word shift is out of range [0..width] ('");
		} else if (LROTATE == node_get_type(expr) ||
			   RROTATE == node_get_type(expr)) {
			StreamMgr_print_error(
				streams,
				"word rotate is out of range [0..width] ('");
		} else {
	...
	case TC_VIOLATION_PARAMS_NUM_ERROR:
		StreamMgr_print_error(
			streams,
			"Wrong number of parameters for function call '");
		StreamMgr_nprint_error(streams, wffprint, "%N", expr);
		StreamMgr_print_error(streams, "'\n");
		break;
	...
	case TC_VIOLATION_PARAMS_TYPE_ERROR:
		StreamMgr_print_error(
			streams,
			"Wrong type of parameters for function call '");
		StreamMgr_nprint_error(streams, wffprint, "%N", expr);
		StreamMgr_print_error(streams, "'\n");
		break;
	...
	case TC_VIOLATION_DIFFERENT_TYPE_PARAMS_ERROR:
		...
		StreamMgr_print_error(
			streams,
			"Supported sets are: [(BV|Bool) | (R|I|Bool)]\n");
		break;
	...
	case TC_VIOLATION_TYPE_WARNING:
		...
		case TYPEOF:
			...
			print_operator(self, errstream, expr);
			StreamMgr_print_error(streams, "\" : ");
			checker_base_print_type(self, errstream, car(expr),
						context);
			break;
		...
		case CAST_TO_UNSIGNED_WORD:
			...
			checker_base_print_type(self, errstream, car(expr),
						context);
			StreamMgr_print_error(streams, " and ");
			checker_base_print_type(self, errstream, cdr(expr),
						context);
			break;
		...
		case BIT_SELECTION:
			...
			checker_base_print_type(self, errstream, car(expr),
						context);
			StreamMgr_print_error(streams, ", ");
			checker_base_print_type(self, errstream, car(cdr(expr)),
						context);
			StreamMgr_print_error(streams, " and ");
			checker_base_print_type(self, errstream, cdr(cdr(expr)),
						context);
			break;

		case WAWRITE:
			StreamMgr_print_error(streams,
					      "operand types of \"WRITE\" : ");
			checker_base_print_type(self, errstream, car(expr),
						context);
			StreamMgr_print_error(streams, ", ");
			checker_base_print_type(self, errstream, car(cdr(expr)),
		...
		case CASE:
			StreamMgr_print_error(
				streams, "types of \"case\" list elements : ");
			checker_base_print_type(self, errstream, cdr(car(expr)),
						context);
			StreamMgr_print_error(streams, " and ");
			checker_base_print_type(self, errstream, cdr(expr),
						context);
		...
		case COUNT:
			StreamMgr_print_error(
				streams,
				"non-boolean expressions for \"count\" operator");
			break;

		case IFTHENELSE:
			StreamMgr_print_error(
				streams,
				"types of \"then\" and \"else\" expressions : ");
			checker_base_print_type(self, errstream, cdr(car(expr)),
						context);
			StreamMgr_print_error(streams, " and ");
			checker_base_print_type(self, errstream, cdr(expr),
		...
		case EBU:
			...
			print_operator(self, errstream, expr);
			StreamMgr_print_error(streams, "\" : ");
			checker_base_print_type(self, errstream, car(car(expr)),
						context);
			StreamMgr_print_error(streams, " and ");
			checker_base_print_type(self, errstream, cdr(car(expr)),
						context);
			   ...
			   node_ptr expr)
{
	...
	case BIT_SELECTION:
		...
		return;
	case CAST_BOOL:
		fprintf(output_stream, "bool");
		return;
	case CAST_WORD1:
		fprintf(output_stream, "word1");
		return;
	case CAST_SIGNED:
		fprintf(output_stream, "signed");
		return;
	case CAST_UNSIGNED:
		fprintf(output_stream, "unsigned");
		return;
	case FLOOR:
		fprintf(output_stream, "floor");
		return;
	case EXTEND:
		fprintf(output_stream, "extend");
		return;
	case AND:
		fprintf(output_stream, "&");
		return;
	case OR:
		...
		return;
	case XOR:
		fprintf(output_stream, "xor");
		return;
	case XNOR:
		fprintf(output_stream, "xnor");
		return;
	case IMPLIES:
		fprintf(output_stream, "->");
		return;
	...
	case NOT:
		...
		return;
	case EX:
		fprintf(output_stream, "EX");
		return;
	case AX:
		fprintf(output_stream, "AX");
		return;
	case EF:
		fprintf(output_stream, "EF");
		return;
	case AF:
		fprintf(output_stream, "AF");
		return;
	case EG:
		fprintf(output_stream, "EG");
		return;
	case AG:
		fprintf(output_stream, "AG");
		return;
	case OP_GLOBAL:
		fprintf(output_stream, "G");
		return;
	case OP_HISTORICAL:
		fprintf(output_stream, "H");
		return;
	case RELEASES:
		fprintf(output_stream, "V");
		return;
	case TRIGGERED:
		fprintf(output_stream, "T");
		return;
	case UNTIL:
		fprintf(output_stream, "U");
		return;
	case SINCE:
	...
	case OP_PREC:
		...
		return;
	case OP_NOTPRECNOT:
		fprintf(output_stream, "Z");
		return;
	case OP_FUTURE:
		fprintf(output_stream, "F");
		return;
	case OP_ONCE:
		fprintf(output_stream, "O");
		return;
	case AU:
		fprintf(output_stream, "A-U");
		return;
	case EU:
		fprintf(output_stream, "E-U");
		return;
	case EBF:
		fprintf(output_stream, "EBF");
		return;
	case ABF:
		fprintf(output_stream, "ABF");
		return;
	case EBG:
		fprintf(output_stream, "EBG");
		return;
	case ABG:
		fprintf(output_stream, "ABG");
		return;
	case ABU:
		fprintf(output_stream, "ABU");
		return;
	case EBU:
		fprintf(output_stream, "EBU");
		return;

	case WRESIZE:
		fprintf(output_stream, "resize");
		return;
	case WSIZEOF:
		fprintf(output_stream, "sizeof");
		return;
	case CAST_TOINT:
		fprintf(output_stream, "toint");
		return;
	...
	case GE:
		...
		return;
	case UNION:
		fprintf(output_stream, "union");
		return;
	case SETIN:
		fprintf(output_stream, "in");
		return;
	case COLON:
		fprintf(output_stream, ":");
		return;
	case WAREAD:
		fprintf(output_stream, "READ");
		return;
	case WAWRITE:
	...
	case CONST_ARRAY:
		fprintf(output_stream, "CONSTARRAY");
		return;
	case TYPEOF:
		fprintf(output_stream, "typeof");
		return;
	...
	case NUMBER_UNSIGNED_WORD: {
		const OptsHandler_ptr opts = OPTS_HANDLER(NuSMVEnv_get_value(
			ENV_OBJECT(self)->environment, ENV_OPTS_HANDLER));

		fprintf(output_stream, "\n(NUMBER_UNSIGNED_WORD ");
		WordNumber_based_print(output_stream, WORD_NUMBER(car(expr)),
	...
	case BOOLEAN:
		...
		return;
	case INTEGER:
		fprintf(output_stream, "(INT)");
		return;
	case REAL:
		fprintf(output_stream, "(REAL)");
		return;
	case SELF:
		fprintf(output_stream, "(SELF)");
		return;
	case SCALAR:
		fprintf(output_stream, "(SCALAR ");
		return;
	case ARRAY:
		fprintf(output_stream, "\n(ARRAY ");
		return;
	case DOT:
		fprintf(output_stream, "(DOT ");
		return;
	case TWODOTS:
		fprintf(output_stream, "..");
		return;
	case NEXT:
		fprintf(output_stream, "NEXT");
		return;
	case SMALLINIT:
		fprintf(output_stream, "\n(SMALLINIT ");
		return;
	...
	case CONTEXT:
		...
		return;
	case BDD:
		fprintf(output_stream, "(BDD TO BE PRINTED)");
		return;
	case SEMI:
		fprintf(output_stream, "\n(SEMI ");
		return;
	...
	case CAST_TO_UNSIGNED_WORD:
		fprintf(output_stream, "to_unsigned_word");
		return;
