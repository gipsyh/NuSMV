...
CheckerCore_ptr CheckerCore_create(const NuSMVEnv_ptr env) {
  ...
  checker_core_init(self, env, "Core SMV Type Checker", NUSMV_EXPR_SYMBOL_FIRST,
...
static SymbType_ptr checker_core_check_expr(CheckerBase_ptr self, node_ptr expr,
  ...
  */
    ...
    */
    if (ResolveSymbol_is_undefined(rs)) {
      ...
      if (!SymbType_is_array(l_type) ||
          !(SymbType_is_boolean(r_type) || SymbType_is_integer(r_type) ||
  ...
  case ATOM: {
    ...
    if (ResolveSymbol_is_undefined(rs)) {
      SymbType_ptr type =
          _VIOLATION(TC_VIOLATION_UNDEF_IDENTIFIER, resolvedName)
    ...
    if (ResolveSymbol_is_define(rs)) {
      ...
      */
      node_ptr bodyDef = SymbTable_get_define_body(symb_table, resolvedName);
      node_ptr ctxDef = SymbTable_get_define_context(symb_table, resolvedName);
  ...
  case CAST_WORD1: {
    ...
    return _SET_TYPE(ctx_expr, node_get_type(expr) == CAST_BOOL
  ...
  case CAST_UNSIGNED: {
    ...
    /* this is not an error after all -> create the proper return type */
    return _SET_TYPE(ctx_expr, node_get_type(expr) == CAST_SIGNED
                                   ? SymbTablePkg_unsigned_word_type(env, 1)
    ...
    /* Check the correctness of new size */
    {
...

      {
        ...
        if ((0 < newWidth) && newWidth <= WordNumber_max_width()) {
          ...
          return _SET_TYPE(
              ctx_expr, SymbType_is_signed_word(type1)
  ...
  case DIVIDE: {
    ...
    if (cdr(expr) != (node_ptr)NULL)
      ...
      nusmv_assert(node_get_type(expr) == PLUS || node_get_type(expr) == MINUS);
      type2 = type1;
    }
    ...
    /* correct types are i->i, r->r, c->c, w[N]->w[N] */
    if (nullType != returnType && (SymbType_is_infinite_precision(returnType) ||
    ...
    /* is this a type error ? */
    if (_VIOLATION(
            (SymbType_is_back_comp(type1) && SymbType_is_back_comp(type2))
  ...
  case NOTEQUAL: {
    ...
    */
    ...
    /* is this a type error ? */
    if (_VIOLATION(SymbType_is_back_comp(type1) && SymbType_is_back_comp(type2)
                       ? TC_VIOLATION_TYPE_BACK_COMP
                       : TC_VIOLATION_TYPE_MANDATORY,
                   ctx_expr)) {
      return _SET_TYPE(ctx_expr, SymbTablePkg_error_type(env));
  ...
  case CASE: {
    ...
    if ((!SymbType_is_boolean(condType)) &&
        _VIOLATION(
            SymbType_is_back_comp(condType) ? TC_VIOLATION_TYPE_BACK_COMP
                                            : TC_VIOLATION_TYPE_MANDATORY,
            Nil == context ? car(expr)
  ...
  case ARRAY_DEF: {
    ...
    for (size = 0, iter = car(expr); iter != Nil; size += 1, iter = cdr(iter)) {
  ...
  case NFUNCTION: {
    ...
    if (ResolveSymbol_is_parameter(rs)) {
      /* this is a parameter */
      node_ptr parameter = SymbTable_get_flatten_actual_parameter(st, res_name);
      ...
      /* the bodies of defines are cached(in BddEnc).
         So, the type of expressions will also be correctly cached.
      */
      node_ptr bodyDef = SymbTable_get_define_body(st, res_name);
      node_ptr ctxDef = SymbTable_get_define_context(st, res_name);
      /* the context is Nil because expr is already flattened */
    ...
    while (Nil != actual_args) {
      ...
      formal_type =
          (SymbType_ptr)NodeList_get_elem_at(formal_args, formal_iter);
...
static boolean checker_core_viol_handler(CheckerBase_ptr self,
  ...
  */
  if (TC_VIOLATION_TYPE_WARNING == violation ||
      (TC_VIOLATION_TYPE_BACK_COMP == violation && opt_backward_comp(opts))) {
    isError = false;
  }
  ...
  case TC_VIOLATION_OUT_OF_WORDARRAY_WIDTH:
    ...
    if (WAWRITE == node_get_type(expr)) {
      StreamMgr_print_error(
          streams, "in WRITE expression the width of address or value operands"
