...
static void compile_fill_hrc_add_var(const NuSMVEnv_ptr env, NodeMgr_ptr nomgr,
					...
					node_ptr parse_tree)
{
...

	{
		...
		for (iter = parse_tree; Nil != iter; iter = cdr(iter)) {
			...
			if (Nil != mod_def &&
			    node_get_type(mod_def) == MODULE) {
				node_ptr name =
					NodeMgr_find_atom(nomgr, caar(mod_def));
	...
	/* now generate the HrcNodes */
	{
		...
		mod_name =
			NodeMgr_find_atom(nomgr, mod_name); /* make it unique */
		...
		{ /* Adds formal/actual parameters to the module instance */
			node_ptr iter;
			for (iter = car(mod_def); Nil != iter;
			     iter = cdr(iter)) {
				node_ptr par =
					NodeMgr_cons(nomgr, car(iter), Nil);
				HrcNode_add_formal_parameter(hrc_result, par);
			}
		}

		/* fills the hrc */
		Compile_fill_hrc_from_mod_body(env, nomgr,
					       cdr(mod_def), /* body */
					       hrc_result, mod_defs);
	}
				    ...
				    hash_ptr mod_defs)
{
	...
	for (decls = mod_body; Nil != decls; decls = cdr(decls)) {
		...
		case DEFINE: {
			node_ptr decl_iter;
			for (decl_iter = car(cur_decl); decl_iter != Nil;
			     decl_iter = cdr(decl_iter)) {
				node_ptr cur_define = car(decl_iter);
				HrcNode_add_define(
					hrc_result,
					NodeMgr_cons(nomgr, car(cur_define),
						     cdr(cur_define)));
			}
			break;
		}
		...
		case IVAR: {
			...
			for (decl_iter = car(cur_decl); decl_iter != Nil;
				...
				compile_fill_hrc_add_var(
					env, nomgr, car(cur_var), cdr(cur_var),
					node_get_type(cur_decl), hrc_result,
					mod_defs);
			}
			break;
		}
		...
		case ASSIGN:
			compile_add_assign_hrc(nomgr, hrc_result,
					       car(cur_decl));
			break;
		...
		case COMPUTE: {
			...
			for (decl_iter = car(cur_decl); decl_iter != Nil;
			     decl_iter = cdr(decl_iter)) {
				HrcNode_add_compute_property_expr(
					hrc_result, car(decl_iter));
			}
			break;
		}
		...
		case COMPASSION: {
			...
			for (decl_iter = car(cur_decl); decl_iter != Nil;
			     decl_iter = cdr(decl_iter)) {
				node_ptr compassion =
					NodeMgr_cons(nomgr, caar(decl_iter),
						     cdar(decl_iter));
				     ...
				     hash_ptr mod_defs)
{
	...
	case MODTYPE: {
       ...
       so the module name can be used as a key. */
		...
		Compile_fill_hrc_from_mod_body(env, nomgr, cdr(child_mod_def),
					       hrc_child, mod_defs);
		break;
	}
	...
	default:
		ErrorMgr_internal_error(ermgr,
					"compile_fill_hrc_add_var: type = %d",
...
{
	HrcNode_ptr iter;
	for (iter = hrc; !HrcNode_is_root(iter);
