...
HrcNode_ptr Compile_hrc_from_parse_tree(const NuSMVEnv_ptr env,
  ...
  /* now generate the HrcNodes */
  {
    ...
    /* fills the hrc */
    Compile_fill_hrc_from_mod_body(env, nomgr, cdr(mod_def), /* body */
                                   hrc_result, mod_defs);
  }
  ...
  for (decls = mod_body; Nil != decls; decls = cdr(decls)) {
    ...
    case DEFINE: {
      ...
      for (decl_iter = car(cur_decl); decl_iter != Nil;
        ...
        HrcNode_add_define(
            hrc_result, NodeMgr_cons(nomgr, car(cur_define), cdr(cur_define)));
      }
      break;
    }
    ...
    case IVAR: {
      ...
      for (decl_iter = car(cur_decl); decl_iter != Nil;
        ...
        compile_fill_hrc_add_var(env, nomgr, car(cur_var), cdr(cur_var),
                                 node_get_type(cur_decl), hrc_result, mod_defs);
      }
      break;
    }
...
static void compile_fill_hrc_invalidate(HrcNode_ptr hrc) {
  HrcNode_ptr iter;
  for (iter = hrc; !HrcNode_is_root(iter); iter = HrcNode_get_parent(iter)) {
