...
static void print_usage(StreamMgr_ptr streams, int argc, char **argv)
{
	...
	StreamMgr_print_error(
		...
		"\t\t name _LTL#_SPECF_N_.\n");
	StreamMgr_print_error(
		streams, "\t<ifile>\t is the file from which the LTL Formula "
			 "to be translated\n\t\t is read.\n");
	StreamMgr_print_error(
		streams,
		"\t<ofile>\t is the file in which the SMV code "
...
int main(int argc, char **argv)
{
	...
	/* check the first argument -- a zero or positive number */
	{
		...
		if (r != (unsigned int)r) {
			StreamMgr_print_error(
				streams,
				"Error: \"%ld\" is a too large number.\n", r);
			exit(1);
		}
	...
	/* open and parse from the input file */
	{
		...
		if ((char *)NULL == argv[i]) {
			StreamMgr_print_error(
				streams,
				"Error: ltl2smv : the input file is not specified.\n");
			exit(1);
		}

		if (Parser_ReadLtlExprFromFile(env, argv[i])) {
			StreamMgr_print_error(
				streams, "Paring Error in LTL expression.\n");
			exit(1);
		}
