...
void Mc_Init(NuSMVEnv_ptr env)
{
	...
	Cmd_CommandAdd(env, "_language_emptyness", CommandLanguageEmptiness, 0,
		       true);
...
void Mc_End(NuSMVEnv_ptr env)
{
	...
	if (!status) {
		ErrorMgr_ptr err_mgr =
			NuSMVEnv_get_value(env, ENV_ERROR_MANAGER);
		ErrorMgr_internal_error(
			err_mgr, "Error while deinitializing commands in MC\n");
	}
}
	...
	if (formula != NIL(char)) {
		...
		CATCH(errmgr)
		{
			if (opt_use_coi_size_sorting(opts)) {
				FlatHierarchy_ptr hierarchy = FLAT_HIERARCHY(
					NuSMVEnv_get_value(env,
							   ENV_FLAT_HIERARCHY));
				PropDb_ordered_verify_all_type(
					prop_db, hierarchy, Prop_Ctl);
			} else
...
static int UsageCheckCtlSpec(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(streams,
			      "usage: check_ctlspec [-h] [-m | -o file] [-n "
	...
	StreamMgr_print_error(
		streams,
		"   -P \"name\"\t\tChecks only the SPEC with the given name\n");
	return 1;
}
...
int CommandCheckInvar(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "hbt:k:j:e:s:mo:n:p:P:")) != EOF) {
		...
		case 's':
			...
			if (0 == strcmp("forward", util_optarg)) {
				...
				options.strategy = BACKWARD;
				   ...
				   strcmp("forward-backward", util_optarg)) {
				...
				StreamMgr_print_error(
					streams,
					"Wrong strategy specified; you must choose between "
		...
		case 'j':
			...
			if (0 == strcmp("steps", util_optarg)) {
				...
				StreamMgr_print_error(
					streams,
					"Wrong heuristic specified; you must "
		...
		case 'k': {
			...
			if (res <= 0) {
				StreamMgr_print_error(
					streams,
					"You must specify a valid integer number as BMC k\n");
				return 1;
			}
			break;
		}
	...
	if (used_e) {
		if ((FORWARD_BACKWARD != options.strategy) &&
			...
			StreamMgr_print_error(
				streams,
				"Unable to use an heuristic without "
	...
	if (used_j) {
		if (used_s && BDD_BMC != options.strategy) {
			StreamMgr_print_error(
				streams,
				"Unable to use an heuristic for bdd-bmc "
	...
	if (used_k) {
		if (used_s && BDD_BMC != options.strategy) {
			StreamMgr_print_error(
				streams,
				"Unable to set BMC k while not in bdd-bmc strategy!\n");
			status = 1;
			goto check_invar_exit;
		}
	}
...
static int UsageCheckInvar(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"usage: check_invar [-h] [-m| -o file] [-s strategy] [-e "
...
#if NUSMV_HAVE_SAT_SOLVER
	StreamMgr_print_error(streams,
			      "   -t \t\t\tWhen using the mixed BDD and BMC "
			      "approach specify the heuristic threshold\n");
	StreamMgr_print_error(streams,
			      "   -k \t\t\tWhen using the mixed BDD and BMC "
	...
	StreamMgr_print_error(
		...
		"   -o file\t\tWrites the generated output to \"file\".\n");
	StreamMgr_print_error(streams,
			      "   -n number\t\tchecks only the INVARSPEC "
...
static int UsageCheckCompute(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		...
		"   -o file\t\tWrites the generated output to \"file\".\n");
	StreamMgr_print_error(streams,
			      "   -n number\t\tConsiders only the compute "
			      "expression with the given index number.\n");
	StreamMgr_print_error(
		streams,
		"   -p \"compute-expr\"\tComputes the given expression.\n");
	StreamMgr_print_error(streams,
			      "   -P name\t\tConsiders only the compute "
...
static int UsageCompute(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: compute [-h] [-m | -o file] [-n number | -p "
			 "\"compute-expr\" | -P \"name\"]\n");
	StreamMgr_print_error(streams,
			      "   -h \t\t\tPrints the command usage.\n");
	StreamMgr_print_error(
...
static int UsageCheckPslSpec(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(streams,
			      "usage: check_pslspec [-h] [-m | -o file] [-n "
...
static int UsageLanguageEmptiness(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(streams,
			      "usage: language_emptyness [-h] [-v] [-a]\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "   -v \t\tPrints some information on the "
			      "list of initial fair states.\n");
	StreamMgr_print_error(
		streams,
		"   -a \t\tChecks all initial states for being fair states.\n");
	StreamMgr_print_error(streams,
			      "      \t\tOtherwise checks for the existence "
	...
	if (formula != NIL(char)) {
		...
		CATCH(errmgr)
		{
			if (opt_use_coi_size_sorting(opts)) {
				...
				PropDb_ordered_verify_all_type(
					prop_db, hierarchy, Prop_Compute);
			} else
