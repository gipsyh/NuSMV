...
/*!
  ...
  \command_args{[-h] [-m | -o output-file] [-n number | -p "ctl-expr [IN
  context]" | -P "name"]}
...
int CommandCheckCtlSpec(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (useMore || (char *)NULL != dbgFileName) {
    if (OUTCOME_SUCCESS !=
      ...
      status = 1;
      goto check_ctlspec_exit;
    }
  }

  if (formula != NIL(char)) {
    SymbTable_ptr st = SYMB_TABLE(NuSMVEnv_get_value(env, ENV_SYMB_TABLE));
    prop_no = PropDb_prop_parse_and_add(prop_db, st, formula, Prop_Ctl, Nil);
    if (prop_no == -1) {
      status = 1;
      goto check_ctlspec_exit;
    }
    ...
    if (Prop_check_type(PropDb_get_prop_at_index(prop_db, prop_no), Prop_Ctl) !=
        0) {
      status = 1;
    } else {
      CATCH(errmgr) { PropDb_verify_prop_at_index(prop_db, prop_no); }
...
static int UsageCheckCtlSpec(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams, "usage: check_ctlspec [-h] [-m | -o file] [-n "
                                 "number | -p \"ctl-expr\" | -P \"name\"]\n");
  ...
  StreamMgr_print_error(
      streams, "   -p \"ctl-expr\"\tChecks only the given CTL formula.\n");
  StreamMgr_print_error(
      streams, "   -P \"name\"\t\tChecks only the SPEC with the given name\n");
  return 1;
}
...
/*!
  ...
  <dl>
    ...
    <dd> When using the mixed BDD and BMC approach specify the heuristic
  threshold. <dt> <tt>-k <i>number</i></tt> <dd> When using the mixed BDD and
  BMC approach specify the BMC max k. <dt> <tt>-j <i>heuristic</i></tt> <dd>
  Force the switch heuristic for the BDD-BMC strategy. <dt> <tt>-p "invar-expr
  [IN context]"</tt> <dd> The command line specified invariant formula to be
  verified. <tt>context</tt> is the module instance name which the variables in
  <tt>invar-expr</tt> must be evaluated in. <dt> <tt>-P name</tt> <dd> Checks
  the INVARSPEC with name <tt>name</tt> in the property database.
  </dl>
*/
...
int CommandCheckInvar(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  while ((c = util_getopt(argc, argv, "hbt:k:j:e:s:mo:n:p:P:")) != EOF) {
    ...
    case 's':
      if (used_s) {
        StreamMgr_print_error(streams,
                              "You cannot specify -s more than once!\n");
      }
      used_s = true;
      ...
      if (0 == strcmp("forward", util_optarg)) {
        ...
        StreamMgr_print_error(
            streams,
            "Wrong strategy specified; you must choose between {'forward', "
        ...
        StreamMgr_print_error(
            streams, "Wrong strategy specified; you must choose between "
    ...
    case 'e':
      if (used_e) {
        StreamMgr_print_error(streams,
                              "You cannot specify -e more than once!\n");
      }
      used_e = true;
      ...
      if (0 == strcmp("zigzag", util_optarg)) {
        ...
        StreamMgr_print_error(streams,
                              "Wrong heuristic specified; you must choose "
                              "between {'zigzag', 'smallest'}.\n");
        return 1;
      }
      break;
    ...
    case 'j':
      if (used_j) {
        StreamMgr_print_error(streams,
                              "You cannot specify -j more than once!\n");
      }
      used_j = true;
      ...
      if (0 == strcmp("steps", util_optarg)) {
        ...
        StreamMgr_print_error(streams, "Wrong heuristic specified; you must "
                                       "choose between {'steps', 'size'}.\n");
        return 1;
      }
      break;
    ...
    case 't': {
      ...
      if (res <= 0) {
        StreamMgr_print_error(streams,
                              "You must specify a valid number as threshold\n");
        return 1;
      }
      break;
    }
    case 'k': {
      ...
      if (res <= 0) {
        StreamMgr_print_error(
            streams, "You must specify a valid integer number as BMC k\n");
        return 1;
      }
      break;
    }
  ...
  if ((BDD_BMC == options.strategy) &&
      (Bmc_check_if_model_was_built(env, errstream, false)))
    return 1;

  if (used_e) {
    if ((FORWARD_BACKWARD != options.strategy) &&
        (BDD_BMC != options.strategy)) {
      StreamMgr_print_error(streams, "Unable to use an heuristic without "
                                     "forward-backward or bdd-bmc strategy.\n");
      status = 1;
  ...
  if (used_j) {
    if (used_s && BDD_BMC != options.strategy) {
      StreamMgr_print_error(streams, "Unable to use an heuristic for bdd-bmc "
                                     "while not in bdd-bmc strategy!\n");
      status = 1;
      goto check_invar_exit;
    }
  }
  ...
  if (used_t) {
    if (used_s && BDD_BMC != options.strategy) {
      StreamMgr_print_error(streams,
                            "Unable to use an heuristic threshold for bdd-bmc "
                            "while not in bdd-bmc strategy!\n");
      status = 1;
      goto check_invar_exit;
    }
  ...
  if (used_k) {
    if (used_s && BDD_BMC != options.strategy) {
      StreamMgr_print_error(
          streams, "Unable to set BMC k while not in bdd-bmc strategy!\n");
      status = 1;
      goto check_invar_exit;
    }
  }
  ...
  if (formula != NIL(char)) {
    SymbTable_ptr st = SYMB_TABLE(NuSMVEnv_get_value(env, ENV_SYMB_TABLE));
    prop_no = PropDb_prop_parse_and_add(prop_db, st, formula, Prop_Invar, Nil);
    if (prop_no == -1) {
      status = 1;
      goto check_invar_exit;
    }
...
#if NUSMV_HAVE_SAT_SOLVER
  StreamMgr_print_error(streams,
                        "usage: check_invar [-h] [-m| -o file] [-s strategy] "
                        "[-e heuristic] [-t number] [-k number] [-j heuristic] "
                        "[-n number | -p \"invar-expr\" | -P \"name\"]\n");
#else
  StreamMgr_print_error(
      streams, "usage: check_invar [-h] [-m| -o file] [-s strategy] [-e "
               "heuristic] [-n number | -p \"invar-expr\" | -P \"name\"]\n");
#endif
  StreamMgr_print_error(streams, "   -h \t\t\tPrints the command usage.\n");
#if NUSMV_HAVE_SAT_SOLVER
  StreamMgr_print_error(
      streams,
      "   -s \t\t\tForce the analysis strategy (Overrides options). Possible "
      "values are {'forward', 'backward', 'forward-backward', 'bdd-bmc'}\n");
#else
  StreamMgr_print_error(
      streams,
      "   -s \t\t\tForce the analysis strategy (Overrides options). Possible "
      "values are {'forward', 'backward', 'forward-backward'}\n");
#endif
  StreamMgr_print_error(
      streams,
      "   -e \t\t\tForce the heuristic in case of forward-backward strategy "
      "(Overrides options). Possible values are {'zigzag', 'smallest'}\n");
#if NUSMV_HAVE_SAT_SOLVER
  StreamMgr_print_error(streams, "   -t \t\t\tWhen using the mixed BDD and BMC "
                                 "approach specify the heuristic threshold\n");
  StreamMgr_print_error(streams, "   -k \t\t\tWhen using the mixed BDD and BMC "
                                 "approach specify the BMC max k\n");
  StreamMgr_print_error(streams,
                        "   -j \t\t\tSpecify the heuristic used to switch from "
                        "BDD to BMC. Possible values are {'steps', 'size'}\n");
#endif
  StreamMgr_print_error(
      streams, "   -m \t\t\tPipes output through the program specified by\n");
  StreamMgr_print_error(
      streams, "      \t\t\tthe \"PAGER\" shell variable if defined,\n");
  StreamMgr_print_error(
      streams, "      \t\t\telse through the UNIX command \"more\".\n");
  StreamMgr_print_error(
      streams, "   -o file\t\tWrites the generated output to \"file\".\n");
  StreamMgr_print_error(streams, "   -n number\t\tchecks only the INVARSPEC "
                                 "with the given index number.\n");
  StreamMgr_print_error(
      streams,
      "   -p \"invar-expr\"\tchecks only the given invariant formula.\n");
  StreamMgr_print_error(
      streams,
      "   -P \"name\"\t\tchecks only the INVARSPEC with the given name.\n");
  return 1;
}
...
static int UsageCheckCompute(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams,
                        "usage: check_compute [-h] [-m | -o file] [-n number | "
                        "-p \"compute-expr\" | -P \"name\"]\n");
  StreamMgr_print_error(streams, "   -h \t\t\tPrints the command usage.\n");
  StreamMgr_print_error(
      streams, "   -m \t\t\tPipes output through the program specified by\n");
  StreamMgr_print_error(
  ...
  StreamMgr_print_error(
      streams, "   -o file\t\tWrites the generated output to \"file\".\n");
  StreamMgr_print_error(streams, "   -n number\t\tConsiders only the compute "
                                 "expression with the given index number.\n");
  StreamMgr_print_error(
      streams, "   -p \"compute-expr\"\tComputes the given expression.\n");
  StreamMgr_print_error(streams, "   -P name\t\tConsiders only the compute "
...
/*!
  ...
  This command is deprecated. It has been substituted by the command
  check_compute.
...
static int UsageCompute(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams,
                        "usage: compute [-h] [-m | -o file] [-n number | -p "
                        "\"compute-expr\" | -P \"name\"]\n");
  StreamMgr_print_error(streams, "   -h \t\t\tPrints the command usage.\n");
  StreamMgr_print_error(
      streams, "   -m \t\t\tPipes output through the program specified by\n");
...
static int UsageCheckPslSpec(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams, "usage: check_pslspec [-h] [-m | -o file] [-n "
...
int CommandLanguageEmptiness(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (get_oreg_justice_emptiness_bdd_algorithm(opts) ==
    ...
    if (allinit) {
      ...
      StreamMgr_print_error(
          streams, "Forward Emerson-Lei cannot be used to check whether all "
  ...
  StreamMgr_print_output(streams, "############################################"
                                  "##########################\n");
  Mc_CheckLanguageEmptiness(env, BDD_FSM(NuSMVEnv_get_value(env, ENV_BDD_FSM)),
                            allinit, verbose);
  StreamMgr_print_output(streams, "############################################"
                                  "##########################\n");
...
static int UsageLanguageEmptiness(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
  StreamMgr_print_error(streams, "   -v \t\tPrints some information on the "
                                 "list of initial fair states.\n");
  StreamMgr_print_error(
      streams, "   -a \t\tChecks all initial states for being fair states.\n");
  StreamMgr_print_error(streams, "      \t\tOtherwise checks for the existence "
                                 "of at least a fair initial state.\n");
  return 1;
}
...
static int mc_cmd_check_compute(NuSMVEnv_ptr env, int argc, char **argv,
  ...
  if (formula != NIL(char)) {
    SymbTable_ptr st = SYMB_TABLE(NuSMVEnv_get_value(env, ENV_SYMB_TABLE));
    prop_no =
        PropDb_prop_parse_and_add(prop_db, st, formula, Prop_Compute, Nil);
    if (prop_no == -1) {
      status = 1;
      goto check_compute_exit;
    }
