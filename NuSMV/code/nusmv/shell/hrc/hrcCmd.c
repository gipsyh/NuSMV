...
int CommandHrcWriteModel(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	StreamMgr_print_error(
		streams, "Warning: %s has been deprecated in favor of %s\n",
	...
	CATCH(errmgr)
	{
		rv = Hrc_dump_model(env, HRC_DUMP_FORMAT_SMV, ofileid,
				    append_suffix, true);
...
int CommandHrcDumpModel(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (opt_verbose_level_gt(opts, 0)) {
		Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
		Logger_log(logger,
			   "Dumping hrc model in format '%s' into file '%s'...",
			   dump_format,
			   (char *)NULL == output_fname ? "stdout" :
							  output_fname);
	}
	...
	Hrc_dump_model(env, dump_format_enum, ofileid, append_suffix,
		       use_indent);
...
static int UsageHrcDumpModel(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"usage: hrc_dump_model [-h]  -f format [-o filename] [-d][-i]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
		"  -f format\tDumps in the given format (debug, smv or xml)\n");
	StreamMgr_print_error(streams,
			      "  -o filename\tDumps output to \"filename\"\n");
	StreamMgr_print_error(
		streams, "  -d \t\tRenames every module name appending the "
			 "suffix \"_hrc\"\n");
