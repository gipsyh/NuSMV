...
int CommandHrcWriteModel(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  StreamMgr_print_error(streams,
                        "Warning: %s has been deprecated in favor of %s\n",
  ...
  if (!NuSMVEnv_has_value(env, ENV_HRC_HIERARCHY)) {
    StreamMgr_print_output(
        streams, "The HRC structure is not available, cannot dump the model\n");
    return 1;
  }
...
static int UsageHrcDumpModel(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
  StreamMgr_print_error(
      streams, "  -f format\tDumps in the given format (debug, smv or xml)\n");
  StreamMgr_print_error(streams,
