...
void Compile_init_cmd(NuSMVEnv_ptr env)
{
	...
	Cmd_CommandAdd(env, "flatten_hierarchy", CommandFlattenHierarchy, 0,
		       false);
	Cmd_CommandAdd(env, "show_vars", CommandShowVars, 0, true);
	Cmd_CommandAdd(env, "encode_variables", CommandEncodeVariables, 0,
		       false);
	Cmd_CommandAdd(env, "build_model", CommandBuildModel, 0, false);
	Cmd_CommandAdd(env, "build_flat_model", CommandBuildFlatModel, 0,
		       false);
	Cmd_CommandAdd(env, "build_boolean_model", CommandBuildBooleanModel, 0,
		       false);
	Cmd_CommandAdd(env, "write_order", CommandWriteOrder, 0, true);
	Cmd_CommandAdd(env, "print_iwls95options", CommandIwls95PrintOption, 0,
		       true);

	/* this is deprecated in 2.4 */
	Cmd_CommandAdd(env, "print_clusterinfo", CommandCPPrintClusterInfo, 0,
		       true);
	...
	Cmd_CommandAdd(env, "get_internal_status", CommandGetInternalStatus, 0,
		       true);
	...
	Cmd_CommandAdd(env, "show_dependencies", CommandShowDependencies, 0,
		       true);
}
...
int CommandProcessModel(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (partition_method != NIL(char)) {
		if (TransType_from_string(partition_method) !=
		    TRANS_TYPE_INVALID) {
			set_partition_method(
				opts, TransType_from_string(partition_method));
		} else {
			StreamMgr_print_error(
				streams,
...
static int UsageProcessModel(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"usage: process_model [-r] [-h] [-i model-file] [-m method]\n");
	StreamMgr_print_error(streams,
			      "   -h \t\t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "   -f \t\t\tForces model construction.\n");
	StreamMgr_print_error(
		streams,
	...
	StreamMgr_print_error(streams,
			      "   -m method\t\tUses \"method\" as partition "
...
int CommandFlattenHierarchy(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (cmp_struct_get_flatten_hrc(cmps)) {
		StreamMgr_print_error(
			streams, "The hierarchy has already been flattened.\n");
		return 1;
	}
	...
	if (cmp_struct_get_hrc_built(cmps)) {
		...
		StreamMgr_print_error(
			streams,
			"At this stage you can dump the (partial) HRC, or use the\n");
...
int CommandEncodeVariables(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "i:hn")) != EOF) {
		...
		case 'i':
			...
			input_order_file_name =
				ALLOC(char, strlen(util_optarg) + 1);
	...
	if (cmp_struct_get_encode_variables(cmps)) {
		StreamMgr_print_error(
			streams, "The variables appear to be already built.\n");
		goto encode_variables_free;
	}
	...
	if (!opt_reorder(opts) && !is_default_order_file(opts) &&
		...
		res = BddEnc_write_var_ordering(
			bdd_enc, get_output_order_file(opts), dump_type);
...
static int UsageEncodeVariables(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(streams,
			      "usage: encode_variables [-h] [-i <file>]\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
		"   -i <file> \tReads variable ordering from file <file>.\n");
	StreamMgr_print_error(streams,
			      "   -n \t\tEncode with BDDs only if the model "
...
int CommandBuildModel(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* The check is not needed since lazy commit will be performed */
		 ...
		 Compile_check_if_model_layer_is_in_bddenc(env, errstream)) {
		/* [SM] Special case
...
static int UsageBuildModel(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\t\tto be used in the following image computations.\n");
	StreamMgr_print_error(streams,
...
static int UsageBuildBooleanModel(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams, "   -f \t\tForces the boolean model construction.\n");
	return 1;
}
...

	{
		...
		if (NULL == order_output_fname) {
         ...
         and keeps the ownership. */
			order_output_fname =
				util_strsav(get_output_order_file(opts));
		}

		retval = BddEnc_write_var_ordering(bdd_enc, order_output_fname,
						   dump_type);
	}
...
static int UsageWriteOrder(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "usage: write_order [-h] | [-b] [(-o | -f) <file>]\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
	...
	StreamMgr_print_error(
		streams, "   -o <file>\tWrites ordering to file <file>.\n");
	StreamMgr_print_error(streams,
			      "   -f <file>\tThe same of option -o. "
...
static int UsagePrintFsmStats(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: print_fsm_stats [-h] [-m] [-o file] [-p]\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage\n");
	StreamMgr_print_error(
		streams,
	...
	StreamMgr_print_error(
		streams,
		"   -p \t\tPrints out the normalized predicates the FSM\n");
	StreamMgr_print_error(
		streams,
		"      \t\tconsists of. A normalized predicate is a boolean\n");
	StreamMgr_print_error(
		streams,
		"      \t\texpression without boolean sub-expressions.\n");
	StreamMgr_print_error(streams,
...
static int UsageGoBmc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(streams,
			      "   -f \t\tForces the model contruction\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage\n");
	return 1;
}
...
int CommandWriteModelFlat(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	CATCH(errmgr)
	{
		...
		FlatHierarchy_ptr hierarchy = FLAT_HIERARCHY(
			NuSMVEnv_get_value(env, ENV_FLAT_HIERARCHY));
		...
		if (!obfuscated) {
			...
			anonymizer = NODE_ANONYMIZER_BASE(
				NodeAnonymizerST_create(env, NULL, 1000, st));

			Compile_WriteObfuscatedFlattenModel(
				env, ofileid, st,
				SymbTable_get_class_layer_names(
					st, (const char *)NULL),
				"MODULE main", hierarchy, print_map, true,
				anonymizer);
...
static int UsageWriteModelFlat(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"usage: write_flat_model [-h] [-A] [-m] [-o filename]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "  -o filename\tWrites output to \"filename\"\n");
	StreamMgr_print_error(streams,
			      "  -A Write the model using variables and "
...
static int UsageWriteModelFlatUdg(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: write_flat_model_udg [-h] [-o filename]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "  -o filename\tWrites output to \"filename\"\n");
	return 1;
...
static int UsageWriteModelFlatBool(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: write_boolean_model [-h] [-o filename]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams, "  -o filename\tWrites output to \"filename\".\n");
	return 1;
}
...
int CommandWriteCoiModel(NuSMVEnv_ptr env, int argc, char **argv)
{
...

	{
		...
		if (prop_no != -1) {
			...
			if (NIL(char) != file_name) {
				output_file =
					OStream_create_file(file_name, false);
			...
			if (print_coi) {
				Compile_write_coi_prop(env, cone, props,
						       output_file);
			} else {
		...
		/* Write COI model for all property in the DB */
         ...
         all properties COI union */
			if (global_coi_model) {
				if (NIL(char) != file_name) {
					output_file = OStream_create_file(
						file_name, false);
					if (OSTREAM(NULL) == output_file) {
						StreamMgr_print_error(
							streams,
				...
				Compile_write_global_coi_fsm(
					env, hierarchy, prop_type, output_file);
				...
				res = Compile_write_properties_coi(
					env, hierarchy, prop_type, print_coi,
					file_name);
				if (res != 0)
					goto write_coi_free;
			}
...
static int UsageWriteCoiModel(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(streams,
			      "usage: write_coi_model [-h] [-o filename]"
			      " [-n \"index\" | -p \"expr\"| -P \"name\"] "
			      "[-c | -l | -i | -s | -q] [-C] [-g]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
		"  -n idx\tWrites COI model for property with index \"idx\"\n");
	StreamMgr_print_error(
		streams, "  -p expr\tWrites COI model for the given property. "
			 "The property type has to be specified\n");
	StreamMgr_print_error(
		streams,
		"  -P name\tWrites COI model for property named \"name\"\n");
	StreamMgr_print_error(
		streams,
		"  -C \t\tOnly print the list of variables in the COI\n");
	StreamMgr_print_error(
		streams,
		"  -g \t\tGroups properties COI and generates one model\n");
	StreamMgr_print_error(
		streams,
		"  -i \t\tWrite COI model only for INVAR properties\n");
	StreamMgr_print_error(
		streams, "  -l \t\tWrite COI model only for LTL properties\n");
	StreamMgr_print_error(
		streams, "  -s \t\tWrite COI model only for PSL properties\n");
	StreamMgr_print_error(
		streams,
		"  -q \t\tWrite COI model only for COMPUTE properties\n");
	StreamMgr_print_error(
		streams, "  -c \t\tWrite COI model only for CTL properties\n");
	StreamMgr_print_error(
		streams, "  -o filename\tWrites output to \"filename\".\n");
	return 1;
}
...
int CommandShowDependencies(NuSMVEnv_ptr env, int argc, char **argv)
{
...

	{
		...
		int parse_result = Parser_ReadNextExprFromString(
			env, next_formula_str, &expression);
		...
		if (!TypeChecker_is_expression_wellformed(type_checker,
							  expression, Nil)) {
			goto show_deps_usage;
		}
...
static int UsageShowDependencies(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "show_dependencies [-h] [-k bound] -e <next_expr>\n");
	StreamMgr_print_error(
		streams,
		"\t-k bound\tStop searching dependencies at step \"bound\"\n");
	StreamMgr_print_error(
		streams,
