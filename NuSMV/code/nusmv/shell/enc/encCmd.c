...
{
	Cmd_CommandAdd(env, "clean_sexp2bdd_cache", CommandCleanSexp2BDDCache,
		       0, true);
...
static int CommandPrintFormula(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	{ /* parsing given formula */
		...
		parse_result = Parser_ReadCmdFromString(
			env, arc, arv, "SIMPWFF ", ";", &constr);
	}

	if (parse_result != 0 || constr == Nil) {
		StreamMgr_print_error(
			streams, "Parsing error: expected an expression.\n");
		goto CommandPrintFormula_fail;
	}
...
static int UsagePrintFormula(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: print_formula [-h] | [-v] | [-f] <expr>\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams, "   -v \t\tPrints explicit models of the formula.\n");
	StreamMgr_print_error(
		streams,
...
static int CommandDumpExpr(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	{ /* checks format */
		unsigned int i;
		for (i = 0; i < sizeof(supported_formats) /
					sizeof(supported_formats[0]);
		     ++i) {
			if (Utils_strcasecmp(supported_formats[i].name,
					     str_format) == 0) {
		...
		if (DUMP_FORMAT_INVALID == format) {
			StreamMgr_print_error(
				streams, "Invalid format. Valid formats are: ");
			for (i = 0; i < sizeof(supported_formats) /
						sizeof(supported_formats[0]);
			     ++i) {
				StreamMgr_print_error(
	...
	res = BddEnc_dump_expr(
		BDD_ENC(NuSMVEnv_get_value(env, ENV_BDD_ENCODER)), parsed_expr,
		str_constr, format, outfile);
...
static int UsageDumpExpr(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"usage: dump_expr [-h] | -e <expr> -f <format> [-o fname]\n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "   -e <expr> \tExpression to be dumped.\n");
	StreamMgr_print_error(
		streams,
		"   -f <format> \tThe format of dumping (e.g. 'dot').\n");
	StreamMgr_print_error(streams,
			      "   -o <fname> \tThe output file name.\n");
	return 1;
}
