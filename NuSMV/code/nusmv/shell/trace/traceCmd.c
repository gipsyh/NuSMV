...
void traceCmd_init(NuSMVEnv_ptr env)
{
	...
	Cmd_CommandAdd(env, "execute_partial_traces",
		       CommandExecutePartialTraces, 0, true);
}
...
int CommandShowTraces(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (traceno == 0) {
		StreamMgr_print_error(
			streams, "There are no traces currently available.\n");
		return 0;
	}
	...
	if (all == false) {
		if (argc != util_optind) {
			res = traceCmd_parse_slice(env, argv[util_optind],
						   &trace, &from_state,
						   &to_state);
	...
	if (number == true) {
		...
		res = TraceMgr_show_traces(tm, plugin_index, all, trace,
					   trace_opt, traceno, from_state,
					   to_state);
	}
...
static int UsageShowTraces(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "  -a \t\tPrints all the currently stored traces.\n");
	StreamMgr_print_error(
		streams,
		"  -t \t\tPrints only the total number of currently stored traces.\n");

	StreamMgr_print_error(
		streams, "  -A \t\tPrints the trace in anonimized form.\n");
	...
	StreamMgr_print_error(streams,
			      "  -p plugin-no\tUses the specified trace "
	...
	StreamMgr_print_error(
		streams,
		"You can control the behavior of this command even through the "
		"variables:\n"
...
static int UsageShowPlugins(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: show_plugins [-h]  [-n  plugin_index | -a]\n");
	StreamMgr_print_error(
		streams, "  -h                Prints the command usage.\n");
	StreamMgr_print_error(
		streams, "  -a                Shows all registered plugins.\n");
	StreamMgr_print_error(
		streams,
...
int CommandReadTrace(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (NIL(char) == filename) {
		...
		if (strcmp(filename, "") == 0) {
			StreamMgr_print_error(
				streams,
				"Error: Input XML file name is empty.\n");
			res = 1;
			goto leave;
		}
	} else {
...
#if NUSMV_HAVE_LIBXML2
	{
		...
		trace = TracePkg_read_trace(env, sexp_fsm, filename,
					    halt_if_undef,
...
static int UsageReadTrace(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"usage: read_trace [-u] [-s] (-h | file_name | -i file_name)\n"
	...
	while ((c = util_getopt(argc, argv, "hvamo:e:")) != EOF) {
		...
		case 'e':
			...
			executor = BASE_TRACE_EXECUTOR(
				TraceMgr_get_complete_trace_executor(
					trace_mgr, engineName));
			if (NULL == executor) {
				StreamMgr_print_error(
					streams,
					"Error: \"%s\" is not a valid executor\n",
					engineName);
				res = UsageExecuteTraces(env);
				goto leave;
			}
...
int CommandExecutePartialTraces(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "hvarmo:e:")) != EOF) {
		...
		case 'e':
			...
			if (restart) {
				...
				StreamMgr_print_error(
					streams,
					"Warning: Option -r is deprecated\n");
			}
	...
	if (PARTIAL_TRACE_EXECUTOR(NULL) != executor ||
		...
		res = TracePkg_execute_partial_traces(env, trace_mgr,
						      output_stream, engineName,
						      verbosity, trace);
		...
		if (TRACE_SEP == *endptr) {
			...
			if (0 == parse_res) {
               ...
               last state. This is not allowed if junk is found */
				...
				StreamMgr_print_error(streams, state_no_err_msg,
						      s);
			}
		}
	} else {
				     ...
				     const boolean is_partial_executor)
{
	NuSMVEnv_ptr const env =
		EnvObject_get_environment(ENV_OBJECT(trace_mgr));
	...
	if (is_partial_executor) {
		registered_executors =
			TraceMgr_get_partial_trace_executor_ids(trace_mgr);
	} else {
		registered_executors =
			TraceMgr_get_complete_trace_executor_ids(trace_mgr);
	}
	...
	if (0 < array_n(registered_executors)) {
		...
		arrayForEachItem(string_ptr, registered_executors, i, tmp)
		{
			...
			if (is_partial_executor) {
				exec_desc =
					TraceMgr_get_partial_trace_executor_desc(
						trace_mgr, id);
			} else {
				exec_desc =
					TraceMgr_get_complete_trace_executor_desc(
						trace_mgr, id);
			}
