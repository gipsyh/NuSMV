...
void traceCmd_init(NuSMVEnv_ptr env) {
  ...
  Cmd_CommandAdd(env, "execute_partial_traces", CommandExecutePartialTraces, 0,
                 true);
}
...
int CommandShowTraces(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  TraceMgr_ptr const tm = TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR));
  int c, res = 0; /* success */
  ...
  while ((c = util_getopt(argc, argv, "hvAatmp:o:d")) != EOF) {
    ...
    case 'p': {
      ...
      if ((strncmp(err_occ[0], "", 1) != 0) || (plugin_index < 0) ||
          (TraceMgr_get_plugin_size(TRACE_MGR(
               NuSMVEnv_get_value(env, ENV_TRACE_MGR))) <= plugin_index)) {
  ...
  if (all == false) {
    if (argc != util_optind) {
      res = traceCmd_parse_slice(env, argv[util_optind], &trace, &from_state,
                                 &to_state);
  ...
  if (number == true) {
    ...
    res = TraceMgr_show_traces(tm, plugin_index, all, trace, trace_opt, traceno,
                               from_state, to_state);
  }
...
static int UsageShowTraces(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      streams, "     \t\tenvironment variable if defined, else through "
               "the UNIX command \"more\".\n");

  StreamMgr_print_error(streams, "  -p plugin-no\tUses the specified trace "
                                 "plugin to explain the trace.\n");

  StreamMgr_print_error(
      streams, "  -o output-file\tWrites the generated output to \"file\".\n");

  StreamMgr_print_error(
      streams, "  trace_number\tThe number of the trace to be printed.\n");

  StreamMgr_print_error(
      streams, "  from_state\tLeft end of the trace slice to be printed.\n");

  StreamMgr_print_error(
      streams, "  to_state\tRight end of the trace slice to printed.\n"
               "  \t\t(if omitted, last step is assumed by default.)\n");
  StreamMgr_print_error(
      streams, "You can control the behavior of this command even through the "
               "variables:\n"
               "traces_regex, traces_show_defines, "
               "traces_show_defines_with_next, traces_hiding_prefix\n");
...
static int UsageShowPlugins(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(streams,
                        "  -a                Shows all registered plugins.\n");
  StreamMgr_print_error(
      streams,
...
int CommandReadTrace(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  while ((c = util_getopt(argc, argv, "husi:")) != EOF) {
    ...
    case 'i':
      ...
      StreamMgr_print_error(streams,
                            "Warning: option -i is deprecated feature.\n");
      break;
  ...
  if (NIL(char) == filename) {
    if (argc == util_optind) {
      StreamMgr_print_error(streams,
                            "Error: Input XML file has to be provided.\n");
      res = 1;
      goto leave;
    }
    if (argc != util_optind + 1) {
      StreamMgr_print_error(
          streams, "Error: Only one input XML file can be specified.\n");
      res = 1;
      goto leave;
    }
    ...
    if (argc > util_optind) {
      StreamMgr_print_error(
          streams, "Error: unknown option is provided to command : %s\n",
          argv[util_optind]);
      res = 1;
      goto leave;
    }
...
#if NUSMV_HAVE_LIBXML2
  {
    ...
    if (TRACE(NULL) == trace) {
      StreamMgr_print_error(
          streams, "Unable to load trace from XML File \"%s\".\n", filename);
      res = 1;
    } else {
      StreamMgr_print_error(
          streams, "Trace is stored at %d index \n",
  ...
  {
    StreamMgr_print_error(streams,
                          "LIBXML2 library is not available on this system.\n");
    StreamMgr_print_error(streams,
                          "Try to recompile %s with the LIBXML2 library.\n",
...
static int UsageReadTrace(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(
      streams, "usage: read_trace [-u] [-s] (-h | file_name | -i file_name)\n"
               "  -h \t Prints the command usage.\n"
  ...
  while ((c = util_getopt(argc, argv, "hvamo:e:")) != EOF) {
    ...
    case 'e':
      ...
      executor = BASE_TRACE_EXECUTOR(
          TraceMgr_get_complete_trace_executor(trace_mgr, engineName));
      if (NULL == executor) {
        StreamMgr_print_error(
            streams, "Error: \"%s\" is not a valid executor\n", engineName);
        res = UsageExecuteTraces(env);
        goto leave;
      }
      break;
...
static int UsageExecuteTraces(NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      streams, "  trace_number\tThe number of the trace to be executed.\n");
...
int CommandExecutePartialTraces(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  while ((c = util_getopt(argc, argv, "hvarmo:e:")) != EOF) {
    ...
    case 'e':
      ...
      executor = TraceMgr_get_partial_trace_executor(
          TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), engineName);
...
static int UsageExecutePartialTraces(NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      streams, "usage: execute_partial_traces [-h] [-v] [-r] [-m | -o file] "
               "[-e engine] [-a | trace_number]\n");
  ...
  StreamMgr_print_error(
      streams, "  -r \t\tPerforms restart on complete states (deprecated).\n");
...
int traceCmd_parse_slice(const NuSMVEnv_ptr env, const char *s, int *trace,
  ...
  if (0 == parse_res) {
      ...
      if (0 == parse_res) {
        if (STATE_SEP == *endptr) {
          ...
          if (0 == parse_res) {
            ...
            StreamMgr_print_error(streams, state_no_err_msg, s);
          }
        }
      } else {
        StreamMgr_print_error(streams, state_no_err_msg, s);
    ...
    StreamMgr_print_error(streams, trace_no_err_msg, s);
  }
...
trace_cmd_print_registered_executors(TraceMgr_ptr const trace_mgr,
  ...
  if (0 < array_n(registered_executors)) {
    StreamMgr_print_error(
        streams, "\t\t(must be one of the following registered executors)\n");
    ...
    StreamMgr_print_error(
        streams, "\t\t(no %s trace executors registered yet.)\n", exec_type);
  }
