...
static inline int isatty(int d)
{
	return 0;
}
#endif
	...
	if (strcmp(fileName, "-") == 0) {
		...
		if (strcmp(mode, "r") == 0) {
			if (OptsHandler_is_option_registered(opt,
							     "open_path")) {
				user_path = OptsHandler_get_string_option_value(
					opt, "open_path");
			}

			if (user_path != NIL(char)) {
				lib_name = CInit_NuSMVObtainLibrary();
				path = ALLOC(char, strlen(user_path) +
							   strlen(lib_name) +
							   10);
				(void)sprintf(path, "%s:%s", user_path,
					      lib_name);
         ...
         */
				realFileName =
					util_file_search(fileName, path, "r");
				FREE(path);
				FREE(lib_name);
			}
		}
		    ...
		    FILE *stream, char *prompt)
{
	...
	array_t *names =
		NIL(array_t); /* initialize so that lint doesn't complain */
...
#if NUSMV_HAVE_LIBREADLINE
		avl_tree *commandTable = (avl_tree *)NuSMVEnv_get_value(
			env, ENV_CMD_COMMAND_TABLE);
...
char *CmdHistorySubstitution(NuSMVEnv_ptr env, char *line, int *changed)
{
	...
	if (NuSMVEnv_has_value(env, ENV_CMD_HISTORY_CHAR)) {
		NuSMVHistChar = PTR_TO_INT(
			NuSMVEnv_get_value(env, ENV_CMD_HISTORY_CHAR));
	}
...
bad_modify:
	...
	if (OptsHandler_is_option_registered(opts, "history_char")) {
		const char *tmp = OptsHandler_get_string_option_value(
			opts, "history_char");
		NuSMVHistChar = tmp[0];
	...
	for (l = line; (*b = *l); l++) {
       ...
       */
			...
			if (*l == NuSMVHistChar) {
				...
				num = getnum(&l);
				if (num > n || num == 0) {
					return (bad_event(streams,
							  n - num + 1));
				}
				b = do_subst(b, array_fetch(char *,
							    cmdHistoryArray,
							    n - num));
			} else {
				...
				if (isdigit(*l)) {
					...
					*l = '\0';
					...
					for (i = n - 1; i >= 0; i--) {
						old = array_fetch(
							char *, cmdHistoryArray,
							i);
						if (strncmp(old, start, len) ==
						    0) {
							b = do_subst(b, old);
							break;
						}
