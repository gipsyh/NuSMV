...
int Cmd_command_not_available(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	StreamMgr_print_error(
		streams, "Error: command %s is currently disabled\n", argv[0]);
...
static int CommandEcho(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (fname != (char *)NULL) {
		...
		if (fout == (FILE *)NULL) {
			...
			StreamMgr_print_error(
				streams,
				"echo: unable to open file %s for writing.\n",
				fname);
			FREE(fname);
...
usage:
	StreamMgr_print_error(
		streams,
		"usage: echo [-h] [-2] [-n] [[-o filename] [-a]] string \n");
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "   -2 \t\tRedirects to the standard error.\n");
	StreamMgr_print_error(
		streams, "   -n \t\tDoes not output the trailing newline.\n");
	StreamMgr_print_error(
		streams,
...
static int CommandMemoryProfile(NuSMVEnv_ptr env, int argc, char **argv)
{
   ...
   */
	...
	c = snprintf(command, sizeof(command),
		     "%s/memoryaccount %s %s/.fmap ./.fmap >%s",
...
#else
	StreamMgr_print_error(streams,
			      "Command not available: " NUSMV_PACKAGE_NAME
			      " has not been ");
...
usage:
	StreamMgr_print_error(streams,
			      "usage: _memory_profile [-h] [-f <filename>]");
	StreamMgr_print_error(streams, "[-p] [-u <units>] <filenames>\n");
	StreamMgr_print_error(streams, "   -h\t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "   -f <file>\tFile to read the purify dump");
	StreamMgr_print_error(streams,
			      " from. The default is \"purify.log\".\n");
	StreamMgr_print_error(streams,
...
usage:
	...
	StreamMgr_print_error(streams, "   -h  Prints the command usage.\n");
	StreamMgr_print_error(
		streams, "   -s  Frees all the used memory before quitting.\n");
...
usage:
	...
	StreamMgr_print_error(streams, "   -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "   num \t\tPrints the last num commands.\n");
	return (1);
}
...
static int CommandHelp(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (argc - util_optind == 0) {
		...
		avl_foreach_item(commandTable, gen, AVL_FORWARD, &key,
				 NIL(char *))
		{
			if ((key[0] == '_') == all) {
...
usage:
	...
	StreamMgr_print_error(
		streams,
		"   -a \t\tPrints help for all internal (hidden) commands.\n");
	StreamMgr_print_error(
		streams, "   -p \t\tDisable the use of a pager like 'more'\n");
	return 1;
}
...
static int CommandSource(NuSMVEnv_ptr env, int argc, char **argv)
{
   ...
   */
	do {
		...
		fp = Cmd_FileOpen(env, argv[lp_file_index], "r", &real_filename,
				  silent);
		if (fp == NULL) {
			FREE(real_filename);
		...
		do {
			...
			if (prompt) {
				char *stmp = ALLOC(
					char,
					strlen(NuSMVCore_get_prompt_string()) +
						1);
			...
			if (CmdFgetsFilec(env, line, MAX_STR, fp,
					  prompt_string) == NULL) {
				...
				if (interactive) {
					...
					status =
						0; /* successful end of 'source' ; loop? */
				}
				break;
			...
			if (interactive && *line != '\0') {
				array_insert_last(char *, cmdHistoryArray,
						  util_strsav(line));
...
usage:
	...
	StreamMgr_print_error(streams, "\t-h Prints the command usage.\n");
	StreamMgr_print_error(
		streams, "\t-p Supplies prompt before reading each line.\n");
	StreamMgr_print_error(streams,
			      "\t-s Silently ignores nonexistent file.\n");
	StreamMgr_print_error(streams,
			      "\t-x Echoes each line as it is executed.\n");
	return 1;
}
	...
	avl_foreach_item(commandTable, gen, AVL_FORWARD, &key, NIL(char *))
	{
		StreamMgr_print_error(
			streams, "==========================================="
				 "===================================\n");
		if (1 == longmaual) {
			int c = snprintf(fname, 1023, "%s/help/%sCmd.txt",
					 lib_name, key);
...
usage:
	...
	StreamMgr_print_error(
		streams, "   -f \t\tPrints the long help for all commands.\n");
	StreamMgr_print_error(
		streams, "      \t\tBy default the short help is printed.\n");
	return 1;
}
