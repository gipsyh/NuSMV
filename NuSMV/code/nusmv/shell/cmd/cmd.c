...
static int apply_alias(NuSMVEnv_ptr env, int *argcp, char ***argvp, int *loop)
{
	...
	for (; *loop < 20; (*loop)++) {
		...
		if (stopit != 0 ||
		    avl_lookup(aliasTable, argv[0], (char **)&alias) == 0) {
			return 0;
		}
		...
		for (i = 0, offset = 0; i < alias->argc; i++, offset++) {
			arg = CmdHistorySubstitution(env, alias->argv[i],
						     &did_subst);
			if (arg == NIL(char)) {
				*argcp = argc;
				*argvp = argv;
		...
		if ((c == '$')) {
			if (findEndDollar == 1) {
				(void)StreamMgr_print_error(
					streams,
					"Cannot have nested $ signs, not found termination\n");
				break;
			}
     ...
     */
		...
		if (!findEndDollar && (endDollarIndex != -1)) {
			...
			if (endDollarIndex > (dollarIndex + 1)) {
				...
				/* if not empty string */
				subStr = ALLOC(char,
					       endDollarIndex - dollarIndex);
				/* copy the variable into another string */
				for (i = 0;
				     i < endDollarIndex - dollarIndex - 1;
				     i++) {
         ...
         */
				if (OptsHandler_is_option_registered(opt,
								     subStr)) {
					value = OptsHandler_get_string_representation_option_value(
						opt, subStr);
             ...
             delimiter) instead of NULL pointer (0x0). */
					...
					if (strchr((char *)value,
						...
						value = variableInterpolationRecur(
							env, value);
						subLen = strlen(value);
           ...
           */
			...
			newCurStr = ALLOC(char, dollarIndex + subLen +
							strlen(curStr) -
							endDollarIndex + 1);
	...
	while (true) {
		...
		if (single_quote || double_quote) {
			StreamMgr_print_error(
				streams, "ignoring unbalanced quote ...\n");
		}
		if (start == p)
			break;
...
static int check_shell_escape(const NuSMVEnv_ptr env, char *p, int *status)
{
	...
	if (OptsHandler_is_option_registered(opt, "shell_char")) {
		const char *tmp =
			OptsHandler_get_string_option_value(opt, "shell_char");
...
static void sigterm(int sig)
{
	const StreamMgr_ptr streams = STREAM_MGR(
		NuSMVEnv_get_value(__sigterm_env__, ENV_STREAM_MANAGER));
	const ErrorMgr_ptr errmgr = ERROR_MGR(
		NuSMVEnv_get_value(__sigterm_env__, ENV_ERROR_MANAGER));
	const OptsHandler_ptr opts = OPTS_HANDLER(
		NuSMVEnv_get_value(__sigterm_env__, ENV_OPTS_HANDLER));
