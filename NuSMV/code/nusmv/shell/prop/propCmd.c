...
void PropPkg_quit_cmd(NuSMVEnv_ptr env) {
  ...
  status =
      status && Cmd_CommandRemove(env, COMMAND_CONVERT_PROPERTY_TO_INVAR_NAME);
...
static int UsageShowProperty(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams,
                        "usage: show_property [-h] [ [[-c | -l | -i | -q] [-u "
                        "| -t | -f]] | [-n index] | [-P name] ] \n");
  ...
  StreamMgr_print_error(
      streams, "  -q \t\tPrints only quantitative properties (COMPUTE).\n");
  StreamMgr_print_error(streams,
                        "  -u \t\tPrints only unchecked properties.\n");
  StreamMgr_print_error(
      streams, "  -t \t\tPrints only those properties found to be true.\n");
  StreamMgr_print_error(
      streams, "  -f \t\tPrints only those properties found to be false.\n");
  StreamMgr_print_error(
      streams, "  -n index\tPrints out the property numbered \"index\".\n");
  StreamMgr_print_error(streams,
  ...
  StreamMgr_print_error(streams, "  -F format\tPrints in given format. Use '-F "
...
int CommandAddProperty(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (NIL(char) != name) {
    ...
    if (PropDb_get_prop_name_index(prop_db, expr_name) >= 0) {
      StreamMgr_print_error(streams, "The property database already contains a"
                                     " property named \"");
...
static int UsageAddProperty(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams,
                        "usage: add_property [-h] [(-c | -l | -i | -s | -q)"
  ...
  StreamMgr_print_error(
      streams, "  -n \"name\"\tSets the name of the property to \"name\"\n");
  return (1);
}
...
int CommandConvertPropertyToInvar(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (0 == cmp_struct_get_read_model(cmps)) {
    StreamMgr_print_error(
        streams, "A model must be read before. Use command read_model\n");
    retval = 1;
    return retval;
  }
...
static int UsageConvertPropertyToInvar(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(
      streams, "For each model property that is convertible to an invarspec,\n"
  ...
  StreamMgr_print_error(
      streams, "  -p \"formula\" \t Convert the given CTL or LTL formula.\n");
