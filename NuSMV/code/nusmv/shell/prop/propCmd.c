...
void PropPkg_quit_cmd(NuSMVEnv_ptr env)
{
	...
	status = status &&
		 Cmd_CommandRemove(env, COMMAND_CONVERT_PROPERTY_TO_INVAR_NAME);
...
/*!
  ...
  with the relative <tt>check</tt> or <tt>add_property<\tt> commands are
  appended to this list. For every property, the following informations are
  displayed: <ul> <li>the identifier of the property (a progressive number);
...
int CommandShowProperty(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "hlciqutfn:vmo:F:sP:")) != EOF) {
		...
		case 'n': {
			if ((type != Prop_NoType) ||
			    (status != Prop_NoStatus) || (prop_no != -1)) {
				if (outFileName != NIL(char))
					FREE(outFileName);
				return (UsageShowProperty(env));
			}
		...
		case 'F': {
			...
			if (strcmp(util_optarg, "help") == 0) {
				StreamMgr_print_error(
					streams, "Available formats are: ");
				for (i = sizeof(formats) / sizeof(formats[0]) -
					 1;
				     i >= 0; --i) {
...

			{
				boolean found = false;
				for (i = 0;
				     i < sizeof(formats) / sizeof(formats[0]);
				     ++i) {
					if (strcmp(util_optarg,
						   formats[i].name) == 0) {
						fmt = formats[i].fmt;
						found = true;
						break;
				...
				if (!found) {
					StreamMgr_print_error(
						streams,
						"Format '%s' is not valid, use '-F help'"
	...
	retval = PropDb_show_property(prop_db, print_props_num, fmt, type,
				      status, prop_no, outstream);
...
static int UsageShowProperty(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: show_property [-h] [ [[-c | -l | -i | -q] [-u "
			 "| -t | -f]] | [-n index] | [-P name] ] \n");
	...
	StreamMgr_print_error(
		streams,
		"  -q \t\tPrints only quantitative properties (COMPUTE).\n");
	StreamMgr_print_error(streams,
	...
	StreamMgr_print_error(
		streams,
		"  -n index\tPrints out the property numbered \"index\".\n");
	StreamMgr_print_error(
		streams,
	...
	StreamMgr_print_error(streams,
			      "  -F format\tPrints in given format. Use '-F "
			      "help' to see available formats.\n");
	StreamMgr_print_error(
		streams, "  -s \t\tPrints the number of stored properties.\n");
	return (1);
}
...
/*!
  ...
  \command_args{[-h] [(-c | -l | -i | -q | -s) -p "formula [IN context]"] [-n
  "name"]}
...
int CommandAddProperty(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (NIL(char) != name) {
		int res = Parser_ReadIdentifierExprFromString(env, name,
							      &expr_name);
		...
		expr_name = CompileFlatten_concat_contexts(env, Nil,
							   car(expr_name));
	...
	res = PropDb_prop_parse_and_add(prop_db, symb_table, prop, type,
					expr_name);
...
static int UsageAddProperty(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: add_property [-h] [(-c | -l | -i | -s | -q)"
	...
	StreamMgr_print_error(
		streams, "  -q \t\tAdds a quantitative (COMPUTE) property.\n");
	StreamMgr_print_error(streams,
			      "  -p \"formula\"\tThe property to be added. "
...
/*!
  \command{check_property} Checks a property into the current list of
  ...
  If no property has been specified via <tt>-n</tt> or <tt>-p</tt> or
  <tt>-P</tt>, then all the properties (of a given type) in the property list
  will be evaluated.
...
static int UsageCheckProperty(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(streams,
			      "usage: check_property [-h]\n"
			      "       [-n number |  -P \"name\"] | [-c | -l "
			      "| -i | -s | -q [-p \"formula\"]]\n");
	StreamMgr_print_error(streams, "  -h \t\t Prints the command usage.\n");
	StreamMgr_print_error(streams,
			      "  -n number \t Checks property number.\n");
	StreamMgr_print_error(streams,
			      "  -P \"name\" \t Checks property name.\n");
	...
	StreamMgr_print_error(streams,
			      "  -q \t\t Checks COMPUTE properties.\n");
	StreamMgr_print_error(streams,
			      "  -p \"formula\"\t Checks the given formula.\n");
	return (1);
}
...
int CommandConvertPropertyToInvar(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "hn:p:P:")) != EOF) {
		...
		case 'n': {
			if (n_specified || p_specified || P_specified)
				return UsageConvertPropertyToInvar(env);
	...
	CATCH(errmgr)
	{
		if (n_specified || P_specified) {
			...
			PROP_DB_FOREACH(prop_db, i)
			{
				...
				ls_status = lsNewEnd(props, (lsGeneric)property,
						     LS_NH);
				nusmv_assert(LS_OK == ls_status);
			}
...
static int UsageConvertPropertyToInvar(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"  -p \"formula\" \t Convert the given CTL or LTL formula.\n");
