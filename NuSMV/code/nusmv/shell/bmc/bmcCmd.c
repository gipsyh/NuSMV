...
static int bmc_CommandCheckPslSpecBmcInc(NuSMVEnv_ptr env, int argc,
					 char **argv);
#endif

static inline int bmc_cmd_gen_solve_ltl_selected_or_all_props(
	const NuSMVEnv_ptr env, const Prop_ptr ltlprop, const int k,
	const int relative_loop, const char *fname,
	const boolean has_to_iter_over_k, const boolean has_to_solve);

static inline int bmc_cmd_gen_solve_ltl_inc_selected_or_all_props(
	const NuSMVEnv_ptr env, const Prop_ptr ltlprop, const int k,
	const int relative_loop, const boolean must_inc_length);

static inline int bmc_cmd_gen_solve_invar_selected_or_all_props(
	const NuSMVEnv_ptr env, const Prop_ptr invarprop, const char *fname,
	const boolean has_to_solve);

static inline int bmc_cmd_gen_solve_invar_een_selected_or_all_props(
	const NuSMVEnv_ptr env, const Prop_ptr invarprop, const int max_k,
	const char *fname, const boolean use_extra_step);

static inline int bmc_cmd_gen_solve_invar_dual_selected_or_all_props(
	NuSMVEnv_ptr const env, Prop_ptr const invarprop, const int max_k,
...
static inline int bmc_cmd_gen_solve_invar_zigzag_selected_or_all_props(
	NuSMVEnv_ptr const env, Prop_ptr const invarprop, const int max_k);

static inline int bmc_cmd_gen_solve_psl_selected_or_all_props(
...
void Bmc_AddCmd(NuSMVEnv_ptr env)
{
	...
	Cmd_CommandAdd(env, "bmc_inc_simulate", Bmc_CommandBmcIncSimulate, 0,
		       true);
	...
	Cmd_CommandAdd(env, "gen_ltlspec_bmc", Bmc_CommandGenLtlSpecBmc, 0,
		       true);
	Cmd_CommandAdd(env, "gen_ltlspec_bmc_onepb",
		       Bmc_CommandGenLtlSpecBmcOnePb, 0, true);
	Cmd_CommandAdd(env, "check_ltlspec_bmc", Bmc_CommandCheckLtlSpecBmc, 0,
		       true);
	...
	Cmd_CommandAdd(env, "check_invar_bmc", Bmc_CommandCheckInvarBmc, 0,
		       true);
	Cmd_CommandAdd(env, "check_pslspec_bmc", bmc_CommandCheckPslSpecBmc, 0,
		       true);

#if NUSMV_HAVE_INCREMENTAL_SAT
	Cmd_CommandAdd(env, "check_ltlspec_bmc_inc",
		       Bmc_CommandCheckLtlSpecBmcInc, 0, true);
	Cmd_CommandAdd(env, "check_invar_bmc_inc", Bmc_CommandCheckInvarBmcInc,
		       0, true);
	Cmd_CommandAdd(env, "check_pslspec_bmc_inc",
		       bmc_CommandCheckPslSpecBmcInc, 0, true);
#endif
				 ...
				 const char *str_loop, const int k)
{
	...
	if (str_loop != (char *)NULL) {
		...
		if (Bmc_Utils_Check_k_l(k, Bmc_Utils_RelLoop2AbsLoop(rel_loop,
								     k)) !=
		    OUTCOME_SUCCESS) {
		...
		*l = Bmc_Utils_ConvertLoopFromString(get_bmc_pb_loop(opts),
						     NULL);
	}
...
int Bmc_CommandBmcSetup(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (Compile_check_if_bool_model_was_built(env, errstream, forced))
		return 1;

	if (cmp_struct_get_bmc_setup(cmps) && !forced) {
		StreamMgr_print_error(
			streams,
			"A call to bmc_setup has already been done.\n");
		return 1;
	}
...
static int UsageBmcSetup(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(streams,
			      "  -f \t\tForces the BMC model to be built.\n");
	return 1;
}
...
int Bmc_CommandBmcSimulate(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (isconstraint) {
		if (time_shift) {
			be_constraints = Bmc_Utils_simple_costraint_from_string(
				be_enc, bdd_enc, strConstr, (Expr_ptr *)NULL);
		} else {
			be_constraints = Bmc_Utils_next_costraint_from_string(
				be_enc, bdd_enc, strConstr, (Expr_ptr *)NULL);
		}
	...
	(void)Bmc_Simulate(env, BE_FSM(NuSMVEnv_get_value(env, ENV_BE_FSM)),
			   bdd_enc, be_constraints, time_shift, steps,
			   printrace, only_changes, mode == Random);
...
static int UsageBmcSimulate(const NuSMVEnv_ptr env)
{
   ...
   * " */
	...
	/* StreamMgr_print_error(streams,  "  -i \t\tEnters simulation's interactive
   * mode.\n"); */
	/* StreamMgr_print_error(streams,  "  -a \t\tDisplays all the state variables
   * (changed and unchanged)\n"); */
	/* StreamMgr_print_error(streams,  "     \t\tin every step of an interactive
   * session.\n"); */
	/* StreamMgr_print_error(streams,  "     \t\tIt works only together with -i
   * option.\n"); */
	StreamMgr_print_error(streams,
...
int Bmc_CommandBmcIncSimulate(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	(void)Bmc_StepWiseSimulation(
		env, BE_FSM(NuSMVEnv_get_value(env, ENV_BE_FSM)), bdd_enc,
		TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), steps,
		be_constraints, time_shift, printrace, only_changes, mode,
		display_all);
...
int Bmc_CommandBmcPickState(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "hc:s:vrai")) != EOF) {
		...
		case 'c':
			...
			break;
		case 's':
			str_label = util_strsav(util_optarg);
			break;
		case 'v':
			verbose = true;
			break;
...
static int UsageBmcPickState(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(streams,
			      "usage: bmc_pick_state [-h] [-v] [-r | -i "
			      "[-a]] [-c \"constr\" | -s trace.state]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
						   ...
						   char **argv)
{
	...
	retval = Bmc_Simulate_bmc_simulate_check_feasible_constraints(
		env, str_constraints, be_constraints, expr_constraints,
		human_readable);
...
static int UsageBmcSimulateCheckFeasibleConstraints(const NuSMVEnv_ptr env)
{
	StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	StreamMgr_print_error(
		streams, "usage: bmc_simulate_check_feasible_constraints [-h] "
			 "[-q] [-c \"next_expr\"] [-c ...]\n");
	StreamMgr_print_error(streams, "  -h \t\t Prints the command usage.\n");
	StreamMgr_print_error(streams,
			      "  -q \t\t Prints the output in compact form.\n");
	StreamMgr_print_error(streams,
			      "  -c next_expr\t Specify one constraint "
			      "whose feasability has to be checked\n"
			      "\t\t (can be used multiple times, order is "
...
int Bmc_CommandGenLtlSpecBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* ----------------------------------------------------------------------- */
	/* Options handling: */
	opt_handling_res = Bmc_cmd_options_handling(env, argc, argv, Prop_Ltl,
						    &ltlprop, &k,
						    &relative_loop, NULL, NULL,
						    &fname, NULL, NULL);
	...
	retval = bmc_cmd_gen_solve_ltl_selected_or_all_props(
		env, ltlprop, k, relative_loop, fname, true, false);
...
static int UsageBmcGenLtlSpec(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\nusage: gen_ltlspec_bmc [-h | -n idx | -p \"formula\" | -P \"name\"] "
	...
	StreamMgr_print_error(
		streams,
		"  -k max_length\tChecks the property using <max_length> value "
		"instead of using the\n\t\tvariable <bmc_length> value.\n");
	StreamMgr_print_error(
		streams,
		"  -l loopback\tChecks the property using <loopback> value "
		"instead of using the\n\t\tvariable <bmc_loopback> value.\n");
	StreamMgr_print_error(
		streams,
		"  -o filename\tUses <filename> as dimacs file instead "
		"of using the\n\t\t\"bmc_dimacs_filename\" variable. "
		"<filename> may contain patterns.\n\n");
...
int Bmc_CommandGenLtlSpecBmcOnePb(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	(void)bmc_cmd_gen_solve_ltl_selected_or_all_props(
		env, ltlprop, k, relative_loop, fname, false, false);
...
static int UsageBmcGenLtlSpecOnePb(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\nusage: gen_ltlspec_bmc_onepb [-h | -n idx | -p \"formula\" | -P "
		"\"name\"] [-k length] [-l loopback]\n\t\t\t [-o filename]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
		"  -n idx\tChecks the LTL property specified with <idx>.\n");
	...
	StreamMgr_print_error(
		streams,
		"  -k length\tChecks the property using <length> value instead "
		"of using the\n\t\tvariable <bmc_length> value.\n");
	StreamMgr_print_error(
		streams,
		"  -l loopback\tChecks the property using <loopback> value "
...
int Bmc_CommandCheckLtlSpecBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* WARNING [MD] ignored error value */
	(void)bmc_cmd_gen_solve_ltl_selected_or_all_props(
		env, ltlprop, k, relative_loop, fname, true, true);
...
static int UsageBmcCheckLtlSpec(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\nusage: check_ltlspec_bmc [-h | -n idx | -p \"formula\" | -P \"name\"] "
		"[-k max_length] [-l loopback]\n\t\t\t [-o <filename>]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
	...
	StreamMgr_print_error(
		...
		"instead of using the\n\t\tvariable <bmc_loopback> value.\n");
	StreamMgr_print_error(streams,
			      "  -o filename\tGenerates dimacs output file "
...
int Bmc_CommandCheckLtlSpecBmcOnePb(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	retval = bmc_cmd_gen_solve_ltl_selected_or_all_props(
		env, ltlprop, k, relative_loop, fname, false, true);
...
static int UsageBmcCheckLtlSpecOnePb(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\nusage: check_ltlspec_bmc_onepb [-h | -n idx | -p \"formula\" | -P "
		"\"name\"] [-k length] [-l loopback]\n\t\t\t [-o filename]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
...
int Bmc_CommandCheckLtlSpecBmcInc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* ----------------------------------------------------------------------- */
	retval = bmc_cmd_gen_solve_ltl_inc_selected_or_all_props(
		env, ltlprop, k, relative_loop, true);
...
static int UsageBmcCheckLtlSpecInc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\nusage: check_ltlspec_bmc_inc [-h | -n idx | -p \"formula\" | "
		"-P \"name\"] [-k max_length] [-l loopback]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
		"  -n idx\tChecks the LTL property specified with <idx> \n"
	...
	StreamMgr_print_error(
		...
		"        \t(using incremental algorithms).\n");
	StreamMgr_print_error(streams,
			      "  -p \"formula\"\tChecks the specified LTL "
			      "property (using incremental algorithms).\n");
	StreamMgr_print_error(
		streams,
...
int Bmc_CommandGenInvarBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* ----------------------------------------------------------------------- */
	/* Options handling: */
	opt_handling_res = Bmc_cmd_options_handling(env, argc, argv, Prop_Invar,
						    &invarprop, NULL, NULL,
						    &algorithm_name, NULL,
						    &fname, NULL, NULL);
	...
	retval = bmc_cmd_gen_solve_invar_selected_or_all_props(
		env, invarprop, fname, !BMC_HAS_TO_SOLVE);
...
int Bmc_CommandCheckInvarBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* ----------------------------------------------------------------------- */
	/* Options handling: */
	opt_handling_res = Bmc_cmd_options_handling(
		env, argc, argv, Prop_Invar, &invarprop, &max_k, NULL,
		&algorithm_name, NULL, &fname, &use_extra_step, NULL);
	...
	use_classic_alg =
		(strcasecmp(algorithm_name, BMC_INVAR_ALG_CLASSIC) == 0);
	FREE(algorithm_name);
	...
	if (use_classic_alg && max_k != -1) {
		StreamMgr_print_error(
			streams,
			"Option -k can be used only when '" BMC_INVAR_ALG_EEN_SORENSSON
	...
	if (use_classic_alg && use_extra_step) {
		StreamMgr_print_error(
			streams,
			"Option -e can be used only when '" BMC_INVAR_ALG_EEN_SORENSSON
			"' algorithm is selected.\n");
		if (fname != (char *)NULL)
			FREE(fname);
		return 1;
	...
	if (use_classic_alg) {
		res = bmc_cmd_gen_solve_invar_selected_or_all_props(
			env, invarprop, fname, BMC_HAS_TO_SOLVE);
	} else {
		res = bmc_cmd_gen_solve_invar_een_selected_or_all_props(
			env, invarprop, max_k, fname, use_extra_step);
	}
...
static int UsageBmcCheckInvar(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"\nusage: check_invar_bmc [-h | -n idx | -p \"formula\" | -P "
		"\"name\"] [-k max_len] [-s] [-a alg] [-o filename]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(
		streams,
		"  -n idx\tChecks the INVAR property specified with <idx>.\n");
	...
	StreamMgr_print_error(
		...
		"\t\tUse only when " BMC_INVAR_ALG_EEN_SORENSSON
		" algorithm is selected.\n"
		"\t\tIf not specified, variable bmc_length is taken.\n");
	StreamMgr_print_error(
		streams,
		"  -e \t\tPerforms an extra step for finding a proof\n"
		"\t\tCan be used only with the " BMC_INVAR_ALG_EEN_SORENSSON
		" algorithm\n");
	StreamMgr_print_error(streams,
			      "  -a alg\tUses the specified algorithm. \n");
	StreamMgr_print_error(
		streams,
		"\t\tValid values are: " BMC_INVAR_ALG_CLASSIC
		", " BMC_INVAR_ALG_EEN_SORENSSON
...
int Bmc_CommandCheckInvarBmcInc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	opt_handling_res = Bmc_cmd_options_handling(
		env, argc, argv, Prop_Invar, &invarprop, &max_k, NULL,
		&algorithm_name, &closure_strategy_name, NULL, NULL, &step_k);
	...
	if (algorithm_name == (char *)NULL) {
		algorithm_name =
			util_strsav((char *)get_bmc_inc_invar_alg(opts));
	}

	if ((strcasecmp(algorithm_name, BMC_INC_INVAR_ALG_DUAL) != 0) &&
	    (strcasecmp(algorithm_name, BMC_INC_INVAR_ALG_FALSIFICATION) !=
	     0) &&
	...
	if ((char *)NULL != closure_strategy_name) {
		...
		if ((strcasecmp(closure_strategy_name, BMC_INVAR_BACKWARD) !=
		     0) &&
		    (strcasecmp(closure_strategy_name, BMC_INVAR_FORWARD)) !=
			    0) {
			StreamMgr_print_error(
				streams,
	...
	if (!strcasecmp(algorithm_name, BMC_INC_INVAR_ALG_DUAL)) {
		algorithm = ALG_DUAL;
	} else if (!strcasecmp(algorithm_name,
		...
		ErrorMgr_internal_error(
			errmgr, "%s:%d:%s unexpected algorithm specified (%s)",
	...
	if ((char *)NULL != closure_strategy_name) {
		...
		} else if (!strcasecmp(closure_strategy_name,
				       BMC_INVAR_FORWARD)) {
			closure_strategy = BMC_INVAR_FORWARD_CLOSURE;
		} else {
			ErrorMgr_internal_error(
				errmgr,
				"%s:%d:%s unexpected closure strategy specified (%s)",
	...
	switch (algorithm) {
	case ALG_DUAL:
		res = bmc_cmd_gen_solve_invar_dual_selected_or_all_props(
			env, invarprop, max_k, closure_strategy);
		break;

	case ALG_FALSIFICATION:
		res = bmc_cmd_gen_solve_invar_fals_selected_or_all_props(
			env, invarprop, max_k, step_k);
		break;

	case ALG_ZIGZAG:
		res = bmc_cmd_gen_solve_invar_zigzag_selected_or_all_props(
			env, invarprop, max_k);
		break;
...
static int UsageBmcCheckInvarInc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"  -K step_size\tOnly for falsification: increment the search of "
		"<step_size>"
	...
	StreamMgr_print_error(
		streams,
		"\n\t\tValid values are: " BMC_INC_INVAR_ALG_DUAL
		", " BMC_INC_INVAR_ALG_ZIGZAG
		", " BMC_INC_INVAR_ALG_FALSIFICATION
	...
	StreamMgr_print_error(
		...
		"\n\t\t(currently this applies only to " BMC_INC_INVAR_ALG_DUAL
		").\n");
	...
	StreamMgr_print_error(
		streams, "\t\tDefault value is '" BMC_INVAR_BACKWARD "'.\n");
...
static int bmc_CommandCheckPslSpecBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	while ((c = util_getopt(argc, argv, "h1gmo:n:p:P:k:l:")) != EOF) {
		...
		case 'n': {
			...
			prop_no = PropDb_get_prop_index_from_string(
				prop_db, util_optarg);
			if (prop_no == -1) {
				status = 1;
				goto label_clean_and_exit;
	...
	if (formula != NIL(char)) {
		...
		prop_no = PropDb_prop_parse_and_add(prop_db, st, formula,
						    Prop_Psl, Nil);
		if (prop_no == -1) {
			status = 1;
			goto label_clean_and_exit;
	...
	if (0 == status) {
		status = bmc_cmd_gen_solve_psl_selected_or_all_props(
			env, prop_no, bmc_dump, !IS_INC_SAT, single_bmc_prob, k,
			l);
	}
...
label_clean_and_exit:
	...
	if (BMC_USAGE == status)
		return UsageCheckPslSpecBmc(env);
	else
		return status;
}

/*!
  \command{check_pslspec_bmc_inc} Performs fair PSL model checking using
  incremental BMC.
					 ...
					 char **argv)
{
	...
	if (0 == status) {
		status = bmc_cmd_gen_solve_psl_selected_or_all_props(
			env, prop_no, !IS_BMC_DUMP, IS_INC_SAT, single_bmc_prob,
			k, l);
	}
...
label_clean_and_exit:
	...
	if (BMC_USAGE == status)
		return UsageCheckPslSpecBmcInc(env);
	else
		return status;
}
...
int Bmc_CommandProfile(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	/* makes sure bmc has been set up */
	if (Bmc_check_if_model_was_built(
		    env, StreamMgr_get_error_stream(streams), false)) {
	...
	if (enable) {
		if (BMC_PROFILER_IS_ENABLED(env)) {
			StreamMgr_print_error(
				streams,
				"The BMC profiler is already enabled.\n");
			FREE(out_file_name);
			return 1;
		} else {
		...
		if (!BMC_PROFILER_IS_ENABLED(env)) {
			StreamMgr_print_error(
				streams,
				"The BMC profiler is already disabled.\n");
			return 1;
		} else {
	...
	if (!BMC_PROFILER_IS_ENABLED(env)) {
		StreamMgr_print_output(streams,
				       "The BMC profiler is disabled.\n");
	} else {
		nusmv_assert((char *)NULL != BMC_PROFILER_GET_OUT_FILE(env));
		StreamMgr_print_output(streams,
				       "The BMC profiler is enabled.\n"
...
CommandProfileUsage:
	StreamMgr_print_error(streams,
			      "usage: bmc_profile [-h][-e <file> | -d]\n");
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "  -e <filename>\tEnable the profiling of BMC "
			      "calls in <file>.\n");
	StreamMgr_print_error(streams,
			      "  -d \t\tDisable the profiling of BMC calls.\n");
	return 1;
}
#endif
...
int Bmc_CommandWatchdog(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (enable) {
		if (!watchdog_opt_set) {
			StreamMgr_print_error(
				streams, "Error: the type of the watchdog was "
					 "not set!\n");
			goto CommandWatchdogUsage;
		}
	...
	/* print the watchdog status */
	{
		...
		if (period < 0) {
			StreamMgr_print_output(streams,
					       "The watchdog is not set\n");
		} else {
			StreamMgr_print_output(
				streams,
				"The watchdog is %s and is set to "
				"%llu.\n",
				status ? "enabled" : "disabled", period);
		}
	}
...
CommandWatchdogUsage:
	...
	StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
	StreamMgr_print_error(streams,
			      "  -e <timeout>\tEnable the watchdog on the "
...
int Bmc_TestTableau(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (Bmc_Utils_IsAllLoopbacks(l)) {
		...
		StreamMgr_print_error(
			streams,
			"Error: the case 'all loops' is not implemented yet.\nPlease "
			"set the variable 'bmc_loopback' to another value.\n\n");
		return 1;
	}
	...
	if (cmp_struct_get_bmc_setup(cmps) == 0) {
		StreamMgr_print_error(
			streams,
			"Please call bmc_setup before use this command.\n");
		return 1;
	}
	...
	/* process command options */
	{
		...
		int prop_no;
		Prop_ptr ltlprop =
			PROP(NULL); /* The property being processed */
		node_ptr ltlspec;
		...
		while ((c = util_getopt(argc, argv, "hn:d:c:po:x")) != EOF) {
			...
			case 'n': {
				...
				if (prop_no >= PropDb_get_size(PROP_DB(
						       NuSMVEnv_get_value(
							       env,
							       ENV_PROP_DB))) ||
				    prop_no < 0) {
					StreamMgr_print_output(
						streams,
						"Error: \"%s\" is not a valid value, must be in the range "
						"[0,%d].\n",
						strNumber,
				...
				ltlprop = PropDb_get_prop_at_index(
					PROP_DB(NuSMVEnv_get_value(
						env, ENV_PROP_DB)),
					prop_no);
				...
				ltlspec = Compile_FlattenSexpExpandDefine(
					symb_table, ltlspec, Nil);
				wff = Wff2Nnf(
					env,
					Compile_detexpr2bexpr(
						BDD_ENC(NuSMVEnv_get_value(
							env, ENV_BDD_ENCODER)),
						ltlspec));
				break;
			...
			case 'o': /* operator specification */
				...
				if (strcmp(szOperator, "G") == 0)
					wff_operator = GWO_Globally;
				else if (strcmp(szOperator, "F") == 0)
					wff_operator = GWO_Future;
				else if (strcmp(szOperator, "U") == 0)
					wff_operator = GWO_Until;
				else if (strcmp(szOperator, "R") == 0)
					wff_operator = GWO_Releases;
				else if (strcmp(szOperator, "H") == 0)
					wff_operator = GWO_Historically;
				else if (strcmp(szOperator, "O") == 0)
					wff_operator = GWO_Once;
				else if (strcmp(szOperator, "S") == 0)
					wff_operator = GWO_Since;
				else if (strcmp(szOperator, "T") == 0)
				...
				if (!usePastOperators &&
					...
					StreamMgr_print_output(
						streams,
						"Error: operator \"%s\" is not allowed, "
						"unless you turn on the \"p\" option.\n",
						szOperator);
					return 1;
				}
	...
	res = Bmc_Test_test_tableau(env, wff, wff_operator, max_depth,
...
static int UsageBmcTestTableau(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(streams,
			      "  -n <prop_idx> \tTest tableau of property "
			      "represented by the given index\n");
	StreamMgr_print_error(
		streams, "  -d <max_depth>\tGenerates a random wff with the "
			 "given max depth (default value is -1)\n");
	StreamMgr_print_error(
		streams,
		"  -c <max_conns>\tGenerates a random wff with the given max "
		"number of connectives\n\t\t\t(default value is 1).\n");
	StreamMgr_print_error(
		streams,
		"  -p \t\t\tGenerate future and past operators\n\t\t\t(only "
		"future operators are generated by default).\n");
	StreamMgr_print_error(
		streams,
		"  -o <operator> \tGenerates a random wff based on the "
		"specified operator, which will \n\t\t\tbe placed at top level. "
		"Valid values are G | F | U | R | H | O | S | T\n");
	StreamMgr_print_error(
		streams,
		"  -x \t\t\tGenerate two smv files with the same set "
		"of random formulas (not tautologies)\n\t\t\t with and "
		"without loop condition, respectively.\n");
	return 1;
}
...
Outcome Bmc_cmd_options_handling(NuSMVEnv_ptr env, int argc, char **argv,
				 Prop_Type prop_type,
				 /* output parameters: */
				 Prop_ptr *res_prop, int *res_k, int *res_l,
				 char **res_a, char **res_s, char **res_o,
				 boolean *res_e, int *res_step_k)
{
	const StreamMgr_ptr streams =
		STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
	FILE *errstream = StreamMgr_get_error_stream(streams);
	...
	while ((c = util_getopt((int)argc, (char **)argv, opt_string)) != EOF) {
		...
		case 'n': {
			...
			if ((*res_prop != PROP(NULL)) ||
			    (str_formula != (char *)NULL) ||
			    (formula_name != (char *)NULL)) {
				ErrorMgr_error_property_already_specified(
					errmgr);
				return OUTCOME_GENERIC_ERROR;
			}
			...
			*res_prop = PropDb_get_prop_at_index(prop_db, prop_idx);
		...
		case 'P': {
			...
			prop_idx =
				PropDb_prop_parse_name(prop_db, formula_name);
		...
		case 'a':
			...
			if (*res_a != (char *)NULL) {
				StreamMgr_print_error(
					streams,
					"Algorithm can be specified only once.\n\n");
				return OUTCOME_GENERIC_ERROR;
			}
	...
	if (str_loop != (char *)NULL) {
		...
		if (Bmc_Utils_Check_k_l(*res_k, Bmc_Utils_RelLoop2AbsLoop(
							rel_loop, *res_k)) !=
		    OUTCOME_SUCCESS) {
			ErrorMgr_error_bmc_invalid_k_l(errmgr, *res_k,
						       rel_loop);
			return OUTCOME_GENERIC_ERROR;
		}
	...
	if (str_formula != (char *)NULL) {
		...
		idx = PropDb_prop_parse_and_add(prop_db, symb_table,
	...
	if (ltlprop == PROP(NULL)) {
		...
		lsForEachItem(props, iterator, prop)
		{
			if (Bmc_GenSolveLtl(env, prop, k, relative_loop,
					    has_to_iter_over_k, has_to_solve,
					    dump_type, fname) != 0) {
				return 1;
			}
		}
		...
		if (Bmc_GenSolveLtl(env, ltlprop, k, relative_loop,
				    has_to_iter_over_k, has_to_solve, dump_type,
				    fname) != 0) {
			return 1;
		}
	}
	...
	const int relative_loop, const boolean must_inc_length)
{
	if (ltlprop == PROP(NULL)) {
		...
		lsForEachItem(props, iterator, prop)
		{
			if (Bmc_GenSolveLtlInc(env, prop, k, relative_loop,
					       must_inc_length) != 0) {
				return 1;
			}
		}
		...
		if (Bmc_GenSolveLtlInc(env, ltlprop, k, relative_loop,
				       must_inc_length) != 0) {
			return 1;
		}
	}
	...
	const boolean has_to_solve)
{
	...
	if (invarprop == PROP(NULL)) {
		...
		lsForEachItem(props, iterator, prop)
		{
			if (Bmc_GenSolveInvar(env, prop, has_to_solve,
					      dump_type, fname) != 0) {
				return 1;
			}
		}
		...
		if (Bmc_GenSolveInvar(env, invarprop, has_to_solve, dump_type,
				      fname) != 0) {
			return 1;
		}
	}
	...
	const char *fname, const boolean use_extra_step)
{
	...
	if (invarprop == PROP(NULL)) {
		...
		lsForEachItem(props, iterator, prop)
		{
			res = Bmc_GenSolveInvar_EenSorensson(env, prop, max_k,
							     dump_type, fname,
							     use_extra_step);
	...
	bmc_invar_closure_strategy closure_strategy)
{
	...
	if (invarprop == PROP(NULL)) {
		...
		lsForEachItem(props, iterator, prop)
		{
			res = Bmc_GenSolveInvarDual(env, prop, max_k,
						    closure_strategy);
		...
		res = Bmc_GenSolveInvarDual(env, invarprop, max_k,
					    closure_strategy);
	}
						   ...
						   const int max_k, int step_k)
{
	...
	if (invarprop == PROP(NULL)) {
		...
		lsForEachItem(props, iterator, prop)
		{
			res = Bmc_GenSolveInvarFalsification(env, prop, max_k,
							     step_k);
		...
		res = Bmc_GenSolveInvarFalsification(env, invarprop, max_k,
						     step_k);
	}
	...
	const int l)
{
	...
	if (prop_no != -1) {
		if (Prop_check_type(PropDb_get_prop_at_index(prop_db, prop_no),
			...
			status = Bmc_Gen_check_psl_property(
				env, PropDb_get_prop_at_index(prop_db, prop_no),
				bmc_dump, inc_sat, single_bmc_prob, k, l);
		}
	} else {
		lsList props;
		lsGen iterator;
...
static int UsageCheckPslSpecBmc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "usage: check_pslspec_bmc [-h] [-m | -o file] [-n "
	...
	StreamMgr_print_error(
		streams,
		"   -p \"psl-expr\"\tChecks only the given PSL formula.\n");
	StreamMgr_print_error(
		streams,
		"   -P \"name\"\t\tChecks only the PSLSPEC with the given name.\n");
	StreamMgr_print_error(
		streams,
		"   -g \t\t\tDumps generated problems in DIMACS format.\n");
	StreamMgr_print_error(
		streams, "   -1 \t\t\tGenerates and solves single problems.\n");
	StreamMgr_print_error(
		streams,
		"   -k bmc_length\tChecks the property using <bmc_length> value "
...
static int UsageCheckPslSpecBmcInc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "usage: check_pslspec_bmc_inc [-h] [-m | -o file] [-n "
			 "number | -p \"psl-expr\" | -P \"name\"]\n");
