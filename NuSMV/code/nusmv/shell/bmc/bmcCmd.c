...
void Bmc_AddCmd(NuSMVEnv_ptr env) {
  ...
  Cmd_CommandAdd(env, "gen_ltlspec_bmc", Bmc_CommandGenLtlSpecBmc, 0, true);
  Cmd_CommandAdd(env, "gen_ltlspec_bmc_onepb", Bmc_CommandGenLtlSpecBmcOnePb, 0,
                 true);
  ...
  Cmd_CommandAdd(env, "check_pslspec_bmc", bmc_CommandCheckPslSpecBmc, 0, true);

#if NUSMV_HAVE_INCREMENTAL_SAT
  Cmd_CommandAdd(env, "check_ltlspec_bmc_inc", Bmc_CommandCheckLtlSpecBmcInc, 0,
                 true);
  Cmd_CommandAdd(env, "check_invar_bmc_inc", Bmc_CommandCheckInvarBmcInc, 0,
                 true);
...
int Bmc_CommandBmcSimulate(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (bmc_simulate_get_curr_sim_trace(env) == TRACE(NULL)) {
    StreamMgr_print_error(
        streams, "No current state set. Use the \"bmc_pick_state\" command.\n");
    return 1;
  }
  ...
  (void)Bmc_Simulate(env, BE_FSM(NuSMVEnv_get_value(env, ENV_BE_FSM)), bdd_enc,
                     be_constraints, time_shift, steps, printrace, only_changes,
                     mode == Random);
...
static int UsageBmcSimulate(const NuSMVEnv_ptr env) {
  ...
  /*         "| [-t \"constr\"]] [-k steps]\n"); */
  StreamMgr_print_error(
      streams, "usage: bmc_simulate [-h] [-p | -v] [-r] [[-c \"constr\"] "
               "| [-t \"constr\"]] [-k steps]\n");
  StreamMgr_print_error(streams, "  -h \t\tPrints the command usage.\n");
  StreamMgr_print_error(
      streams,
...
int Bmc_CommandBmcIncSimulate(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  (void)Bmc_StepWiseSimulation(
      ...
      TRACE_MGR(NuSMVEnv_get_value(env, ENV_TRACE_MGR)), steps, be_constraints,
      time_shift, printrace, only_changes, mode, display_all);
...
int Bmc_CommandBmcPickState(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (str_label != (char *)NULL) {
    ...
    if (str_constr != (char *)NULL) {
      CATCH(errmgr) {
        be_constr = Bmc_Utils_simple_costraint_from_string(
            be_enc, bdd_enc, str_constr, (Expr_ptr *)NULL);
      }
      FAIL(errmgr) {
  ...
  retval = Bmc_Simulate_bmc_pick_state(env, label, be_constr, tr_number, mode,
                                       display_all, verbose);
...
static int UsageBmcPickState(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams, "usage: bmc_pick_state [-h] [-v] [-r | -i "
...
int Bmc_CommandBmcSimulateCheckFeasibleConstraints(NuSMVEnv_ptr env, int argc,
  ...
  retval = Bmc_Simulate_bmc_simulate_check_feasible_constraints(
      env, str_constraints, be_constraints, expr_constraints, human_readable);
...
static int UsageBmcSimulateCheckFeasibleConstraints(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(streams,
                        "usage: bmc_simulate_check_feasible_constraints [-h] "
                        "[-q] [-c \"next_expr\"] [-c ...]\n");
  StreamMgr_print_error(streams, "  -h \t\t Prints the command usage.\n");
  StreamMgr_print_error(streams,
                        "  -q \t\t Prints the output in compact form.\n");
  StreamMgr_print_error(streams, "  -c next_expr\t Specify one constraint "
...
int Bmc_CommandGenLtlSpecBmc(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  /* Options handling: */
  opt_handling_res =
      Bmc_cmd_options_handling(env, argc, argv, Prop_Ltl, &ltlprop, &k,
                               &relative_loop, NULL, NULL, &fname, NULL, NULL);
...
static int UsageBmcGenLtlSpecOnePb(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      streams, "\t\tIf no property is specified, checks all LTL properties.\n");
  StreamMgr_print_error(
      streams, "  -k length\tChecks the property using <length> value instead "
               "of using the\n\t\tvariable <bmc_length> value.\n");
  StreamMgr_print_error(
      streams, "  -l loopback\tChecks the property using <loopback> value "
               "instead of using the\n\t\tvariable <bmc_loopback> value.\n");
...
int Bmc_CommandCheckLtlSpecBmcInc(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  opt_handling_res =
      Bmc_cmd_options_handling(env, argc, argv, Prop_Ltl, &ltlprop, &k,
                               &relative_loop, NULL, NULL, NULL, NULL, NULL);
  ...
  /* ----------------------------------------------------------------------- */
  retval = bmc_cmd_gen_solve_ltl_inc_selected_or_all_props(env, ltlprop, k,
                                                           relative_loop, true);
...
static int UsageBmcCheckLtlSpecInc(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      streams, "  -P \"name\"\tChecks the LTL property specified with <name> \n"
               "        \t(using incremental algorithms).\n");
  StreamMgr_print_error(streams, "  -p \"formula\"\tChecks the specified LTL "
...
int Bmc_CommandGenInvarBmc(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  /* Options handling: */
  opt_handling_res =
      Bmc_cmd_options_handling(env, argc, argv, Prop_Invar, &invarprop, NULL,
                               NULL, &algorithm_name, NULL, &fname, NULL, NULL);
  ...
  retval = bmc_cmd_gen_solve_invar_selected_or_all_props(env, invarprop, fname,
                                                         !BMC_HAS_TO_SOLVE);
...
static int UsageBmcGenInvar(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      streams, "  -n idx\tChecks the INVAR property specified with <idx>.\n");
  StreamMgr_print_error(
      streams,
...
int Bmc_CommandCheckInvarBmc(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  opt_handling_res = Bmc_cmd_options_handling(
      env, argc, argv, Prop_Invar, &invarprop, &max_k, NULL, &algorithm_name,
      NULL, &fname, &use_extra_step, NULL);
  ...
  if (use_classic_alg && max_k != -1) {
    StreamMgr_print_error(
        streams, "Option -k can be used only when '" BMC_INVAR_ALG_EEN_SORENSSON
  ...
  if (use_classic_alg && use_extra_step) {
    StreamMgr_print_error(
        streams, "Option -e can be used only when '" BMC_INVAR_ALG_EEN_SORENSSON
                 "' algorithm is selected.\n");
    if (fname != (char *)NULL)
      FREE(fname);
    return 1;
  ...
  if (use_classic_alg) {
    res = bmc_cmd_gen_solve_invar_selected_or_all_props(env, invarprop, fname,
                                                        BMC_HAS_TO_SOLVE);
  } else {
...
int Bmc_CommandCheckInvarBmcInc(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  opt_handling_res = Bmc_cmd_options_handling(
      env, argc, argv, Prop_Invar, &invarprop, &max_k, NULL, &algorithm_name,
      &closure_strategy_name, NULL, NULL, &step_k);
  ...
  if ((char *)NULL != closure_strategy_name) {
    ...
    if (strcasecmp(algorithm_name, BMC_INC_INVAR_ALG_DUAL) != 0) {
      StreamMgr_print_error(streams, "Closure strategy can be specified for "
                                     "'" BMC_INC_INVAR_ALG_DUAL "'"
                                     " only.\n");
  ...
  if (!strcasecmp(algorithm_name, BMC_INC_INVAR_ALG_DUAL)) {
    ...
    ErrorMgr_internal_error(errmgr,
                            "%s:%d:%s unexpected algorithm specified (%s)",
  ...
  if ((char *)NULL != closure_strategy_name) {
    if (!strcasecmp(closure_strategy_name, BMC_INVAR_BACKWARD)) {
      ...
      ErrorMgr_internal_error(
          errmgr, "%s:%d:%s unexpected closure strategy specified (%s)",
  ...
  case ALG_FALSIFICATION:
    res = bmc_cmd_gen_solve_invar_fals_selected_or_all_props(env, invarprop,
                                                             max_k, step_k);
    break;

  case ALG_ZIGZAG:
    res = bmc_cmd_gen_solve_invar_zigzag_selected_or_all_props(env, invarprop,
                                                               max_k);
    break;
...
static int UsageBmcCheckInvarInc(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(
      ...
      "\n\t\t(using incremental algorithms).\n");
  StreamMgr_print_error(
      streams, "\t\tIf no property is specified, checks all INVAR properties"
               "\n\t\t(using incremental algorithms).\n");
  ...
  StreamMgr_print_error(
      ...
      ", " BMC_INC_INVAR_ALG_ZIGZAG ", " BMC_INC_INVAR_ALG_FALSIFICATION
...
int Bmc_CommandProfile(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  /* makes sure bmc has been set up */
  if (Bmc_check_if_model_was_built(env, StreamMgr_get_error_stream(streams),
                                   false)) {
...
int Bmc_CommandWatchdog(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  while ((c = util_getopt((int)argc, (char **)argv, "he:t:")) != EOF) {
    ...
    case 'e': {
      ...
      if (timeout < 0) {
        StreamMgr_print_error(
            streams, "Error: \"%d\" should be non-negative!\n", timeout);
        goto CommandWatchdogUsage;
      }
      enable = true;
    } break;
...
int Bmc_TestTableau(NuSMVEnv_ptr env, int argc, char **argv) {
  ...
  if (Bmc_Utils_IsAllLoopbacks(l)) {
    /* not implemented yet */
    StreamMgr_print_error(
        streams, "Error: the case 'all loops' is not implemented yet.\nPlease "
  ...
  /* process command options */
  {
    ...
    while ((c = util_getopt(argc, argv, "hn:d:c:po:x")) != EOF) {
      ...
      case 'n': {
        ...
        if (prop_no >= PropDb_get_size(
          ...
          StreamMgr_print_output(
              ...
              "[0,%d].\n",
              strNumber,
              PropDb_get_size(PROP_DB(NuSMVEnv_get_value(env, ENV_PROP_DB))) -
                  1);
          return 1;
        }

        ltlprop = PropDb_get_prop_at_index(
            PROP_DB(NuSMVEnv_get_value(env, ENV_PROP_DB)), prop_no);

        if (Prop_get_type(ltlprop) != Prop_Ltl) {
          StreamMgr_print_error(
              streams, "Error: property %d is not of type LTL\n", prop_no);
          return 1;
        }
        ...
        wff =
            Wff2Nnf(env, Compile_detexpr2bexpr(
      ...
      case 'o': /* operator specification */
        ...
        if (!usePastOperators &&
            (wff_operator == GWO_Historically || wff_operator == GWO_Once ||
             wff_operator == GWO_Since || wff_operator == GWO_Triggered)) {
          StreamMgr_print_output(streams,
...
static int UsageBmcTestTableau(const NuSMVEnv_ptr env) {
  StreamMgr_ptr streams =
      STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  StreamMgr_print_error(
      streams, "usage: _bmc_test_tableau [-h] | [-n <property_index>] | \n \
                         [[ -d <max_depth>] [-c <max_conns>] [-o <operator>] [-p]]\n");
  StreamMgr_print_error(streams, "  -h \t\t\tPrints the command usage.\n");
  StreamMgr_print_error(streams, "  -n <prop_idx> \tTest tableau of property "
                                 "represented by the given index\n");
  StreamMgr_print_error(streams,
                        "  -d <max_depth>\tGenerates a random wff with the "
                        "given max depth (default value is -1)\n");
  StreamMgr_print_error(
      streams, "  -c <max_conns>\tGenerates a random wff with the given max "
               "number of connectives\n\t\t\t(default value is 1).\n");
  StreamMgr_print_error(
      streams, "  -p \t\t\tGenerate future and past operators\n\t\t\t(only "
               "future operators are generated by default).\n");
  StreamMgr_print_error(
      streams, "  -o <operator> \tGenerates a random wff based on the "
...
Outcome Bmc_cmd_options_handling(NuSMVEnv_ptr env, int argc, char **argv,
  ...
  while ((c = util_getopt((int)argc, (char **)argv, opt_string)) != EOF) {
    ...
    case 'e':
      ...
      if (e_specified) {
        StreamMgr_print_error(
            streams, "Option -e cannot be specified more than once.\n");
        return OUTCOME_GENERIC_ERROR;
      }
      *res_e = true;
    ...
    case 'n': {
      ...
      prop_idx = PropDb_get_prop_index_from_string(prop_db, str_prop_idx);
      FREE(str_prop_idx);
    ...
    case 'P': {
      ...
      prop_idx = PropDb_prop_parse_name(prop_db, formula_name);
    ...
    case 's':
      ...
      if (*res_s != (char *)NULL) {
        StreamMgr_print_error(
            streams, "Closure strategy can be specified only once.\n\n");
        return OUTCOME_GENERIC_ERROR;
      }
  ...
  if (str_formula != (char *)NULL) {
    ...
    idx = PropDb_prop_parse_and_add(prop_db, symb_table, str_formula, prop_type,
                                    Nil);
    if (idx == -1) {
      FREE(str_formula);
      return OUTCOME_GENERIC_ERROR;
...
static inline int bmc_cmd_gen_solve_ltl_selected_or_all_props(
  ...
  if (ltlprop == PROP(NULL)) {
    ...
    lsForEachItem(props, iterator, prop) {
      if (Bmc_GenSolveLtl(env, prop, k, relative_loop, has_to_iter_over_k,
    ...
    if (Bmc_GenSolveLtl(env, ltlprop, k, relative_loop, has_to_iter_over_k,
                        has_to_solve, dump_type, fname) != 0) {
      return 1;
    }
  }
...
static inline int bmc_cmd_gen_solve_ltl_inc_selected_or_all_props(
  ...
  if (ltlprop == PROP(NULL)) {
    ...
    lsForEachItem(props, iterator, prop) {
      if (Bmc_GenSolveLtlInc(env, prop, k, relative_loop, must_inc_length) !=
          0) {
        return 1;
      }
    }
    ...
    if (Bmc_GenSolveLtlInc(env, ltlprop, k, relative_loop, must_inc_length) !=
        0) {
      return 1;
    }
  }
...
static inline int bmc_cmd_gen_solve_invar_selected_or_all_props(
  ...
  if (invarprop == PROP(NULL)) {
    ...
    lsForEachItem(props, iterator, prop) {
      if (Bmc_GenSolveInvar(env, prop, has_to_solve, dump_type, fname) != 0) {
    ...
    if (Bmc_GenSolveInvar(env, invarprop, has_to_solve, dump_type, fname) !=
        0) {
...
static inline int bmc_cmd_gen_solve_invar_een_selected_or_all_props(
  ...
  if (invarprop == PROP(NULL)) {
    ...
    lsForEachItem(props, iterator, prop) {
      res = Bmc_GenSolveInvar_EenSorensson(env, prop, max_k, dump_type, fname,
                                           use_extra_step);
    ...
    res = Bmc_GenSolveInvar_EenSorensson(env, invarprop, max_k, dump_type,
...
/*!
  ...
  property or over all the properties
*/
static inline int bmc_cmd_gen_solve_invar_fals_selected_or_all_props(
    NuSMVEnv_ptr const env, Prop_ptr const invarprop, const int max_k,
    int step_k) {
  int res = 0;
  ...
  if (prop_no != -1) {
    ...
    lsForEachItem(props, iterator, prop) {
      if (Prop_is_psl_ltl(prop)) {
        status = Bmc_Gen_check_psl_property(env, prop, bmc_dump, inc_sat,
                                            single_bmc_prob, k, l);
      }
...
static int UsageCheckPslSpecBmc(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(streams,
                        "usage: check_pslspec_bmc [-h] [-m | -o file] [-n "
...
static int UsageCheckPslSpecBmcInc(const NuSMVEnv_ptr env) {
  ...
  StreamMgr_print_error(streams,
                        "usage: check_pslspec_bmc_inc [-h] [-m | -o file] [-n "
                        "number | -p \"psl-expr\" | -P \"name\"]\n");
