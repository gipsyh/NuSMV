...
static inline int sbmc_cmd_gen_solve_ltl_selected_or_all_props(
	...
	const boolean is_single_prob, const boolean has_to_solve);
...
static inline int sbmc_cmd_gen_solve_psl_selected_or_all_props(
	...
	const int l, const boolean check_compl,
...
void SBmc_AddCmd(NuSMVEnv_ptr env)
{
	...
	Cmd_CommandAdd(env, "check_ltlspec_sbmc", Sbmc_CommandCheckLtlSpecSBmc,
		       0, true);
	Cmd_CommandAdd(env, "gen_ltlspec_sbmc", Sbmc_CommandGenLtlSpecSBmc, 0,
		       true);
	Cmd_CommandAdd(env, "check_pslspec_sbmc", sbmc_CommandCheckPslSpecSbmc,
		       0, true);

#if NUSMV_HAVE_INCREMENTAL_SAT
	Cmd_CommandAdd(env, "check_ltlspec_sbmc_inc",
		       Sbmc_CommandLTLCheckZigzagInc, 0, true);
	Cmd_CommandAdd(env, "check_pslspec_sbmc_inc",
		       sbmc_CommandCheckPslSpecSbmcInc, 0, true);
#endif
}
...
int Sbmc_CommandCheckLtlSpecSBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	res = sbmc_cmd_gen_solve_ltl_selected_or_all_props(env, ltlprop, k,
							   relative_loop, fname,
							   is_single_prob,
							   BMC_HAS_TO_SOLVE);
...
static int UsageSBMCCheckLtlSpec(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "  -1 \t\tGenerates and solves a single problem.\n");
...
int Sbmc_CommandGenLtlSpecSBmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	opt_handling_res = sbmc_cmd_options_handling(
		env, argc, argv, Prop_Ltl, &ltlprop, &k, &relative_loop, &fname,
		NULL, NULL /* -N -c */, &is_single_prob);
	...
	(void)sbmc_cmd_gen_solve_ltl_selected_or_all_props(env, ltlprop, k,
							   relative_loop, fname,
							   is_single_prob,
							   !BMC_HAS_TO_SOLVE);
...
int Sbmc_CommandLTLCheckZigzagInc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	opt_handling_res = sbmc_cmd_options_handling(
		...
		&do_virtual_unrolling, &do_completeness_check,
		NULL /* single problem */
	);
...
static int sbmc_CommandCheckPslSpecSbmc(NuSMVEnv_ptr env, int argc, char **argv)
{
	...
	if (0 == status) {
		status = sbmc_cmd_gen_solve_psl_selected_or_all_props(
			...
			l, !HAS_TO_CHECK_COMPL, HAS_TO_UNROLL);
	}
					   ...
					   char **argv)
{
	...
	if (0 == status) {
		status = sbmc_cmd_gen_solve_psl_selected_or_all_props(
			...
			k, l, check_compl, does_virtual_unrol);
	}
					 ...
					 boolean *res_1)
{
	...
	if (str_formula != (char *)NULL) {
		...
		idx = PropDb_prop_parse_and_add(
			PROP_DB(NuSMVEnv_get_value(env, ENV_PROP_DB)), st,
	...
	const boolean do_virtual_unrolling, const boolean do_completeness_check)
{
	if (ltlprop == PROP(NULL)) {
		...
		Prop_ptr prop;
		lsList const props =
			PropDb_prepare_prop_list(prop_db, Prop_Ltl);
...
static int UsageCheckPslSpecSbmc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams, "usage: check_pslspec_sbmc [-h] [-m | -o file] [-n "
...
static int UsageCheckPslSpecSbmcInc(const NuSMVEnv_ptr env)
{
	...
	StreamMgr_print_error(
		streams,
		"   -c \t\t\tPerforms completeness check (implies -i)\n");
	StreamMgr_print_error(
		streams,
